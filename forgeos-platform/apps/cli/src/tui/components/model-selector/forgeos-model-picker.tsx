...
export {
	...
	FORGEOS_MODEL_PICKER_TIER_LABELS,
	type ForgeOSModelPickerBrowse,
	type ForgeOSModelPickerEntry,
	type ForgeOSModelPickerItem,
	type ForgeOSModelPickerTier,
	freeTierDescriptionFor,
} from "./forgeos-model-entries";
...
export function useForgeOSRecommendedModels() {
	const [data, setData] = useState<ForgeOSRecommendedModelsData | null>(null);
...
export function ForgeOSModelPicker(props: {
	...
	for (let i = 0; i < entries.length; i++) {
		...
		if (entry.kind === "model") {
			if (entry.tier !== lastTier) {
				lastTier = entry.tier;
				const label = FORGEOS_MODEL_PICKER_TIER_LABELS[entry.tier];
				rows.push(
					<box
						key={`tier-${entry.tier}`}
						paddingX={1}
