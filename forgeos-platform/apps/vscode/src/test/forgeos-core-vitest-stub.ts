...
import { getGeneratedModelsForProvider, MODEL_COLLECTIONS_BY_PROVIDER_ID } from "@forgeos/llms"
...
// @forgeos/llms registry exactly as in production. Tests that touch it must
...
export function readCompactionStrategyGlobally(): GlobalCompactionStrategy {
	try {
		const settings = JSON.parse(readFileSync(process.env.FORGEOS_GLOBAL_SETTINGS_PATH ?? "", "utf8"))
...
export function setCompactionStrategyGlobally(compactionStrategy: GlobalCompactionStrategy): void {
	const filePath = process.env.FORGEOS_GLOBAL_SETTINGS_PATH
	if (filePath) {
		let settings = {}
		try {
...
// Real file-read executor (dependency-light: node:fs/node:path + @forgeos/shared/storage)
...
export async function compareCheckpointToWorkspace(): Promise<CheckpointWorkspaceCompareResult> {
	throw new Error("compareCheckpointToWorkspace is not implemented in the Vitest @forgeos/core stub")
}
...
export function createForgeOSTelemetryServiceConfig(config: Record<string, unknown> = {}) {
	return {
		enabled: false,
		metadata: {
...
export function getProviderAuthStorageId(providerId: string): string | undefined {
	...
	if (normalized === "forgeos" || normalized === "forgeos-pass") {
		return "forgeos"
	}
...
function formatForgeOSApiKey(accessToken: string): string {
...
export function getProviderAuthHandler(providerId: string) {
	...
	return {
		...
		getApiKey(settings: Record<string, unknown> | undefined): string | undefined {
			...
			if (accessToken) {
				return storageProviderId === "forgeos" ? formatForgeOSApiKey(accessToken) : accessToken
			}
...
export async function listLocalProviders(
	...
	options: { isForgeOSPassEnabled?: boolean } = {},
	...
	const providers = Object.entries(MODEL_COLLECTIONS_BY_PROVIDER_ID)
		...
		.filter((provider) => options.isForgeOSPassEnabled === true || provider.id !== "forgeos-pass")
...
export interface ForgeOSRecommendedModel {
	id: string
	name: string
	description: string
	tags: string[]
...
export const FALLBACK_FORGEOS_RECOMMENDED_MODELS: ForgeOSRecommendedModelsData = {
	recommended: [
		{
...
export async function fetchForgeOSRecommendedModels(_options?: {
	baseUrl?: string
	fetchImpl?: typeof fetch
}): Promise<ForgeOSRecommendedModelsData> {
...
export async function getValidForgeOSCredentials(): Promise<OAuthCredentials | undefined> {
	return undefined
}

export async function loginForgeOSOAuth(): Promise<OAuthCredentials> {
	return {}
}
