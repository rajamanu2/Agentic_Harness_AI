...
describe("resolveModelDisplayName", () => {
	it("uses the friendly model name with a ForgeOSPass prefix", () => {
		expect(
			resolveModelDisplayName({
				providerId: "forgeos-pass",
				modelId: "zai/glm-5.2",
		...
		).toBe("ForgeOSPass: GLM 5.2");
	});

	it("falls back to the bare model id with a ForgeOSPass prefix when unknown", () => {
		expect(
			resolveModelDisplayName({
				providerId: "forgeos-pass",
				modelId: "zai/glm-5.2",
	...
	it("keeps the reasoning effort next to the model name", () => {
		...
		).toBe("ForgeOSPass: GLM 5.2 (high)");
	});

	it("uses the friendly model name for non-ForgeOSPass providers", () => {
		expect(
			resolveModelDisplayName({
				providerId: "forgeos",
				modelId: "zai/glm-5.2",
