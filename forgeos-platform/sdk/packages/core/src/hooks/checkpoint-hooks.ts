...
import type { AgentHooks, BasicLogger } from "@forgeos/shared";
import { countUserRunMessages } from "../session/user-run-messages";

const execFile = promisify(execFileCallback);
const CHECKPOINT_STASH_MESSAGE_PREFIX = "forgeos checkpoint session=";
...
async function createUntrackedParentCommit(
	...
	const tempDir = await mkdtemp(join(tmpdir(), "forgeos-checkpoint-"));
	...
	try {
		...
		const commit = await runGitWithIndex(cwd, indexFile, [
			...
			tree,
			"-m",
			"untracked files on forgeos checkpoint",
		]);
...
/**
 * Deletes all private git refs under refs/forgeos/checkpoints/{sessionId}/ that
...
export async function deleteCheckpointRefs(
	...
	if (!cwd) return;
	const prefix = `refs/forgeos/checkpoints/${sessionId}/`;
	try {
...
export async function retainCheckpointRefs(
	...
	await Promise.allSettled(
		checkpoints.map((entry) =>
			runGit(cwd, [
				"update-ref",
				`refs/forgeos/checkpoints/${sessionId}/${entry.runCount}`,
				entry.ref,
			]),
		),
	);
...
export function createCheckpointHooks(
	...
	const createCheckpoint = async (
		...
		const privateRef = `refs/forgeos/checkpoints/${options.sessionId}/${runCount}`;
		try {
