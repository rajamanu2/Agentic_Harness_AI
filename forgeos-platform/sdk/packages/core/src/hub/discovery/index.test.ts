...
type EnvSnapshot = {
	...
	FORGEOS_HUB_BUILD_ID: string | undefined;
	FORGEOS_HUB_BUILD_EPOCH_MS: string | undefined;
	FORGEOS_HUB_DISCOVERY_PATH: string | undefined;
};

function captureEnv(): EnvSnapshot {
	return {
		FORGEOS_DATA_DIR: process.env.FORGEOS_DATA_DIR,
		FORGEOS_HUB_BUILD_ID: process.env.FORGEOS_HUB_BUILD_ID,
		FORGEOS_HUB_BUILD_EPOCH_MS: process.env.FORGEOS_HUB_BUILD_EPOCH_MS,
...
function restoreEnv(snapshot: EnvSnapshot): void {
	...
	process.env.FORGEOS_HUB_BUILD_ID = snapshot.FORGEOS_HUB_BUILD_ID;
	process.env.FORGEOS_HUB_BUILD_EPOCH_MS = snapshot.FORGEOS_HUB_BUILD_EPOCH_MS;
	process.env.FORGEOS_HUB_DISCOVERY_PATH = snapshot.FORGEOS_HUB_DISCOVERY_PATH;
}
...
describe("hub discovery", () => {
	...
	it("honors an explicit hub discovery path override", () => {
		snapshot = captureEnv();
		process.env.FORGEOS_HUB_DISCOVERY_PATH = "/tmp/custom-hub-discovery.json";
	...
	it("allows tests to override the unbundled source build identity", () => {
		snapshot = captureEnv();
		delete process.env.FORGEOS_HUB_BUILD_ID;
		expect(resolveHubBuildId()).toMatch(/^source-/);

		process.env.FORGEOS_HUB_BUILD_ID = "e2e-build";
	...
	it("allows tests to override the build epoch and treats sources as unordered", () => {
		snapshot = captureEnv();
		delete process.env.FORGEOS_HUB_BUILD_EPOCH_MS;
		expect(resolveHubBuildEpochMs()).toBeUndefined();

		process.env.FORGEOS_HUB_BUILD_EPOCH_MS = "12345";
