...
/**
 * ForgeOS's GitHub avatar URL
 */
const FORGEOS_AVATAR_URL = "https://avatars.githubusercontent.com/u/184127137"
...
export class VscodeCommentReviewController extends CommentReviewController implements vscode.Disposable {
	...
	constructor() {
		...
		this.commentController = vscode.comments.createCommentController("forgeos-ai-review", "ForgeOS AI Review")
	}
	...
	addReviewComment(comment: ReviewComment): void {
		...
		const commentObj: vscode.Comment = {
			...
			author: {
				name: "ForgeOS",
				iconPath: vscode.Uri.parse(FORGEOS_AVATAR_URL),
			},
		}
	...
	/**
	 * Close all tabs that use the forgeos-diff URI scheme (both diff views and regular text documents)
	 */
	async closeDiffViews(): Promise<void> {
		const tabs = vscode.window.tabGroups.all
			...
			.filter((tab) => {
				...
				// Check for regular text document tabs with forgeos-diff scheme (opened during comment reveal)
