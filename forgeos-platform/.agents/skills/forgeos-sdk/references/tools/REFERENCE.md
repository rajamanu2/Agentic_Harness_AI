...
Tools are how agents interact with the world. The ForgeOS SDK supports both built-in tools (via ForgeOSCore) and custom tools you define yourself.

## Creating Custom Tools

Use `createTool()` from `@forgeos/sdk` (or `@forgeos/shared`):
...
## Built-in Tools (ForgeOSCore Only)

When using `ForgeOSCore` with `enableTools: true`, these tools are available automatically:
...
ForgeOSCore can connect to MCP (Model Context Protocol) servers for additional tools. Configure in `.forgeos/mcp-servers.json`:
...
- `../forgeoscore/REFERENCE.md` - Using tools with ForgeOSCore
