 ...
 *   4. FORGEOS_BUNDLE_OVERRIDE / the forgeos.rollout.bundleOverride setting
 ...
 *      once per window
 *   7. the nightly identity (manifest name forgeos-nightly) switches the
...
function makeSandbox({
	...
	const sandbox = mkdtempSync(path.join(tmpdir(), "forgeos-ab-smoke-"));
	cpSync(
	...
	writeFileSync(
		...
		JSON.stringify({ "forgeos.generatedMachineId": "smoke-machine" }),
	);
	return sandbox;
}
...
await runScenario(
	...
	{ seed: { "forgeos.rollout.bundle": "next" } },
	async ({ api, sandbox }) => {
...
await runScenario(
	...
	async ({ context, api, fetchCalls }) => {
		...
		assert.deepEqual(api, { bundle: "legacy" });
		assert.equal(context.globalState._dump()["forgeos.rollout.bundle"], "next");
...
await runScenario(
	...
	async ({ context, api, fetchCalls }) => {
		...
		assert.deepEqual(api, { bundle: "next" });
		assert.equal(context.globalState._dump()["forgeos.rollout.bundle"], "legacy");
		const [featureFlagCalled] = featureFlagCalledCaptures(fetchCalls);
		assert.ok(featureFlagCalled, "rollout refresh must emit the PostHog feature-flag exposure event");
...
await runScenario(
	...
	{ env: { FORGEOS_BUNDLE_OVERRIDE: "next" } },
	async ({ api }) => {
		assert.deepEqual(api, { bundle: "next" });
	},
);
...
await runScenario(
	...
	{
		seed: { "forgeos.rollout.bundle": "next" },
		settings: { "forgeos.rollout.bundleOverride": "legacy" },
	},
	async ({ api }) => {
		assert.deepEqual(api, { bundle: "legacy" });
	},
...
await runScenario(
	...
	{
		seed: { "forgeos.rollout.bundle": "legacy" },
		settings: { "forgeos.rollout.bundleOverride": "next" },
	},
	async ({ api }) => {
		assert.deepEqual(api, { bundle: "next" });
	},
...
await runScenario(
	...
	async ({ context, api, fetchCalls }) => {
		...
		assert.equal(state["forgeos.rollout.bundle"], "legacy");
		assert.equal(
			state["forgeos.rollout.nextActivationFailedVersion"],
			"4.1.0-smoke",
		);
		assert.equal(
...
await runScenario(
	...
	{
		contextPackageJSON: { name: "forgeos-nightly" },
		settings: { "forgeos-nightly.rollout.bundleOverride": "next" },
	},
	async ({ api, context }) => {
		assert.deepEqual(api, { bundle: "next" });
		assert.deepEqual(executedCommands[0], [
		...
		assert.equal(item.text, "ForgeOS: Next");
