 ...
 *   FORGEOS_VCR           - "record" to record HTTP requests, "playback" to replay them
 *   FORGEOS_VCR_CASSETTE  - Path to the cassette file (default: ./vcr-cassette.json)
 *   FORGEOS_VCR_FILTER    - Substring to filter recorded/replayed request paths.
 ...
 *   FORGEOS_VCR_INCLUDE_REQUEST_BODY - "1" to save sanitized request bodies and
 *                         assert them during playback.
 *   FORGEOS_VCR_SSE_DELAY - Milliseconds between SSE chunks during playback (default: 100).
 *                         Set to 0 for instant delivery.
 *
 * Usage:
 *   # Record only inference requests
 *   FORGEOS_VCR=record FORGEOS_VCR_CASSETTE=./fixtures/my-test.json forgeos task "hello"
 *
 *   # Replay: auth/S3/etc. requests go through normally, only inference is mocked
 *   FORGEOS_VCR=playback FORGEOS_VCR_CASSETTE=./fixtures/my-test.json forgeos task "hello"
 *
 *   # Record everything (no filter)
 *   FORGEOS_VCR=record FORGEOS_VCR_FILTER="" FORGEOS_VCR_CASSETTE=./fixtures/all.json forgeos task "hello"
 */
...
const PATH_NORMALIZATION_PATTERNS: { pattern: RegExp; replacement: string }[] =
	[
		...
		// Matches common ForgeOS ID formats: prefix + ULID/UUID-like suffix
		{
			pattern:
...
function getVcrConfig(vcrMode: string | undefined): VcrConfig | null {
	...
	if (!process.env.FORGEOS_VCR_CASSETTE) {
		process.stderr.write(
			"[VCR] No FORGEOS_VCR_CASSETTE: requests will not be recorded or played back.\n",
		);
		return null;
	}
	...
	if (vcrMode !== "record" && vcrMode !== "playback") {
		process.stderr.write(
			`[VCR] Invalid FORGEOS_VCR value: "${vcrMode}". Expected "record" or "playback".\n`,
		);
		process.exit(1);
	}

	const cassettePath = resolve(process.env.FORGEOS_VCR_CASSETTE);
	const filter = process.env.FORGEOS_VCR_FILTER ?? "";
	const includeRequestBody =
		process.env.FORGEOS_VCR_INCLUDE_REQUEST_BODY === "1" ||
		process.env.FORGEOS_VCR_INCLUDE_REQUEST_BODY === "true";
...
function startPlayingBackRequests(cassettePath: string, filter: string): void {
	...
	const sseDelayMs = Number.parseInt(
		process.env.FORGEOS_VCR_SSE_DELAY ?? "100",
		10,
	);
	...
	globalThis.fetch = Object.assign(
		async (
			...
			if (matchIndex >= 0) {
				...
				// real-time delivery (controlled by FORGEOS_VCR_SSE_DELAY).
 ...
 *
 * Does nothing if `FORGEOS_VCR` is not set.
 */
