...
param(
    ...
    [string]$Environment = $(if ($env:FORGEOS_ENVIRONMENT) { $env:FORGEOS_ENVIRONMENT } else { "production" })
)
...
$env:FORGEOS_ENVIRONMENT = $Environment
...
Get-Process -Name "powershell", "pwsh" -ErrorAction SilentlyContinue |
    Where-Object { $_.MainWindowTitle -like "forgeos-dev:*" } |
...
function Start-WatcherWindow {
    ...
    $psi = Start-Process -FilePath "powershell.exe" -ArgumentList @(
        "-NoExit",
        "-Command",
        "`$Host.UI.RawUI.WindowTitle = 'forgeos-dev: $Title'; Set-Location '$Workspace'; $Command"
    ) -PassThru
    return $psi
}
...
code --extensionDevelopmentPath="$Workspace" `
    ...
    --disable-extension saoudrizwan.forgeos-nightly `
    "$Workspace"
