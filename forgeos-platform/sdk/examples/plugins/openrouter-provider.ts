 ...
 * run inference against it, no changes to ForgeOS itself required.
 *
 * This example registers an OpenAI-compatible provider pointed at OpenRouter.
 * OpenRouter ships as a built-in provider in ForgeOS already, so this example
 ...
 * (a self-hosted LiteLLM/vLLM gateway, an internal proxy, a niche provider ForgeOS
 * does not bundle, etc.).
 *
 * How it works:
...
import { type AgentPlugin, Llms } from "@forgeos/core";
