...
import { ForgeOSStorage } from "./ForgeOSStorage"
import type { BlobStoreSettings } from "./types"

export type { BlobStoreSettings } from "./types"

/**
 * S3/R2/Azure blob storage implementation of ForgeOSStorage.
 * Uses AWS S3, Cloudflare R2, or Azure Blob Storage as the backend storage.
 */
class ForgeOSBlobStorage extends ForgeOSStorage {
	override name = "ForgeOSBlobStorage"

	private static store: ForgeOSBlobStorage | null = null
	static get instance(): ForgeOSBlobStorage {
		if (!ForgeOSBlobStorage.store) {
			ForgeOSBlobStorage.store = new ForgeOSBlobStorage()
		}
		return ForgeOSBlobStorage.store
	}
	...
	public init(settings?: BlobStoreSettings) {
		...
		try {
			if (!ForgeOSBlobStorage.isConfigured(settings)) {
			...
			if (adapter) {
				...
				Logger.log(`[ForgeOSBlobStorage] Adapter created for ${settings.adapterType}`)
			}
		} catch (error) {
			// Log but don't throw - allow startup to continue
			Logger.error("[ForgeOSBlobStorage] initialization failed:", error)
		}
	}
 ...
 */
export const blobStorage = ForgeOSBlobStorage.instance
