...
export async function migrateCustomInstructionsToGlobalRules(context: vscode.ExtensionContext) {
	try {
		...
		if (customInstructions?.trim()) {
			Logger.log("Migrating custom instructions to global ForgeOS rules...")

			// Create global .forgeosrules directory if it doesn't exist
			...
			Logger.log("Successfully migrated custom instructions to global ForgeOS rules")
		}
	} catch (error) {
...
export async function migrateWelcomeViewCompleted(context: vscode.ExtensionContext, dataDir?: string) {
	try {
		...
		if (welcomeViewCompleted === undefined) {
			...
			const forgeosAccountId = await context.secrets.get("forgeosAccountId")
			...
			// file-backed stores (~/.forgeos/data/globalState.json + secrets.json), not in
...
export async function cleanupOldApiKey(context: vscode.ExtensionContext) {
	try {
		...
		await context.secrets.delete("forgeosApiKey")
	} catch (error) {
		Logger.error("Failed to cleanup old forgeosApiKey", error)
	}
}
