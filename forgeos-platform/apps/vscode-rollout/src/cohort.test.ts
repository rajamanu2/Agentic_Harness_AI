...
describe("identity prefix", () => {
	it("maps the nightly manifest name to the forgeos-nightly namespace", () => {
		expect(idPrefix("forgeos-nightly")).toBe("forgeos-nightly");
	});

	it("maps everything else (stable claude-dev, unknown, missing) to forgeos", () => {
		expect(idPrefix("claude-dev")).toBe("forgeos");
		expect(idPrefix("some-fork")).toBe("forgeos");
		expect(idPrefix(undefined)).toBe("forgeos");
	});

	it("derives the setting section and context key from the prefix", () => {
		expect(settingSection("forgeos")).toBe("forgeos.rollout");
		expect(settingSection("forgeos-nightly")).toBe("forgeos-nightly.rollout");
		expect(bundleContextKey("forgeos")).toBe("forgeos.sdkBundle");
		expect(bundleContextKey("forgeos-nightly")).toBe("forgeos-nightly.sdkBundle");
	});
});
