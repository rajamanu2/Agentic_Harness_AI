...
- `AgentRuntimeEvent` from `@forgeos/agents`, delivered through `agent.subscribe(listener)`.
- `AgentEvent` / `CoreSessionEvent` from `@forgeos/core`, delivered through core session adapters and `forgeos.subscribe(listener)`.
...
`AgentEvent` is the host-facing event shape used by `@forgeos/core` session orchestration. For direct `Agent` usage, prefer `agent.subscribe(...)` and `AgentRuntimeEvent`.
...
## ForgeOSCore Session Events

Use `forgeos.subscribe()` for session-level events:
...
```typescript
const unsubscribe = forgeos.subscribe(listener, { sessionId })
```
...
For a complete working example of streaming agent events to a browser via SSE, see the [multi-agent example](https://github.com/forgeos/forgeos/tree/main/apps/examples/multi-agent). It spawns multiple agents in parallel and streams each agent's events to separate UI cards.
