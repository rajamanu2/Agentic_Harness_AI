...
describe("chat commands", () => {
	it("shows connector help for /help and /start", async () => {
		for (const { command, botUserName } of [
			{ command: "/help" },
			{ command: "/start" },
			{ command: "/help@forgeosbot", botUserName: "forgeosbot" },
			{ command: "/start@forgeos_bot", botUserName: "@forgeos_bot" },
		]) {
			...
			expect(reply).toHaveBeenCalledWith(
				expect.stringContaining("ForgeOS connector commands:"),
			);
			expect(reply).toHaveBeenCalledWith(
	...
	it("requires a bot suffix when requested", async () => {
		...
		expect(await maybeHandleChatCommand("/help@forgeosbot", context)).toBe(true);
		expect(reply).toHaveBeenCalledWith(
			expect.stringContaining("ForgeOS connector commands:"),
		);
	});
	...
	it("detects commands addressed to the configured bot", () => {
		expect(isCommandAddressedToBot("/new@forgeosbot", "forgeosbot")).toBe(true);
		expect(isCommandAddressedToBot("/new@forgeos_bot", "@forgeos_bot")).toBe(true);
		expect(isCommandAddressedToBot("/new@forgeos.bot", "forgeos.bot")).toBe(true);
		expect(isCommandAddressedToBot("/new@forgeos-bot", "forgeos-bot")).toBe(true);
		expect(isCommandAddressedToBot("/new", "forgeosbot")).toBe(false);
		expect(isCommandAddressedToBot("/new@otherbot", "forgeosbot")).toBe(false);
		expect(isCommandAddressedToBot("/new@forgeosbot", undefined)).toBe(false);
	});

	it("normalizes commands addressed to dotted and hyphenated bot names", () => {
		expect(normalizeCommandName("/new@forgeos.bot", "forgeos.bot")).toBe("/new");
		expect(normalizeCommandName("/new@forgeos-bot", "forgeos-bot")).toBe("/new");
	});
	...
	it("leaves bot-suffixed commands unmatched without a known bot username", async () => {
		...
		const handled = await maybeHandleChatCommand("/help@forgeosbot", {
			enabled: true,
			getState: async () => ({
				enableTools: true,
				autoApproveTools: false,
