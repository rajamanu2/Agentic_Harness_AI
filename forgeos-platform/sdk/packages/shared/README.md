...
`@forgeos/shared` owns shared cross-package primitives (session common types/utilities).

Node-only filesystem path resolvers live under the storage subpath export:

- `@forgeos/shared/storage`
- examples: `resolveForgeOSDataDir`, `resolveDbDataDir`, `resolveSessionDataDir`, `resolveTeamDataDir`
...
(`@forgeos/cli`, `@forgeos/code`) so request/response contracts are not duplicated
...
- ForgeOS account action payloads (`ForgeOSAccountActionRequest`)
