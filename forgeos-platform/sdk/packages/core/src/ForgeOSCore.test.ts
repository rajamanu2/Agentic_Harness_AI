...
import type { ForgeOSCoreStartInput } from "./forgeos-core/types";
import type {
	StartSessionInput,
	StartSessionResult,
...
import type { AgentResult } from "@forgeos/shared";
import { ForgeOSCore } from "./ForgeOSCore";
import { NoOpFeatureFlagsProvider } from "./services/feature-flags";

function createStartInput(): ForgeOSCoreStartInput {
	return {
		config: {
...
describe("ForgeOSCore", () => {
	...
	it("keeps raw reads canonical and offers an explicit display projection", async () => {
		...
		const core = await ForgeOSCore.create();
	...
	it("compares a checkpoint to the current workspace through the public SDK API", async () => {
		const dir = mkdtempSync(join(tmpdir(), "forgeos-core-compare-"));
		let core: ForgeOSCore | undefined;
		try {
			git(dir, ["init", "-b", "main"]);
	...
	it("applies start-session bootstraps before delegating to the host", async () => {
		...
		const applyToStartSessionInput = vi.fn(
			async (input: ForgeOSCoreStartInput) => ({
				...input,
				config: {
					...input.config,
	...
	it("prefers the per-session telemetry service over the ForgeOSCore one", async () => {
		const host = {
			runtimeAddress: undefined,
	...
	it("can list sessions without hydrating message history", async () => {
		...
		// ForgeOSCore/listSessionHistory; the host list contract only receives the
		// numeric scan limit.
	...
	it("exposes event automation through ForgeOSCore instead of CronService", async () => {
		const root = mkdtempSync(join(tmpdir(), "forgeos-core-automation-"));
		const cronDir = join(root, ".forgeos", "cron");
		const reportsDir = join(cronDir, "reports");
		const dbPath = join(root, ".forgeos", "data", "db", "cron.db");
