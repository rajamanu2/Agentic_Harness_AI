...
import { formatForgeOSCredits, loadForgeOSAccountSnapshot } from "./forgeos-account";
...
export async function resolveForgeOSWelcomeLine(input: {
	config: Config;
	forgeosApiBaseUrl?: string;
	forgeosProviderSettings?: ProviderSettings;
}): Promise<string | undefined> {
	if (input.config.providerId !== "forgeos") {
		return undefined;
	}
	try {
		const snapshot = await loadForgeOSAccountSnapshot(input);
		const parts = [
			snapshot.user.email,
			`Credits: ${formatForgeOSCredits(snapshot.displayedBalance)}`,
		];
