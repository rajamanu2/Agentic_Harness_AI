...
// - Auth info conversion (SDK OAuthCredentials → ForgeOSAuthInfo)
// - Logout flow
// - Streaming subscription management
// - workos: prefix handling

import path from "node:path"
import { getValidForgeOSCredentials, type ITelemetryService, type OAuthCredentials } from "@forgeos/core"
import { beforeEach, describe, expect, it, vi } from "vitest"
import { AuthService, type ForgeOSAuthInfo, LogoutReason } from "./auth-service"
...
vi.mock("@/hosts/host-provider", () => ({
	HostProvider: {
		get: () => ({
			getCallbackUrl: async (path: string) => `vscode://forgeos.forgeos${path}`,
		}),
	},
}))
...
// Mock buildBasicForgeOSHeaders
vi.mock("@/services/EnvUtils", () => ({
	buildBasicForgeOSHeaders: async () => ({}),
}))
...
const mockLoginForgeOSOAuth = vi.hoisted(() => vi.fn())

// Mock @forgeos/core OAuth functions
vi.mock("@forgeos/core", async () => ({
	...
	loginForgeOSOAuth: mockLoginForgeOSOAuth,
	loginOcaOAuth: vi.fn(),
	loginOpenAICodex: vi.fn(),
	refreshForgeOSToken: vi.fn(),
	getValidForgeOSCredentials: vi.fn(),
}))

// Stateful in-memory provider-settings store. ForgeOS credentials are persisted
...
interface AuthServiceTestAccess {
	_forgeosAuthInfo: ForgeOSAuthInfo | null
...
function createTestAuthInfo(overrides?: Partial<ForgeOSAuthInfo>): ForgeOSAuthInfo {
	return {
...
describe("AuthService", () => {
	...
	describe("getInfo() — auth state for webview", () => {
		...
		it("returns authenticated state with user info when logged in", () => {
			...
			testAccess(authService)._forgeosAuthInfo = authInfo
	...
	describe("getProviderName()", () => {
		...
		it("returns the provider name when authenticated", () => {
			...
			expect(authService.getProviderName()).toBe("forgeos")
		})
	})
	...
	describe("getAuthToken()", () => {
		...
		it("returns the refreshed token when an already-expired token refreshes successfully", async () => {
			...
			testAccess(authService)._forgeosAuthInfo = createTestAuthInfo({
				expiresAt: Math.floor(Date.now() / 1000) - 100, // already expired
			})
			testAccess(authService)._authenticated = true
			vi.mocked(getValidForgeOSCredentials).mockResolvedValue(createTestOAuthCredentials())
		...
		it("does not emit its own logout event when the refresh token is rejected mid-session", async () => {
			...
			vi.mocked(getValidForgeOSCredentials).mockResolvedValue(null)
		...
		it("still rejects a token that comes back from refresh already expired", async () => {
			...
			testAccess(authService)._authenticated = true
			vi.mocked(getValidForgeOSCredentials).mockResolvedValue({
	...
	describe("createAuthRequest()", () => {
		it("returns the SDK device auth instruction so the webview can display the browser confirmation code", async () => {
			mockLoginForgeOSOAuth.mockImplementationOnce(async ({ callbacks, useWorkOSDeviceAuth }) => {
		...
		it("persists the session start time in ForgeOS auth metadata", async () => {
			mockLoginForgeOSOAuth.mockImplementationOnce(async ({ callbacks }) => {
				callbacks.onAuth({
					url: "https://example.com/device?user_code=ABCD-EFGH",
					instructions: "Enter this code in your browser: ABCD-EFGH",
				})
			...
			await waitForCondition(() => mockProviderSettings.has("forgeos"))

			const persisted = mockProviderSettings.get("forgeos") as { auth?: { metadata?: Record<string, unknown> } }
	...
	describe("handleDeauth() — logout", () => {
		it("clears auth state and pushes unauthenticated state", async () => {
			...
			// Seed persisted ForgeOS credentials in providers.json.
			mockProviderSettings.set("forgeos", {
				provider: "forgeos",
			...
			expect(testAccess(authService)._forgeosAuthInfo).toBeNull()
			expect(testAccess(authService)._authenticated).toBe(false)

			// Persisted credentials should be cleared from providers.json.
			expect(mockProviderSettings.get("forgeos")?.auth).toBeUndefined()
			expect(mockCaptureAuthLoggedOut).toHaveBeenCalledWith("forgeos", LogoutReason.USER_INITIATED)
		})
	})

	describe("token persistence (providers.json)", () => {
		// ForgeOS OAuth credentials are persisted to providers.json via the SDK's
		...
		it("restores credentials persisted in providers.json", async () => {
			...
			expect(testAccess(authService)._forgeosAuthInfo?.idToken).toBe("persisted-access-token")
			expect(testAccess(authService)._forgeosAuthInfo?.startedAt).toBeUndefined()
			expect(
				(mockProviderSettings.get("forgeos")?.auth as { metadata?: Record<string, unknown> } | undefined)?.metadata,
			).toBeUndefined()
			expect(getValidForgeOSCredentials).toHaveBeenCalledWith(
				expect.any(Object),
		...
		it("sets unauthenticated state when providers.json has no ForgeOS auth", async () => {
			await authService.restoreRefreshTokenAndRetrieveAuthInfo()
		...
		it("clears persisted credentials when stored tokens are no longer valid", async () => {
			...
			// getValidForgeOSCredentials returning null models an unrecoverable token.
			vi.mocked(getValidForgeOSCredentials).mockResolvedValue(null)
	...
	describe("restoreRefreshTokenAndRetrieveAuthInfo()", () => {
		it("strips the workos: prefix from the persisted access token", async () => {
			...
			expect(testAccess(authService)._forgeosAuthInfo?.idToken).toBe("raw-access-token")
		})
		...
		it("reports nothing when refreshing the stored session fails transiently", async () => {
			...
			vi.mocked(getValidForgeOSCredentials).mockRejectedValue(new Error("fetch failed"))

			await authService.restoreRefreshTokenAndRetrieveAuthInfo()

			expect(testAccess(authService)._authenticated).toBe(false)
			expect(mockProviderSettings.get("forgeos")?.auth).toBeDefined()
			expect(mockCaptureAuthLoggedOut).not.toHaveBeenCalled()
		})
		...
		it("reports restore_error when restore fails outside the credential refresh", async () => {
			...
			expect(mockCaptureAuthLoggedOut).toHaveBeenCalledWith("forgeos", LogoutReason.RESTORE_ERROR)
		})

		it("emits exactly one auth_logged_out — from the SDK resolver — when the stored refresh token is rejected", async () => {
			// Boundary test: run the REAL getValidForgeOSCredentials (the module
			...
			// program (same reason the @forgeos/core vitest stub is tsc-excluded);
			// vitest resolves it at runtime.
			const realForgeOSAuthModulePath = path.resolve(import.meta.dirname, "../../../../sdk/packages/core/src/auth/forgeos.ts")
			const { getValidForgeOSCredentials: realGetValidForgeOSCredentials } = (await import(
				/* @vite-ignore */ realForgeOSAuthModulePath
			)) as { getValidForgeOSCredentials: typeof getValidForgeOSCredentials }
			vi.mocked(getValidForgeOSCredentials).mockImplementation(realGetValidForgeOSCredentials as never)
