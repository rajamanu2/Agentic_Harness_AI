...
class NoOpErrorProvider implements IErrorProvider {
	async captureException(error: Error | ForgeOSError, properties?: Record<string, unknown>): Promise<void> {
		Logger.error("[NoOpErrorProvider] captureException called", { error: error.message || String(error), properties })
	}

	public logException(error: Error | ForgeOSError, _properties?: Record<string, unknown>): void {
