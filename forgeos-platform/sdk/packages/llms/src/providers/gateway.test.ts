...
async function captureReasoningOptions({
	...
	providerId: "forgeos" | "openrouter";
	options: Record<string, unknown>;
	defaults?: GatewayModelHandleOptions;
}): Promise<unknown> {
	mockSuccessfulStream();
	const isForgeOS = providerId === "forgeos";
	const modelId = isForgeOS ? "anthropic/claude-sonnet-4.6" : "openai/gpt-5.4";
...
const originalCaptureProviderRequest =
	process.env.FORGEOS_CAPTURE_PROVIDER_REQUEST;
const originalCaptureWire = process.env.FORGEOS_CAPTURE_WIRE;
const originalCaptureDir = process.env.FORGEOS_CAPTURE_DIR;
const originalCaptureDataDir = process.env.FORGEOS_DATA_DIR;
const originalCaptureMaxPreviewBytes =
	process.env.FORGEOS_CAPTURE_MAX_PREVIEW_BYTES;
const originalCaptureCleanup = process.env.FORGEOS_CAPTURE_CLEANUP;
...
describe("sdk-gateway", () => {
	beforeEach(() => {
		...
		if (originalCaptureProviderRequest === undefined) {
			delete process.env.FORGEOS_CAPTURE_PROVIDER_REQUEST;
		} else {
			process.env.FORGEOS_CAPTURE_PROVIDER_REQUEST =
				originalCaptureProviderRequest;
		}
		if (originalCaptureWire === undefined) {
			delete process.env.FORGEOS_CAPTURE_WIRE;
		} else {
			process.env.FORGEOS_CAPTURE_WIRE = originalCaptureWire;
		}
		if (originalCaptureDir === undefined) {
			delete process.env.FORGEOS_CAPTURE_DIR;
		} else {
			process.env.FORGEOS_CAPTURE_DIR = originalCaptureDir;
		}
		if (originalCaptureDataDir === undefined) {
			delete process.env.FORGEOS_DATA_DIR;
		} else {
			process.env.FORGEOS_DATA_DIR = originalCaptureDataDir;
		}
		if (originalCaptureMaxPreviewBytes === undefined) {
			delete process.env.FORGEOS_CAPTURE_MAX_PREVIEW_BYTES;
		} else {
			process.env.FORGEOS_CAPTURE_MAX_PREVIEW_BYTES =
				originalCaptureMaxPreviewBytes;
		}
		if (originalCaptureCleanup === undefined) {
			delete process.env.FORGEOS_CAPTURE_CLEANUP;
		} else {
			process.env.FORGEOS_CAPTURE_CLEANUP = originalCaptureCleanup;
		}
	});
	...
	it("uses the OpenRouter image transport for dedicated ForgeOS image models", async () => {
		generateImageSpy.mockResolvedValue({
			images: [{ mediaType: "image/png", base64: "aGVsbG8=" }],
		});
		const gateway = createGateway({
	...
	it("uses the OpenRouter image transport for mixed ForgeOS image models", async () => {
		streamTextSpy.mockReturnValue({
			fullStream: makeStreamParts([
				{
					type: "file",
					...
					models: [
						{
							...
							name: "ForgeOS Gemini Image Test",
							modalities: {
								input: ["text", "image"],
								output: ["text", "image"],
							},
	...
	it("reads nested raw market cost for forgeos before falling back to pricing", async () => {
		streamTextSpy.mockReturnValue({
			fullStream: makeStreamParts([
				{
					type: "finish",
	...
	it("passes only canonical effort options for non-anthropic openai-compatible reasoning models", async () => {
		...
		expect(call?.providerOptions?.forgeos).not.toHaveProperty("reasoningSummary");
	});
	...
	it("passes routed GLM reasoning include/exclude provider options", async () => {
		...
		expect(streamTextSpy).toHaveBeenNthCalledWith(
			...
			expect.objectContaining({
				providerOptions: expect.objectContaining({
					...
					forgeos: expect.objectContaining({
						reasoning: { exclude: true },
					}),
				}),
			}),
		...
		);
		{
			...
			expect(call.providerOptions?.forgeos).not.toEqual(
				expect.objectContaining({ reasoning: expect.anything() }),
			);
			expect(call.providerOptions?.openaiCompatible).not.toEqual(
				expect.objectContaining({ reasoning: expect.anything() }),
	...
	it("allows unregistered model ids on known providers", async () => {
		streamTextSpy.mockReturnValue({
			fullStream: makeStreamParts([
				{ type: "text-delta", textDelta: "ForgeOS custom model" },
				{ type: "finish", usage: { inputTokens: 4, outputTokens: 2 } },
			]),
		});

		const gateway = createGateway({
			providerConfigs: [{ providerId: "forgeos", apiKey: "test-key" }],
		});
	...
	it("writes AI SDK prompt captures with request metadata correlation", async () => {
		...
		process.env.FORGEOS_CAPTURE_PROVIDER_REQUEST = "summary";
		process.env.FORGEOS_CAPTURE_DIR = captureDir;
		streamTextSpy.mockReturnValue({
			fullStream: makeStreamParts([
				{
					type: "finish",
	...
	it("honors the full capture preview byte cap override", async () => {
		...
		process.env.FORGEOS_CAPTURE_PROVIDER_REQUEST = "full";
		process.env.FORGEOS_CAPTURE_DIR = captureDir;
		process.env.FORGEOS_CAPTURE_MAX_PREVIEW_BYTES = "24";
		streamTextSpy.mockReturnValue({
			fullStream: makeStreamParts([{ type: "finish", finishReason: "stop" }]),
		});
	...
	it("does not write provider request captures without an explicit capture or data dir", async () => {
		...
		try {
			...
			await expect(
				access(join(tempCwd, ".forgeos", "provider-request-captures")),
			).rejects.toThrow();
		} finally {
			process.chdir(cwd);
		}
	...
	it("wraps provider fetch for wire capture while delegating to the configured fetch", async () => {
		...
		process.env.FORGEOS_CAPTURE_PROVIDER_REQUEST = "summary";
		process.env.FORGEOS_CAPTURE_WIRE = "true";
		process.env.FORGEOS_CAPTURE_DIR = captureDir;
	...
	it("prunes old capture files unless cleanup is disabled", async () => {
		...
		process.env.FORGEOS_CAPTURE_DIR = keepDir;
		process.env.FORGEOS_CAPTURE_CLEANUP = "off";
		await collect(
			await gateway.stream({
				providerId: "openrouter",
				modelId: "anthropic/claude-test",
