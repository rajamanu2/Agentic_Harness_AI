...
describe("TaskCancel Hook", () => {
	...
	describe("Hook Input Format", () => {
		it("should receive task metadata with completionStatus", async () => {
			const hookPath = path.join(tempDir, ".forgeosrules", "hooks", "TaskCancel")
...
console.log(JSON.stringify({
  ...
  errorMessage: ""
		...
		it("should receive all common hook input fields", async () => {
			...
			const hookScript = `#!/usr/bin/env node
const input = JSON.parse(require('fs').readFileSync(0, 'utf-8'));
const hasAllFields = input.forgeosVersion && input.hookName === 'TaskCancel' && 
...
console.log(JSON.stringify({
  ...
  errorMessage: ""
			...
			const workspaceHookPath = path.join(tempDir, ".forgeosrules", "hooks", "TaskCancel")
