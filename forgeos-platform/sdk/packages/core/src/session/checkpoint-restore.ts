...
export async function beginWorktreeRestoreTransaction(
	...
	const privateRef = `refs/forgeos/restore-transactions/${transactionId}`;
	...
	await execFile(
		...
		[
			...
			"--include-untracked",
			"--message",
			`forgeos restore transaction ${transactionId}`,
		],
		{ windowsHide: true },
	);
...
async function resolveCheckpointKind(
	...
	// Checkpoints created before `kind` was persisted used ForgeOS's stash
