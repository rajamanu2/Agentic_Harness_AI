# [experimental] @forgeos/llms

`@forgeos/llms` is the model and provider layer for the ForgeOS SDK. It gives
...
- `@forgeos/llms/runtime` for declarative config and runtime registry creation
- `@forgeos/llms/providers` for handler creation and provider settings/types
- `@forgeos/llms/models` for model catalogs and query helpers
- `@forgeos/llms` root exports for the gateway registry and shared llm contracts

## Installation

```bash
npm install @forgeos/llms zod
```

## Quick Start

```ts
import { createHandler } from "@forgeos/llms";
...
```ts
import { createLlmsRuntime, defineLlmsConfig } from "@forgeos/llms/runtime";
```

### Providers

Use `@forgeos/llms/providers` for:
...
`@forgeos/llms` and `@forgeos/shared`.

### Models

Use `@forgeos/llms/models` when you need generated provider/model metadata for
...
```ts
import { transcribeAudio } from "@forgeos/llms";
...
```ts
import { createStreamingAudioTranscriptionSession } from "@forgeos/llms";
...
- `@forgeos/llms`: runtime-focused convenience entrypoint
- `@forgeos/llms/node`: explicit Node/runtime entrypoint
- `@forgeos/llms/browser`: browser-safe bundle
- `@forgeos/llms/runtime`: focused runtime entrypoint
- `@forgeos/llms/models`: model catalog/query entrypoint
- `@forgeos/llms/providers`: provider handler/settings entrypoint

## Related Packages

- `@forgeos/agents`: agent loop and tool execution
- `@forgeos/core`: stateful runtime assembly and provider settings storage
...
1. Ensure provider keys are present in your environment (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`, `FORGEOS_API_KEY`, etc.).
...
bun -F @forgeos/llms run test:live
```
...
- When recording provider cassettes for committed replay tests, set `FORGEOS_VCR=record` and `FORGEOS_VCR_INCLUDE_REQUEST_BODY=1` so playback also verifies the sanitized request body contract.

OpenAI Codex subscription live runs use the saved OAuth credentials from `~/.forgeos/data/settings/providers.json` after `forgeos auth --provider openai-codex`. Point the plain or reasoning suite at `packages/llms/src/tests/live-providers.openai-codex.example.json` or `packages/llms/src/tests/live-providers.openai-codex.reasoning.example.json`.
...
To check that disabling reasoning actually suppresses reasoning output across models, use `packages/llms/src/tests/live-providers.reasoning-disabled.example.json`; it covers direct and routed provider paths across `forgeos`, `openai`, `openrouter`, `anthropic`, `gemini`, `vercel-ai-gateway`, `zai`, and `deepseek` where model support exists, with `reasoning.enabled: false` and the strongest available no-reasoning expectation for each provider.
