...
describe("SdkSessionEventCoordinator", () => {
	...
	it("translates and emits session messages, then posts state", async () => {
		const message: ForgeOSMessage = { ts: 1, type: "say", say: "text", text: "hello" }
	...
	it("marks a submitted queued prompt as a new streaming turn", async () => {
		const message: ForgeOSMessage = { ts: 1, type: "say", say: "user_feedback", text: "queued prompt" }
	...
	it("zeros usage and api request message cost for free ForgeOS models", async () => {
		const { coordinator, options, event } = makeCoordinator({
			isForgeOSFreeModel: vi.fn().mockResolvedValue(true),
			task: { taskId: "task-1" },
			translation: {
				messages: [
					{
	...
	it("leaves mistake-limit recovery to the SDK callback instead of mutating tool-error events", async () => {
		const message: ForgeOSMessage = { ts: 1, type: "say", say: "tool", text: "{}", partial: false }
		const { coordinator, options, event } = makeCoordinator({
			translation: {
				messages: [message],
				sessionEnded: false,
...
function makeCoordinator(input: Partial<MakeCoordinatorInput> = {}) {
	...
	const options = {
		...
		isForgeOSFreeModel: input.isForgeOSFreeModel,
