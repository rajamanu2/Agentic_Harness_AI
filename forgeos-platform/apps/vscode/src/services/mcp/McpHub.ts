...
export class McpHub {
	...
	async updateServerConnectionsRPC(newServers: Record<string, McpServerConfig>): Promise<void> {
		...
		for (const [name, config] of Object.entries(newServers)) {
			...
			if (!currentConnection) {
				...
				// Only ForgeOS-specific settings changed - update in-memory state without restart
				...
				// Also update ForgeOS-specific settings in the stored config.
	...
	async updateServerConnections(newServers: Record<string, McpServerConfig>): Promise<void> {
		...
		// Track if any connection-level changes occurred (excludes ForgeOS-specific settings)
		...
		// For ForgeOS-specific settings changes, the RPC response already updated the webview,
	 ...
	 * Excludes ForgeOS-specific settings that don't affect the MCP client.
	 *
	 * ## ForgeOS-specific settings (don't require restart):
	 ...
	 *
	 * ## Adding new ForgeOS-specific settings:
	...
	private configsRequireRestart(oldConfig: McpServerConfig, newConfig: McpServerConfig): boolean {
		// Exclude ForgeOS-specific settings from comparison (add new ones here).
	...
	private setupFileWatcher(name: string, config: Extract<McpServerConfig, { type: "stdio" }>) {
		...
		if (filePath) {
			// we use chokidar instead of onDidSaveTextDocument because it doesn't require the file to be open in the editor. The settings config is better suited for onDidSave since that will be manually updated by the user or ForgeOS (and we want to detect save events, not every file change)
