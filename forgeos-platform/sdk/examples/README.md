# ForgeOS SDK Examples

Learn how to build with the ForgeOS SDK through practical, runnable examples.

## 📁 Plugin, Hook, and Automation Examples

Plugins extend the CLI and SDK with custom capabilities. Install local files, GitHub file URLs, package directories, git repos, and npm packages with `forgeos plugin install`:
...
- Export a reusable plugin module for `.forgeos/plugins`
...
```bash
forgeos plugin install https://github.com/forgeos/forgeos/blob/main/sdk/examples/plugins/weather-metrics.ts
...
```bash
forgeos plugin install https://github.com/forgeos/forgeos/blob/main/sdk/examples/plugins/typescript-lsp/index.ts
forgeos -i "Find where createTool is defined"
```
...
forgeos -i "Use subagents to inspect this repository and report back."
```
...
**Example file-based and event-driven automation specs** for global `~/.forgeos/cron/`:
...
```bash
mkdir -p ~/.forgeos/cron
cp examples/cron/changelog-generator.cron.md ~/.forgeos/cron/
mkdir -p ~/.forgeos/cron/events
cp examples/cron/events/pr-changelog-check.event.md ~/.forgeos/cron/events/
```
...
Hooks live in `.forgeos/hooks/` and are named after the event they handle (PreToolUse, PostToolUse, TaskStart, etc.):
...
# Or Python
cp examples/hooks/PreToolUse.py ~/.forgeos/hooks/PreToolUse.py
chmod +x ~/.forgeos/hooks/PreToolUse.py

# Or TypeScript (runs via bun)
cp examples/hooks/PreToolUse.ts ~/.forgeos/hooks/PreToolUse.ts
chmod +x ~/.forgeos/hooks/PreToolUse.ts

forgeos -i "do something"  # Hooks will execute automatically
```
...
```bash
npm add @forgeos/core
```

Add `@forgeos/agents` or `@forgeos/llms` only if you intentionally want lower-level control. For RPC client helpers, prefer importing from `@forgeos/core` when you want the app-facing SDK surface.

Current SDK layering:

- `@forgeos/core` owns config discovery/watchers, runtime plugin loading, and the context pipeline
- context compaction is core-owned and runs through turn preparation before model calls
- most app integrations should stay on the `@forgeos/core` surface unless they intentionally need lower-level agent or model control
...
- [ForgeOS SDK README](../packages/README.md)
