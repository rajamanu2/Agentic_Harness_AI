...
import {
	...
	isHubDaemonProcess,
	resolveForgeOSBuildEnv,
	withResolvedForgeOSBuildEnv,
} from "@forgeos/shared";
import {
...
const COMPILED_BUN_HUB_DAEMON_ARG = "--forgeos-hub-daemon";
...
function openDetachedHubLogFile(): { fd: number; logPath: string } | undefined {
	try {
		const logPath = join(resolveForgeOSDataDir(), "logs", "hub-daemon.log");
...
function resolveDefaultHubOwnerContext() {
	return resolveForgeOSBuildEnv() === "production"
		? resolveProductionHubOwnerContext()
		: resolveSharedHubOwnerContext();
}
 ...
 *
 * Exported for `forgeos doctor`, which must not mistake a shielded live hub for
...
async function retireLegacySharedHub(owner: HubOwnerContext): Promise<void> {
	if (resolveForgeOSBuildEnv() !== "production") {
		return;
	}
...
function resolveLaunchCommand(
	...
	return {
		...
		cwd: workspaceRoot,
		env: {
			...withResolvedForgeOSBuildEnv(process.env),
			FORGEOS_NO_INTERACTIVE: "1",
...
async function ensureDetachedHubServerLocked(
	...
	const hasExplicitEndpoint =
		...
		!!process.env.FORGEOS_HUB_PORT?.trim();
	...
	if (expected?.url) {
		...
		if (isReusableHubRecord(expected)) {
			...
			const upgradeHint = retiredUnusableDiscovery
				? " This can happen immediately after upgrading from a build that wrote an empty hub auth token; run 'forgeos doctor fix' to stop the old daemon and repair local hub discovery."
				: "";
			throw new Error(
				`A compatible ForgeOS Hub is already running at ${expectedUrl}, but its discovery record is missing or unreadable and no usable auth token is available. Run 'forgeos doctor fix' to repair local hub discovery.${upgradeHint}`,
			);
		}
		...
		if (expectedOutcome === "deferred_busy") {
			...
			if (endpointOverrides.allowPortFallback !== true && endpoint.port !== 0) {
				throw new Error(
					`An older ForgeOS Hub is running at ${expectedUrl} and is still serving active sessions, so it was not replaced, but no usable auth token is available to attach to it. Finish those sessions, or run 'forgeos doctor fix' to stop the hub.`,
				);
			}
		}
		if (
			...
			throw new Error(
				`An incompatible ForgeOS Hub is already running at ${expectedUrl} and could not be retired automatically. Run 'forgeos doctor fix' to stop stale hub daemons before starting a new hub.`,
			);
		}
	}
	...
	while (Date.now() < deadline) {
		...
		if (nextExpected?.url && !isReusableHubRecord(nextExpected)) {
			...
			if (
				...
				throw new Error(
					`An incompatible ForgeOS Hub is still running at ${expectedUrl} and could not be retired automatically. Run 'forgeos doctor fix' to stop stale hub daemons before starting a new hub.`,
				);
			}
		}
