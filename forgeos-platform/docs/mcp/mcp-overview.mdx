...
title: "MCP"
sidebarTitle: "MCP"
description: "Add, configure, and use MCP servers in ForgeOS."
---

MCP (Model Context Protocol) lets ForgeOS use external tools and data sources through MCP servers.

## What MCP gives you

- Connect ForgeOS to external APIs and services
- Add custom tools beyond built-in ForgeOS tools
- Use either local servers or remote hosted servers

## Quick start

1. Open **MCP Servers** in ForgeOS
...
- **CLI:** `~/.forgeos/mcp.json`
- **IDE extensions:**
  1. In the ForgeOS panel, click the **MCP Servers** icon (stacked server icon in the top toolbar).
...
MCP also works in ForgeOS CLI. Configure servers in CLI MCP settings and use the same server definitions.

You can also list servers non-interactively:

```bash
forgeos config mcp
forgeos config mcp --json
```
