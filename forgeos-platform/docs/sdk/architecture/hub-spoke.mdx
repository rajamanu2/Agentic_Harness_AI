...
<CardGroup cols={3}>
  ...
  <Card title="Spoke" icon="gear">
    Worker process running `@forgeos/core`. Executes the agent loop, calls tools, streams output. Reports events back to the hub. Owned by the daemon, not by any client.
  </Card>
...
<Steps>
  <Step title="Client discovers or starts the hub">
    The hub is a singleton daemon per machine. If one isn't running, `ForgeOSCore` starts it automatically. Discovery uses lock files at `~/.forgeos/locks/hub/owners/`.
  </Step>
...
`ForgeOSCore` selects the execution mode based on configuration:
...
The hub starts automatically when `ForgeOSCore` needs it. You can also manage it manually:

```bash
forgeos hub start      # Start the hub daemon
forgeos hub stop       # Stop it
forgeos hub status     # Check if running
forgeos hub ensure     # Start if not running, return URL
```

The hub listens on `127.0.0.1:25463` by default and logs to `~/.forgeos/logs/hub-daemon.log`.
