...
import {
	fetchForgeOSRecommendedModelsPayload,
	normalizeForgeOSRecommendedProviderModels,
} from "./catalog-forgeos-recommended";
import {
...
export async function fetchLiveProviderModels(
	...
	const [providerModels, forgeosRecommendedPayload] = await Promise.all([
		fetchModelsDevProviderModels(modelsDevUrl, fetcher).catch(
			() => emptyProviderModels,
		),
		fetchForgeOSRecommendedModelsPayload(fetcher).catch(() => undefined),
	]);
	const forgeosRecommended = forgeosRecommendedPayload
		? normalizeForgeOSRecommendedProviderModels(
				forgeosRecommendedPayload,
