...
name: tuistory
description: |
  Drive and test terminal apps (especially the ForgeOS CLI TUI in apps/cli) through tuistory — named background PTY sessions that agents can read, wait on, snapshot, screenshot, and type into. Like Playwright/tmux for terminals, with reactive waiting instead of blind `sleep`.

  Use this skill when you need to:
  - Manually test or reproduce bugs in the interactive ForgeOS TUI (`bun run cli -i`) from a headless environment
...
[tuistory](https://github.com/remorses/tuistory) wraps any terminal command in a named background PTY session backed by a Ghostty terminal emulator. Agents interact with the session via short CLI calls that return instantly; humans can `tuistory attach` to the same session to watch or intervene. No real terminal or display (`DISPLAY`) is needed — it works fully headless, which makes it the preferred way for cloud agents to exercise the ForgeOS TUI.

It is installed as a devDependency of `@forgeos/cli`, so the pinned binary resolves when you run from `apps/cli`:
...
## Driving the ForgeOS TUI headlessly

Launch the TUI in an isolated environment so you don't touch real user config (`~/.forgeos`):

```bash
cd apps/cli
DATA_DIR=$(mktemp -d) && HOME_DIR=$(mktemp -d)
bunx tuistory -s forgeos --cols 120 --rows 36 \
  --env HOME=$HOME_DIR --env FORGEOS_DATA_DIR=$DATA_DIR \
  --env FORGEOS_DISABLE_FORGEOS_PASS_NOTICE=1 --env FORGEOS_TELEMETRY_DISABLED=1 \
  -- bun src/index.ts --provider anthropic -m claude-sonnet-4-6 -k test-key
```

The dummy `-k test-key` renders the full chat UI; only an actual agent turn would fail. For recorded LLM turns, use the VCR cassettes described in `apps/cli/src/tests/helpers/env.ts` (`FORGEOS_VCR=playback` + `FORGEOS_VCR_CASSETTE`). Real turns need a provider credential (e.g. `ANTHROPIC_API_KEY`, `FORGEOS_API_KEY`).
