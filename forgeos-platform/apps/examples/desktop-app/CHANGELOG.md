# ForgeOS Code Desktop Changelog
...
- Extended thinking budgets reach the provider again on ForgeOS Pass — they had silently stopped applying when the gateway moved off the generic OpenAI-compatible path.
- Two ForgeOS installs on different builds no longer shut each other's Hub down in a loop, which was killing live sessions with an abnormal disconnect.
...
- Reconnecting to a stale managed ForgeOS Hub daemon is fixed. When another ForgeOS install ships a newer Hub, the app now prompts to update and restart — and stages the app update first, so it no longer relaunches into the same version and immediately re-prompts.
...
- Added "Get an API key" links for popular providers in onboarding and Settings → Models, plus a link to the ForgeOS dashboard from the ForgeOS API key form.
- The onboarding welcome step now explains what ForgeOS is.
...
- ForgeOS Code now ships as a single universal macOS download that runs natively on both Apple Silicon and Intel — no more picking the right architecture. Existing per-architecture installs migrate to it automatically on their next update.
...
- Empty model responses are now retried on every provider, not just Ollama, fixing hard "Model returned empty response" failures on OpenRouter, ForgeOS, and OpenAI-compatible endpoints.
...
- Model pickers show proper display names for ForgeOS free models and recommended models.
...
- Unknown or removed legacy model IDs now fall back to the default ForgeOS model instead of failing.
- Fixed agentic compaction not persisting reliably, so long conversations resume in the right state.
- Fixed a `.forgeosrules` single file (the older format) aborting the whole rules and config scan.
...
- Free ForgeOS models are now supported and labeled "(free)" in model pickers, with a clear message — including reset time — when you hit the free-tier limit.
...
- Clarified the auto-update setting — it's now "Keep CLI up to date" and explains that it governs the `forgeos` terminal command, not the app itself (the app updates separately).
...
- Added a ForgeOS API key path to onboarding, and you can now cancel a pending browser sign-in instead of being stuck waiting for it.
...
- First public release of ForgeOS Code for macOS: a desktop app for running and inspecting ForgeOS agent sessions, signed and notarized for Apple Silicon and Intel.
