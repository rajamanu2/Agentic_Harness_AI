---
title: "CLI Reference"
description: "Complete command reference for ForgeOS CLI including all commands, flags, and configuration options."
---

```bash
forgeos --help           # Show all commands
forgeos <command> --help # Show help for a specific command
```

## Synopsis

```bash
forgeos [options] [command] [prompt]
```

## Help Menu (Source of Truth)

```text
Usage: forgeos [options] [command] [prompt]

ForgeOS CLI - AI coding assistant in your terminal
...
Options:
  ...
  --config <path>              Configuration directory (default: ~/.forgeos/data/settings)
  --data-dir <path>            Use isolated local state at this directory path (default: ~/.forgeos)
  ...
  --hooks-dir <path>           Directory path to additional hooks for runtime hook injection (default: ~/.forgeos/hooks)
...
```bash
forgeos
forgeos "your prompt here"
forgeos "Run tests and fix failures"
echo "prompt" | forgeos
```
...
```bash
cat payload.json | forgeos hook
```

### `plugin`

Manage ForgeOS plugins. Install plugins from file URLs, npm, git repositories, or local paths. See [Plugins](/customization/plugins) for full details and the plugin manifest format.

```bash
forgeos plugin install <source>  # Install a plugin
forgeos plugin i <source>        # Shorthand alias
```
...
Try it with the [TypeScript Navigation Plugin](https://github.com/forgeos/typescript-lsp-plugin):
...
Show ForgeOS CLI version number.
...
|----------|-------------|
| `FORGEOS_DATA_DIR` | Custom configuration directory (replaces `~/.forgeos/data/`) |
...
| `FORGEOS_BUILD_ENV` | Set to `development` for debug features |
| `FORGEOS_DEBUG_PORT_BASE` | Base port for Node.js inspector |
| `FORGEOS_COMMAND_PERMISSIONS` | JSON policy restricting shell commands (see below) |
