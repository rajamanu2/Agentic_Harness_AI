...
function stripForgeOSApiKeyPrefix(accessToken: string): string {
	const token = accessToken.trim();
	return token.toLowerCase().startsWith(WORKOS_TOKEN_PREFIX)
...
function createForgeOSAuthHandler(input: {
	providerId: string;
	storageProviderId?: string;
}): ProviderAuthHandler {
	return createOAuthHandler({
		...
		formatAccessToken: formatForgeOSApiKey,
		normalizeStoredAccessToken: stripForgeOSApiKeyPrefix,
		...
		refresh: ({ settings, credentials, forceRefresh, telemetry }) =>
			getValidForgeOSCredentials(
				credentials as ForgeOSOAuthCredentials,
				{
					apiBaseUrl:
						settings.baseUrl?.trim() || getForgeOSEnvironmentConfig().apiBaseUrl,
					telemetry,
...
const providerAuthHandlers = [
	createForgeOSAuthHandler({ providerId: "forgeos" }),
	createForgeOSAuthHandler({
		providerId: "forgeos-pass",
		storageProviderId: "forgeos",
	}),
	createOAuthHandler({
		providerId: "oca",
		login: ({ settings, callbacks, telemetry }) =>
