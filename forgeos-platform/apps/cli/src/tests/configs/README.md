...
Each subdirectory here is a **ForgeOS config directory** used by one or more test suites.
Tests point to a config via `forgeosEnv("<name>")` in `tests/e2e/cli/helpers/env.ts`.
...
2. Create `configs/<name>/data/settings/forgeos_mcp_settings.json` (can be `{ "mcpServers": {} }`).
3. Reference it in tests with `forgeosEnv("<name>")`.
