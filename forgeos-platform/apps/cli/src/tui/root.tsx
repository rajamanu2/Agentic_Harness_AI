...
import { formatDisplayUserInput } from "@forgeos/shared";
...
import { shouldSuppressForgeOSCliMigrationNoticeForActiveProvider } from "../kanban-migration/notice";
...
function App(props: TuiProps) {
	...
	const [appView, setAppView] = useState<AppView>(() => {
		if (process.env.FORGEOS_FORCE_ONBOARDING === "1") return "onboarding";
	...
	const openAccount = useAccountDialog({
		dialog,
		termHeight,
		loadAccount: props.loadForgeOSAccount,
		switchAccount: props.switchForgeOSAccount,
	...
	useEffect(() => {
		...
		if (
			shouldSuppressForgeOSCliMigrationNoticeForActiveProvider(currentProviderId)
		) {
	...
	useEffect(() => {
		...
		void dialog
			...
			.then((update) => {
				if (update) {
					(onHubUpdateRestart ?? exitForgeOS)();
					return;
				}
				showToast(
					"Hub still differs from this CLI. Run 'forgeos update' and restart when convenient.",
					"info",
				);
				refocusTextareaRef.current();
			})
	...
	}, [dialog, exitForgeOS, hubBuildMismatch, onHubUpdateRestart, showToast]);
