...
import { createSessionId } from "@forgeos/shared"
...
export interface SdkTaskStartCoordinatorOptions {
	...
	isForgeOSManagedProviderActive: () => boolean
	emitForgeOSAuthError: (task?: string) => void
...
export class SdkTaskStartCoordinator {
	...
	async initTask(
		...
		try {
			...
			if (usesForgeOSAccountAuth(config.providerId) && !config.apiKey) {
				Logger.warn(
					`[SdkController] ${config.providerId} provider selected but no ForgeOS auth token — emitting auth error`,
				)
				...
				this.options.emitForgeOSAuthError(prompt)
				return undefined
			}
	...
	private emitInitialTaskMessage(sessionId: string, task: string, images?: string[], files?: string[]): void {
		...
		const taskMessage: ForgeOSMessage = {
			ts: Date.now(),
			type: "say",
			say: "task",
			text: task,
	...
	private handleInitError(error: unknown, sessionId?: string): void {
		...
		;(globalThis as Record<string, unknown>).__forgeos_last_init_error = errorDetails
		;(globalThis as Record<string, unknown>).__forgeos_last_init_error_raw = error
	...
	private handleReinitError(taskId: string, error: unknown): void {
		...
		const isForgeOSAuthReinit =
			this.options.isForgeOSManagedProviderActive() &&
			(reinitErrorMsg.includes(FORGEOS_ACCOUNT_AUTH_ERROR_MESSAGE) ||
				reinitErrorMsg.toLowerCase().includes("missing api key") ||
				reinitErrorMsg.toLowerCase().includes("unauthorized"))

		if (isForgeOSAuthReinit) {
