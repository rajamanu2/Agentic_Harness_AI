import type { AvailableRuntimeCommand } from "@forgeos/core"
...
describe("expandSlashCommands", () => {
	...
	it("resolves a typed filename to a frontmatter-renamed workflow via records", () => {
		...
		const records = [{ name: "ship-it", filePath: "/repo/.forgeosrules/workflows/release.md" }]
	...
	it("resolves a typed filename to a frontmatter-renamed workflow whose name normalizes", () => {
		...
		const records = [{ id: "ship it", name: "Ship It", filePath: "/repo/.forgeosrules/workflows/release.md" }]
		expect(expandSlashCommands("/release.md", renamed, { workflowRecords: records })).toBe("Ship it carefully.")
		// The renamed command stays governed by its file's toggle.
		expect(
			expandSlashCommands("/release.md", renamed, {
...
describe("buildDisabledWorkflowNames", () => {
	it("disables records whose file toggle is off, by exact command name", () => {
		const disabled = buildDisabledWorkflowNames({
			records: [
				{ name: "Release", filePath: "/home/user/Documents/ForgeOS/Workflows/Release.md" },
				{ name: "notes", filePath: "/home/user/Documents/ForgeOS/Workflows/notes.txt" },
				{ name: "keep", filePath: "/home/user/Documents/ForgeOS/Workflows/keep.md" },
			],
			globalToggles: {
				"/home/user/Documents/ForgeOS/Workflows/Release.md": false,
				"/home/user/Documents/ForgeOS/Workflows/notes.txt": false,
				"/home/user/Documents/ForgeOS/Workflows/keep.md": true,
			},
		})
	...
	it("matches the toggle by file basename even when frontmatter renames the command", () => {
		const disabled = buildDisabledWorkflowNames({
			records: [{ name: "ship-it", filePath: "/repo/.forgeosrules/workflows/release.md" }],
			workspaceToggles: { "/repo/.forgeosrules/workflows/release.md": false },
		})
	...
	it("keeps a name enabled when any scope has it enabled", () => {
		...
		const records = [{ name: "release", filePath: "/repo/.forgeosrules/workflows/release.md" }]
		expect(
		...
		expect(
			buildDisabledWorkflowNames({
				...
				workspaceToggles: { "/repo/.forgeosrules/workflows/release.md": true },
			}),
		).toEqual(new Set())
	})
	...
	it("governs each command by its own record when similar names span scopes", () => {
		...
		const records = [
			{ name: "Release", filePath: "/repo/.forgeosrules/workflows/Release.md" },
			{ name: "release", filePath: "/repo/.forgeos/remote-config/workflows/release.md" },
		]
		expect(
			buildDisabledWorkflowNames({
				records,
	...
	it("treats records without any toggle entry as enabled", () => {
		const disabled = buildDisabledWorkflowNames({
			records: [{ name: "fresh", filePath: "/home/user/.forgeos/workflows/fresh.md" }],
	...
	it("governs remote-config records by name-keyed remote toggles", () => {
		const disabled = buildDisabledWorkflowNames({
			records: [
				{ name: "org-standards", filePath: "/repo/.forgeos/remote-config/workflows/org-standards.md" },
				{ name: "org-review", filePath: "/repo/.forgeos/remote-config/workflows/org-review.md" },
				{ name: "org-default", filePath: "C:\\repo\\.forgeos\\remote-config\\workflows\\org-default.md" },
			],
	...
	it("keys remote toggles off the materialized filename even when frontmatter aliases the command", () => {
		const disabled = buildDisabledWorkflowNames({
			records: [{ name: "friendly-alias", filePath: "/repo/.forgeos/remote-config/workflows/org-standards.md" }],
	...
	it("matches remote toggles whose config names get sanitized during materialization", () => {
		...
		const disabled = buildDisabledWorkflowNames({
			records: [{ name: "org-standards", filePath: "/repo/.forgeos/remote-config/workflows/org-standards.md" }],
			remoteToggles: { "Org Standards": false },
		})
		expect(disabled).toEqual(new Set(["org-standards"]))
	})
	...
	it("merges colliding sanitized remote names as enabled-if-any-enabled", () => {
		...
		const records = [{ name: "org-standards", filePath: "/repo/.forgeos/remote-config/workflows/org-standards.md" }]
		expect(
			buildDisabledWorkflowNames({
				records,
	...
	it("matches remote toggles for names longer than the materializer's 80-char cap", () => {
		...
		const disabled = buildDisabledWorkflowNames({
			records: [{ name: materializedName, filePath: `/repo/.forgeos/remote-config/workflows/${materializedName}.md` }],
	...
	it("collects disabled names across local and remote scopes", () => {
		const disabled = buildDisabledWorkflowNames({
			records: [
				...
				{ name: "hotfix", filePath: "C:\\repo\\.forgeosrules\\workflows\\hotfix.md" },
				{ name: "org-standards", filePath: "/repo/.forgeos/remote-config/workflows/org-standards.md" },
			],
			globalToggles: { "/global/dir/deploy.md": false, "/global/dir/keep.md": true },
			workspaceToggles: { "C:\\repo\\.forgeosrules\\workflows\\hotfix.md": false },
			remoteToggles: { "org-standards": false },
		})
