#!/usr/bin/env bash
# ForgeOS Hook: PreToolUse (Require Review)
# Pauses for review before writing to critical files
# Copy to ~/.forgeos/hooks/PreToolUse.sh and chmod +x
...
if [ "$tool" = "editor" ] || [ "$tool" = "write_file" ]; then
  if [[ $file =~ (package\.json|tsconfig|\.env|secrets|config\.(ts|js)|\.forgeos) ]]; then
