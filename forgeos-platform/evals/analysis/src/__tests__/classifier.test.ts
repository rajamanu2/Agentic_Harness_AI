...
describe("FailureClassifier", () => {
	...
	describe("Provider Bug Detection", () => {
		it("detects Gemini signature issue", () => {
			...
			expect(failures[0].issue_url).toBe("https://github.com/forgeos/forgeos/issues/7974")
		})

		it("detects Claude tool format issue", () => {
			const logs = `ForgeOS tried to use write_to_file without value for required parameter 'content'`
			const failures = classifier.classify(logs)

			expect(failures.length).toBeGreaterThan(0)
			expect(failures[0].name).toBe("claude_tool_format")
			expect(failures[0].category).toBe("provider_bug")
			expect(failures[0].issue_url).toBe("https://github.com/forgeos/forgeos/issues/7998")
		})
	})
