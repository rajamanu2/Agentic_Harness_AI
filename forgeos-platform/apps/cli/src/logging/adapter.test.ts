...
describe("createCliLoggerAdapter", () => {
	...
	it("resolves default runtime config from data dir", () => {
		...
		delete process.env.FORGEOS_LOG_PATH;
		delete process.env.FORGEOS_LOG_LEVEL;
		delete process.env.FORGEOS_LOG_NAME;
	...
	it("maps core logger metadata with error payload", () => {
		...
		const snapshot = withEnvSnapshot();
		process.env.FORGEOS_DATA_DIR = dataDir;
		process.env.FORGEOS_LOG_ENABLED = "0";
		try {
			const adapter = createCliLoggerAdapter({ runtime: "cli" });
			expect(() => {
	...
	it("falls back when log destination path is not writable", () => {
		...
		process.env.FORGEOS_LOG_PATH = unwritablePath;
		delete process.env.FORGEOS_LOG_LEVEL;
		delete process.env.FORGEOS_LOG_NAME;
		delete process.env.FORGEOS_LOG_ENABLED;
