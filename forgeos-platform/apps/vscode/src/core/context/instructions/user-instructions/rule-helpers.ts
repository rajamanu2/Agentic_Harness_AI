...
export async function synchronizeRuleToggles(
	...
	currentToggles: ForgeOSRulesToggles,
	allowedFileExtension = "",
	excludedPaths: string[][] = [],
): Promise<ForgeOSRulesToggles> {
...
export function synchronizeRemoteRuleToggles(
	...
	const updatedToggles: ForgeOSRulesToggles = {}
 ...
 */
export function combineRuleToggles(toggles1: ForgeOSRulesToggles, toggles2: ForgeOSRulesToggles): ForgeOSRulesToggles {
 ...
 */
const getRuleFilesTotalContent = async (rulesFilePaths: string[], basePath: string, toggles: ForgeOSRulesToggles) => {
	return (await getRuleFilesTotalContentWithMetadata(rulesFilePaths, basePath, toggles)).content
}

const LOCAL_RULE_PATHS = {
	forgeosRules: ".forgeosrules",
	workflows: ".forgeosrules/workflows",
} as const
...
type RuleFileController = {
	stateManager: {
		getGlobalSettingsKey(key: "globalWorkflowToggles" | "globalForgeOSRulesToggles"): ForgeOSRulesToggles
		setGlobalState(key: "globalWorkflowToggles" | "globalForgeOSRulesToggles", value: ForgeOSRulesToggles): void
 ...
 * Used by getGlobalForgeOSRules and getLocalForgeOSRules.
 */
...
function getRemoteRulesTotalContentWithMetadata(
	remoteRules: GlobalInstructionsFile[],
	remoteToggles: ForgeOSRulesToggles,
...
/**
 * Handles converting any directory into a file (specifically used for .forgeosrules and .forgeosrules/workflows)
 * The old .forgeosrules file or .forgeosrules/workflows file will be renamed to a default filename
 * Doesn't do anything if the dir already exists or doesn't exist
 * Returns whether there are any uncaught errors
 */
async function ensureLocalForgeOSDirExists(forgeosrulePath: string, defaultRuleFilename: string): Promise<boolean> {
	try {
		const exists = await fileExistsAtPath(forgeosrulePath)

		if (exists && !(await isDirectory(forgeosrulePath))) {
			// logic to convert .forgeosrules file into directory, and rename the rules file to {defaultRuleFilename}
			const content = await fs.readFile(forgeosrulePath, "utf8")
			const tempPath = forgeosrulePath + ".bak"
			await fs.rename(forgeosrulePath, tempPath) // create backup
			try {
				await fs.mkdir(forgeosrulePath, { recursive: true })
				await fs.writeFile(path.join(forgeosrulePath, defaultRuleFilename), content, "utf8")
				...
				try {
					await fs.rm(forgeosrulePath, { recursive: true, force: true }).catch(() => {})
					await fs.rename(tempPath, forgeosrulePath) // restore backup
				} catch (_restoreError) {}
...
export const createRuleFile = async (isGlobal: boolean, filename: string, cwd: string, type: string) => {
	try {
		...
		if (isGlobal) {
			...
			if (type === "workflow") {
				const globalForgeOSWorkflowFilePath = await disk.ensureWorkflowsDirectoryExists()
				filePath = path.join(globalForgeOSWorkflowFilePath, filename)
			} else {
				const globalForgeOSRulesFilePath = await disk.ensureRulesDirectoryExists()
				filePath = path.join(globalForgeOSRulesFilePath, filename)
			}
		} else {
			const localForgeOSRulesFilePath = path.resolve(cwd, LOCAL_RULE_PATHS.forgeosRules)

			const hasError = await ensureLocalForgeOSDirExists(localForgeOSRulesFilePath, "default-rules.md")
			if (hasError === true) {
				return { filePath: null, fileExists: false }
			}

			await fs.mkdir(localForgeOSRulesFilePath, { recursive: true })

			if (type === "workflow") {
				const localWorkflowsFilePath = path.resolve(cwd, LOCAL_RULE_PATHS.workflows)

				const hasError = await ensureLocalForgeOSDirExists(localWorkflowsFilePath, "default-workflows.md")
				if (hasError === true) {
					return { filePath: null, fileExists: false }
				}
				...
				// forgeosrules file creation
				filePath = path.join(localForgeOSRulesFilePath, filename)
			}
		}
