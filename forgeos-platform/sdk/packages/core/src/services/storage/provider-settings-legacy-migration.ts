...
import { ReasoningLevelSchema } from "@forgeos/shared";
...
interface LegacyGlobalState {
	...
	planModeForgeOSModelId?: string;
	actModeForgeOSModelId?: string;
...
interface LegacySecrets {
	apiKey?: string;
	forgeosApiKey?: string;
	"forgeos:forgeosAccountId"?: string;
	forgeosAccountId?: string;
...
export type LegacyForgeOSUserInfo = {
	idToken: string;
	expiresAt: number;
	refreshToken: string;
	userInfo: {
...
/**
 * Resolves legacy ForgeOS account auth data from the raw `forgeos:forgeosAccountId`
 ...
 * unparseable JSON.
 */
export function resolveLegacyForgeOSAuth(
	...
	try {
		const data = JSON.parse(trimmed) as LegacyForgeOSUserInfo;
		if (!data) {
			return undefined;
		}
		const expiresAt =
...
function resolveLegacyStorage(
	...
	const dataDir = options.dataDir ?? resolveForgeOSDataDir();
...
function resolveModelForProvider(
	...
	const providerModelKeyById: Record<string, keyof LegacyGlobalState> = {
		...
		forgeos: `${modePrefix}ForgeOSModelId` as keyof LegacyGlobalState,
...
function getDefaultModelForProvider(providerId: string): string | undefined {
	...
	// ForgeOS's generated block holds a few free models while its default,
...
/**
 * ForgeOS is a fixed-catalog provider: an unknown legacy model id (retired
 ...
 * runtime catalog (the collection model list, which `buildForgeOSModels` in
 * @forgeos/llms mirrors), folding alias spellings onto their canonical ids
 ...
 */
function resolveKnownForgeOSModel(
	providerId: string,
	modelId: string | undefined,
): string | undefined {
	if (!modelId || providerId !== "forgeos") {
		return modelId;
	}
	const catalogModels =
...
function buildLegacyProviderSettings(
	...
	const secretByProvider: Record<string, string | undefined> = {
		...
		forgeos: legacySecrets.forgeosApiKey,
	...
	if (providerId === "forgeos") {
		try {
			const legacyAuthString = trimNonEmpty(
				legacySecrets["forgeos:forgeosAccountId"],
			);

			if (legacyAuthString) {
				providerSpecific.auth = {
					...(providerSpecific.auth ?? {}),
					...resolveLegacyForgeOSAuth(legacyAuthString),
				};
			}
		} catch {
			// Failed to parse stored forgeos auth data
		}
	}
	if (
...
function collectCandidateProviderIds(
	...
	if (trimNonEmpty(legacySecrets.forgeosApiKey)) candidates.add("forgeos");
	const legacyForgeOSAuth = resolveLegacyForgeOSAuth(
		trimNonEmpty(legacySecrets["forgeos:forgeosAccountId"]),
	);
	if (
		legacyForgeOSAuth?.accessToken ||
		legacyForgeOSAuth?.refreshToken ||
		legacyForgeOSAuth?.accountId
	) {
		candidates.add("forgeos");
	}
