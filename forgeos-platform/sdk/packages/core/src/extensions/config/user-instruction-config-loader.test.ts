	...
	it("resolves legacy-compatible default search paths", () => {
		...
		expect(resolveSkillsConfigSearchPaths(workspacePath)).toEqual(
			expect.arrayContaining([
				join(workspacePath, ".forgeosrules", "skills"),
				join(workspacePath, ".forgeos", "skills"),
		...
		expect(resolveRulesConfigSearchPaths(workspacePath)).toEqual(
			expect.arrayContaining([
				...
				join(workspacePath, ".forgeosrules"),
				join(workspacePath, ".forgeos", "rules"),
			]),
		);
		expect(
		...
		expect(paths).toContain(join(workspacePath, ".forgeosrules", "workflows"));
		expect(paths).toContain(join(workspacePath, ".forgeos", "workflows"));
		expect(
			paths.some(
				(p) =>
		...
		expect(paths).not.toContain(
			join(process.env.HOME ?? "~", ".forgeos", "data", "workflows"),
		);
	});

	it("discovers managed plugin instruction roots from workspace .forgeos manifests", () => {
		const workspacePath = "/repo/demo";
		expect(
			createSkillsConfigDefinition({ workspacePath }).directories,
		).toContain(join(workspacePath, ".forgeos"));
		expect(
...
Document rollout and rollback steps.`,
	...
	// Regression test for https://github.com/forgeos/forgeos/issues/12151: a leading UTF-8 BOM
...
Escalation runbook`,
	...
	it("still loads all rules when .forgeosrules is a legacy single file", async () => {
		const tempRoot = await mkdtemp(
			join(tmpdir(), "core-user-instructions-forgeosrules-file-"),
		);
		tempRoots.push(tempRoot);
		...
		const globalRulesDir = join(tempRoot, "home", ".forgeos", "rules");
		await mkdir(workspaceRoot, { recursive: true });
		await mkdir(globalRulesDir, { recursive: true });
		// Legacy single-file ruleset: `.forgeosrules/skills` and
		// `.forgeosrules/workflows` now resolve through a file (ENOTDIR), which
		// must not abort scanning of the other config sources.
		await writeFile(
			join(workspaceRoot, ".forgeosrules"),
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			...
			tempRoots.push(tempRoot);
			const skillsDir = join(tempRoot, ".forgeos", "skills");
...
Use conventional commits.`,
	...
	it("loads enterprise-style managed rules, workflows, and skills through the default workspace watcher", async () => {
		...
		const pluginRoot = join(tempRoot, ".forgeos", "enterprise");
...
Use the security review checklist.`,
	...
	it("lets workspace .forgeos workflows override legacy .forgeosrules workflows with the same name", async () => {
		const tempRoot = await mkdtemp(
			join(tmpdir(), "core-user-instructions-workflow-precedence-"),
		);
		tempRoots.push(tempRoot);

		await mkdir(join(tempRoot, ".forgeosrules", "workflows"), {
			recursive: true,
		});
		await mkdir(join(tempRoot, ".forgeos", "workflows"), { recursive: true });
		await writeFile(
			join(tempRoot, ".forgeosrules", "workflows", "release.md"),
			`---
name: release
---
Legacy release workflow.`,
		...
		await writeFile(
			join(tempRoot, ".forgeos", "workflows", "release.md"),
			`---
name: release
---
