...
const WAITING_ROW: ForgeOSMessage = {
	ts: WAITING_ROW_TS,
	type: "say",
	say: "reasoning",
	partial: true,
...
interface MessagesAreaProps {
	task: ForgeOSMessage
	groupedMessages: (ForgeOSMessage | ForgeOSMessage[])[]
	modifiedMessages: ForgeOSMessage[]
...
export const MessagesArea: React.FC<MessagesAreaProps> = ({
	...
	const { forgeosMessages, turnState } = useExtensionState()
	const lastRawMessage = useMemo(() => forgeosMessages.at(-1), [forgeosMessages])
	...
	const scrolledPastUserMessageIndex = useMemo(() => {
		...
		return forgeosMessages.findIndex((msg) => msg.ts === scrolledPastUserMessage.ts)
	}, [forgeosMessages, scrolledPastUserMessage])
	...
	const forcePendingResponseLoader = isPendingResponseUnconfirmed(chatState.pendingResponse, turnState, forgeosMessages.length)
	...
	const displayedGroupedMessages = useMemo<(ForgeOSMessage | ForgeOSMessage[])[]>(() => {
