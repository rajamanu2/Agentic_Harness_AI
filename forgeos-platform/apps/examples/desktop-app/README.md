...
Tauri desktop shell + Bun sidecar backend + Next.js UI for running and inspecting ForgeOS chat sessions.
...
`FORGEOS_SIDECAR_SKIP_SHELL_PATH=1` to disable. Implementation and details:
[`sidecar/shell-path.ts`](./sidecar/shell-path.ts).

## Web Visual System

The framework-neutral color, typography, radius, and navigation contract lives
in the internal [`@forgeos/ui`](../../../sdk/packages/ui/README.md) workspace
package. Other ForgeOS web surfaces can take only its tokens or opt into the
...
(`.forgeos/skills/publish-desktop/SKILL.md`).
...
2. The desktop backend starts the Bun sidecar, which discovers or starts the
   canonical shared ForgeOS Hub and exposes one websocket transport (`/transport`)
...
- The view is wired to the same scheduler APIs used by `forgeos schedule` through Tauri commands and `scripts/routine-schedules.ts`.
...
- Session artifacts are written under `~/.forgeos/data/sessions/<sessionId>/` (or `FORGEOS_SESSION_DATA_DIR`).
...
`~/.forgeos/data/logs/code.log` by default. Telemetry continues to honor the global
...
- `FORGEOS_LOG_ENABLED=0` disables file logging.
- `FORGEOS_LOG_LEVEL` sets the Pino level (for example, `debug` or `warn`).
- `FORGEOS_LOG_PATH` overrides the log destination.
- `FORGEOS_LOG_NAME` overrides the logger name.
...
enable the same console stream with `NEXT_PUBLIC_FORGEOS_DEBUG_LOGS=1` at build
time, or at runtime from DevTools with
`localStorage.setItem("forgeos.debugLogs", "1")` followed by a reload. Diagnostic
...
- Speech input requires an enabled provider whose models.dev metadata identifies
  ...
  **Settings → Models → Voice input**. That selection is stored separately from
  ...
  `~/.forgeos/data/settings/providers.json`; provider credentials remain in their
