...
Spin up background subagents from any ForgeOS SDK agent. Each subagent runs as its own session with its own provider, model, and system prompt — useful for parallel recon, planning, implementation, and review.
...
const forgeos = await ForgeOSCore.create({ backendMode: "auto" });
...
Pass the plugin **directory** as the path. The loader reads `package.json` and discovers entry points from the `forgeos.plugins` field.
...
`start_subagent` accepts `preset` and/or `instructions`. When `preset` is omitted it defaults to `phantom` (configurable via `FORGEOS_SUBAGENT_DEFAULT_PRESET`).
...
|---|---|---|---|
| Agents | `agents/` next to `index.ts` | `~/.forgeos/data/settings/agents/` | `<cwd>/.forgeos/agents/` |
| Skills | `skills/` next to `index.ts` | `~/.forgeos/data/settings/skills/` | `<cwd>/.forgeos/skills/` |
...
Files live under `~/.forgeos/data/plugins/subagents/handoffs/<conversationId>/`. Paths are validated to prevent traversal.
