...
interface ForgeOSStorageChangeEvent {
	readonly key: string
}

export type StorageEventListener = (event: ForgeOSStorageChangeEvent) => Promise<void>
 ...
 * VSCode's Memento and ForgeOSSyncStorage both satisfy this interface.
 */
export interface ForgeOSMemento {
 ...
 */
export abstract class ForgeOSStorage {
	/**
	 * The name of the storage, used for logging purposes.
	 */
	protected name = "ForgeOSStorage"
	/**
 ...
 * Unlike ForgeOSStorage (string key-value, async), this stores any JSON-serializable
 * values and provides synchronous access - required for VSCode Memento compatibility.
 */

export type SyncStorageEventListener = (event: ForgeOSStorageChangeEvent) => void

export abstract class ForgeOSSyncStorage<T = any> {
