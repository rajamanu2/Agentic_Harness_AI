...
describe("ProviderSettingsManager", () => {
	...
	it("resolves auth storage settings for providers registered with a storage provider id", () => {
		...
		expect(manager.getProviderSettings("forgeos-pass")).toEqual({
		...
		expect(manager.getProviderConfig("forgeos-pass")).toMatchObject({
	...
	it("falls back to forgeos when last-used provider is forgeos-pass and the feature is disabled", () => {
		const tempDir = mkdtempSync(
			path.join(os.tmpdir(), "core-provider-settings-"),
		);
		tempDirs.push(tempDir);
		...
		expect(
			manager.getLastUsedProviderSettings({ isForgeOSPassEnabled: false }),
		).toEqual({
			provider: "forgeos",
			model: "anthropic/claude-sonnet-4.6",
			baseUrl: "https://api.example.test",
		...
		expect(
			manager.getLastUsedProviderConfig({ isForgeOSPassEnabled: false }),
		).toMatchObject({
			providerId: "forgeos",
			apiKey: "workos:shared-token",
			baseUrl: "https://api.example.test",
	...
	it("returns default forgeos settings when forgeos-pass is last-used and no forgeos settings exist", () => {
		const tempDir = mkdtempSync(
			path.join(os.tmpdir(), "core-provider-settings-"),
		);
		tempDirs.push(tempDir);
