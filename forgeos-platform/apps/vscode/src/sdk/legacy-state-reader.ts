...
//
// Reads on-disk state written in the pre-SDK storage format from the ForgeOS
...
/**
 * Resolve the ForgeOS data directory.
 * Priority: FORGEOS_DATA_DIR env > FORGEOS_DIR env + "/data" > ~/.forgeos/data
 */
...
function mcpSettingsPath(dataDir?: string): string {
	return path.join(resolveDataDir(dataDir), "settings", "forgeos_mcp_settings.json")
}
 ...
 * Returns an empty array if the file is missing or corrupt.
 */
export function readUiMessages(taskId: string, dataDir?: string): ForgeOSMessage[] {
	const messages = readJsonFile<ForgeOSMessage[]>(uiMessagesPath(taskId, dataDir), [])
