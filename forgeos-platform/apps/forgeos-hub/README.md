...
A browser dashboard for the local ForgeOS hub. Open it to see who's connected, what sessions are running, drive a session from a chat box, and restart the hub when you need a fresh daemon.
...
- start a new session from an initial prompt — workspace/provider/model are reused from the most recent session, or `FORGEOS_PROVIDER` / `FORGEOS_MODEL` env vars
...
The dashboard registers two clients with the hub: a `forgeos-hub-server` (via `ForgeOSCore`) for driving sessions and a `forgeos-hub-server` (via `HubUIClient`) for the admin view.
...
This starts the Vite webview server on <http://127.0.0.1:5173> and the hub dashboard on <http://127.0.0.1:8787>. Open the dashboard URL; the served page loads webview modules from Vite, so changes under `src/webview/src` hot reload without rebuilding. Use `FORGEOS_HUB_WEBVIEW_DEV_PORT` or `FORGEOS_HUB_WEBVIEW_DEV_HOST` to change the Vite bind address.

To start a brand-new session, the dashboard needs to know which provider and model to use. It picks them up automatically from the most recent session on the hub. If there are no recent sessions, set `FORGEOS_PROVIDER` and `FORGEOS_MODEL` in the environment before running.
...
| `FORGEOS_HUB_DASHBOARD_PORT` | `8787` | Dashboard HTTP/WebSocket port. |
...
| `FORGEOS_PROVIDER` | unset | Fallback provider id when no recent session is available to copy from. |
| `FORGEOS_MODEL` | unset | Fallback model id when no recent session is available to copy from. |
...
cd apps/forgeos-hub
HOST=0.0.0.0 \
FORGEOS_HUB_DASHBOARD_PORT=8787 \
...
1. Detaches the dashboard's `ForgeOSCore` and `HubUIClient` from the current hub.
