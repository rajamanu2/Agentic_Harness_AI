import { CORE_BUILD_VERSION } from "@forgeos/core";
...
export interface ForgeOSHubDashboardServer {
	listenUrl: string;
	publicUrl: string;
	inviteUrl: string;
	bindHost: string;
...
export async function startForgeOSHubDashboardServer(): Promise<ForgeOSHubDashboardServer> {
			...
			async message(socket, raw) {
				...
				try {
					...
					if (frame.type === "desktopCommand") {
						...
						const session = await ctx.forgeos.get(frame.sessionId);
						const metadata =
							session?.metadata && typeof session.metadata === "object"
								? (session.metadata as Record<string, unknown>)
								: {};
						await ctx.forgeos.update(frame.sessionId, {
...
export function printForgeOSHubDashboardServerInfo(
	server: ForgeOSHubDashboardServer,
): void {
	console.log(`ForgeOS Hub dashboard listening: ${server.listenUrl}`);
	console.log(`ForgeOS Hub public URL: ${server.publicUrl}`);
	console.log(`hub endpoint: ${server.hubUrl}`);
	if (server.inviteRequired) {
		console.log(`ForgeOS Hub invite URL: ${server.inviteUrl}`);
...
if (import.meta.main) {
	const server = await startForgeOSHubDashboardServer();
	printForgeOSHubDashboardServerInfo(server);
}
