...
To load this plugin from a file in ForgeOSCore, pass its path in `pluginPaths`:

```typescript
// /absolute/path/to/github.ts
import { type AgentPlugin, createTool } from "@forgeos/sdk"
...
```bash
forgeos plugin install https://github.com/your-org/your-repo/blob/main/plugins/github-plugin.ts
```

Single-file plugins can only import Node builtins and `@forgeos/*`. As soon as you need an npm dependency (`zod`, an HTTP client, etc.) you must ship as a package: a directory with a `package.json` that declares a `forgeos` field for entry points and your runtime `dependencies`. Dependencies under the `@forgeos/` scope are provided by the host runtime -- the installer strips these and runs `npm install` for the rest, so declare any `@forgeos/*` package you import as an optional peer dependency:
...
```bash
forgeos plugin install https://github.com/your-org/forgeos-github-plugin.git
```
...
Each bundled skill follows the same directory format as any other [ForgeOS skill](/customization/skills). When the plugin is installed or loaded through `pluginPaths`, ForgeOS discovers those skills automatically.

See [Plugins](/customization/plugins) for the full manifest format, directory layout, and the [typescript-lsp-plugin](https://github.com/forgeos/typescript-lsp-plugin) for a complete working example.
