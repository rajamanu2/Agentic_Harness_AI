 ...
 * `FORGEOS_CONNECTOR_DETACHED_CHILD` plus one per adapter
 * (`FORGEOS_SLACK_CONNECT_CHILD`, `FORGEOS_TELEGRAM_CONNECT_CHILD`, ...). They are
 * owned by the CLI, so match them by shape rather than importing upward.
 */
const CONNECTOR_CHILD_MARKER_PATTERN =
	/^FORGEOS_(?:CONNECTOR_DETACHED_CHILD|[A-Z0-9]+_CONNECT_CHILD)$/;
...
export function buildConnectorChildEnv(
	...
	delete childEnv[FORGEOS_CONNECTOR_STARTING_INSTANCE_ENV];
