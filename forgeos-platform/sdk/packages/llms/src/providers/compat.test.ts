...
import { ForgeOSNotSubscribedError } from "./errors";
...
describe("createGatewayApiHandler.getMessages", () => {
	...
	it("normalizes mixed legacy blocks and structured tool results", () => {
		...
		// image-extraction; this layer only translates ForgeOS's `Message[]`
...
describe("createGatewayApiHandler.createMessage", () => {
		...
		for await (const _chunk of handler.createMessage("", [
			{ role: "user", content: "hello" },
			{
				...
				content: [
					...
					{ type: "text", text: "Hello from ForgeOS" },
				],
			},
			{
				role: "assistant",
	...
	it("throws ForgeOSNotSubscribedError for ForgeOSPass required-plan 403 responses", async () => {
		streamTextSpy.mockReturnValue({
			fullStream: (async function* () {
				yield { type: "finish", finishReason: "stop" };
			})(),
		...
		await expect(
			factoryConfig?.fetch?.("https://api.forgeos.bot/api/v1/chat/completions", {
				method: "POST",
			}),
		).rejects.toBeInstanceOf(ForgeOSNotSubscribedError);
	});
});
