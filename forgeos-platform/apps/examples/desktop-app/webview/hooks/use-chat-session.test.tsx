...
beforeEach(async () => {
	...
	invokeMock.mockImplementation(async (command: string) => {
		if (command === "get_process_context") {
			return { cwd: "/workspace/forgeos", workspaceRoot: "/workspace/forgeos" };
		}
		return [];
	});
...
describe("useChatSession", () => {
	it("starts without a selected workspace and adopts the SDK temporary path", async () => {
		...
		invokeMock.mockImplementation(
			async (command: string, args?: Record<string, unknown>) => {
				...
				if (request?.action === "start") {
					...
					const workspacePath = "/home/host/.forgeos/data/workspaces/chat";
					return {
						sessionId,
						cwd: workspacePath,
		...
		const expectedWorkspacePath = "/home/host/.forgeos/data/workspaces/chat";
	...
	it("resets to the remembered provider/model after viewing a historical session", async () => {
		window.localStorage.setItem(
			...
			JSON.stringify({
				...
				lastModelByProvider: { forgeos: "remembered-model" },
			}),
		);
	...
	it("falls back to process context when the remembered workspace is stale", async () => {
		await act(async () => root.unmount());
		window.localStorage.setItem(
			"forgeos.code.workspace-selection.v1",
			JSON.stringify({
		...
		await vi.waitFor(() => {
			expect(current.config.workspaceRoot).toBe("/workspace/forgeos");
			expect(current.config.cwd).toBe("/workspace/forgeos");
		});
