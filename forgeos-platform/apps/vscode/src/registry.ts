...
const prefix = name === "claude-dev" ? "forgeos" : name
 ...
 * In production, all commands are registered under "forgeos" for consistency.
 */
const ForgeOSCommands = {
	...
	FixWithForgeOS: prefix + ".fixWithForgeOS",
...
export const ExtensionRegistryInfo = {
	...
	version,
	publisher,
	commands: ForgeOSCommands,
	views: ForgeOSViewIds,
}
...
export interface HostInfo {
	...
	os: string
	/**
	 * The type of the forgeos host environment, e.g. 'VSCode Extension', 'ForgeOS for JetBrains', 'CLI'
	...
	hostVersion?: string
	/**
	 * The version of ForgeOS that the host client is running
	 */
...
export const HostRegistryInfo = {
	init: async (distinctId: string) => {
		...
		const extensionVersion = host.forgeosVersion || ExtensionRegistryInfo.version
		const platform = host.platform || "unknown"
		const os = process.platform || "unknown"
		const ide = host.forgeosType || "unknown"
