...
description: "Create an API key and make your first request to the ForgeOS API in under a minute."
---
...
- A ForgeOS account at [app.forgeos.bot](https://app.forgeos.bot)
- `curl` or any HTTP client (Python, Node.js, etc.)

## Create an API Key

<Steps>
  <Step title="Sign in to app.forgeos.bot">
    Go to [app.forgeos.bot](https://app.forgeos.bot) and sign in with your account.
  </Step>
...
| `402 Payment Required` | Your account has insufficient credits. Add credits at [app.forgeos.bot](https://app.forgeos.bot) |
| Empty response | Make sure `messages` is a non-empty array with at least one user message |
| Connection timeout | Verify your network can reach `api.forgeos.bot`. Check proxy settings if on a corporate network |
...
<CardGroup cols={2}>
  ...
  <Card title="SDK Examples" icon="code" href="/api/sdk-examples">
    Use the API from Python, Node.js, or the ForgeOS CLI.
  </Card>
</CardGroup>
