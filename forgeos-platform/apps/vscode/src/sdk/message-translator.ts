...
//
// Translates SDK session events into ForgeOSMessage[] for webview consumption.
// The webview expects ForgeOSMessage objects with ask/say types; this module
// maps SDK CoreSessionEvent and AgentEvent types to that format.
//
// Key mappings:
// - SDK "chunk" event (agent stream) → ForgeOSMessage say="text" with partial=true
// - SDK "agent_event" content_start (text) → ForgeOSMessage say="text" with partial=true
// - SDK "agent_event" content_start (reasoning) → ForgeOSMessage say="reasoning" with partial=true
// - SDK "agent_event" content_start (tool) → ForgeOSMessage say="tool" with partial=true
//   IMPORTANT: The webview's ChatRow.tsx parses message.text as JSON when
//   say==="tool", expecting ForgeOSSayTool format: {tool, path, content, ...}.
//   We must convert SDK tool names (read_files, editor, run_commands, etc.)
//   and their inputs to this format.
// - SDK "agent_event" content_start (tool: MCP) → ForgeOSMessage say="use_mcp_server" with partial=true
//   MCP tools use serverName__toolName naming convention. The webview renders
//   MCP tool calls via say/ask="use_mcp_server" with ForgeOSAskUseMcpServer JSON.
// - SDK "agent_event" content_end (tool: MCP) → say="use_mcp_server" + say="mcp_server_response"
// - SDK "agent_event" content_end → ForgeOSMessage with partial=false
// - SDK "agent_event" content_start (tool: attempt_completion) → ForgeOSMessage say="completion_result"
// - SDK "agent_event" content_end (tool: attempt_completion) → ForgeOSMessage say="completion_result" (final)
...
// - SDK "agent_event" error → ForgeOSMessage say="error"
// - SDK "agent_event" usage → ForgeOSMessage say="api_req_started" with ForgeOSApiReqInfo JSON
// - SDK "ended" event → finalizes the session

import type { CoreSessionEvent } from "@forgeos/core"
import { PATCH_MARKERS, projectSessionMessagesForDisplay } from "@forgeos/core"
import type { MessageWithMetadata as SdkMessage } from "@forgeos/llms"
import { type AgentEvent, formatDisplayUserInput } from "@forgeos/shared"
import { COMMAND_OUTPUT_STRING } from "@shared/combineCommandSequences"
import type {
	ForgeOSApiReqInfo,
	ForgeOSAskUseMcpServer,
	ForgeOSAskUseSubagents,
	ForgeOSCompactionInfo,
	ForgeOSMessage,
	ForgeOSSay,
	ForgeOSSaySubagentStatus,
	ForgeOSSayTool,
	ForgeOSSubagentUsageInfo,
	SubagentStatusItem,
} from "@shared/ExtensionMessage"
import { Logger } from "@shared/services/Logger"
import * as path from "path"
import { arePathsEqual, getDesktopDir } from "@/utils/path"
import { FORGEOS_FREE_PROMOTION_ENDED_ERROR_CODE, isForgeOSFreePromotionEndedMessage } from "../services/error/ForgeOSError"
...
/**
 * Result of translating a single SDK event into ForgeOSMessages.
 * May produce zero or more messages.
 */
export interface TranslationResult {
	/** Messages produced by this event */
	messages: ForgeOSMessage[]
...
function normalizeUsageEvent(usageEvent: {
	...
	// cache reads/writes included. Classic ForgeOS/webview metrics expect
...
export class MessageTranslatorState {
	...
	/** Build a ForgeOSSaySubagentStatus from the current entries */
	buildSubagentStatus(overallStatus: ForgeOSSaySubagentStatus["status"]): ForgeOSSaySubagentStatus {
...
/**
 * Tools whose ForgeOSSayTool.path is a filesystem path. webFetch/webSearch/
 * useSkill and MCP tools reuse `path` for URLs, queries, and names, so they
 * are deliberately excluded.
 */
const FILESYSTEM_PATH_TOOLS: ReadonlySet<ForgeOSSayTool["tool"]> = new Set([
	"readFile",
	"listFilesTopLevel",
	"listFilesRecursive",
...
/**
 * Relativize a ForgeOSSayTool's filesystem paths against the task cwd before it
 ...
 */
function toDisplaySayTool(sayTool: ForgeOSSayTool, cwd: string | undefined): ForgeOSSayTool {
...
// ---------------------------------------------------------------------------
// SDK tool name → classic ForgeOSSayTool mapping
// ---------------------------------------------------------------------------

/**
 * Map an SDK tool name and its input to a ForgeOSSayTool object that the
 * webview's ChatRow.tsx can render.
 *
 * The webview does `JSON.parse(message.text) as ForgeOSSayTool` when
 * `say === "tool"`, so the text MUST be valid ForgeOSSayTool JSON.
 *
 ...
 *   MCP tools (serverName__toolName)   → (handled before reaching sdkToolToForgeOSSayTool — emitted as say="use_mcp_server")
 */
function sdkToolToForgeOSSayTool(toolName: string, input?: unknown): ForgeOSSayTool {
	...
	switch (toolName) {
		...
		case "replace_in_file": {
			...
			// this the card mislabels a prepend/insert as "ForgeOS wants to create a new file".
		...
		default: {
			...
			const filePath =
				...
				""
			return {
				tool: toolName as ForgeOSSayTool["tool"],
				path: filePath,
			}
		}
	}
...
/**
 * Map a read request's line range onto ForgeOSSayTool fields. An omitted start_line with an
 ...
 */
function readLineRangeFields(read: FileReadRequest | undefined): Pick<ForgeOSSayTool, "readLineStart" | "readLineEnd"> {
...
/**
 * Split a multi-file apply_patch string into one ForgeOSSayTool per file so each
 * "ForgeOS wants to edit this file" row renders only that file's diff (forgeos#9904).
 *
 ...
 */
function splitApplyPatchByFile(patch: string): ForgeOSSayTool[] {
	const lines = patch.split("\n")
	const blocks: { tool: ForgeOSSayTool["tool"]; path: string; lines: string[] }[] = []
	let current: { tool: ForgeOSSayTool["tool"]; path: string; lines: string[] } | undefined
	...
	for (const line of lines) {
		...
		if (marker) {
			...
			const tool: ForgeOSSayTool["tool"] =
	...
	// pathless row can't route to the per-file diff view (forgeos#9904).
...
/**
 * Build a ForgeOSAskUseMcpServer JSON payload for MCP tool calls.
...
function buildMcpToolPayload(mcpInfo: { serverName: string; toolName: string }, input?: unknown): string {
	const parsedInput = parseToolInput(input)
	// Format arguments as a JSON string (matching classic ForgeOSAskUseMcpServer.arguments)
	...
	} satisfies ForgeOSAskUseMcpServer)
}
...
/**
 * Build the ForgeOS approval ask message for an SDK tool approval request.
 ...
 */
export function buildToolApprovalAskMessage(toolName: string, input: unknown, ts: number, cwd?: string): ForgeOSMessage {
	...
	if (toolName === "spawn_agent") {
		...
		return {
			...
			text: JSON.stringify({
				prompts: [taskPrompt],
			} satisfies ForgeOSAskUseSubagents),
			partial: false,
		}
	}
	...
	return {
		...
		type: "ask",
		ask: "tool",
		text: JSON.stringify(toDisplaySayTool(sdkToolToForgeOSSayTool(toolName, input), cwd)),
		partial: false,
	}
}
...
/**
 * Translate an SDK AgentEvent into ForgeOSMessage(s).
 */
/**
 ...
 */
export function parseCompactionNoticeMetadata(metadata: Record<string, unknown> | undefined): ForgeOSCompactionInfo | undefined {
...
export function buildCompactionMessage(info: ForgeOSCompactionInfo, ts: number): ForgeOSMessage {
	return {
		ts,
		type: "say",
		say: "compaction",
...
function translateAgentEvent(event: AgentEvent, state: MessageTranslatorState): ForgeOSMessage[] {
	const messages: ForgeOSMessage[] = []
	...
	switch (event.type) {
			...
			switch (event.contentType) {
				...
				case "tool": {
					...
					if (toolName === "spawn_agent") {
						...
						const approvalPayload: ForgeOSAskUseSubagents = {
							prompts: allPrompts,
						}
						messages.push({
							ts: state.getSpawnAgentPromptsTs(),
							type: "say",
							say: "use_subagents" as ForgeOSSay,
					...
					// with ForgeOSAskUseMcpServer JSON, not generic say="tool".
					const mcpInfo = parseMcpToolName(toolName)
					if (mcpInfo) {
						...
						messages.push({
							ts: state.getStreamingToolTs(),
							type: "say",
							say: "use_mcp_server" as ForgeOSSay,
							text: mcpPayload,
							partial: true,
						})
						break
					...
					// All other tools → say="tool" with ForgeOSSayTool JSON
					...
					// reproduce for files ≥2, orphaning those partial rows (forgeos#9904).
					const sayTool = toDisplaySayTool(sdkToolToForgeOSSayTool(toolName, input), state.currentCwd())
					messages.push({
						ts: state.getStreamingToolTs(),
						type: "say",
						say: "tool",
		...
		case "content_update": {
			...
			// these into the ForgeOSSaySubagentStatus format for the rich UI.
			...
			switch (event.contentType) {
				...
				case "tool": {
					...
					if (toolName === "spawn_agent") {
						...
						const overallStatus: ForgeOSSaySubagentStatus["status"] = allDone
							? hasFailed
								? "failed"
								: "completed"
							: "running"
						...
						if (allDone) {
							const usagePayload: ForgeOSSubagentUsageInfo = {
								source: "subagents",
							...
							messages.push({
								ts: state.nextTs(),
								type: "say",
								say: "subagent_usage" as ForgeOSSay,
					...
					if (mcpInfoEnd) {
						...
						if (mcpOutputStr) {
							messages.push({
								ts: state.nextTs(),
								type: "say",
								say: "mcp_server_response" as ForgeOSSay,
								text: mcpOutputStr,
								partial: false,
							})
						}
					...
					// instead of the whole multi-file patch (forgeos#9904). Single-file
					...
					const sayTool = toDisplaySayTool(sdkToolToForgeOSSayTool(toolName, storedInput), state.currentCwd())
		...
		case "error": {
			...
			// The webview uses ForgeOSError.parse() on the `api_req_failed` text to
			...
			// may contain JSON from the API (e.g. ForgeOS provider's 402 response with
			// `code: "insufficient_credits"`). We try to reshape it into the
			// ForgeOSError-serialized format the webview expects so that ErrorRow
...
export function translateSessionEvent(event: CoreSessionEvent, state: MessageTranslatorState): TranslationResult {
	...
	switch (event.type) {
		...
		case "agent_event": {
			// Sub-agent events should NOT produce ForgeOSMessages in the main chat.
		...
		case "pending_prompts": {
			...
			// to ForgeOSMessages at this layer
			break
		}
...
function agentEventToMessages(event: AgentEvent, state: MessageTranslatorState): ForgeOSMessage[] {
...
function appendPersistedMetricsMessage(
	forgeosMessages: ForgeOSMessage[],
...
function finalizePersistedToolUse(
	...
	// produces the final non-partial ForgeOSMessage shape the webview expects.
...
export interface SdkMessagesToForgeOSMessagesOptions {
	/**
...
/**
 * Convert SDK-persisted LLM messages back into the ForgeOSMessage format used by
 * the webview. Keep this in the live message translator so history rendering
 * and streaming rendering share the same SDK tool → ForgeOS UI mapping.
 */
export function sdkMessagesToForgeOSMessages(
	messages: SdkMessageWithMetrics[],
	minter?: MessageIdMinter,
	options?: SdkMessagesToForgeOSMessagesOptions,
): ForgeOSMessage[] {
	const forgeosMessages: ForgeOSMessage[] = []
	...
	const flushUnmatchedToolUses = () => {
		for (const toolUse of pendingToolUses.values()) {
			forgeosMessages.push(...finalizePersistedToolUse(toolUse, state))
		}
	...
	const upsertForgeOSMessages = (updates: ForgeOSMessage[]) => {
		for (const update of updates) {
			const existingIndex = forgeosMessages.findIndex((m) => m.ts === update.ts)
			if (existingIndex !== -1) {
				forgeosMessages[existingIndex] = update
			} else {
				forgeosMessages.push(update)
			}
		}
	}
	...
	for (const { message, sourceIndex } of projectSessionMessagesForDisplay(messages)) {
		...
		if (message.role === "assistant") {
			...
			if (typeof message.content === "string") {
				...
				appendPersistedMetricsMessage(forgeosMessages, message, state)
				continue
			}
			...
			if (text) {
				...
				if (!isSyntheticSdkUserMessage(message)) {
					forgeosMessages.push({
						ts: state.nextTs(),
						type: "say",
						say: forgeosMessages.length === 0 ? "task" : "user_feedback",
						text,
						partial: false,
					})
				}
		...
		for (const block of message.content) {
			...
			forgeosMessages.push(...finalizePersistedToolUse(toolUse, state, block.content, block.is_error))
		}
	}
...
/**
 * Reshape an SDK error into the serialized ForgeOSError JSON the webview's
...
export function reshapeErrorForWebview(
	...
	// The ForgeOSError-JSON branches below are forgeos-provider flows (balance,
	// spend limit), so "forgeos" stays their fallback id. The missing-credential
	// message instead gets the raw value: defaulting there would name the wrong
	// provider when the active provider id is unknown.
	const forgeosErrorProviderId = providerId ?? "forgeos"
	const rawMessage = error.message ?? "Unknown error"

	// A retired forgeos-free/ model answers "model not found" once its free
	...
	if (isForgeOSFreePromotionEndedMessage(rawMessage, modelId)) {
		return JSON.stringify({
			message: rawMessage,
			code: FORGEOS_FREE_PROMOTION_ENDED_ERROR_CODE,
			providerId: forgeosErrorProviderId,
			modelId,
			details: {
				code: FORGEOS_FREE_PROMOTION_ENDED_ERROR_CODE,
				message: rawMessage,
	...
	if (!parsed) {
		...
		// ForgeOSError-compatible JSON the webview expects.
	...
	// ForgeOSError.getErrorType() to return ForgeOSErrorType.Balance
	...
	// ForgeOSError.parse() can still extract what it can.
