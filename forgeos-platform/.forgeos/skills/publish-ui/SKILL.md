---
name: publish-ui
description: Prepare, validate, and publish standalone @forgeos/ui npm releases. Use when bumping the UI package version, publishing latest or next through ui-publish.yml, checking UI release readiness, or completing the one-time npm trusted-publishing bootstrap.
---

# Publish UI

Release `@forgeos/ui` independently from the ForgeOS SDK runtime packages.
...
npm view @forgeos/ui dist-tags versions --json
...
```sh
bun install --filter @forgeos/ui --filter @forgeos/code --frozen-lockfile
bun -F @forgeos/ui typecheck
bun -F @forgeos/ui test
bun -F @forgeos/ui test:package
bun -F @forgeos/ui build-storybook
bun -F @forgeos/code test:chat-ui
```
...
Use this only while `npm view @forgeos/ui` returns `E404`. npm requires the
...
1. Merge the package and `ui-publish.yml` to `main`. Start from a clean,
   ...
   access to the `@forgeos` npm organization. The `npm trust` command in step 4
