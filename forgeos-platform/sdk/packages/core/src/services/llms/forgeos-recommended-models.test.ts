...
} from "./forgeos-recommended-models";
...
const ENDPOINT_PAYLOAD = {
	...
	free: [
		...
		{ id: "forgeos-free/glm-5.2", name: "forgeos-free/glm-5.2", description: "" },
		{
	...
	forgeosPass: [
		{ id: "forgeos-pass/glm-5.2", name: "forgeos-pass/glm-5.2", description: "" },
		{ id: "forgeos-pass/mystery", name: "forgeos-pass/mystery", description: "" },
	],
};
...
const CATALOG = {
	...
	forgeos: {
		...
		"forgeos-free/glm-5.2": model("forgeos-free/glm-5.2", "GLM-5.2 (free)"),
	...
	"forgeos-pass": {
		"forgeos-pass/glm-5.2": model("forgeos-pass/glm-5.2", "GLM-5.2"),
	},
};

function namesOf(data: ForgeOSRecommendedModelsData) {
	return {
		recommended: data.recommended.map((m) => m.name),
		free: data.free.map((m) => m.name),
		forgeosPass: data.forgeosPass.map((m) => m.name),
	};
}

describe("fetchForgeOSRecommendedModels", () => {
	it("resolves display names from the models catalog", async () => {
		const data = await fetchForgeOSRecommendedModels({
			baseUrl: BASE_URL,
	...
	it("returns the bundled fallback untouched when the endpoint fails", async () => {
		...
		expect(data).toEqual(FALLBACK_FORGEOS_RECOMMENDED_MODELS);
	});
});
