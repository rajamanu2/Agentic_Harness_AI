...
/**
 * All valid hook types that can be created and executed by ForgeOS.
...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "ForgeOS", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".forgeosrules", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".forgeosrules", "hooks")
}
