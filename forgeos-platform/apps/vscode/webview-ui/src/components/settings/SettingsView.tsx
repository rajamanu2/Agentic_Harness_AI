...
import { ResetStateRequest } from "@shared/proto/forgeos/state"
import type { UserOrganization } from "@shared/proto/index.forgeos"
import {
	CheckCheck,
	FlaskConical,
	HardDriveDownload,
...
import { type ForgeOSUser, useForgeOSAuth } from "@/context/ForgeOSAuthContext"
...
interface SettingsTab {
	...
	headerText: string
	icon: LucideIcon
	hidden?: (params?: { user: ForgeOSUser | null; activeOrganization: UserOrganization | null }) => boolean
}
...
const SETTINGS_TABS: SettingsTab[] = [
	...
	// Only show in dev mode
	{
		...
		headerText: "Debug",
		icon: FlaskConical,
		hidden: ({ user } = { user: null, activeOrganization: null }) => !IS_DEV && !isForgeOSInternalTester(user?.email || ""),
	},
]
...
const SettingsView = ({ onDone, targetSection }: SettingsViewProps) => {
	...
	const { activeOrganization, forgeosUser } = useForgeOSAuth()
		...
		<Tab>
			...
			<div className="flex flex-1 overflow-hidden">
				<TabList
					...
					value={activeTab}>
					{SETTINGS_TABS.filter((tab) => !tab.hidden?.({ user: forgeosUser, activeOrganization })).map(renderTabItem)}
				</TabList>
