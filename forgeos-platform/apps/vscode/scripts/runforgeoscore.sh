...
# This installs the forgeos-core app to the user's home directory,
...
CORE_DIR=~/.forgeos/core
INSTALL_DIR=$CORE_DIR/dev-instance/
LOG_FILE=~/.forgeos/forgeos-core-service.log
...
pkill -f forgeos-core.js || true

# Detect platform name using the same logic as ForgeOSDirs.kt in the plugin.
...
echo pwd: $(pwd)
set -x
NODE_PATH=$BINARY_MODULES_DIR:./node_modules DEV_WORKSPACE_FOLDER=/tmp/ node --max-old-space-size=${NODE_MAX_OLD_SPACE_SIZE:-8192} --heapsnapshot-near-heap-limit=1 forgeos-core.js 2>&1 | tee $LOG_FILE
