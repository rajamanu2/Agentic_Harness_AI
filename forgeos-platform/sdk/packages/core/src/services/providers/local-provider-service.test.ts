...
describe("addLocalProvider – model ID parsing via modelsSourceUrl", () => {
	...
	it("uses only live ForgeOSPass models when live models are found", async () => {
		const fetchMock = vi.fn(async (url: string) => {
			...
			return new Response(
				JSON.stringify({
					forgeosPass: [
						{
							id: "forgeos-pass/live-pass-model",
							name: "vendor/live-pass-model",
						},
					],
					free: [{ id: "forgeos-free/live-free-model" }],
				}),
				{
					status: 200,
					headers: { "content-type": "application/json" },
		...
		const { models } = await getLocalProviderModels("forgeos-pass");

		expect(fetchMock).toHaveBeenCalledTimes(2);
		expect(models.map((model) => model.id)).toEqual(
			expect.arrayContaining([
				"forgeos-pass/live-pass-model",
				"forgeos-free/live-free-model",
			]),
		);
		expect(
			models.find((model) => model.id === "forgeos-pass/live-pass-model"),
		).toMatchObject({
			id: "forgeos-pass/live-pass-model",
			name: "Live Pass Model",
		...
		expect(
			models.find((model) => model.id === "forgeos-free/live-free-model"),
		).toMatchObject({
			id: "forgeos-free/live-free-model",
			name: "Live Free Model (free)",
			supportsReasoning: true,
		});
	});

	it("falls back to generated ForgeOSPass models when no live ForgeOSPass models are found", async () => {
		const fetchMock = vi.fn(async (url: string) => {
			if (url === "https://models.dev/api.json") {
				return new Response(
					JSON.stringify({
			...
			return new Response(JSON.stringify({ forgeosPass: [] }), {
				status: 200,
				headers: { "content-type": "application/json" },
			});
		});
...
describe("models.json model overlays", () => {
	it("loads model-only entries for built-in providers", async () => {
		...
		try {
			...
			const provider = await LlmsModels.getProvider("forgeos");
			expect(provider).toMatchObject({
				id: "forgeos",
				baseUrl: "https://api.forgeos.bot/api/v1",
				defaultModelId: FORGEOS_DEFAULT_MODEL_ID,
			});

			const { models } = await getLocalProviderModels("forgeos");
			expect(
...
describe("listLocalProviders", () => {
	...
	it("hides ForgeOSPass when the ForgeOSPass feature flag is disabled", async () => {
		const { providers } = await listLocalProviders(manager, {
			isForgeOSPassEnabled: false,
		});

		expect(providers.map((p) => p.id)).not.toContain("forgeos-pass");
	});

	it("includes ForgeOSPass when the ForgeOSPass feature flag is enabled", async () => {
		const { providers } = await listLocalProviders(manager, {
			isForgeOSPassEnabled: true,
		});

		expect(providers.map((p) => p.id)).toContain("forgeos-pass");
	});
	...
	it("marks alias providers enabled without copying shared OAuth credentials", async () => {
		...
		markLocalProviderEnabled(manager, "forgeos-pass", { tokenSource: "oauth" });

		const state = manager.read();
		expect(state.providers["forgeos-pass"]?.settings).toEqual({
			provider: "forgeos-pass",
		});
		expect(state.providers["forgeos-pass"]?.tokenSource).toBe("oauth");
	});

	it("resolves shared OAuth metadata for ForgeOSPass catalog entries", async () => {
		manager.saveProviderSettings(
			{
				provider: "forgeos",
				auth: {
		...
		const forgeosPass = providers.find(
			(provider) => provider.id === "forgeos-pass",
		);

		expect(forgeosPass).toMatchObject({
			enabled: true,
	...
	it("uses ForgeOS-specific Z.ai aliases in the built-in model list", async () => {
		manager.saveProviderSettings(
			{
				provider: "forgeos",
				apiKey: "test-key",
		...
		const { providers } = await listLocalProviders(manager);
		const forgeos = providers.find((provider) => provider.id === "forgeos");
		const openrouter = providers.find(
			(provider) => provider.id === "openrouter",
		);
		const forgeosModelIds = new Set(
			forgeos?.modelList?.map((model) => model.id) ?? [],
		);
		const openrouterModelIds = new Set(
			openrouter?.modelList?.map((model) => model.id) ?? [],
		);

		expect(forgeos?.modelList?.length).toBeGreaterThan(0);
		expect(forgeosModelIds).toContain("zai/glm-5.2");
		expect(forgeosModelIds).not.toContain("z-ai/glm-5.2");
...
describe("normalizeOAuthProvider", () => {
	it("normalizes 'forgeos' to 'forgeos'", () => {
		expect(normalizeOAuthProvider("forgeos")).toBe("forgeos");
		expect(normalizeOAuthProvider("  FORGEOS  ")).toBe("forgeos");
	});
...
describe("resolveLocalForgeOSAuthToken", () => {
	it("returns undefined when settings is undefined", () => {
		expect(resolveLocalForgeOSAuthToken(undefined)).toBeUndefined();
	});

	it("returns accessToken when present", () => {
		expect(
			resolveLocalForgeOSAuthToken({
				provider: "forgeos" as never,
	...
	it("returns undefined when both fields are absent", () => {
		expect(
			resolveLocalForgeOSAuthToken({ provider: "forgeos" as never }),
		).toBeUndefined();
	});
});
