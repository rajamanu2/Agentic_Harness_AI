...
> {
			...
			super
				...
				.addHelpText(
					...
					[
						...
						"  - Use --allowed-user-id or `forgeos connect` to restrict Telegram access.",
	...
	protected override async runWithOptions(
		...
		if (
			...
			process.env.FORGEOS_TELEGRAM_CONNECT_CHILD !== "1"
		) {
		...
		const backgroundExitCode = await this.maybeRunInBackground({
			...
			childEnvVar: "FORGEOS_TELEGRAM_CONNECT_CHILD",
			statePath,
			readState: (path) => this.readConnectorState(path),
			...
			foregroundHint:
				"[telegram] use `forgeos connect telegram -i ...` to run in the foreground",
