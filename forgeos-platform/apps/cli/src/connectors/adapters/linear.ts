...
import type { ConnectLinearOptions, LinearConnectorState } from "@forgeos/shared";
...
> {
	...
	protected override async runWithOptions(
		...
		const backgroundExitCode = await this.maybeRunInBackground({
			...
			childEnvVar: "FORGEOS_LINEAR_CONNECT_CHILD",
			statePath,
			readState: (path) => this.readConnectorState(path),
			...
			foregroundHint:
				"[linear] use `forgeos connect linear -i ...` to run in the foreground",
