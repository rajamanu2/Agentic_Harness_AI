...
// Minimal ChatState stub. forgeosAsk/lastMessage are the only derived values the send path reads.
function makeChatState(messages: ForgeOSMessage[], overrides: Partial<ChatState> = {}): ChatState {
	...
	const state = {
		...
		forgeosAsk: last?.type === "ask" ? last.ask : undefined,
		task: messages.at(0),
...
const completedConversation: ForgeOSMessage[] = [
...
describe("useMessageHandlers — send routing", () => {
	...
	it("after a completed turn (no forgeosAsk), Enter continues the conversation via askResponse — NOT newTask", async () => {
		mockTurnState = { phase: "completed", seq: 7 }
		const { result } = renderHook(() => useMessageHandlers(completedConversation, makeChatState(completedConversation)))
	...
	it("shows a pending chat bubble immediately when sending a message to a task resumed from history", async () => {
		...
		const historyConversation: ForgeOSMessage[] = [
	...
	it("does not show a pending chat bubble for a streaming follow-up that will be queued", async () => {
		...
		const streamingConversation: ForgeOSMessage[] = [
			{ ts: 1, type: "say", say: "task", text: "task" },
	...
	it("rejects a pending approval when the composer is submitted with typed feedback", async () => {
		...
		const approvalConversation: ForgeOSMessage[] = [
			{ ts: 1, type: "say", say: "task", text: "task" },
	...
	it("does not show a pending chat bubble when answering an active follow-up question with freeform text", async () => {
		...
		const questionConversation: ForgeOSMessage[] = [
			{ ts: 1, type: "say", say: "task", text: "task" },
			{
				ts: 2,
				type: "ask",
	...
	// (emitForgeOSAuthError for the ForgeOS provider, say:"error" otherwise).
