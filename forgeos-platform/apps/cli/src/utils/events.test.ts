...
import type { AgentEvent, TeamEvent } from "@forgeos/core";
...
describe("handleEvent text formatting", () => {
	...
	it("formats ForgeOSPass limit agent errors before writing to stderr", () => {
		handleEvent(
			{
				type: "error",
				error: new Error(
		...
		expect(errorOutput).toContain("ForgeOSPass limit reached");
		expect(errorOutput).toContain("Switch to ForgeOS usage-based billing");
		expect(errorOutput).toContain("--provider forgeos");
	});
	...
	it("formats removed free model errors using the configured model id", () => {
		handleEvent(
			...
			{ modelId: "forgeos-free/retired-model" } as Config,
		);
