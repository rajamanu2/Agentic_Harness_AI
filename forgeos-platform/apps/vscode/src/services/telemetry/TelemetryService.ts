...
import { ForgeOSAccountUserInfo } from "@/services/auth/AuthService"
...
export type TelemetryMetadata = {
	/**
	 * The extension or forgeos-core version. JetBrains and CLI have different
	 * versioning than the VSCode Extension, but on those platforms this will be the _forgeos-core version_
	 * which uses the same as the versioning as the VSCode extension.
	 */
	extension_version: string
	/**
	 * The type of forgeos distribution, e.g VSCode Extension, JetBrains Plugin or CLI. This
	...
	forgeos_type: string
	/**
	 * The version of the host-side ForgeOS distribution package: the JetBrains plugin version
...
/**
 * TelemetryService handles telemetry event tracking for the ForgeOS extension
...
export class TelemetryService {
	...
	public static readonly METRICS = {
		TASK: {
			TURNS_TOTAL: "forgeos.turns.total",
			TURNS_PER_TASK: "forgeos.turns.per_task",
			TOKENS_INPUT_TOTAL: "forgeos.tokens.input.total",
			TOKENS_INPUT_PER_RESPONSE: "forgeos.tokens.input.per_response",
			TOKENS_OUTPUT_TOTAL: "forgeos.tokens.output.total",
			TOKENS_OUTPUT_PER_RESPONSE: "forgeos.tokens.output.per_response",
			COST_TOTAL: "forgeos.cost.total",
			COST_PER_EVENT: "forgeos.cost.per_event",
		},
		CACHE: {
			WRITE_TOTAL: "forgeos.cache.write.tokens.total",
			WRITE_PER_EVENT: "forgeos.cache.write.tokens.per_event",
			READ_TOTAL: "forgeos.cache.read.tokens.total",
			READ_PER_EVENT: "forgeos.cache.read.tokens.per_event",
			HITS_TOTAL: "forgeos.cache.hits.total",
		},
		TOOLS: {
			CALLS_TOTAL: "forgeos.tool.calls.total",
			CALLS_PER_TASK: "forgeos.tool.calls.per_task",
		},
		ERRORS: {
			TOTAL: "forgeos.errors.total",
			PER_TASK: "forgeos.errors.per_task",
		},
		API: {
			TTFT_SECONDS: "forgeos.api.ttft.seconds",
			DURATION_SECONDS: "forgeos.api.duration.seconds",
			THROUGHPUT_TOKENS_PER_SECOND: "forgeos.api.throughput.tokens_per_second",
		},
		HOOKS: {
			EXECUTIONS_TOTAL: "forgeos.hooks.executions.total",
			DURATION_SECONDS: "forgeos.hooks.duration.seconds",
			FAILURES_TOTAL: "forgeos.hooks.failures.total",
			CANCELLATIONS_TOTAL: "forgeos.hooks.cancellations.total",
			CONTEXT_MODIFICATIONS_TOTAL: "forgeos.hooks.context_modifications.total",
			CACHE_ACCESSES_TOTAL: "forgeos.hooks.cache.accesses.total",
		},
		AI_OUTPUT: {
			ACCEPTED_LINES_ADDED: "forgeos.ai_output.accepted.lines_added.total",
			ACCEPTED_LINES_DELETED: "forgeos.ai_output.accepted.lines_deleted.total",
			ACCEPTED_LINES_CHANGED: "forgeos.ai_output.accepted.lines_changed.total",
			ACCEPTED_FILES_CREATED: "forgeos.ai_output.accepted.files_created.total",
			ACCEPTED_FILES_DELETED: "forgeos.ai_output.accepted.files_deleted.total",
			ACCEPTED_FILES_MOVED: "forgeos.ai_output.accepted.files_moved.total",
			REJECTED_LINES_ADDED: "forgeos.ai_output.rejected.lines_added.total",
			REJECTED_LINES_DELETED: "forgeos.ai_output.rejected.lines_deleted.total",
			REJECTED_LINES_CHANGED: "forgeos.ai_output.rejected.lines_changed.total",
			REJECTED_FILES_CREATED: "forgeos.ai_output.rejected.files_created.total",
			REJECTED_FILES_DELETED: "forgeos.ai_output.rejected.files_deleted.total",
			REJECTED_FILES_MOVED: "forgeos.ai_output.rejected.files_moved.total",
		},
		GRPC: {
			RESPONSE_SIZE_BYTES: "forgeos.grpc.response.size_bytes",
		},
		MIGRATION: {
			// Fires whenever ForgeOS checks an old pre-SDK task and decides whether/how to migrate it.
			LEGACY_TASK_ATTEMPTS_TOTAL: "forgeos.migration.legacy_task.attempts.total",
			// Fires when the user opens an old task and ForgeOS successfully copies it into SDK session storage.
			LEGACY_TASK_SUCCESS_TOTAL: "forgeos.migration.legacy_task.success.total",
			// Fires when ForgeOS tried to migrate an old task but failed while building or writing the SDK session.
			LEGACY_TASK_FAILURES_TOTAL: "forgeos.migration.legacy_task.failures.total",
			// Fires when no migration happens because it is unnecessary or impossible, e.g. already migrated or missing old messages.
			LEGACY_TASK_SKIPPED_TOTAL: "forgeos.migration.legacy_task.skipped.total",
			// Fires for every migration decision; measures how long the check/migration took.
			LEGACY_TASK_DURATION_SECONDS: "forgeos.migration.legacy_task.duration.seconds",
			// Fires when ForgeOS finds old conversation messages; records how many old messages were found.
			LEGACY_TASK_LEGACY_MESSAGES_COUNT: "forgeos.migration.legacy_task.legacy_messages.count",
			// Fires after conversion; records how many messages made it into SDK-compatible form.
			LEGACY_TASK_CONVERTED_MESSAGES_COUNT: "forgeos.migration.legacy_task.converted_messages.count",
			// Fires when history is listed; counts old pre-SDK tasks still waiting to be migrated.
			LEGACY_TASK_PENDING_COUNT: "forgeos.migration.legacy_task.pending.count",
			// Fires when history is listed; counts old tasks that already made it safely into SDK session storage.
			LEGACY_TASK_MIGRATED_COUNT: "forgeos.migration.legacy_task.migrated.count",
		},
	}
	...
	private static readonly EVENTS = {
		...
		TASK: {
			...
			// Tracks when individual ForgeOS rules are toggled on/off
		...
		UI: {
			...
			// Tracks when the ForgeOS panel becomes visible
	...
	public static async create(): Promise<TelemetryService> {
		...
		const metadata: TelemetryMetadata = {
			...
			...(hostVersion.forgeosVersion ? { host_plugin_version: hostVersion.forgeosVersion } : {}),
			platform: hostVersion.platform || "unknown",
			platform_version: hostVersion.version || "unknown",
			forgeos_type: hostVersion.forgeosType || "unknown",
			os_type: os.platform(),
	...
	public async updateTelemetryState(didUserOptIn: boolean): Promise<void> {
		...
		if (hostSetting.isEnabled === Setting.DISABLED) {
			// Only show warning if user has opted in to ForgeOS telemetry but host telemetry is disabled
			if (didUserOptIn) {
				void HostProvider.window
					.showMessage({
						type: ShowMessageType.WARNING,
						message:
							"Anonymous ForgeOS error and usage reporting is enabled, but IDE telemetry is disabled. To enable error and usage reporting for this extension, enable telemetry in IDE settings.",
						options: {
							items: ["Open Settings"],
						},
					})
	 ...
	 * checkpoints, focus chain, legacy-task migration), so @forgeos/core cannot
	 * emit them and the missing piece is a call site here. Tracked in
	 * https://linear.app/forgeos-bot/issue/ENG-2401
	 *
	 * Anything whose event @forgeos/core already emits was deleted instead: a
	 * second capture path for a core-owned event is how the
	 * task.provider_api_error double-emission happened (forgeos/forgeos#12820).
	 *
	 ...
	 */
	public identifyAccount(userInfo: ForgeOSAccountUserInfo) {
		const propertiesWithMetadata: TelemetryProperties = {
			...this.telemetryMetadata,
		}
	...
	/**
	 * Records when individual ForgeOS rules are toggled on/off
	 ...
	 */
	public captureForgeOSRuleToggled(ulid: string, ruleFileName: string, enabled: boolean, isGlobal: boolean) {
