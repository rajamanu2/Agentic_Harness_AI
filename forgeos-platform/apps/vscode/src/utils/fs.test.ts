...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "forgeos-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .forgeosrules/workflows directory specifically", async () => {
		// Create a test directory structure
		const forgeosrulesDirTest = path.join(tmpDir, "forgeosrules-test")
		const forgeosrulesDirPath = path.join(forgeosrulesDirTest, ".forgeosrules")

		// Create .forgeosrules directory and root files
		await fs.mkdir(forgeosrulesDirPath, { recursive: true })
		await fs.writeFile(path.join(forgeosrulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(forgeosrulesDirPath, "settings.js"), "// settings")

		// Create .forgeosrules/other directory and files
		const otherDirPath = path.join(forgeosrulesDirPath, "other")
		...
		// Create .forgeosrules/workflows directory and files
		const workflowsDirPath = path.join(forgeosrulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(forgeosrulesDirPath)
		...
		const filteredFiles = await readDirectory(forgeosrulesDirPath, [[".forgeosrules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(forgeosrulesDirPath, "config.json"),
			path.resolve(forgeosrulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(forgeosrulesDirPath, [
		...
		const rootOnlyFiles = [path.resolve(forgeosrulesDirPath, "config.json"), path.resolve(forgeosrulesDirPath, "settings.js")]

		multiExcludeFiles.sort().should.deepEqual(rootOnlyFiles.sort())
	})

	it("should exclude .forgeosrules/hooks directory specifically", async () => {
		// Create a test directory structure
		const forgeosrulesDirTest = path.join(tmpDir, "forgeosrules-hooks-test")
		const forgeosrulesDirPath = path.join(forgeosrulesDirTest, ".forgeosrules")
		...
		// Create .forgeosrules/hooks directory and files
		const hooksDirPath = path.join(forgeosrulesDirPath, "hooks")
		...
		const filteredFiles = await readDirectory(forgeosrulesDirPath, [[".forgeosrules", "hooks"]])
