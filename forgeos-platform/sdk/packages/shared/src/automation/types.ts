 ...
 *
 * - `one_off`: a single run materialized from a `.forgeos/cron/*.md` spec.
 * - `schedule`: recurring runs materialized from a `.forgeos/cron/*.cron.md` spec.
 * - `event`: reactive runs materialized from a `.forgeos/cron/events/*.event.md`
...
export interface AutomationSpecSource {
	/**
	 * POSIX-normalized path of the source file relative to `.forgeos/cron/`.
