...
// fake "Conversation Summary" instead of compacting (FORGEOS-2503).

import type { Message as SdkMessage } from "@forgeos/llms"
import type { ForgeOSCompactionInfo, ForgeOSMessage } from "@shared/ExtensionMessage"
...
const COMPACTION_UNSUPPORTED_MESSAGE = "Compaction is not supported by this runtime yet. Please update ForgeOS and try again."
...
export class SdkCompactionCoordinator {
	...
	private async runCompaction(sdkHost: SdkSessionHost, sessionId: string): Promise<void> {
		...
		let noticeInfo: ForgeOSCompactionInfo | undefined
		try {
	...
	private emitCompactionRow(info: ForgeOSCompactionInfo, ts: number, sessionId: string): void {
	...
	private emitInfo(text: string, sessionId?: string): void {
		...
		const infoMessage: ForgeOSMessage = {
			ts: Date.now(),
			type: "say",
			say: "info",
			text,
