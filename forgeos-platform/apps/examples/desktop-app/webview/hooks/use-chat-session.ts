...
// unless the user opts in via `localStorage.setItem("forgeos:debug-logs", "1")`.
let verboseCoreLogs: boolean | undefined;

function shouldLogVerboseCoreLogs(): boolean {
	if (verboseCoreLogs === undefined) {
		try {
			verboseCoreLogs = window.localStorage.getItem("forgeos:debug-logs") === "1";
		} catch {
		...
		(sid: string, detail: string) => {
			...
			const content = [
				...
				looksCredentialRelated
					? "Check your model connection in Settings → Models (or sign in with ForgeOS), then try again."
					: "",
			]
				.filter(Boolean)
				.join(" ");
