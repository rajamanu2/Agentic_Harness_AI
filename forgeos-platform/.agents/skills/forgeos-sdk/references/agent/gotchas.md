...
`@forgeos/agents` (and by extension, the `Agent` class) is browser-safe with no Node.js dependencies. However, `@forgeos/core` and `ForgeOSCore` require Node.js 22+. If you import from `@forgeos/sdk`, you get everything including the Node-only code. For browser usage, import directly from `@forgeos/agents`:
...
- Using `ForgeOSCore` with compaction for long sessions
...
- `../forgeoscore/REFERENCE.md` - Use ForgeOSCore for persistence
