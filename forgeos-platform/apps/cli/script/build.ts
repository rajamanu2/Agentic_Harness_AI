...
console.log(`Building @forgeos/cli v${version}`);
...
if (!buildOptions.skipSdkBuild) {
	...
	await $`bun -F @forgeos/cli build`.cwd(rootDir);
}

const hubWebviewSource = join(cliDir, "../forgeos-hub/src/webview");
const hubWebviewDist = join(cliDir, "../forgeos-hub/dist/webview");
...
if (shouldBuildHubWebview()) {
	console.log("Building ForgeOS Hub webview...");
	await $`bun -F @forgeos/forgeos-hub build:webview`.cwd(rootDir);
}
...
async function buildCompiledBinary(input: {
	...
	const tmpDir = join("/tmp", `forgeos-build-${input.dirName}`);
	const tmpOutfile = join(
		tmpDir,
		input.outfile.endsWith(".exe") ? "forgeos.exe" : "forgeos",
	);
...
for (const item of targets) {
	...
	const name = `@forgeos/cli-${displayOs}-${item.arch}`;
	const dirName = `cli-${displayOs}-${item.arch}`;
	const binaryName = item.os === "win32" ? "forgeos.exe" : "forgeos";
	...
	if (existsSync(hubWebviewDist)) {
		const hubWebviewDest = join(cliDir, `dist/${dirName}/forgeos-hub/webview`);
		mkdirSync(join(cliDir, `dist/${dirName}/forgeos-hub`), {
			recursive: true,
		});
		...
		`${JSON.stringify(
			{
				name,
				version,
				description: `ForgeOS CLI binary for ${displayOs} ${item.arch}`,
				os: [item.os],
				cpu: [item.arch],
				...(repository ? { repository } : {}),
				bin: {
					forgeos: `bin/${binaryName}`,
				},
			},
			null,
			2,
