...
export const FORGEOS_PLUGIN_IDLE_TIMEOUT_MS_ENV = "FORGEOS_PLUGIN_IDLE_TIMEOUT_MS";
...
export interface PluginSandboxOptions extends PluginTargeting {
	 ...
	 * to the `FORGEOS_PLUGIN_IMPORT_TIMEOUT_MS` env var when this option is not
	 ...
	 * `FORGEOS_PLUGIN_IDLE_TIMEOUT_MS`.
	 */
...
function getPlatformPackageName(): string {
	...
	return `@forgeos/cli-${platform}-${process.arch}`;
}
...
> {
	...
	const importTimeoutMs = withTimeoutFallback(
		...
		4000,
		"FORGEOS_PLUGIN_IMPORT_TIMEOUT_MS",
	);
	...
	const extensions: NonNullable<AgentConfig["extensions"]> = descriptors.map(
		(descriptor) => {
			const extension: SandboxedAgentExtension = {
				...
				__forgeosPluginPath: descriptor.pluginPath,
