...
afterEach(() => {
	...
	delete document.documentElement.dataset.forgeosAccent;
	delete document.documentElement.dataset.forgeosHubTheme;
...
describe("hub theme", () => {
	it("applies a saved theme before the system preference", () => {
		...
		expect(document.documentElement.dataset.forgeosHubTheme).toBe("dark");
	});
	...
	it("applies the system preference before the first paint when unsaved", () => {
		...
		expect(document.documentElement.dataset.forgeosHubTheme).toBe("light");
	});
...
describe("hub accent", () => {
	...
	it("round-trips through storage and the html dataset", () => {
		...
		expect(document.documentElement.dataset.forgeosAccent).toBe("graphite");
	...
	it("clears the dataset attribute for the default accent", () => {
		...
		expect(document.documentElement.dataset.forgeosAccent).toBe("ember");
		applyHubAccent(DEFAULT_HUB_ACCENT);
		expect(document.documentElement.dataset.forgeosAccent).toBeUndefined();
	});
});
