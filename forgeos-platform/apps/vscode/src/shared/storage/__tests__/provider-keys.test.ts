...
import { getProviderCollectionSync } from "@forgeos/llms"
...
describe("Provider key mapping", () => {
	...
	it("returns the openrouter default for the openrouter-routed providers", () => {
		...
		expect(getProviderDefaultModelId("forgeos")).to.equal(openrouterDefault)
	...
	it("uses provider-specific model key behavior for ForgeOS", () => {
		expect(getProviderModelIdKey("forgeos", "act")).to.equal("actModeForgeOSModelId")
		expect(getProviderModelIdKey("forgeos", "plan")).to.equal("planModeForgeOSModelId")
	})

	it("uses separate model keys for ForgeOSPass", () => {
		expect(getProviderModelIdKey("forgeos-pass", "act")).to.equal("actModeForgeOSPassModelId")
		expect(getProviderModelIdKey("forgeos-pass", "plan")).to.equal("planModeForgeOSPassModelId")
	})
