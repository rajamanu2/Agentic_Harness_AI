import type { ForgeOSCoreStartInput, ITelemetryService } from "@forgeos/core"
import { beforeEach, describe, expect, it, vi } from "vitest"

const mockForgeOSCoreCreate = vi.hoisted(() => vi.fn())
const mockCreateVscodeExtraTools = vi.hoisted(() => vi.fn(async () => []))

vi.mock("@forgeos/core", async () => {
	const actual = await vi.importActual<typeof import("@forgeos/core")>("@forgeos/core")
	return {
		...actual,
		ForgeOSCore: {
			create: mockForgeOSCoreCreate,
		},
	}
})
...
describe("VscodeSessionHost telemetry wiring", () => {
	beforeEach(() => {
		mockForgeOSCoreCreate.mockReset()
		mockForgeOSCoreCreate.mockResolvedValue({ runtimeAddress: undefined })
		mockCreateVscodeExtraTools.mockReset().mockResolvedValue([])
	})

	it("passes shared telemetry to ForgeOSCore.create", async () => {
		...
		expect(mockForgeOSCoreCreate).toHaveBeenCalledWith(expect.objectContaining({ telemetry }))
	})
	...
	it("injects shared telemetry into CoreSessionConfig when remote config did not provide one", async () => {
		...
		const prepare = mockForgeOSCoreCreate.mock.calls[0][0].prepare
	...
	it("preserves telemetry already supplied by remote config", async () => {
		...
		await VscodeSessionHost.create({
			...
			getRemoteConfigIntegration: () =>
				({
					applyToStartSessionInput: (input: ForgeOSCoreStartInput) => ({
						...input,
						config: {
							...input.config,
	...
	it("passes custom editor and apply_patch executors into tool executor capabilities", async () => {
		...
		const capabilities = mockForgeOSCoreCreate.mock.calls[0][0].capabilities
	...
	it("applies remote config before appending VS Code extra tools", async () => {
		...
		const applyToStartSessionInput = vi.fn(async (input: ForgeOSCoreStartInput) => ({
			...input,
			config: {
				...input.config,
