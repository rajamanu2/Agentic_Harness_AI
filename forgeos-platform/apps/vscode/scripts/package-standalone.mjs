...
async function installNodeDependencies() {
	...
	// directory (dist-standalone), driven by the "forgeos-core" runtime-files
 ...
 * When forgeos-core is installed, the installer should use the correct module for the current platform.
 */
async function packageAllBinaryDeps() {
	...
	for (const module of SUPPORTED_BINARY_MODULES) {
		...
		if (!fs.existsSync(src)) {
			console.warn(`Warning: Trying to install binaries for the module '${module}', but it is not being used by forgeos.`)
			continue
		}
		...
		for (const { platform, arch, targetDir } of TARGET_PLATFORMS) {
			...
			// `.node` binaries load in the Node that runs forgeos-core.
...
async function zipDistribution() {
	...
	// Add the whole forgeos directory under "extension", except the for the ignored files.
