...
const {
	...
	mockSpawnSync: vi.fn(),
	mockResolveForgeOSDataDir: vi.fn(() => "/tmp/forgeos-data"),
...
vi.mock("@forgeos/core", () => ({
	resolveForgeOSDataDir: mockResolveForgeOSDataDir,
...
describe("runDoctorCommand", () => {
	...
	afterEach(() => {
		vi.clearAllMocks();
		mockResolveForgeOSDataDir.mockReturnValue("/tmp/forgeos-data");
	...
	it("sees the hub through the set-aside record during the shielded update window", async () => {
		...
		mockSpawnSync.mockImplementation((command: string, args?: string[]) => {
			...
			if (
				command === "pgrep" &&
				Array.isArray(args) &&
				args[2] === "--forgeos-hub-daemon"
			) {
				return {
					status: 0,
					stdout: "50174 /usr/local/bin/forgeos --forgeos-hub-daemon\n",
				};
			}
			return { status: 1, stdout: "" };
		});
...
describe("createDoctorCommand log subcommand", () => {
	...
	it("opens the log file for doctor log", async () => {
		...
		mockResolveForgeOSDataDir.mockReturnValue(dataDir);
...
describe("doctor supervision reporting", () => {
	...
	it("reports what the hub is supervising", async () => {
		...
		await expect(runDoctorJson()).resolves.toMatchObject({
			supervisedConnectors: [
				{ channel: "slack", instanceId: "forgeos-slack", state: "backoff" },
			],
		});
	});
	...
	it("formats restart and failure state so a crash loop is visible", () => {
		expect(
			...
			"slack | instance=forgeos-slack | state=failed | origin=adopted | pid=42 | restarts=5 | lastExit=1 | error=invalid token",
		);
	});
	...
	it("leaves out fields that do not apply to a healthy connector", () => {
		expect(
			...
			"telegram | instance=forgeos_bot | state=running | origin=spawned | pid=7",
		);
	});
});
