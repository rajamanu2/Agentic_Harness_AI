export const HUB_THEME_STORAGE_KEY = "forgeos-hub-theme";
...
export const HUB_THEME_BOOTSTRAP_SCRIPT = `(() => {
	...
	root.dataset.forgeosHubTheme = theme;
})();`;
...
export function applyHubTheme(theme: HubTheme): HubTheme {
	...
	document.documentElement.dataset.forgeosHubTheme = theme;
	return theme;
}
...
export const HUB_ACCENT_STORAGE_KEY = "forgeos.code.accent.v1";

/**
 * Accent palettes selectable in Settings. "violet" is the built-in brand
 * accent from @forgeos/ui tokens; the others override the interactive tokens
 * via `[data-forgeos-accent]` blocks in globals.css.
 */
...
export function applyHubAccent(accent: HubAccent): HubAccent {
	if (accent === DEFAULT_HUB_ACCENT) {
		...
		document.documentElement.dataset.forgeosAccent = accent;
	}
	return accent;
}
