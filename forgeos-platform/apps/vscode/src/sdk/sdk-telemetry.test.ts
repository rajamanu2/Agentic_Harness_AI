import type { ConfiguredTelemetryHandle, ITelemetryService } from "@forgeos/core"
...
vi.mock("@forgeos/core", () => ({
	createForgeOSTelemetryServiceConfig: coreTelemetryMocks.createConfig,
	createConfiguredTelemetryHandle: coreTelemetryMocks.createHandle,
}))

const telemetryState = vi.hoisted(() => ({
	forgeosTelemetrySetting: "unset" as string | undefined,
	hostSetting: 1,
	hostVersion: {
		platform: "VS Code",
		version: "1.103.0",
		forgeosType: "VSCode Extension",
	} as { platform?: string; version?: string; forgeosType?: string; forgeosVersion?: string },
	...
	StateManager: {
		get: () => ({
			getGlobalSettingsKey: (key: string) =>
				key === "telemetrySetting" ? telemetryState.forgeosTelemetrySetting : undefined,
		}),
	},
}))
...
describe("VscodeTelemetryPolicyService", () => {
	beforeEach(() => {
		telemetryState.forgeosTelemetrySetting = "unset"
		...
		vi.stubEnv("FORGEOS_ROLLOUT_VARIANT", "")
	})
	...
	it("adds rollout metadata as SDK common properties", () => {
		vi.stubEnv("FORGEOS_ROLLOUT_VARIANT", "next")
		coreTelemetryMocks.createHandle.mockReturnValue(createHandle())
	...
	it("allows ordinary events when host telemetry is enabled and ForgeOS telemetry is not disabled", async () => {
		const handle = createHandle()
		const service = new VscodeTelemetryPolicyService(handle)
	...
	it("drops ordinary events when ForgeOS telemetry is disabled but allows required events while host telemetry is enabled", async () => {
		telemetryState.forgeosTelemetrySetting = "disabled"
		const handle = createHandle()
		const service = new VscodeTelemetryPolicyService(handle)
		await settlePromises()
	...
	it("applies the full host identity metadata before enabling events", async () => {
		telemetryState.hostVersion = {
			...
			version: "2026.1.1",
			forgeosType: "ForgeOS for JetBrains",
			forgeosVersion: "1.1.61",
		}
		const handle = createHandle()
		const service = new VscodeTelemetryPolicyService(handle)
		await settlePromises()
		...
		expect(handle.telemetry.updateMetadata).toHaveBeenCalledWith({
			...
			forgeos_type: "ForgeOS for JetBrains",
			platform: "IntelliJ IDEA Ultimate",
	...
	it("omits the metadata fields the host does not report", async () => {
		// The default host version has no forgeosVersion, so host_plugin_version must be absent.
		const handle = createHandle()
		const service = new VscodeTelemetryPolicyService(handle)
		await settlePromises()
