...
import { FORGEOS_PASS_PROVIDER_ID } from "@/core/controller/models/handleForgeOSPassProviderSelection"
import { ForgeOSAccountService } from "@/services/account/ForgeOSAccountService"
...
async function makeAuthenticatedRequest<T>(endpoint: string, organizationId: string): Promise<T> {
	...
	const url = new URL(apiEndpoint, ForgeOSEnv.config().apiBaseUrl).toString()
...
async function fetchRemoteConfigForOrganization(organizationId: string): Promise<RemoteConfig | undefined> {
	try {
		...
		const configData = await makeAuthenticatedRequest<{ value: string; enabled: boolean }>(
			FORGEOS_API_ENDPOINT.REMOTE_CONFIG,
			organizationId,
		)
...
async function fetchApiKeysForOrganization(organizationId: string): Promise<APIKeySettings> {
	try {
		...
		const response = await makeAuthenticatedRequest<{ providerApiKeys: string }>(FORGEOS_API_ENDPOINT.API_KEYS, organizationId)
...
async function discoverRemoteConfigOrg(): Promise<{ organizationId: string; discoveredValue?: string } | undefined> {
	const accountService = ForgeOSAccountService.getInstance()
...
function isForgeOSPassSelected(controller: Controller): boolean {
	const apiConfiguration = controller.stateManager.getApiConfiguration()

	return (
		apiConfiguration.planModeApiProvider === FORGEOS_PASS_PROVIDER_ID ||
		apiConfiguration.actModeApiProvider === FORGEOS_PASS_PROVIDER_ID
	)
}
...
async function ensureUserInOrgWithRemoteConfig(controller: Controller): Promise<RemoteConfig | undefined> {
	...
	if (isForgeOSPassSelected(controller)) {
		clearRemoteConfig()
