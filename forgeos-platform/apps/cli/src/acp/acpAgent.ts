...
import { isLikelyAuthError, type MessageWithMetadata } from "@forgeos/shared";
import { getPersistedProviderApiKey } from "../commands/auth";
import { resolveSystemPrompt } from "../runtime/prompt";
import { subscribeToAgentEvents } from "../runtime/session-events";
import { createCliCore } from "../session/session";
import { isForgeOSOrgIndividualInferenceSubscriptionErrorMessage } from "../utils/forgeos-pass-errors";
...
export class AcpAgent implements Agent {
	...
	isSessionReady() {
		...
		if (!this.authResult && !process.env.FORGEOS_API_KEY) {
	...
	async newSession(params: NewSessionRequest): Promise<NewSessionResponse> {
		...
		const providerId =
			process.env.FORGEOS_PROVIDER ?? this.authResult?.providerId ?? "forgeos";
		...
		// provider's own catalog: `forgeos-pass` uses `forgeos-pass/…` ids that mean
		// nothing to `forgeos`, and vice versa.
		const defaultModelId = await resolveDefaultModelId(
			providerId,
			process.env.FORGEOS_MODEL,
			providerModels,
		);
	...
	async setSessionConfigOption(
		...
		switch (params.configId) {
			case "provider": {
				if (process.env.FORGEOS_PROVIDER) {
					throw RequestError.invalidParams(
						undefined,
						"Cannot change provider: FORGEOS_PROVIDER environment variable is set",
					);
				}
				...
				// be listed first (for forgeos-pass that is an unrelated free model).
			...
			case ORGANIZATION_CONFIG_ID: {
				try {
					await switchForgeOSOrganization({
	...
	private get accountApiKey(): string {
		return process.env.FORGEOS_API_KEY ?? this.authResult?.apiKey ?? "";
	}

	private async getOrganizationConfigOption(
		providerId: string,
	): Promise<SessionConfigOption | undefined> {
		if (!usesForgeOSAccount(providerId)) {
			return undefined;
		}
		const organizations = await fetchForgeOSOrganizations({
			apiKey: this.accountApiKey,
			providerSettingsManager: this.providerSettingsManager,
		});
	...
	private async buildConfig(session: SessionState): Promise<Config> {
		...
		const providerId = process.env.FORGEOS_PROVIDER ?? session.currentProviderId;
		const apiKey = process.env.FORGEOS_API_KEY ?? this.authResult?.apiKey ?? "";
