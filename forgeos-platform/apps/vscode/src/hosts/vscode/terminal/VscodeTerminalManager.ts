...
export class VscodeTerminalManager {
	...
	runCommand(terminalInfo: ITerminalInfo, command: string): ITerminalProcessResultPromise {
		...
		process.once("error", () => {
			...
			// Evict the terminal from ForgeOS reuse without disposing potentially
			// active user work.
		...
		process.once("unobserved_command", (outcome) => {
			...
			this.evictTerminal(vscodeTerminalInfo)
			// Markerless streams (for example, an SSH session) and commands ForgeOS no
	 ...
	 * actually stops instead of continuing to run after ForgeOS stops observing it.
	...
	async getOrCreateTerminal(cwd: string, profileId: string = this.defaultTerminalProfile): Promise<ITerminalInfo> {
		...
		const matchingTerminal = terminals.find((t) => {
			...
			const terminalCwd = t.terminal.shellIntegration?.cwd // one of forgeos's commands could have changed the cwd of the terminal
			if (!terminalCwd) {
