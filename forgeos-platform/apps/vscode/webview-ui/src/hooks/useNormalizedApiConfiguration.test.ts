import { ApiFormat, ResolveModelInfoResponse } from "@shared/proto/forgeos/models"
...
describe("useNormalizedApiConfiguration", () => {
	...
	it("uses ForgeOS-specific model fields instead of stale generic or OpenRouter fields", async () => {
		setApiConfiguration({
			actModeApiProvider: "forgeos",
			actModeApiModelId: "openai/gpt-5.4",
			actModeOpenRouterModelId: "anthropic/claude-sonnet-4.5",
			actModeForgeOSModelId: "anthropic/claude-sonnet-4.6",
		})
		mockResolveModelInfo.mockResolvedValue(modelInfoResponse("forgeos", "anthropic/claude-sonnet-4.6"))

		const { result } = renderHook(() => useNormalizedApiConfiguration("act"))

		await waitFor(() => expect(result.current.selectedModelId).toBe("anthropic/claude-sonnet-4.6"))
		expect(mockResolveModelInfo).toHaveBeenCalledWith({ providerId: "forgeos", modelId: "anthropic/claude-sonnet-4.6" })
	})

	it("asks the backend for the ForgeOS default when no ForgeOS-specific model is selected", async () => {
		setApiConfiguration({
			actModeApiProvider: "forgeos",
			actModeApiModelId: "openai/gpt-5.4",
			actModeOpenRouterModelId: "anthropic/claude-sonnet-4.5",
		...
		await waitFor(() => expect(result.current.selectedModelId).toBe("anthropic/claude-sonnet-4.6"))
		expect(mockResolveModelInfo).toHaveBeenCalledWith({ providerId: "forgeos", modelId: undefined })
	})
