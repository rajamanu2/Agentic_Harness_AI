...
describe("ForgeOSAccountService.fetchUserRemoteConfig", () => {
	let service: ForgeOSAccountService
	...
	beforeEach(() => {
		...
		service = new ForgeOSAccountService()
	})
...
describe("ForgeOSAccountService.fetchFeaturebaseToken", () => {
	...
	it("returns undefined when the request throws due to missing auth token", async () => {
		sandbox
			.stub(service as unknown as { authenticatedRequest: () => unknown }, "authenticatedRequest")
			.rejects(new Error("No ForgeOS account auth token found"))
