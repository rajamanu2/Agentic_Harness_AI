...
interface ForgeOSAuthStatusProps {
	message: string | null
}

export const ForgeOSAuthStatus = ({ message }: ForgeOSAuthStatusProps) => {
		...
		<div className="rounded border border-neutral-500/30 bg-vscode-editor-background p-3 text-vscode-foreground text-center">
			...
			{authCode ? (
				<div className="mt-2 flex items-center justify-center gap-2">
					...
					<Button aria-label="Copy ForgeOS sign-in code" onClick={handleCopy} size="sm" variant="secondary">
						{didCopy ? "Copied" : "Copy"}
					</Button>
				</div>
			) : null}
