...
export class PostHogClientProvider {
	 ...
	 * For exceptions, we only capture those from the ForgeOS extension.
	 * this is specifically to avoid capturing errors from anything other than ForgeOS
	 */
	static eventFilter(event: EventMessage | null) {
		...
		// Check if any exception is from ForgeOS
		for (let i = 0; i < exceptionList.length; i++) {
			const stacktrace = exceptionList[i].stacktrace
			// Fast check: error message contains "forgeos"
			if (stacktrace?.value?.toLowerCase().includes("forgeos")) {
				return event
			}
			...
			if (frames?.length) {
				for (let j = 0; j < frames.length; j++) {
					...
					// The CLI filename will include "forgeos"
					if (fileName?.includes("saoudrizwan") || fileName?.includes("forgeos")) {
						return event
					}
				}
			}
