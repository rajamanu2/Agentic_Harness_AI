...
const stateKey = "forgeos-hub-webview-state";
...
function createBrowserSocket(): WebSocket {
	...
	browserSocket.addEventListener("message", (event) => {
		try {
			...
			dispatchHostMessage({
				type: "error",
				text: "Received an invalid message from the ForgeOS Hub server.",
			});
		}
	});
	browserSocket.addEventListener("close", () => {
		...
		dispatchHostMessage({
			type: "status",
			text: "Disconnected from the ForgeOS Hub server.",
		});
	});
	browserSocket.addEventListener("error", () => {
		...
		dispatchHostMessage({
			type: "error",
			text: "Failed to connect to the ForgeOS Hub server.",
		});
	});
	return browserSocket;
}
