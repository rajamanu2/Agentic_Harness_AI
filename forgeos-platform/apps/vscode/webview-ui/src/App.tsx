import type { Boolean, EmptyRequest } from "@shared/proto/forgeos/common"
...
import { openForgeOSPassSubscriptionIfPending } from "./components/onboarding/forgeosPassSubscribe"
...
const AppContent = () => {
	...
	const { forgeosUser, organizations, activeOrganization } = useForgeOSAuth()
	...
	// Open the ForgeOSPass subscription page once auth completes. Lives here (not in OnboardingView)
	// because handleAuthCallback unmounts onboarding before the forgeosUser update arrives.
	useEffect(() => {
		if (forgeosUser?.uid) {
			openForgeOSPassSubscriptionIfPending(forgeosUser.appBaseUrl)
		}
	}, [forgeosUser?.uid, forgeosUser?.appBaseUrl])
