import type { Banner, BannerAction, BannerRules, BannersResponse } from "@shared/ForgeOSBanner"
import { BannerActionType, type BannerCardData } from "@shared/forgeos/banner"
...
export class BannerService {
	...
	public getWelcomeBanners(): BannerCardData[] | undefined {
		const isLocal = process.env.IS_DEV === "true" || process.env.FORGEOS_ENVIRONMENT === "local"
		const flagEnabled = isLocal || featureFlagsService.getBooleanFlagEnabled(FeatureFlag.REMOTE_WELCOME_BANNERS)

		if (!flagEnabled) {
			return undefined
		}
		const bypassDismissals = process.env.IS_DEV === "true" || process.env.FORGEOS_ENVIRONMENT === "local"
	...
	public async sendBannerEvent(bannerId: string, eventType: "dismiss"): Promise<void> {
		try {
			const url = new URL("/banners/v2/messages", ForgeOSEnv.config().apiBaseUrl).toString()
	...
	private buildFetchUrl(): string {
		const url = new URL("/banners/v2/messages", ForgeOSEnv.config().apiBaseUrl)
