import type { AgentConfig } from "@forgeos/shared";
...
describe("createSpawnAgentTool", () => {
	...
	it("appends workspace metadata for forgeos sub-agents when missing", async () => {
		const { createSpawnAgentTool } = await import("./spawn-agent-tool.js");
		runMock.mockResolvedValue({
			text: "ok",
			iterations: 1,
		...
		const workspaceMetadata = `# Workspace Configuration
{
  "workspaces": {
	...
	it("does not duplicate workspace metadata for forgeos sub-agents", async () => {
		const { createSpawnAgentTool } = await import("./spawn-agent-tool.js");
		runMock.mockResolvedValue({
			text: "ok",
			iterations: 1,
