...
description: "Automated GitHub issue analysis using ForgeOS CLI to identify root causes."
---

Automated GitHub issue analysis using ForgeOS CLI. This script uses ForgeOS's autonomous AI capabilities to fetch, analyze, and identify root causes of GitHub issues, outputting clean, parseable results that can be easily integrated into your development workflows.

<Note>
**New to ForgeOS CLI?** This sample assumes you have already completed the [Installation Guide](https://docs.forgeos.bot/getting-started/installing-forgeos) and authenticated with `forgeos auth`. If you haven't set up ForgeOS CLI yet, please start there first.
</Note>

<Frame>
  <img src="https://storage.googleapis.com/forgeos_public_images/cli-rca.gif" alt="CLI Root Cause Analysis Demo" width="600" />
</Frame>
...
- **ForgeOS CLI** installed and authenticated ([Installation Guide](https://docs.forgeos.bot/getting-started/installing-forgeos))
- **At least one AI model provider** configured (e.g., OpenRouter, Anthropic, OpenAI)
- **Basic familiarity** with ForgeOS CLI commands
...
```bash
curl -O https://raw.githubusercontent.com/forgeos/forgeos/main/src/samples/cli/github-issue-rca/analyze-issue.sh
```
...
<Note>
The script will automatically handle everything: fetching the issue, analyzing it with ForgeOS, and displaying the results. The analysis typically takes 30-60 seconds depending on the issue complexity.
</Note>
...
**1. `forgeos --auto-approve true --json "$PROMPT: $ISSUE_URL"`**
   - `forgeos` is the ForgeOS CLI binary
...
This sample demonstrates how to build an autonomous GitHub issue analysis tool using ForgeOS CLI:

1. **Building autonomous CLI tools** using ForgeOS's capabilities
2. **Parsing structured JSON output** from ForgeOS CLI
...
- [CLI Installation Guide](https://docs.forgeos.bot/getting-started/installing-forgeos)
- [CLI Reference Documentation](https://docs.forgeos.bot/cli/cli-reference)
- [Headless Mode](https://docs.forgeos.bot/usage/cli-overview#headless-mode)
