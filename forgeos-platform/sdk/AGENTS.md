---
description: Development reference for the ForgeOS SDK workspace.
globs: "*.ts,*.tsx,*.js,*.jsx,*.json,*.md"
alwaysApply: true
---

# ForgeOS SDK — Development Reference
...
- `@forgeos/shared`: shared contracts, schemas, path helpers, hook engine, extension registry, low-level utilities
- `@forgeos/llms`: provider settings/config, model catalogs, provider manifests, gateway contracts, handler creation
- `@forgeos/agents`: stateless agent loop, tool orchestration, hook/extension runtime, event streaming
- `@forgeos/core`: stateful orchestration, session lifecycle, storage, config watching, plugin loading, default tools, telemetry. Exposes `@forgeos/core/hub` for discovery, the detached daemon entry, WebSocket clients, and session/UI client adapters, plus `@forgeos/core/hub/daemon-entry` for launching the shared daemon

### Dependency Direction

```mermaid
flowchart TD
  shared["@forgeos/shared"] --> llms["@forgeos/llms"] & agents["@forgeos/agents"] & core["@forgeos/core"]
  llms --> agents & core
  agents --> core
...
- model/provider schemas or handler behavior: `@forgeos/llms`
- stateless loop, tool orchestration, streaming, hook/extension runtime: `@forgeos/agents`
- session lifecycle, storage, config watching, default tools, plugin loading, telemetry, hub runtime services, hub discovery, hub daemon spawn, and session-oriented client helpers (`HubSessionClient`, `HubUIClient`, `connectToHub`): `@forgeos/core` (hub pieces live under `src/hub/`)
- remote-config schemas, managed instruction materialization, blob upload metadata, and OpenTelemetry config normalization: `@forgeos/shared/src/remote-config`
...
```sh
bun -F @forgeos/shared test
bun -F @forgeos/llms test
bun -F @forgeos/agents test
bun -F @forgeos/core test:unit
bun -F @forgeos/cli test:unit
```

If a focused test command fails with a missing `@forgeos/*` export or missing `dist/` file, build the relevant dependency package or run `bun run build:sdk`, then rerun the same test command. Treat that as a workspace setup issue, not as evidence of a source-code bug.
...
- For `@forgeos/llms` provider/model routing rules, follow [packages/llms/AGENTS.md](./packages/llms/AGENTS.md).
