...
import { ModelCapabilitySchema } from "@forgeos/shared"
...
const modelInfoKeysByProvider: Partial<Record<string, ModelInfoKeys>> = {
	...
	forgeos: { plan: "planModeForgeOSModelInfo", act: "actModeForgeOSModelInfo" },
...
function writeStateFields(providerId: ProviderId, patch: ProviderConfigPatch): void {
	...
	if (provider === "forgeos" && "auth" in patch) {
		writeStateKey("forgeosApiKey", patch.auth?.accessToken)
		writeStateKey("forgeosAccountId", patch.auth?.accountId)
	}
