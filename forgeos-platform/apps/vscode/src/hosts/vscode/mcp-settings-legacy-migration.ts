...
const MCP_SETTINGS_FILE_NAME = "forgeos_mcp_settings.json"
...
export async function getLegacyMcpSettingsSources(vscodeContext: vscode.ExtensionContext): Promise<LegacyMcpSource[]> {
	...
	sources.push({
		id: "documentsForgeOSMcp",
		path: path.join(documentsDir, "ForgeOS", "MCP", MCP_SETTINGS_FILE_NAME),
	})
	return sources
}
...
export function getSharedMcpSettingsPath(storage: StorageContext): string {
	...
	const explicitDataDir = process.env.FORGEOS_DATA_DIR?.trim()
	if (explicitDataDir) {
	...
	return path.join(forgeosDir, "data", "settings", MCP_SETTINGS_FILE_NAME)
}
