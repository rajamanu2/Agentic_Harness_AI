...
import type { ForgeOSCoreListHistoryOptions, SessionHistoryRecord } from "@forgeos/core"
import type { MessageWithMetadata as SdkMessage } from "@forgeos/llms"
import { formatDisplayUserInput, parseUserInputMode } from "@forgeos/shared"
import { resolveSessionDataDir } from "@forgeos/shared/storage"
...
export interface SdkTaskHistoryOptions {
	 ...
	 * state/taskHistory.json and tasks/<id>/ instead of ~/.forgeos/data.
	 */
...
type SdkTaskHistoryListOptions = ForgeOSCoreListHistoryOptions & {
	offset?: number
}
...
export class SdkTaskHistory {
	...
	async listHistory(options: SdkTaskHistoryListOptions = {}): Promise<SessionHistoryRecord[]> {
		...
		const hostOptions: ForgeOSCoreListHistoryOptions = { ...options }
	...
	async getForgeOSMessages(taskId: string): Promise<ForgeOSMessage[]> {
		...
		const forgeosMessages = sdkMessagesToForgeOSMessages(
		...
		if (sdkRecord && legacyTask) {
			return mergeLegacyUiMessagesWithResumedSdkMessages(readUiMessages(taskId, legacyTask.dataDir), forgeosMessages)
		}
