 ...
 * in @forgeos/core's user-instruction-config-loader). The SDK strips the
...
/**
 * Verbatim port of @forgeos/shared's private `sanitizeSegment`
...
/** Matches files materialized from remote config (`.forgeos/remote-config/…`). */
const REMOTE_CONFIG_PATH_REGEX = /[/\\]\.forgeos[/\\]remote-config[/\\]/
 ...
 * extension, but the webview autocomplete (and legacy ForgeOS versions) surface
