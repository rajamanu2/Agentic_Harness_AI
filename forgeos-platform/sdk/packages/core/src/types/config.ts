import type { ModelInfo } from "@forgeos/llms";
import type {
	AgentConfig,
	AgentHooks,
	AgentMode,
...
/**
 * Public ForgeOSCore start configuration. The execution host resolves `cwd`
 * before constructing a runtime, assigning the shared chat workspace when both
 * workspace paths are omitted.
 */
export type ForgeOSCoreStartConfig = Omit<CoreSessionConfig, "cwd"> & {
	cwd?: string;
};
