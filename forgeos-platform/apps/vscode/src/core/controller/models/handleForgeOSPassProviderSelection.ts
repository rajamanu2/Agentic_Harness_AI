...
export const FORGEOS_PASS_PROVIDER_ID = "forgeos-pass"

/**
 * ForgeOSPass always uses the user's personal ForgeOS account balance.
 *
 ...
 */
export function clearOrganizationForForgeOSPassProviderSelection(
	...
	controller.accountService.switchAccount(undefined).catch((error) => {
		Logger.debug("Failed to switch ForgeOSPass to personal account", { error })
	})
}
