 ...
 * Returns array of:
 * { serviceName: string, serviceKey: string, protoPackage: "forgeos"|"host", moduleBase: string }
 */
async function parseServicesWithFiles(protoDir, protoFiles) {
	...
	for (const relPath of protoFiles) {
		...
		const pkg = relPath.startsWith("host/") ? "host" : "forgeos"
...
async function generateForgeOSClientPy(outDir, services) {
...
class ForgeOSClient:
    """
    Unified Python client analogous to src/generated/grpc-go/client/ForgeOSClient.

    Usage:
        client = ForgeOSClient("localhost:17611")
        client.connect()
...
`
	...
	await fs.writeFile(path.join(clientDir, "forgeos_client.py"), content)
}
...
async function generatePythonClient(protoDir, pyOutDir, clientDir, protoFiles) {
	...
	// forgeos_client.py (unified that composes service wrappers)
	await generateForgeOSClientPy(clientDir, services)
}
...
[project]
name = "forgeos-grpc-python"
version = "0.1.0"
description = "Generated Python gRPC stubs and client wrappers for ForgeOS protos"
