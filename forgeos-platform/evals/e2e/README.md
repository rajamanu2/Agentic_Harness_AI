...
Full end-to-end tests using real-world tasks from forgeos-bench.

## Overview

These tests run ForgeOS against production-grade coding problems derived from actual user sessions. Each task:
- Starts with a broken codebase in Docker
- Gives ForgeOS the task description
...
npx tsx evals/e2e/run-forgeos-bench.ts

# Run specific task
npx tsx evals/e2e/run-forgeos-bench.ts --tasks discord

# Use different provider/model
npx tsx evals/e2e/run-forgeos-bench.ts --provider openai --model gpt-4o

# Run on Daytona cloud (faster, parallel)
export DAYTONA_API_KEY=dtn_...
npx tsx evals/e2e/run-forgeos-bench.ts --env daytona

# Output to JSON
npx tsx evals/e2e/run-forgeos-bench.ts --output results.json
```
...
Current tasks from forgeos-bench (12 total):
...
Results are written to `evals/forgeos-bench/jobs/` directory by Harbor:
...
└── 2025-01-25__10-00-00/
    ...
    └── <task-id>__<hash>/
        ...
        ├── agent/forgeos.txt      # Conversation log
