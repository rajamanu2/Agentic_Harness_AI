...
const ORG_FORGEOS_PASS_RESTRICTION_MESSAGE =
	"Organization accounts cannot use ForgeOSPass subscriptions.";

const OrgForgeOSPassRestrictionError = () => {
	...
	const handleSwitchToPersonalAccount = async () => {
		...
		try {
			...
			console.error("Failed to switch to personal ForgeOS account:", error);
			setError(
	...
	return (
		<div
			...
			data-testid="org-forgeos-pass-restriction-error"
		>
			<div className="text-error mb-2">
				Organization account cannot use ForgeOSPass
			</div>
			<div className="text-(--vscode-descriptionForeground) text-xs wrap-anywhere">
				{ORG_FORGEOS_PASS_RESTRICTION_MESSAGE}
			</div>
			<VSCodeButton
...
export { ORG_FORGEOS_PASS_RESTRICTION_MESSAGE };
export default OrgForgeOSPassRestrictionError;
