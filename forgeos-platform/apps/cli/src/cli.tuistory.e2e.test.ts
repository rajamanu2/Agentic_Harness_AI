...
function createCliEnv(
	...
	return {
		...
		// Without this, the ForgeOSPass promo dialog renders over the chat view.
...
describe("cli tuistory e2e", () => {
	...
	it("launches config view directly with `forgeos config`", async () => {
	...
	it("dismisses the ForgeOSPass promo with any key and marks it as shown", async () => {
		// Re-enable the promo dialog that the shared env suppresses.
		const env = createCliEnv({ FORGEOS_DISABLE_FORGEOS_PASS_NOTICE: undefined });
		const dataDir = env.FORGEOS_DATA_DIR as string;
		const session = await launchCli([], env);

		await session.waitForText("Try ForgeOSPass", { timeout: LAUNCH_TIMEOUT_MS });
		...
		await session.text({
			waitFor: (text) => !text.includes("Try ForgeOSPass"),
			timeout: UI_TIMEOUT_MS,
		});

		const screen = await session.text();
		expect(screen).toContain("What can I do for you?");
		expect(screen).not.toContain("Open ForgeOSPass");
		...
		expect(readFileSync(markerPath, "utf8")).toContain(
			'"forgeos-cli-forgeos-pass-intro": true',
		);
	});
});
