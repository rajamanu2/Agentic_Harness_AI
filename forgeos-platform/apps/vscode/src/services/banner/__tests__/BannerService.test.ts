...
import type { BannerRules } from "@shared/ForgeOSBanner"
import { expect } from "chai"
import * as sinon from "sinon"
import { ForgeOSEnv, Environment } from "@/config"
...
describe("BannerService", () => {
	...
	beforeEach(() => {
		...
		// Mock ForgeOSEnv.config() to return a valid config
		sandbox.stub(ForgeOSEnv, "config").returns({
			environment: Environment.production,
			appBaseUrl: "https://app.forgeos-mock.bot",
			apiBaseUrl: "https://api.forgeos-mock.bot",
			mcpBaseUrl: "https://api.forgeos-mock.bot/v1/mcp",
		})
	...
	describe("IDE Type Detection", () => {
		...
		it('should return "jetbrains" when ide is "ForgeOS for JetBrains" (case-insensitive)', async () => {
			stubHostInfo({ ide: "ForgeOS for JetBrains" })
			mockFetch.resolves(createSuccessResponse(emptyResponse))
		...
		it("should prefer ide field over platform field for detection", async () => {
			stubHostInfo({ ide: "ForgeOS for JetBrains", platform: "Visual Studio Code 1.103.0" })
			mockFetch.resolves(createSuccessResponse(emptyResponse))
