...
import type { ToolPolicy } from "@forgeos/core";

import { registerDisposable } from "@forgeos/shared";
...
async function createProviderSettingsManager() {
	const { ProviderSettingsManager } = await import("@forgeos/core");
 ...
 * config directory before commander parses, because setForgeOSDir() must run
...
function writePromptArgError(args: string[]): void {
	...
	writeErr(
		`Unknown command or unquoted prompt: ${renderedArgs}\nPrompt text must be passed as a single quoted argument, for example: forgeos "fix the tests". Use "forgeos --help" to see available commands and flags.`,
	);
}
...
export async function runCli(): Promise<void> {
	...
	const { setForgeOSDir, setHomeDir } = await import("@forgeos/shared/storage");
	if (configDir) {
		setForgeOSDir(configDir);
	}
	setHomeDir(homedir());
	...
	const authCmd = program
		...
		.option(
			"--data-dir <dir>",
			"Use isolated local state at <dir> instead of ~/.forgeos (enables sandbox mode)",
		)
		...
		.action(async (positionalProvider: string | undefined) => {
			...
			// The early pre-pass in runCli() also calls setForgeOSDir(), but only
			...
			// settings manager is constructed against ~/.forgeos.
			if (opts.config?.trim()) {
				const { setForgeOSDir } = await import("@forgeos/shared/storage");
				setForgeOSDir(opts.config.trim());
			}
			// Honor --data-dir before constructing the provider settings manager
			// so writes land under the chosen data dir instead of ~/.forgeos.
			configureSandboxEnvironment({
				enabled: !!opts.dataDir || process.env.FORGEOS_SANDBOX?.trim() === "1",
	...
	const pluginCmd = program
		.command("plugin")
		.description("Manage ForgeOS Plugins")
		.action(() => {
			pluginCmd.help();
		});
	const pluginInstallCmd = pluginCmd
		...
		.alias("i")
		.description(
			"Install a ForgeOS Plugin from an official keyword, npm, git, URL, or a local path",
		)
		.argument(
			"<source>",
		...
		.option("--json", "Output as JSON")
		.option("--cwd <path>", "Install to <path>/.forgeos/plugins")
	...
	const pluginUninstallCmd = pluginCmd
		...
		.alias("remove")
		.alias("rm")
		.description("Uninstall a ForgeOS Plugin by name or path")
		.argument("<name>", "plugin package name, installed slug, or plugin path")
		.option("--json", "Output as JSON")
		.option(
			"--cwd <path>",
			"Search <path>/.forgeos/plugins before global plugins",
		)
	...
	const skillCmd = program
		.command("skill")
		.description("Manage ForgeOS Skills via the open skills CLI (npx skills)")
		.allowUnknownOption()
		.passThroughOptions()
		...
		.addHelpText(
			...
			"\nForwards to the open skills CLI via npx. Examples:\n" +
				"  forgeos skill add <owner/repo>       Add a skill into ForgeOS\n" +
				"  forgeos skill install <owner/repo>   Alias for add\n" +
				"  forgeos skill list                   List installed skills\n" +
				"  forgeos skill remove                 Remove installed skills\n" +
				"  forgeos skill uninstall              Alias for remove\n" +
				"\nadd/install and remove/uninstall default to '--agent forgeos' unless you pass your own --agent.\n" +
	...
	const connectCmd = program
		...
		.argument("[channel]", "Channel to connect ForgeOS CLI to")
	...
	const mcpCmd = program
		...
		.action(async () => {
			if (isFullTTY) {
				...
				writeln(
					"MCP wizard requires a TTY. Use forgeos config mcp to list servers.",
				);
			}
		});
	...
	const dashboardCmd = program
		.command("dashboard")
		.description("Start the ForgeOS Hub dashboard and open it in a browser")
		.option("--config <dir>", "configuration directory")
	...
	program
		.command("version")
		.description("Show ForgeOS CLI version number")
		.action(async () => {
	...
	if (args.id !== undefined) {
		...
		startupTarget = "chat";
		process.env.FORGEOS_HOOK_AGENT_RESUME = "1";
	} else {
		delete process.env.FORGEOS_HOOK_AGENT_RESUME;
	}
	if (startupTarget) {
		args = {
			...args,
	...
	if (args.hooksDir?.trim()) {
		process.env.FORGEOS_HOOKS_DIR = args.hooksDir.trim();
	}
	...
	// when FORGEOS_SANDBOX=1 is set in the environment (in which case the data
	// dir falls back to $FORGEOS_SANDBOX_DATA_DIR or /tmp/forgeos-sandbox).
	const sandboxEnabled =
		!!args.dataDir || process.env.FORGEOS_SANDBOX?.trim() === "1";
	...
	// These components operate before/outside ForgeOSCore sessions, so the
	...
	try {
		const persistedForgeOSAccountId = providerSettingsManager
			.getProviderSettings("forgeos")
			?.auth?.accountId?.trim();
		if (persistedForgeOSAccountId) {
			setCliFeatureFlagsAccountContext({ id: persistedForgeOSAccountId });
		}
		refreshCliFeatureFlagsInBackground();
		...
		const provider = normalizeProviderId(
			args.provider?.trim() || lastUsedProviderSettings?.provider || "forgeos",
		);
		let selectedProviderSettings =
			providerSettingsManager.getProviderSettings(provider);

		// Apply locally persisted ForgeOS account identity so subsequent events
		...
		// lightweight subcommand and pre-auth CLI flows. See FORGEOS-2406.
		if (provider === "forgeos") {
		...
		if (args.interactive || !args.prompt) {
			...
			const initialForgeOSProviderSettings =
				provider === "forgeos" ? selectedProviderSettings : undefined;
			let initialNotice:
			...
			if (
				...
				const { getForgeOSCliMigrationNotice, markForgeOSCliMigrationNoticeShown } =
					await import("./kanban-migration/notice");
				initialNotice = getForgeOSCliMigrationNotice(undefined, process.env, {
			...
			await runInteractive(config, userInstructionService, resumeSessionId, {
				...
				forgeosApiBaseUrl: initialForgeOSProviderSettings?.baseUrl,
				forgeosProviderSettings: initialForgeOSProviderSettings,
				startupTarget,
				initialNotice,
