...
cp examples/hooks/PreToolUse.sh .forgeos/hooks/
chmod +x .forgeos/hooks/PreToolUse.sh
forgeos -i "do something"  # See tool calls logged to stderr
```
...
```bash
mkdir -p .forgeos/hooks
cp examples/hooks/PostToolUse.sh .forgeos/hooks/
chmod +x .forgeos/hooks/PostToolUse.sh
forgeos -i "do something"  # See tool results logged and enriched
```
...
```bash
mkdir -p .forgeos/hooks
cp examples/hooks/PreToolUse_BlockDestructive.sh .forgeos/hooks/PreToolUse.sh
chmod +x .forgeos/hooks/PreToolUse.sh
forgeos -i "clean up the repo"  # Destructive operations will be blocked
```
...
```bash
mkdir -p .forgeos/hooks
cp examples/hooks/PreToolUse_RequireReview.sh .forgeos/hooks/PreToolUse.sh
chmod +x .forgeos/hooks/PreToolUse.sh
forgeos -i "update dependencies"  # Critical file writes will pause for review
```
...
```bash
mkdir -p .forgeos/hooks
cp examples/hooks/PreToolUse_InjectFileContext.sh .forgeos/hooks/PreToolUse.sh
chmod +x .forgeos/hooks/PreToolUse.sh
forgeos -i "review the configuration"  # Related files will be mentioned automatically
```
...
```bash
mkdir -p .forgeos/hooks
cp examples/hooks/TaskStart.sh .forgeos/hooks/
cp examples/hooks/TaskComplete.sh .forgeos/hooks/
cp examples/hooks/SessionShutdown.sh .forgeos/hooks/
chmod +x .forgeos/hooks/Task*.sh .forgeos/hooks/SessionShutdown.sh
forgeos -i "do something"  # Session lifecycle will be logged
```
...
```bash
mkdir -p .forgeos/hooks
cp examples/hooks/PreToolUse.py .forgeos/hooks/
chmod +x .forgeos/hooks/PreToolUse.py
forgeos -i "do something"  # Python hook will log tool calls
```
...
```bash
mkdir -p .forgeos/hooks
cp examples/hooks/PostToolUse.py .forgeos/hooks/
chmod +x .forgeos/hooks/PostToolUse.py
forgeos -i "do something"  # Python hook will enrich tool results
```
...
```bash
mkdir -p .forgeos/hooks
cp examples/hooks/PreToolUse_InjectContext.py .forgeos/hooks/PreToolUse.py
chmod +x .forgeos/hooks/PreToolUse.py
forgeos -i "add a new feature"  # Related files and environment will be injected
```
...
```bash
mkdir -p .forgeos/hooks
cp examples/hooks/PreToolUse.ts .forgeos/hooks/
chmod +x .forgeos/hooks/PreToolUse.ts
forgeos -i "do something"  # TypeScript hook will execute via bun
```
...
```bash
mkdir -p .forgeos/hooks
cp examples/hooks/PostToolUse.ts .forgeos/hooks/
chmod +x .forgeos/hooks/PostToolUse.ts
forgeos -i "do something"  # TypeScript hook will execute via bun
```
...
```bash
mkdir -p .forgeos/hooks
cp examples/hooks/PreToolUse_ModifyInput.ts .forgeos/hooks/PreToolUse.ts
chmod +x .forgeos/hooks/PreToolUse.ts
forgeos -i "install dependencies"  # npm install will have --save-exact added automatically
```
...
**File hooks** go in `.forgeos/hooks/` and must be named after the event they handle:

```bash
mkdir -p .forgeos/hooks

# Copy PreToolUse example (pick your language)
cp examples/hooks/PreToolUse.sh .forgeos/hooks/PreToolUse.sh      # Bash
cp examples/hooks/PreToolUse.py .forgeos/hooks/PreToolUse.py      # Python
cp examples/hooks/PreToolUse.ts .forgeos/hooks/PreToolUse.ts      # TypeScript

# Copy PostToolUse example
cp examples/hooks/PostToolUse.sh .forgeos/hooks/PostToolUse.sh    # Bash
cp examples/hooks/PostToolUse.py .forgeos/hooks/PostToolUse.py    # Python
cp examples/hooks/PostToolUse.ts .forgeos/hooks/PostToolUse.ts    # TypeScript
```

### 2. Make it executable

```bash
chmod +x .forgeos/hooks/PreToolUse.*
chmod +x .forgeos/hooks/PostToolUse.*
```

### 3. Test it

```bash
forgeos -i "test prompt"
# Or load from a custom hooks directory:
forgeos --hooks-dir ./my-hooks -i "test prompt"
```
...
```bash
forgeos --verbose "your prompt"
```

### Test a hook manually
```bash
echo '{"tool_call": {"name": "read_files", "input": {"filePath": "test.ts"}}}' | .forgeos/hooks/PreToolUse.sh
```

### Check hook output
```bash
.forgeos/hooks/PreToolUse.sh < input.json | jq .
```
...
```bash
forgeos plugin install https://github.com/forgeos/forgeos/blob/main/sdk/examples/hooks/custom-compaction-hook.example.ts --cwd .

forgeos -i "Search the codebase for dispatcher usage, then summarize it"
```
...
| ------- | --------------- | ------------- | -------- |
| `custom-compaction-hook.example.ts` (in `.forgeos/plugins/`) | `hooks.beforeModel` runtime hook | Agent runtime request messages with runtime parts such as `tool-call`, `tool-result`, `reasoning`, `image`, and `file` | Cases needing runtime-hook context, current runtime snapshot, or direct request mutation |
...
- **Use multiple hooks** — different event files can coexist in `.forgeos/hooks/`
