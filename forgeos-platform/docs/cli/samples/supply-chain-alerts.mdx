...
description: "Schedule the ForgeOS CLI to scan your machine for compromised packages with Bumblebee and text you on Telegram when it finds one."
---
...
- The ForgeOS CLI scheduler, which runs an agent on a cron schedule.
- The ForgeOS CLI Telegram connector, which delivers the result to a chat.

The end result: every morning, a ForgeOS agent pulls the latest threat intelligence, runs a read-only scan, and texts you a green check if you are clean or a red alert with details if you are exposed.

```mermaid
flowchart TD
    cron["forgeos schedule (daily)"] --> agent["ForgeOS agent"]
...
- Node.js 22 or newer (for the ForgeOS CLI).
- Go 1.22 or newer (to build Bumblebee).
- A Telegram account.
- An AI provider key, or a ForgeOS account.

## 1. Install the ForgeOS CLI

```bash
npm install -g forgeos
forgeos # run once to configure inference provider and model
```
...
<Steps>
  ...
  <Step title="Start the connector">
    ...
    ```bash
    forgeos connect telegram -k "<BOT-TOKEN>" --cwd ~/tools/bumblebee
    ```
...
<Warning>
By default, anyone who finds your bot can message it and it will run tasks on your machine. Lock it down before leaving the connector running. The `forgeos connect` wizard can guide you through Telegram user ID setup, or you can message [@userinfobot](https://t.me/userinfobot) and restart the connector with your allowed user ID:
...
Create the schedule with the ForgeOS CLI on the same machine and pass the delivery method explicitly. The running Telegram connector delivers the result to its chat:

```bash
forgeos schedule create "supply-chain-watch" \
  --cron "0 8 * * *" \
...
- The connector process (`forgeos connect telegram`) must stay running for delivery to work. Run it under a process manager (systemd, launchd, `pm2`, or a `tmux`/`screen` session) so it survives reboots.
- The hub runs the schedule and starts automatically when you create one. If it is not running, start it with `forgeos hub start`.
- Manage schedules anytime with `forgeos schedule list`, `forgeos schedule pause <id>`, `forgeos schedule resume <id>`, and `forgeos schedule delete <id>`.
