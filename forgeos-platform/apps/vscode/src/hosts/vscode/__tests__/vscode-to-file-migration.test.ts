...
describe("vscode-to-file-migration", () => {
	...
	beforeEach(() => {
		...
		originalMcpSettingsPath = process.env.FORGEOS_MCP_SETTINGS_PATH
		tempDir = path.join(os.tmpdir(), `migration-test-${Date.now()}-${Math.random().toString(36).slice(2)}`)
		fs.mkdirSync(tempDir, { recursive: true })
		process.env.FORGEOS_MCP_SETTINGS_PATH = path.join(tempDir, "runtime-mcp-settings", "forgeos_mcp_settings.json")
	...
	describe("sentinel behavior", () => {
		...
		it("should run only MCP settings migration when v1 storage export sentinels are already present", async () => {
			...
			mockCtx._workspaceStateStore.set("localForgeOSRulesToggles", { "rule-1": true })
			...
			fs.writeFileSync(
				path.join(extensionStorage, "settings", "forgeos_mcp_settings.json"),
			...
			;(storageContext.workspaceState.get("localForgeOSRulesToggles") === undefined).should.be.true()
			storageContext.globalState.get("__vscodeMigrationVersion")!.should.equal(3)
			storageContext.workspaceState.get("__vscodeMigrationVersion")!.should.equal(3)
		})
		...
		it("should migrate workspace state when globals already migrated (new workspace)", async () => {
			...
			const stored = storageContext.workspaceState.get("localForgeOSRulesToggles") as any
		...
		it("should migrate globals when workspace already migrated", async () => {
			...
			const mockCtx = createMockVSCodeContext()
			mockCtx._globalStateStore.set("mode", "plan")
			mockCtx._workspaceStateStore.set("localForgeOSRulesToggles", { "rule-1": true }) // should be skipped
	...
	describe("legacy MCP settings migration", () => {
		function sharedMcpSettingsPath() {
			return process.env.FORGEOS_MCP_SETTINGS_PATH!
		}
		...
		it("writes to the runtime MCP settings resolver path rather than storage.dataDir", async () => {
			...
			fs.existsSync(path.join(storageContext.dataDir, "settings", "forgeos_mcp_settings.json")).should.be.false()
	...
	describe("workspace state migration", () => {
		it("should migrate workspace state keys", async () => {
			...
			const mockCtx = createMockVSCodeContext()
			mockCtx._workspaceStateStore.set("localForgeOSRulesToggles", toggles)
		...
		it("should NOT overwrite existing workspace state", async () => {
			...
			storageContext.workspaceState.set("localForgeOSRulesToggles", existingToggles)

			const mockCtx = createMockVSCodeContext()
			mockCtx._workspaceStateStore.set("localForgeOSRulesToggles", { "rule-vscode": true })
...
describe("createStorageContext", () => {
	...
	it("should create all three stores", () => {
		const ctx = createStorageContext({ forgeosDir: tempDir, workspacePath: "/fake/workspace" })

		ctx.globalState.should.be.instanceOf(ForgeOSFileStorage)
		ctx.secrets.should.be.instanceOf(ForgeOSFileStorage)
		ctx.workspaceState.should.be.instanceOf(ForgeOSFileStorage)
	})
	...
	it("should produce deterministic workspace hashes", () => {
		const ctx1 = createStorageContext({ forgeosDir: tempDir, workspacePath: "/some/project" })
		const ctx2 = createStorageContext({ forgeosDir: tempDir, workspacePath: "/some/project" })
	...
	it("should produce different hashes for different workspaces", () => {
		const ctx1 = createStorageContext({ forgeosDir: tempDir, workspacePath: "/project-a" })
		const ctx2 = createStorageContext({ forgeosDir: tempDir, workspacePath: "/project-b" })
	...
	it("should store and retrieve values correctly", () => {
		const ctx = createStorageContext({ forgeosDir: tempDir, workspacePath: "/test" })
	...
	// The data dir must resolve like the SDK's resolveForgeOSDataDir and the
	// legacy reader's resolveDataDir: FORGEOS_DATA_DIR first. Ignoring it split
	...
	describe("environment-based data dir resolution", () => {
		const previousForgeOSDir = process.env.FORGEOS_DIR
		const previousForgeOSDataDir = process.env.FORGEOS_DATA_DIR

		afterEach(() => {
			restoreEnv("FORGEOS_DIR", previousForgeOSDir)
			restoreEnv("FORGEOS_DATA_DIR", previousForgeOSDataDir)
		})
		...
		it("should honor FORGEOS_DATA_DIR when no forgeosDir option is given", () => {
			const isolatedDataDir = path.join(tempDir, "isolated-data")
			process.env.FORGEOS_DATA_DIR = isolatedDataDir
		...
		it("should prefer FORGEOS_DATA_DIR over FORGEOS_DIR", () => {
			const isolatedDataDir = path.join(tempDir, "isolated-data")
			process.env.FORGEOS_DATA_DIR = isolatedDataDir
			process.env.FORGEOS_DIR = path.join(tempDir, "forgeos-home")
		...
		it("should let an explicit forgeosDir option win over FORGEOS_DATA_DIR", () => {
			process.env.FORGEOS_DATA_DIR = path.join(tempDir, "isolated-data")
		...
		it("should trim whitespace-padded env values like the SDK resolvers", () => {
			...
			ctx.dataDir.should.equal(path.join(tempDir, "forgeos-home", "data"))
		})
	})
})
