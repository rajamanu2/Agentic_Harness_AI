	...
	describe("StdioHookRunner", () => {
		it(
			...
			async () => {
				...
				const hookPath = path.join(tempDir, ".forgeosrules", "hooks", "PreToolUse")
  ...
  contextModification: "Received tool: " + input.preToolUse.toolName
	...
	describe("PostToolUse Hook", () => {
		it("should receive execution results", async () => {
			const hookPath = path.join(tempDir, ".forgeosrules", "hooks", "PostToolUse")
			const hookScript = `#!/usr/bin/env node
const input = JSON.parse(require('fs').readFileSync(0, 'utf-8'));
console.log(JSON.stringify({
  cancel: false,
  contextModification: "Tool succeeded: " + input.postToolUse.success
	...
	describe("Hook Discovery", () => {
		it("should generate Windows PowerShell bridge files with real newlines", async () => {
			const hooksDir = path.join(tempDir, ".forgeosrules", "hooks")
...
const hasAllFields = input.forgeosVersion && input.hookName && input.timestamp && 
  ...
  contextModification: hasAllFields ? "All fields present" : "Missing fields"
	...
	describe("Global Hooks", () => {
		...
		beforeEach(async () => {
			...
			workspaceHooksDir = path.join(tempDir, ".forgeosrules", "hooks")
...
console.log(JSON.stringify({
  ...
  contextModification: "GLOBAL: Context added"
			...
			const workspaceHookPath = path.join(tempDir, ".forgeosrules", "hooks", "PreToolUse")
