...
const MARKER_START = "__FORGEOS_SIDECAR_PATH_START__";
const MARKER_END = "__FORGEOS_SIDECAR_PATH_END__";
...
function writeFakeShell(
	...
	const dir = mkdtempSync(join(tmpdir(), "forgeos-shell-path-"));
	tempDirs.push(dir);
...
describe("ensureLoginShellPath", () => {
	...
	it("skips when the escape hatch is set", async () => {
		const env: NodeJS.ProcessEnv = {
			PATH: "/usr/bin",
			FORGEOS_SIDECAR_SKIP_SHELL_PATH: "1",
		};
