...
log("Running standalone forgeos", ExtensionRegistryInfo.version)
log(`FORGEOS_ENVIRONMENT: ${process.env.FORGEOS_ENVIRONMENT}`)

// WE WILL HAVE TO MIGRATE THIS FROM DATA TO v1 LATER
const SETTINGS_SUBFOLDER = "data"

export function initializeContext(forgeosDir?: string) {
	const FORGEOS_DIR = forgeosDir || process.env.FORGEOS_DIR || `${os.homedir()}/.forgeos`
	const DATA_DIR = path.join(FORGEOS_DIR, SETTINGS_SUBFOLDER)
	...
	const extensionContext: ExtensionContext = {
		...
		storagePath: WORKSPACE_STORAGE_DIR, // Deprecated, not used in forgeos.
		globalStorageUri: URI.file(DATA_DIR),
		globalStoragePath: DATA_DIR, // Deprecated, not used in forgeos.

		// Logs are global per extension, not per workspace.
		logUri: URI.file(DATA_DIR),
		logPath: DATA_DIR, // Deprecated, not used in forgeos.

		extensionUri: URI.file(EXTENSION_DIR),
		extensionPath: EXTENSION_DIR, // Deprecated, not used in forgeos.
