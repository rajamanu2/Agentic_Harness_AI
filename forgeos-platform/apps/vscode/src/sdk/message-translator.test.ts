...
import type { Message as SdkMessage } from "@forgeos/llms"
import type { AgentEvent, MessageWithMetadata } from "@forgeos/shared"
import type { ForgeOSAskUseMcpServer, ForgeOSSayTool } from "@shared/ExtensionMessage"
...
describe("translateSessionEvent — agent_event content_start", () => {
	...
	it("translates tool content_start to partial tool message", () => {
		...
		expect(result.messages[0].partial).toBe(true)
		// sdkToolToForgeOSSayTool converts "read_files" → "readFile" and
		// the text is JSON.stringify(ForgeOSSayTool)
...
describe("translateSessionEvent — agent_event content_end", () => {
	...
	it("translates tool content_end with output", () => {
		...
		const result = translateSessionEvent(event, state)
		expect(result.messages).toHaveLength(1)
		// Tool content_end produces a ForgeOSSayTool JSON with the tool name
		expect(result.messages[0].say).toBe("tool")
		expect(result.messages[0].text).toContain("readFile")
		expect(result.messages[0].partial).toBe(false)
	})
...
describe("translateSessionEvent — agent_event error", () => {
	...
	it("reshapes insufficient_credits error into ForgeOSError-compatible format", () => {
		const state = new MessageTranslatorState()
		...
		// The api_req_failed text should be structured JSON that ForgeOSError.parse() can handle
	...
	it("reshapes SPEND_LIMIT_EXCEEDED error into ForgeOSError-compatible format", () => {
		const state = new MessageTranslatorState()
		const errorJson = JSON.stringify({
	...
	it("reshapes plain-text insufficient credits error into ForgeOSError-compatible format", () => {
		const state = new MessageTranslatorState()
		...
		const event: CoreSessionEvent = {
			...
			payload: {
				...
				event: {
					type: "error",
					error: {
						message: "Insufficient balance. Your ForgeOS Credits balance is $-0.14",
					},
				} as AgentEvent,
			},
		}
	...
	it("reshapes plain-text 'Not enough credits' error into ForgeOSError-compatible format", () => {
		const state = new MessageTranslatorState()
		const event: CoreSessionEvent = {
			type: "agent_event",
			payload: {
	...
	it("reshapes plain-text spend limit error into ForgeOSError-compatible format", () => {
		const state = new MessageTranslatorState()
		const event: CoreSessionEvent = {
			type: "agent_event",
			payload: {
	...
	it("preserves ForgeOSPass period limit errors for specialized webview rendering", () => {
		const state = new MessageTranslatorState(undefined, () => "forgeos-pass")
		const message = "You have reached your weekly ForgeOSpass limit. The limit resets in 7d, please try again later."
		const event: CoreSessionEvent = {
			type: "agent_event",
			payload: {
				sessionId: "session-1",
...
describe("sdkToolToForgeOSSayTool — fetch_web_content and skills (S6-39, S6-40)", () => {
...
describe("sdkToolToForgeOSSayTool — search_codebase (S6-47)", () => {
...
describe("sdkToolToForgeOSSayTool — editor diff rendering (S6-48)", () => {
	...
	describe("sdkToolToForgeOSSayTool — apply_patch content (S6-48)", () => {
...
describe("apply_patch multi-file split (forgeos#9904)", () => {
	...
	it("content_start streams a single whole-patch preview row for a multi-file patch", () => {
		...
		// split happens only at content_end so the finalized ids match (forgeos#9904).
		expect(result.messages).toHaveLength(1)
		expect(result.messages[0].partial).toBe(true)
		const tool = JSON.parse(result.messages[0].text!)
		expect(tool.tool).toBe("editedExistingFile")
	...
	it("reconciles start→end by ts: N rows, none left partial (forgeos#9904 orphan guard)", () => {
		const state = new MessageTranslatorState()
...
describe("MCP tool rendering (serverName__toolName convention)", () => {
	it("content_start for MCP tool emits say='use_mcp_server' with ForgeOSAskUseMcpServer payload", () => {
		const state = new MessageTranslatorState()
		const event: CoreSessionEvent = {
			type: "agent_event",
			payload: {
		...
		expect(msg.partial).toBe(true)
		const payload = JSON.parse(msg.text!) as ForgeOSAskUseMcpServer
	...
	it("MCP tool with empty input omits arguments", () => {
		...
		const payload = JSON.parse(result.messages[0].text!) as ForgeOSAskUseMcpServer
		expect(payload.serverName).toBe("notion")
...
describe("tool display paths are relativized to the cwd", () => {
	...
	const parseTool = (text: string | undefined) => JSON.parse(text ?? "{}") as ForgeOSSayTool
	...
	it("relativizes persisted-history tool paths via options.cwd", () => {
		...
		const forgeosMessages = sdkMessagesToForgeOSMessages(messages, undefined, { cwd: CWD })
		const toolMessage = forgeosMessages.find((m) => m.say === "tool")
	...
	it("rehydrates generated images from persisted SDK history", () => {
		...
		const forgeosMessages = sdkMessagesToForgeOSMessages(messages)
		const imageMessage = forgeosMessages.find((message) => message.media?.length)
	...
	it("renders provider model activities through the persisted local-tool path", () => {
		...
		const forgeosMessages = sdkMessagesToForgeOSMessages(messages)
		const toolMessage = forgeosMessages.find((message) => message.say === "tool")

		expect(toolMessage).toBeDefined()
		expect(parseTool(toolMessage?.text)).toMatchObject({
			tool: "webSearch",
		})
		expect(forgeosMessages).toContainEqual(
			expect.objectContaining({
				type: "say",
