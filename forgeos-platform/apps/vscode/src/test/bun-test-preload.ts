...
// directly and stub `vscode`/`@forgeos/core` per-file) and delete this preload's
...
//   @forgeos/core  -> src/test/forgeos-core-vitest-stub.ts (lightweight SDK stub)
//
// bun's runtime plugin `onResolve` hook does NOT intercept these (`vscode` is
// host/builtin-like and `@forgeos/core` is a symlinked workspace package — both
...
// process. (Other workspace packages — @forgeos/llms, @forgeos/shared,
// @forgeos/shared/storage — and the tsconfig `paths` aliases resolve on their own.)
//
// bun's ESM linker statically validates every named import against the names on
// the mock namespace. The stub only implements the @forgeos/core exports these
// tests exercise, but other modules in the import graph statically import
// additional names (e.g. `prepareRemoteConfigCoreIntegration`, `ForgeOSCore`,
// `createMcpTools`); a missing name is a hard "Export named 'X' not found" link
// error. So we seed the mock namespace with every name the real @forgeos/core
...
import * as realForgeOSCore from "@forgeos/core"
import * as LlmsModels from "@forgeos/llms"
import * as forgeosCoreStub from "./forgeos-core-vitest-stub"
import * as vscodeStub from "./vscode-vitest-stub"

const forgeosCoreNamespace: Record<string, unknown> = {}
for (const name of Object.keys(realForgeOSCore)) {
	forgeosCoreNamespace[name] = undefined
}
Object.assign(forgeosCoreNamespace, forgeosCoreStub)

bunBeforeEach(() => {
	forgeosCoreStub.resetModelsFileState()
...
mock.module("@forgeos/core", () => forgeosCoreNamespace)
...
//                           (@forgeos/core, vscode), calling bun's `import()` from
...
const actualNamespaceRegistry: Record<string, unknown> = {
	"@forgeos/core": forgeosCoreNamespace,
