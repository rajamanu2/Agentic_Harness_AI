	...
	static getRecorder(controller: Controller): IRecorder {
		if (!GrpcRecorderBuilder.recorder) {
			GrpcRecorderBuilder.recorder = GrpcRecorder.builder()
				.enableIf(process.env.GRPC_RECORDER_ENABLED === "true" && process.env.FORGEOS_ENVIRONMENT === "local")
...
function testFilters(): GrpcRequestFilter[] {
	...
	return [
		...
		(req) => ["forgeos.UiService", "forgeos.McpService", "forgeos.WebService"].includes(req.service),
		(req) =>
			[
