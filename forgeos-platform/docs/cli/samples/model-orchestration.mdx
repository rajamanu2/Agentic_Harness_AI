...
ForgeOS CLI's `--config` and `--thinking` flags enable sophisticated multi-model workflows. Instead of using a single model for all tasks, you can route different work to different models based on cost, capability, and specialization.
...
See our production GitHub Actions workflow that uses ForgeOS CLI for automated PR reviews: [forgeos-pr-review.yml](https://github.com/forgeos/forgeos/blob/main/.github/workflows/forgeos-pr-review.yml)
...
SUMMARY=$(echo "$ISSUE_CONTENT" | forgeos --auto-approve true --config ~/.forgeos-haiku \
  "summarize this issue in 2-3 sentences")

# Phase 2: Detailed plan with expensive model + thinking
PLAN=$(echo "$SUMMARY" | forgeos --auto-approve true --thinking high --config ~/.forgeos-opus \
  "create detailed implementation plan with edge cases")

# Phase 3: Execute with mid-tier model
echo "$PLAN" | forgeos --auto-approve true --config ~/.forgeos-sonnet \
  "implement the plan from above"
```

<Note>
Each `forgeos` invocation needs to complete before passing output to the next phase. Use shell variables to store intermediate results rather than piping `forgeos` commands directly.
</Note>
...
mkdir -p ~/.forgeos-haiku ~/.forgeos-sonnet ~/.forgeos-opus

# Configure each with different models
forgeos --config ~/.forgeos-haiku auth anthropic --modelid claude-haiku-4-20250514
forgeos --config ~/.forgeos-sonnet auth anthropic --modelid claude-sonnet-4-20250514  
forgeos --config ~/.forgeos-opus auth anthropic --modelid claude-opus-4-5-20251101

# Or use different providers entirely
forgeos --config ~/.forgeos-gemini auth gemini --modelid gemini-2.0-flash-exp
forgeos --config ~/.forgeos-codex auth openai-codex --modelid gpt-5-latest
```

Now you can switch models per-task with `--config`:

```bash
forgeos --config ~/.forgeos-haiku "quick task"
forgeos --config ~/.forgeos-opus "complex reasoning task"
```
...
git show | forgeos --auto-approve true --config ~/.forgeos-gemini "review and save to gemini-review.md" &
git show | forgeos --auto-approve true --config ~/.forgeos-codex "review and save to codex-review.md" &
git show | forgeos --auto-approve true --config ~/.forgeos-opus "review and save to opus-review.md" &

# Wait for all to complete
wait

# Synthesize
cat *-review.md | forgeos --auto-approve true "create consensus review"
```

<Note>
Parallel execution requires managing multiple ForgeOS instances. See [Multi-instance workflows](/usage/cli-overview#automation-patterns) for details.
</Note>

## Extended Thinking for Complex Tasks

Use the `--thinking` flag when ForgeOS needs to analyze multiple approaches:

```bash
# Without thinking: Fast but may miss nuances
forgeos --auto-approve true "refactor this codebase"

# With thinking: Slower but more thorough
forgeos --auto-approve true --thinking high \
...
gh pr view $PR | forgeos --auto-approve true --config ~/.forgeos-haiku \
    "list all issues to fix, output as JSON"

# Opus with thinking: Deep analysis only if issues found
if [ -s issues.json ]; then
    forgeos --auto-approve true --thinking high --config ~/.forgeos-opus \
...
git diff main | forgeos --auto-approve true --config ~/.forgeos-gemini "security review" > gemini-sec.md &
git diff main | forgeos --auto-approve true --config ~/.forgeos-opus "security review" > opus-sec.md &
git diff main | forgeos --auto-approve true --config ~/.forgeos-codex "security review" > codex-sec.md &
wait

# High-priority: Issues all 3 models found
cat *-sec.md | forgeos --auto-approve true "find security issues all 3 reviews mentioned"
```
...
<Columns cols={2}>
  ...
  <Card title="Headless Mode" icon="robot" href="/usage/cli-overview#headless-mode">
    Run ForgeOS autonomously in scripts, CI/CD pipelines, and automated workflows.
  </Card>
  
  <Card title="ForgeOS Provider" icon="brain" href="/getting-started/forgeos-provider">
  ...
  <Card title="CI/CD Integration" icon="github" href="/cli/samples/github-integration">
    Automate GitHub workflows with ForgeOS CLI
  </Card>
</Columns>
