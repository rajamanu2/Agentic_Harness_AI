		...
		const session = new SessionRuntime(
			makeAgentConfig({
				...
				extensionContext: {
					...
					client: { name: "forgeos-sdk", version: "1.2.3" },
...
describe("SessionRuntime auth retry", () => {
	const authFailure: Partial<AgentRunResult> = {
		status: "failed",
		error: new Error(
			"Unauthorized: Please make sure you're using the latest version of ForgeOS and re-authenticate your ForgeOS account.",
		),
	};
