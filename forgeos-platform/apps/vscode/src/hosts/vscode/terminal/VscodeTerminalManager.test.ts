...
describe("VscodeTerminalManager", () => {
	...
	it("creates a fresh terminal after timing out an unconfirmed reused-terminal cwd change", async () => {
		...
		const targetCwd = "/tmp/forgeos-target"
		...
		const terminalInfo: TerminalInfo = {
			...
			terminal: {
				shellIntegration: {
					cwd: vscode.Uri.file("/tmp/forgeos-original"),
	...
	it("reuses a terminal after its cwd change is confirmed", async () => {
		const targetCwd = "/tmp/forgeos-target"
		let currentCwd = vscode.Uri.file("/tmp/forgeos-original")
		const terminalInfo: TerminalInfo = {
			id: 1,
			busy: false,
			lastCommand: "",
	...
	it("releases a reused terminal reservation when showing it fails", async () => {
		const terminalInfo: TerminalInfo = {
			...
			lastActive: Date.now(),
			terminal: {
				shellIntegration: { cwd: vscode.Uri.file("/tmp/forgeos-original") },
				show: sandbox.stub().throws(new Error("terminal closed")),
			} as unknown as vscode.Terminal,
		}
		sandbox.stub(TerminalRegistry, "getAllTerminals").returns([terminalInfo])

		await assert.rejects(manager.getOrCreateTerminal("/tmp/forgeos-target"), /terminal closed/)
	...
	it("creates a fresh terminal when the reused-terminal cwd command cannot start", async () => {
		...
		const terminal = (await manager.getOrCreateTerminal("/tmp/forgeos-target")) as unknown as TerminalInfo
		try {
			assert.notEqual(terminal, terminalInfo)
			assert.equal(terminal.busy, true)
			assert.equal(terminalInfo.busy, false)
	...
	it("releases a reused terminal reservation when it closes during cwd setup", async () => {
		...
		const rejectedAcquisition = assert.rejects(manager.getOrCreateTerminal("/tmp/forgeos-target"), /exited while preparing/)
	...
	it("reserves different terminals for parallel acquisitions", async () => {
		setVscodeHostProviderMock()
		const first = (await manager.getOrCreateTerminal("/tmp/forgeos-parallel")) as unknown as TerminalInfo
		const second = (await manager.getOrCreateTerminal("/tmp/forgeos-parallel")) as unknown as TerminalInfo
	...
	it("does not reuse a terminal after its command stream fails", async () => {
		setVscodeHostProviderMock()
		const terminalInfo = TerminalRegistry.createTerminal("/tmp/forgeos-stream-error")
		sandbox.stub(terminalInfo.terminal, "shellIntegration").get(() => ({
			cwd: vscode.Uri.file("/tmp/forgeos-stream-error"),
		...
		const nextTerminal = (await manager.getOrCreateTerminal("/tmp/forgeos-stream-error")) as unknown as TerminalInfo
		try {
	...
	it("continues terminal acquisition when pending cleanup fails", async () => {
		...
		try {
			acquiredTerminal = (await manager.getOrCreateTerminal("/tmp/forgeos-after-cleanup-error")) as unknown as TerminalInfo
	...
	it("defers fallback terminal disposal until the next terminal acquisition", async () => {
		...
		try {
			...
			nextTerminal = (await nextManager.getOrCreateTerminal("/tmp/forgeos-next-command")) as unknown as TerminalInfo
	...
	it("preserves a detached fallback terminal across the next terminal acquisition", async () => {
		...
		try {
			...
			nextTerminal = (await manager.getOrCreateTerminal("/tmp/forgeos-next-command")) as unknown as TerminalInfo
