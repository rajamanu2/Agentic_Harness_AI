...
// Local `createUID` helper. The forgeose source imports this from
// `@forgeos/shared` (see `packages/shared/dist/identifier.ts`), but
...
// exact forgeose implementation (`${prefix}_${nanoid(length)}`).
 ...
 * `@forgeos/core`, which constructs models itself to share gateway/telemetry
 ...
 * runtime builds an `AgentModel` internally via `@forgeos/llms`. This is the
 ...
 * construct the model itself via `@forgeos/llms`.
 */
 ...
 *
 * while `@forgeos/core` (which owns model construction) continues to use
