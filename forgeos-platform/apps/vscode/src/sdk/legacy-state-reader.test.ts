...
beforeEach(() => {
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "forgeos-legacy-state-"))
})
...
describe("resolveDataDir", () => {
	...
	it("falls back to FORGEOS_DATA_DIR env", () => {
		const original = process.env.FORGEOS_DATA_DIR
		process.env.FORGEOS_DATA_DIR = "/env/data"
		try {
			expect(resolveDataDir()).toBe("/env/data")
		} finally {
			process.env.FORGEOS_DATA_DIR = original
		}
	})

	it("trims a whitespace-padded FORGEOS_DATA_DIR like createStorageContext", () => {
		const original = process.env.FORGEOS_DATA_DIR
		process.env.FORGEOS_DATA_DIR = "  /env/data  "
		try {
			expect(resolveDataDir()).toBe("/env/data")
	...
	it("falls back to FORGEOS_DIR/data", () => {
		const originalData = process.env.FORGEOS_DATA_DIR
		const originalDir = process.env.FORGEOS_DIR
		delete process.env.FORGEOS_DATA_DIR
		process.env.FORGEOS_DIR = "/forgeos"
		try {
			// path.join keeps the platform separator (\ on Windows), so compare
			// against a joined path rather than a hardcoded POSIX string.
			expect(resolveDataDir()).toBe(path.join("/forgeos", "data"))
		} finally {
			process.env.FORGEOS_DATA_DIR = originalData
			process.env.FORGEOS_DIR = originalDir
		}
	})

	it("falls back to ~/.forgeos/data", () => {
		const originalData = process.env.FORGEOS_DATA_DIR
		const originalDir = process.env.FORGEOS_DIR
		delete process.env.FORGEOS_DATA_DIR
		delete process.env.FORGEOS_DIR
		try {
			expect(resolveDataDir()).toBe(path.join(os.homedir(), ".forgeos", "data"))
		} finally {
			process.env.FORGEOS_DATA_DIR = originalData
			process.env.FORGEOS_DIR = originalDir
		}
...
describe("readMcpSettings", () => {
	...
	it("reads forgeos_mcp_settings.json from settings/ subdirectory", () => {
		...
		writeJson(path.join(tempDir, "settings", "forgeos_mcp_settings.json"), settings)
	...
	it("returns empty mcpServers for corrupt JSON", () => {
		const filePath = path.join(tempDir, "settings", "forgeos_mcp_settings.json")
		fs.mkdirSync(path.dirname(filePath), { recursive: true })
...
describe("readAllLegacyState", () => {
	...
	it("reads all state files at once", () => {
		...
		writeJson(path.join(tempDir, "settings", "forgeos_mcp_settings.json"), {
			mcpServers: {
