/**
 * Early SDK logger for components that operate before/outside of `ForgeOSCore`
 * sessions, plus a secret-hashing helper for credential diagnostics.
 *
 * `ForgeOSCore.create({ logger })` receives a `BasicLogger` but it is only
 * threaded to session-scoped components. `ProviderSettingsManager`,
 * `RuntimeOAuthTokenManager`, and the ForgeOS auth functions in `forgeos.ts` are
 * constructed or called before `ForgeOSCore` exists or outside a session.
