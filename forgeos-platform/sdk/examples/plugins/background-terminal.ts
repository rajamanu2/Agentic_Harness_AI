 ...
 * under ForgeOS's data directory, and optionally steers a completion summary back
 * into the current session when the command exits.
 *
 * CLI usage:
 *   forgeos plugin install https://github.com/forgeos/forgeos/blob/main/sdk/examples/plugins/background-terminal.ts --cwd .
 *   forgeos -i "Start the dev server in the background and keep working"
 */
...
const FORGEOS_DATA_DIR =
	process.env.FORGEOS_DATA_DIR || join(homedir(), ".forgeos", "data");
const JOBS_DIR = join(FORGEOS_DATA_DIR, "plugins", "background-shell", "jobs");
