...
// module translates them to proto ForgeOSMessages and pushes them through
...
import type { ForgeOSMessage, ExtensionState } from "@shared/ExtensionMessage"
import type { ForgeOSMessage as ProtoForgeOSMessage } from "@shared/proto/forgeos/ui"
import { convertForgeOSMessageToProto } from "@shared/proto-conversions/forgeos-message"
 ...
 * 1. Translates them to ForgeOSMessage[] via the message translator
 * 2. Converts each ForgeOSMessage to proto format
...
export class WebviewGrpcBridge {
	...
	createListener(): SessionEventListener {
		return (messages: ForgeOSMessage[], event: CoreSessionEvent) => {
	 ...
	 */
	private handleSessionEvent(messages: ForgeOSMessage[], event: CoreSessionEvent): void {
	...
	/**
	 * Push a ForgeOSMessage to the webview via the subscribeToPartialMessage stream.
	 */
	private async pushPartialMessage(message: ForgeOSMessage): Promise<void> {
		try {
			const protoMessage: ProtoForgeOSMessage = convertForgeOSMessageToProto(message)
...
/**
 * Standalone function to push a ForgeOSMessage to the webview.
 * Useful for one-off messages outside the bridge's event loop.
 */
export async function pushMessageToWebview(message: ForgeOSMessage): Promise<void> {
	try {
		const protoMessage: ProtoForgeOSMessage = convertForgeOSMessageToProto(message)
		await sendPartialMessageEvent(protoMessage)
	} catch (error) {
