import { buildModelInfoNameMap, type ModelInfo, openAiModelInfoSafeDefaults, resolveForgeOSPassModelInfo } from "@shared/api"
import { StringRequest } from "@shared/proto/forgeos/common"
import type { OnboardingModel, OnboardingModelGroup, OpenRouterModelInfo } from "@shared/proto/index.forgeos"
...
import ForgeOSLogoWhite from "@/assets/ForgeOSLogoWhite"
...
import { setPendingForgeOSPassSubscribe } from "./forgeosPassSubscribe"
import {
	FORGEOSPASS_GROUP,
	getCapabilities,
...
type ModelSelectionProps = {
	userType: NEW_USER_TYPE.FREE | NEW_USER_TYPE.POWER | NEW_USER_TYPE.FORGEOS_PASS
...
function getModelGroupKey(userType: ModelSelectionProps["userType"]): keyof OnboardingModelsByGroup {
	if (userType === NEW_USER_TYPE.FORGEOS_PASS) {
		return "forgeosPass"
	}
...
const ModelSelection = ({
	...
	const isForgeOSPass = userType === NEW_USER_TYPE.FORGEOS_PASS
	const modelGroups = onboardingModels[getModelGroupKey(userType)]
	// ForgeOSPass costs are covered by the subscription, so prices are hidden.
	const hidePrice = isForgeOSPass
	...
	// No curated ForgeOSPass models available: show an empty state rather than other models.
	if (isForgeOSPass && modelGroups.length === 0) {
		return (
			<div className="flex w-full max-w-lg flex-col items-center justify-center my-8 px-2 text-center">
				<p className="text-foreground text-sm m-0">No ForgeOSPass models are available right now.</p>
		...
		<div className="flex flex-col w-full items-center px-2">
			<div className="flex w-full max-w-lg flex-col gap-6 my-4">
				{modelGroups.map((group) => {
					const isForgeOSPassGroup = group.group === FORGEOSPASS_GROUP
					return (
						<div className="flex flex-col gap-3" key={group.group}>
							<h4
								className={cn(
									"text-sm font-bold text-foreground/70 mb-2",
									isForgeOSPassGroup ? "normal-case" : "uppercase",
								)}>
			...
			{/* SEARCH MODEL — hidden for ForgeOSPass, whose selection is constrained to the curated list. */}
			{!isForgeOSPass && (
...
const OnboardingStepContent = ({
	...
	if (userType === NEW_USER_TYPE.FREE || userType === NEW_USER_TYPE.POWER || userType === NEW_USER_TYPE.FORGEOS_PASS) {
		return (
			<ModelSelection
				models={models}
...
const OnboardingViewContent = ({ onboardingModels }: { onboardingModels: OnboardingModelGroup }) => {
	...
	const { commitSelection } = useProviderConfig("forgeos")
	...
	const models = useMemo(() => getForgeOSUIOnboardingGroups(onboardingModels), [onboardingModels])
	// Gate on models so a fallback/empty response can't route users into the dead-end empty step.
	const showForgeOSPass = models.forgeosPass.length > 0
	const userTypeSelections = useMemo(() => getUserTypeSelections(showForgeOSPass), [showForgeOSPass])
	// ForgeOSPass model IDs (e.g. "forgeos-pass/glm-5.2") aren't keyed in openRouterModels,
	// so resolve their info via the slug-based lookup used by ForgeOSPassProvider.
	...
	useEffect(() => {
		setSearchTerm("")
		const groupKey = userType === NEW_USER_TYPE.FORGEOS_PASS ? "forgeosPass" : userType === NEW_USER_TYPE.POWER ? "power" : "free"
		// ForgeOSPass must stay within its curated list (never fall back to a free/OpenRouter model
		// under the forgeos-pass provider). Free/Frontier fall back to free if their group is empty.
		const modelGroup = userType === NEW_USER_TYPE.FORGEOS_PASS ? models[groupKey][0] : (models[groupKey][0] ?? models.free[0])
	...
	const finishOnboarding = useCallback(
		async (updateModelId: boolean, step: number, markCompleted = true) => {
			...
			// Guard: never save a non-ForgeOSPass model id under the forgeos-pass provider.
			const isForgeOSPassModel = selectedModelId.startsWith("forgeos-pass/")
			if (modelSelected) {
				if (userType === NEW_USER_TYPE.FORGEOS_PASS && isForgeOSPassModel) {
					const forgeosPassModelInfo = resolveForgeOSPassModelInfo(selectedModelId, openRouterModelsByName)
					await handleFieldsChange({
						planModeForgeOSPassModelId: selectedModelId,
						actModeForgeOSPassModelId: selectedModelId,
						planModeForgeOSPassModelInfo: forgeosPassModelInfo,
						actModeForgeOSPassModelInfo: forgeosPassModelInfo,
						planModeApiProvider: "forgeos-pass",
						actModeApiProvider: "forgeos-pass",
					})
				} else if (userType !== NEW_USER_TYPE.FORGEOS_PASS) {
					const selectedModelInfo = forgeosModels[selectedModelId] ??
					...
					await handleFieldsChange({
						planModeForgeOSModelId: selectedModelId,
						actModeForgeOSModelId: selectedModelId,
						planModeForgeOSModelInfo: selectedModelInfo,
						actModeForgeOSModelInfo: selectedModelInfo,
						planModeApiProvider: "forgeos",
						actModeApiProvider: "forgeos",
					})
				} else {
					// ForgeOSPass selected but the id isn't a forgeos-pass/ model: skip the write
					// (avoids a bad provider config) and log so the no-op is observable.
					console.error(`Skipped ForgeOSPass provider setup: unexpected model id "${selectedModelId}"`)
				}
			}
	...
	const handleFooterAction = useCallback(
		async (action: "signin" | "next" | "back" | "done" | "signup") => {
			...
			switch (action) {
				case "signup":
					// ForgeOSPass: record the intent so App opens the subscription page once auth
					// completes (App outlives this view, which unmounts on auth). Login flow unchanged.
					setPendingForgeOSPassSubscribe(userType === NEW_USER_TYPE.FORGEOS_PASS)
					captureNavigation("signup_clicked", stepNumber + 1)
					setStepNumber(stepNumber + 1)
					await loginAndFinishOnboarding(true, stepNumber + 1)
					break
				case "signin":
					setPendingForgeOSPassSubscribe(false)
				...
				case "next":
					...
					break
				case "back":
					// Abandon any pending ForgeOSPass subscription redirect when the user goes back.
					setPendingForgeOSPassSubscribe(false)
	...
	return (
		<div className="fixed inset-0 p-0 flex flex-col w-full">
			<div className="h-full px-5 xs:mx-10 overflow-auto flex flex-col gap-4 items-center justify-center">
				<ForgeOSLogoWhite className="size-16 flex-shrink-0" />
				...
				<div className="flex-1 w-full flex max-w-lg overflow-y-auto min-h-0">
					<OnboardingStepContent
						models={Object.keys(forgeosModels).length > 0 ? forgeosModels : openRouterModels}
				...
				<footer className="flex w-full max-w-lg flex-col gap-3 my-2 px-2 overflow-hidden flex-shrink-0">
					{stepDisplayInfo.buttons.map((btn) => {
						// Block ForgeOSPass signup when no ForgeOSPass model is selected (e.g. empty list).
						const isLoginAction = btn.action === "signin" || btn.action === "signup"
						const showSpinner = isActionLoading && isLoginAction
						const disabled =
							isActionLoading ||
							(btn.action === "signup" && userType === NEW_USER_TYPE.FORGEOS_PASS && !selectedModelId)
						return (
							<Button
