...
describe("ForgeOS Extension", () => {
	...
	it("should verify extension ID matches package.json", async () => {
		...
		const forgeosExtensionApi = vscode.extensions.getExtension(id)

		forgeosExtensionApi?.id.should.equal(id)
	})

	it("should successfully execute the plus button command", async () => {
		await new Promise((resolve) => setTimeout(resolve, 400))
		await vscode.commands.executeCommand("forgeos.plusButtonClicked")
	})
