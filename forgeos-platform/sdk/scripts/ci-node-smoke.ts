...
async function main(): Promise<void> {
	const packDir = await mkdtemp(join(tmpdir(), "forgeos-node-smoke-packs-"));
	const smokeDir = await mkdtemp(join(tmpdir(), "forgeos-node-smoke-"));
	const sessionsDir = await mkdtemp(join(tmpdir(), "forgeos-node-sessions-"));
	const npmCacheDir = await mkdtemp(join(tmpdir(), "forgeos-node-npm-cache-"));
	...
	try {
		...
		await writeFile(
			...
			`${JSON.stringify(
				{
					name: "forgeos-node-smoke",
					private: true,
					type: "module",
					dependencies: {
						"@forgeos/core": `file:${tarballs.core}`,
						"@forgeos/agents": `file:${tarballs.agents}`,
						"@forgeos/llms": `file:${tarballs.llms}`,
						"@forgeos/shared": `file:${tarballs.shared}`,
					},
				},
				null,
				2,
		...
		const smokeSource =
			nodeMajor >= 24
				? `
					const { SqliteSessionStore } = await import("@forgeos/core");
					const store = new SqliteSessionStore({ sessionsDir: process.env.FORGEOS_DATA_DIR });
					try {
						store.init();
				...
				`
				: `
					const { resolveSessionBackend } = await import("@forgeos/core");
