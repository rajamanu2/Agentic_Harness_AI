...
describe("SdkModeCoordinator", () => {
	...
	it("preserves composer content when the rebuild aborts on a forgeos auth error", async () => {
		const activeSession = makeActiveSession()
		const task = makeTask("old-session", planMessages())
		const { coordinator, options, state } = makeCoordinator({
			activeSession,
		...
		expect(options.emitForgeOSAuthError).toHaveBeenCalledOnce()
		expect(options.sessions.fireAndForgetSend).not.toHaveBeenCalled()
		expect(options.messages.appendAndEmit).not.toHaveBeenCalled()
	...
	it("emits an auth error and skips replacement when the target forgeos provider has no token", async () => {
		const activeSession = makeActiveSession()
		const { coordinator, options, state } = makeCoordinator({
			activeSession,
			config: {
	...
	it("cancels and finalizes a running turn before rebuilding for mode change", async () => {
		...
		expect(options.messages.finalizeMessagesForSave).toHaveBeenCalledWith(task.messageStateHandler.getForgeOSMessages())
...
function makeTask(taskId: string, messages: Array<Partial<ForgeOSMessage>> = []) {
	return {
		taskId,
		messageStateHandler: {
			getForgeOSMessages: vi.fn(() => messages as ForgeOSMessage[]),
		},
	} as unknown as {
		taskId: string
		messageStateHandler: { getForgeOSMessages: () => ForgeOSMessage[] }
	}
}

function planMessages(): Array<Partial<ForgeOSMessage>> {
