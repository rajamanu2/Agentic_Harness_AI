...
describe("SdkTaskControlCoordinator", () => {
	...
	it("cancels a running ForgeOS task when the user signs out", async () => {
		const activeSession = makeActiveSession()
		const { coordinator, options } = makeCoordinator({ activeSession })

		await coordinator.cancelForgeOSTaskOnSignOut(true)

		expect(activeSession.sdkHost.abort).toHaveBeenCalledWith("session-123")
		expect(options.sessions.setRunning).toHaveBeenCalledWith(false)
	})

	it("does not cancel a non-ForgeOS task when the user signs out", async () => {
		const activeSession = makeActiveSession()
		const { coordinator, options } = makeCoordinator({ activeSession })

		await coordinator.cancelForgeOSTaskOnSignOut(false)
	...
	it("shows a task by creating a proxy, loading messages, and appending a fresh resume ask", async () => {
		...
		const activeSession = makeActiveSession()
		const sdkForgeOSMessages: ForgeOSMessage[] = [
		...
		const { coordinator, options, state } = makeCoordinator({
			...
			task: existingTask,
			hasHistoryItem: true,
			forgeosMessages: sdkForgeOSMessages,
		...
		expect(options.taskHistory.getForgeOSMessages).toHaveBeenCalledWith("task-1")
		expect(state.task?.messageStateHandler.getForgeOSMessages()).toEqual([
			{ ts: 1, type: "say", say: "task", text: "hello" },
			{ ts: 2, type: "ask", ask: "completion_result", text: "" },
	...
	it("clears a pending approval when switching tasks so the new task input is not consumed", async () => {
		...
		await vi.waitFor(() => expect(pendingTask.messageStateHandler.getForgeOSMessages()).toHaveLength(1))
	...
	it("shows a legacy task with a warning and a resume ask", async () => {
		const legacyMessages: ForgeOSMessage[] = [{ ts: 1, type: "say", say: "task", text: "legacy task" }]
		const { coordinator, options, state } = makeCoordinator({
			hasHistoryItem: true,
			forgeosMessages: legacyMessages,
			isLegacyTask: true,
		})
	...
	it("does not show a task that is missing from history", async () => {
		...
		expect(options.taskHistory.getForgeOSMessages).not.toHaveBeenCalled()
	...
	it("appends a resume ask and sets the resumable phase when showing an interrupted (cancelled) task", async () => {
		...
		expect(state.task?.messageStateHandler.getForgeOSMessages().at(-1)).toEqual(
			expect.objectContaining({ type: "ask", ask: "resume_task" }),
		)
	...
	it("keeps the newest selection when an older open's history lookup resolves last", async () => {
		const { coordinator, options, state } = makeCoordinator({
			hasHistoryItem: true,
			forgeosMessages: [{ ts: 1, type: "say", say: "task", text: "hello" }],
			sessionStatus: "cancelled",
		})
	...
	it("abandons a superseded showTaskWithId so the newest selection wins", async () => {
		...
		let resolveFirstRead: ((messages: ForgeOSMessage[]) => void) | undefined
		options.taskHistory.getForgeOSMessages.mockImplementationOnce(
			() =>
				new Promise<ForgeOSMessage[]>((resolve) => {
		...
		expect(state.task?.taskId).toBe("task-new")
		expect(state.task?.messageStateHandler.getForgeOSMessages().length).toBeGreaterThan(0)
	...
	it("abandons a superseded showTaskWithId when the user clears the task", async () => {
		...
		let resolveRead: ((messages: ForgeOSMessage[]) => void) | undefined
		options.taskHistory.getForgeOSMessages.mockImplementationOnce(
			() =>
				new Promise<ForgeOSMessage[]>((resolve) => {
					resolveRead = resolve
	...
	it("does not install the new task proxy until its messages are loaded", async () => {
		...
		let resolveGetForgeOSMessages: ((messages: ForgeOSMessage[]) => void) | undefined
		const getForgeOSMessagesDeferred = new Promise<ForgeOSMessage[]>((resolve) => {
			resolveGetForgeOSMessages = resolve
		})
		...
		options.taskHistory.getForgeOSMessages.mockReturnValueOnce(getForgeOSMessagesDeferred)

		let setTaskHadMessages: boolean | undefined
		options.setTask.mockImplementation((task: any) => {
			setTaskHadMessages = (task?.messageStateHandler?.getForgeOSMessages?.() ?? []).length > 0
			state.task = task
		})

		const inFlight = coordinator.showTaskWithId("task-1")

		// While getForgeOSMessages is still pending, the new task proxy must not be
		...
		resolveGetForgeOSMessages?.(sdkForgeOSMessages)
		await inFlight
...
function makeCoordinator(input: Partial<MakeCoordinatorInput> = {}) {
	...
	const options = {
		...
		messages: {
			...
			finalizeMessagesForSave: vi.fn((messages: ForgeOSMessage[]) =>
		...
		taskHistory: {
			getForgeOSMessages: vi.fn().mockResolvedValue(input.forgeosMessages ?? []),
		...
		taskHistory: SdkTaskControlCoordinatorOptions["taskHistory"] & {
			...
			getForgeOSMessages: ReturnType<typeof vi.fn>
...
function makeTask(taskId: string, messages: ForgeOSMessage[] = []) {
	return {
		taskId,
		messageStateHandler: {
			getForgeOSMessages: vi.fn(() => messages),
			clear: vi.fn(),
		},
	}
}
