...
async function actionCreate(client: HubScheduleClient): Promise<void> {
	...
	const prompt = await p.text({
		message: "What should ForgeOS do?",
...
export async function runScheduleWizard(): Promise<number> {
	...
	const address = resolveAddress(process.env.FORGEOS_HUB_ADDRESS);
	...
	if (!ensured.ok) {
		...
		p.log.error(
			"Schedules require the hub server. Start it with: forgeos hub start",
		);
		p.outro("Failed");
		return 1;
	}
