import type { ConsecutiveMistakeLimitContext, ConsecutiveMistakeLimitDecision } from "@forgeos/shared"
import type { ForgeOSAskQuestion, ForgeOSMessage, TurnPhase } from "@shared/ExtensionMessage"
...
export class SdkInteractionCoordinator {
	...
	async handleConsecutiveMistakeLimitReached(
		...
		const errorMessage: ForgeOSMessage = {
			...
			type: "say",
			say: "error",
			text: `ForgeOS ran into ${context.consecutiveMistakes} errors in a row and stopped the task.\n\nLatest: ${latest}\n\nSend a message to give ForgeOS guidance and continue the task.`,
			partial: false,
		}
	...
	async handleRequestToolApproval(request: ToolApprovalRequest): Promise<{ approved: boolean; reason?: string }> {
		...
		const toolAskMessage: ForgeOSMessage = buildToolApprovalAskMessage(
			request.toolName,
			request.input,
			this.nextMessageTs(),
			this.options.getCwd?.(),
	...
	async handleAskQuestion(question: string, options: string[], _context: unknown): Promise<string> {
		const askData: ForgeOSAskQuestion = {
			question,
			options: options?.length ? options : undefined,
		}
		const askMessage: ForgeOSMessage = {
			ts: this.nextMessageTs(),
			type: "ask",
			ask: "followup",
	...
	resolvePendingToolApproval(
		...
		responseType: ForgeOSAskResponse | undefined,
		images?: string[],
		files?: string[],
	): boolean {
