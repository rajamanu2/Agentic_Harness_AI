import { readTuiThemeGlobally, setTuiThemeGlobally } from "@forgeos/core";
...
/**
 * Resolves the theme to boot with: FORGEOS_THEME env override first, then the
 * persisted setting, falling back to terminal auto-detection.
 */
export function getInitialThemeId(): string {
	const fromEnv = process.env.FORGEOS_THEME?.trim();
	if (fromEnv) {
