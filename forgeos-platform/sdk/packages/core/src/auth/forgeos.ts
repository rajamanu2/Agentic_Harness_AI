...
export type ForgeOSTokenResolution = {
	forceRefresh?: boolean;
	refreshBufferMs?: number;
	retryableTokenGraceMs?: number;
};

interface ForgeOSAuthApiUser {
	subject: string | null;
	email: string;
	name: string;
	forgeosUserId: string | null;
	accounts: string[] | null;
}

interface ForgeOSAuthResponseData {
	accessToken: string;
	refreshToken?: string;
	tokenType: string;
	expiresAt: string;
	userInfo: ForgeOSAuthApiUser;
}

type ForgeOSTokenResponse = {
	success: boolean;
	data: ForgeOSAuthResponseData;
};
...
export interface ForgeOSOAuthProviderOptions {
	apiBaseUrl: string;
	headers?: HeaderInput;
...
export interface ForgeOSOAuthCredentials extends OAuthCredentials {
	metadata?: {
		provider?: string;
		tokenType?: string;
		userInfo?: ForgeOSAuthApiUser;
...
class ForgeOSOAuthTokenError extends Error {
	...
	constructor(
		...
		super(message);
		this.name = "ForgeOSOAuthTokenError";
...
function toForgeOSCredentials(
	responseData: ForgeOSAuthResponseData,
	provider: string | undefined,
	fallback: Partial<ForgeOSOAuthCredentials> = {},
): ForgeOSOAuthCredentials {
	const accountId = responseData.userInfo.forgeosUserId ?? fallback.accountId;
...
function getAuthCredentialTelemetryProperties(
	credentials: ForgeOSOAuthCredentials,
): AuthCredentialTelemetryProperties {
...
function requireForgeOSTokenResponse(
	payload: ForgeOSTokenResponse,
	message: string,
): ForgeOSAuthResponseData {
...
async function requestWorkOSDeviceAuthorization(
	...
	if (!response.ok) {
		throw new ForgeOSOAuthTokenError(
...
async function registerWorkOSTokens(
	...
	options: ForgeOSOAuthProviderOptions,
	provider?: string,
): Promise<ForgeOSOAuthCredentials> {
	const body = {
	...
	const json = (await response.json()) as ForgeOSTokenResponse;
	return toForgeOSCredentials(
		requireForgeOSTokenResponse(json, "Invalid token exchange response"),
...
export async function loginForgeOSOAuth(
	options: ForgeOSOAuthProviderOptions & {
		callbacks: OAuthLoginCallbacks;
	},
): Promise<ForgeOSOAuthCredentials> {
	captureAuthStarted(options.telemetry, options.provider ?? "forgeos");
	...
	try {
		let credentials: ForgeOSOAuthCredentials;
		if (useWorkOSDeviceAuth) {
			const clientId = getForgeOSEnvironmentConfig().workOsClientId;
		...
		captureAuthFailed(
			...
			{
				requestId:
					error instanceof ForgeOSOAuthTokenError ? error.requestId : undefined,
			},
		);
		throw error;
	} finally {
...
export async function startForgeOSDeviceAuth(options?: {
	requestTimeoutMs?: number;
}): Promise<{
	deviceCode: string;
	userCode: string;
	...
	return await requestWorkOSDeviceAuthorization(
		getForgeOSEnvironmentConfig().workOsClientId,
		options,
	);
}

export async function completeForgeOSDeviceAuth(options: {
	deviceCode: string;
	expiresInSeconds: number;
	pollIntervalSeconds: number;
	apiBaseUrl: string;
	...
	const providerName = options.provider ?? "forgeos";
	captureAuthStarted(options.telemetry, providerName);
	try {
		const workosTokens = await pollWorkOSTokens({
			clientId: getForgeOSEnvironmentConfig().workOsClientId,
...
export async function refreshForgeOSToken(
	current: ForgeOSOAuthCredentials,
	options: ForgeOSOAuthProviderOptions,
): Promise<ForgeOSOAuthCredentials> {
	...
	sdkDebug(
		`forgeos.refresh.request url=${refreshUrl} refreshTokenHash=${hashSecret(current.refresh)}`,
	);
	...
	if (!response.ok) {
		...
		sdkDebug(
			`forgeos.refresh.error status=${response.status} errorCode=${details.code ?? "none"} message=${details.message ?? "none"} requestId=${requestId ?? "none"}`,
		);
		throw new ForgeOSOAuthTokenError(
	...
	const result = toForgeOSCredentials(
		requireForgeOSTokenResponse(json, "Invalid token refresh response"),
		provider,
		current,
	);
	sdkDebug(
		`forgeos.refresh.success newAccessTokenHash=${hashSecret(result.access)} newRefreshTokenHash=${hashSecret(result.refresh)} expires=${result.expires}`,
	);
	return result;
}

/**
 * Resolve valid ForgeOS credentials, refreshing when needed.
 *
 ...
 */
export async function getValidForgeOSCredentials(
	currentCredentials: ForgeOSOAuthCredentials | null,
	providerOptions: ForgeOSOAuthProviderOptions,
	options?: ForgeOSTokenResolution,
): Promise<ForgeOSOAuthCredentials | null> {
	if (!currentCredentials) {
		sdkDebug("forgeos.getCredentials outcome=no_current_credentials");
		return null;
	}
	...
	if (
		...
		sdkDebug(
			`forgeos.getCredentials outcome=still_valid forceRefresh=${forceRefresh}`,
		);
		return currentCredentials;
	}

	sdkDebug(
		`forgeos.getCredentials outcome=needs_refresh forceRefresh=${forceRefresh} accessTokenHash=${hashSecret(currentCredentials.access)}`,
	);

	try {
		return await refreshForgeOSToken(currentCredentials, providerOptions);
	} catch (error) {
		const authTelemetryDetails = getAuthTelemetryDetails(currentCredentials);
		const requestIdDetails =
			error instanceof ForgeOSOAuthTokenError && error.requestId
				? { request_id: error.requestId }
				: {};
		const failureDetails = {
			status: error instanceof ForgeOSOAuthTokenError ? error.status : undefined,
			errorCode:
				error instanceof ForgeOSOAuthTokenError ? error.errorCode : undefined,
			...requestIdDetails,
			...authTelemetryDetails,
			errorName: error instanceof Error ? error.name : undefined,
		};
		if (error instanceof ForgeOSOAuthTokenError && error.isLikelyInvalidGrant()) {
			sdkDebug(
				`forgeos.getCredentials outcome=invalid_grant status=${error.status} errorCode=${error.errorCode ?? "none"}`,
			);
			// This is the single owner of the involuntary-logout event for the
			// ForgeOS provider — callers observing the `null` return must NOT
			...
			captureAuthLoggedOut(
				...
				providerOptions.provider ?? "forgeos",
				"token_invalid",
				{
					status: error.status,
		...
		if (currentCredentials.expires - Date.now() > retryableTokenGraceMs) {
			...
			sdkDebug(
				`forgeos.getCredentials outcome=transient_failure_kept_current error=${error instanceof Error ? error.message : String(error)}`,
			);
			captureAuthRefreshSoftFailure(
				providerOptions.telemetry,
				providerOptions.provider ?? "forgeos",
		...
		// credentials over it, logging out every ForgeOS process on the machine.
		sdkDebug(
			`forgeos.getCredentials outcome=transient_failure_rethrown error=${error instanceof Error ? error.message : String(error)}`,
		);
