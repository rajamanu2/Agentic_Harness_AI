...
import {
	...
	ToggleForgeOSRuleRequest,
	ToggleCursorRuleRequest,
	ToggleSkillRequest,
	ToggleWindsurfRuleRequest,
	ToggleWorkflowRequest,
} from "@shared/proto/forgeos/file"
...
const ForgeOSRulesToggleModal: React.FC = () => {
	const {
		globalForgeOSRulesToggles = {},
		...
		hooksEnabled,
		setGlobalForgeOSRulesToggles,
		setLocalForgeOSRulesToggles,
		...
		if (isVisible) {
			FileServiceClient.refreshRules({} as EmptyRequest)
				.then((response: RefreshedRules) => {
					...
					if (response.globalForgeOSRulesToggles?.toggles) {
						setGlobalForgeOSRulesToggles(response.globalForgeOSRulesToggles.toggles)
					}
					if (response.localForgeOSRulesToggles?.toggles) {
						setLocalForgeOSRulesToggles(response.localForgeOSRulesToggles.toggles)
					}
	...
	const globalRules = Object.entries(globalForgeOSRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))

	// Format local rules for display with proper typing
	const localRules = Object.entries(localForgeOSRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleForgeOSRule(
			ToggleForgeOSRuleRequest.create({
			...
			.catch((error) => {
				console.error("Error toggling ForgeOS rule:", error)
			})
	}
	...
	const toggleWindsurfRule = (rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleWindsurfRule(
			...
			.then((response: ForgeOSRulesToggles) => {
				if (response.toggles) {
				...
				<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition}>
					...
					<div className="flex-shrink-0 px-3 pt-2">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									Rules allow you to provide ForgeOS with system-level guidance. Think of them as a persistent way
									to include context and preferences for your projects or globally for every conversation.{" "}
									<VSCodeLink
										className="text-xs"
										href="https://docs.forgeos.bot/features/forgeos-rules"
								...
								<p>
									Workflows allow you to define a series of steps to guide ForgeOS through a repetitive set of
									...
									<VSCodeLink
										...
										href="https://docs.forgeos.bot/features/slash-commands/workflows">
										Docs
									</VSCodeLink>
								</p>
							) : currentView === "skills" ? (
								<p>
									Skills are reusable instruction sets that ForgeOS can activate on-demand. When a task matches a
									skill's description, ForgeOS uses the <span className="font-bold">use_skill</span> tool to load
									the full instructions.
								</p>
							) : (
								<p>
									Hooks allow you to execute custom scripts at specific points in ForgeOS's execution lifecycle,
							...
							<>
								...
								<div className="flex items-center gap-2 px-3 py-3 mb-4 bg-vscode-inputValidation-warningBackground border-l-[3px] border-vscode-inputValidation-warningBorder">
									...
									<span className="text-base">
										...
										<VSCodeLink
											href="https://docs.forgeos.bot/customization/skills"
											style={{ display: "inline", fontSize: "inherit" }}>
											Learn more
										</VSCodeLink>
									</span>
							...
							<>
								<div className="text-xs text-description mb-4">
									<p>
										...
										<VSCodeLink
											className="text-xs"
											href="https://docs.forgeos.bot/features/hooks"
											style={{ display: "inline", fontSize: "inherit" }}>
											Docs
										</VSCodeLink>
									</p>
								...
								{workspaceHooks.map((workspace, index) => (
									<div
										...
										<div className="text-sm font-normal mb-2">
											{workspace.workspaceName}/.forgeosrules/hooks/
										</div>
										<div className="flex flex-col gap-0">
											{workspace.hooks
												.sort((a, b) => a.name.localeCompare(b.name))
...
export default ForgeOSRulesToggleModal
