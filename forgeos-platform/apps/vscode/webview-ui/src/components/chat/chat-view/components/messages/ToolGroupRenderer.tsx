import { ForgeOSMessage, ForgeOSSayTool } from "@shared/ExtensionMessage"
...
const getCurrentActivities = (allMessages: ForgeOSMessage[]): ForgeOSMessage[] => {
	...
	const activities: ForgeOSMessage[] = []
 ...
 */
export function buildToolsWithReasoning(messages: ForgeOSMessage[]): ToolWithReasoning[] {
 ...
 */
function parseToolSafe(text: string | undefined): ForgeOSSayTool {
	try {
		return JSON.parse(text || "{}") as ForgeOSSayTool
	} catch {
		return {} as ForgeOSSayTool
	}
}
 ...
 */
function getToolDisplayInfo(tool: ForgeOSSayTool) {
 ...
 */
export function getToolGroupSummaryFromParsedTools(tools: ForgeOSSayTool[]): string {
	...
	return parts.length === 0 ? "Context" : "ForgeOS" + action + parts.join(", ")
}
