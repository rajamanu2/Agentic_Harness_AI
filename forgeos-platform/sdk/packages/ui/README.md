...
Shared visual foundations and reusable React presentation primitives for ForgeOS
...
`npm view @forgeos/ui version`; an `E404` means the first release is still pending.
...
Use `@forgeos/ui@next` only for deliberate previews. Monorepo consumers use
`"@forgeos/ui": "workspace:*"` instead.

## Entry points

| Import | Contents | Runtime requirement |
| --- | --- | --- |
| `@forgeos/ui` | Button, icon-button, agent ask-question, approval-card, Aurora, hero-heading, prompt-queue, quick-action, search-combobox, and session-status React primitives | React 18.3 or 19 and Tailwind v4 |
| `@forgeos/ui/components.css` | Styles, namespaced Tailwind mappings, and source registration for the root React primitives | Tailwind v4 and theme tokens |
| `@forgeos/ui/theme/palette.css` | ForgeOS-owned light/dark solid and alpha color scales | CSS |
| `@forgeos/ui/theme/tokens.css` | Light/dark custom properties only | CSS |
| `@forgeos/ui/theme/scoped-tokens.css` | Light/dark custom properties scoped to `.forgeos-ui-theme` | CSS |
| `@forgeos/ui/theme/theme.css` | Tailwind v4 semantic mapping and dark variant | Tailwind v4 |
| `@forgeos/ui/components/markdown.css` | Optional framework-neutral Markdown and Streamdown treatment | Theme tokens |
| `@forgeos/ui/theme/base.css` | Optional document, Markdown, scrollbar, selection, and cursor styles | Tailwind v4 |
| `@forgeos/ui/theme/index.css` | Complete theme: tokens, Tailwind mapping, and base styles | Tailwind v4 |
| `@forgeos/ui/components/agent-chat` | Conversation, message, reasoning, action, and tool-activity React primitives | React 18.3 or 19 |
| `@forgeos/ui/components/agent-chat.css` | Framework-neutral styles for the agent-chat primitives | Theme tokens |

`SessionStatus` uses semantic tone colors by default. Set
`--forgeos-ui-session-status-color` on the component to override its dot color
...
```tsx
import { Button, IconButton } from "@forgeos/ui";
...
@import "@forgeos/ui/components/markdown.css";
```

```tsx
<section className="forgeos-ui-theme">
	<div className="forgeos-markdown">...</div>
</section>
```

Dark values activate when `.dark` is on the wrapper or an ancestor.

Embedded hosts should not import `@forgeos/ui/theme/theme.css`; it intentionally
maps generic Tailwind names such as `bg-background` for ForgeOS-owned surfaces.
...
normal Tailwind utilities inherit ForgeOS defaults without custom adapters.

Theme authors work through three layers:

1. ForgeOS-owned 12-step solid and alpha palettes: Slate as `--neutral-*`,
...
license; `@forgeos/ui` does not depend on Radix Colors at runtime.
...
Storybook currently runs from a ForgeOS monorepo checkout. It is not hosted or
included in the npm package; deployment can be added once the catalog and
ownership model settle.

## Layering and compatibility

- Import the ForgeOS theme after Tailwind so its default typography values win.
