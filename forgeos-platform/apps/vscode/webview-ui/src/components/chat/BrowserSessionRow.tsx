...
import { BrowserAction, BrowserActionResult, ForgeOSMessage, ForgeOSSayBrowserAction } from "@shared/ExtensionMessage"
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: ForgeOSMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: ForgeOSMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: ForgeOSMessage[] = []
		let nextActionMessages: ForgeOSMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					{isAutoApproved ? "ForgeOS is using the browser:" : "ForgeOS wants to use the browser:"}
				</span>
			</div>
			<div
				style={{
