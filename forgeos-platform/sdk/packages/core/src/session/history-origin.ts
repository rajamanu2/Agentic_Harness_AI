import type { ClientContext } from "@forgeos/shared";
import {
	SessionSource,
...
export function resolveClientSessionSource(
	...
	if (identity.includes("forgeos-platform")) {
		return SessionSource.WEB;
	}
	if (identity.includes("forgeos-cli") || identity.includes("forgeos-acp")) {
		return SessionSource.CLI;
	}
	if (identity.includes("forgeos-sdk") || identity.includes("forgeos-core")) {
