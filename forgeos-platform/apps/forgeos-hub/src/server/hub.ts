...
import type { HubUINotifyPayload } from "@forgeos/shared";
...
export async function attachHub(ctx: HubContext): Promise<void> {
	...
	ctx.forgeos = await ForgeOSCore.create({
		clientName: "forgeos-hub",
		backendMode: "hub",
		capabilities: {
		...
		hub: {
			...
			clientType: "forgeos-hub-chat",
			displayName: "ForgeOS Hub Chat",
			workspaceRoot,
		},
	});
	...
	ctx.uiClient = new HubUIClient({
		address: ctx.hubUrl,
		authToken: ctx.hubAuthToken,
		clientType: "forgeos-hub-server",
		displayName: "ForgeOS Hub Server",
	});
	...
	ctx.forgeos.subscribe((event) => handleSessionEvent(ctx, event));
...
export async function detachHub(ctx: HubContext): Promise<void> {
	...
	try {
		await ctx.forgeos?.dispose();
	} catch {
		// ignore
	}
	ctx.forgeos = undefined;
	ctx.clients.clear();
	ctx.sessions.clear();
