...
function ForgeOSPassBrowserPageContent(
...
export function ForgeOSPassSubscriptionContent(
	props: ChoiceContext<boolean> & {
		providerName: string;
	},
) {
	const subscriptionUrl = useMemo(
		() =>
			buildForgeOSPassSubscriptionPageUrl(getForgeOSEnvironmentConfig().appBaseUrl),
		[],
	);

	return (
		<ForgeOSPassBrowserPageContent
			{...props}
...
export function OAuthLoginContent(
	...
	const [mode, setMode] = useState<"browser" | "device">(
		providerId === "forgeos" ? "device" : "browser",
	);
	const [status, setStatus] = useState("Opening browser...");
...
export function OAuthApiKeyInputContent(
	...
	return (
		<box flexDirection="column" paddingX={1} gap={1}>
			...
			<text fg="gray">
				Use an API key from your ForgeOS dashboard instead of OAuth login. This
