...
export function usesForgeOSAccount(providerId: string): boolean {
...
interface ForgeOSAccountInput {
	apiKey: string;
	providerSettingsManager: ProviderSettingsManager;
}

// ForgeOS access tokens expire between runs, so account requests resolve
...
function createAccountService(input: ForgeOSAccountInput): ForgeOSAccountService {
	const { providerSettingsManager } = input;
	const settings = providerSettingsManager.getProviderSettings("forgeos");
	return new ForgeOSAccountService({
		...
		getAuthToken: async () => {
			...
			return (
				getPersistedProviderApiKey(
					"forgeos",
					providerSettingsManager.getProviderSettings("forgeos"),
				) ||
				input.apiKey ||
				undefined
			);
...
export async function fetchForgeOSOrganizations(
	input: ForgeOSAccountInput,
...
export function buildOrganizationConfigOption(
	...
	return {
		...
		name: "Account",
		description:
			"The ForgeOS account usage is billed to — your personal account or an organization",
		category: "account",
...
export async function switchForgeOSOrganization(
	input: ForgeOSAccountInput & { organizationId: string | null },
): Promise<void> {
	const service = createAccountService(input);
...
export function getAcpOrgSubscriptionMessage(): string {
	return [
		"Organization accounts cannot use ForgeOSPass subscriptions.",
		'Switch the "Account" session option to Personal to keep using ForgeOSPass,',
		'or switch the "Provider" option to ForgeOS to bill your organization.',
	].join(" ");
}
