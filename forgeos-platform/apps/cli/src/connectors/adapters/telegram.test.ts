...
import type { ConnectTelegramOptions } from "@forgeos/shared";
...
const originalForgeOSDataDir = process.env.FORGEOS_DATA_DIR;
...
function useTempForgeOSDataDir(): string {
	const dataDir = mkdtempSync(join(tmpdir(), "forgeos-telegram-test-"));
...
describe("telegramConnector", () => {
	...
	it("rejects mixing --allowed-user-id with the hook command env var", () => {
		const originalHookCommand = process.env.FORGEOS_CONNECT_HOOK_COMMAND;
		process.env.FORGEOS_CONNECT_HOOK_COMMAND = "echo noop";
		try {
			expect(() =>
				parseTelegramArgs([
					"--bot-token",
			...
			if (originalHookCommand === undefined) {
				delete process.env.FORGEOS_CONNECT_HOOK_COMMAND;
			} else {
				process.env.FORGEOS_CONNECT_HOOK_COMMAND = originalHookCommand;
			}
		}
	});
	...
	it("validates a token before reporting its connector as already running", async () => {
		const dataDir = useTempForgeOSDataDir();
...
describe("telegram slash command delivery", () => {
	it("receives bot_command updates intercepted by the telegram chat library", async () => {
		...
		const dir = mkdtempSync(join(tmpdir(), "forgeos-telegram-slash-"));
