...
Routes ForgeOS inference through the [VS Code Language Model API](https://code.visualstudio.com/api/extension-guides/language-model)
(`vscode.lm`), letting ForgeOS use models contributed by any extension that
...
- **`vscode-lm-handler.ts`** — `VsCodeLmHandler`, a ForgeOS SDK `ApiHandler`
  (`@forgeos/llms`) backed by `vscode.lm`. Selects a chat model, streams the
...
The SDK's `@forgeos/llms` handler registry (`registerHandler`) exists for providers
