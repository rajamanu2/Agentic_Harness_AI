...
import type { HubSessionClient, HubSessionRow } from "@forgeos/core";
import { ensureParentDir, resolveForgeOSDataDir } from "@forgeos/core";
import {
...
export const FORGEOS_CONNECTOR_DETACHED_CHILD_ENV =
	"FORGEOS_CONNECTOR_DETACHED_CHILD";
...
function buildDetachedConnectorEnv(
	...
	const childEnv = {
		...withResolvedForgeOSBuildEnv(env),
		[childEnvKey]: "1",
		[FORGEOS_CONNECTOR_DETACHED_CHILD_ENV]: "1",
	};
	delete childEnv[FORGEOS_RUN_AS_HUB_DAEMON_ENV];
	return childEnv;
}
 ...
 *
 * Uses O_EXCL so two concurrent `forgeos connect` launches cannot both observe
