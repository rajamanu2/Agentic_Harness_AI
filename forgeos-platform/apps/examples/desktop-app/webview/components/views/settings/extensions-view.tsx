...
const sectionDescriptions: Record<CustomizationSection, string> = {
	Rules: "Review project and global rule files that shape ForgeOS behavior.",
...
const sectionCommands: Record<CustomizationSection, string> = {
	Rules: "forgeos config rules",
	Hooks: "forgeos config hooks",
	MCP: "forgeos mcp install",
	Skills: "forgeos skill",
	Agents: "forgeos config agents",
	Plugins: "forgeos plugin",
	Tools: "forgeos config tools",
};
	...
	const { projectRules, globalRules } = useMemo(() => {
		...
		for (const rule of rules) {
			...
			if (
				...
				normalized.includes("/.forgeosrules/")
			) {
				project.push(rule);
			} else {
				global.push(rule);
	...
	const { projectHooks, globalHooks } = useMemo(() => {
		...
		for (const hook of hooks) {
			...
			if (
				normalizedRoot &&
				normalized.startsWith(`${normalizedRoot}/`) &&
				normalized.includes("/.forgeosrules/hooks")
			) {
				project.push(hook);
			} else {
				global.push(hook);
	...
	const { projectPlugins, globalPlugins } = useMemo(() => {
		...
		for (const plugin of plugins) {
			...
			if (
				normalizedRoot &&
				normalized.startsWith(`${normalizedRoot}/`) &&
				normalized.includes("/.forgeos/plugins")
			) {
				project.push(plugin);
			} else {
				global.push(plugin);
