...
import type { BasicLogger } from "@forgeos/shared";
...
describe("createOpenTelemetryTelemetryService", () => {
	...
	it("does not create configured telemetry when globally opted out", () => {
		...
		telemetry.updateMetadata({ forgeos_type: "cli-updated" });
	...
	it("propagates updateMetadata to OTEL logger output", async () => {
		...
		telemetry.updateMetadata({ forgeos_type: "kanban-updated" });
		...
		expect(emittedAttributes.forgeos_type).toBe("kanban-updated");
