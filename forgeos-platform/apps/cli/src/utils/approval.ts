...
import type { ToolApprovalRequest, ToolApprovalResult } from "@forgeos/shared";
...
async function requestDesktopToolApprovalFromCore(
	...
	if (!cachedDesktopApprovalRequester) {
		cachedDesktopApprovalRequester = import("@forgeos/core")
			.then((module) => {
				const fn = (
					module as {
				...
				if (typeof fn !== "function") {
					throw new Error(
						"Installed @forgeos/core does not expose requestDesktopToolApproval",
					);
				}
				return fn;
			})
	...
	const approvalDir = process.env.FORGEOS_TOOL_APPROVAL_DIR?.trim();
...
export async function requestToolApproval(
	...
	const mode = process.env.FORGEOS_TOOL_APPROVAL_MODE?.trim().toLowerCase();
	if (mode === "desktop") {
