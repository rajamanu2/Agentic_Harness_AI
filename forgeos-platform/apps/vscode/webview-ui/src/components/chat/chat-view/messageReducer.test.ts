...
function msg(ts: number, seq: number, epoch: number, partial = false, text = `m${ts}`): ForgeOSMessage {
...
describe("messageReducer — deterministic", () => {
	...
	it("RC2: a same-epoch snapshot omitting the last message does NOT shrink the transcript", () => {
		...
		const ask: ForgeOSMessage = { ts: 3, type: "ask", ask: "completion_result", text: "", seq: 3, epoch: 1 }
	...
	it("unstamped (classic/legacy) messages merge at epoch 0 by ts", () => {
		let s = createReplicaState()
		const a: ForgeOSMessage = { ts: 1, type: "say", say: "text", text: "a" }
		const b: ForgeOSMessage = { ts: 2, type: "say", say: "text", text: "b" }
		s = applyMessage(s, a)
		s = applyMessage(s, b)
	...
	function canonical(log: ForgeOSMessage[]): ReplicaState {
		let s = createReplicaState()
		for (const m of log) {
			s = applyMessage(s, m)
		}
	...
	describe("messageReducer — order independence", () => {
		...
		it("dropping non-final copies still converges once finals arrive in any order", () => {
			const finalsByTs = new Map<number, ForgeOSMessage>()
			for (const m of log) {
	...
	describe("messageReducer — transcript is never emptied for a live conversation", () => {
		...
		it("a lone newer-epoch partial does NOT discard a populated transcript", () => {
			...
			const stray: ForgeOSMessage = { ts: 99, type: "say", say: "api_req_started", text: "{}", seq: 1, epoch: 2 }
