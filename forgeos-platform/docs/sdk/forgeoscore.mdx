---
title: "ForgeOSCore"
sidebarTitle: "ForgeOSCore"
description: "The full ForgeOS harness: built-in tools, sessions, approvals, scheduling, and hub support."
---

`ForgeOSCore` is the full ForgeOS harness as a programmable runtime. It gives you everything ForgeOS ships out of the box: built-in tools for files, shell, search, and web; session persistence and message history; tool approval callbacks; local, hub, and remote backends; scheduling and automation APIs; and plugin support.

Under the hood, `ForgeOSCore` uses the `Agent` class from `@forgeos/agents`. You can use `Agent` directly if you want to skip the harness and wire everything yourself: your own tools, your own persistence, your own lifecycle.

## When to use which

| Need | Use |
|------|-----|
| Sessions, persistence, message history | `ForgeOSCore` |
| Built-in tools for files, shell, search, web fetch | `ForgeOSCore` |
| Hub/remote runtime | `ForgeOSCore` |
| Scheduling or event automation | `ForgeOSCore` |
| Multi-agent teams | `ForgeOSCore` |
...
`ForgeOSCore` wraps runtime execution with application features:
...
| Method | Purpose |
|--------|---------|
| `ForgeOSCore.create(options)` | Create the runtime |
...
See [ForgeOSCore reference](/sdk/reference/forgeos-core) for exact signatures.
...
`ForgeOSCore` stores session manifests and messages as files. A session result includes:
...
`Agent` (also exported as `AgentRuntime`) is the stateless primitive that `ForgeOSCore` builds on. Use it directly when you want full control or don't need the harness.
