...
import type { ForgeOSAccountOrganization } from "@forgeos/core";
import type { ChoiceContext } from "@opentui-ui/dialog";
import { useDialogKeyboard } from "@opentui-ui/dialog/react";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import {
	type ForgeOSAccountSnapshot,
	formatForgeOSCredits,
	isForgeOSAccountAuthErrorMessage,
...
type AccountState =
	...
	| { status: "loaded"; snapshot: ForgeOSAccountSnapshot }
...
const LOADED_ACTIONS: AccountAction[] = [
	{
		id: "change-model",
		label: "Change model",
		description: "Open the ForgeOS model selector",
		enabled: true,
	},
	{
		id: "change-account",
...
const UNAUTHENTICATED_ACTIONS: AccountAction[] = [
	{
		...
		description: "Use ForgeOS OAuth",
		enabled: true,
	},
	{
		id: "learn-more",
		label: "Learn more",
		description: "Open forgeos.bot",
		enabled: true,
	},
];
...
function userInitial(snapshot: ForgeOSAccountSnapshot): string {
	const candidate =
...
function accountActions(snapshot: ForgeOSAccountSnapshot): AccountAction[] {
...
function organizationDescription(org: ForgeOSAccountOrganization): string {
...
export function AccountDialogContent(
	props: ChoiceContext<AccountDialogAction> & {
		loadAccount: () => Promise<ForgeOSAccountSnapshot>;
	...
	const reload = useCallback(async () => {
		...
		try {
			...
			if (generation.current === currentGeneration) {
				setState({
					status: isForgeOSAccountAuthErrorMessage(message)
						? "unauthenticated"
						: "error",
					message,
				});
	...
	if (state.status === "loading") {
		return (
			<box flexDirection="column" paddingX={1} gap={1}>
				<text fg={palette.act}>ForgeOS Account</text>
	...
	if (state.status === "unauthenticated") {
		return (
			<box flexDirection="column" paddingX={1} gap={1}>
				<text fg={palette.act}>ForgeOS Account</text>
				<text>Sign in or create a ForgeOS account.</text>
				<text fg="gray">
	...
	return (
		<box flexDirection="column" paddingX={1} gap={1}>
			...
			<box flexDirection="column" border borderColor="gray" paddingX={1}>
				...
				<AccountField
					label="Credits"
					value={formatForgeOSCredits(loaded.displayedBalance)}
				/>
				{loaded.activeOrganization && (
					<AccountField
						label="Personal"
						value={formatForgeOSCredits(loaded.balance.balance)}
					/>
				)}
				<AccountField
					label="Organizations"
