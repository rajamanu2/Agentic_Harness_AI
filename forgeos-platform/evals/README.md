# ForgeOS Evaluation Framework

A layered testing system for measuring ForgeOS's performance at different levels.

> Note: Smoke tests (Layer 2) are partially disabled while the eval framework is repointed at the new SDK CLI. The scenarios under `evals/smoke-tests/` are preserved and `npm run eval:smoke:run` still works against whatever `forgeos` is on `$PATH` (install with `npm i -g forgeos`). The old build-and-link helpers and the auto-running `forgeos-evals-regression.yml` workflow are off until someone wires the build step at the new SDK CLI.
...
│
├── e2e/                   # Full E2E with forgeos-bench (hours)
│   └── run-forgeos-bench.ts
│
├── forgeos-bench/           # Real-world tasks (git submodule)
...
- Runs the `forgeos` CLI with `--config`, `-y`, `-t`, and `-m`

```bash
# Set API key (ForgeOS provider)
export FORGEOS_API_KEY=sk-...
...
Location: `evals/e2e/` + `evals/forgeos-bench/`
...
- Nightly: E2E tests with forgeos-bench are not yet implemented, see TODO
...
Contribute to [forgeos/forgeos-bench](https://github.com/forgeos/forgeos-bench)

## Resources

- [forgeos-bench tasks](evals/forgeos-bench/README.md)
- [Smoke test scenarios](evals/smoke-tests/README.md)

## TODO

- [ ] **Nightly E2E CI**: Add scheduled workflow for forgeos-bench tests
