...
ForgeOS works with VSCode's multi-root workspaces, letting you manage multiple project folders or repositories in a single window. Whether you're working with a monorepo or separate Git repositories, ForgeOS can read files, write code, and run commands across all of them.

<Frame>
  <video
    src="https://storage.googleapis.com/forgeos_public_images/multiworkspace.mp4"
    autoPlay
    muted
    loop
    playsInline
...
- **ForgeOS rules** only work in the primary workspace folder
...
ForgeOS can complete tasks that span multiple projects or repositories:
...
ForgeOS supports both patterns, as well as hybrid setups where some folders are Git repositories and others are not. The key difference: with multiple repositories, each folder has its own `.git` directory and ForgeOS tracks them independently.
...
When you open separate Git repositories in one workspace, ForgeOS treats each as an independent project with its own version control.

### What ForgeOS Tracks Per Repository

For each workspace folder, ForgeOS detects:
...
This means ForgeOS understands that your frontend and backend might be at different commits, on different branches, or even use different version control systems.

<Note>
While ForgeOS detects VCS information for all workspace folders, certain features only use the **primary workspace** (the first folder): [ForgeOS rules](/customization/forgeos-rules), [skills](/customization/skills#triggering-skills-with-slash-commands), and [Git-related features](/core-workflows/working-with-files) like `@git` mentions.
</Note>
...
ForgeOS understands natural references to your workspaces:
...
- ForgeOS needs to resolve ambiguity
...
Each service has its own repository. ForgeOS can update the proto definitions and regenerate clients across all services.
...
The types repository defines interfaces used by both client and server. When you update a type, ForgeOS can fix both consumers.
...
Mix of repositories and plain folders. ForgeOS adapts to each folder's configuration.
...
[ForgeOS rules](/customization/forgeos-rules) (`.forgeosrules/` directory) only work in the **primary workspace** (the first folder in your workspace). Rules in other workspace folders are ignored.

**Workaround:** Place shared rules in the primary workspace, or use global rules (`~/Documents/ForgeOS/Rules/`) which apply everywhere.

### Checkpoints

[Checkpoints](/core-workflows/checkpoints) are disabled in multi-root workspace mode. ForgeOS displays a warning when this happens.
...
4. **Consider the primary workspace** for ForgeOS rules placement
...
- Use [Plan mode](/core-workflows/plan-and-act) to let ForgeOS understand structure first
