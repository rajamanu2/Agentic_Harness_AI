...
const THEME_BOOTSTRAP_SCRIPT = `<script id="forgeos-hub-theme-bootstrap">
  (() => {
    try {
      const theme = window.localStorage.getItem("forgeos-hub-theme");
...
export function normalizeWebviewIndexHtml(html: string): string {
	...
	if (normalized.includes('id="forgeos-hub-theme-bootstrap"')) {
		return normalized;
	}
...
<head>
  ...
  <link rel="icon" type="image/svg+xml" href="${devServerUrl}/forgeos-logo-filled.svg" />
  <title>ForgeOS Hub</title>
</head>
<body>
  <div id="root"></div>
...
export class WebviewAssets {
	...
	private async serveIndex(): Promise<Response> {
		...
		return createTextResponse(
			"ForgeOS Hub webview is not built. Run `bun run build:webview` from apps/forgeos-hub.",
			503,
		);
	}
