#!/usr/bin/env npx tsx
/**
 * forgeos-bench Runner
 *
 * Runs real-world tasks from forgeos-bench using Harbor framework.
 ...
 *
 * Usage:
 *   npx tsx evals/e2e/run-forgeos-bench.ts [options]
 *
 * Options:
...
function getTaskList(forgeosBenchDir: string, filter?: string): string[] {
	const tasksDir = path.join(forgeosBenchDir, "tasks")
...
function runHarborTask(forgeosBenchDir: string, taskId: string, options: RunOptions): TaskResult {
	...
	const harborArgs = ["run", "-p", `tasks/${taskId}`, "-a", "forgeos-cli", "-m", harborModel, "--env", options.env]
	...
	try {
		...
		const jobsDir = path.join(forgeosBenchDir, "jobs")
...
async function main() {
	...
	// Find forgeos-bench directory
	const forgeosBenchDir = path.join(__dirname, "..", "forgeos-bench")
	if (!fs.existsSync(forgeosBenchDir)) {
		console.error(`forgeos-bench not found at: ${forgeosBenchDir}`)
		console.error("Ensure the submodule is initialized: git submodule update --init")
		process.exit(1)
	}

	// Get task list
	const tasks = getTaskList(forgeosBenchDir, options.tasks)
	if (tasks.length === 0) {
		console.error("No tasks found matching filter:", options.tasks)
		process.exit(1)
	}

	console.log(`forgeos-bench E2E Runner`)
	console.log(`======================`)
	...
	for (const taskId of tasks) {
		...
		for (let trial = 0; trial < options.trials; trial++) {
			...
			const result = runHarborTask(forgeosBenchDir, taskId, options)
			results.push(result)
