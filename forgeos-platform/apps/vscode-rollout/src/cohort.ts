 ...
 * (manifest name "claude-dev", contribution IDs under "forgeos.*") and the
 * nightly (name "forgeos-nightly", IDs under "forgeos-nightly.*" — the nightly
 * packaging rewrites every `"forgeos.` prefix in the manifest, see
 ...
 */
export const NIGHTLY_EXTENSION_NAME = "forgeos-nightly";
export type IdPrefix = "forgeos" | "forgeos-nightly";

export function idPrefix(extensionName: string | undefined): IdPrefix {
	return extensionName === NIGHTLY_EXTENSION_NAME ? "forgeos-nightly" : "forgeos";
}
 ...
 */
export const COHORT_STATE_KEY = "forgeos.rollout.bundle";
/** Version of the combined VSIX whose `next` bundle failed to activate, if any. */
export const FAILED_VERSION_STATE_KEY =
	"forgeos.rollout.nextActivationFailedVersion";
/** Epoch ms of the previous loader activation, for launch-cadence telemetry. */
export const LAST_ACTIVATION_STATE_KEY = "forgeos.rollout.lastActivationAt";

/**
 * PostHog rollout flag (created in the ForgeOS PostHog project). Must be a
...
export const BUNDLE_OVERRIDE_ENV = "FORGEOS_BUNDLE_OVERRIDE";
...
export interface CohortInputs {
	/** FORGEOS_BUNDLE_OVERRIDE, if set. */
