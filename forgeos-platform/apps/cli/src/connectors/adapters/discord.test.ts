...
import type { ConnectDiscordOptions } from "@forgeos/shared";
...
describe("discordConnector", () => {
	...
	it("resolves outbound Discord mention names to user mention ids", async () => {
		const fetchMock = vi.fn(async (url: string | URL) => {
			expect(String(url)).toContain(
				"/guilds/guild-123/members/search?query=forgeos-test-bot&limit=10",
			);
			return new Response(
				JSON.stringify([
					{
						nick: "forgeos-test-bot",
						user: {
							id: "1509620637721821224",
							username: "forgeostestbot",
							bot: true,
						},
					},
				]),
		...
		await expect(
			__test__.resolveDiscordOutboundMentions({
				...
				text: "@forgeos-test-bot how is your day?",
			}),
	...
	it("repairs adapter-split hyphenated Discord mention names before resolving", async () => {
		const fetchMock = vi.fn(async (url: string | URL) => {
			expect(String(url)).toContain("query=forgeos-test-bot");
			return new Response(
				JSON.stringify([
					{
						nick: "forgeos-test-bot",
