...
describe("spawnDetachedConnector", () => {
	it("preserves the connect subcommand when building detached connector args", () => {
		expect(
			__test__.buildDetachedConnectorArgs(
				["connect", "telegram"],
				["-m", "ForgeOSAdapterBot", "-k", "token-123"],
			),
		).toEqual([
			"connect",
			"telegram",
	...
	it("marks detached children and removes the hub-daemon-only environment flag", () => {
		const env = {
			FORGEOS_BUILD_ENV: "production",
			FORGEOS_RUN_AS_HUB_DAEMON: "1",
			UNCHANGED: "value",
		};

		expect(
			__test__.buildDetachedConnectorEnv("FORGEOS_TELEGRAM_CONNECT_CHILD", env),
		).toEqual({
			FORGEOS_BUILD_ENV: "production",
			FORGEOS_CONNECTOR_DETACHED_CHILD: "1",
			FORGEOS_TELEGRAM_CONNECT_CHILD: "1",
			UNCHANGED: "value",
		});
		expect(env.FORGEOS_RUN_AS_HUB_DAEMON).toBe("1");
	});
});
...
describe("detached connector log rotation", () => {
	it("keeps one generation once the log grows past the cap", () => {
		const dir = mkdtempSync(join(tmpdir(), "forgeos-connector-log-"));
		const logPath = join(dir, "forgeos-slack.log");
