...
// ForgeOSCore does not choose a model automatically; each session config must provide one.
// These example defaults use the ForgeOS gateway with Claude Sonnet, and can be overridden with env vars.
const providerId = process.env.FORGEOS_PROVIDER_ID ?? "forgeos";
const modelId = process.env.FORGEOS_MODEL_ID ?? "anthropic/claude-sonnet-4.6";
const apiKey = process.env.FORGEOS_API_KEY;
...
let forgeos: ForgeOSCore | undefined;
...
async function ensureForgeOS(): Promise<ForgeOSCore> {
	if (forgeos) {
		return forgeos;
	}

	forgeos = await ForgeOSCore.create({
		clientName: "forgeos-core-cli-agent",
		backendMode: "local",
		capabilities: {
			requestToolApproval,
		},
	});
	unsubscribe = forgeos.subscribe((event) => {
...
async function startSession(): Promise<string> {
	const runtime = await ensureForgeOS();
...
console.log("ForgeOSCore CLI Agent (type 'exit' to quit)\n");
...
try {
	...
	rl.close();
	unsubscribe?.();
	if (activeSessionId && forgeos) {
		await forgeos.stop(activeSessionId).catch(() => undefined);
	}
