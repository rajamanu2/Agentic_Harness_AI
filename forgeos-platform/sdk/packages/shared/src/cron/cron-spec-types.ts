 ...
 * Split out from `@forgeos/core` so the parser can live in `shared` where
 * multiple consumers (hub daemon, CLI tools, enterprise adapters) can validate
 * the same `.forgeos/cron/` spec format without pulling in core's stateful
 * orchestration layer.
 */
 ...
 * - everything else under `.forgeos/cron/*.md` -> one_off
 */
...
export interface CronSpecParseResult {
	...
	externalId: string;
	/** Normalized posix-style path relative to `.forgeos/cron/`. */
	relativePath: string;
