...
describe("plugin-config-loader", () => {
	...
	it("discovers plugin modules recursively", async () => {
		...
		try {
			...
			await writeFile(
				join(root, ".a.js.forgeos-plugin.js"),
				"export default {}",
				"utf8",
			);
	...
	it("includes shared search-path plugins", async () => {
		...
		try {
			process.env.HOME = home;
			setHomeDir(home);
			const workspacePlugins = join(workspace, ".forgeos", "plugins");
			const userPlugins = join(home, ".forgeos", "plugins");
			const documentsPlugins = join(home, "Documents", "ForgeOS", "Plugins");
