# ForgeOSCore API Reference

## Creating ForgeOSCore

```typescript
import { ForgeOSCore } from "@forgeos/sdk"

const forgeos = await ForgeOSCore.create(options: ForgeOSCoreOptions)
```

### ForgeOSCoreOptions

```typescript
interface ForgeOSCoreOptions {
...
```typescript
const session = await forgeos.start(input: ForgeOSCoreStartInput)
```
...
```typescript
interface ForgeOSCoreStartInput {
  prompt: string
...
`extensions` passes plugin objects directly. `pluginPaths` points to directories with `package.json` containing a `forgeos.plugins` field. Set `extensionContext.workspace` so plugins receive `ctx.workspaceInfo` in their `setup()` call -- without it, `ctx.workspaceInfo` is undefined.
...
```typescript
const result = await forgeos.send({
...
```typescript
const unsubscribe = forgeos.subscribe(
...
```typescript
const sessions: SessionRecord[] = await forgeos.list(50)
```

### get(sessionId)

```typescript
const session: SessionRecord = await forgeos.get(sessionId)
```

### readMessages(sessionId)

```typescript
const messages: AgentMessage[] = await forgeos.readMessages(sessionId)
```

### getAccumulatedUsage(sessionId)

```typescript
const usage = await forgeos.getAccumulatedUsage(sessionId)
...
```typescript
await forgeos.update(sessionId, { title: "New title" })
```

### abort(sessionId, reason?)

```typescript
await forgeos.abort(sessionId, "User cancelled")
```
...
```typescript
await forgeos.delete(sessionId)
```
...
```typescript
await forgeos.restore({ sessionId, checkpointId })
```
...
```typescript
await forgeos.dispose("Shutting down")
```
...
When `automation` is enabled in `ForgeOSCore.create()`:
...
forgeos.automation.start()
forgeos.automation.stop()
forgeos.automation.reconcile(specs)
forgeos.automation.ingestEvent(event)
forgeos.automation.listEvents()
forgeos.automation.listSpecs()
forgeos.automation.listRuns()
```
...
```typescript
// Read settings
const settings = await forgeos.settings.list()

// Toggle tools, plugins, MCP servers
await forgeos.settings.toggle({ type: "tool", name: "bash", enabled: true })
```
