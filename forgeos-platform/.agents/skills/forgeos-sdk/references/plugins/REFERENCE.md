...
A ForgeOS plugin is a TypeScript module that extends any agent built on the ForgeOS SDK. The same plugin runs in the ForgeOS CLI, VS Code and JetBrains extensions, and any custom app built on `@forgeos/core`.
...
2. Plugin package -- a directory with `package.json`, npm dependencies, and optionally bundled assets. Installable via `forgeos plugin install`.
...
Plugins can declare normalized event types and emit them into ForgeOS automation. Hosts that don't have automation enabled simply ignore both -- feature-detect `ctx.automation`.
...
- `<workspace>/.forgeos/plugins/` -- project-scoped plugins.
...
Save as `my-plugin.ts`, drop in `.forgeos/plugins/`:
...
- `forgeos.plugins` -- the discovery contract. Array of entries, each with `paths` (entry files) and `capabilities` (pre-declared, validated before importing).
- `peerDependencies` for `@forgeos/core` -- the host already provides it. Marking it optional lets you typecheck in isolation.
...
Add a `runDemo()` in your plugin file (see the single-file template above) that boots a real `ForgeOSCore` session:
...
- `ctx.workspaceInfo` is undefined in SDK tests -- the host didn't pass `extensionContext.workspace`. In SDK code, set it explicitly (see the ForgeOSCore loading example above).
...
- `../forgeoscore/REFERENCE.md` - Using plugins with ForgeOSCore
