...
const getForgeOSVersion = () => {
	try {
		const extensions = execSync("code --list-extensions --show-versions").toString()
		const forgeosMatch = extensions.match(/claude-dev@(\d+\.\d+\.\d+)/)
		return forgeosMatch ? forgeosMatch[1] : "Not installed"
	} catch (_err) {
...
const collectSystemInfo = () => {
	...
	return {
		...
		forgeosVersion: getForgeOSVersion(),
	}
}
...
const createIssueUrl = (systemInfo, issueTitle) => {
	return (
		`https://github.com/forgeos/forgeos/issues/new?template=bug_report.yml` +
		`&title=${issueTitle}` +
		`&operating-system=${systemInfo.os}` +
		`&forgeos-version=${systemInfo.forgeosVersion}` +
		`&system-info=${
