...
if (!target || !["sdk", "cli"].includes(target)) {
	...
	console.error("  sdk   Publish @forgeos/{shared,llms,agents,core,sdk} to npm");
	console.error("  cli   Publish forgeos from an existing cli-vX.Y.Z git tag");
	console.error("");
...
async function releaseSDK(version: string): Promise<number> {
	...
	if (dryRun) {
		...
		for (const workspace of SDK_PUBLISH_ORDER) {
			console.log(`    - @forgeos/${workspace}@${version}`);
		}
	}
	console.log(`${"═".repeat(60)}\n`);
...
async function releaseCLI(version: string): Promise<number> {
	...
	if (dryRun) {
		...
		console.log("    npm install -g forgeos");
	}
	console.log(`${"═".repeat(60)}\n`);
