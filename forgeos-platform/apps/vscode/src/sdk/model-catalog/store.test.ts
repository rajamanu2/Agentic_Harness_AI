import { syncStoredProviderRegistration } from "@forgeos/core"
...
describe("createProviderConfigStore", () => {
	...
	// bypassing the @forgeos/core mock above): the store's converters must pass
	...
	it("round-trips every SDK model capability and a full override set under the real stored-entry schema", async () => {
		const { ModelCapabilitySchema } = await import("@forgeos/shared")
		// vi.importActual bypasses the @forgeos/core mock above and resolves via
		// the vitest alias to the stub, which re-exports the real schema.
		const { StoredModelEntrySchema } = (await vi.importActual("@forgeos/core")) as {
