...
export function isVisibleCheckpointUserMessage(message: ForgeOSMessage): boolean {
	return message.type === "say" && (message.say === "task" || message.say === "user_feedback")
}

export function isCheckpointAnswerMessage(messages: ForgeOSMessage[], index: number): boolean {
...
export function isCheckpointRunUserMessage(messages: ForgeOSMessage[], index: number): boolean {
	return isVisibleCheckpointUserMessage(messages[index]) && !isCheckpointAnswerMessage(messages, index)
}

export function getCheckpointRunCountForMessage(messages: ForgeOSMessage[], targetIndex: number): number | undefined {
...
export function findVisibleCheckpointUserMessageByRun(
	...
	runCount: number,
): { message: ForgeOSMessage; index: number } | undefined {
	let seenUsers = 0
