...
describe("provider-ids", () => {
	...
	it("registers ForgeOSPass as a distinct ForgeOS-compatible built-in provider", async () => {
		expect(BUILT_IN_PROVIDER_IDS).toContain("forgeos-pass");
		...
		for (const providerId of ["forgeos", "forgeos-pass"]) {
			const registration = BUILTIN_PROVIDER_REGISTRATIONS.find(
				(item) => item.manifest.id === providerId,
			);
			await expect(registration?.loadProvider?.()).resolves.toMatchObject({
				createProvider: createForgeOSProvider,
			});
		}
	});
