...
describe("resolveProviderConfig", () => {
	...
	it("uses catalog aliases when loading live models", async () => {
		...
		const resolved = await resolveProviderConfig("forgeos", {
	...
	it("uses only live ForgeOS recommended models for ForgeOSPass when live models are found", async () => {
		...
		const resolved = await resolveProviderConfig("forgeos-pass", {
			loadLatestOnInit: true,
			failOnError: false,
			cacheTtlMs: 0,
		...
		expect(resolved?.knownModels?.["forgeos-pass/live-pass-model"]).toMatchObject(
			{
				id: "forgeos-pass/live-pass-model",
				name: "Live Pass Model",
		...
		expect(resolved?.knownModels?.["forgeos-pass/mimo-v2.5-pro"]).toBeUndefined();
	});

	it("keeps ForgeOSPass models ahead of newer free models in the served catalog", async () => {
		const fetchMock = vi.fn(async (url: string) => {
			if (url === "https://models.test/api.json") {
				return new Response(
					JSON.stringify({
		...
		expect(resolved?.knownModels?.["forgeos-free/live-free-model"]).toMatchObject(
			{
		...
		expect(
			resolved?.knownModels?.["forgeos-free/live-free-model"]?.pricing,
		).toEqual({
			input: 0,
			output: 0,
			cacheRead: 0,
	...
	it("adds forgeos-free models from the recommended endpoint to the ForgeOS catalog", async () => {
		const fetchMock = vi.fn(async (url: string) => {
			if (url === "https://models.test/api.json") {
				return new Response(
					JSON.stringify({
	...
	it("falls back to generated ForgeOSPass models when no live ForgeOSPass models are found", async () => {
		...
		expect(fetchMock).toHaveBeenCalledTimes(2);
		expect(resolved?.knownModels?.["forgeos-pass/mimo-v2.5-pro"]?.name).toBe(
			"MiMo-V2.5-Pro",
		);
		expect(
	...
	it("prefers Vercel-style Z.ai ids in ForgeOS known models", async () => {
		const resolved = await resolveProviderConfig("forgeos");
	...
	it("preserves explicit ForgeOS known model overrides for alias ids", async () => {
		const resolved = await resolveProviderConfig("forgeos", undefined, {
			providerId: "forgeos",
