...
import { resolveForgeOSDir } from "@forgeos/shared/storage";
...
const OFFICIAL_PLUGINS_REPO = "https://github.com/forgeos/plugins.git";
const MARKETPLACE_CATALOG_URL =
	process.env.FORGEOS_MARKETPLACE_CATALOG_URL?.trim() ||
...
function resolveForgeOSInvocation(): { command: string; argsPrefix: string[] } {
	...
	return { command: "forgeos", argsPrefix: [] };
}
...
function getGlobalSkillPaths(skillName: string): string[] {
	return [
		join(resolveForgeOSDir(), "skills", skillName, "SKILL.md"),
...
function ensureGlobalSkillsDirWritable(): void {
	...
	try {
		...
		const probePath = join(
			skillsDir,
			`.forgeos-marketplace-write-test-${process.pid}-${Date.now()}`,
		);
...
async function installSkill(
	...
	if (!isGlobalSkillInstalled(entry)) {
		throw new Error(
			`Skill install completed, but ${entry.name ?? entry.id} was not found in ForgeOS's global skills directories.`,
		);
	}
	return {
		id: entry.id,
		type: entry.type,
		status: "installed",
		message: `Installed ${entry.name ?? entry.id} globally for ForgeOS.`,
		output,
	};
}
...
async function installPlugin(
	...
	const { command, argsPrefix } = resolveForgeOSInvocation();
