...
function setupHostProvider(extensionContext: any, extensionDir: string, dataDir: string) {
	...
	// forgeos-core expects the binaries to be unpacked in the directory where it is running.
...
/**
 * Gracefully shutdown the forgeos-core process by:
...
function showHelp() {
	Logger.log(`
ForgeOS Core - Standalone Server

Usage: node forgeos-core.js [options]
...
Options:
  ...
  -c, --config <path>            Directory for ForgeOS data storage (default: ~/.forgeos)
