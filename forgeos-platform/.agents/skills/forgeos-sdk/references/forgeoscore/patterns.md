...
```typescript
import { ForgeOSCore, createTool } from "@forgeos/sdk"
import { z } from "zod"
...
const sessions = await forgeos.list(10)
for (const session of sessions) {
  console.log(`${session.id}: ${session.title}`)
}

// Read messages from a past session
const messages = await forgeos.readMessages(sessions[0].id)
for (const msg of messages) {
  console.log(`[${msg.role}] ${msg.content}`)
}

// Check usage
const usage = await forgeos.getAccumulatedUsage(sessions[0].id)
...
process.on("SIGTERM", async () => {
  await forgeos.dispose("SIGTERM received")
  process.exit(0)
})
...
const viewer = await ForgeOSCore.create({
