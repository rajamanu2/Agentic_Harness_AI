...
function resolveRuntimeConfig(): Required<RuntimeLoggerConfig> {
	const enabledValue = process.env.FORGEOS_LOG_ENABLED?.trim().toLowerCase();
	return {
		enabled: enabledValue !== "0" && enabledValue !== "false",
		level: resolveLogLevel(process.env.FORGEOS_LOG_LEVEL),
		destination:
			process.env.FORGEOS_LOG_PATH?.trim() ||
			join(resolveForgeOSDataDir(), "logs", "code.log"),
		name: process.env.FORGEOS_LOG_NAME?.trim() || "forgeos-code.sidecar",
		bindings: {},
	};
}
...
function writeDestinationFallbackWarning(path: string, error: unknown): void {
	try {
		...
		process.stderr.write(
			`[forgeos-code] Unable to open log file ${path}; falling back to stderr (${message})\n`,
		);
	} catch {
