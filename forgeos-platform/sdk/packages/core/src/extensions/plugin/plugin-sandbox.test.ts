...
import {
	FORGEOS_PLUGIN_IDLE_TIMEOUT_MS_ENV,
...
describe("plugin-sandbox", () => {
	...
	beforeAll(async () => {
		...
		await writeFile(
			...
			[
				...
				"      inputSchema: { type: 'object', properties: { value: { type: 'string' } }, required: ['value'] },",
				"      execute: async (input) => {",
				"        globalThis.__forgeosPluginHost?.emitEvent?.('test_event', { value: input.value });",
				"        return { ok: true };",
				"      },",
				"    });",
				"  },",
		...
		await writeFile(
			...
			[
				...
				"  hooks: { afterRun(ctx) {",
				"    globalThis.__forgeosPluginHost?.emitEvent?.('run_end_hook', {",
		...
		const sdkDepDir = join(dir, "node_modules", "@forgeos", "shared");
		...
		await writeFile(
			...
			[
				"import { sdkMarker } from '@forgeos/shared';",
				"export default {",
				"  name: sdkMarker,",
				"  manifest: { capabilities: ['tools'] },",
				"};",
		...
		await writeFile(
			...
			[
				...
				"export default {",
				"  name: YAML.stringify({ host: !!resolveForgeOSDataDir() }).trim(),",
				"  manifest: { capabilities: ['tools'] },",
				"};",
			].join("\n"),
			"utf8",
		...
		await writeFile(
			...
			[
				"import { createTool } from '@forgeos/agents';",
				"export default {",
	...
	it("strips non-serializable host values from the tool context before IPC", async () => {
		...
		const result = await echoTool?.execute({ value: "ok" }, {
			...
			metadata: {
				...
				__forgeosInternalTelemetry: cyclicTelemetry,
			},
	...
	it("respects FORGEOS_PLUGIN_IMPORT_TIMEOUT_MS env var when options.importTimeoutMs is unset", async () => {
		...
		try {
			...
			vi.stubEnv("FORGEOS_PLUGIN_IMPORT_TIMEOUT_MS", "150");
			await expect(
	...
	it("reclaims an idle plugin process and reinitializes it on the next call", async () => {
		...
		try {
			...
			vi.stubEnv(FORGEOS_PLUGIN_IDLE_TIMEOUT_MS_ENV, "1000");
			vi.useFakeTimers();
	...
	it("ignores the npm wrapper platform package bootstrap when running from source", async () => {
		const previousWrapperPath = process.env.FORGEOS_WRAPPER_PATH;
		...
		const wrapperPath = join(wrapperBinDir, "forgeos");
		const events: Array<{ name: string; payload?: unknown }> = [];
		...
		try {
			...
			await writeFile(
				...
				JSON.stringify({
					name: `@forgeos/cli-${platform}-${process.arch}`,
					version: "0.0.0-test",
					type: "module",
				}),
				"utf8",
			...
			if (previousWrapperPath === undefined) {
				...
				process.env.FORGEOS_WRAPPER_PATH = previousWrapperPath;
			}
			vi.resetModules();
