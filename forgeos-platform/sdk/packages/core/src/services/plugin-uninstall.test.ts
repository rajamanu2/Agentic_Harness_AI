...
describe("plugin uninstall service", () => {
	...
	beforeEach(() => {
		...
		originalGlobalSettingsPath = process.env.FORGEOS_GLOBAL_SETTINGS_PATH;
	...
	it("uninstalls an installed package plugin by package name", async () => {
		...
		await writeFile(
			...
			JSON.stringify(
				{
					name: "forgeos-installed-plugin-test",
					forgeos: {
		...
		await writeFile(
			...
			JSON.stringify({ name: "forgeos-internal-bundled-skills-demo" }, null, 2),
			"utf8",
		);
		await writeFile(
			entryPath,
		...
		const result = await uninstallPlugin({
			name: "forgeos-internal-bundled-skills-demo",
		});
	...
	it("uninstalls a direct plugin file by path", async () => {
		const pluginPath = join(home, ".forgeos", "plugins", "direct-plugin.ts");
		await mkdir(join(home, ".forgeos", "plugins"), { recursive: true });
		await writeFile(
			pluginPath,
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			const pluginPath = join(home, ".forgeos", "plugins", "mcp-plugin.ts");
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			const pluginRoot = join(home, ".forgeos", "plugins");
