...
vi.mock("@forgeos/core", async (importOriginal: any) => {
...
// pulls in @forgeos/core, @forgeos/llms and @forgeos/shared, so the first test to call
...
describe("ProviderCatalog Phase 3.5 listProviders", () => {
	it("returns SDK provider listings with top-level picker metadata", async () => {
		...
		expect(mocks.listLocalProviders).toHaveBeenCalledWith(expect.anything(), { isForgeOSPassEnabled: true })
	})
	...
	it("refetches provider listings after explicit invalidation", async () => {
		...
		mocks.listLocalProviders
			.mockResolvedValueOnce({
				providers: [{ id: "forgeos", name: "ForgeOS", protocol: "anthropic", client: "anthropic", source: "system" }],
			})
			.mockResolvedValueOnce({
				providers: [
					{ id: "forgeos-pass", name: "ForgeOSPass", protocol: "anthropic", client: "anthropic", source: "system" },
				],
			})
		const catalog = createProviderCatalog(makeReader({ providerId: parseProviderId("forgeos") }))

		const first = await catalog.listProviders()
		catalog.invalidateProviderListings()
		const second = await catalog.listProviders()

		expect(first[0]?.id).toBe("forgeos")
		expect(second[0]?.id).toBe("forgeos-pass")
