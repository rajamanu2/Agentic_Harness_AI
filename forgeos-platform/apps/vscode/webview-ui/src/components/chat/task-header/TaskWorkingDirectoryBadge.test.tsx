...
describe("isTaskCwdOutsideWorkspace", () => {
	...
	it("is true when the cwd is outside every workspace root", () => {
		expect(isTaskCwdOutsideWorkspace("/tmp/forgeos-hello", roots("/home/user/project"), "linux")).toBe(true)
	})
	...
	it("is false when the cwd is unknown or roots are missing", () => {
		...
		expect(isTaskCwdOutsideWorkspace("/tmp/forgeos-hello", [], "linux")).toBe(false)
		expect(isTaskCwdOutsideWorkspace("/tmp/forgeos-hello", undefined, "linux")).toBe(false)
	})
...
describe("TaskWorkingDirectoryBadge", () => {
	it("renders the cwd basename and full-path tooltip when outside the workspace", () => {
		render(
			<TaskWorkingDirectoryBadge
				platform="linux"
				taskCwd="/tmp/forgeos-hello"
				workspaceRoots={roots("/home/user/project")}
			/>,
		)
		expect(screen.getByText("forgeos-hello")).toBeDefined()
		expect(screen.getByText(/working directory is \/tmp\/forgeos-hello/)).toBeDefined()
	})
	...
	it("renders nothing when workspace roots are not yet known", () => {
		const { container } = render(
			<TaskWorkingDirectoryBadge platform="linux" taskCwd="/tmp/forgeos-hello" workspaceRoots={[]} />,
		)
		expect(container.innerHTML).toBe("")
	})
})
