...
import { createForgeOSAPI } from "./exports"
...
import { addToForgeOS } from "./core/controller/commands/addToForgeOS"
import { explainWithForgeOS } from "./core/controller/commands/explainWithForgeOS"
import { fixWithForgeOS } from "./core/controller/commands/fixWithForgeOS"
import { improveWithForgeOS } from "./core/controller/commands/improveWithForgeOS"
...
import { registerForgeOSOutputChannel } from "./hosts/vscode/hostbridge/env/debugLog"
import {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// After this, all platforms (VSCode, CLI, JetBrains) read from ~/.forgeos/data/.
	...
	const handleUri = async (uri: vscode.Uri) => {
		...
		if (isTaskUri) {
			await openForgeOSSidebarForTaskUri()
		}
	...
	// `ext.evaluate`. Gated on FORGEOS_CAPTURE_BROWSER so it never ships in prod.
	if (process.env.FORGEOS_CAPTURE_BROWSER === "1" || process.env.FORGEOS_CAPTURE_BROWSER === "true") {
		;(globalThis as Record<string, unknown>).__forgeosHandleUri = (url: string) => SharedUriHandler.handleUri(url)
	}

	// Register size testing commands in development mode
	if (IS_DEV) {
		vscode.commands.executeCommand("setContext", "forgeos.isDevMode", IS_DEV)
		// Use dynamic import to avoid loading the module in production
		import("./dev/commands/tasks")
			.then((module) => {
				const devTaskCommands = module.registerTaskCommands(webview.controller)
				context.subscriptions.push(...devTaskCommands)
				Logger.log("[ForgeOS Dev] Dev mode activated & dev commands registered")
			})
			.catch((error) => {
				Logger.log(`[ForgeOS Dev] Failed to register dev commands: ${error}`)
			})
	}
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Add to ForgeOS (Always available)
					const addAction = new vscode.CodeAction("Add to ForgeOS", vscode.CodeActionKind.QuickFix)
					addAction.command = {
						command: commands.AddToChat,
						title: "Add to ForgeOS",
						arguments: [expandedRange, context.diagnostics],
					}
					actions.push(addAction)

					// Explain with ForgeOS (Always available)
					const explainAction = new vscode.CodeAction("Explain with ForgeOS", vscode.CodeActionKind.RefactorExtract) // Using a refactor kind
					explainAction.command = {
						command: commands.ExplainCode,
						title: "Explain with ForgeOS",
						arguments: [expandedRange],
					}
					actions.push(explainAction)

					// Improve with ForgeOS (Always available)
					const improveAction = new vscode.CodeAction("Improve with ForgeOS", vscode.CodeActionKind.RefactorRewrite) // Using a refactor kind
					improveAction.command = {
						command: commands.ImproveCode,
						title: "Improve with ForgeOS",
						arguments: [expandedRange],
					}
					actions.push(improveAction)

					// Fix with ForgeOS (Only if diagnostics exist)
					if (context.diagnostics.length > 0) {
						const fixAction = new vscode.CodeAction("Fix with ForgeOS", vscode.CodeActionKind.QuickFix)
						fixAction.isPreferred = true
						fixAction.command = {
							command: commands.FixWithForgeOS,
							title: "Fix with ForgeOS",
							arguments: [expandedRange, context.diagnostics],
						}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.AddToChat, async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
			...
			await addToForgeOS(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.FixWithForgeOS, async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
			const context = await getContextForCommand(range, diagnostics)
			if (!context) {
				return
			}
			await fixWithForgeOS(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.ExplainCode, async (range: vscode.Range) => {
			...
			await explainWithForgeOS(context.controller, context.commandContext)
		}),
	)
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.ImproveCode, async (range: vscode.Range) => {
			...
			await improveWithForgeOS(context.controller, context.commandContext)
		}),
	)
...
\`\`\``
				...
				await addToForgeOS(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
				...
				await explainWithForgeOS(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
...
\`\`\``
				...
				await improveWithForgeOS(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.Walkthrough, async () => {
			await vscode.commands.executeCommand("workbench.action.openWalkthrough", `${context.extension.id}#ForgeOSWalkthrough`)
	...
	const unsubSecrets = storageContext.secrets.onDidChange((event) => {
		if (event.key === "forgeos:forgeosAccountId") {
	...
	Logger.log(`[ForgeOS] extension activated in ${performance.now() - activationStartTime} ms`)

	return createForgeOSAPI(webview.controller)
}
...
function setupHostProvider(context: ExtensionContext) {
	const outputChannel = registerForgeOSOutputChannel(context)
	outputChannel.appendLine("[ForgeOS] Setting up VS Code host...")
...
async function openForgeOSSidebarForTaskUri(): Promise<void> {
	...
	Logger.warn("Task URI handling timed out waiting for ForgeOS sidebar visibility")
}
...
async function cleanupLegacyVSCodeStorage(context: ExtensionContext): Promise<void> {
	try {
		...
		// Migrate custom instructions to global ForgeOS rules (one-time cleanup)
