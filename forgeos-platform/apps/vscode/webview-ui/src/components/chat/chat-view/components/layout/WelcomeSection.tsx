import { BANNER_DATA, BannerAction, BannerActionType, BannerCardData } from "@shared/forgeos/banner"
import { EmptyRequest } from "@shared/proto/forgeos/common"
import type { Worktree } from "@shared/proto/forgeos/worktree"
import { TrackWorktreeViewOpenedRequest } from "@shared/proto/forgeos/worktree"
...
export const WelcomeSection: React.FC<WelcomeSectionProps> = ({
	...
	const bannerConfig = useMemo((): BannerCardData[] => {
		...
		return BANNER_DATA.filter((banner) => {
			...
			if (banner.isForgeOSUserOnly !== undefined) {
				return banner.isForgeOSUserOnly === !!forgeosUser
			}
		...
		(action: BannerAction) => {
			switch (action.action) {
				...
				case BannerActionType.InstallCli:
					StateServiceClient.installForgeOSCli({}).catch((error) =>
	...
	}, [bannerConfig, banners, forgeosUser, handleBannerAction, handleBannerDismiss])
								...
								{currentWorktree && (
									<Tooltip>
										...
										<TooltipContent side="bottom">
											View and manage git worktrees. Great for running parallel ForgeOS tasks.
										</TooltipContent>
									</Tooltip>
								)}
							</div>
