...
The Telegram connector bridges a Telegram Bot API bot into ForgeOS CLI sessions. It is a polling connector, so it does not need a public webhook URL. The connector process must stay running while Telegram access is active.
...
forgeos connect telegram -i -k "$TELEGRAM_BOT_TOKEN"

# Read credentials from env vars.
TELEGRAM_BOT_TOKEN=123456:ABCDEF... forgeos connect telegram

# Override the workspace and model used for Telegram sessions.
forgeos connect telegram -k "$TELEGRAM_BOT_TOKEN" --cwd /path/to/repo --provider forgeos --model openai/gpt-5.3-codex

# Disable tools for untrusted Telegram surfaces.
forgeos connect telegram -k "$TELEGRAM_BOT_TOKEN" --no-tools
...
- Starts or reuses a ForgeOS RPC-backed session for each Telegram thread.
...
For participant restrictions, run the interactive connector wizard with `forgeos connect`. The Telegram wizard asks whether to restrict access, points you to `@userinfobot`, and configures your numeric Telegram user ID.

You can also pass the user ID directly:

```bash
forgeos connect telegram -k "$TELEGRAM_BOT_TOKEN" --allowed-user-id 12345
```
...
```bash
forgeos schedule create "Daily summary" \
  --cron "0 9 * * *" \
