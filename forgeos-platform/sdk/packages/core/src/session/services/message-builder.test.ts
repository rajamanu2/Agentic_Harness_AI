...
describe("MessageBuilder", () => {
	...
	it("parses message-builder limit environment overrides", () => {
		const builder = new MessageBuilder(
			getMessageBuilderOptionsFromEnv({
				FORGEOS_MESSAGE_BUILDER_MAX_TOOL_RESULT_CHARS: "96",
				FORGEOS_MESSAGE_BUILDER_MAX_TOTAL_TEXT_BYTES: "5000",
				FORGEOS_MESSAGE_BUILDER_MIN_OUTDATED_REWRITE_BYTES: "0",
			}),
		);
		const messages: Message[] = [
			{
	...
	it("parses an env disable value for outdated-read rewrites", () => {
		const builder = new MessageBuilder(
			getMessageBuilderOptionsFromEnv({
				FORGEOS_MESSAGE_BUILDER_MIN_OUTDATED_REWRITE_BYTES: "disable",
			}),
		);
	...
	it("ignores zero env overrides instead of disabling the limits", () => {
		const options = getMessageBuilderOptionsFromEnv({
			FORGEOS_MESSAGE_BUILDER_MAX_TOOL_RESULT_CHARS: "0",
			FORGEOS_MESSAGE_BUILDER_MAX_TOTAL_TEXT_BYTES: "0",
		});
 ...
 * the default ForgeOS tools (run_commands, read_files, search_codebase).
 *
