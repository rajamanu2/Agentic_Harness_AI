...
type SessionManager = ForgeOSCore;

/** Minimal plugin host interface injected by the runtime via globalThis. */
interface ForgeOSPluginHost {
	emitEvent?: (name: string, payload?: unknown) => void;
}

declare global {
	var __forgeosPluginHost: ForgeOSPluginHost | undefined;
}
...
function resolveForgeOSDirPath(): string {
	const explicitDir = process.env.FORGEOS_DIR?.trim();
	if (explicitDir) {
		return explicitDir;
	}
	return join(resolveDefaultHomeDir(), ".forgeos");
}

function resolveForgeOSDataDirPath(): string {
	const explicitDir = process.env.FORGEOS_DATA_DIR?.trim();
	if (explicitDir) {
		return explicitDir;
	}
	return join(resolveForgeOSDirPath(), "data");
}

function resolveGlobalAgentsDirPath(): string {
	return join(resolveForgeOSDataDirPath(), "settings", "agents");
}

const HANDOFFS_DIR = join(
	resolveForgeOSDataDirPath(),
	"plugins",
	"subagents",
	"handoffs",
);
const GLOBAL_SKILLS_DIR = join(resolveForgeOSDataDirPath(), "settings", "skills");
...
const DEFAULT_PROVIDER_ID = envOr("FORGEOS_SUBAGENT_PROVIDER_ID", "forgeos");
const DEFAULT_MODEL_ID = envOr(
	"FORGEOS_SUBAGENT_MODEL_ID",
	"anthropic/claude-sonnet-4.6",
);
type SubagentBackendMode = "auto" | "hub" | "local";

const DEFAULT_BACKEND_MODE = envOr("FORGEOS_SUBAGENTS_BACKEND_MODE", "auto");
const DEFAULT_AGENT_PRESET = envOr("FORGEOS_SUBAGENT_DEFAULT_PRESET", "phantom");
...
function parseFrontmatter(md: string): {
	...
	// UTF-8 BOM (see forgeos/forgeos#12151).
	md = stripUtf8Bom(md);
	const m = md.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/);
...
function readAgentDefinitions(baseCwd: string): AgentDefinition[] {
	const dirs: Array<{ path: string; source: AgentDefinition["source"] }> = [
		...
		{ path: join(baseCwd, ".forgeos", "agents"), source: "project" },
	];
...
function readSkillDefinitions(baseCwd: string): SkillDefinition[] {
	const dirs: Array<{ path: string; source: SkillDefinition["source"] }> = [
		...
		{ path: join(baseCwd, ".forgeos", "skills"), source: "project" },
	];
...
function emitSteer(sessionId: string | undefined, prompt: string): void {
	if (sessionId && prompt.trim()) {
		globalThis.__forgeosPluginHost?.emitEvent?.("steer_message", {
			sessionId,
			prompt,
		});
	}
...
async function getSessionManager(): Promise<SessionManager> {
	sessionManagerPromise ??= ForgeOSCore.create({
			...
			toRegisteredTool(
				createTool({
					...
					async execute(input, ctx) {
						const mgr = await getSessionManager();
						const baseCwd = envOr("FORGEOS_SUBAGENT_CWD", process.cwd());
