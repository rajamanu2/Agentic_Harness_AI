 ...
 * which cannot fit ForgeOS's agentic prompts. Single source of truth — the
...
function firstGeneratedModelId(providerId: string): string {
	// Use the catalog's authored order, not release-date order. The forgeos-pass
...
// Vercel-only model ids surfaced for the ForgeOS provider while the OpenRouter
// catalog lacks them (ForgeOS's backend routes these to Vercel AI Gateway).
// Remove an id once the OpenRouter catalog lists it.
const VERCEL_ONLY_FORGEOS_MODEL_IDS: readonly string[] = [
...
function buildForgeOSModels(): Record<string, ModelInfo> {
	// ForgeOS is OpenRouter-backed generally, but its recommended-model endpoint
		...
		Object.entries(generatedModels("vercel-ai-gateway")).filter(
			([modelId]) =>
				isCanonicalModelIdForAliasRules(
					modelId,
					VERCEL_OPENROUTER_MODEL_ID_ALIAS_RULES,
				) || VERCEL_ONLY_FORGEOS_MODEL_IDS.includes(modelId),
		),
	);
...
function createForgeOSLikeSpec(
	...
	return {
		id: input.id,
		name: input.name,
		description: input.description ?? "ForgeOS API endpoint",
		...
		apiKeyEnv: ["FORGEOS_API_KEY"],
		defaults: {
			get baseUrl(): string {
				return `${getForgeOSEnvironmentConfig().apiBaseUrl}/api/v1`;
			},
			...input.defaults,
		},
		metadata: {
...
async function handleForgeOSResponseError(
	response: Response,
	providerId: string,
): Promise<void> {
	...
	if (isForgeOSOrgIndividualInferenceSubscriptionMessage(body)) {
		throw new ForgeOSOrgIndividualInferenceSubscriptionError(providerId);
	}

	if (isForgeOSFreeModelLimitMessage(body)) {
		throw new ForgeOSFreeModelLimitError(body, providerId);
	}

	const forgeosPassLimitMessage = extractForgeOSPassLimitMessage(body);
	if (forgeosPassLimitMessage) {
		throw new ForgeOSPassLimitError(forgeosPassLimitMessage, providerId);
	}

	if (isForgeOSNotSubscribedMessage(body)) {
		throw new ForgeOSNotSubscribedError(providerId);
	}
}

const forgeos = createForgeOSLikeSpec({
	id: "forgeos",
	family: "forgeos",
	name: "ForgeOS Usage-Billing",
	popular: 1,
	modelsFactory: buildForgeOSModels,
	defaultModelId: FORGEOS_DEFAULT_MODEL_ID,
	defaults: {
		options: {
			onResponseError: async (response: Response) => {
				await handleForgeOSResponseError(response, "forgeos");
			},
		},
	},
});

const forgeosPass = createForgeOSLikeSpec({
	id: FORGEOS_PASS_PROVIDER_ID,
	family: "forgeos",
	name: "ForgeOSPass",
	popular: 2,
	description: "ForgeOS API endpoint with ForgeOSPass models",
	modelsProviderId: FORGEOS_PASS_PROVIDER_ID,
	defaultModelId: firstGeneratedModelId(FORGEOS_PASS_PROVIDER_ID),
	metadata: { usageCostDisplay: "subscription" },
	defaults: {
		options: {
			onResponseError: async (response: Response) => {
				await handleForgeOSResponseError(response, FORGEOS_PASS_PROVIDER_ID);
			},
		},
	},
});
...
/**
 * Handwritten providers plus generated providers that require ForgeOS-specific
...
const OPENAI_COMPATIBLE_SPEC_OVERRIDES: BuiltinSpecOverride[] = [
	...
	},
	{
		...
		// routing policy (`apiLineBaseUrls`), which is ForgeOS-specific.
		id: "moonshot",
		apiLineBaseUrls: {
 ...
 * in providers.generated.ts and only retain ForgeOS-owned behavior here.
 */
const BUILTIN_SPEC_OVERRIDES: BuiltinSpecOverride[] = [
	...
	},
	{
		...
		// gateway sends ForgeOS's tool definitions (which the provider drops)
