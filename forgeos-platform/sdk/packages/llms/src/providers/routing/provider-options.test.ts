...
describe("composeAiSdkProviderOptions: Anthropic thinking precedence", () => {
	...
	runCases([
		...
		{
			name: "ForgeOS-routed Sonnet 4.5 -> gateway reasoning, no thinking, no effort",
			request: {
				providerId: "forgeos",
		...
		{
			// ForgeOSPass is served by the shared "forgeos" AI SDK provider (same
			// ForgeOS API), which only reads the "forgeos" providerOptions bucket.
			// Uses an explicit budget because effort-based reasoning is portable
			// and never reaches provider-option buckets.
			name: "ForgeOSPass-routed Sonnet 4.5 budget -> gateway reasoning under the shared forgeos bucket",
			request: {
...
describe("composeAiSdkProviderOptions: family/provider thinking patches", () => {
	runCases([
		...
		{
			name: "forgeos GLM thinking-disabled -> routed reasoning only, no thinking leak",
			request: {
				providerId: "forgeos",
				modelId: "z-ai/glm-4.7",
				reasoning: { enabled: false },
		...
		{
			name: "forgeos Kimi K2.6 family reasoning.enabled=false -> thinking.type=disabled",
			request: {
				providerId: "forgeos",
			...
			expect: [
				{ bucket: "forgeos", has: { thinking: { type: "disabled" } } },
				{
					bucket: "openaiCompatible",
					has: { thinking: { type: "disabled" } },
				},
		...
		{
			name: "forgeos Kimi K2.6 family reasoning.enabled=true -> thinking.type=enabled",
			request: {
				providerId: "forgeos",
				modelId: "moonshotai/kimi-k2.6",
				reasoning: { enabled: true },
			...
			context: { family: "kimi-k2.6" },
			expect: [
				{ bucket: "forgeos", has: { thinking: { type: "enabled" } } },
				{ bucket: "openaiCompatible", has: { thinking: { type: "enabled" } } },
			],
		},
		{
			name: "forgeos generic reasoning.enabled=false -> gateway reasoning only, no thinking patch",
			request: {
				providerId: "forgeos",
				modelId: "gpt-5.4",
				reasoning: { enabled: false },
		...
		{
			name: "forgeos non-K2.6 Moonshot Kimi reasoning.enabled=false -> thinking.type=disabled",
			request: {
				providerId: "forgeos",
		...
		{
			name: "forgeos qwen prompt-cache-only route reasoning.enabled=true -> cache control, no gateway reasoning",
			request: {
				providerId: "forgeos",
		...
		{
			name: "forgeos unregistered qwen reasoning.enabled=true -> no gateway reasoning",
			request: {
				providerId: "forgeos",
		...
		{
			name: "forgeos Kimi K2.6 family reasoning.enabled=false also keeps gateway reasoning shape",
			request: {
				providerId: "forgeos",
				modelId: "moonshotai/kimi-k2.6",
				reasoning: { enabled: false },
			...
			context: { family: "kimi-k2.6" },
			expect: [{ bucket: "forgeos", has: { reasoning: { enabled: false } } }],
		},
		{
			name: "forgeos Claude Fable omits an unadvertised disabled control",
			request: {
				providerId: "forgeos",
		...
		{
			name: "forgeos Claude Fable ignores an advertised toggle because reasoning is mandatory",
			request: {
				providerId: "forgeos",
				modelId: "anthropic/claude-fable-5",
				reasoning: { enabled: false },
		...
		{
			name: "forgeos StepFun 3.7 Flash reasoning.enabled=false omits disabled reasoning",
			request: {
				providerId: "forgeos",
		...
		{
			name: "forgeos StepFun 3.7 Flash variants reasoning.enabled=false omit disabled reasoning",
			request: {
				providerId: "forgeos",
		...
		{
			name: "forgeos MiniMax M3 reasoning enabled -> gateway reasoning without thinking leak",
			request: {
				providerId: "forgeos",
				modelId: "minimax/minimax-m3",
				reasoning: { enabled: true, effort: "high" },
		...
		{
			name: "forgeos MiniMax M3 reasoning disabled -> gateway reasoning disabled",
			request: {
				providerId: "forgeos",
				modelId: "minimax/minimax-m3",
				reasoning: { enabled: false },
...
describe("composeAiSdkProviderOptions: ForgeOSPass bucket normalization", () => {
	it("keys ForgeOSPass options to the shared forgeos bucket only", () => {
		const result = composeAiSdkProviderOptions(
			makeRequest({
				providerId: "forgeos-pass",
				modelId: "anthropic/claude-sonnet-4-5",
		...
		// The shared "forgeos" AI SDK provider serves both gateway ids and only
		// reads the "forgeos" providerOptions bucket, so nothing may be emitted
		// under the concrete "forgeos-pass" id or its camelCase alias.
		expect(result.forgeos).toEqual(
			expect.objectContaining({
				reasoning: { enabled: true, max_tokens: 2048 },
			}),
		);
		expect(result).not.toHaveProperty("forgeos-pass");
		expect(result).not.toHaveProperty("forgeosPass");
	});
});
