 ...
 * exercising and debugging ForgeOS's MCP OAuth flow locally, including failure
 * modes such as:
 *
 ...
 *                         for exercising ForgeOS's OAuth state-expiry window).
 ...
 *
 * Then add an MCP server to ForgeOS pointing at:
 *   http://127.0.0.1:7777/mcp   (type: streamableHttp)
 *
 * Click "Authenticate" in ForgeOS; a browser opens the /authorize consent page.
 */
...
interface TestServerOptions {
	 ...
	 * so you can add several distinct MCP servers to ForgeOS at once and exercise
...
class TestServer {
	...
	private async handleMcp(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
		...
		if (!hasBearer) {
			// This is what triggers ForgeOS's OAuth flow: 401 + WWW-Authenticate
		...
		// succeeds and ForgeOS shows the server as connected.
		const body = await this.readJsonBody(req)
...
/**
 * Build a paste-ready `mcpServers` fragment for forgeos_mcp_settings.json from a
...
if (isMain) {
	...
	void (async () => {
		...
		console.log("\nPaste into ~/.forgeos/data/settings/forgeos_mcp_settings.json (merge under mcpServers):\n")
