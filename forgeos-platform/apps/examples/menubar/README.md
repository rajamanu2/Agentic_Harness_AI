...
![ForgeOS Hub Monitor preview](./assets/hub-monitor-preview.jpg)
...
Any Client (CLI, VS Code, agents)
    ...
    ▼
Hub WebSocket Server (@forgeos/core/hub/server.ts)
    │
...
`apps/cli/src/index.ts` when it is present; packaged builds use `forgeos` from
`PATH`, or the executable specified by `FORGEOS_CLI_PATH`.
