...
import type { ForgeOSAccountUserInfo } from "@/services/auth/AuthService"
...
function makeUserInfo(
	overrides: Partial<ForgeOSAccountUserInfo> & { orgOverrides?: Record<string, unknown> } = {},
): ForgeOSAccountUserInfo {
...
describe("OpenTelemetryTelemetryProvider.identifyUser", () => {
	...
	it("should handle user with no active organization", () => {
		...
		const userInfo: ForgeOSAccountUserInfo = {
			id: "user-1",
	...
	it("should clear stale org attributes when switching from org to no-org", () => {
		...
		const userNoOrg: ForgeOSAccountUserInfo = {
			id: "user-1",
			displayName: "Test User",
			email: "test@example.com",
			createdAt: new Date().toISOString(),
