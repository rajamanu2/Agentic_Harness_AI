...
import { HubSessionClient } from "@forgeos/core";
 ...
 * running, users can still find the result later via `forgeos history`.
 *
...
export async function runZen(
	...
	if (
		process.env.FORGEOS_SESSION_BACKEND_MODE?.trim().toLowerCase() === "local"
	) {
		writeErr(
			"--zen requires the hub backend but FORGEOS_SESSION_BACKEND_MODE=local is set.",
		);
		process.exitCode = 1;
		return;
	}
