...
import type { GatewayStreamRequest } from "@forgeos/shared";
import { estimateTokens } from "@forgeos/shared";
...
function readCaptureMode(): CaptureMode {
	const raw = process.env.FORGEOS_CAPTURE_PROVIDER_REQUEST?.trim().toLowerCase();
...
function isWireCaptureEnabled(): boolean {
	return process.env.FORGEOS_CAPTURE_WIRE?.trim().toLowerCase() === "true";
}

function readMaxPreviewBytes(): number {
	const parsed = Number(process.env.FORGEOS_CAPTURE_MAX_PREVIEW_BYTES);
...
function isCleanupEnabled(): boolean {
	return process.env.FORGEOS_CAPTURE_CLEANUP?.trim().toLowerCase() !== "off";
}

function resolveCaptureDir(): string | undefined {
	const explicit = process.env.FORGEOS_CAPTURE_DIR?.trim();
	if (explicit) {
		return resolve(explicit);
	}
	const dataDir = process.env.FORGEOS_DATA_DIR?.trim();
	if (dataDir) {
