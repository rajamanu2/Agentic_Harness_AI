...
describe("Skills Utility Functions", () => {
	...
	const GLOBAL_SKILLS_DIR = path.join("/home", "user", ".forgeos", "skills")
	...
	beforeEach(() => {
		...
		getSkillsDirectoriesForScanStub.returns([
			{ path: path.join(TEST_CWD, ".forgeosrules", "skills"), source: "project" },
			{ path: path.join(TEST_CWD, ".forgeos", "skills"), source: "project" },
...
Instructions here`)
		...
		// Regression test for https://github.com/forgeos/forgeos/issues/12151:
...
This is a test skill.`)
		...
		it("should discover skills from project .forgeosrules/skills directory", async () => {
			const projectSkillsDir = path.join(TEST_CWD, ".forgeosrules", "skills")
...
Use analogies and ASCII diagrams when explaining code.`)
		...
		it("should discover skills from project .forgeos/skills directory", async () => {
			const forgeosSkillsDir = path.join(TEST_CWD, ".forgeos", "skills")
			const skillDir = path.join(forgeosSkillsDir, "debugging")
			const skillMdPath = path.join(skillDir, "SKILL.md")

			fileExistsStub.withArgs(forgeosSkillsDir).resolves(true)
			fileExistsStub.withArgs(skillMdPath).resolves(true)
			isDirectoryStub.withArgs(forgeosSkillsDir).resolves(true)
			readdirStub.withArgs(forgeosSkillsDir).resolves(["debugging"])
			statStub.withArgs(skillDir).resolves({ isDirectory: () => true })
			readFileStub.withArgs(skillMdPath, "utf-8").resolves(`---
name: debugging
	...
	describe("Remote Skills", () => {
		...
		describe("Override resolution (remote > disk-global > project)", () => {
			...
			it("remote overrides project skill of same name", async () => {
				const entries = [makeEntry("coding", "Remote coding")]
				const projDir = path.join(TEST_CWD, ".forgeosrules", "skills")
...
describe("setSkillDisabledInFrontmatter", () => {
	...
	it("writes disabled: true to the SKILL.md when disabling a disk skill", async () => {
		const skillPath = path.join("/home", "user", ".forgeos", "skills", "s", "SKILL.md")
