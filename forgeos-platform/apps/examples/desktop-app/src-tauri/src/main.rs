...
const TRAY_ICON_ID: &str = "forgeos-code";
...
#[derive(Default)]
struct AuxiliaryServicesState {
    processes: Mutex<Vec<Child>>,
}

impl AuxiliaryServicesState {
    fn start(&self, resource_dir: &Path) {
        let runtime = resource_dir.join("resources").join("forgeos-runtime");
        let engine = runtime.join("engine").join(if cfg!(windows) {
            "forgeos-engine.exe"
        } else {
            "forgeos-engine"
        });
        let control_plane = runtime.join("control-plane");
        let java = control_plane
            .join("jre-all")
            .join("bin")
            .join(if cfg!(windows) { "javaw.exe" } else { "java" });
        let jar = control_plane.join("forgeos-control-plane.jar");
        ...
        if engine.exists() {
            ...
            command
                .current_dir(engine.parent().unwrap_or(&runtime))
                .stdin(Stdio::null())
                .stdout(Stdio::null())
                .stderr(Stdio::null());
            ...
            match command.spawn() {
                ...
                Err(error) => eprintln!("[forgeos-engine] startup failed: {error}"),
            }
        }
        if java.exists() && jar.exists() {
            ...
            match command.spawn() {
                Ok(child) => children.push(child),
                Err(error) => eprintln!("[forgeos-control-plane] startup failed: {error}"),
            }
        }
    }

    fn stop(&self) {
        if let Ok(mut children) = self.processes.lock() {
...
fn resolve_desktop_backend_binary_path(context: &AppContext) -> Option<PathBuf> {
    ...
    let explicit = std::env::var("FORGEOS_CODE_SIDECAR_BIN")
        .ok()
...
fn resolve_mcp_settings_path() -> Result<PathBuf, String> {
    if let Ok(value) = std::env::var("FORGEOS_MCP_SETTINGS_PATH") {
    ...
    Ok(PathBuf::from(home)
        ...
        .join("data")
        .join("settings")
        .join("forgeos_mcp_settings.json"))
}
...
fn restart_to_apply_update(
    app: tauri::AppHandle,
    backend_state: State<'_, Arc<DesktopBackendState>>,
    auxiliary_state: State<'_, Arc<AuxiliaryServicesState>>,
) {
...
/// flows that need an update staged right now (e.g. the "ForgeOS Hub was
...
fn setup_tray_icon(app: &tauri::App) -> tauri::Result<()> {
    ...
    let menu = MenuBuilder::new(app)
        .text(
            TRAY_OPEN_MENU_ID,
            format!("ForgeOS v{}", app.package_info().version),
        )
        .item(&status)
        .separator()
    ...
    // This is the same glyph used by webview/components/forgeos-logo.tsx,
    // rasterized without a background. Template mode lets macOS tint it for
    // the current menu-bar appearance.
    #[cfg(target_os = "macos")]
    let icon =
        tauri::image::Image::from_bytes(include_bytes!("../icons/tray/forgeos-template.png"))?;
    #[cfg(not(target_os = "macos"))]
    let icon = app
        .default_window_icon()
        .cloned()
...
fn main() {
    ...
    let auxiliary_services = Arc::new(AuxiliaryServicesState::default());
        ...
        .run(|app_handle, event| match event {
            ...
            RunEvent::ExitRequested { .. } | RunEvent::Exit => {
                app_handle
                    ...
                    .stop();
                app_handle
                    .state::<Arc<AuxiliaryServicesState>>()
                    .inner()
                    .stop();
            }
            _ => {}
