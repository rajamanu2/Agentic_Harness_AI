...
//
// Creates and manages SDK sessions using ForgeOSCore. This factory handles:
// - Creating ForgeOSCore instances with proper configuration
// - Building session config from legacy state (provider, model, API key)
// - Custom session persistence adapter reading ~/.forgeos/data/tasks/
// - Mapping HistoryItem ↔ SDK session fields
//
// The factory does NOT handle UI concerns — that's the SdkController's job.

import {
	type ForgeOSCoreStartInput,
...
import type { ProviderApiLine, ModelInfo as SdkModelInfo } from "@forgeos/llms"
import {
...
import { ForgeOSClient } from "@shared/forgeos"
 ...
 * standalone forgeos-core (JetBrains), where `vscode` is a Proxy-stub module and
 * direct API reads would yield non-string values. The hostbridge returns the
 * per-host values (e.g. "ForgeOS for JetBrains" + IDE version on JetBrains).
 */
...
const PROVIDER_API_KEY_MAP: Record<string, keyof ApiConfiguration> = {
	...
	forgeos: "forgeosApiKey",
	"forgeos-pass": "forgeosApiKey",
	ollama: "ollamaApiKey",
...
const PROVIDER_MODEL_ID_MAP: Record<string, { plan: keyof ApiConfiguration; act: keyof ApiConfiguration }> = {
	...
	forgeos: { plan: "planModeForgeOSModelId", act: "actModeForgeOSModelId" },
	"forgeos-pass": { plan: "planModeForgeOSPassModelId", act: "actModeForgeOSPassModelId" },
...
const DEFAULT_PROVIDER_ID = "forgeos"
...
export function resolveApiKey(providerId: string, config: ApiConfiguration): string | undefined {
	...
	if (authHandler) {
		...
		// resolve shared storage (e.g. forgeos-pass -> forgeos) without VS Code
...
export function resolveModelId(providerId: string, mode: Mode, config: ApiConfiguration): string | undefined {
	...
	// session use a different model than the ForgeOS provider UI shows.
...
export async function buildSessionConfig(input: SessionConfigInput): Promise<CoreSessionConfig> {
	...
	try {
		...
		systemPrompt = buildForgeOSSystemPrompt({
			ide: "VS Code",
			workspaceRoot,
			workspaceName,
		...
		systemPrompt = "You are ForgeOS, a highly skilled software engineer. Help the user with their request."
	}
	...
	try {
		...
		// the @forgeos/llms registry. Reading models from that registry ensures custom
	...
	const providerConfig = {
		...
		// otherwise falls back to a small default output cap (FORGEOS-2911).
	...
	const config: CoreSessionConfig = {
		...
		extensionContext: {
			...
			client: {
				name: hostIdentity?.forgeosType || ForgeOSClient.VSCode,
				version: hostIdentity?.forgeosVersion || ExtensionRegistryInfo.version,
 ...
 */
export function buildStartSessionInput(config: CoreSessionConfig, input: SessionConfigInput): ForgeOSCoreStartInput {
	return {
		config,
