...
Learn how to build with the ForgeOS SDK through working examples, ordered from simple to complex.
...
bun install
bun run build:sdk
export FORGEOS_API_KEY="forgeos_..."
bun dev
```
...
| [forgeos-core-cli-agent](./forgeos-core-cli-agent) | Interactive terminal chat powered by ForgeOSCore. | `ForgeOSCore.create()`, `forgeos.start()`, `forgeos.send()`, built-in tools, streaming |
...
`@forgeos/sdk` re-exports everything from `@forgeos/core`. You only need `@forgeos/agents` or `@forgeos/llms` if you want lower-level control over the agent runtime or model gateway directly.

## Learn more

- [SDK package docs](../../sdk/packages/README.md)
- [Architecture guide](../../ARCHITECTURE.md)
- [Plugin examples](../../examples/plugins) - extend the ForgeOS SDK and CLI with custom tools and event hooks
- [Hook examples](../../examples/hooks) - lifecycle hooks for logging, blocking, and injection for ForgeOS SDK and CLI
