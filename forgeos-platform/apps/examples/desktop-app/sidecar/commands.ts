...
import { resolveAudioTranscriptionRoute } from "@forgeos/llms";
import {
...
import { readFileSyncStrippingUtf8Bom } from "@forgeos/shared/node";
import packageJson from "../package.json";
import { FORGEOS_ACCOUNT_NOT_AUTHENTICATED_RESULT } from "../webview/lib/forgeos-account-state";
...
// ForgeOS access tokens expire between app launches, so account requests must
...
let forgeosOAuthTokenManager: RuntimeOAuthTokenManager | undefined;

async function resolveFreshForgeOSAuthToken(
	ctx: SidecarContext,
	manager: ProviderSettingsManager,
): Promise<string | undefined> {
	let refreshError: Error | undefined;
	try {
		forgeosOAuthTokenManager ??= new RuntimeOAuthTokenManager();
		const resolution = await forgeosOAuthTokenManager.resolveProviderApiKey({
			providerId: "forgeos",
		});
	...
	const persisted = resolveLocalForgeOSAuthToken(
		manager.getProviderSettings("forgeos"),
	);
	...
	if (!persisted && refreshError) {
		ctx.logger?.error?.("ForgeOS auth token refresh failed with no fallback", {
			error: refreshError,
		});
		captureAuthRefreshSoftFailure(ctx.telemetry, "forgeos", {
...
async function handleRoutineScheduleCommand(
	...
	if (command === "create_routine_schedule") {
		...
		const created = await clientCommand("schedule.create", {
			...
			prompt,
			modelSelection: {
				providerId: asTrimmedString(args?.provider) ?? "forgeos",
				modelId: asTrimmedString(args?.model) ?? FORGEOS_DEFAULT_MODEL_ID,
			},
...
async function listUserInstructionConfigs(
	...
	// (apps/forgeos-hub/src/server/user-instructions.ts) even if the preset
	// defaults change.
...
export async function handleCommand(
	...
	if (command === "forgeos_account") {
		...
		const authToken = await resolveFreshForgeOSAuthToken(ctx, manager);
		if (!authToken) {
			return FORGEOS_ACCOUNT_NOT_AUTHENTICATED_RESULT;
		}
		const settings = manager.getProviderSettings("forgeos");
		const accountService = new ForgeOSAccountService({
			apiBaseUrl:
				settings?.baseUrl?.trim() || getForgeOSEnvironmentConfig().apiBaseUrl,
			getAuthToken: async () => authToken,
		});
		return await executeForgeOSAccountAction(
			args as ForgeOSAccountActionRequest,
			accountService,
		);
	}
	...
	if (command === "list_provider_catalog") {
		...
		return await listLocalProviders(manager, { isForgeOSPassEnabled: true });
	}
