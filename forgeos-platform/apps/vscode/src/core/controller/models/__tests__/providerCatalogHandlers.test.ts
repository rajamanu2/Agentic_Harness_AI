...
import { ApiFormat, ModelOverrides } from "@/shared/proto/forgeos/models"
...
describe("provider model catalog handlers", () => {
	...
	it("readProviderConfig redacts secrets", async () => {
		...
		const providerId = parseProviderId("forgeos")
		...
		const response = await readProviderConfig(controller, { value: "forgeos" })
