 ...
 */
const POSTHOG_HOST = "https://data.forgeos.bot";
 ...
 * id: the loader must never write to the shared ~/.forgeos state files.
 */
async function getDistinctId(): Promise<string> {
	const generated = await readSharedGlobalStateKey("forgeos.generatedMachineId");
...
async function readSharedGlobalStateKey(key: string): Promise<unknown> {
	try {
		...
		const raw = await readFile(
			path.join(forgeosDir, "data", "globalState.json"),
			"utf8",
		);
