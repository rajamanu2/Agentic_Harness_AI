 ...
 *   - textual rewrites: "claude-dev" -> "forgeos-nightly" everywhere, and every
 *     `"forgeos.` ID prefix -> `"forgeos-nightly.` (commands, settings, view IDs,
 *     when-clauses that START with the key — mid-string references like
 *     `config.forgeos.x` are NOT rewritten, same as the standalone nightly)
...
export const NIGHTLY_NAME = "forgeos-nightly";
export const NIGHTLY_DISPLAY_NAME = "ForgeOS (Nightly)";
...
export function nightlifyPackageJson(rawContent, version) {
	...
	const content = rawContent
		...
		.replaceAll('"forgeos.', `"${NIGHTLY_NAME}.`);
