# ForgeOS Automation Examples

This directory contains example automation specs for file-based and event-driven automation in ForgeOS. Use these as templates to set up your own recurring or event-driven tasks.
...
ForgeOS automation supports two types of specs:

1. **Recurring specs** (`.cron.md`) — Run on a schedule
2. **Event-driven specs** (`.event.md`) — Run when an event occurs

Both can be enabled in `.forgeos/cron/` to be picked up by the hub or SDK.
...
cp examples/cron/daily-code-review.cron.md ~/.forgeos/cron/
# Edit the spec: set workspaceRoot, model, etc.
# Spec is reconciled on startup; next run enqueued automatically
```

**One-off specs:**
For one-time tasks, save as `.forgeos/cron/<name>.md` (no `.cron` infix) and omit the `schedule` field.
...
Event-driven specs live in `.forgeos/cron/events/` and trigger when normalized events are ingested.
...
cp examples/cron/events/pr-review.event.md ~/.forgeos/cron/events/
...
- Connector adapters
- `forgeos.automation.ingestEvent()` in the SDK
...
```bash
mkdir -p ~/.forgeos/cron/events
cp examples/cron/events/local-manual-test.event.md ~/.forgeos/cron/events/

# Start the hub with automation enabled
# In another shell, ingest a test event
node -e "
  const { HubWebSocketClient } = require('@forgeos/core');
...
```bash
mkdir -p ~/.forgeos/cron/events
cp examples/cron/events/local-plugin-event.event.md ~/.forgeos/cron/events/

# Load the plugin that emits these events
forgeos plugin install https://github.com/forgeos/forgeos/blob/main/sdk/examples/plugins/automation-events.ts

# Run CLI with automation enabled; the plugin emits events
forgeos --enable-automation -i "Test automation events"
```
...
**In the CLI:**
```bash
forgeos --enable-automation
```

### 5. Monitor runs

Completed and failed runs are reported to `.forgeos/cron/reports/<run-id>.md` with:
...
- [ForgeOS SDK Examples](../) — Other integration examples
