...
export function sharedSessionDataDir(): string {
	return process.env.FORGEOS_SESSION_DATA_DIR?.trim() || resolveSessionDataDir();
}
...
export function toolApprovalDir(): string {
	return (
		process.env.FORGEOS_TOOL_APPROVAL_DIR?.trim() ||
...
export function resolveMcpSettingsPath(): string {
	return (
		process.env.FORGEOS_MCP_SETTINGS_PATH?.trim() ||
		join(homedir(), ".forgeos", "data", "settings", "forgeos_mcp_settings.json")
	);
}
...
function kanbanDataRoot(): string {
	return (
		process.env.FORGEOS_KANBAN_DATA_DIR?.trim() ||
		join(homedir(), ".forgeos", "apps", "kanban")
	);
}
