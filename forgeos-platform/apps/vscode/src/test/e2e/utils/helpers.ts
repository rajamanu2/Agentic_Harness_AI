...
import { ForgeOSApiServerMock } from "../fixtures/server"
...
export class E2ETestHelper {
	...
	public async signin(webview: Frame): Promise<void> {
		await webview.getByRole("button", { name: "Login to ForgeOS" }).click({ delay: 100 })

		// Verify start up page is no longer visible
		await expect(webview.getByRole("button", { name: "Login to ForgeOS" })).not.toBeVisible()
	...
	public static async openForgeOSSidebar(page: Page): Promise<void> {
		await page.getByRole("tab", { name: /ForgeOS/ }).locator("a").click()
	}
...
/**
 * NOTE: Use the `e2e` test fixture for all E2E tests to test the ForgeOS extension.
 *
 * Extended Playwright test configuration for ForgeOS E2E testing.
 *
 * This test configuration provides a comprehensive setup for end-to-end testing of the ForgeOS VS Code extension,
 ...
 *
 * Fixtures provided:
 * - `server`: Shared ForgeOSApiServerMock instance for API mocking (reused across all tests)
 ...
 * - `page`: Playwright Page object representing the main VS Code window with ForgeOS sidebar opened
 * - `sidebar`: Playwright Frame object representing the ForgeOS extension's sidebar iframe
 *
 * @returns Extended test object with all fixtures available for E2E test scenarios:
 * - **server**: Automatically starts and manages a ForgeOSApiServerMock instance
 ...
 * - **page**: Configures the main VS Code window with notifications disabled and ForgeOS sidebar open
 * - **sidebar**: Provides access to the ForgeOS extension's sidebar frame
 *
 * @example
 * ```typescript
 ...
 * - Installs the ForgeOS extension in development mode
...
export const e2e = test
	.extend<{ server: ForgeOSApiServerMock | null }>({
		server: async ({}, use) => {
			// Start server if it doesn't exist
			if (!ForgeOSApiServerMock.globalSharedServer) {
				await ForgeOSApiServerMock.startGlobalServer()
			}
			await use(ForgeOSApiServerMock.globalSharedServer)
		},
	})
	...
	.extend<{ openVSCode: (workspacePath: string) => Promise<ElectronApplication> }>({
		openVSCode: async ({ userDataDir, channel }, use, testInfo) => {
			...
			await use(async (workspacePath: string) => {
				// Create isolated ForgeOS data directory for this test
				const forgeosTestDir = mkdtempSync(path.join(os.tmpdir(), "forgeos-e2e-"))
				const forgeosDataDir = path.join(forgeosTestDir, "data")
				...
				const app = await _electron.launch({
					...
					env: {
						...
						TEMP_PROFILE: "true",
						E2E_TEST: "true",
						FORGEOS_ENVIRONMENT: "local",
						FORGEOS_DIR: forgeosTestDir, // Isolate test data from user's ~/.forgeos
						FORGEOS_DATA_DIR: forgeosDataDir, // Keep SDK/shared storage off the user's real ForgeOS data dir
	...
	.extend<{ app: ElectronApplication; forgeosTestDir: string }>({
		app: async ({ openVSCode, userDataDir, extensionsDir, workspaceType, workspaceDir, multiRootWorkspaceDir }, use) => {
			...
			try {
				...
				// Clean up the isolated ForgeOS data directory
				// Find all temp directories matching our pattern
				const tmpDir = os.tmpdir()
				try {
					const entries = readdirSync(tmpDir)
					for (const entry of entries) {
						if (entry.startsWith("forgeos-e2e-")) {
	...
	.extend<{ sidebar: Frame }>({
		sidebar: async ({ page, helper, server }, use) => {
			await E2ETestHelper.openForgeOSSidebar(page)
