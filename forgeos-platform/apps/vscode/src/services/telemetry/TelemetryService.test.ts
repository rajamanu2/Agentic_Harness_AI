...
describe("Telemetry system is abstracted and can easily switch between providers", () => {
	...
	const MOCK_METADATA: TelemetryMetadata = {
		...
		forgeos_type: "forgeos-unit-test",
		platform: "Test-IDE",
	...
	describe("Telemetry Service", () => {
		...
		it("should derive host_plugin_version from the host's forgeosVersion", async () => {
			const cases = [
				{
					hostVersion: {
			...
			for (const { hostVersion, expectedHostPluginVersion } of cases) {
				...
				try {
					...
					assert.ok(logSpy.calledOnce, `service should emit an event (${hostVersion.forgeosType})`)
	...
	describe("Factory Configuration", () => {
		it("should return default configurations", () => {
			...
			const isSelfHostedStub = sinon.stub(ForgeOSEndpoint, "isSelfHosted").returns(false)
		...
		it("should NOT include PostHog config when in selfHosted mode", () => {
			// Stub ForgeOSEndpoint.isSelfHosted() to return true (selfHosted mode)
			const isSelfHostedStub = sinon.stub(ForgeOSEndpoint, "isSelfHosted").returns(true)
		...
		it("should include PostHog config when NOT in selfHosted mode and config is valid", () => {
			// Stub ForgeOSEndpoint.isSelfHosted() to return false (normal mode)
			const isSelfHostedStub = sinon.stub(ForgeOSEndpoint, "isSelfHosted").returns(false)
			const isPostHogConfigValidStub = sinon.stub(posthogConfigModule, "isPostHogConfigValid").returns(true)
