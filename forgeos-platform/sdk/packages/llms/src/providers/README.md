...
Plugin hooks can observe ForgeOS's conversation state, but they run before Core's
...
Provider client fetch(...)
  captureStage = wire_request, only when FORGEOS_CAPTURE_WIRE=true
        |
        v
...
issue is in ForgeOS's final provider formatting/build step. If it is already in
...
| --- | --- | --- | --- |
| `FORGEOS_CAPTURE_PROVIDER_REQUEST` | `off`, `summary`, `full` | `off` | Enables provider request capture. |
| `FORGEOS_CAPTURE_WIRE` | `true`, `false` | `false` | Wraps provider `fetch` to capture literal request bodies. |
| `FORGEOS_CAPTURE_DIR` | filesystem path | unset | Explicit output directory for capture files. |
| `FORGEOS_CAPTURE_CLEANUP` | `on`, `off` | `on` | Prunes old capture files. Set `off` to keep local files. |
| `FORGEOS_CAPTURE_MAX_PREVIEW_BYTES` | positive integer | `65536` | Full-mode payload preview byte cap. |
| `FORGEOS_DATA_DIR` | filesystem path | unset | Fallback base directory. Captures write to `FORGEOS_DATA_DIR/provider-request-captures`. |

If neither `FORGEOS_CAPTURE_DIR` nor `FORGEOS_DATA_DIR` is set, capture no-ops. This
...
```text
${FORGEOS_CAPTURE_DIR}/<captureId>.<captureStage>.<attempt>.provider-request.json
```

or, when only `FORGEOS_DATA_DIR` is set:

```text
${FORGEOS_DATA_DIR}/provider-request-captures/<captureId>.<captureStage>.<attempt>.provider-request.json
```
...
When `FORGEOS_CAPTURE_CLEANUP` is on, the SDK opportunistically prunes capture
files older than 24 hours. Consumers may also delete files after processing them.
Set `FORGEOS_CAPTURE_CLEANUP=off` when you need to keep local capture files for
manual inspection.
...
- `payload`: only in `full` mode, truncated to `FORGEOS_CAPTURE_MAX_PREVIEW_BYTES`
...
```bash
export FORGEOS_DATA_DIR="$(mktemp -d)"
export FORGEOS_CAPTURE_PROVIDER_REQUEST=summary
export FORGEOS_CAPTURE_WIRE=true

forgeos --provider openrouter --model openai/gpt-4o-mini "Say hello"

ls "$FORGEOS_DATA_DIR/provider-request-captures"
```
...
```bash
export FORGEOS_DATA_DIR="$(mktemp -d)"
export FORGEOS_CAPTURE_PROVIDER_REQUEST=full
export FORGEOS_CAPTURE_WIRE=true
export FORGEOS_CAPTURE_MAX_PREVIEW_BYTES=1000000
# Optional: keep files after consumers process them.
# export FORGEOS_CAPTURE_CLEANUP=off
```
