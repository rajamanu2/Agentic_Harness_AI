...
describe("connector launch command", () => {
	const originalLaunchSpec = process.env[FORGEOS_CONNECTOR_CLI_LAUNCH_ENV];

	afterEach(() => {
		if (originalLaunchSpec === undefined) {
			delete process.env[FORGEOS_CONNECTOR_CLI_LAUNCH_ENV];
		} else {
			process.env[FORGEOS_CONNECTOR_CLI_LAUNCH_ENV] = originalLaunchSpec;
		}
	});
	...
	it("uses compiled CLI subcommands without Bun flags", () => {
		expect(
			__test__.buildCliConnectCommand(["telegram", "--bot-token", "token"], {
				execPath: "/Applications/ForgeOS/bin/forgeos",
				cliPath: "/repo/apps/cli/src/index.ts",
				exists: () => true,
			}),
		).toEqual({
