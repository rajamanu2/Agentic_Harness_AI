...
async function resolveProviderModelMap(
	...
	const isForgeOSPass = providerId === FORGEOS_PASS_PROVIDER_ID;
	if (!config && !isForgeOSPass) {
	...
	const resolved = await resolveProviderConfig(
		providerId,
		{
			loadLatestOnInit: isForgeOSPass,
	...
	if (isForgeOSPass && resolved?.knownModels) {
		return resolved.knownModels;
	}
...
export async function listLocalProviders(
	...
	if (options.isForgeOSPassEnabled !== true) {
		providers = providers.filter(
			(provider) => provider.id !== FORGEOS_PASS_PROVIDER_ID,
		);
	}
...
export function resolveLocalForgeOSAuthToken(
	settings: ProviderSettings | undefined,
): string | undefined {
