...
description: "Configure Single Sign-On (SSO) for ForgeOS Enterprise via WorkOS AuthKit."
---

## Overview
ForgeOS Enterprise integrates with your identity provider (IdP) via **WorkOS AuthKit** for SSO.

This page describes, at a high level, how SSO is set up for ForgeOS Enterprise using WorkOS AuthKit.
...
1) **ForgeOS Dashboard (app.forgeos.bot)**
...
## Using the ForgeOS Dashboard
Use the ForgeOS Dashboard at https://app.forgeos.bot to:
...
To connect your IdP to ForgeOS Enterprise, configure your identity provider in **WorkOS AuthKit**:
...
ForgeOS Enterprise’s default SSO integration is **via WorkOS**.
...
1) Attempt an SSO sign-in from https://app.forgeos.bot.
2) Confirm the sign-in completes (you are redirected back successfully).

## Troubleshooting
- **Redirect URI mismatch**: confirm the redirect/callback URL configured in WorkOS matches what was provided during your ForgeOS Enterprise onboarding.
