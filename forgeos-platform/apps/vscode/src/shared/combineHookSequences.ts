import { ForgeOSMessage } from "./ExtensionMessage"

/**
 * Hook metadata extracted from hook message text.
 * Mirrors the ForgeOSSayHook interface but represents parsed data.
 */
...
function getSay(msg: ForgeOSMessage): string | undefined {
 ...
 */
function isToolOrCommandMessage(msg: ForgeOSMessage): boolean {
 ...
 */
function parseHookMetadata(hookMessage: ForgeOSMessage): HookMetadata | null {
 ...
 */
function dedupeToolOrCommandMessagesByTimestamp(messages: ForgeOSMessage[]): ForgeOSMessage[] {
...
function combineHookWithOutputs(
	hookMessage: ForgeOSMessage,
	startIndex: number,
	messages: ForgeOSMessage[],
): { combined: ForgeOSMessage; nextIndex: number } {
 ...
 */
function combineAllHooks(messages: ForgeOSMessage[]): ForgeOSMessage[] {
	// Pass 1: Build map of combined hooks by timestamp
	const combinedHooksByTs = new Map<number, ForgeOSMessage>()
 ...
 */
function findImmediateNextToolTimestamp(hookIndex: number, messages: ForgeOSMessage[]): number | null {
 ...
 */
function buildPreToolUseMap(processedMessages: ForgeOSMessage[], originalMessages: ForgeOSMessage[]): Map<number, ForgeOSMessage[]> {
	const map = new Map<number, ForgeOSMessage[]>()
 ...
 */
function reorderWithPreToolUseHooks(messages: ForgeOSMessage[], preToolUseMap: Map<number, ForgeOSMessage[]>): ForgeOSMessage[] {
	const result: ForgeOSMessage[] = []
 ...
 *
 * @param messages Array of ForgeOSMessage objects to process
 * @returns New array with hooks combined and PreToolUse hooks reordered
 */
export function combineHookSequences(messages: ForgeOSMessage[]): ForgeOSMessage[] {
