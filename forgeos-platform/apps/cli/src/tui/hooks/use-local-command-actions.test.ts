...
describe("runLocalSlashCommandAction", () => {
	...
	it("exits ForgeOS with quit", () => {
		vi.useFakeTimers();
		const exitForgeOS = vi.fn();
		const actions = makeActions({ exitForgeOS });
		...
		try {
			...
			expect(handled).toBe(true);
			expect(exitForgeOS).not.toHaveBeenCalled();

			vi.runAllTimers();

			expect(exitForgeOS).toHaveBeenCalledOnce();
		} finally {
			vi.useRealTimers();
		}
	});
