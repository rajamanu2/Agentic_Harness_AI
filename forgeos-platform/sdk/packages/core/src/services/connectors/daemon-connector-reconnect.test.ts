import { FORGEOS_CONNECTOR_STARTING_INSTANCE_ENV } from "@forgeos/shared";
...
describe("reconnectDaemonConnectors", () => {
	const originalStartingInstance =
		process.env[FORGEOS_CONNECTOR_STARTING_INSTANCE_ENV];

	afterEach(() => {
		vi.clearAllMocks();
		if (originalStartingInstance === undefined) {
			delete process.env[FORGEOS_CONNECTOR_STARTING_INSTANCE_ENV];
		} else {
			process.env[FORGEOS_CONNECTOR_STARTING_INSTANCE_ENV] =
	...
	it("starts a persisted connector through the supervisor", async () => {
		...
		mocks.reconnectPersistedConnectors.mockImplementation(async (options) => {
			...
			return [{ channel: "telegram", instanceId: "forgeos_bot", ok }];
		});

		await expect(
			reconnectDaemonConnectors(vi.fn(), supervisor),
		).resolves.toEqual([
			{ channel: "telegram", instanceId: "forgeos_bot", ok: true },
		]);
	...
	it("restarts a connector that survived the previous hub", async () => {
		delete process.env[FORGEOS_CONNECTOR_STARTING_INSTANCE_ENV];
		const { supervisor, starts } = createSupervisor({
			supervised: [{ channel: "telegram", instanceId: "forgeos_bot" }],
		});
		const log = vi.fn();
		mocks.reconnectPersistedConnectors.mockImplementation(async (options) => {
			await options.start({
		...
		expect(log).toHaveBeenCalledWith(
			"[connect] restarting surviving telegram connector forgeos_bot for the new hub session",
		);
	});

	it("does not reconnect the connector instance that is starting this daemon", async () => {
		process.env[FORGEOS_CONNECTOR_STARTING_INSTANCE_ENV] = JSON.stringify({
			channel: "telegram",
			instanceId: "forgeos_bot",
		});
		...
		expect(isHealthy?.({ channel: "telegram", instanceId: "forgeos_bot" })).toBe(
			true,
		);
		...
		expect(isHealthy?.({ channel: "slack", instanceId: "forgeos_bot" })).toBe(
			false,
		);
	});
	...
	it("reports an already-running instance without treating it as started", async () => {
		...
		mocks.reconnectPersistedConnectors.mockImplementation(async (options) => {
			...
			return [{ channel: "slack", instanceId: "forgeos-slack", ok }];
		});

		await expect(reconnectDaemonConnectors(log, supervisor)).resolves.toEqual([
			{ channel: "slack", instanceId: "forgeos-slack", ok: false },
		]);
		expect(log).toHaveBeenCalledWith(
			"[connect] slack connector forgeos-slack is already running under this hub",
		);
	});
