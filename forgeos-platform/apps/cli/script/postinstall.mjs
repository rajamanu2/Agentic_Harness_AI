...
// Post-install script for ForgeOS CLI.
//
// Creates a hard link (or copy fallback) from the platform-specific binary
// to bin/.forgeos for fast startup on subsequent runs.
//
...
function shieldRunningHubDiscovery() {
	const explicitPath = process.env.FORGEOS_HUB_DISCOVERY_PATH?.trim();
	const dataDir =
		process.env.FORGEOS_DATA_DIR?.trim() ||
		path.join(
			process.env.FORGEOS_DIR?.trim() || path.join(os.homedir(), ".forgeos"),
			"data",
		);
	const recordPath =
...
function main() {
	...
	const arch = os.arch();
	const packageName = `@forgeos/cli-${platform}-${arch}`;
	const binaryName = "forgeos";
	...
	const binDir =
		path.basename(__dirname) === "script"
			...
			: path.join(__dirname, "bin");
	const target = path.join(binDir, ".forgeos");
	...
	console.log(`Cached forgeos binary at ${target}`);
}
