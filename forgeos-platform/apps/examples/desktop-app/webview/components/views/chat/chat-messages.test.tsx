...
describe("ChatMessages tool disclosures", () => {
	it.each([
		...
		const icon = container.querySelector(".forgeos-chat-tool-icon svg");
	...
	it("renders a detail-less tool summary as static text", async () => {
		...
		expect(
			container.querySelector(".forgeos-chat-tool")?.classList.contains("my-0"),
		).toBe(true);
	});
	...
	it("shimmers a tool title only while its result is pending", async () => {
		...
		const pendingTitle = container.querySelector(
			".forgeos-chat-tool-label > span",
		);
		expect(pendingTitle?.classList.contains("forgeos-chat-streaming-title")).toBe(
			true,
		);
		...
		expect(
			completedTitle?.classList.contains("forgeos-chat-streaming-title"),
		).toBe(false);
	});
	...
	it("renders consecutive tool calls as individual rows", async () => {
		...
		expect(container.querySelectorAll(".forgeos-chat-tool")).toHaveLength(5);
	...
	it("pre-expands tool groups that contain edit diffs", async () => {
		...
		const [editBlock, readBlock] = [
			...container.querySelectorAll(".forgeos-chat-tool"),
		];
		// The edit group's diff panel is visible without a click…
		expect(editBlock?.querySelector(".forgeos-chat-tool-content")).not.toBeNull();
		// …while the read group stays collapsed.
		expect(readBlock?.querySelector(".forgeos-chat-tool-content")).toBeNull();
	});
	...
	it("opens a streaming group when an edit diff arrives after mount", async () => {
		...
		expect(container.querySelector(".forgeos-chat-tool-content")).toBeNull();
		...
		expect(container.querySelector(".forgeos-chat-tool-content")).not.toBeNull();
	});
	...
	it("preserves interleaved tool activity order", async () => {
		...
		const labels = [...container.querySelectorAll(".forgeos-chat-tool")].map(
			(row) => row.textContent ?? "",
		);
	...
	it("spaces reasoning, content, and tool blocks with a single gap-2", async () => {
		...
		const message = container.querySelector(
			'.forgeos-chat-message[data-role="assistant"]',
		);
		const messageList = message?.parentElement;
		const content = message?.querySelector(".forgeos-chat-message-content");
		...
		for (const element of [
			message,
			content?.querySelector(".forgeos-chat-reasoning"),
			content?.lastElementChild,
			container.querySelector(".forgeos-chat-tool"),
		]) {
	...
	it("positions hidden message actions outside the message layout", async () => {
		...
		const userMessage = container.querySelector(
			'.forgeos-chat-message[data-role="user"]',
		);
		const userActions = userMessage?.querySelector(
			":scope > .forgeos-chat-message-actions",
		);
		...
		const userAction = userActions?.querySelector(".forgeos-chat-message-action");
		expect(userAction?.getAttribute("data-slot")).toBe("icon-button");
		const assistantActionButtons = [
			...(assistantActions?.querySelectorAll(".forgeos-chat-message-action") ??
				[]),
		];
	...
	it("leaves vertical scrolling to the conversation viewport", async () => {
		...
		const content = container.querySelector(".forgeos-chat-conversation-content");
...
describe("ChatMessages reasoning disclosure", () => {
	it("shimmers the thinking title only while reasoning is streaming", async () => {
		...
		const streamingTitle = container.querySelector(
			".forgeos-chat-reasoning-trigger > span",
		);
		expect(
			streamingTitle?.classList.contains("forgeos-chat-streaming-title"),
		).toBe(true);
	...
	it("shows elapsed thinking time with the border-left disclosure style", async () => {
		...
		expect(trigger?.querySelector(".forgeos-chat-disclosure-icon")).toBeNull();
		expect(trigger?.classList.contains("text-sm")).toBe(true);
		expect(trigger?.classList.contains("text-xs")).toBe(false);

		await act(async () => trigger?.click());

		const content = container.querySelector(".forgeos-chat-reasoning-content");
		expect(trigger?.getAttribute("aria-expanded")).toBe("true");
	...
	it("hangs expanded reasoning and tool panels off the same left rail", async () => {
		...
		const reasoningTrigger = container.querySelector(
			".forgeos-chat-reasoning-trigger",
		) as HTMLButtonElement | null;
		const toolTrigger = container.querySelector(
			".forgeos-chat-tool-trigger",
		) as HTMLButtonElement | null;
		await act(async () => {
		...
		const reasoningContent = container.querySelector(
			".forgeos-chat-reasoning-content",
		);
		const toolContent = container.querySelector(".forgeos-chat-tool-content");
		...
		const details = toolContent?.querySelector(".forgeos-chat-tool-details");
	...
	it("keeps the reasoning panel inside the shape the hover-suppress rule targets", async () => {
		...
		expect(
			actions?.matches(
				".forgeos-chat-message:has(> .forgeos-chat-message-content .forgeos-chat-reasoning) > .forgeos-chat-message-actions:not([data-visible='true'])",
			),
		).toBe(true);
	...
	it("combines consecutive assistant reasoning into one disclosure", async () => {
		...
		const disclosures = container.querySelectorAll(".forgeos-chat-reasoning");
	...
	it("keeps reasoning disclosures separate across tool activity", async () => {
		...
		expect(container.querySelectorAll(".forgeos-chat-reasoning")).toHaveLength(2);
	});
	...
	it("shows while starting", async () => {
		...
		expect(
			[...container.querySelectorAll("span")]
				...
				?.classList.contains("forgeos-chat-streaming-title"),
		).toBe(true);
	});
...
describe("ChatMessages tool approvals", () => {
	it("renders the shared card and forwards its decisions", async () => {
		...
		const card = container.querySelector(".forgeos-ui-agent-approval-card");
