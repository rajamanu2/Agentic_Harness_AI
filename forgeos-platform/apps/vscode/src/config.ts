...
/**
 * Error thrown when the ForgeOS configuration file exists but is invalid.
 * This error prevents ForgeOS from starting to avoid misconfiguration in enterprise environments.
 */
export class ForgeOSConfigurationError extends Error {
	constructor(message: string) {
		super(message)
		this.name = "ForgeOSConfigurationError"
	}
}

class ForgeOSEndpoint {
	private static _instance: ForgeOSEndpoint | null = null
	...
	private constructor() {
		...
		const _env = process?.env?.FORGEOS_ENVIRONMENT_OVERRIDE || process?.env?.FORGEOS_ENVIRONMENT
	...
	/**
	 * Initializes the ForgeOSEndpoint singleton.
	 ...
	 * @throws ForgeOSConfigurationError if the endpoints.json file exists but is invalid
	 */
	public static async initialize(extensionFsPath: string): Promise<void> {
		if (ForgeOSEndpoint._initialized) {
			return
		}

		ForgeOSEndpoint._extensionFsPath = extensionFsPath
		ForgeOSEndpoint._instance = new ForgeOSEndpoint()

		// Try to load on-premise config from file
		const endpointsConfig = await ForgeOSEndpoint.loadEndpointsFile()
		if (endpointsConfig) {
			ForgeOSEndpoint._instance.onPremiseConfig = endpointsConfig
			Logger.log("ForgeOS running in self-hosted mode with custom endpoints")
		}

		ForgeOSEndpoint._initialized = true
	}

	/**
	 * Returns true if the ForgeOSEndpoint has been initialized.
	 */
	public static isInitialized(): boolean {
		return ForgeOSEndpoint._initialized
	}

	/**
	 * Checks if ForgeOS is running in self-hosted/on-premise mode.
	...
	public static isSelfHosted(): boolean {
		...
		return ForgeOSEndpoint.config.environment === Environment.selfHosted
	}
	...
	public static isBundledConfig(): boolean {
		if (!ForgeOSEndpoint._initialized || !ForgeOSEndpoint._instance) {
			throw new Error("ForgeOSEndpoint not initialized. Call ForgeOSEndpoint.initialize() first.")
		}
		return ForgeOSEndpoint._instance.isBundled
	}
	 ...
	 * @throws Error if not initialized
	 */
	public static get instance(): ForgeOSEndpoint {
		if (!ForgeOSEndpoint._initialized || !ForgeOSEndpoint._instance) {
			throw new Error("ForgeOSEndpoint not initialized. Call ForgeOSEndpoint.initialize() first.")
		}
		return ForgeOSEndpoint._instance
	}
	...
	public static get config(): EnvironmentConfig {
		return ForgeOSEndpoint.instance.config()
	}

	/**
	 * Returns the path to the endpoints.json configuration file.
	 * Located at ~/.forgeos/endpoints.json
	 */
	private static getEndpointsFilePath(): string {
		return path.join(os.homedir(), ".forgeos", "endpoints.json")
	}
	...
	private static getBundledEndpointsFilePath(): string {
		return path.join(ForgeOSEndpoint._extensionFsPath, "endpoints.json")
	}

	/**
	 * Loads and validates the endpoints.json file.
	 * Checks bundled location first, then falls back to user directory.
	 * Priority: bundled endpoints.json → ~/.forgeos/endpoints.json → null (standard mode)
	 * @returns The validated endpoints config, or null if no file exists
	 * @throws ForgeOSConfigurationError if a file exists but is invalid
	 */
	private static async loadEndpointsFile(): Promise<EndpointsFileSchema | null> {
		// 1. Try bundled file
		const bundledPath = ForgeOSEndpoint.getBundledEndpointsFilePath()
		try {
			...
			try {
				...
				throw new ForgeOSConfigurationError(
			...
			const config = ForgeOSEndpoint.validateEndpointsSchema(data, bundledPath)
			// Mark as bundled enterprise distribution
			ForgeOSEndpoint._instance!.isBundled = true
			return config
		} catch (error) {
			if (error instanceof ForgeOSConfigurationError) {
				throw error
			}
			// Bundled file doesn't exist or is not accessible, try user file
		}

		// 2. Try ~/.forgeos/endpoints.json
		const userPath = ForgeOSEndpoint.getEndpointsFilePath()
		try {
		...
		try {
			...
			return ForgeOSEndpoint.validateEndpointsSchema(data, userPath)
		} catch (error) {
			if (error instanceof ForgeOSConfigurationError) {
				throw error
			}
	 ...
	 * @throws ForgeOSConfigurationError if validation fails
	 */
	private static validateEndpointsSchema(data: unknown, filePath: string): EndpointsFileSchema {
		if (typeof data !== "object" || data === null) {
			throw new ForgeOSConfigurationError(`Endpoints configuration file (${filePath}) must contain a JSON object`)
		}
	...
	public setEnvironment(env: string) {
		if (this.onPremiseConfig) {
			throw new Error("Cannot change environment in on-premise mode. Endpoints are configured via ~/.forgeos/endpoints.json")
		}
 ...
 * Usage:
 * - ForgeOSEnv.config() to get the current config.
 * - ForgeOSEnv.setEnvironment(Environment.local) to change the environment.
 *
 * IMPORTANT: ForgeOSEndpoint.initialize() must be called before using ForgeOSEnv.
 */
export const ForgeOSEnv = {
	config: () => ForgeOSEndpoint.config,
	setEnvironment: (env: string) => ForgeOSEndpoint.instance.setEnvironment(env),
	getEnvironment: () => ForgeOSEndpoint.instance.getEnvironment(),
}

// Export the class for initialization
export { ForgeOSEndpoint }
