...
async function createProviderModule(
	...
	switch (kind) {
		case "forgeos": {
			const { createForgeOSProviderModule } = await import("./vendors/forgeos");
			return createForgeOSProviderModule(config, context);
		}
		case "openai": {
 ...
 * forgeos, generic OpenAI-compatible endpoints), not just local Ollama. An
...
export const createForgeOSProvider = createAiSdkProvider("forgeos");
