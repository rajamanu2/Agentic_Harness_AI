 ...
 */
export function isLowStakesTool(message: ForgeOSMessage): boolean {
 ...
 */
export function isToolGroup(item: ForgeOSMessage | ForgeOSMessage[]): item is ForgeOSMessage[] & { _isToolGroup: true } {
	return Array.isArray(item) && (item as ForgeOSMessage[] & { _isToolGroup?: boolean })._isToolGroup === true
}

function isDuplicateAskOptionEcho(message: ForgeOSMessage, previousMessage: ForgeOSMessage | undefined): boolean {
	if (
		message.type !== "say" ||
	...
	try {
		const parsed = JSON.parse(previousMessage.text || "{}") as ForgeOSAskQuestion | ForgeOSPlanModeResponse
...
function isVisibleCheckpointUserMessage(message: ForgeOSMessage): boolean {
	return message.type === "say" && (message.say === "task" || message.say === "user_feedback")
}

function isCheckpointAnswerMessage(messages: ForgeOSMessage[], index: number): boolean {
...
export function canRestoreWorkspaceFromMessage(messages: ForgeOSMessage[], messageTs: number | undefined): boolean {
 ...
 */
export function filterVisibleMessages(messages: ForgeOSMessage[]): ForgeOSMessage[] {
	return messages.filter((message, index, arr) => {
		...
		switch (message.ask) {
			case "completion_result":
				// don't show a chat row for a completion_result ask without text. This specific type of message only occurs if forgeos wants to execute a command as part of its completion result, in which case we interject the completion_result tool with the execute_command tool.
				if (message.text === "") {
					return false
				}
				break
		...
		switch (message.say) {
			...
			case "text":
				// Sometimes forgeos returns an empty text message, we don't want to render these. (We also use a say text for user messages, so in case they just sent images we still render that)
 ...
 */
function isBrowserSessionMessage(message: ForgeOSMessage): boolean {
	if (message.type === "ask") {
 ...
 */
export function groupMessages(visibleMessages: ForgeOSMessage[]): (ForgeOSMessage | ForgeOSMessage[])[] {
	const result: (ForgeOSMessage | ForgeOSMessage[])[] = []
	let currentGroup: ForgeOSMessage[] = []
		...
		if (message.ask === "browser_action_launch" || message.say === "browser_action_launch") {
			...
			if (isBrowserSessionMessage(message)) {
				...
				if (message.say === "browser_action") {
					const browserAction = JSON.parse(message.text || "{}") as ForgeOSSayBrowserAction
 ...
 */
export function getToolsNotInCurrentActivities(toolGroupMessages: ForgeOSMessage[], allMessages: ForgeOSMessage[]): ForgeOSMessage[] {
	...
	let mostRecentApiReq: ForgeOSMessage | null = null
 ...
 *
 * Note: this operates on a flat `ForgeOSMessage[]` (e.g. `modifiedMessages`) rather than
 ...
 */
export function isApiReqAbsorbable(apiReqTs: number, allMessages: ForgeOSMessage[]): boolean {
	const apiReqIndex = allMessages.findIndex((m) => m.ts === apiReqTs && m.say === "api_req_started")
	if (apiReqIndex === -1) {
		return false
	}
 ...
 */
function isApiReqFollowedOnlyByLowStakesTools(index: number, messages: (ForgeOSMessage | ForgeOSMessage[])[]): boolean {
	let hasLowStakesTool = false
	let hasReasoning = false
 ...
 */
export function groupLowStakesTools(groupedMessages: (ForgeOSMessage | ForgeOSMessage[])[]): (ForgeOSMessage | ForgeOSMessage[])[] {
	const result: (ForgeOSMessage | ForgeOSMessage[])[] = []
	let toolGroup: ForgeOSMessage[] = []
	let pendingReasoning: ForgeOSMessage[] = []
	let pendingApiReq: ForgeOSMessage[] = []
	let hasTools = false
	const pendingTools: ForgeOSMessage[] = []
	...
	const commitToolGroup = () => {
		if (toolGroup.length > 0 && hasTools) {
			const group = toolGroup as ForgeOSMessage[] & { _isToolGroup: boolean }
