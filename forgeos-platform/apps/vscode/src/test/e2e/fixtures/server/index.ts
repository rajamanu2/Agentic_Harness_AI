...
import type { BalanceResponse, OrganizationBalanceResponse, UserResponse } from "../../../../shared/ForgeOSAccount"
import {
	E2E_MOCK_API_RESPONSES,
	E2E_MOCK_FORGEOS_MODELS,
	E2E_MOCK_FORGEOS_RECOMMENDED_MODELS,
	E2E_MOCK_EDITOR_TOOL_CALL,
	E2E_MOCK_POWERSHELL_TOOL_CALL,
	E2E_REGISTERED_MOCK_ENDPOINTS,
} from "./api"
import { ForgeOSDataMock } from "./data"

const E2E_API_SERVER_PORT = 7777

export const MOCK_FORGEOS_API_SERVER_URL = `http://localhost:${E2E_API_SERVER_PORT}`

const useVerboseLogging = process.env.FORGEOS_E2E_TESTS_VERBOSE === "true"
function log(...args: unknown[]) {
	if (useVerboseLogging) {
		console.log("[ForgeOSApiServerMock]", ...args)
	}
}

export class ForgeOSApiServerMock {
	static globalSharedServer: ForgeOSApiServerMock | null = null
	...
	public readonly API_USER = new ForgeOSDataMock("personal")
	...
	public static async startGlobalServer(): Promise<ForgeOSApiServerMock> {
		log("=== SERVER FIXTURE CALLED ===")
		if (ForgeOSApiServerMock.globalSharedServer) {
			log("Using existing global server")
			return ForgeOSApiServerMock.globalSharedServer
		}

		log("Starting global server...")
		const server = createServer((req: IncomingMessage, res: ServerResponse) => {
			// Parse URL and method
			const parsedUrl = new URL(req.url || "/", MOCK_FORGEOS_API_SERVER_URL)
			...
			if (isAuthRequired && authToken) {
				...
				const user = ForgeOSApiServerMock.globalSharedServer!.API_USER.getUserByToken(normalizedAuthToken)
				if (!user) {
					return sendApiError("Invalid token", 401)
				}
				ForgeOSApiServerMock.globalSharedServer!.setCurrentUser(user)
			}
			...
			const handleRequest = async () => {
				...
				const routeMatch = ForgeOSApiServerMock.matchRoute(path, method)
				...
				const controller = ForgeOSApiServerMock.globalSharedServer!
				...
				// API v1 endpoints
				if (baseRoute === "/api/v1") {
					if (endpoint === "/ai/forgeos/recommended-models" && method === "GET") {
						return sendJson(E2E_MOCK_FORGEOS_RECOMMENDED_MODELS)
					}

					if (endpoint === "/ai/forgeos/models" && method === "GET") {
						return sendJson({ data: E2E_MOCK_FORGEOS_MODELS })
					}
					...
					if (endpoint === "/auth/token" && method === "POST") {
						...
						// Return format matching ForgeOSAuthProvider expectations
						return sendApiResponse({
							...
							choices: [
								{
									...
									message: {
										role: "assistant",
										content: "Hello! I'm a mock ForgeOS API response.",
									},
									finish_reason: "stop",
								},
							],
		...
		const controller = new ForgeOSApiServerMock(server)
		ForgeOSApiServerMock.globalSharedServer = controller

		// Track connections for proper cleanup
		server.on("connection", (socket) => {
			ForgeOSApiServerMock.globalSockets.add(socket)
			socket.on("close", () => {
				ForgeOSApiServerMock.globalSockets.delete(socket)
			})
		})
		...
		await new Promise<void>((resolve, reject) => {
			server.listen(E2E_API_SERVER_PORT, (error?: Error) => {
				if (error) {
					...
					log(`ForgeOSApiServerMock listening on port ${E2E_API_SERVER_PORT}`)
					resolve()
				}
			})
		})
	...
	public static async stopGlobalServer(): Promise<void> {
		...
		const server = ForgeOSApiServerMock.globalSharedServer.server

		// Clean shutdown - destroy all socket connections first
		ForgeOSApiServerMock.globalSockets.forEach((socket) => socket.destroy())
		ForgeOSApiServerMock.globalSockets.clear()
		...
		ForgeOSApiServerMock.globalSharedServer = null
	}
}
