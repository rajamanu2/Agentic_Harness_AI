...
import type { GeneratedMedia } from "@forgeos/shared"
...
import { ForgeOSFeatureSetting } from "./ForgeOSFeatureSetting"
import { BannerCardData } from "./forgeos/banner"
import { ForgeOSRulesToggles } from "./forgeos-rules"
import { HistoryItem } from "./HistoryItem"
import { McpDisplayMode } from "./McpDisplayMode"
import { ForgeOSMessageModelInfo } from "./messages"
import { OnboardingModelGroup } from "./proto/forgeos/state"
...
export const COMMAND_CANCEL_TOKEN = "__forgeos_command_cancel__"
export interface ExtensionState {
	 ...
	 * renders the footer/buttons/thinking indicator from this, NOT from the tail of forgeosMessages.
	...
	stateVersion?: number
	/**
	 * Conversation/replica fence for this snapshot (see ForgeOSMessage.epoch). A snapshot with a
	...
	distinctId: string
	globalForgeOSRulesToggles: ForgeOSRulesToggles
	localForgeOSRulesToggles: ForgeOSRulesToggles
	localWorkflowToggles: ForgeOSRulesToggles
	globalWorkflowToggles: ForgeOSRulesToggles
	localCursorRulesToggles: ForgeOSRulesToggles
	localWindsurfRulesToggles: ForgeOSRulesToggles
	remoteRulesToggles?: ForgeOSRulesToggles
	remoteWorkflowToggles?: ForgeOSRulesToggles
	localAgentsRulesToggles: ForgeOSRulesToggles
	mcpResponsesCollapsed?: boolean
	useAutoCondense?: boolean
	compactionStrategy?: string
	webSearchEnabled?: boolean
	subagentsEnabled?: boolean
	worktreesEnabled?: ForgeOSFeatureSetting
	favoritedModelIds: string[]
	// NEW: Add workspace information
	workspaceRoots: WorkspaceRoot[]
	primaryRootIndex: number
	isMultiRootWorkspace: boolean
	multiRootSetting: ForgeOSFeatureSetting
 ...
 * this instead of inferring mode from the tail of forgeosMessages.
 */
...
export interface TurnState {
	phase: TurnPhase
	/** ts of the ForgeOSMessage this phase is "about" (e.g. the pending approval/ask). */
	anchorTs?: number
...
export interface ForgeOSMessage {
	ts: number
	type: "ask" | "say"
	ask?: ForgeOSAsk
	say?: ForgeOSSay
...
export interface ForgeOSSayTool {
	tool:
		| "editedExistingFile"
		| "newFileCreated"
		| "fileDeleted"
...
export interface ForgeOSSayBrowserAction {
...
export interface ForgeOSSaySubagentStatus {
...
export interface ForgeOSAskUseMcpServer {
	serverName: string
...
export interface ForgeOSAskUseSubagents {
	prompts: string[]
}

export interface ForgeOSPlanModeResponse {
	response: string
	options?: string[]
	selected?: string
}

export interface ForgeOSAskQuestion {
	question: string
	options?: string[]
	selected?: string
}

export interface ForgeOSApiReqInfo {
	request?: string
	tokensIn?: number
	tokensOut?: number
	cacheWrites?: number
	cacheReads?: number
	cost?: number
	cancelReason?: ForgeOSApiReqCancelReason
 ...
 */
export interface ForgeOSCompactionInfo {
...
export interface ForgeOSSubagentUsageInfo {
	source: "subagents"
	tokensIn: number
	tokensOut: number
	cacheWrites: number
...
type ForgeOSApiReqCancelReason = "streaming_failed" | "user_cancelled" | "retries_exhausted"
