...
and debugging ForgeOS's MCP OAuth flow locally.
...
2. **MCP StreamableHTTP resource server**:
   - `POST /mcp` — returns `401 + WWW-Authenticate: Bearer resource_metadata="..."`
     until authenticated (this is what triggers ForgeOS's OAuth flow), then a
...
- **State expiry** — ForgeOS's `McpOAuthManager` enforces a state lifetime
...
- **Denial** — the consent page's Deny button (or `--auto-deny`) redirects back
  with `error=access_denied`, so you can observe how ForgeOS handles a denial.
...
the nested `transport` shape used by `forgeos_mcp_settings.json`) in addition to
the banner — merge it under `mcpServers` in
`~/.forgeos/data/settings/forgeos_mcp_settings.json`. Or add an MCP server
(StreamableHTTP) in ForgeOS by hand, pointing at:
...
| `--instances <n>` | Start N independent servers, each on its own random port (implies `--random-port`). Use to add several MCP servers to ForgeOS at once. |
...
each one to ForgeOS as a separate StreamableHTTP server to exercise concurrent
...
Because OAuth state is keyed by **server name** in `forgeos_mcp_settings.json`,
each ForgeOS server entry gets its own independent tokens — even if two point at
the same URL.

## Reproducing specific bugs

**"OAuth state expired" race** — make the user take longer than ForgeOS's
...
- Under `FORGEOS_CAPTURE_BROWSER=1` (see `src/utils/env.ts`), the authorization URL
  ForgeOS tries to open is captured instead of launched. The harness `curl`s the
  ...
  extension via `globalThis.__forgeosHandleUri(...)` (see the debug harness README,
  "Testing MCP OAuth").
