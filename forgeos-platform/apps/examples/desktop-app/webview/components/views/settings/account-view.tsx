...
const DASHBOARD_URL = "https://app.forgeos.bot/dashboard";
const USER_CREDITS_URL =
	"https://app.forgeos.bot/dashboard/account?tab=credits&redirect=true";
const ORGANIZATION_CREDITS_URL =
	"https://app.forgeos.bot/dashboard/organization?tab=credits&redirect=true";
const CREATE_ORGANIZATION_URL = "https://app.forgeos.bot/onboarding?step=1";
const CREATE_ACCOUNT_URL = "https://app.forgeos.bot";

function normalizeAccountViewError(error: unknown): Error {
	const message = error instanceof Error ? error.message : String(error);
	if (message.includes("unsupported desktop command: forgeos_account")) {
		return new Error(
...
function isAccountAuthError(message: string): boolean {
	...
	return (
		normalized.includes("no forgeos account auth token found") ||
...
async function fetchAccountUser(): Promise<ForgeOSAccountUser> {
	return await desktopClient.invoke<ForgeOSAccountUser>("forgeos_account", {
		action: "forgeosAccount",
		operation: "fetchMe",
	});
}

async function fetchAccountBalance(): Promise<ForgeOSAccountBalance> {
	return await desktopClient.invoke<ForgeOSAccountBalance>("forgeos_account", {
		action: "forgeosAccount",
...
async function fetchAccountOrganizations(): Promise<
	ForgeOSAccountOrganization[]
> {
	return await desktopClient.invoke<ForgeOSAccountOrganization[]>(
		"forgeos_account",
		{
			action: "forgeosAccount",
...
async function fetchOrganizationBalance(
	...
	return await desktopClient.invoke<ForgeOSAccountOrganizationBalance>(
		"forgeos_account",
		{
			action: "forgeosAccount",
...
async function fetchUsageTransactions(): Promise<
	ForgeOSAccountUsageTransaction[]
> {
	return await desktopClient.invoke<ForgeOSAccountUsageTransaction[]>(
		"forgeos_account",
		{
			action: "forgeosAccount",
...
async function fetchOrganizationUsageTransactions(
	...
	return await desktopClient.invoke<ForgeOSAccountOrganizationUsageTransaction[]>(
		"forgeos_account",
		{
			action: "forgeosAccount",
...
async function fetchPaymentTransactions(): Promise<
	ForgeOSAccountPaymentTransaction[]
> {
	return await desktopClient.invoke<ForgeOSAccountPaymentTransaction[]>(
		"forgeos_account",
		{
			action: "forgeosAccount",
...
async function switchActiveAccount(
	...
	await desktopClient.invoke("forgeos_account", {
		action: "forgeosAccount",
...
export function AccountView() {
	...
	// Overview data
	const [user, setUser] = useState<ForgeOSAccountUser | null>(null);
	const [balance, setBalance] = useState<ForgeOSAccountBalance | null>(null);
	const [organizationBalance, setOrganizationBalance] =
		useState<ForgeOSAccountOrganizationBalance | null>(null);
	...
	const loadOverview = useCallback(async () => {
		...
		try {
			...
			if (isForgeOSAccountNotAuthenticatedResult(userData)) {
				resetAccountData();
				setSignedOut(true);
				return;
			}
			...
			if (
				isForgeOSAccountNotAuthenticatedResult(balanceData) ||
				isForgeOSAccountNotAuthenticatedResult(orgsData)
			) {
				resetAccountData();
				setSignedOut(true);
				return;
			...
			const organizationBalanceData = nextActiveOrganization
				...
				: null;
			if (isForgeOSAccountNotAuthenticatedResult(organizationBalanceData)) {
				resetAccountData();
				setSignedOut(true);
				return;
			}
	...
	const loadUsage = useCallback(async () => {
		...
		try {
			...
			if (isForgeOSAccountNotAuthenticatedResult(data)) {
				resetAccountData();
				setSignedOut(true);
				setActiveTab("overview");
				return;
			...
			<div className="mx-auto flex max-w-xl flex-col items-center gap-4 py-8 text-center">
				...
				<div>
					...
					<p className="mt-2 text-sm text-muted-foreground">
						Connect your ForgeOS account to review credits, usage, billing, and
