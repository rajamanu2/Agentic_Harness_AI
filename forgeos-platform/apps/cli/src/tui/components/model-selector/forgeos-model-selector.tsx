...
export function ForgeOSModelSelectorContent(
			...
			if (entry.kind === "model") {
				if (entry.tier !== lastTier) {
					...
					rows.push({
						key: `tier-${entry.tier}`,
						kind: "header",
						label: FORGEOS_MODEL_PICKER_TIER_LABELS[entry.tier],
						description:
				...
				rows.push({
					key: entry.model.id,
					kind: "model",
					// Names arrive display-ready from fetchForgeOSRecommendedModels
...
export function ForgeOSModelSelectorDialogContent(
	props: ChoiceContext<string> & {
		currentModel: string;
		currentProviderName: string;
		loadEntries: () => Promise<ForgeOSModelPickerEntry[]>;
	},
) {
	const { dismiss, dialogId, loadEntries } = props;
	const [state, setState] = useState<ForgeOSModelEntriesState>({
		status: "loading",
		message: "Loading ForgeOS models...",
	});
	...
	const reload = useCallback(async () => {
		...
		setState({ status: "loading", message: "Loading ForgeOS models..." });
		try {
	...
	if (state.status === "loaded") {
		return <ForgeOSModelSelectorContent {...props} entries={state.entries} />;
	}
