import { HookInfo, HooksToggles, WorkspaceHooks } from "@shared/proto/forgeos/file"
...
export async function refreshHooks(
	...
	const globalHooksDir = globalHooksDirOverride || path.join(os.homedir(), "Documents", "ForgeOS", "Hooks")
	...
	for (const workspacePath of workspacePaths.paths) {
		const workspaceHooksDir = path.join(workspacePath, ".forgeosrules", "hooks")
