...
const HOST_PROVIDED_SDK_PREFIX = "@forgeos/";
...
function getPluginRoot(cwd: string | undefined): string {
	return cwd
		? join(cwd, ".forgeos", "plugins")
		: join(resolveForgeOSDir(), "plugins");
}
...
function getManifestPaths(manifest: PluginPackageManifest | null): string[] {
	const entries = manifest?.forgeos?.plugins;
...
function removeInstalledHostProvidedSdkDependencies(
	...
	const forgeosScopeDir = join(packageRoot, "node_modules", "@forgeos");
	if (!existsSync(forgeosScopeDir)) {
		return;
	}
	for (const entry of statSafeReadDir(forgeosScopeDir)) {
		const packageName = `@forgeos/${entry.name}`;
		if (packageName === preservePackageName) {
			continue;
		}
		rmSync(join(forgeosScopeDir, entry.name), {
			recursive: true,
			force: true,
		});
	}
...
async function installNpmPackage(
	...
	await writeFile(
		...
		JSON.stringify({ name: "forgeos-plugin-install", private: true }, null, 2),
		"utf8",
	);
...
async function installOfficialPlugin(
	...
	if (!existsSync(sourceRoot) || !statSync(sourceRoot).isDirectory()) {
		throw new Error(
			`Official ForgeOS plugin "${parsed.slug}" was not found at plugins/${parsed.slug} in ${officialPluginsRepo}`,
		);
	}
...
export async function installPlugin(
	...
	const npmCommand =
		options.npmCommand ?? (process.env.FORGEOS_NPM_COMMAND?.trim() || "npm");
