import { captureSdkError } from "@forgeos/shared";
import type { ForgeOSCoreOptions } from "../../forgeos-core/types";
import {
...
function resolveConfiguredBackendMode(
	options: ForgeOSCoreOptions,
): RuntimeHostMode {
	if (options.backendMode) {
		return options.backendMode;
	}
	if (process.env.FORGEOS_VCR?.trim()) {
		return "local";
	}
	const raw = process.env.FORGEOS_SESSION_BACKEND_MODE?.trim().toLowerCase();
...
function createLocalBackend(options: ForgeOSCoreOptions): SessionBackend {
	try {
