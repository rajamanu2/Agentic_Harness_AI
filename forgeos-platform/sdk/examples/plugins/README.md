# ForgeOS Plugin Examples

A plugin is a single file (or directory) that extends any ForgeOS agent — CLI, Kanban, VS Code, JetBrains, or anything built on the Core SDK. Drop one in, get new tools, hooks, providers, or message rewriters everywhere.
...
| [automation-events.ts](./automation-events.ts) | Plugin-emitted automation events | Registers a normalized `local.plugin_event` automation event type and, when `FORGEOS_LOCAL_EVENT_INTERVAL_MS` is set, periodically emits demo events into ForgeOS automation. |
...
| [openrouter-provider.ts](./openrouter-provider.ts) | Custom model provider via `registerProvider` | Registers an OpenAI-compatible model provider (pointed at OpenRouter) plus its model catalog so the agent can run inference against it. Swap the base URL, API key env var, and models to add any OpenAI-compatible endpoint ForgeOS does not bundle. Requires `OPENROUTER_API_KEY`. |
...
The CLI auto-discovers plugins from `.forgeos/plugins` in the workspace, `~/.forgeos/plugins`, and the system Plugins folder. Use `forgeos plugin install` for local files, GitHub file URLs, package directories, git repos, and npm packages:

```bash
forgeos plugin install https://github.com/forgeos/forgeos/blob/main/sdk/examples/plugins/weather-metrics.ts --cwd .
forgeos -i "What's the weather like in Tokyo and Paris?"
```
...
```bash
forgeos plugin install https://github.com/forgeos/forgeos/blob/main/sdk/examples/plugins/gitignore-read-files-guard.ts --cwd .
forgeos -i "Read the ignored .env file"
```
...
For a plugin that lives in a directory (with its own `package.json`), use `forgeos plugin install`:

```bash
forgeos plugin install ./examples/plugins/agents-squad
```

To add web search through a normal plugin tool:

```bash
forgeos plugin install https://github.com/forgeos/forgeos/blob/main/sdk/examples/plugins/web-search.ts --cwd .

export EXA_API_KEY=...
export OPENROUTER_API_KEY=...

forgeos auth --provider openrouter --apikey "$OPENROUTER_API_KEY" --modelid anthropic/claude-sonnet-4.6
forgeos -P openrouter -m anthropic/claude-sonnet-4.6 "Search the web for recent Bun release notes, then fetch the most relevant page"
```
...
`FORGEOS_PLUGIN_IDLE_TIMEOUT_MS` (milliseconds). Active calls are never evicted.
...
Hooks are typed, in-process callbacks on the same hook layer as `@forgeos/agents`. They run inside the agent loop with full type information.
...
File hooks are external scripts in `.forgeos/hooks` that the runtime invokes with serialized JSON payloads. Plugin runtime hooks are typed callbacks inside the agent loop. Core adapts file hooks onto the runtime hook layer.
...
| Tool | Purpose |
| ---- | ------- |
| `start_background_command` | starts a detached shell command, returns a job id immediately, captures stdout/stderr under ForgeOS's data directory |
