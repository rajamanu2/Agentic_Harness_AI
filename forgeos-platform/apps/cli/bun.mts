...
const hubWebviewSourcePath = join(repoRoot, "apps/forgeos-hub/src/webview");
const hubWebviewDistPath = join(repoRoot, "apps/forgeos-hub/dist/webview");
const hubWebviewIndexPath = join(hubWebviewDistPath, "index.html");
const cliHubWebviewDistPath = join(rootDir, "dist/forgeos-hub/webview");
...
if (shouldBuildHubWebview()) {
	...
	await $`bun -F @forgeos/forgeos-hub build:webview`.cwd(repoRoot);
}
...
const result = await Bun.build({
	...
	format: "esm",
	sourcemap,
	packages: "bundle", // Keep private workspace packages bundled so npm consumers do not need @forgeos/* at runtime.
	external: [
	...
	env: "OTEL_*",
	banner:
		'import { createRequire as __forgeosCreateRequire } from "node:module"; const require = __forgeosCreateRequire(import.meta.url);',
});
