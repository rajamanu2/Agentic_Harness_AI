 ...
 *  1. "Nested transport" format written by the ForgeOS CLI (`forgeos mcp add`)
...
describe("McpSettingsSchema", () => {
	// -------------------------------------------------------------------------
	// Nested transport format (written by the ForgeOS CLI)
	// -------------------------------------------------------------------------
	...
	describe("nested transport format (CLI-authored)", () => {
		...
		it("preserves metadata field from CLI-authored nested format", () => {
			const metadata = { addedBy: "forgeos-cli", version: "1.2.3" }
			const input = {
				mcpServers: {
					myServer: {
