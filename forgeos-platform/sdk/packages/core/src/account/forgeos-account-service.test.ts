...
import { ForgeOSAccountService } from "./forgeos-account-service";

describe("ForgeOSAccountService", () => {
	it("fetches current user balance and sends auth header", async () => {
		const fetchImpl = vi.fn(async (input: unknown, init?: RequestInit) => {
			expect(String(input)).toBe(
				"https://api.forgeos.bot/api/v1/users/user-1/balance",
			);
		...
		const service = new ForgeOSAccountService({
	...
	it("resolves organization member id from /users/me when not provided", async () => {
		...
		expect(String(fetchImpl.mock.calls[1][0])).toBe(
			"https://api.forgeos.bot/api/v1/organizations/org-1/members/member-9/usages",
		);
	...
	it("fetches remote config with organizations list", async () => {
		...
		const fetchImpl = vi.fn(async (input: unknown) => {
			expect(String(input)).toBe(
				"https://api.forgeos.bot/api/v1/users/me/remote-config",
			);
			return new Response(
	...
	it("fetches available individual subscription plans with auth header", async () => {
		const plansPayload = [
			{
				id: "plan-1",
				displayName: "ForgeOSPass Monthly",
				interval: "Monthly",
		...
		const fetchImpl = vi.fn(async (input: unknown, init?: RequestInit) => {
			expect(String(input)).toBe(
				"https://api.forgeos.bot/api/v1/plans?type=individual",
			);
			expect(init?.headers).toMatchObject({
				Authorization: "Bearer workos:token-123",
			});
	...
	it("fetches the current user's subscription plan", async () => {
		...
		const fetchImpl = vi.fn(async (input: unknown, init?: RequestInit) => {
			expect(String(input)).toBe("https://api.forgeos.bot/api/v1/users/me/plan");
			expect(init?.headers).toMatchObject({
				Authorization: "Bearer workos:token-123",
			});
			return new Response(
	...
	it("surfaces plain text request failures without JSON parse errors", async () => {
		...
		await expect(service.fetchMe()).rejects.toThrow(
			"ForgeOS account request failed with status 401: Authentication failed",
		);
	});
	...
	it("surfaces invalid success payloads without JSON parse internals", async () => {
		...
		await expect(service.fetchMe()).rejects.toThrow(
			"ForgeOS account response was not valid JSON",
		);
	});
