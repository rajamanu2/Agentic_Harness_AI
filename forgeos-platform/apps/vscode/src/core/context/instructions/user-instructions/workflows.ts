...
function mergeToggleStateAfterScan(
	scanned: ForgeOSRulesToggles,
	preScan: ForgeOSRulesToggles,
	current: ForgeOSRulesToggles,
): ForgeOSRulesToggles {
	const merged: ForgeOSRulesToggles = {}
...
async function doRefreshWorkflowToggles(
	...
	const globalForgeOSWorkflowsFilePath = await ensureWorkflowsDirectoryExists()
	const scannedGlobalToggles = await synchronizeRuleToggles(globalForgeOSWorkflowsFilePath, globalWorkflowToggles)
