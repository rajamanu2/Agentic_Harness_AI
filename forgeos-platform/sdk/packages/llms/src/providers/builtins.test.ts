...
function findForgeOSSpec() {
	const spec = BUILTIN_SPECS.find((s) => s.id === "forgeos");
	if (!spec) {
		throw new Error("forgeos builtin spec not found");
	}
	return spec;
}

describe("forgeos builtin spec defaults.baseUrl", () => {
	const originalEnvironment = process.env[FORGEOS_ENVIRONMENT_ENV];

	beforeEach(() => {
		delete process.env[FORGEOS_ENVIRONMENT_ENV];
	});
	...
	afterEach(() => {
		if (originalEnvironment === undefined) {
			...
			process.env[FORGEOS_ENVIRONMENT_ENV] = originalEnvironment;
		}
	});

	it("re-resolves baseUrl when FORGEOS_ENVIRONMENT changes between reads", () => {
		const spec = findForgeOSSpec();

		expect(spec.defaults?.baseUrl).toBe(
			`${FORGEOS_ENVIRONMENTS.production.apiBaseUrl}/api/v1`,
		);

		process.env[FORGEOS_ENVIRONMENT_ENV] = "staging";
		expect(spec.defaults?.baseUrl).toBe(
			`${FORGEOS_ENVIRONMENTS.staging.apiBaseUrl}/api/v1`,
		);

		process.env[FORGEOS_ENVIRONMENT_ENV] = "local";
		expect(spec.defaults?.baseUrl).toBe(
			`${FORGEOS_ENVIRONMENTS.local.apiBaseUrl}/api/v1`,
		);
...
describe("forgeos builtin models", () => {
	it("exposes its canonical default model ID", () => {
		expect(findForgeOSSpec().defaultModelId).toBe(FORGEOS_DEFAULT_MODEL_ID);
	});

	it("prefers Vercel-style Z.ai model ids over equivalent OpenRouter ids", async () => {
		const models = await getModelsForProvider("forgeos");
...
describe("forgeos-pass builtin spec", () => {
	it("registers a distinct ForgeOS-compatible provider with a custom model list", async () => {
		const models = await getModelsForProvider("forgeos-pass");
		const provider = await getProvider("forgeos-pass");

		expect(provider).toMatchObject({
			id: "forgeos-pass",
			name: "ForgeOSPass",
			baseUrl: `${FORGEOS_ENVIRONMENTS.production.apiBaseUrl}/api/v1`,
...
describe("built-in provider metadata", () => {
	it("declares OpenRouter image transport for ForgeOS-compatible gateways", async () => {
		await expect(getProvider("forgeos")).resolves.toMatchObject({
			metadata: {
				imageTransport: "openrouter",
				responseEnvelope: "success-data",
			},
		});
		await expect(getProvider("forgeos-pass")).resolves.toMatchObject({
			metadata: {
				imageTransport: "openrouter",
				responseEnvelope: "success-data",
			},
	...
	it("uses generated specs directly when no runtime override is required", () => {
		// moonshot is intentionally absent: it carries a ForgeOS-specific
