...
Global settings, secrets and workspace state are stored in **file-backed JSON stores** under `~/.forgeos/data/`. This is the shared storage layer used by VSCode, CLI, and JetBrains.

## Key Abstractions

### `StorageContext` (src/shared/storage/storage-context.ts)
The entry point. Created via `createStorageContext()` and passed to `StateManager.initialize()`. Contains three `ForgeOSFileStorage` instances:
- `globalState` → `~/.forgeos/data/globalState.json`
- `secrets` → `~/.forgeos/data/secrets.json` (mode 0o600)
- `workspaceState` → `~/.forgeos/data/workspaces/<hash>/workspaceState.json`

### `ForgeOSFileStorage` (src/shared/storage/ForgeOSFileStorage.ts)
...
Remember that your data may be read by a different client than the one that wrote it. For example, a value written by ForgeOS in JetBrains may be read by ForgeOS CLI.
