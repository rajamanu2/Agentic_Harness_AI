...
import { ForgeOSLogo } from "@/components/forgeos-logo";
import {
	AlertDialog,
	AlertDialogAction,
	AlertDialogCancel,
...
export function AgentSidebar({
	...
	const accountName = username?.trim() || "ForgeOS";
			...
			setHubStatus({
				...
				error:
					error instanceof Error
						? error.message
						: "Unable to read ForgeOS Hub status.",
				url: null,
			});
		}
	}, []);
					...
					<div className="flex min-w-0 items-center gap-0.5">
						...
						>
							...
							<HoverCardContent
								...
								side="bottom"
							>
								<p className="text-sm font-medium">ForgeOS</p>
								...
								<div className="mt-3 border-border border-t pt-3">
									<div className="flex items-center gap-2 text-xs">
										...
										<span className="font-medium">
											ForgeOS Hub @{hubPort(hubStatus?.url ?? null) ?? "unknown"}
										</span>
									</div>
									{hubStatus && !hubStatus.connected && (
										<p className="mt-1 text-[11px] text-destructive">
											{hubStatus.error ?? "ForgeOS Hub is not connected."}
										</p>
									)}
								</div>
							</HoverCardContent>
