...
const hoisted = vi.hoisted(() => ({
	...
	startForgeOSDeviceAuth: vi.fn(),
	completeForgeOSDeviceAuth: vi.fn(),
...
vi.mock("@forgeos/core", () => ({
	...
	startForgeOSDeviceAuth: hoisted.startForgeOSDeviceAuth,
	completeForgeOSDeviceAuth: hoisted.completeForgeOSDeviceAuth,
	saveLocalProviderOAuthCredentials: hoisted.saveLocalProviderOAuthCredentials,
	ProviderSettingsManager: class {},
}));

vi.mock("@forgeos/shared", () => ({
	getForgeOSEnvironmentConfig: () => ({ apiBaseUrl: "https://api.example" }),
}));
...
describe("onboarding auth telemetry forwarding", () => {
	beforeEach(() => {
		...
		hoisted.startForgeOSDeviceAuth.mockReset();
		hoisted.completeForgeOSDeviceAuth.mockReset();
	...
	it("forwards telemetry into completeForgeOSDeviceAuth for the device-code flow", async () => {
		hoisted.startForgeOSDeviceAuth.mockResolvedValueOnce({
			deviceCode: "dc",
			userCode: "uc",
		...
		hoisted.completeForgeOSDeviceAuth.mockResolvedValueOnce({
			access: "a",
			refresh: "r",
			expires: 0,
			accountId: "acct-1",
		...
		expect(hoisted.completeForgeOSDeviceAuth).toHaveBeenCalledTimes(1);
		const [opts] = hoisted.completeForgeOSDeviceAuth.mock.calls[0];
		expect(opts.telemetry).toBe(fakeTelemetry);
		// We do NOT forward telemetry to startForgeOSDeviceAuth — auth_started is
		// emitted by completeForgeOSDeviceAuth, so passing telemetry to the start
		// helper would double-emit the event.
		expect(hoisted.startForgeOSDeviceAuth).toHaveBeenCalledWith();
