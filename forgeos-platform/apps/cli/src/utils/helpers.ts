...
import { type HookEventPayload, parseHookEventPayload } from "@forgeos/shared";
import { ensureHookLogDir } from "@forgeos/shared/storage";
...
export async function appendHookAudit(event: HookEventPayload): Promise<void> {
	...
	const envPath = process.env.FORGEOS_HOOKS_LOG_PATH?.trim() || undefined;
...
export function resolveSandboxDataDir(
	...
	const envDir = process.env.FORGEOS_SANDBOX_DATA_DIR?.trim();
	const baseDir =
		explicitDir?.trim() || envDir || join(tmpdir(), "forgeos-sandbox");
...
export function configureSandboxEnvironment(options: {
	...
	process.env.FORGEOS_SANDBOX = "1";
	process.env.FORGEOS_SANDBOX_DATA_DIR = dataDir;
	process.env.FORGEOS_DATA_DIR = dataDir;
	process.env.FORGEOS_DB_DATA_DIR = join(dataDir, "db");
	process.env.FORGEOS_SESSION_DATA_DIR = join(dataDir, "sessions");
	process.env.FORGEOS_TEAM_DATA_DIR = join(dataDir, "teams");
	process.env.FORGEOS_PROVIDER_SETTINGS_PATH = join(
		dataDir,
		"settings",
		"providers.json",
	);
	process.env.FORGEOS_HOOKS_LOG_PATH = join(dataDir, "logs", "hooks.jsonl");
	return dataDir;
}
