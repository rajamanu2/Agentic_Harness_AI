# Copilot Instructions for ForgeOS

This is a VS Code extension. Read `.forgeosrules/general.md` for tribal knowledge and nuanced patterns.
...
1. **Define** in `proto/forgeos/*.proto`. Naming: `PascalCaseService`, `camelCase` RPCs, `PascalCase` Messages. Use `common.proto` shared types for simple data.
...
- Adding enums (e.g. `ForgeOSSay`) → also update `src/shared/proto-conversions/forgeos-message.ts`.
...
1. `proto/forgeos/models.proto` — add to `ApiProvider` enum.
...
1. Add enum to `ForgeOSDefaultTool` in `src/shared/tools.ts`.
...
6. If tool has UI: add `ForgeOSSay` enum in proto → `ExtensionMessage.ts` → `forgeos-message.ts` → `ChatRow.tsx`.
