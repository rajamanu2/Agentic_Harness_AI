...
): Pick<ForgeOSCoreStartInput["config"], "reasoningEffort" | "thinking"> {
...
export function resolveLaunchContext(
	...
	const providerId =
		...
		process.env.FORGEOS_PROVIDER?.trim() ??
		"";
	const modelId =
		override?.model ??
		override?.modelId ??
		...
		process.env.FORGEOS_MODEL?.trim() ??
		"";
	const root =
	...
	if (!providerId || !modelId) {
		throw new Error(
			"No provider/model available. Start a session in another ForgeOS client first, or set FORGEOS_PROVIDER and FORGEOS_MODEL.",
		);
	}
	return {
		workspaceRoot: root,
...
function buildSessionStartInput(
	...
	return {
		...
		config: {
			...
			teamName: options?.teamName ?? "forgeos-hub",
...
async function loadHistoryFor(
	...
	if (!ctx.forgeos) return [];
	try {
		return (await ctx.forgeos.readMessages(sessionId)) as unknown[];
	} catch (error) {
...
export async function createSession(
	...
	if (!ctx.forgeos) throw new Error("Hub is not connected.");
	const context = resolveLaunchContext(ctx, config);
	const mode = config?.mode === "plan" ? "plan" : "act";
	const result = await ctx.forgeos.start(
...
export async function deleteSession(
	...
	if (!ctx.forgeos) throw new Error("Hub is not connected.");
	const deleted = await ctx.forgeos.delete(sessionId);
	if (!deleted) {
		ctx.send(peer, {
			type: "status",
...
export async function abortPeerTurn(
	...
	if (!ctx.forgeos || !peer.selectedSessionId) return;
	rejectPendingApprovalsForSession(
		ctx,
		peer.selectedSessionId,
		"Turn aborted before approval was resolved.",
	);
	await ctx.forgeos.abort(peer.selectedSessionId);
...
export async function forkPeerSession(
	...
	try {
		const rawMessages = (await ctx.forgeos.readMessages(
			forkedFromSessionId,
		...
		const sourceSession = await ctx.forgeos.get(forkedFromSessionId);
		if (!sourceSession) {
			ctx.send(peer, {
				type: "fork_error",
		...
		peer.selectedSessionId = result.sessionId;
		const newSession = await ctx.forgeos.get(result.sessionId);
...
export async function restorePeerSession(
	...
	const sourceSession = await ctx.forgeos.get(sourceSessionId);
	if (!sourceSession) {
		ctx.send(peer, {
			type: "error",
	...
	const result = await ctx.forgeos.restore({
	...
	peer.selectedSessionId = result.sessionId;
	const restoredSession = await ctx.forgeos.get(result.sessionId);
...
export async function initializePeer(
	...
	await syncHubClientsAndSessions();
	ctx.send(peer, { type: "status", text: "ForgeOS Hub is ready." });
