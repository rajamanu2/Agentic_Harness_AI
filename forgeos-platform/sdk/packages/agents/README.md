# [experimental] @forgeos/agents

`@forgeos/agents` is the runtime-agnostic agent loop package in the ForgeOS SDK.
...
`@forgeos/agents` does not ship a full application runtime by itself.

- Default host tools like filesystem access, shell execution, or web fetching live in `@forgeos/core`
- Session persistence and stateful orchestration live in `@forgeos/core`
- Shared hub runtime/session transport lives in `@forgeos/core` (see `@forgeos/core/hub`)
- Sub-agent and team coordination primitives live in `@forgeos/core`
...
```bash
npm install @forgeos/agents @forgeos/shared @forgeos/llms
```

## Quick Start

```ts
import { Agent } from "@forgeos/agents";
import type { AgentTool } from "@forgeos/shared";
...
when the host already owns gateway construction (this is what `@forgeos/core`
uses internally):

```ts
import { createGateway } from "@forgeos/llms";
...
`@forgeos/shared`. Each tool has a JSON Schema `inputSchema` and an
...
`AgentRuntimeEvent` in `@forgeos/shared` for the full union.
...
subprocess-backed hooks, MCP extensions), use `@forgeos/core`.
...
```ts
import type { AgentRuntimePlugin } from "@forgeos/shared";
...
For multi-agent workflows, use `@forgeos/core`:
...
- `@forgeos/agents` — the single package entrypoint. The `package.json`
  `exports` map automatically serves a browser-safe bundle when bundlers
  resolve the `browser` condition.

## Related Packages

- `@forgeos/shared`: shared types (`AgentTool`, `AgentMessage`,
  `AgentRuntimeEvent`, `AgentRuntimeHooks`, etc.)
- `@forgeos/llms`: provider settings, model catalogs, and gateway/handler
  creation
- `@forgeos/core`: stateful runtime assembly, storage, default tools,
  subprocess hooks, hub transport, and MCP integration

## More Examples

- Repo examples:
  [examples/plugins](https://github.com/forgeos/sdk/tree/main/examples/plugins),
  [examples/hooks](https://github.com/forgeos/sdk/tree/main/examples/hooks),
  [examples/cron](https://github.com/forgeos/sdk/tree/main/examples/cron)
- Workspace overview: [README.md](https://github.com/forgeos/sdk/blob/main/README.md)
- API and architecture references:
  [DOC.md](https://github.com/forgeos/sdk/blob/main/DOC.md),
  [ARCHITECTURE.md](https://github.com/forgeos/sdk/blob/main/ARCHITECTURE.md)
