...
describe("plugin install command", () => {
	...
	let originalForgeOSDataDir: string | undefined;
	...
	async function createOfficialPluginsRepo(
		...
		runGitCommand(repo, ["config", "user.name", "ForgeOS Test"]);
	...
	afterEach(() => {
		...
		if (originalForgeOSDataDir === undefined) {
			delete process.env.FORGEOS_DATA_DIR;
		} else {
			process.env.FORGEOS_DATA_DIR = originalForgeOSDataDir;
		}
	...
	it("parses GitHub plugin file URLs as remote sources", () => {
		expect(
			parsePluginSource(
				"https://github.com/forgeos/forgeos/blob/main/sdk/examples/plugins/weather-metrics.ts",
			),
		).toEqual({
			type: "remote",
			url: "https://raw.githubusercontent.com/forgeos/forgeos/main/sdk/examples/plugins/weather-metrics.ts",
	...
	it("parses raw plugin file URLs as remote sources", () => {
		expect(
			parsePluginSource(
				"https://raw.githubusercontent.com/forgeos/forgeos/main/sdk/examples/plugins/weather-metrics.ts",
			),
		).toEqual({
			type: "remote",
			url: "https://raw.githubusercontent.com/forgeos/forgeos/main/sdk/examples/plugins/weather-metrics.ts",
	...
	it("installs a local plugin file into the global plugin root", async () => {
		...
		expect(result.installPath).toContain(join(home, ".forgeos", "plugins"));
	...
	it("installs a remote plugin file into the workspace plugin root", async () => {
		...
		expect(result.installPath).toContain(
			join(workspace, ".forgeos", "plugins", "_installed", "remote"),
		);
		expect(result.entryPaths).toHaveLength(1);
		expect(existsSync(result.entryPaths[0] ?? "")).toBe(true);
		...
		expect(
			discoverPluginModulePaths(join(workspace, ".forgeos", "plugins")),
	...
	it("installs an official plugin slug from the configured collection repo", async () => {
		...
		expect(result.installPath).toContain(
			join(workspace, ".forgeos", "plugins", "_installed", "official"),
		);
		expect(result.entryPaths).toHaveLength(1);
		expect(readFileSync(result.entryPaths[0] ?? "", "utf8")).toContain(
			"official-web-search",
	...
	it("reports a clear error when an official plugin slug is missing", async () => {
		...
		await expect(
			...
			/Official ForgeOS plugin "missing-plugin" was not found at plugins\/missing-plugin/,
		);
	});
	...
	it("keeps explicit relative paths as local plugin installs", async () => {
		...
		expect(result.installPath).toContain(
			join(workspace, ".forgeos", "plugins", "_installed", "local"),
		);
		const wrapperManifest = JSON.parse(
			readFileSync(join(result.installPath, "package.json"), "utf8"),
		) as { name?: string };
	...
	it("installs into cwd plugin root when cwd is provided", async () => {
		...
		const wrapperManifest = JSON.parse(
			readFileSync(join(result.installPath, "package.json"), "utf8"),
		) as { name?: string; forgeos?: { plugins?: Array<{ paths?: string[] }> } };
		expect(wrapperManifest.name).toBe("plugin-package");
		expect(wrapperManifest.forgeos?.plugins?.[0]?.paths).toHaveLength(1);
		expect(wrapperManifest.forgeos?.plugins?.[0]?.paths?.[0]).toContain(
			"package/index.ts",
		);
		...
		const discovered = discoverPluginModulePaths(
			join(workspace, ".forgeos", "plugins"),
		);
		expect(discovered).toEqual(result.entryPaths);
	...
	it("omits and removes host SDK packages from npm-sourced installs", async () => {
		...
		writeFileSync(
			...
			[
				...
				'mkdir -p "$prefix/node_modules/@forgeos/core"',
				'printf \'%s\\n\' \'{"name":"published-plugin","type":"module","forgeos":{"plugins":["index.ts"]}}\' > "$prefix/node_modules/published-plugin/package.json"',
				"printf '%s\\n' \"export default { name: 'published-plugin', manifest: { capabilities: ['tools'] } };\" > \"$prefix/node_modules/published-plugin/index.ts\"",
				'printf \'%s\\n\' \'{"name":"@forgeos/core"}\' > "$prefix/node_modules/@forgeos/core/package.json"',
				"exit 0",
			].join("\n"),
			{ encoding: "utf8", mode: 0o755 },
		);
		...
		expect(
			existsSync(
				join(result.installPath, "package", "node_modules", "@forgeos", "core"),
			),
		).toBe(false);
		expect(existsSync(result.entryPaths[0] ?? "")).toBe(true);
	});
	...
	it("prints JSON output for command callers", async () => {
		...
		try {
			...
			expect(parsed.installPath).toContain(join(home, ".forgeos", "plugins"));
	...
	it("does not run MCP OAuth follow-up for JSON plugin installs", async () => {
		process.env.FORGEOS_MCP_SETTINGS_PATH = join(root, "mcp-settings.json");
...
`,
		...
		const originalSettingsPath = process.env.FORGEOS_MCP_SETTINGS_PATH;
		process.env.FORGEOS_MCP_SETTINGS_PATH = join(
			blockedDirectory,
		...
		try {
			...
			if (originalSettingsPath === undefined) {
				...
				process.env.FORGEOS_MCP_SETTINGS_PATH = originalSettingsPath;
			}
		}
	});
