...
let tempForgeOSDir: string;
let previousForgeOSDir: string | undefined;

beforeEach(async () => {
	tempForgeOSDir = await mkdtemp(join(tmpdir(), "desktop-marketplace-"));
	previousForgeOSDir = process.env.FORGEOS_DIR;
	process.env.FORGEOS_DIR = tempForgeOSDir;
});

afterEach(async () => {
	if (previousForgeOSDir === undefined) {
		delete process.env.FORGEOS_DIR;
	} else {
		process.env.FORGEOS_DIR = previousForgeOSDir;
	}
	await rm(tempForgeOSDir, { recursive: true, force: true });
});
	...
	it("still short-circuits when the directory contains a plugin module", async () => {
		...
		await writeFile(
			...
			JSON.stringify({
				name: "goal",
				private: true,
				forgeos: { plugins: [{ paths: ["./package/index.ts"] }] },
			}),
		);
		await writeFile(
