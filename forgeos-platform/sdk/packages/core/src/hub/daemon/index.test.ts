...
const {
	...
	resolveForgeOSDataDir: vi.fn(() => "/tmp/forgeos-data"),
	...
	FORGEOS_RUN_AS_HUB_DAEMON_ENV: "FORGEOS_RUN_AS_HUB_DAEMON",
}));

const originalRunAsHubDaemon = process.env[FORGEOS_RUN_AS_HUB_DAEMON_ENV];
const originalConnectorCliLaunch = process.env.FORGEOS_CONNECTOR_CLI_LAUNCH;
...
vi.mock("@forgeos/shared", () => ({
	FORGEOS_RUN_AS_HUB_DAEMON_ENV,
	FORGEOS_HUB_PORT: 25463,
	FORGEOS_HUB_DEV_PORT: 25466,
	...
	isHubDaemonProcess: (env: NodeJS.ProcessEnv = process.env) =>
		env[FORGEOS_RUN_AS_HUB_DAEMON_ENV] === "1",
	resolveForgeOSBuildEnv: () => "production",
	withResolvedForgeOSBuildEnv: (env: NodeJS.ProcessEnv) => env,
}));
...
describe("ensureDetachedHubServer", () => {
	...
	afterEach(() => {
		...
		if (originalRunAsHubDaemon === undefined) {
			...
			process.env[FORGEOS_RUN_AS_HUB_DAEMON_ENV] = originalRunAsHubDaemon;
		}
		if (originalConnectorCliLaunch === undefined) {
			delete process.env.FORGEOS_CONNECTOR_CLI_LAUNCH;
		} else {
			process.env.FORGEOS_CONNECTOR_CLI_LAUNCH = originalConnectorCliLaunch;
		}
	});

	it("does not use port 0 for default production startup", async () => {
		process.env.FORGEOS_CONNECTOR_CLI_LAUNCH = JSON.stringify({
			launcher: "bun",
		...
		expect(spawnOptions?.env?.[FORGEOS_RUN_AS_HUB_DAEMON_ENV]).toBe("1");
		expect(spawnOptions?.env?.FORGEOS_CONNECTOR_CLI_LAUNCH).toBe(
			process.env.FORGEOS_CONNECTOR_CLI_LAUNCH,
		);
	});
	...
	it("retries a transient ETXTBSY spawn failure while starting the detached daemon", async () => {
		...
		try {
			const textFileBusy = Object.assign(
				new Error(
					"ETXTBSY: text file is busy, posix_spawn '/usr/local/bin/forgeos'",
				),
				{ code: "ETXTBSY" },
			);
			spawn
	...
	it("throws a targeted error when an incompatible hub cannot be retired", async () => {
		...
		try {
			...
			const pending = expect(
				...
				"An incompatible ForgeOS Hub is already running at ws://127.0.0.1:25463/hub and could not be retired automatically.",
			);
			await vi.runAllTimersAsync();
	...
	it("throws when a compatible expected hub has no discovery record", async () => {
		...
		await expect(ensureDetachedHubServer("/workspace")).rejects.toThrow(
			"A compatible ForgeOS Hub is already running at ws://127.0.0.1:25463/hub, but its discovery record is missing or unreadable and no usable auth token is available.",
		);
		expect(spawn).not.toHaveBeenCalled();
	});
