...
test.describe("forgeos --json persisted messages contract - authenticated @live", () => {
	const sessionDataDir = mkdtempSync(
		join(tmpdir(), "forgeos-headless-messages-contract-"),
	);
	test.use({
		...
		env: forgeosEnv("default", {
			...
			FORGEOS_SESSION_DATA_DIR: sessionDataDir,
		}),
	});
