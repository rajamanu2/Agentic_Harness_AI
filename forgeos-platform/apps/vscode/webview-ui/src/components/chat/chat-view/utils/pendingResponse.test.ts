...
describe("withPendingUserMessage", () => {
	it("provides an immediate task row before the backend transcript exists", () => {
		const task: ForgeOSMessage = { ts: 10, type: "say", say: "task", text: "new task", partial: false }
	...
	it("replaces the optimistic task with its backend confirmation without duplication", () => {
		const optimistic: ForgeOSMessage = { ts: 10, type: "say", say: "task", text: "new task", partial: false }
		const confirmed: ForgeOSMessage = { ...optimistic, ts: 11 }
	...
	it("keeps a follow-up bubble until the matching backend message arrives", () => {
		const task: ForgeOSMessage = { ts: 1, type: "say", say: "task", text: "task" }
		const followup: ForgeOSMessage = { ts: 3, type: "say", say: "user_feedback", text: "more" }
