...
import { PLUGIN_FILE_EXTENSIONS } from "@forgeos/shared";
...
const HOST_PROVIDED_SDK_SPECIFIERS = [
	...
	"@forgeos/core",
	"@forgeos/core/hub",
	"@forgeos/core/hub/daemon-entry",
	"@forgeos/core/telemetry",
	"@forgeos/llms",
	"@forgeos/llms/browser",
	"@forgeos/shared",
	"@forgeos/shared/automation",
	"@forgeos/shared/browser",
	"@forgeos/shared/storage",
	"@forgeos/shared/db",
	"@forgeos/shared/types",
];
...
function collectWorkspaceAliases(root: string): Record<string, string> {
	...
	const candidates: Record<string, string> = {
		"@forgeos/sdk": resolve(root, "packages/sdk/src/index.ts"),
		"@forgeos/agents": resolve(root, "packages/agents/src/index.ts"),
		"@forgeos/core": resolve(root, "packages/core/src/index.ts"),
		"@forgeos/llms": resolve(root, "packages/llms/src/index.ts"),
		"@forgeos/shared": resolve(root, "packages/shared/src/index.ts"),
		"@forgeos/shared/storage": resolve(
			root,
			"packages/shared/src/storage/index.ts",
		),
		"@forgeos/shared/db": resolve(root, "packages/shared/src/db/index.ts"),
	};
...
function isForgeOSSdkSpecifier(specifier: string): boolean {
	return getPackageName(specifier).startsWith("@forgeos/");
}
...
function getHostPackageSearchRoots(): string[] {
	...
	const wrapperPath = process.env.FORGEOS_WRAPPER_PATH?.trim();
	if (wrapperPath) {
...
function isPackageBasedPlugin(pluginFilePath: string): boolean {
	// Walk up from the plugin file looking for a package.json with a `forgeos`
	// manifest. Stop at the first package.json we encounter; if it doesn't
	// declare `forgeos` we've left the plugin boundary (e.g. hit the workspace
	// root).  Also cap the traversal so we never wander far from the plugin
	// search root (.forgeos/plugins).
	const MAX_DEPTH = 4;
	let current = dirname(pluginFilePath);
	for (let depth = 0; depth < MAX_DEPTH; depth++) {
		...
		if (existsSync(packageJsonPath)) {
			try {
				...
				return pkg != null && typeof pkg === "object" && "forgeos" in pkg;
			} catch {
				return false;
			}
		}
	...
	for (const specifier of collectStaticModuleSpecifiers(source)) {
		...
		if (isBareSpecifier(specifier)) {
			...
			if (
				...
				(isForgeOSSdkSpecifier(specifier) &&
...
function collectPluginImportAliases(
	...
	for (const specifier of staticSpecifiers) {
		if (
			...
			(isForgeOSSdkSpecifier(specifier) || preferHostRuntimeDependencies)
		) {
...
export async function importPluginModule(
	...
	const jiti = createJiti(pluginPath, {
		...
		// every nested import in the plugin, sees `import "@forgeos/core"` with no
