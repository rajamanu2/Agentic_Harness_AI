...
import { buildForgeOSSystemPrompt, formatUserCommandBlock } from "@forgeos/shared";
import {
...
type SessionConnectionUpdate = Parameters<
	ForgeOSCore["updateSessionConnection"]
>[1];
...
function getSessionManager(ctx: SidecarContext): ForgeOSCore {
...
async function handleStart(
	...
	const startResult = await manager.start({
		...splitCoreSessionConfig(coreConfig as unknown as ForgeOSCoreStartConfig),
	...
	const restarted = await manager.start({
		...splitCoreSessionConfig(
			buildCoreSessionConfig({
				...
				sessionId,
				systemPrompt,
			}) as unknown as ForgeOSCoreStartConfig,
		),
		source: SessionSource.DESKTOP,
		interactive: true,
...
async function handleSend(
	...
	try {
		...
		let result: Awaited<ReturnType<ForgeOSCore["send"]>>;
		try {
