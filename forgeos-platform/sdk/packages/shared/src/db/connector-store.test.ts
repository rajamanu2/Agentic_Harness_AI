...
describe("SqliteConnectorStore", () => {
	...
	const previousSettingsPath = process.env.FORGEOS_CONNECTOR_SETTINGS_PATH;
	const previousDbPath = process.env.FORGEOS_CONNECTORS_DB_PATH;
	const tempRoots: string[] = [];

	afterEach(() => {
		process.env.FORGEOS_DATA_DIR = previousDataDir;
		process.env.FORGEOS_CONNECTOR_SETTINGS_PATH = previousSettingsPath;
		process.env.FORGEOS_CONNECTORS_DB_PATH = previousDbPath;
		if (previousDataDir === undefined) delete process.env.FORGEOS_DATA_DIR;
		if (previousSettingsPath === undefined)
			delete process.env.FORGEOS_CONNECTOR_SETTINGS_PATH;
		if (previousDbPath === undefined)
			delete process.env.FORGEOS_CONNECTORS_DB_PATH;
	...
	function useTempDataDir(): string {
		...
		tempRoots.push(root);
		process.env.FORGEOS_DATA_DIR = root;
		delete process.env.FORGEOS_CONNECTOR_SETTINGS_PATH;
		delete process.env.FORGEOS_CONNECTORS_DB_PATH;
		return root;
	}
	...
	it("refreshes stored launch args while preserving autostart state", () => {
		useTempDataDir();
		withStore((store) => {
			store.recordConnected("telegram", "forgeos_bot", ["-k", "123:token"]);
			store.setEnabled("telegram", false);
			store.upsertConfig({
				channel: "telegram",
				values: { "-k": "456:rotated" },
			});
			expect(store.getConnection("telegram", "forgeos_bot")?.connectArgs).toEqual(
				["-k", "123:token"],
			);
			store.upsertConfig({
				channel: "telegram",
			...
			const connection = store.getConnection("telegram", "forgeos_bot");
	...
	it("round-trips empty connect args for env-only starts", () => {
		useTempDataDir();
		withStore((store) => {
			store.recordConnected("telegram", "forgeos_bot", []);

			expect(store.getConnection("telegram", "forgeos_bot")?.connectArgs).toEqual(
				[],
			);
			expect(store.getConnection("telegram", "forgeos_bot")?.enabled).toBe(true);
		});
	});
	...
	it("deletes dashboard config without clearing CLI autostart state", () => {
		...
		withStore((store) => {
			...
			expect(store.getConnection("telegram", "forgeos_bot")).toEqual(
				expect.objectContaining({
					channel: "telegram",
