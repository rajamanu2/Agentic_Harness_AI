...
describe("SdkTaskStartCoordinator", () => {
	...
	it("emits a ForgeOS auth error instead of starting when the forgeos provider has no token", async () => {
		const { coordinator, options } = makeCoordinator({ config: { providerId: "forgeos", modelId: "model", apiKey: "" } })

		const sessionId = await coordinator.initTask("needs auth")

		expect(sessionId).toBeUndefined()
		expect(options.emitForgeOSAuthError).toHaveBeenCalledWith("needs auth")
		expect(options.captureProviderApiError).not.toHaveBeenCalled()
		expect(options.sessions.startNewSession).not.toHaveBeenCalled()
	})

	it("emits a ForgeOS auth error instead of starting when ForgeOSPass has no token", async () => {
		const { coordinator, options } = makeCoordinator({ config: { providerId: "forgeos-pass", modelId: "model", apiKey: "" } })

		const sessionId = await coordinator.initTask("needs forgeospass auth")

		expect(sessionId).toBeUndefined()
		expect(options.emitForgeOSAuthError).toHaveBeenCalledWith("needs forgeospass auth")
		expect(options.captureProviderApiError).not.toHaveBeenCalled()
		expect(options.sessions.startNewSession).not.toHaveBeenCalled()
	})
	...
	it("emits a plain chat error when session start fails (e.g. provider misconfigured)", async () => {
		...
		expect(sessionId).toBeUndefined()
		expect(options.emitForgeOSAuthError).not.toHaveBeenCalled()
	...
	it("emits ForgeOS auth errors when reinitialization fails due auth", async () => {
		const { coordinator, options } = makeCoordinator()
		options.sessionConfigBuilder.build.mockRejectedValue(new Error("missing api key"))
		options.isForgeOSManagedProviderActive.mockReturnValue(true)

		await coordinator.reinitExistingTaskFromId("task-1")

		expect(options.emitForgeOSAuthError).toHaveBeenCalledWith()
...
function makeCoordinator(input: Partial<MakeCoordinatorInput> = {}) {
	...
	const options = {
		...
		isForgeOSManagedProviderActive: vi.fn(() => false),
		emitForgeOSAuthError: vi.fn(),
		...
		isForgeOSManagedProviderActive: ReturnType<typeof vi.fn>
		emitForgeOSAuthError: ReturnType<typeof vi.fn>
