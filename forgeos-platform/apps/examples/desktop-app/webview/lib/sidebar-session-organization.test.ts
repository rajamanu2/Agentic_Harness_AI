...
describe("sidebar session organization", () => {
	...
	it("uses the repository directory instead of the full workspace path", () => {
		expect(workspaceDisplayName("/Users/saoud/code/forgeos/")).toBe("forgeos");
		expect(workspaceDisplayName("C:\\Users\\saoud\\code\\forgeos\\")).toBe(
			"forgeos",
		);
	});

	it("labels chat workspace groups as Chat", () => {
		const path = "/home/host/.forgeos/data/workspaces/chat";
