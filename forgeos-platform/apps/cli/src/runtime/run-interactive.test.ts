...
describe("resumeInteractiveSession", () => {
	const originalAgentResume = process.env.FORGEOS_HOOK_AGENT_RESUME;

	afterEach(() => {
		if (originalAgentResume === undefined) {
			delete process.env.FORGEOS_HOOK_AGENT_RESUME;
		} else {
			process.env.FORGEOS_HOOK_AGENT_RESUME = originalAgentResume;
		}
	});
	...
	it("starts the selected session directly without ensuring an empty session first", async () => {
		...
		const resumeSession = vi.fn(async () => {
			expect(process.env.FORGEOS_HOOK_AGENT_RESUME).toBe("1");
			return messages;
		});
	...
	it("restores the hook state when the selected session cannot resume", async () => {
		...
		expect(process.env.FORGEOS_HOOK_AGENT_RESUME).toBeUndefined();
	});
});
