...
function isForgeOSAccountOAuthProvider(providerId: string): boolean {
...
export function runOAuthAuthFlow(input: {
	...
	loginLocalProvider(
		...
		.then((credentials) => {
			...
			if (isForgeOSAccountOAuthProvider(input.providerId)) {
...
export function runDeviceCodeAuthFlow(input: {
	...
	const apiBaseUrl =
		existing?.baseUrl?.trim() || getForgeOSEnvironmentConfig().apiBaseUrl;

	// `startForgeOSDeviceAuth` only requests the user/device code pair; the
	// `auth_started` telemetry event is emitted by `completeForgeOSDeviceAuth`
	// (which owns the actual login lifecycle), so we intentionally do NOT
	// pass telemetry into `startForgeOSDeviceAuth` here.
	startForgeOSDeviceAuth()
		.then((result) => {
			if (input.isAborted()) return;
			const verifyUrl =
