...
describe("CLI distribution package shape", () => {
	it("rejects direct source package publishing by default", () => {
		...
		expect(shouldAllowDirectPublish({ FORGEOS_ALLOW_DIRECT_PUBLISH: "1" })).toBe(
			true,
		);
	...
	it("packs the generated npm wrapper package", async () => {
		const packageDir = await mkdtemp(join(tmpdir(), "forgeos-cli-pack-"));
		try {
			...
			await writeFile(
				join(packageDir, "bin", "forgeos"),
				[
					"#!/usr/bin/env node",
					'console.log("forgeos wrapper smoke test");',
					"",
				].join("\n"),
			);
			await chmod(join(packageDir, "bin", "forgeos"), 0o755);
			await writeFile(
