...
import type { ForgeOSAccountUser } from "@forgeos/core";
...
function makeUser(overrides: Partial<ForgeOSAccountUser> = {}): ForgeOSAccountUser {
	return {
		id: "user-1",
...
describe("account context", () => {
	...
	it("classifies signed-out errors separately from transient failures", () => {
		expect(
			isSignedOutAccountError(new Error("No ForgeOS account auth token found")),
		).toBe(true);
		expect(
			isSignedOutAccountError(
				new Error(
	...
	it("fetches the signed-in user on mount and caches the identity", async () => {
		...
		await vi.waitFor(() => {
			...
			expect(probeText("account-org")).toBe("ForgeOS Bot Inc");
		});
		expect(invoke).toHaveBeenCalledWith("forgeos_account", {
