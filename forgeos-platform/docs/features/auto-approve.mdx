...
description: "Control which actions ForgeOS can take without asking for approval."
---

Auto Approve lets you decide which actions ForgeOS can take without prompting you each time. It keeps you out of approval popups during routine work while letting you keep tight control over high-risk actions.
...
<Frame>
	<video
		style={{ width: "100%" }}
		src="https://storage.googleapis.com/forgeos_public_images/autoapprove.mp4"
		autoPlay
		controls
		playsInline
	/>
...
Auto Approve is evaluated per tool call. When ForgeOS is about to read a file, edit a file, run a command, or use the browser, it checks your Auto Approve settings for that category.
...
- **Terminal commands**: ForgeOS treats commands as either safe or requiring approval. "Execute safe commands" covers safe commands. "Execute all commands" extends to commands flagged as requiring approval.
- **Notifications**: If enabled, ForgeOS sends OS-level notifications when approval is required, and when an auto-approved terminal command has been running for 30 seconds.
...
ForgeOS does not use a fixed allowlist. The model marks each command with a `requires_approval` flag based on the command and arguments. These are examples, not guarantees.
...
YOLO mode is Auto Approve on steroids. Check the box and ForgeOS auto-approves everything: file changes, terminal commands, browser actions, MCP tools, and mode transitions.

<Note>
**Warning: This is dangerous.** YOLO mode disables all safety checks. ForgeOS executes whatever it decides without asking permission.
</Note>
...
**Trusted, repetitive tasks** where you've validated ForgeOS's approach and want to eliminate approval overhead.

**Demonstration purposes** where you want to show ForgeOS's capabilities without interruptions.
...
- **Monitor the output.** ForgeOS still shows what it's doing. Watch the terminal and file changes.
- **Keep version control handy.** Git becomes your safety net.

### Enabling YOLO Mode

Navigate to ForgeOS Settings, then Features, and check "YOLO Mode." No confirmation dialogs. Once enabled, ForgeOS auto-approves all actions immediately. Uncheck to disable.
