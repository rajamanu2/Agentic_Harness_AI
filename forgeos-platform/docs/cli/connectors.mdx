...
---
<Warning>
  This feature currently only applies to ForgeOS CLI.
</Warning>
...
Run `forgeos connect` to open an interactive wizard that guides you through platform selection, credential entry, security configuration, and advanced options (provider, model, system prompt, agent mode).
...
|----------|---------------|---------------------|
| Telegram | `forgeos connect telegram` | Bot token |
| Slack | `forgeos connect slack` | Bot token plus webhook signing secret/base URL or socket app token |
| Discord | `forgeos connect discord` | Application ID, bot token, public key, base URL |
| Google Chat | `forgeos connect gchat` | Service account credentials JSON, base URL |
| WhatsApp | `forgeos connect whatsapp` | Phone number ID, access token, app secret, verify token, base URL |
| Linear | `forgeos connect linear` | API key, webhook signing secret, base URL |
...
<Steps>
  <Step title="Create a Telegram bot">
    ...
    1. Enter a display name (e.g., "ForgeOS")
    2. Enter a username ending in `bot` (e.g., `forgeos_myname_bot`). Must be unique across Telegram.
    3. BotFather responds with your bot token (looks like `7123456789:AAH...`)
  </Step>

  <Step title="Start the connector">
    ```bash
    forgeos connect telegram -k <BOT-TOKEN>
    ```
...
By default, anyone who finds your bot can message it and it will execute tasks on your machine. The `forgeos connect` wizard asks whether to restrict Telegram access and can configure this for you.
...
Use `--hook-command` only when you need custom access logic. The hook receives each incoming message with sender info via stdin. Your script returns `{"action": "allow"}` or `{"action": "deny", "message": "reason"}`. Without `--allowed-user-id` or `--hook-command`, everything is auto-approved, so restrict Telegram bots that can reach a running ForgeOS instance.
...
```bash
forgeos connect gchat --credentials <JSON> --base-url <URL>
```
...
```bash
forgeos connect whatsapp --phone-id <ID> --token <TOKEN> --app-secret <SECRET> --base-url <URL>
```
...
```bash
forgeos connect linear --api-key <KEY> --signing-secret <SECRET> --base-url <URL>
```
...
```bash
# Terminal 1
forgeos connect telegram -k $TELEGRAM_TOKEN

# Terminal 2
forgeos connect slack --bot-token $SLACK_TOKEN --signing-secret $SECRET --base-url $URL
```

Connectors require the hub. Start it with `forgeos hub start` if it doesn't auto-start.
