...
import type { ForgeOSAccountOrganization, ForgeOSAccountUser } from "@forgeos/core";
import {
	createContext,
	useCallback,
	useContext,
...
export const ACCOUNT_IDENTITY_STORAGE_KEY = "forgeos.code.account-identity.v1";

const SIGNED_OUT_ERROR_MARKERS = [
	"No ForgeOS account auth token found",
	"no longer valid",
];

type AccountContextValue = {
	user: ForgeOSAccountUser | null;
...
export function parseCachedAccountUser(
	raw: string | null,
): ForgeOSAccountUser | null {
	if (!raw) {
		return null;
	}
	try {
		const parsed = JSON.parse(raw) as { user?: ForgeOSAccountUser | null };
...
function readCachedAccountUser(): ForgeOSAccountUser | null {
...
function writeCachedAccountUser(user: ForgeOSAccountUser | null): void {
	if (typeof window === "undefined") {
		return;
	}
	try {
...
export function AccountProvider({ children }: { children: React.ReactNode }) {
	...
	const refreshAccount = useCallback(async () => {
		try {
			const me = await desktopClient.invoke<ForgeOSAccountUser>("forgeos_account", {
			...
			if (isForgeOSAccountNotAuthenticatedResult(me)) {
				setUser(null);
