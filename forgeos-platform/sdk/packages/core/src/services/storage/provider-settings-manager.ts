...
import { resolveProviderSettingsPath } from "@forgeos/shared/storage";
...
export class ProviderSettingsManager {
	...
	read(): StoredProviderSettings {
		...
		try {
			...
			if (result.success) {
				...
				const forgeosAuth = result.data.providers["forgeos"]?.settings?.auth;
				sdkDebug(
					`providers.read providers=[${Object.keys(result.data.providers).join(",")}] lastUsed=${result.data.lastUsedProvider ?? "none"} forgeosAuthPresent=${!!forgeosAuth?.accessToken} forgeosAccessTokenHash=${hashSecret(forgeosAuth?.accessToken)} forgeosRefreshTokenHash=${hashSecret(forgeosAuth?.refreshToken)}`,
				);
				return result.data;
			}
		} catch {
	...
	write(state: StoredProviderSettings): void {
		...
		// ForgeOS processes (CLI, extension, hub) share this file; a bare
	...
	saveProviderSettings(
		...
		this.write(next);
		const prevForgeOSAuth = previous.providers["forgeos"]?.settings?.auth;
		const nextForgeOSAuth =
			validatedSettings.provider === "forgeos"
				? validatedSettings.auth
				: next.providers["forgeos"]?.settings?.auth;
		const authDropped =
			!!prevForgeOSAuth?.accessToken && !nextForgeOSAuth?.accessToken;
		sdkDebug(
			`providers.save providerId=${providerId} tokenSource=${tokenSource} forgeosAuthWasPresent=${!!prevForgeOSAuth?.accessToken} forgeosAuthIsPresent=${!!nextForgeOSAuth?.accessToken} authDropped=${authDropped}`,
		);
		return next;
	}
	...
	private resolveLastUsedProviderId(
		...
		if (
			...
			options.isForgeOSPassEnabled === false
		) {
			return FORGEOS_PROVIDER_ID;
		}
