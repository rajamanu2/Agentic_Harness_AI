...
export function generateManifest(nextPkg, legacyPkg, version) {
	...
	// The nightly packaging rewrites the whole `forgeos.*` ID namespace to
	// `forgeos-nightly.*` (scripts/nightlify.mjs), so the context key and the
	...
	const prefix = nextPkg.name === "forgeos-nightly" ? "forgeos-nightly" : "forgeos";
...
function loaderSettings(prefix) {
	return {
		[`${prefix}.rollout.bundleOverride`]: {
			...
			scope: "application",
			markdownDescription:
				"Manual override for ForgeOS's staged extension rollout. `next` forces the new (SDK-based) extension, `legacy` forces the previous one, `auto` follows the remote rollout assignment. Takes effect on window reload.",
		},
	};
}
...
function injectLoaderConfiguration(configuration, prefix) {
	...
	return { title: "ForgeOS", ...(configuration ?? {}), properties };
}
