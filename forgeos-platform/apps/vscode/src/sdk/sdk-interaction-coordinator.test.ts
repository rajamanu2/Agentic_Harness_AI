import type { AgentEvent } from "@forgeos/shared"
...
vi.mock("@core/storage/disk", () => ({
	saveForgeOSMessages: vi.fn().mockResolvedValue(undefined),
}))
...
describe("SdkInteractionCoordinator", () => {
	it("emits a tool approval ask and resolves approval from askResponse state", async () => {
		...
		const forgeosMessages = task.messageStateHandler.getForgeOSMessages()
		expect(forgeosMessages).toHaveLength(1)
		expect(forgeosMessages[0].type).toBe("ask")
		expect(forgeosMessages[0].ask).toBe("tool")
		expect(JSON.parse(forgeosMessages[0].text || "{}")).toMatchObject({ tool: "readFile", path: "README.md" })
		expect(listener).toHaveBeenCalledOnce()

		expect(coordinator.resolvePendingToolApproval(undefined, "yesButtonClicked")).toBe(true)
		expect(recordApprovedToolMessage).toHaveBeenCalledWith("tool-call", forgeosMessages[0].ts)
	...
	it("records the real approval row timestamp that the translator reuses", async () => {
		...
		await vi.waitFor(() => expect(task.messageStateHandler.getForgeOSMessages()).toHaveLength(1))
		const approvalTs = task.messageStateHandler.getForgeOSMessages()[0].ts
	...
	it("resolves denied tool approval with the user reason", async () => {
		...
		const forgeosMessages = task.messageStateHandler.getForgeOSMessages()
		expect(forgeosMessages[0]).toMatchObject({ type: "ask", ask: "command", text: "npm test" })
		...
		expect(task.messageStateHandler.getForgeOSMessages()[1]).toMatchObject({
			type: "say",
			say: "user_feedback",
			text: "too risky",
			images: ["image.png"],
	...
	it("denies edit tools with an explicit file-was-not-modified reason", async () => {
		...
		await vi.waitFor(() => expect(task.messageStateHandler.getForgeOSMessages().length).toBeGreaterThanOrEqual(3))
	...
	it("routes message responses as queued follow-ups without resolving pending tool approval", async () => {
		...
		expect(setTurnPhase).toHaveBeenLastCalledWith("awaiting_approval", task.messageStateHandler.getForgeOSMessages()[0].ts)
	...
	it("records generic no-button approval denials for UI suppression", async () => {
		...
		expect(task.messageStateHandler.getForgeOSMessages()).toHaveLength(1)
	...
	it("auto-approves without emitting UI when the live settings allow the tool", async () => {
		...
		expect(task.messageStateHandler.getForgeOSMessages()).toHaveLength(0)
	...
	it("emits an MCP approval ask with server, tool, and arguments", async () => {
		...
		const [message] = task.messageStateHandler.getForgeOSMessages()
	...
	it("emits ask_question and resolves it with rendered user feedback", async () => {
		...
		expect(task.messageStateHandler.getForgeOSMessages()).toMatchObject([
	...
	it("shows an error row and stops immediately when the mistake limit is reached", async () => {
		...
		const errorText = task.messageStateHandler.getForgeOSMessages()[0].text ?? ""
		expect(errorText).toContain("3 errors in a row")
		expect(errorText).toContain("tool_execution_failed: bad arguments")
		expect(errorText).toContain("Send a message to give ForgeOS guidance")
	})
