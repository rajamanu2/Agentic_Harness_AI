/**
 * ForgeOSTempManager - Manages temporary files for ForgeOS with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "forgeos" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for ForgeOS's temporary files.
 */
class ForgeOSTempManagerImpl {
	private readonly tempDir: string
	private cleanupIntervalId: NodeJS.Timeout | null = null

	constructor() {
		// Uses system temp directory with a dedicated "forgeos" subdirectory when possible:
		// macOS: /var/folders/xx/.../T/forgeos
		// Windows: C:\Users\{user}\AppData\Local\Temp\forgeos
		// Linux: /tmp/forgeos
		const baseTempDir = os.tmpdir()
		const forgeosTempDir = path.join(baseTempDir, "forgeos")

		try {
			fs.mkdirSync(forgeosTempDir, { recursive: true })
			this.tempDir = forgeosTempDir
		} catch {
	...
	/**
	 * Clean up old ForgeOS temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the ForgeOS temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(`ForgeOS temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`)
			}
		} catch (error) {
			Logger.error("Error during ForgeOS temp cleanup", error)
		}
...
export const ForgeOSTempManager = new ForgeOSTempManagerImpl()
