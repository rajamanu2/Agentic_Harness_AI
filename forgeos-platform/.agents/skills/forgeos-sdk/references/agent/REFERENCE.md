...
The `Agent` class (also exported as `AgentRuntime`) is the lightweight, stateless agent loop from `@forgeos/agents`. It handles the core iteration cycle: send messages to an LLM, execute tool calls, collect results, and repeat until the task is done.

## When to Use Agent

| Use Agent when... | Use ForgeOSCore instead when... |
|---|---|
...
| You need browser compatibility | You need config discovery from `.forgeos/` |
