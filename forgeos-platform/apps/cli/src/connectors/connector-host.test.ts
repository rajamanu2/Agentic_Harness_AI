...
describe("handleConnectorUserTurn", () => {
	...
	it("keeps tools disabled when connector startup forced no-tools", async () => {
		...
		await handleConnectorUserTurn({
			...commonInput,
			text: "/tools@ForgeOSAdapterBot on",
		});
		...
		await handleConnectorUserTurn({
			...commonInput,
			text: "/yolo@ForgeOSAdapterBot on",
		});
	...
	it("recovers when the bound session is wedged on a run that never drained", async () => {
		// ForgeOS Mom's failure: the thread pointed at a session whose runtime still
	...
	it("mutes a specific participant in the current connector thread", async () => {
		...
		await handleConnectorUserTurn({
			...
			text: "/mute@ForgeOSAdapterBot <@bob>",
		});
		await handleConnectorUserTurn({
			...commonInput,
		...
		await handleConnectorUserTurn({
			...commonInput,
			thread: alice.thread as never,
			text: "/unmute@ForgeOSAdapterBot <@bob>",
		});
		await handleConnectorUserTurn({
			...commonInput,
			thread: bob.thread as never,
