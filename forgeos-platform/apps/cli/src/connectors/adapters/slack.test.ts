import type { ConnectSlackOptions } from "@forgeos/shared";
...
describe("slack binding lookup", () => {
	...
	it("strips the leading bot mention from Slack message text", () => {
		...
		expect(
			__test__.stripSlackBotMention("<@U0B8E8H3U1F|forgeos> hi", "U0B8E8H3U1F"),
		).toBe("hi");
		expect(
			__test__.stripSlackBotMention("  @U0B8E8H3U1F: hi", "U0B8E8H3U1F"),
		).toBe("hi");
	...
	it("keeps Slack text that does not start with the bot mention", () => {
		...
		expect(__test__.stripSlackBotMention("@forgeos hi", "U0B8E8H3U1F")).toBe(
			"@forgeos hi",
		);
