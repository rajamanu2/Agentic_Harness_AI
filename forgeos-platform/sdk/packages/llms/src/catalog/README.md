...
in ForgeOS request defaults, product-level safety limits, or provider-specific
...
ForgeOS may still ask for fewer output tokens on a given request. That belongs in
...
```bash
bun -F @forgeos/llms generate:models
```
