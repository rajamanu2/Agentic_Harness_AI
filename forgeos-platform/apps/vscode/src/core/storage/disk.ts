...
export const GlobalFileNames = {
	...
	forgeosRecommendedModels: "forgeos_recommended_models.json",
	mcpSettings: "forgeos_mcp_settings.json",
	forgeosRules: ".forgeosrules",
	workflows: ".forgeosrules/workflows",
	hooksDir: ".forgeosrules/hooks",
...
/**
 * Returns the cross-platform path to the ForgeOS home directory (~/.forgeos).
 * This works on macOS, Linux, and Windows:
 * - macOS: /Users/username/.forgeos
 * - Linux: /home/username/.forgeos
 * - Windows: C:\Users\username\.forgeos
 *
 * This is intended to eventually replace ~/Documents/ForgeOS as the global config location.
 */
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const forgeosRulesDir = path.join(userDocumentsPath, "ForgeOS", "Rules")
	try {
		await fs.mkdir(forgeosRulesDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "ForgeOS", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return forgeosRulesDir
}

export async function ensureWorkflowsDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const forgeosWorkflowsDir = path.join(userDocumentsPath, "ForgeOS", "Workflows")
	try {
		await fs.mkdir(forgeosWorkflowsDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "ForgeOS", "Workflows") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return forgeosWorkflowsDir
}

export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "ForgeOS", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "ForgeOS", "MCP") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function ensureHooksDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const forgeosHooksDir = path.join(userDocumentsPath, "ForgeOS", "Hooks")
	try {
		await fs.mkdir(forgeosHooksDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "ForgeOS", "Hooks") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return forgeosHooksDir
}

/**
 * Returns the global skills directory path (~/.forgeos/skills) without creating it.
 */
 ...
 * 3. Each workspace root's .forgeosrules/hooks directory (if they exist)
 *
...
/**
 * Gets the paths to the workspace's .forgeosrules/hooks directories to search for
	...
	return (
		await Promise.all(
			workspaceRootPaths.map(async (workspaceRootPath) => {
				// Look for a .forgeosrules/hooks folder in this workspace root.
