import type { UsageTransaction as ForgeOSAccountUsageTransaction, PaymentTransaction } from "@shared/ForgeOSAccount"
import { isForgeOSInternalTester } from "@shared/internal/account"
import type { UserOrganization } from "@shared/proto/forgeos/account"
...
import { type ForgeOSUser, handleSignOut } from "@/context/ForgeOSAuthContext"
...
import { convertProtoUsageTransactions, getForgeOSUris, getMainRole } from "./helpers"
...
type ForgeOSAccountViewProps = {
	forgeosUser: ForgeOSUser
	userOrganizations: UserOrganization[] | null
	activeOrganization: UserOrganization | null
	forgeosEnv: "Production" | "Staging" | "Local"
}

type CachedData = {
	balance: number | null
	usageData: ForgeOSAccountUsageTransaction[]
	paymentsData: PaymentTransaction[]
	lastFetchTime: number
}

const ForgeOSEnvOptions = ["Production", "Staging", "Local"] as const

const AccountView = ({ onDone, forgeosUser, organizations, activeOrganization }: AccountViewProps) => {
			...
			<div className="grow flex flex-col px-5 overflow-y-auto">
				{forgeosUser?.uid ? (
					<ForgeOSAccountView
						...
						forgeosEnv={environment === "local" ? "Local" : environment === "staging" ? "Staging" : "Production"}
						forgeosUser={forgeosUser}
						key={forgeosUser.uid}
...
const ForgeOSAccountView = ({ forgeosUser, userOrganizations, activeOrganization, forgeosEnv }: ForgeOSAccountViewProps) => {
	const { email, displayName, appBaseUrl, uid } = forgeosUser
	...
	const [usageData, setUsageData] = useState<ForgeOSAccountUsageTransaction[]>([])
	...
	const isForgeOSTester = useMemo(() => (email ? isForgeOSInternalTester(email) : false), [email])
	...
	const forgeosUrl = appBaseUrl || "https://app.forgeos.bot"
			...
			<div className="flex flex-col h-full">
				...
				<div className="w-full flex gap-2 flex-col min-[225px]:flex-row">
					<div className="w-full min-[225px]:w-1/2">
						<VSCodeButtonLink appearance="primary" className="w-full" href={getForgeOSUris(forgeosUrl, "dashboard").href}>
							Dashboard
						</VSCodeButtonLink>
					</div>
				...
				<CreditBalance
					balance={balance}
					creditUrl={getForgeOSUris(forgeosUrl, "credits", dropdownValue === uid ? "account" : "organization")}
				...
				{isForgeOSTester && environment !== "selfHosted" && (
					<div className="w-full gap-1 items-end">
						<VSCodeDivider className="w-full my-3" />
						<div className="text-sm font-semibold">ForgeOS Environment</div>
						<VSCodeDropdown
							className="w-full mt-1"
							currentValue={forgeosEnv}
							onChange={async (e) => {
								const target = e.target as HTMLSelectElement
								if (target?.value) {
									const value = target.value as "Local" | "Staging" | "Production"
									updateSetting("forgeosEnv", value.toLowerCase())
								}
							}}>
							{ForgeOSEnvOptions.map((env) => (
