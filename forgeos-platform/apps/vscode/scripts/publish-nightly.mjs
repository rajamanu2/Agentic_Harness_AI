 ...
 *    - Changes name to "forgeos-nightly"
 *    - Changes displayName to "ForgeOS (Nightly)"
 ...
 *   `forgeos-nightly` (this is what the scheduled daily nightly workflow
 *   uses). Pass --pre-release to instead publish to the pre-release
 *   channel of `forgeos-nightly` (used for manual publishes from feature
...
const config = {
	...
	nightlyName: "forgeos-nightly",
	originalName: "claude-dev",
	nightlyDisplayName: "ForgeOS (Nightly)",
	...
	get vsixPath() {
		return path.join(this.distDir, "forgeos-nightly.vsix")
	},
	get nodeModulesPath() {
...
class NightlyPublisher {
	 ...
	 * changes package.json name from "claude-dev" to "forgeos-nightly". If we don't
	 * align this link, vsce's dependency detection (`npm list --production`) fails
	 * with ELSPROBLEMS (missing forgeos-nightly + extraneous claude-dev).
	 */
	 ...
	 */
	updatePackageJson() {
		// Replace any occurrences forgeos. or claude-dev with nightly name
		const rawContent = fs.readFileSync(config.packageJsonPath, "utf-8")
		const content = rawContent.replaceAll("claude-dev", config.nightlyName).replaceAll('"forgeos.', `"${config.nightlyName}.`)
	...
	packageExtension(isPreRelease = false) {
		...
		const args = [
			...
			// (the @forgeos/* workspace symlinks point outside the package).
			"--no-dependencies",
...
Options:
  --pre-release    Publish to the pre-release channel of forgeos-nightly.
