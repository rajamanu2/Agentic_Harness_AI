	...
	E2E_WORKSPACE_TYPES.forEach(({ title, workspaceType }) => {
		e2e.extend({
			...
			try {
				...
				await sidebar.waitForSelector('span:has-text("ForgeOS wants to edit this file:")')
				...
				expect(readFileSync(editedFilePath, "utf-8")).toContain('export const name = "forgeos"')
			} finally {
