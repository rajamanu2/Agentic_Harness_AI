...
export async function resumeInteractiveSession(
	...
	const previousAgentResume = process.env.FORGEOS_HOOK_AGENT_RESUME;
	...
	try {
		...
		if (previousAgentResume === undefined) {
			...
			process.env.FORGEOS_HOOK_AGENT_RESUME = previousAgentResume;
		}
		throw error;
	}
...
export async function runInteractive(
	...
	tuiApp = await renderOpenTui({
		...
		loadWelcomeLine: async () =>
			await resolveForgeOSWelcomeLine({
				config,
				forgeosApiBaseUrl: options?.forgeosApiBaseUrl,
				forgeosProviderSettings: options?.forgeosProviderSettings,
			}),
		loadForgeOSAccount: async () =>
			await loadForgeOSAccountSnapshot({
				config,
				forgeosApiBaseUrl: options?.forgeosApiBaseUrl,
			}),
		...
		switchForgeOSAccount: async (organizationId) =>
		...
		onSubmit: async (input, mode, delivery, attachments, onCommandOutput) => {
			...
			try {
				...
				zeroTurnCost = await shouldZeroForgeOSFreeModelCost(config);
		...
		onAccountChange: async () => {
			...
			await loadForgeOSAccountSnapshot({
				...
				logCliError(
					config.logger,
					"ForgeOS account refresh after account change failed",
					{ error },
				);
			});
			await sessionRuntime.restartWithCurrentMessages();
		...
		// when the TUI was launched through `forgeos history`.
	...
	if (updateCliAfterExit) {
		...
		writeln(
			"The shared ForgeOS Hub was updated by another ForgeOS installation. Updating this CLI…",
		);
		const { checkForUpdates } = await import("../commands/update");
		const exitCode = await checkForUpdates({ includeKanban: false });
		writeln(
			exitCode === 0
				? "Start forgeos again to reconnect to the updated Hub."
				: "Update did not complete. Run 'forgeos update' manually, then start forgeos again.",
		);
	}
}
