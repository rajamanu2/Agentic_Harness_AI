...
if [ "$#" -ne 3 ]; then
    ...
    echo ""
    echo "Example:"
    echo "  $0 forgeos-jetbrains-3.55.0.zip forgeos-jetbrains-3.55.0-enterprise.zip endpoints.json"
    exit 1
fi
...
# JetBrains plugins typically have a structure like: forgeos/lib/
# We need to add endpoints.json to the root of the plugin directory
PLUGIN_DIR="$TEMP_DIR/forgeos"
...
echo "When installed in JetBrains IDEs, ForgeOS will automatically use the bundled configuration."
