...
// cancelTask, …) to the ForgeOS SDK (@forgeos/core) and bridges SDK events to
...
import { formatDisplayUserInput, type RemoteConfig, type RemoteConfigBundle } from "@forgeos/shared"
import type { ApiConfiguration } from "@shared/api"
import type { ChatContent } from "@shared/ChatContent"
import { FORGEOS_ACCOUNT_AUTH_ERROR_MESSAGE } from "@shared/ForgeOSAccount"
import { mentionRegexGlobal } from "@shared/context-mentions"
import type { ForgeOSApiReqInfo, ForgeOSMessage, ExtensionState } from "@shared/ExtensionMessage"
import type { HistoryItem } from "@shared/HistoryItem"
import { DeleteAllTaskHistoryCount, type GetTaskHistoryRequest, TaskHistoryArray, TaskResponse } from "@shared/proto/forgeos/task"
import type { Settings } from "@shared/storage/state-keys"
import type { Mode } from "@shared/storage/types"
import type { TelemetrySetting } from "@shared/TelemetrySetting"
import type { ForgeOSCheckpointRestore } from "@shared/WebviewMessage"
...
import { ForgeOSError } from "@/services/error/ForgeOSError"
import { McpHub } from "@/services/mcp/McpHub"
import { telemetryService } from "@/services/telemetry"
import type { ForgeOSExtensionContext } from "@/shared/forgeos"
import { toLegacyApiProvider } from "@/shared/model-catalog/provider-helpers"
import { ShowMessageRequest, ShowMessageType } from "@/shared/proto/host/window"
import { Logger } from "@/shared/services/Logger"
import { isForgeOSManagedProvider } from "@/shared/utils/forgeos"
import { arePathsEqual, getDesktopDir } from "@/utils/path"
import { ForgeOSAccountService } from "./account-service"
import { AuthService, LogoutReason } from "./auth-service"
import { BUILTIN_SLASH_COMMANDS } from "./builtin-slash-commands"
import { buildStartSessionInput, createHistoryItemFromSession } from "./forgeos-session-factory"
...
function usesForgeOSAccountAuth(providerId: string): boolean {
	return getProviderAuthStorageId(providerId) === "forgeos"
}
...
export class Controller {
	...
	mcpHub: McpHub
	accountService: ForgeOSAccountService
	...
	private pendingForgeOSAuthRetryPrompt?: string
	...
	// materialized by remote config under `.forgeos/remote-config/`. Used to expand
	...
	constructor(readonly context: ForgeOSExtensionContext) {
		...
		// IMPORTANT: Use ~/.forgeos/data/settings/ for the settings directory,
		...
		// settings file lives at ~/.forgeos/data/settings/forgeos_mcp_settings.json
		...
		this.accountService = ForgeOSAccountService.getInstance()
		...
		this.messageTranslatorState = new MessageTranslatorState(
			...
			// retired forgeos-free/ models (the error payload itself never names one).
		...
		this.sessions = new SdkSessionLifecycle({
			...
			onSendError: async (error, sessionId) => {
				...
				const isForgeOSAuthError =
					isForgeOSManagedProvider(providerId) &&
					(errorMessage.includes(FORGEOS_ACCOUNT_AUTH_ERROR_MESSAGE) ||
						errorMessage.toLowerCase().includes("missing api key") ||
						errorMessage.toLowerCase().includes("unauthorized"))

				if (isForgeOSAuthError) {
					...
					this.emitForgeOSAuthError()
				} else if (isForgeOSManagedProvider(providerId) && this.isForgeOSBalanceError(errorMessage)) {
					this.captureProviderFailure({
						sessionId,
						error,
						providerId,
					...
					this.emitForgeOSBalanceError(errorMessage)
				} else {
					this.captureProviderFailure({
						sessionId,
						error,
		...
		this.mode = new SdkModeCoordinator({
			...
			emitForgeOSAuthError: () => this.emitForgeOSAuthErrorWithTelemetry(),
		...
		this.followups = new SdkFollowupCoordinator({
			...
			buildStartSessionInput,
			resolveContextMentions: (text) => this.resolveContextMentions(text),
			isForgeOSManagedProviderActive: () => this.isForgeOSManagedProviderActive(),
			emitForgeOSAuthError: () => this.emitForgeOSAuthErrorWithTelemetry(),
			resetMessageTranslator: () => this.resetMessageTranslatorAndFence(),
			postStateToWebview: () => this.postStateToWebview(),
			onResumeFailed: () => {
		...
		this.taskStart = new SdkTaskStartCoordinator({
			...
			clearTask: async () => {
				this.pendingForgeOSAuthRetryPrompt = undefined
			...
			resolveContextMentions: (text) => this.resolveContextMentions(text),
			isForgeOSManagedProviderActive: () => this.isForgeOSManagedProviderActive(),
			emitForgeOSAuthError: (task) => this.emitForgeOSAuthErrorWithTelemetry(task),
	...
	private async refreshRemoteConfig(): Promise<void> {
		...
		// `.forgeos/remote-config/`. Refresh the watcher so slash-command expansion
	 ...
	 * (`.forgeosrules/workflows`, `.forgeos/workflows`, …) and remote-config files
	 * materialized under `<root>/.forgeos/remote-config/{workflows,skills,rules}`.
	 *
	 ...
	 * workspace (`~/.forgeos/data/workspaces/chat`, seeded with an AGENTS.md
	...
	/**
	 * Subscribe to session events translated to ForgeOSMessages.
	...
	/**
	 * Check if the active API provider uses ForgeOS account auth for the current mode.
	 */
	private isForgeOSManagedProviderActive(): boolean {
		return isForgeOSManagedProvider(this.getActiveProviderId())
	}
	...
	private captureProviderFailure(event: ProviderFailureTelemetry): void {
		...
		const forgeosError = ForgeOSError.transform(event.error, model, provider)

		telemetryService.captureProviderApiError({
			ulid,
			model,
			provider,
			errorMessage: forgeosError.message || String(event.error),
			errorStatus: forgeosError.status,
			requestId: forgeosError.requestId,
	...
	private emitForgeOSAuthErrorWithTelemetry(task?: string, sessionId?: string): void {
		this.emitForgeOSAuthError(task)
		this.captureProviderFailure({
			sessionId: sessionId ?? this.task?.taskId,
			error: FORGEOS_ACCOUNT_AUTH_ERROR_MESSAGE,
	...
	/**
	 * Emit a proper auth error for the 'forgeos' provider when the user is not
	 * logged in. The message sequence drives ErrorRow to render the
	 * "Sign in to ForgeOS" button.
	 *
	 * Message sequence:
	 *   1. say:'task'           – the user's message text
	 *   2. say:'api_req_started' – opens the API request row
	 *   3. ask:'api_req_failed'  – ForgeOSError JSON → ErrorRow renders auth UI
	 */
	private emitForgeOSAuthError(task?: string): void {
		const ts = Date.now()
		this.pendingForgeOSAuthRetryPrompt = task
		...
		const forgeosError = new ForgeOSError(
			{ message: FORGEOS_ACCOUNT_AUTH_ERROR_MESSAGE, status: 401 },
			undefined, // modelId
			"forgeos",
		)
		const serializedError = forgeosError.serialize()
	 ...
	 * by reshaping it into ForgeOSError format and inspecting the result.
	 */
	private isForgeOSBalanceError(errorMessage: string): boolean {
		try {
	...
	/**
	 * Emit a balance error for the 'forgeos' provider when the user has insufficient
	 * credits. Produces the same message sequence as emitForgeOSAuthError so the
	 * webview renders the "Buy Credits" button via CreditLimitError.
	 *
	 * Message sequence:
	 *   1. say:'api_req_started' – streamingFailedMessage holds the ForgeOSError JSON
	 *   2. ask:'api_req_failed'  – ForgeOSError JSON → ErrorRow renders balance UI
	 */
	private emitForgeOSBalanceError(rawErrorMessage: string): void {
		const ts = Date.now()
		...
		// ForgeOSError-serialized JSON that the webview's ErrorRow expects.
	...
	async askResponse(prompt?: string, images?: string[], files?: string[]): Promise<void> {
		if (this.pendingForgeOSAuthRetryPrompt !== undefined && this.task?.taskState?.askResponse === "yesButtonClicked") {
			const retryPrompt = this.pendingForgeOSAuthRetryPrompt
			this.pendingForgeOSAuthRetryPrompt = undefined
	...
	async editMessageAndRegenerate(input: {
		...
		const forgeosMessages = currentTask.messageStateHandler.getForgeOSMessages()
		const targetIndex = forgeosMessages.findIndex((message) => message.ts === input.messageTs)
		if (targetIndex === -1) {
			throw new Error("Message to edit was not found")
		}
		const targetMessage = forgeosMessages[targetIndex]
		if (targetMessage.type !== "say" || (targetMessage.say !== "task" && targetMessage.say !== "user_feedback")) {
			throw new Error("Only user messages can be edited")
		}

		const userOrdinal = forgeosMessages
			.slice(0, targetIndex + 1)
			.filter((message) => message.type === "say" && (message.say === "task" || message.say === "user_feedback")).length
		const canRestoreWorkspace = getCheckpointRunCountForMessage(forgeosMessages, targetIndex) !== undefined
		...
		try {
			...
			const historyTitle =
				userOrdinal === 1
					? editedText
					: extractSdkUserText(firstUserMessage ?? {}) || forgeosMessages[0]?.text || editedText
			...
			if (usesForgeOSAccountAuth(config.providerId) && !config.apiKey) {
				this.emitForgeOSAuthErrorWithTelemetry(editedText)
				return
			}
			...
			const visibleMessages = forgeosMessages.slice(0, targetIndex)
	...
	async restoreCheckpoint(input: { checkpointRunCount: number; restoreType: ForgeOSCheckpointRestore }): Promise<void> {
		...
		const currentMessages = currentTask.messageStateHandler.getForgeOSMessages()
		...
		if (config && usesForgeOSAccountAuth(config.providerId) && !config.apiKey) {
			this.emitForgeOSAuthErrorWithTelemetry(restoredText)
			return
		}
	 ...
	 * receives a state with no currentTaskItem/forgeosMessages and flashes back
	...
	async handleSignOut(): Promise<void> {
		...
		await this.taskControl.cancelForgeOSTaskOnSignOut(isForgeOSManagedProvider(sessionProviderId))
	...
	async handleAuthCallback(customToken: string, provider: string | null = null): Promise<void> {
		await this.authService.handleAuthCallback(customToken, provider ?? "forgeos")
