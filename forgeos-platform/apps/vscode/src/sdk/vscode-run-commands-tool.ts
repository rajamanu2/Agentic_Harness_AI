 ...
 * ForgeOSTempManager's periodic cleanup (age + total-size caps) is the backstop
...
function createDetachedCommandLog(terminalCommand: string, existingLines: string[]): DetachedCommandLog {
	const logFilePath = ForgeOSTempManager.createTempFilePath("proceed-while-running")
...
function formatDetachedResult(logFilePath: string, output: string, reason: DetachReason): string {
	return [
		reason === "user"
			...
			: `The command was still starting or running after ${FOREGROUND_COMMAND_AUTO_PROCEED_MS / 1000} seconds, so ForgeOS automatically proceeded while leaving it running in the terminal.`,
