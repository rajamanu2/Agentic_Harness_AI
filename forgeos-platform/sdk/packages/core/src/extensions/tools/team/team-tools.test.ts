...
import { resolveTeamDataDir } from "@forgeos/shared/storage";
...
describe("resolveTeamDataDir", () => {
	...
	it("uses FORGEOS_TEAM_DATA_DIR when set", () => {
		snapshot = captureEnv();
		process.env.FORGEOS_TEAM_DATA_DIR = "/tmp/team-dir";
		process.env.FORGEOS_DATA_DIR = "/tmp/forgeos-data";
		expect(resolveTeamDataDir()).toBe("/tmp/team-dir");
	});

	it("falls back to FORGEOS_DATA_DIR/teams", () => {
		snapshot = captureEnv();
...
describe("createAgentTeamsTools runtime behavior", () => {
	...
	it("injects workspace metadata into forgeos teammate system prompt", async () => {
		const spawnTeammate = vi.fn();
		const runtime = {
			getMemberRole: vi.fn(() => "lead"),
			isTeammateActive: vi.fn(() => false),
