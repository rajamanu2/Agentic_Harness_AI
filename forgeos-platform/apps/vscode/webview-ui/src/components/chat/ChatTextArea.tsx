...
import { FileSearchRequest, FileSearchType, RelativePathsRequest } from "@shared/proto/forgeos/file"
import { PlanActMode, TogglePlanActModeRequest } from "@shared/proto/forgeos/state"
...
import ForgeOSRulesToggleModal from "../forgeos-rules/ForgeOSRulesToggleModal"
	...
	(
		...
		const modelDisplayName = useMemo(() => {
			...
			switch (selectedProvider) {
				...
				case "forgeos-pass":
					// Free models selected on ForgeOSPass go through ForgeOS usage billing,
					// so label them the same way as the forgeos provider
					return selectedModelId.startsWith("forgeos-pass/")
						? `${selectedProvider}:${selectedModelId.replace(/^forgeos-pass\//, "")}`
						: `forgeos:${selectedModelId}`
				case "openai":
				...
				<div className="flex justify-between items-center -mt-[2px] px-3 pb-2">
					...
					<Tooltip>
						<TooltipContent
							...
							side="top">
							{`In ${shownTooltipMode === "act" ? "Act" : "Plan"}  mode, ForgeOS will ${shownTooltipMode === "act" ? "complete the task immediately" : "gather information to architect a plan"}`}
