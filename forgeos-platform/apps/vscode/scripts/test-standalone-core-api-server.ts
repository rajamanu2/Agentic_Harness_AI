...
/**
 * Simple ForgeOS gRPC Server
 *
 * This script provides a minimal way to run the ForgeOS core gRPC service
 ...
 *   2. ForgeOSApiServerMock (mock implementation of the ForgeOS API)
 ...
 *   FORGEOS_DIST_DIR - Override distribution directory (default: PROJECT_ROOT/dist-standalone)
 *   FORGEOS_CORE_FILE - Override core file name (default: forgeos-core.js)
 ...
 *   FORGEOS_ENVIRONMENT - Environment setting (default: local)
 *
...
import { ForgeOSApiServerMock } from "../src/test/e2e/fixtures/server/index"
...
const FORGEOS_ENVIRONMENT = process.env.FORGEOS_ENVIRONMENT || "local"
...
const distDir = process.env.FORGEOS_DIST_DIR || path.join(projectRoot, "dist-standalone")
const forgeosCoreFile = process.env.FORGEOS_CORE_FILE || "forgeos-core.js"
const coreFile = path.join(distDir, forgeosCoreFile)

const childProcesses: ChildProcess[] = []

async function main(): Promise<void> {
	console.log("Starting Simple ForgeOS gRPC Server...")
	...
	if (!fs.existsSync(coreFile)) {
		...
		console.error("  FORGEOS_DIST_DIR - Override distribution directory")
		console.error("  FORGEOS_CORE_FILE - Override core file name")
		console.error("")
	...
	try {
		...
		console.log("ForgeOS API Server started in-process")
	} catch (error) {
		console.error("Failed to start ForgeOS API Server:", error)
		process.exit(1)
	}

	const extensionsDir = path.join(distDir, "vsce-extension")
	const userDataDir = mkdtempSync(path.join(os.tmpdir(), "vsce"))
	const forgeosTestWorkspace = mkdtempSync(path.join(os.tmpdir(), "forgeos-test-workspace-"))
	...
	const hostbridge: ChildProcess = spawn("bun", [path.join(__dirname, "test-hostbridge-server.ts")], {
		...
		env: {
			...process.env,
			TEST_HOSTBRIDGE_WORKSPACE_DIR: forgeosTestWorkspace,
	...
	const baseArgs = ["--enable-source-maps", "--require", workosFetchMockPath, path.join(distDir, "forgeos-core.js")]
	...
	console.log(`Starting ForgeOS Core Service... (useC8=${USE_C8})`)
	...
	const shutdown = async () => {
		...
		await ForgeOSApiServerMock.stopGlobalServer()

		try {
			rmSync(userDataDir, { recursive: true, force: true })
			rmSync(forgeosTestWorkspace, { recursive: true, force: true })
	...
	console.log(`ForgeOS gRPC Server is running on 127.0.0.1:${PROTOBUS_PORT}`)
	console.log("Press Ctrl+C to stop")
}

if (require.main === module) {
	main().catch((err) => {
		console.error("Failed to start simple ForgeOS server:", err)
		process.exit(1)
	})
}
