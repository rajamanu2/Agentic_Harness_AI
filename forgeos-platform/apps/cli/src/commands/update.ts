...
import { resolveForgeOSBuildEnv } from "@forgeos/shared";
...
const DEFAULT_PACKAGE_NAME = "forgeos";
...
export function getInstallationInfo(currentVersion: string): InstallationInfo {
	...
	try {
		const scriptPath = realpathSync(
			process.env.FORGEOS_WRAPPER_PATH || process.argv[1] || "",
		).replace(/\\/g, "/");
 ...
 * forgeos processes are running swaps the binary under them — their respawn
 ...
 *
 * Skipped for npx, dev, unknown installs. Disable with FORGEOS_NO_AUTO_UPDATE=1.
 */
export function autoUpdateOnStartup(): void {
	if (process.env.IS_DEV === "true") return;
	if (process.env.FORGEOS_NO_AUTO_UPDATE === "1") return;
...
async function otherCliClientsAttached(): Promise<boolean> {
	...
	const client = new NodeHubClient({
		...
		displayName: "forgeos update check",
	});
	try {
...
export async function checkForUpdates(
	...
	writeln(
		`${c.cyan}Checking for updates${includeKanban ? " to ForgeOS CLI and kanban" : ""}…${c.reset}`,
	);
	...
	try {
		...
		if (cliUpdateAvailable && latestVersion) {
			if (!updateCommand) {
				writeln(
					`${c.dim}Unable to determine ForgeOS update command. Please update manually with your package manager.${c.reset}`,
				);
				hadFailure = true;
			} else {
				...
				try {
					...
					if (exitCode === 0) {
						...
						writeln(
							`${c.dim}The update takes effect the next time forgeos starts.${c.reset}`,
						);
					} else {
						writeErr(
							`ForgeOS update failed (exit code ${exitCode}). Try running: ${manualUpdateCommand.command}`,
						);
						hadFailure = true;
					}
				} catch (error) {
					...
					writeErr(
						`Failed to run ForgeOS update command ${manualUpdateCommand.command}: ${message}`,
					);
					hadFailure = true;
				}
			}
