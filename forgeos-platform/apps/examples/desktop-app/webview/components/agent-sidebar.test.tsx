...
beforeEach(() => {
	...
	invoke.mockReset();
	invoke.mockRejectedValue(new Error("No ForgeOS account auth token found"));
...
describe("AgentSidebar session organization", () => {
	...
	it("builds the hover overview with branch and secondary metadata last", () => {
		...
		expect(getSessionOverviewItems(thread)).toEqual([
			["Workspace", "forgeos", "/projects/forgeos"],
		...
		expect(getSessionOverviewItems(makeThread("forgeos", 5))).not.toContainEqual([
			"Branch",
			expect.anything(),
		]);
		expect(
	...
	it("shows the signed-in account and active organization in the footer", async () => {
		...
		await vi.waitFor(() => {
			...
			expect(container.textContent).toContain("ForgeOS Bot Inc");
		});
		expect(container.textContent).not.toContain("ForgeOS Desktop");
		...
		const organizationName = [
			...(accountButton?.querySelectorAll("span") ?? []),
		].find((element) => element.textContent === "ForgeOS Bot Inc");
	...
	it("shows the desktop app version and connected Hub when the logo is hovered", async () => {
		...
		const logoButton = container.querySelector('[aria-label="ForgeOS home"]');
		...
		await vi.waitFor(() => {
			...
			expect(document.body.textContent).toContain("ForgeOS Hub @25463");
	...
	it("hosts back and forward navigation in the draggable sidebar title bar", async () => {
		...
		expect(titleBar?.textContent).not.toContain("ForgeOS Code");
	...
	it("places the logo and icon-only new-session action below the title bar", async () => {
		...
		const logo = container.querySelector('[aria-label="ForgeOS home"]');
	...
	it("uses only the ForgeOS logo for home in the collapsed sidebar", async () => {
		await act(async () => {
			root.render(
				<AccountProvider>
		...
		expect(container.querySelector('[aria-label="ForgeOS home"]')).not.toBeNull();
