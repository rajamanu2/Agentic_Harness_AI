...
vi.mock("@/services/account/ForgeOSAccountService", () => ({
...
vi.mock("@/services/EnvUtils", () => ({ buildBasicForgeOSHeaders: async () => ({}) }))
...
describe("SdkRemoteConfigControlPlane", () => {
	...
	it("wraps discovered remote config in a bundle", async () => {
		...
		expect(bundle?.source).toBe("forgeos-extension-remote-config")
