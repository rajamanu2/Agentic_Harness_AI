...
import { ForgeOSIgnoreController, LOCK_TEXT_SYMBOL } from "../ignore/ForgeOSIgnoreController"
...
export const formatResponse = {
	...
	forgeosIgnoreError: (path: string) =>
		`Access to ${path} is blocked by the .forgeosignore file settings. You must try to continue in the task without using this file, or ask the user to update the .forgeosignore file.`,

	permissionDeniedError: (reason: string) =>
		`Command execution blocked by FORGEOS_COMMAND_PERMISSIONS: ${reason}. You must try a different approach or ask the user to update the permission settings.`,
...
(This is an automated message, so do not respond to it conversationally.)`,
	...
	formatFilesList: (
		...
		files: string[],
		didHitLimit: boolean,
		forgeosIgnoreController?: ForgeOSIgnoreController,
	): string => {
		const sorted = files
			.map((file) => {
			...
			// Sort so files are listed under their respective directories to make it clear what files are children of what directories. Since we build file list top down, even if file list is truncated it will show directories that forgeos can then explore further.
			.sort((a, b) => {
		...
		const forgeosIgnoreParsed = forgeosIgnoreController
			? sorted.map((filePath) => {
					// path is relative to absolute path, not cwd
					// validateAccess expects either path relative to cwd or absolute path
					// otherwise, for validating against ignore patterns like "assets/icons", we would end up with just "icons", which would result in the path not being ignored.
					const absoluteFilePath = path.resolve(absolutePath, filePath)
					const isIgnored = !forgeosIgnoreController.validateAccess(absoluteFilePath)
					if (isIgnored) {
		...
		if (didHitLimit) {
			return `${forgeosIgnoreParsed.join(
				"\n",
			)}\n\n(File list truncated. Use list_files on specific subdirectories if you need to explore further.)`
		}
		if (forgeosIgnoreParsed.length === 0 || (forgeosIgnoreParsed.length === 1 && forgeosIgnoreParsed[0] === "")) {
...
(Remember: If it seems the user wants you to use tools only available in Act Mode, you should ask the user to "toggle to Act mode" (use those words) - they will have to manually do this themselves with the Plan/Act toggle button below. You do not have the ability to switch to Act Mode yourself, and must wait for the user to do it themselves once they are satisfied with the plan. You also cannot present an option to toggle to Act mode, as this will be something you need to direct the user to do manually themselves.)`
	...
	forgeosIgnoreInstructions: (content: string) =>
		`# .forgeosignore\n\n(The following is provided by a root-level .forgeosignore file where the user has specified files and directories that should not be accessed. When using list_files, you'll notice a ${LOCK_TEXT_SYMBOL} next to files that are blocked. Attempting to access the file's contents e.g. through read_file will result in an error.)\n\n${content}\n.forgeosignore`,

	forgeosRulesGlobalDirectoryInstructions: (globalForgeOSRulesFilePath: string, content: string) =>
		`# .forgeosrules/\n\nThe following is provided by a global .forgeosrules/ directory, located at ${globalForgeOSRulesFilePath.toPosix()}, where the user has specified instructions for all working directories:\n\n${content}`,

	forgeosRulesLocalDirectoryInstructions: (cwd: string, content: string) =>
		`# .forgeosrules/\n\nThe following is provided by a root-level .forgeosrules/ directory where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${content}`,

	forgeosRulesLocalFileInstructions: (cwd: string, content: string) =>
		`# .forgeosrules\n\nThe following is provided by a root-level .forgeosrules file where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${content}`,
