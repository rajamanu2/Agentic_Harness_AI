...
Module.prototype.require = function (id) {
	...
	// extension host, so `require("@forgeos/core")` fails before tests start.
	...
	if (id === "@forgeos/core") {
		...
		const getProviderAuthStorageId = (providerId) => {
			const normalized = String(providerId || "")
				.trim()
				.toLowerCase()
			if (normalized === "forgeos" || normalized === "forgeos-pass") return "forgeos"
			if (normalized === "oca" || normalized === "openai-codex") return normalized
			return undefined
		}
		const formatForgeOSApiKey = (token) => {
		...
		const getProviderAuthHandler = (providerId) => {
			...
			return {
				...
				getApiKey(settings) {
					...
					if (accessToken) return storageProviderId === "forgeos" ? formatForgeOSApiKey(accessToken) : accessToken
		...
		return {
			createForgeOSTelemetryServiceConfig: (config = {}) => ({
				enabled: false,
				metadata: {
			...
			fetchForgeOSRecommendedModels: async () => ({ recommended: [], free: [] }),
			...
			getValidForgeOSCredentials: async () => undefined,
			loginForgeOSOAuth: async () => undefined,
			loginOcaOAuth: async () => undefined,
			loginOpenAICodex: async () => undefined,
		}
	}

	if (id === "@forgeos/shared") {
		return {
