# ForgeOS Hooks Documentation

## Overview

ForgeOS hooks allow you to execute custom scripts at specific points in the agentic workflow. Hooks can be placed in either:
- **Global hooks directory**: `~/Documents/ForgeOS/Hooks/` (applies to all workspaces)
- **Workspace hooks directory**: `.forgeosrules/hooks/` (applies to the workspace the repo is part of)

Hooks run automatically when enabled.

## Enabling Hooks

1. Open ForgeOS settings in VSCode
...
- **Global Location**: `~/Documents/ForgeOS/Hooks/TaskStart`
- **Workspace Location**: `.forgeosrules/hooks/TaskStart`
...
- **Global Location**: `~/Documents/ForgeOS/Hooks/TaskResume`
- **Workspace Location**: `.forgeosrules/hooks/TaskResume`
...
- **Global Location**: `~/Documents/ForgeOS/Hooks/TaskCancel`
- **Workspace Location**: `.forgeosrules/hooks/TaskCancel`
...
- **Global Location**: `~/Documents/ForgeOS/Hooks/TaskComplete`
- **Workspace Location**: `.forgeosrules/hooks/TaskComplete`
...
- **Global Location**: `~/Documents/ForgeOS/Hooks/UserPromptSubmit`
- **Workspace Location**: `.forgeosrules/hooks/UserPromptSubmit`

### PreToolUse Hook
- **When**: Runs BEFORE a tool is executed
- **Purpose**: Validate parameters, block execution, or add context
- **Global Location**: `~/Documents/ForgeOS/Hooks/PreToolUse`
- **Workspace Location**: `.forgeosrules/hooks/PreToolUse`

### PostToolUse Hook
- **When**: Runs AFTER a tool completes
- **Purpose**: Observe results, track patterns, or add context
- **Global Location**: `~/Documents/ForgeOS/Hooks/PostToolUse`
- **Workspace Location**: `.forgeosrules/hooks/PostToolUse`
...
- **Global Location**: `~/Documents/ForgeOS/Hooks/PreCompact`
- **Workspace Location**: `.forgeosrules/hooks/PreCompact`

## Cross-Platform Hook Format

ForgeOS uses a git-style approach for hooks that works consistently across all platforms:
...
Like git hooks, ForgeOS executes hook files through a shell that interprets the shebang line:
...
```bash
# Create hook file
nano ~/Documents/ForgeOS/Hooks/PreToolUse

# Make executable
chmod +x ~/Documents/ForgeOS/Hooks/PreToolUse
```
...
# Log to file
echo "$input" >> ~/.forgeos/hook-logs/tool-usage.jsonl
...
ForgeOS supports two levels of hooks:

### Global Hooks
- **Location**: `~/Documents/ForgeOS/Hooks/` (macOS/Linux)
...
### Workspace Hooks
- **Location**: `.forgeosrules/hooks/` in each workspace root
...
1. The global hooks directory is automatically created at:
   - macOS/Linux: `~/Documents/ForgeOS/Hooks/`
...
3. Enable hooks in ForgeOS settings
...
```bash
#!/usr/bin/env bash
# ~/Documents/ForgeOS/Hooks/PreToolUse
...
If you have multiple workspace roots, you can place hooks in each root's `.forgeosrules/hooks/` directory. All hooks (global and workspace) may execute concurrently. Their results will be combined:
...
- Look for errors in VSCode's Output panel (ForgeOS channel)
