...
import { CLI_PROMO_CODE } from "../../../utils/forgeos-pass-errors";
import {
	type DialogDismissKey,
	isAnyKeyDismiss,
} from "../../utils/dialog-keys";

const FORGEOS_PASS_SUBSCRIPTION_PATH = "/dashboard/subscription";
const DEFAULT_APP_BASE_URL = "https://app.forgeos.bot";
 ...
 *
 * The key is written both to the provider's auth storage entry (forgeos-pass
 * stores credentials under "forgeos") and to the provider's own entry: settings
 ...
 *
 * Like the ForgeOSPass promo dialog, this screen must never depend on Esc
...
export function buildForgeOSPassSubscriptionPageUrl(
	appBaseUrl: string | undefined,
): string {
	const url = new URL(
		FORGEOS_PASS_SUBSCRIPTION_PATH,
