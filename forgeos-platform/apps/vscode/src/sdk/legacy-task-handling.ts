import { type ContentBlock, formatDisplayUserInput, type MessageWithMetadata } from "@forgeos/shared"
...
export function mergeLegacyUiMessagesWithResumedSdkMessages(
	legacyUiMessages: ForgeOSMessage[],
	sdkForgeOSMessages: ForgeOSMessage[],
): ForgeOSMessage[] {
	const warningIndex = sdkForgeOSMessages.findIndex((message) => message.text?.includes(LEGACY_RESUME_MODEL_WARNING))
	if (warningIndex === -1) {
		return sdkForgeOSMessages
	}

	const resumedMessages = sdkForgeOSMessages.slice(warningIndex + 1)
