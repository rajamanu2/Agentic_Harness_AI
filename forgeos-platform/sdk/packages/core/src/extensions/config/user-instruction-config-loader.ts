...
import { stripUtf8Bom } from "@forgeos/shared";
import {
...
function isIgnorableDirectoryError(error: unknown): boolean {
	...
	return (
		...
		// ENOTDIR: a path component is a file, e.g. `.forgeosrules/workflows`
		// when `.forgeosrules` is a legacy single-file ruleset. Treat it like a
...
async function discoverManagedPluginRoots(
	forgeosDirectoryPath: string,
): Promise<string[]> {
	try {
		const entries = await readdir(forgeosDirectoryPath, { withFileTypes: true });
		const pluginRoots: string[] = [];
		for (const entry of entries) {
			if (!entry.isDirectory()) {
				continue;
			}
			const pluginRoot = join(forgeosDirectoryPath, entry.name);
...
function parseMarkdownFrontmatter(
	...
	// regex below never matches a file that starts with "\uFEFF---" (see forgeos/forgeos#12151).
...
async function discoverSkillFiles(
	...
	if (basename(directoryPath) === ".forgeos") {
...
export function createSkillsConfigDefinition(
	...
	const managedRoot = options?.workspacePath
		? join(options.workspacePath, ".forgeos")
		: undefined;
