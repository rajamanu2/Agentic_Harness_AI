...
describe("hasProviderChanged", () => {
	it("distinguishes provider switches from model switches", () => {
		expect(
			hasProviderChanged(
				{ provider: "forgeos", model: "anthropic/claude-sonnet-4.6" },
		...
		expect(
			hasProviderChanged(
				{ provider: "forgeos", model: "anthropic/claude-sonnet-4.6" },
				{ provider: "forgeos", model: "openai/gpt-5.3-codex" },
			),
		).toBe(false);
	});
	...
	it("omits workspace paths and returns the SDK-resolved chat workspace", async () => {
		const start = vi.fn(async (input: { config: Record<string, unknown> }) => {
			...
			return {
				...
				manifest: {
					cwd: "/home/host/.forgeos/data/workspaces/chat",
					workspace_root: "/home/host/.forgeos/data/workspaces/chat",
				},
		...
		expect(result).toEqual({
			sessionId: "session-pathless",
			cwd: "/home/host/.forgeos/data/workspaces/chat",
			workspaceRoot: "/home/host/.forgeos/data/workspaces/chat",
		});
...
describe("first-send connection updates", () => {
	...
	it.each([
		...
		const { ctx, send, sessionId } = createContext();
		const previousSessionDataDir = process.env.FORGEOS_SESSION_DATA_DIR;
		const testSessionDataDir = join(
			tmpdir(),
			`forgeos-desktop-attachments-${Date.now()}-${delivery ?? "immediate"}`,
		);
		...
		try {
			process.env.FORGEOS_SESSION_DATA_DIR = testSessionDataDir;
			await handleChatSessionCommand(ctx, {
				action: "send",
				sessionId,
				prompt: "",
			...
			if (previousSessionDataDir === undefined) {
				...
				process.env.FORGEOS_SESSION_DATA_DIR = previousSessionDataDir;
			}
	...
	it("deletes materialized attachments when a queued prompt is removed", async () => {
		...
		const testSessionDataDir = join(
			tmpdir(),
			`forgeos-desktop-attachments-remove-${Date.now()}`,
		);
	...
	it("deletes tracked attachments when a session is reset", async () => {
		...
		const testSessionDataDir = join(
			tmpdir(),
			`forgeos-desktop-attachments-reset-${Date.now()}`,
		);
	...
	it("preserves tracked attachments across re-attach", async () => {
		...
		const testSessionDataDir = join(
			tmpdir(),
			`forgeos-desktop-attachments-attach-${Date.now()}`,
		);
	...
	it("restores the previous provider runtime when replacement startup fails", async () => {
		...
		const previousKanbanDataDir = process.env.FORGEOS_KANBAN_DATA_DIR;
		const testKanbanDataDir = join(
			tmpdir(),
			`forgeos-provider-rollback-${process.pid}`,
		);
		process.env.FORGEOS_KANBAN_DATA_DIR = testKanbanDataDir;
		start
		...
		try {
			...
			await handleChatSessionCommand(ctx, {
				action: "send",
				sessionId,
				prompt: "continue with ForgeOS",
				config: { ...baseConfig },
			});
			expect(send).toHaveBeenCalledOnce();
		} finally {
			if (previousKanbanDataDir === undefined) {
				delete process.env.FORGEOS_KANBAN_DATA_DIR;
			} else {
				process.env.FORGEOS_KANBAN_DATA_DIR = previousKanbanDataDir;
			}
	...
	it("restores the previous provider when replacement label sync fails", async () => {
		...
		const testKanbanDataDir = join(
			tmpdir(),
			`forgeos-provider-label-rollback-${process.pid}`,
		);
		process.env.FORGEOS_KANBAN_DATA_DIR = testKanbanDataDir;
		try {
			updateSessionConnection
...
describe("workspace metadata prewarming", () => {
	it("reuses one in-flight scan and consumes it only once", async () => {
		...
		const cwd = "/tmp/forgeos-desktop-prewarm-reuse";
	...
	it("evicts failed scans so the next session can retry", async () => {
		...
		const cwd = "/tmp/forgeos-desktop-prewarm-retry";
	...
	it("keeps different workspaces in separate single-flight entries", () => {
		...
		prewarmWorkspaceMetadata("/tmp/forgeos-desktop-prewarm-a", load);
		prewarmWorkspaceMetadata("/tmp/forgeos-desktop-prewarm-b", load);
	...
	it("refreshes a prewarm that is older than the startup window", async () => {
		...
		const cwd = "/tmp/forgeos-desktop-prewarm-expired";
...
describe("runtime slash command expansion on send", () => {
	...
	function createWorkspaceWithSkill(): string {
		...
		const skillDir = join(workspace, ".forgeos", "skills", "desktop-send-skill");
...
Follow the desktop send skill instructions.`,
	...
	function createContext(workspace: string) {
		...
		const session = {
			config: { provider: "forgeos", model: "test-model", cwd: workspace },
			messages: [],
			promptsInQueue: [],
			busy: false,
			startedAt: Date.now(),
