...
import type { ForgeOSApiReqInfo, ForgeOSMessage } from "@shared/ExtensionMessage"
import { Logger } from "@/shared/services/Logger"
import type { MessageIdMinter } from "./message-id-minter"
import type { TaskProxy } from "./task-proxy"
import { pushMessageToWebview } from "./webview-grpc-bridge"

export type SessionEventListener = (messages: ForgeOSMessage[], event: CoreSessionEvent) => void
...
export class SdkMessageCoordinator {
	...
	private saveForgeOSMessagesTimer: ReturnType<typeof setTimeout> | undefined
	 ...
	 */
	private stamp(messages: ForgeOSMessage[]): void {
	...
	cancelPendingSave(): void {
		if (this.saveForgeOSMessagesTimer) {
			clearTimeout(this.saveForgeOSMessagesTimer)
			this.saveForgeOSMessagesTimer = undefined
		}
	}
	...
	emitSessionEvents(messages: ForgeOSMessage[], event: CoreSessionEvent): void {
	...
	appendMessages(messages: ForgeOSMessage[]): void {
	...
	appendAndEmit(messages: ForgeOSMessage[], event: CoreSessionEvent): void {
		this.appendMessages(messages)
		this.emitSessionEvents(messages, event)
	}

	emitHookMessage(message: ForgeOSMessage): void {
	 ...
	 */
	finalizeMessagesForSave(messages: ForgeOSMessage[]): ForgeOSMessage[] {
		return messages.map((msg, index) => {
			...
			if (updated.type === "say" && updated.say === "api_req_started") {
				try {
					const info: ForgeOSApiReqInfo = JSON.parse(updated.text || "{}")
