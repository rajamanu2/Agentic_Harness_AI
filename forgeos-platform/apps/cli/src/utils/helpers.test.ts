...
type EnvSnapshot = {
	...
	FORGEOS_HOOKS_LOG_PATH: string | undefined;
	FORGEOS_SESSION_ID: string | undefined;
...
function captureEnv(): EnvSnapshot {
	return {
		...
		FORGEOS_HOOKS_LOG_PATH: process.env.FORGEOS_HOOKS_LOG_PATH,
		FORGEOS_SESSION_ID: process.env.FORGEOS_SESSION_ID,
...
function restoreEnv(snapshot: EnvSnapshot): void {
	...
	process.env.FORGEOS_HOOKS_LOG_PATH = snapshot.FORGEOS_HOOKS_LOG_PATH;
	process.env.FORGEOS_SESSION_ID = snapshot.FORGEOS_SESSION_ID;
...
describe("parseArgs", () => {
	...
	it("parses provider via -P shorthand", () => {
		const parsed = parseArgs(["-P", "forgeos"]);
		expect(parsed.provider).toBe("forgeos");
	});

	it("enables sandbox automatically when --data-dir is set", () => {
		const parsed = parseArgs(["--data-dir", "./.tmp-forgeos"]);
		expect(parsed.sandbox).toBe(true);
		expect(parsed.dataDir).toBe("./.tmp-forgeos");
	});
...
describe("format helpers", () => {
	...
	// Regression tests for https://github.com/forgeos/forgeos/issues/13036:
...
describe("hook payload validation and audit logging", () => {
	...
	it("writes hook audits to global log", async () => {
		...
		delete process.env.FORGEOS_HOOKS_LOG_PATH;
		delete process.env.FORGEOS_SESSION_ID;
...
describe("sandbox environment", () => {
	it("sets sandbox-specific storage paths", () => {
		...
		const previous = {
			FORGEOS_SANDBOX: process.env.FORGEOS_SANDBOX,
			FORGEOS_SANDBOX_DATA_DIR: process.env.FORGEOS_SANDBOX_DATA_DIR,
			FORGEOS_DATA_DIR: process.env.FORGEOS_DATA_DIR,
			FORGEOS_DB_DATA_DIR: process.env.FORGEOS_DB_DATA_DIR,
			FORGEOS_SESSION_DATA_DIR: process.env.FORGEOS_SESSION_DATA_DIR,
		...
		try {
			...
			expect(process.env.FORGEOS_SANDBOX).toBe("1");
			expect(process.env.FORGEOS_SANDBOX_DATA_DIR).toBe(
				path.join(root, "sandbox-state"),
			);
			expect(process.env.FORGEOS_DATA_DIR).toBe(path.join(root, "sandbox-state"));
			expect(process.env.FORGEOS_DB_DATA_DIR).toBe(
				path.join(root, "sandbox-state", "db"),
			);
			expect(process.env.FORGEOS_SESSION_DATA_DIR).toBe(
				path.join(root, "sandbox-state", "sessions"),
			);
			expect(process.env.FORGEOS_TEAM_DATA_DIR).toBe(
				path.join(root, "sandbox-state", "teams"),
			);
			expect(process.env.FORGEOS_PROVIDER_SETTINGS_PATH).toBe(
				path.join(root, "sandbox-state", "settings", "providers.json"),
			);
			expect(process.env.FORGEOS_HOOKS_LOG_PATH).toBe(
				path.join(root, "sandbox-state", "logs", "hooks.jsonl"),
			);
		} finally {
			process.env.FORGEOS_SANDBOX = previous.FORGEOS_SANDBOX;
			process.env.FORGEOS_SANDBOX_DATA_DIR = previous.FORGEOS_SANDBOX_DATA_DIR;
			process.env.FORGEOS_DATA_DIR = previous.FORGEOS_DATA_DIR;
			process.env.FORGEOS_DB_DATA_DIR = previous.FORGEOS_DB_DATA_DIR;
			process.env.FORGEOS_SESSION_DATA_DIR = previous.FORGEOS_SESSION_DATA_DIR;
			process.env.FORGEOS_TEAM_DATA_DIR = previous.FORGEOS_TEAM_DATA_DIR;
			process.env.FORGEOS_PROVIDER_SETTINGS_PATH =
				previous.FORGEOS_PROVIDER_SETTINGS_PATH;
			process.env.FORGEOS_HOOKS_LOG_PATH = previous.FORGEOS_HOOKS_LOG_PATH;
