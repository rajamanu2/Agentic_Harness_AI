...
description: "Configure AWS Bedrock to use AWS CLI profiles for authentication with ForgeOS. Best for SSO/federated roles and secure enterprise access."
---

### Overview

ForgeOS offers the option of utilizing AWS credentials or AWS profiles to access AWS Bedrock services. SSO/Federated roles are suggested over Legacy IAM configuration; this guide describes how to configure your environment so that ForgeOS uses SSO roles for authentication.
...
3. Continue configuring your profile using [the `aws configure sso` CLI wizard](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-sso.html#cli-configure-sso-configure)
    ...
    - Note which profile name you attach to your AWS account, this is needed to configure ForgeOS in the following steps

4. If you haven't already done so, install VSCode and the ForgeOS extension. Consult the [Getting Started](/getting-started/installing-forgeos) page for guidance.

5. Open the ForgeOS extension, then click on the settings button ⚙️ to select your API Provider.
...
<Frame>
	<img
		src="https://storage.googleapis.com/forgeos_public_images/docs/assets/forgeos-aws-setup-markup%20(1).png"
		alt="AWS Bedrock configuration in ForgeOS settings showing profile authentication setup"
	/>
</Frame>
