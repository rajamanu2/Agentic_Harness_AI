// ---------------------------------------------------------------------------
// CLI headless use cases  (forgeos -y / forgeos --json / piped stdin)
//
// These tests run forgeos as a child process (no TUI harness) and assert on
// stdout, stderr, and exit codes.
//
// Tests tagged @live require a configured provider and are skipped by default.
// Run them with:  FORGEOS_BIN=... npm test -- headless @live
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// forgeos -y "tell me a joke"
// Golden path: prints only LLM output (no chrome), then exits 0.
// Unauthenticated: prints "Not authenticated" and exits 1.
// ---------------------------------------------------------------------------
test.describe("forgeos -y (headless auth failure mode) - unauthenticated", () => {
	test.use({
		program: { file: FORGEOS_BIN, args: ["-y", "tell me a joke"] },
		...TERMINAL_WIDE,
...
// ---------------------------------------------------------------------------
// echo "max paulus" | forgeos "print only the second word I gave you"
// Piped stdin test - uses TUI harness with stdin pre-written
// ---------------------------------------------------------------------------
test.describe("piped stdin | forgeos - unauthenticated", () => {
	test.use({
		program: {
			file: "sh",
			args: [
				"-c",
				`echo "max paulus" | ${FORGEOS_BIN} "print only the second word I gave you"`,
			],
		},
		...TERMINAL_WIDE,
		env: forgeosEnv("unauthenticated"),
...
// ---------------------------------------------------------------------------
// forgeos -y --verbose "tell me a joke" 2>&1
// Golden path: prints model info, prompt, api request, reasoning, task_completion lines
// ---------------------------------------------------------------------------
test.describe("forgeos -y --verbose - unauthenticated", () => {
	test.use({
		program: {
			file: "sh",
			args: ["-c", `${FORGEOS_BIN} -y --verbose "tell me a joke" 2>&1`],
		},
		...TERMINAL_WIDE,
		env: forgeosEnv("unauthenticated"),
	});
...
// ---------------------------------------------------------------------------
// forgeos -y --json "tell me a joke"
// Headless yolo with JSON output (one JSON object per line)
// ---------------------------------------------------------------------------
test.describe("forgeos -y --json - unauthenticated", () => {
	test.use({
		program: { file: FORGEOS_BIN, args: ["-y", "--json", "tell me a joke"] },
		...TERMINAL_WIDE,
		env: forgeosEnv("unauthenticated"),
	});
...
test.describe("forgeos (headless prompt mode) - authenticated @live", () => {
	test.use({
		program: { file: FORGEOS_BIN, args: ["tell me a joke"] },
		...TERMINAL_WIDE,
		env: forgeosEnv("default", {
			FORGEOS_VCR_CASSETTE: "./fixtures/headless-yolo-basic.json",
		}),
	});
...
// ---------------------------------------------------------------------------
// echo "max paulus" | forgeos "..." - authenticated
// Piped stdin test with valid credentials
// ---------------------------------------------------------------------------
test.describe("piped stdin | forgeos - authenticated", () => {
	test.use({
		program: {
			file: "sh",
			args: [
				"-c",
				`echo "butterfly horse country" | ${FORGEOS_BIN} "print only the second word I gave you"`,
			],
		},
		...TERMINAL_WIDE,
		env: forgeosEnv("default", {
			FORGEOS_VCR_CASSETTE: "./fixtures/headless-piped-stdin.json",
		}),
	});
...
// ---------------------------------------------------------------------------
// forgeos --verbose "tell me a joke" 2>&1 - authenticated
// Golden path: prints model info, prompt, api request, reasoning, task_completion
// ---------------------------------------------------------------------------
test.describe("forgeos --verbose - authenticated @live", () => {
	test.use({
		program: {
			file: "sh",
			args: ["-c", `${FORGEOS_BIN} --verbose "tell me a joke" 2>&1`],
		},
		...TERMINAL_WIDE,
		env: forgeosEnv("default", {
			FORGEOS_VCR_CASSETTE: "./fixtures/headless-verbose.json",
		}),
	});
...
// ---------------------------------------------------------------------------
// forgeos --json "tell me a joke" - authenticated
// All output must conform to JSON (one JSON object per line)
// ---------------------------------------------------------------------------
test.describe("forgeos --json - authenticated @live", () => {
	test.use({
		program: { file: FORGEOS_BIN, args: ["--json", "tell me a joke"] },
		...TERMINAL_WIDE,
		env: forgeosEnv("default", {
			FORGEOS_VCR_CASSETTE: "./fixtures/headless-json.json",
		}),
	});
...
// ---------------------------------------------------------------------------
// forgeos -t 2 -y "tell me a joke"
// Timeout: should print "Error: Timeout" and exit 1
// ---------------------------------------------------------------------------
test.describe("forgeos -t (timeout) - headless yolo", () => {
	test.use({
		program: {
			file: FORGEOS_BIN,
		...
		...TERMINAL_WIDE,
		env: forgeosEnv("default", {
			FORGEOS_VCR_CASSETTE: "./fixtures/headless-timeout.json",
		}),
	});
...
test.describe("forgeos --json -t (timeout) - JSON mode", () => {
	test.use({
		program: {
			file: FORGEOS_BIN,
		...
		...TERMINAL_WIDE,
		env: forgeosEnv("default", {
			FORGEOS_VCR_CASSETTE: "./fixtures/headless-json-timeout.json",
		}),
	});
...
// ---------------------------------------------------------------------------
// forgeos -y -m <model-id> "what model are you"
// Model flag in headless mode - should use specified model but not persist
// ---------------------------------------------------------------------------
test.describe("forgeos -m (model flag in headless) @live", () => {
	test.use({
		program: {
			file: FORGEOS_BIN,
		...
		...TERMINAL_WIDE,
		env: forgeosEnv("default", {
			FORGEOS_VCR_CASSETTE: "./fixtures/headless-model-flag.json",
		}),
	});
	...
	test("prints a message and exits 0 with --model flag", async ({
		...
		await expectVisible(terminal, /ForgeOS/i);
		await expectExitCode(terminal, EXIT_CODE_SUCCESS);
	});
});
