...
description: Use when preparing, tagging, and publishing a ForgeOS Code desktop app (apps/examples/desktop-app) release. Guides changelog drafting, version bumps in package.json + tauri.conf.json, desktop-vX.Y.Z tags, and the desktop-publish GitHub workflow that builds, signs, notarizes, and updates the auto-update feed.
---

# Desktop App Release

Use this skill when the user asks to release the desktop app, publish ForgeOS Code, bump the desktop version, create a `desktop-vX.Y.Z` tag, or trigger the desktop publish workflow.
...
The sidecar bundles `@forgeos/core` and friends from the monorepo, so SDK changes ship inside the desktop app too. Fold user-visible SDK changes (providers, models, behavior fixes) into the notes; skip purely internal ones.
...
```sh
curl -sL https://github.com/forgeos/forgeos/releases/download/desktop-latest/latest.json | head -30
```
