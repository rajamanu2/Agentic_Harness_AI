...
function isBundledDaemonEntryInvocation(): boolean {
	...
	return (
		process.argv.includes("--forgeos-hub-daemon") ||
		isBunEmbeddedEntry ||
...
function formatClientLabel(clientType: string | undefined): string {
	...
	if (normalized.includes("forgeos")) {
		return "ForgeOS";
	}
	return normalized
		.split(/[-_\s]+/)
...
if (claimedDaemonSentinel || isBundledDaemonEntryInvocation()) {
	await import("@forgeos/core/hub/daemon-entry");
} else {
	main().catch((err) => {
		const msg = err instanceof Error ? err.message : String(err);
