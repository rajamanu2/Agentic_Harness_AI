This is the **ForgeOS** monorepo. Toolchain is **Bun 1.3.13** (package manager + task runner) with **Node >=22** as the runtime. Do not use npm/yarn/pnpm.

## Cloud Agent Instructions

### ForgeOS CLI
- Run from source: `bun run cli` (interactive: `bun run cli -i`; one-shot: append a prompt). This resolves to `apps/cli` and **auto-spawns the `@forgeos/forgeos-hub` daemon** — you do not start the hub separately.
- Inspect local health with `bun run cli doctor`; `bun run cli version` prints the version.
- An actual agent turn requires an **LLM provider credential**. With no credentials the default `forgeos` provider fails fast with an `Unauthorized` error and the interactive TUI shows a provider sign-in screen. Configure via `forgeos auth` or provider env vars (e.g. `ANTHROPIC_API_KEY`, `FORGEOS_API_KEY`, `OPENROUTER_API_KEY`); see `apps/cli/README.md`.

### Build / Lint / test
- SDK packages (`@forgeos/shared|llms|agents|core|sdk`) resolve each other through compiled `dist/` (their `exports` point only at `dist/`, with no `development` source condition). You **must** run `bun run build:sdk` after changing SDK dependencies/source before running the CLI or SDK tests, otherwise imports fail with missing `@forgeos/*` / missing `dist/` errors. Running processes do **not** hot-reload SDK source changes — rebuild and restart.\
- Known cloud-env test artifact: `@forgeos/core` test `src/services/workspace/workspace-manifest.test.ts > readGitWorkspaceState > prefers origin and returns the current branch` fails because cloud VMs configure git `insteadOf` rules that rewrite GitHub remotes to `https://x-access-token:...@github.com/...`. This is an environment artifact, not a code bug.
- Some `@forgeos/cli` e2e assertions (`bun -F @forgeos/cli test:e2e`) may fail on exact tool-listing string formats; treat as pre-existing test drift, not an environment problem.
...
- **Run it (dev host):** `DISPLAY=:1 code --no-sandbox --user-data-dir=/tmp/vscode-userdata --extensionDevelopmentPath=/workspace/apps/vscode <some-folder>`, then click the ForgeOS icon in the Activity Bar to open the webview. (`--no-sandbox` is required in this container.)
...
### Desktop app (`apps/examples/desktop-app`, package `@forgeos/code`)
