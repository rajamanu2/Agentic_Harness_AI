...
import { isModelToolEnabledGlobally, readCompactionStrategyGlobally } from "@forgeos/core"
...
import { getForgeOSOnboardingModels } from "../models/getForgeOSOnboardingModels"
...
export async function getStateToPostToWebview(controller: {
	...
	const onboardingModels = getForgeOSOnboardingModels()
	...
	const globalForgeOSRulesToggles = stateManager.getGlobalStateKey("globalForgeOSRulesToggles")
	...
	const localForgeOSRulesToggles = stateManager.getWorkspaceStateKey("localForgeOSRulesToggles")
	...
	const currentTaskItem = controller.task?.taskId
		...
		: undefined
	const forgeosMessages = [...(controller.task?.messageStateHandler?.getForgeOSMessages?.() || [])]
	...
	const forgeosConfig = ForgeOSEnv.config()
	const environment = forgeosConfig.environment
	...
	return {
		...
		environment,
		distinctId,
		globalForgeOSRulesToggles: globalForgeOSRulesToggles || {},
		localForgeOSRulesToggles: localForgeOSRulesToggles || {},
