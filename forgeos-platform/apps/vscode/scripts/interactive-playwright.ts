...
/**
 * Interactive Playwright launcher for the ForgeOS VS Code extension.
 *
 * Overview:
 *  - Starts the mock ForgeOS API server (from the e2e test fixtures).
 *  - Downloads a stable build of VS Code (via @vscode/test-electron).
 *  - Creates a temporary VS Code user profile directory.
 *  - Installs and links the ForgeOS extension (from dist/e2e.vsix and the dev path).
 *  - Opens a test workspace and automatically reveals the ForgeOS sidebar.
 ...
 *        bun run test:e2e:ui
 *
 *   3. VS Code will launch with the ForgeOS extension loaded and gRPC recording enabled.
 *
...
import { ForgeOSApiServerMock } from "../src/test/e2e/fixtures/server"
...
async function main() {
	...
	console.log("VSCode with ForgeOS extension is now running!")
