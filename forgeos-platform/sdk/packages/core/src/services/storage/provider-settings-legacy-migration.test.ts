...
import * as LlmsModels from "@forgeos/llms";
import { afterEach, describe, expect, it } from "vitest";
import {
	type LegacyForgeOSUserInfo,
	migrateLegacyProviderSettings,
	resolveLegacyForgeOSAuth,
...
describe("migrateLegacyProviderSettings", () => {
	...
	it("migrates legacy ForgeOS OAuth account auth even without a forgeosApiKey", () => {
		const tempDir = mkdtempSync(
			path.join(os.tmpdir(), "core-legacy-provider-"),
		);
		tempDirs.push(tempDir);
		...
		writeFileSync(
			...
			JSON.stringify(
				{
					"forgeos:forgeosAccountId": makeForgeOSAccountJson({
						idToken: "legacy-forgeos-access",
						refreshToken: "legacy-forgeos-refresh",
		...
		expect(result.migrated).toBe(true);
		expect(manager.getProviderSettings("forgeos")?.auth).toEqual({
			accessToken: "legacy-forgeos-access",
			refreshToken: "legacy-forgeos-refresh",
			expiresAt: 1_750_000_000_000,
			accountId: "user-123",
		});
		expect(manager.read().providers.forgeos?.tokenSource).toBe("migration");
	});

	it("falls back to the default ForgeOS model when the legacy model id is unknown", () => {
		const tempDir = mkdtempSync(
			path.join(os.tmpdir(), "core-legacy-provider-"),
		);
		tempDirs.push(tempDir);
			...
			JSON.stringify(
				{
					...
					actModeForgeOSModelId: "some-model/that-no-longer-exists",
				},
				null,
				2,
			),
		...
		writeFileSync(
			path.join(tempDir, "secrets.json"),
			JSON.stringify({ forgeosApiKey: "legacy-forgeos-key" }, null, 2),
		);
		...
		const expectedDefault =
			LlmsModels.getProviderCollectionSync("forgeos")?.provider.defaultModelId;
		expect(expectedDefault).toBeTruthy();
		expect(manager.getProviderSettings("forgeos")?.model).toBe(expectedDefault);
	});

	it("keeps a legacy ForgeOS model id the catalog knows", () => {
		const tempDir = mkdtempSync(
			path.join(os.tmpdir(), "core-legacy-provider-"),
		);
		tempDirs.push(tempDir);
			...
			JSON.stringify(
				{
					mode: "act",
					actModeApiProvider: "forgeos",
					actModeForgeOSModelId: "openai/gpt-5.5",
				},
				null,
				2,
			),
		...
		expect(manager.getProviderSettings("forgeos")?.model).toBe("openai/gpt-5.5");
	});

	it("falls back to the default ForgeOS model for suffixed variant ids like :1m", () => {
		const tempDir = mkdtempSync(
			path.join(os.tmpdir(), "core-legacy-provider-"),
		);
		tempDirs.push(tempDir);
			...
			JSON.stringify(
				{
					mode: "act",
					actModeApiProvider: "forgeos",
					actModeForgeOSModelId: "anthropic/claude-sonnet-4.5:1m",
				},
				null,
				2,
			),
		...
		expect(manager.getProviderSettings("forgeos")?.model).toBe(
			LlmsModels.getProviderCollectionSync("forgeos")?.provider.defaultModelId,
		);
	});

	it("folds legacy alias ForgeOS model ids onto their canonical catalog ids", () => {
		...
		const catalogModels =
			LlmsModels.getProviderCollectionSync("forgeos")?.models ?? {};
			...
			JSON.stringify(
				{
					mode: "act",
					actModeApiProvider: "forgeos",
					actModeForgeOSModelId: aliasModelId,
				},
				null,
				2,
			),
		...
		expect(manager.getProviderSettings("forgeos")?.model).toBe(canonicalModelId);
	});
...
// =============================================================================
// resolveLegacyForgeOSAuth – pure in-memory tests
// =============================================================================

/** Builds a realistic LegacyForgeOSUserInfo JSON string. */
function makeForgeOSAccountJson(
	overrides: Partial<LegacyForgeOSUserInfo> & { userId?: string } = {},
): string {
	return JSON.stringify({
		...
		userInfo: overrides.userInfo ?? {
			...
			forgeosBenchConsent: false,
			createdAt: "2025-01-01T00:00:00Z",
			updatedAt: "2025-01-01T00:00:00Z",
		},
		provider: overrides.provider ?? "google",
		startedAt: overrides.startedAt ?? Date.now(),
	} satisfies LegacyForgeOSUserInfo);
}

describe("resolveLegacyForgeOSAuth", () => {
	it("extracts all auth fields from a complete legacy account JSON", () => {
		const result = resolveLegacyForgeOSAuth(
			makeForgeOSAccountJson({
				idToken: "my-id-token",
	...
	it("maps idToken to accessToken", () => {
		const result = resolveLegacyForgeOSAuth(
			makeForgeOSAccountJson({ idToken: "tok-abc" }),
		);
	...
	it("preserves millisecond expiresAt values", () => {
		const result = resolveLegacyForgeOSAuth(
			makeForgeOSAccountJson({ expiresAt: 9999999999999 }),
		);
	...
	it("normalizes classic second-based expiresAt values to milliseconds", () => {
		const result = resolveLegacyForgeOSAuth(
			makeForgeOSAccountJson({ expiresAt: 1_750_000_000 }),
		);
	...
	it("maps userInfo.id to accountId", () => {
		const result = resolveLegacyForgeOSAuth(
			makeForgeOSAccountJson({ userId: "uid-xyz" }),
		);
	...
	it("returns undefined accountId when userInfo is missing entirely", () => {
		...
		const result = resolveLegacyForgeOSAuth(raw);
	...
	it("returns undefined for invalid json", () => {
		expect(resolveLegacyForgeOSAuth(undefined)).toBeUndefined();
		expect(resolveLegacyForgeOSAuth("")).toBeUndefined();
		expect(resolveLegacyForgeOSAuth("   \n\t  ")).toBeUndefined();
		expect(resolveLegacyForgeOSAuth("not-json{{{")).toBeUndefined();
		expect(resolveLegacyForgeOSAuth("null")).toBeUndefined();
	});
