...
describe("DefaultRuntimeBuilder team persistence boundary", () => {
	...
	it("forwards forgeos workspace metadata to teammate runtime bootstrap config", async () => {
		const { DefaultRuntimeBuilder } = await import("./runtime-builder");
  ...
  "workspaces": {
    "/repo/demo": {
      ...
      "latestGitBranchName": "main"
		...
		const forgeosBootstrapCall = (
			bootstrapAgentTeamsMock.mock.calls as unknown as Array<[BootstrapCall]>
		)[0]?.[0];
		expect(forgeosBootstrapCall).toBeDefined();
		expect(
			forgeosBootstrapCall?.teammateConfigProvider.getRuntimeConfig(),
		).toEqual(
			expect.objectContaining({
				providerId: "forgeos",
				cwd: "/repo/demo",
