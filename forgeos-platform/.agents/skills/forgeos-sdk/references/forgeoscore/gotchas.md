...
`ForgeOSCore` holds resources (file watchers, database connections, hub connections). Failing to call `dispose()` can leave orphan processes and file locks.
...
ForgeOSCore and `@forgeos/core` require Node.js 22 or later. If you're on an older version, you'll get runtime errors. Check with `node --version`.

## Session Config vs Global Config

Tool policies can be set at two levels:
- Global: in `ForgeOSCore.create({ toolPolicies })` -- applies to all sessions
- Per-session: in `forgeos.start({ toolPolicies })` -- overrides global for that session
...
- Pre-warm the hub with `forgeos hub ensure` CLI command
- Accept the one-time startup cost and let subsequent sessions reuse the hub

## Session Storage Location

Sessions are stored at `~/.forgeos/data/sessions/`. This includes:
...
ForgeOSCore discovers plugins from:
- Global: `~/.forgeos/plugins/`
- Workspace: `.forgeos/plugins/`
...
`forgeos.send()` only works on sessions that are still active. If a session has already completed, `send()` may return `undefined` or fail. Check session status with `forgeos.get(sessionId)` first.
...
For long-running sessions, message history grows and eventually exceeds the model's context window. ForgeOSCore handles this via compaction, which summarizes older messages. Configure it via `compactionConfig`:
