...
async function createWorkspaceWithHook(
	...
	const hooksDir = join(workspace, ".forgeosrules", "hooks");
...
describe("createHookConfigFileHooks", () => {
	const originalHomeDir = dirname(
		dirname(resolveDocumentsForgeOSDirectoryPath()),
	);
	const originalForgeOSDir = resolveForgeOSDir();
	let isolatedRoot = "";

	beforeAll(async () => {
		isolatedRoot = await mkdtemp(join(tmpdir(), "hooks-home-"));
		const isolatedHomeDir = join(isolatedRoot, "home");
		const isolatedForgeOSDir = join(isolatedRoot, "forgeos");
		await mkdir(isolatedHomeDir, { recursive: true });
		await mkdir(isolatedForgeOSDir, { recursive: true });
		setHomeDir(isolatedHomeDir);
		setForgeOSDir(isolatedForgeOSDir);
	});

	afterAll(async () => {
		setHomeDir(originalHomeDir);
		setForgeOSDir(originalForgeOSDir);
		if (isolatedRoot) {
	...
	it("writes audit tool timing and completed turn payloads", async () => {
		...
		const originalLogPath = process.env.FORGEOS_HOOKS_LOG_PATH;
		process.env.FORGEOS_HOOKS_LOG_PATH = outputPath;
		try {
			...
			if (originalLogPath === undefined) {
				...
				process.env.FORGEOS_HOOKS_LOG_PATH = originalLogPath;
			}
	...
	it("dispatches agent_start and prompt_submit exactly once when both are configured", async () => {
		...
		try {
			await writeFile(
				join(workspace, ".forgeosrules", "hooks", "UserPromptSubmit.js"),
				`let data='';process.stdin.on('data',c=>data+=c);process.stdin.on('end',()=>{require('node:fs').appendFileSync(${JSON.stringify(outputPath)}, data.trim()+"\\n");});\n`,
				"utf8",
			);
			const hooks = createHookConfigFileHooks({
