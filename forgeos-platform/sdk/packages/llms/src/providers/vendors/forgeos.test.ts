...
import { createForgeOS, createForgeOSProviderModule } from "./forgeos";

describe("createForgeOS", () => {
	...
	it("exposes web search as a provider-defined client tool", () => {
		const forgeos = createForgeOS({
			apiKey: "test-key",
			baseURL: "https://api.forgeos.bot/api/v1",
			fetch: fetchMock,
		});

		const tool = forgeos.tools.webSearch();

		expect(tool.type).toBe("provider");
		expect(tool.id).toBe("forgeos.web_search");
		expect(tool.execute).toBeTypeOf("function");
	});

	it("executes web search through the ForgeOS search endpoint", async () => {
		fetchMock.mockResolvedValue(
			new Response(
				JSON.stringify({
					data: {
						results: [{ title: "ForgeOS", url: "https://forgeos.bot" }],
					},
				}),
				{ status: 200 },
			),
		...
		expect(fetchMock).toHaveBeenCalledWith(
			"https://api.forgeos.bot/api/v1/search/websearch",
	...
	it.each([
		...
		await forgeos(modelId).doGenerate({
	...
	it.each([
		...
		fetchMock.mockResolvedValue(jsonCompletionResponse(modelId));
		const module = await createForgeOSProviderModule(
			{
				providerId,
				apiKey: "test-key",
				baseUrl: "https://api.forgeos.bot/api/v1",
		...
		// the providerOptions bucket the shared "forgeos" AI SDK provider reads.
		// Both gateway ids ("forgeos" and "forgeos-pass") hit the same ForgeOS API
		// through this provider, so their options key to the "forgeos" bucket.
