---
name: forgeos-sdk
description: Comprehensive ForgeOS SDK skill for building AI agents. Covers the Agent runtime, ForgeOSCore sessions, custom tools, plugins, events, LLM providers, scheduling, multi-agent teams, and production deployment. Use for any task involving @forgeos/sdk or its sub-packages.
metadata:
   references: agent, forgeoscore
---

# ForgeOS SDK Skill

Consolidated skill for building AI agents with the ForgeOS SDK. Use the decision trees below to find the right entry point and API surface, then load detailed references.

## Critical Rules

Follow these rules in all ForgeOS SDK code:

1. Install with `npm install @forgeos/sdk`. The `@forgeos/sdk` package re-exports everything from `@forgeos/core`, `@forgeos/agents`, `@forgeos/llms`, and `@forgeos/shared`.
2. Requires Node.js 22 or later.
3. Use `createTool()` from `@forgeos/sdk` (or `@forgeos/shared`) to define tools. Tool names must be `snake_case`.
...
6. When using `ForgeOSCore`, always call `dispose()` when done to clean up resources.
7. The standalone `Agent` and `ForgeOSCore` have different event systems. For `Agent`: use `agent.subscribe()` to get `AgentRuntimeEvent` types (text streaming is `"assistant-text-delta"`, result text is `result.outputText`). For `ForgeOSCore`: use `forgeos.subscribe()` to get `CoreSessionEvent` types (text streaming is `"chunk"` with `payload.type === "text"`, result text is `result.text`). There is no top-level `onEvent` field on `AgentRuntimeConfig` -- use `agent.subscribe()` or `hooks.onEvent` instead. Do not use event types like `"content_update"` or `"content_start"` with `agent.subscribe()` -- those are internal legacy types from the ForgeOSCore adapter layer.

## How to Use This Skill

### Reference File Structure

The two main API surfaces (`Agent` and `ForgeOSCore`) follow a 4-file pattern. Cross-cutting concepts are single-file guides.
...
./references/forgeoscore/REFERENCE.md       # Start here for full runtime
...
- Tool errors crashing the agent -> `agent/gotchas.md` or `forgeoscore/gotchas.md`
- Provider auth failures -> `providers/REFERENCE.md`
- Session not persisting -> `forgeoscore/gotchas.md`
- Token usage too high -> `production/REFERENCE.md` (cost control)
- Hub connection issues -> `forgeoscore/gotchas.md`
...
| ForgeOSCore | `./references/forgeoscore/REFERENCE.md` | Full runtime with sessions, persistence, built-in tools |
...
| `@forgeos/core` | Sessions, persistence, built-in tools, config, hub |
| `@forgeos/agents` | Stateless agent loop, tool orchestration, streaming |
| `@forgeos/llms` | LLM provider gateway |
| `@forgeos/shared` | Types, tool helpers, hook engine |

## Resources

Repository: https://github.com/forgeos/forgeos
SDK Source: https://github.com/forgeos/forgeos/tree/main/sdk
Documentation: https://docs.forgeos.bot/sdk/overview
Discord: https://discord.gg/forgeos
