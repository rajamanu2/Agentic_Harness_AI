...
// Convergent-replica reducer for the webview's forgeosMessages transcript.
//
...
function seqOf(message: ForgeOSMessage): number {
	return message.seq ?? 0
}

/** Replace the replica's transcript wholesale at a new epoch (new task / history load). */
function resetTo(epoch: number, messages: ForgeOSMessage[], stateVersion: number, turnState?: TurnState): ReplicaState {
...
/**
 * Apply one incoming ForgeOSMessage (from the partial-message stream OR from within a state
 ...
 */
export function applyMessage(state: ReplicaState, incoming: ForgeOSMessage): ReplicaState {
...
export function applyStateSnapshot(
	state: ReplicaState,
	snapshotMessages: ForgeOSMessage[],
	snapshotEpoch = 0,
	snapshotVersion = 0,
