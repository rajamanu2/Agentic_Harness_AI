 ...
 * - FORGEOS_OTEL_TELEMETRY_ENABLED: "1" to enable OpenTelemetry (default: off)
 * - FORGEOS_OTEL_METRICS_EXPORTER: Comma-separated list: "console", "otlp"
 * - FORGEOS_OTEL_LOGS_EXPORTER: Comma-separated list: "console", "otlp"
 * - FORGEOS_OTEL_EXPORTER_OTLP_PROTOCOL: "grpc", "http/json", or "http/protobuf"
 * - FORGEOS_OTEL_EXPORTER_OTLP_ENDPOINT: OTLP collector endpoint (if not using specific endpoints)
 * - FORGEOS_OTEL_EXPORTER_OTLP_HEADERS: Comma-separated key-value pairs (e.g., "key1=value1,key2=value2")
 * - FORGEOS_OTEL_EXPORTER_OTLP_METRICS_PROTOCOL: Metrics-specific protocol override
 * - FORGEOS_OTEL_EXPORTER_OTLP_METRICS_ENDPOINT: Metrics-specific endpoint override
 * - FORGEOS_OTEL_EXPORTER_OTLP_LOGS_PROTOCOL: Logs-specific protocol override
 * - FORGEOS_OTEL_EXPORTER_OTLP_LOGS_ENDPOINT: Logs-specific endpoint override
 * - FORGEOS_OTEL_METRIC_EXPORT_INTERVAL: Milliseconds between metric exports (default: 60000)
 * - FORGEOS_OTEL_EXPORTER_OTLP_INSECURE: "true" to disable TLS for gRPC (for local development)
 * - FORGEOS_OTEL_LOG_BATCH_SIZE: Maximum batch size for log records (default: 512)
 * - FORGEOS_OTEL_LOG_BATCH_TIMEOUT: Maximum time to wait before exporting logs in ms (default: 5000)
 * - FORGEOS_OTEL_LOG_MAX_QUEUE_SIZE: Maximum queue size for log records (default: 2048)
 *
 * @private
...
function getRuntimeOtelConfig(): OpenTelemetryClientConfig {
	return {
		enabled: process.env.FORGEOS_OTEL_TELEMETRY_ENABLED === "true",
		metricsExporter: process.env.FORGEOS_OTEL_METRICS_EXPORTER,
		logsExporter: process.env.FORGEOS_OTEL_LOGS_EXPORTER,
		otlpProtocol: process.env.FORGEOS_OTEL_EXPORTER_OTLP_PROTOCOL,
		otlpEndpoint: process.env.FORGEOS_OTEL_EXPORTER_OTLP_ENDPOINT,
		otlpMetricsProtocol: process.env.FORGEOS_OTEL_EXPORTER_OTLP_METRICS_PROTOCOL,
		otlpMetricsEndpoint: process.env.FORGEOS_OTEL_EXPORTER_OTLP_METRICS_ENDPOINT,
		otlpLogsProtocol: process.env.FORGEOS_OTEL_EXPORTER_OTLP_LOGS_PROTOCOL,
		otlpLogsEndpoint: process.env.FORGEOS_OTEL_EXPORTER_OTLP_LOGS_ENDPOINT,
		metricExportInterval: process.env.FORGEOS_OTEL_METRIC_EXPORT_INTERVAL
			? Number.parseInt(process.env.FORGEOS_OTEL_METRIC_EXPORT_INTERVAL, 10)
			: undefined,
		otlpInsecure: process.env.FORGEOS_OTEL_EXPORTER_OTLP_INSECURE === "true",
		logBatchSize: process.env.FORGEOS_OTEL_LOG_BATCH_SIZE
			? Math.max(1, Number.parseInt(process.env.FORGEOS_OTEL_LOG_BATCH_SIZE, 10))
			: undefined,
		logBatchTimeout: process.env.FORGEOS_OTEL_LOG_BATCH_TIMEOUT
			? Math.max(1, Number.parseInt(process.env.FORGEOS_OTEL_LOG_BATCH_TIMEOUT, 10))
			: undefined,
		logMaxQueueSize: process.env.FORGEOS_OTEL_LOG_MAX_QUEUE_SIZE
			? Math.max(1, Number.parseInt(process.env.FORGEOS_OTEL_LOG_MAX_QUEUE_SIZE, 10))
			: undefined,
		otlpHeaders: process.env.FORGEOS_OTEL_EXPORTER_OTLP_HEADERS
			? parseKeyPairsIntoRecord(process.env.FORGEOS_OTEL_EXPORTER_OTLP_HEADERS)
			: undefined,
	}
}
