...
Prompt Storage allows enterprises to automatically back up ForgeOS conversation history to cloud storage (AWS S3 or Cloudflare R2). This provides a centralized repository for compliance, audit trails, and usage analysis while maintaining local storage as the primary source of truth.

## Overview

Every ForgeOS task conversation is stored locally in `~/.forgeos/data/tasks/<taskId>/api_conversation_history.json`. When prompt storage is enabled, a background sync worker automatically uploads these conversation files to your configured S3 or R2 bucket.
...
```mermaid
graph LR
    A[User] --> B[ForgeOS Extension]
    B --> C[Local Storage<br/>~/.forgeos/data/tasks/]
...
- ❌ Workspace files not accessed by ForgeOS
- ❌ API keys or secrets  
  ...
  <Tab title="AWS S3">
    ...
    <Steps>
      <Step title="Create S3 Bucket">
        Create a dedicated S3 bucket for ForgeOS conversation storage:
        
        ```bash
        aws s3 mb s3://your-forgeos-prompts --region us-east-1
        ```
      ...
      <Step title="Create IAM Policy">
          ...
          "Statement": [
            {
              ...
              "Resource": "arn:aws:s3:::your-forgeos-prompts/*"
            },
            {
              "Effect": "Allow",
              "Action": [
        ...
        Save this as `forgeos-prompt-storage-policy.json` and create the policy:
        
        ```bash
        aws iam create-policy \
          --policy-name ForgeOSPromptStorage \
          --policy-document file://forgeos-prompt-storage-policy.json
        ```
      </Step>
      ...
      <Step title="Create IAM User">
        ...
        ```bash
        aws iam create-user --user-name forgeos-prompt-uploader
        
        aws iam attach-user-policy \
          --user-name forgeos-prompt-uploader \
          --policy-arn arn:aws:iam::YOUR_ACCOUNT_ID:policy/ForgeOSPromptStorage
        
        aws iam create-access-key --user-name forgeos-prompt-uploader
        ```
        
        Save the `AccessKeyId` and `SecretAccessKey` from the output.
      </Step>

      <Step title="Configure in ForgeOS Dashboard">
        In the ForgeOS admin console at [app.forgeos.bot](https://app.forgeos.bot):
  ...
  <Tab title="Cloudflare R2">
    ...
    <Steps>
      <Step title="Create R2 Bucket">
        ...
        4. Name your bucket (e.g., `forgeos-prompts`)
      ...
      <Step title="Generate API Token">
        ...
        3. Configure permissions:
           - **Token name**: ForgeOS Prompt Storage
...
- On first startup, scans all existing tasks in `~/.forgeos/data/tasks/`
- Queues conversation files that haven't been uploaded
- Useful for enabling prompt storage on an existing ForgeOS deployment
...
While prompt storage operates independently, it integrates with ForgeOS's observability system:
...
aws cloudwatch get-metric-statistics \
  ...
  --dimensions Name=BucketName,Value=your-forgeos-prompts \
  ...
  "Statement": [
    {
      ...
      "Action": "s3:*",
      "Resource": [
        "arn:aws:s3:::your-forgeos-prompts/*",
        "arn:aws:s3:::your-forgeos-prompts"
      ],
      "Condition": {
        "Bool": {
...
aws s3api put-bucket-logging \
  ...
  --bucket-logging-status '{
    "LoggingEnabled": {
      ...
      "TargetPrefix": "forgeos-prompts-access/"
    }
  }'
```
...
2. Look for `[ForgeOSBlobStorage]` and `[SyncWorker]` log entries
...
<CardGroup cols={3}>
  ...
  <Card title="Telemetry" icon="chart-simple" href="/enterprise-solutions/monitoring/telemetry">
    Learn about ForgeOS's built-in anonymous usage tracking
  </Card>
