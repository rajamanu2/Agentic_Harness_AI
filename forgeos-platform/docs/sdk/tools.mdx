...
`ForgeOSCore` can enable the built-in tool suite:
...
For complete examples of tools in action, see the [cli-agent](https://github.com/forgeos/forgeos/tree/main/apps/examples/cli-agent) (shell tool) and [code-review-bot](https://github.com/forgeos/forgeos/tree/main/apps/examples/code-review-bot) (multiple tools with completion lifecycle).
...
With `ForgeOSCore`, custom tools are passed as `extraTools` in session config:
...
Tools work along side MCP. `ForgeOSCore` can load MCP settings through its runtime/config extension path. MCP tools are registered alongside built-in and custom tools when MCP support is enabled for the session.
