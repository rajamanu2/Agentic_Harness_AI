...
describe("plugin-loader", () => {
	...
	beforeAll(async () => {
		...
		const sdkDir = join(dir, "node_modules", "@forgeos", "shared");
		...
		await writeFile(
			...
			[
				"import { safeJsonStringify } from '@forgeos/shared';",
				"import { resolveForgeOSDataDir } from '@forgeos/shared/storage';",
				"import YAML from 'yaml';",
				"export default {",
				"  name: typeof safeJsonStringify === 'function' ? YAML.stringify({ ok: !!resolveForgeOSDataDir() }) : 'invalid',",
				"  manifest: { capabilities: ['tools'] },",
				"};",
			].join("\n"),
			"utf8",
		...
		await writeFile(
			...
			[
				"import { createConfiguredTelemetryHandle } from '@forgeos/core/telemetry';",
				"export default {",
	...
	it("resolves standalone plugin dependencies from the npm wrapper path", async () => {
		...
		await mkdir(depDir, { recursive: true });
		await writeFile(join(wrapperBinDir, "forgeos"), "#!/usr/bin/env node\n");
		await writeFile(
			join(depDir, "package.json"),
			JSON.stringify({
		...
		try {
			process.env.FORGEOS_WRAPPER_PATH = join(wrapperBinDir, "forgeos");
