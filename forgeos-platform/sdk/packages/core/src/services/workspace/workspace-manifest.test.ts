...
import { emptyWorkspaceManifest, upsertWorkspaceInfo } from "@forgeos/shared";
...
describe("readGitWorkspaceState", () => {
	test("prefers origin and returns the current branch", async () => {
		...
		await git.addRemote("origin", "git@github.com:forgeos/forgeos.git");

		await expect(readGitWorkspaceState(dir)).resolves.toEqual({
			url: "git@github.com:forgeos/forgeos.git",
