import { resolveProviderRequestHeaders } from "@forgeos/llms";
import type {
	AgentEvent,
	AgentFinishReason,
	AgentResult,
...
export class HubRuntimeHost implements RuntimeHost {
	...
	async restoreSession(
		...
		const messages = Array.isArray(reply.payload?.messages)
			? (reply.payload.messages as import("@forgeos/llms").Message[])
			: undefined;
	...
	async readSessionMessages(
		sessionId: string,
	): Promise<import("@forgeos/llms").MessageWithMetadata[]> {
		const target = sessionId.trim();
		if (!target) {
			return [];
		}
		...
		return Array.isArray(messages)
			? (messages as import("@forgeos/llms").MessageWithMetadata[])
			: [];
	}
