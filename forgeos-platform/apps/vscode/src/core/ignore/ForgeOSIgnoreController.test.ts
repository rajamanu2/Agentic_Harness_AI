...
import { ForgeOSIgnoreController } from "./ForgeOSIgnoreController"
import "should"

describe("ForgeOSIgnoreController", () => {
	let tempDir: string
	let controller: ForgeOSIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .forgeosignore file
		await fs.writeFile(
			path.join(tempDir, ".forgeosignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new ForgeOSIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .forgeosignore file", async () => {
			const result = controller.validateAccess(".forgeosignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .forgeosignore", async () => {
			// Create a new .forgeosignore with comments
			await fs.writeFile(
				path.join(tempDir, ".forgeosignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .forgeosignore gracefully", async () => {
			// Create a new controller in a directory without .forgeosignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new ForgeOSIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .forgeosignore", async () => {
			await fs.writeFile(path.join(tempDir, ".forgeosignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .forgeosignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".forgeosignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .forgeosignore
			controller = new ForgeOSIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .forgeosignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .forgeosignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".forgeosignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".forgeosignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
