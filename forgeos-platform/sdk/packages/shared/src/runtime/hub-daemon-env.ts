export const FORGEOS_RUN_AS_HUB_DAEMON_ENV = "FORGEOS_RUN_AS_HUB_DAEMON";
export const FORGEOS_CONNECTOR_CLI_LAUNCH_ENV = "FORGEOS_CONNECTOR_CLI_LAUNCH";
export const FORGEOS_CONNECTOR_STARTING_INSTANCE_ENV =
	"FORGEOS_CONNECTOR_STARTING_INSTANCE";
export const FORGEOS_CONNECTOR_SUPERVISED_ENV = "FORGEOS_CONNECTOR_SUPERVISED";
 ...
 * on EADDRINUSE against the real hub. Observed as every `forgeos` invocation from
 * a Slack connector agent failing, `forgeos --help` included, because the
...
export function claimHubDaemonProcess(
	...
	claimedHubDaemonProcess = env[FORGEOS_RUN_AS_HUB_DAEMON_ENV] === "1";
	delete env[FORGEOS_RUN_AS_HUB_DAEMON_ENV];
...
export function isHubDaemonProcess(
	...
	if (env) {
		return env[FORGEOS_RUN_AS_HUB_DAEMON_ENV] === "1";
	}
	return (
		claimedHubDaemonProcess ?? process.env[FORGEOS_RUN_AS_HUB_DAEMON_ENV] === "1"
	);
}
...
export function setConnectorCliLaunchSpec(
	...
	env[FORGEOS_CONNECTOR_CLI_LAUNCH_ENV] = JSON.stringify(spec);
}
 ...
 * — inherits its environment. An inherited marker makes a nested `forgeos connect`
...
export function claimSupervisedConnectorProcess(
	...
	claimedSupervisedConnectorProcess =
		env[FORGEOS_CONNECTOR_SUPERVISED_ENV] === "1";
	delete env[FORGEOS_CONNECTOR_SUPERVISED_ENV];
...
export function isSupervisedConnectorProcess(
	...
	if (env) {
		return env[FORGEOS_CONNECTOR_SUPERVISED_ENV] === "1";
	}
	return (
		claimedSupervisedConnectorProcess ??
...
export function setStartingConnectorInstance(
	...
	env[FORGEOS_CONNECTOR_STARTING_INSTANCE_ENV] = JSON.stringify(ref);
}

export function readStartingConnectorInstance(
	env: Record<string, string | undefined> = process.env,
): ConnectorInstanceRef | undefined {
	const raw = env[FORGEOS_CONNECTOR_STARTING_INSTANCE_ENV];
	if (!raw) {
		return undefined;
	}
	try {
...
export function readConnectorCliLaunchSpec(
	...
	const raw = env[FORGEOS_CONNECTOR_CLI_LAUNCH_ENV];
	if (!raw) {
		return undefined;
	}
	try {
