import type { ForgeOSMessage, ForgeOSSayTool, TurnState } from "@shared/ExtensionMessage"
 ...
 */
export function getButtonConfig(message: ForgeOSMessage | undefined, _mode: Mode = "act"): ButtonConfig {
	if (!message) {
...
function isInertStatusMessage(message: ForgeOSMessage): boolean {
	if (message.type !== "say") {
		return false
	}
 ...
 */
export function getButtonConfigForMessages(messages: ForgeOSMessage[], mode: Mode = "act"): ButtonConfig {
...
export function buttonsForPhase(
	turnState: TurnState,
	anchoredMessage: ForgeOSMessage | undefined,
