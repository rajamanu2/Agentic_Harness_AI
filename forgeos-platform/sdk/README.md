...
The ForgeOS SDK is a TypeScript framework for building AI agents that can edit files, run shell commands, browse the web, call APIs, and use any custom tool you give them. It's the same engine that powers [ForgeOS](https://github.com/forgeos/forgeos), packaged as a library you can embed in your own applications.
...
| [VS Code Extension App](apps/examples/vscode) | VS Code extension example that runs ForgeOS sessions over the RPC runtime |
...
```typescript
import { createTool } from "@forgeos/sdk"
...
## ForgeOSCore: Full Runtime

When you need session persistence, built-in tools, config discovery, and multi-process support, use `ForgeOSCore`:

```typescript
import { ForgeOSCore } from "@forgeos/sdk"

const forgeos = await ForgeOSCore.create({ clientName: "my-app" })

const session = await forgeos.start({
...
`ForgeOSCore` gives the agent built-in tools (`bash`, `editor`, `read_files`, `apply_patch`, `search`, `fetch_web`), persists sessions to SQLite, discovers config from `.forgeos/` directories, and optionally connects to an RPC sidecar for scheduled agents and cross-process session management.
...
|---------|-------------|
| `@forgeos/sdk` | Everything you need -- install this one |
| `@forgeos/core` | Sessions, persistence, built-in tools, config discovery, RPC |
| `@forgeos/agents` | Stateless agent loop with tool execution and streaming |
| `@forgeos/llms` | LLM provider gateway (Anthropic, OpenAI, Google, Bedrock, Mistral, and more) |
| `@forgeos/shared` | Types, tool creation helpers, hook engine |

`@forgeos/sdk` is an alias for `@forgeos/core` that re-exports from all packages, so a single install gives you the full API. The individual packages are available if you want a minimal dependency footprint.

## CLI

The ForgeOS CLI gives you terminal access to the full SDK:
...
# Single prompt
forgeos "Refactor the auth module to use JWT"

# Schedule an agent to run daily
forgeos schedule create "PR summary" --cron "0 9 * * MON-FRI" --prompt "Summarize open PRs"

# Connect a Telegram bot created with @BotFather
forgeos connect telegram -k "$TELEGRAM_BOT_TOKEN"
...
Full documentation at [docs.forgeos.bot/sdk](https://docs.forgeos.bot/sdk/overview):

- [Quickstart](https://docs.forgeos.bot/sdk/quickstart) -- zero to running agent in 5 minutes
- [Core Concepts](https://docs.forgeos.bot/sdk/agents) -- agents, sessions, tools, events, extensions, hooks
- [Guides](https://docs.forgeos.bot/sdk/guides/building-an-agent) -- end-to-end tutorials for common patterns
- [Architecture](https://docs.forgeos.bot/sdk/architecture/overview) -- how the SDK is structured and why
- [API Reference](https://docs.forgeos.bot/sdk/reference/forgeos-core) -- every method, type, and config option
