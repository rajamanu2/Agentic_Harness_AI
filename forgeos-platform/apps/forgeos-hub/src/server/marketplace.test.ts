...
describe("marketplace installer", () => {
	const originalWrapperPath = process.env.FORGEOS_WRAPPER_PATH;
	const originalForgeOSDir = process.env.FORGEOS_DIR;
	const originalHome = process.env.HOME;
	const originalMcpSettingsPath = process.env.FORGEOS_MCP_SETTINGS_PATH;

	afterEach(() => {
		if (originalWrapperPath === undefined) {
			delete process.env.FORGEOS_WRAPPER_PATH;
		} else {
			process.env.FORGEOS_WRAPPER_PATH = originalWrapperPath;
		}
		...
		if (originalMcpSettingsPath === undefined) {
			...
			process.env.FORGEOS_MCP_SETTINGS_PATH = originalMcpSettingsPath;
		}
		vi.restoreAllMocks();
	});
	...
	function createInstalledOfficialPlugin(
		...
		const sourceKey = `official:https://github.com/forgeos/plugins.git#plugins/${slug}`;
	...
	it("runs skills globally for ForgeOS without prompts", async () => {
		const homeDir = mkdtempSync(join(tmpdir(), "forgeos-marketplace-home-"));
	...
	it("skips skill install commands when the global skill already exists", async () => {
		const homeDir = mkdtempSync(join(tmpdir(), "forgeos-marketplace-home-"));
		process.env.HOME = homeDir;
		mkdirSync(join(homeDir, ".agents", "skills", "forgeos-sdk"), {
			recursive: true,
		});
		writeFileSync(
			join(homeDir, ".agents", "skills", "forgeos-sdk", "SKILL.md"),
			"---\nname: forgeos-sdk\n---\n",
		);
		const spawnCommand = vi.fn(async () => ({
			exitCode: 0,
		...
		await expect(
			installMarketplaceEntry(
				{
					entry: {
						...
						type: "skill",
						name: "ForgeOS SDK",
						install: { args: ["forgeos/sdk-skill"] },
					},
				},
				{ spawnCommand },
			),
			...
			status: "installed",
			message: "ForgeOS SDK is already installed.",
		});
		expect(spawnCommand).not.toHaveBeenCalled();
	});

	it("reports ForgeOS global skills as marketplace-installed", () => {
		const forgeosDir = mkdtempSync(join(tmpdir(), "forgeos-marketplace-forgeos-"));
		process.env.FORGEOS_DIR = forgeosDir;
		mkdirSync(join(forgeosDir, "skills", "forgeos-sdk"), {
			recursive: true,
		});
		writeFileSync(
			join(forgeosDir, "skills", "forgeos-sdk", "SKILL.md"),
			"---\nname: forgeos-sdk\n---\n",
		);
		...
		).toEqual({ installedKeys: ["skill:forgeos-sdk"] });
	});

	it("accepts skill installs that create ForgeOS global skills", async () => {
		const homeDir = mkdtempSync(join(tmpdir(), "forgeos-marketplace-home-"));
		const forgeosDir = join(homeDir, ".forgeos");
		process.env.HOME = homeDir;
		process.env.FORGEOS_DIR = forgeosDir;
		const spawnCommand = vi.fn(async () => {
			mkdirSync(join(forgeosDir, "skills", "forgeos-sdk"), {
		...
		await expect(
			...
			status: "installed",
			message: "Installed ForgeOS SDK globally for ForgeOS.",
		});
	});

	it("removes ForgeOS global marketplace skills without prompts", async () => {
		const homeDir = mkdtempSync(join(tmpdir(), "forgeos-marketplace-home-"));
		process.env.HOME = homeDir;
		const skillDir = join(homeDir, ".agents", "skills", "forgeos-sdk");
		...
		await expect(
			...
			status: "uninstalled",
			message: "Uninstalled ForgeOS SDK.",
		});
		expect(spawnCommand).toHaveBeenCalledWith("npx", [
			"-y",
			"skills@latest",
					...
					skills: [
						{
							id: "forgeos-sdk",
							name: "forgeos-sdk",
							path: "/workspace/project/.agents/skills/forgeos-sdk/SKILL.md",
						},
					],
				},
			),
	...
	it("rejects skill installs that do not create a global skill", async () => {
		...
		).rejects.toThrow("was not found in ForgeOS's global skills directories");
	});

	it("runs official plugin installs through the current ForgeOS CLI", async () => {
		process.env.FORGEOS_WRAPPER_PATH = "/usr/local/bin/forgeos";
		const spawnCommand = vi.fn(async () => ({
			exitCode: 0,
		...
		expect(spawnCommand).toHaveBeenCalledWith("/usr/local/bin/forgeos", [
			"plugin",
			"install",
	...
	it("runs MCP installs through the current ForgeOS CLI without prompts", async () => {
		process.env.FORGEOS_WRAPPER_PATH = "/usr/local/bin/forgeos";
		const spawnCommand = vi.fn(async () => ({
			exitCode: 0,
	...
	it("uninstalls official marketplace plugins through the shared core service", async () => {
		const forgeosDir = mkdtempSync(join(tmpdir(), "forgeos-marketplace-plugin-"));
		process.env.FORGEOS_DIR = forgeosDir;
		const installPath = createInstalledOfficialPlugin(forgeosDir, "goal");
		const spawnCommand = vi.fn(async () => ({
			exitCode: 0,
			stdout: "",
			stderr: "",
	...
	it("uninstalls MCP marketplace entries from ForgeOS MCP settings", async () => {
		const settingsPath = join(
			mkdtempSync(join(tmpdir(), "forgeos-marketplace-mcp-")),
			"forgeos_mcp_settings.json",
		);
		process.env.FORGEOS_MCP_SETTINGS_PATH = settingsPath;
		writeFileSync(
			settingsPath,
			JSON.stringify(
				{
	...
	it("uninstalls local MCP servers by name", async () => {
		const settingsPath = join(
			mkdtempSync(join(tmpdir(), "forgeos-local-mcp-")),
			"forgeos_mcp_settings.json",
		);
		process.env.FORGEOS_MCP_SETTINGS_PATH = settingsPath;
		writeFileSync(
	...
	it("uninstalls local skills by removing their configured skill directory", async () => {
		const workspaceRoot = mkdtempSync(join(tmpdir(), "forgeos-local-skill-"));
		const skillDir = join(workspaceRoot, ".forgeos", "skills", "review");
		mkdirSync(skillDir, { recursive: true });
	...
	it("reports official plugin marketplace entries installed from ForgeOS home", () => {
		const forgeosDir = mkdtempSync(join(tmpdir(), "forgeos-marketplace-test-"));
		process.env.FORGEOS_DIR = forgeosDir;
		const sourceKey =
			"official:https://github.com/forgeos/plugins.git#plugins/goal";
		const hash = createHash("sha256")
			.update(sourceKey)
			.digest("hex")
			.slice(0, 12);
		mkdirSync(
			join(forgeosDir, "plugins", "_installed", "official", `goal-${hash}`),
			{
				recursive: true,
			},
		);
	...
	it("does not report plugin inventory substring matches as installed", () => {
		process.env.FORGEOS_DIR = mkdtempSync(
			join(tmpdir(), "forgeos-marketplace-test-"),
		);
			...
			listMarketplaceInstalledEntries(
				...
				},
				{
					plugins: [
						{
							name: "goal-helper",
							path: "/workspace/.forgeos/plugins/goal-helper/index.ts",
						},
					],
				},
			),
	...
	it("fetches the marketplace catalog through the server helper", async () => {
		...
		expect(fetchImpl).toHaveBeenCalledWith(
			"https://forgeos.github.io/marketplace/catalog.json",
