...
describe("RuntimeOAuthTokenManager", () => {
	...
	it("resolves ForgeOSPass OAuth using ForgeOS storage and WorkOS formatting", async () => {
		const getProviderSettings = vi.fn().mockReturnValue({
			provider: "forgeos",
			baseUrl: "https://api.forgeos.test",
			auth: {
		...
		getValidForgeOSCredentials.mockResolvedValueOnce({
			access: "access-new",
			refresh: "refresh-new",
			expires: 4_000_000_000_000,
			accountId: "acct-new",
		...
		expect(getValidForgeOSCredentials).toHaveBeenCalledWith(
			...
			expect.objectContaining({ apiBaseUrl: "https://api.forgeos.test" }),
			{ forceRefresh: false },
		);
		expect(result).toMatchObject({
