import { ForgeOSEndpoint } from "@/config"
import {
...
export type TelemetryProviderConfig =
	 ...
	 * @param bypassUserSettings - When true, telemetry is sent regardless of the user's ForgeOS telemetry opt-in/opt-out settings.
	 * This is used for:
	 * 	- User-controlled collectors configured via environment variables (e.g., FORGEOS_OTEL_TELEMETRY_ENABLED).
...
export class TelemetryProviderFactory {
	...
	public static getDefaultConfigs(): TelemetryProviderConfig[] {
		...
		if (!ForgeOSEndpoint.isSelfHosted() && isPostHogConfigValid(posthogConfig)) {
			configs.push({ type: "posthog", ...posthogConfig })
		}

		// Skip build-time OTEL in selfHosted mode - enterprise customers should not send telemetry to ForgeOS's collector
		...
		if (!ForgeOSEndpoint.isSelfHosted() && otelConfig) {
			configs.push({
				type: "opentelemetry",
				config: otelConfig,
		...
		if (runtimeOtelConfig) {
			configs.push({
				...
				// If the user has `FORGEOS_OTEL_TELEMETRY_ENABLED` in his environment, enable
				// OTEL regardless of his ForgeOS telemetry settings
