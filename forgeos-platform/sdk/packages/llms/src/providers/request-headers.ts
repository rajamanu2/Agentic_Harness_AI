import { decodeJwtPayload } from "@forgeos/shared";
...
const DEFAULT_FORGEOS_REQUEST_HEADERS: Record<string, string> = {
	"HTTP-Referer": "https://forgeos.bot",
	"X-Title": "ForgeOS",
	"X-IS-MULTIROOT": "false",
	"X-CLIENT-TYPE": "forgeos-sdk",
};

function isForgeOSBillingProvider(providerId: string): boolean {
...
function resolveForgeOSClientVersion(
...
function buildForgeOSRequestHeaders(
	input: ResolveProviderRequestHeadersInput,
): Record<string, string> | undefined {
	if (!isForgeOSBillingProvider(input.providerId)) {
		return undefined;
	}
	const source = resolveSource(input.source, input.defaultSource);
	const clientType = trimNonEmpty(input.client?.name) ?? `forgeos-${source}`;
	const clientVersion = resolveForgeOSClientVersion(input.client);
	const platform = trimNonEmpty(input.client?.platform) ?? source;
	const platformVersion =
		trimNonEmpty(input.client?.platformVersion) ?? clientVersion;
	return {
		...DEFAULT_FORGEOS_REQUEST_HEADERS,
		"User-Agent": `ForgeOS/${clientVersion}`,
...
function buildOpenAICodexRequestHeaders(
	...
	return {
		...
		"User-Agent": `ForgeOS/${trimNonEmpty(input.openAiCodex?.userAgentVersion) ?? "1.0.0"}`,
...
function resolveRequiredProviderHeaders(
	...
	return (
		buildForgeOSRequestHeaders(input) ?? buildOpenAICodexRequestHeaders(input)
	);
}
