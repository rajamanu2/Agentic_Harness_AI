...
export function normalizeAutomationOptions(
	options: ForgeOSCoreAutomationOptions | boolean | undefined,
): ForgeOSCoreAutomationOptions | undefined {
...
export function normalizeAutomationCronScope(
	scope: ForgeOSCoreAutomationOptions["cronScope"],
): "global" | "workspace" | undefined {
	if (scope === "user") return "global";
	return scope;
}

export class ForgeOSCoreAutomationController implements ForgeOSCoreAutomationApi {
...
export interface ForgeOSCoreAutomationRuntimeHandlersInput {
	host: RuntimeHost;
	getExtensionContext(): ExtensionContext | undefined;
}

export function createForgeOSCoreAutomationRuntimeHandlers(
	input: ForgeOSCoreAutomationRuntimeHandlersInput,
	...
	return {
		...
		async sendSession(sessionId, request) {
			...
			if (!result) {
				throw new Error("ForgeOSCore automation runtime returned no result");
			}
			return { result: toChatTurnResult(result) };
		},
		async abortSession(sessionId) {
			await host.abort(sessionId, new Error("ForgeOSCore automation abort"));
			return { applied: true };
		},
...
export interface ForgeOSCoreAutomationExtensionContextInput {
	automationService?: CronService;
	automation: ForgeOSCoreAutomationApi;
...
export function createForgeOSCoreAutomationExtensionContext(
	input: ForgeOSCoreAutomationExtensionContextInput,
