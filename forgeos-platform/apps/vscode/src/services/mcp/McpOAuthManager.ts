...
// (~/.forgeos/data/settings/forgeos_mcp_settings.json) under each server's `oauth`
// key, in the format @forgeos/core (CLI, JetBrains) reads and writes:
//
...
//  - Writes scope to ONE server's `oauth` key via @forgeos/core's
...
//    @forgeos/core's authorizeMcpServerOAuth, which binds a local loopback
 ...
 * providers. Matches @forgeos/core's DEFAULT_HTTP_MCP_REDIRECT_URL — the actual
 ...
 * @forgeos/core defaults; extras tolerate concurrent flows from other ForgeOS
 ...
 * reads/writes the shared settings file in @forgeos/core's format.
 *
 ...
 */
class ForgeOSOAuthClientProvider implements OAuthClientProvider {
	constructor(
 ...
 * HTTP-callback authorization flow via @forgeos/core.
 */
export class McpOAuthManager {
	...
	async getOrCreateProvider(serverName: string, serverUrl: string): Promise<OAuthClientProvider> {
		...
		const provider = new ForgeOSOAuthClientProvider(serverName, await this.getSettingsPath())
	 ...
	 *
	 * Delegates to @forgeos/core's authorizeMcpServerOAuth (the exact flow the
