...
async function main(): Promise<number> {
	...
	const testDir = await mkdtemp(join(tmpdir(), "forgeos-pkg-verify-"));
	const packDir = await mkdtemp(join(tmpdir(), "forgeos-pkg-packs-"));
	const bunCacheDir = await mkdtemp(join(tmpdir(), "forgeos-pkg-bun-cache-"));
		...
		for (const pkg of published) {
			...
			await writeFile(
				...
				[
					...
					`import { join } from "node:path";`,
					`try {`,
					`  const root = await import("@forgeos/core");`,
					`  if (typeof root.ForgeOSCore?.create !== "function") {`,
					`    console.error("  FAIL @forgeos/core: root export is missing ForgeOSCore.create");`,
					`    process.exit(1);`,
					`  }`,
					`} catch (error) {`,
					`  const message = error instanceof Error ? error.message : String(error);`,
					`  console.error("  FAIL @forgeos/core: published runtime shape is invalid:", message);`,
					`  process.exit(1);`,
					`}`,
					`console.log("  OK @forgeos/core publish shape");`,
				].join("\n"),
			);
			try {
				const proc = Bun.spawn(["bun", testFile], {
