...
describe("State Keys Type Safety", () => {
	...
	describe("Type Exports", () => {
		...
		it("should export usable key types", () => {
			...
			const localKey: LocalStateKey = "localForgeOSRulesToggles"
		...
		it("should have LocalState keys map to ForgeOSRulesToggles", () => {
			const localState: Partial<LocalState> = {
				localForgeOSRulesToggles: {},
				localCursorRulesToggles: { "some-rule": true },
			}
			expect(localState.localForgeOSRulesToggles).to.deep.equal({})
		})
	})
	...
	describe("Edge Cases", () => {
		...
		it("should handle keys with special characters", () => {
			// The forgeos:forgeosAccountId key has a colon
			expect(SecretKeys).to.include("forgeos:forgeosAccountId")
			expect(isSecretKey("forgeos:forgeosAccountId")).to.be.true
		})
