...
Teams are a ForgeOSCore feature. Enable them in the session config:

```typescript
import { ForgeOSCore } from "@forgeos/sdk"

const forgeos = await ForgeOSCore.create({ clientName: "team-app" })
...
```bash
forgeos --team-name auth-sprint "Continue -- pick up incomplete tasks"
```
...
```bash
# Start a new team
forgeos --team-name auth-sprint "Plan and implement user auth with tests"

# Resume work
forgeos --team-name auth-sprint "What's the status? Continue with unfinished tasks."

# Different team for a different workstream
forgeos --team-name perf-sprint "Profile the API endpoints and optimize the slowest 3"
```
