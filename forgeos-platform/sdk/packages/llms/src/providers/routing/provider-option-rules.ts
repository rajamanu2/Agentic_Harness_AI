...
const forgeosGatewayReasoningRule: ProviderOptionRule = {
	id: "provider.forgeos.reasoning",
	phase: "provider-reasoning",
	description: "ForgeOS gateway accepts the shared gateway reasoning shape.",
	applies: (input) => isForgeOSProvider(input.request.providerId),
	build: (input) =>
...
const forgeosMiniMaxM3GatewayReasoningRule: ProviderOptionRule = {
	id: "provider.forgeos.minimax-m3.gateway-reasoning",
	phase: "provider-reasoning",
	description:
		"ForgeOS-routed MiniMax M3 keeps the gateway reasoning shape instead of leaking generic thinking.",
	applies: (input) =>
		isForgeOSProvider(input.request.providerId) && isMiniMaxM3(input),
	suppresses: { genericThinking: true },
	build: () => undefined,
};
...
const forgeosReasoningDisabledThinkingRule: ProviderOptionRule = {
	id: "provider.forgeos.disable-thinking",
	phase: "provider",
	description:
		"ForgeOS-routed non-Kimi-K2.6 Moonshot Kimi models use thinking.type=disabled when reasoning is disabled.",
	applies: (input) =>
		isForgeOSProvider(input.request.providerId) &&
...
export const PROVIDER_OPTION_RULES: ReadonlyArray<ProviderOptionRule> = [
	...
	forgeosGatewayReasoningRule,
	openRouterReasoningRule,
	forgeosMiniMaxM3GatewayReasoningRule,
	vercelReasoningRule,
	directMoonshotReasoningRule,
	fireworksReasoningRule,
	geminiThinkingRule,
	forgeosReasoningDisabledThinkingRule,
	kimiK26ThinkingRule,
