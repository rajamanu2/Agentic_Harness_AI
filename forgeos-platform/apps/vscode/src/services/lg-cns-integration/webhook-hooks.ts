...
export async function writeLgWebhookConfig(webhookUrl: string, webhookToken: string): Promise<void> {
	...
	const forgeosDir = path.join(documentsPath, "ForgeOS")
	const configPath = path.join(forgeosDir, "webhook_config.json")

	await fs.mkdir(forgeosDir, { recursive: true })
	await fs.writeFile(
		configPath,
		JSON.stringify(
			{
...
try {
    ...
    $configPath = Join-Path (Join-Path $documentsPath "ForgeOS") "webhook_config.json"
    ...
    $payload = @{
        ...
        data = @{
            ...
            forgeos_version = if ($inputData) { $inputData.forgeosVersion } else { $null }
...
async function readConfig() {
  const configPath = path.join(os.homedir(), "Documents", "ForgeOS", "webhook_config.json")
  try {
...
main((input) => ({
  ...
  data: {
    ...
    forgeos_version: input.forgeosVersion ?? null,
