...
const DEFAULT_PROVIDER = "forgeos"
const DEFAULT_MODEL = "anthropic/claude-sonnet-4.5"

// Models to test - can be overridden with --model flag
const MODELS: string[] = [DEFAULT_MODEL]

// Check if forgeos CLI is available
function checkForgeOSCli(): boolean {
	try {
		execSync("which forgeos", { encoding: "utf-8", timeout: 5000 })
		return true
	} catch {
		return false
	}
...
// Use user's existing ForgeOS config (already has auth configured)
// For CI, this would be set up by the auth step before tests run
const FORGEOS_CONFIG_DIR = path.join(process.env.HOME || "", ".forgeos")
...
function configureAuth(options: { provider: string; apiKey: string; modelId: string; baseUrl?: string }): {
	...
	fs.mkdirSync(FORGEOS_CONFIG_DIR, { recursive: true })

	try {
		// Run quick auth setup (non-interactive when all flags provided)
		const args = [`forgeos auth --config "${FORGEOS_CONFIG_DIR}"`, `-p "${options.provider}"`, `-k "${options.apiKey}"`, `-m "${options.modelId}"`]
		if (options.baseUrl) {
...
function ensureScenarioAuth(scenario: SmokeScenario, modelId: string): { ok: boolean; error?: string } {
	...
	const apiKeyEnv = scenario.auth?.apiKeyEnv || (provider === DEFAULT_PROVIDER ? "FORGEOS_API_KEY" : undefined)
	...
	// For default provider, local developers can rely on existing ~/.forgeos auth.
...
async function runTrial(scenario: SmokeScenario, modelId: string, trialWorkdir: string): Promise<TrialResult> {
	...
	// Provider is configured via `forgeos auth` before running tests
	const args = [
		"--config",
		FORGEOS_CONFIG_DIR, // Use shared config directory for auth
	...
	try {
		// Run forgeos CLI
		const result = await runForgeOSWithTimeout(args, trialWorkdir, scenario.timeout * 1000)
...
// Run forgeos CLI with timeout
interface ForgeOSResult {
	success: boolean
	error?: string
	stdout: string
	stderr: string
...
function runForgeOSWithTimeout(args: string[], cwd: string, timeoutMs: number): Promise<ForgeOSResult> {
	return new Promise((resolve) => {
		let stdout = ""
		let stderr = ""

		const proc = spawn("forgeos", args, {
			cwd,
			env: { ...process.env },
...
async function main() {
	...
	// Check forgeos CLI is available
	if (!checkForgeOSCli()) {
		console.error("ERROR: forgeos CLI not found in PATH")
		console.error("")
		console.error("For local development:")
		console.error("  Install forgeos from npm or link the SDK CLI from apps/cli")
		console.error("")
		console.error("For CI:")
		console.error("  Ensure a forgeos binary is available on PATH")
		process.exit(1)
	}
