...
const HUB_AUTH_PROTOCOL_PREFIX = "forgeos-hub-auth.";
					...
					: setTimeout(() => {
							...
							reject(
								new HubCommandError(
									command,
									"hub_command_timeout",
									`Hub command ${command} timed out after ${effectiveTimeoutMs}ms (hub=${this.currentUrl}, requestId=${requestId}, clientId=${this.clientId}). Check hub-daemon.log for matching command.start/command.slow entries, or run 'forgeos doctor fix' to restart the hub.`,
								),
							);
						}, effectiveTimeoutMs);
