...
describe("ForgeOSAccountService.fetchFeaturebaseToken", () => {
	it("returns featurebaseJwt on success", async () => {
		const fetchImpl = vi.fn(async (input: unknown, init?: RequestInit) => {
			expect(String(input)).toBe(
				"https://api.forgeos.bot/api/v1/users/me/featurebase-token",
			);
...
describe("executeForgeOSAccountAction - fetchFeaturebaseToken", () => {
	it("dispatches fetchFeaturebaseToken", async () => {
		const service: ForgeOSAccountOperations = {
			fetchMe: vi.fn(async () => ({
				id: "u1",
		...
		const result = await executeForgeOSAccountAction(
			{ action: "forgeosAccount", operation: "fetchFeaturebaseToken" },
			service,
		);
...
describe("RpcForgeOSAccountService.fetchFeaturebaseToken", () => {
	it("sends provider action payload and parses response", async () => {
		...
		const service = new RpcForgeOSAccountService({ runProviderAction });
