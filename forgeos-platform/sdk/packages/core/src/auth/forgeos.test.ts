...
import type { ForgeOSOAuthCredentials } from "./forgeos";
import {
...
function createCredentials(
	overrides: Partial<ForgeOSOAuthCredentials> = {},
...
describe("auth/forgeos getValidForgeOSCredentials", () => {
	afterEach(() => {
		vi.restoreAllMocks();
	...
	it("returns existing credentials when not expired", async () => {
		...
		const result = await getValidForgeOSCredentials(current, PROVIDER_OPTIONS);
	...
	it("returns null when refresh fails with invalid_grant", async () => {
		...
		const capture = vi.fn();
		const result = await getValidForgeOSCredentials(current, {
			...PROVIDER_OPTIONS,
	...
	it("omits request_id when a failed response has no request ID header", async () => {
		...
		const capture = vi.fn();
		await getValidForgeOSCredentials(current, {
			...PROVIDER_OPTIONS,
			telemetry: { capture } as never,
		});
	...
	it("keeps current credentials on transient refresh error while token remains valid", async () => {
		...
		const capture = vi.fn();
		const result = await getValidForgeOSCredentials(
			current,
	...
	it("throws on transient refresh error when the token is already expired", async () => {
		...
		const capture = vi.fn();
		await expect(
			getValidForgeOSCredentials(current, {
				...PROVIDER_OPTIONS,
				telemetry: { capture } as never,
			}),
...
describe("auth/forgeos loginForgeOSOAuth", () => {
	afterEach(() => {
		vi.restoreAllMocks();
		globalThis.fetch = ORIGINAL_FETCH;
	});
...
describe("auth/forgeos completeForgeOSDeviceAuth", () => {
	afterEach(() => {
		vi.restoreAllMocks();
		globalThis.fetch = ORIGINAL_FETCH;
	});
