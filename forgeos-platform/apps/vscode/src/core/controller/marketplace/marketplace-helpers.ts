...
import { DeleteSkillRequest, ToggleSkillRequest } from "@shared/proto/forgeos/file"
import {
	MarketplaceCatalog,
	MarketplaceEntry,
...
const MARKETPLACE_CATALOG_URL = "https://forgeos.github.io/marketplace/catalog.json"
...
function resolveForgeOSHome(): string {
	return process.env.FORGEOS_DIR?.trim() || join(homedir(), ".forgeos")
}
...
async function installSkillMarketplaceEntry(entry: MarketplaceEntry, args: string[]): Promise<MarketplaceInstallResult> {
	const command = "npx"
	const commandArgs = ["-y", "skills@latest", "add", ...args, "-g", "-a", "forgeos", "-y"]
...
function isGlobalForgeOSPath(filePath: string | undefined): boolean {
	if (!filePath || filePath.startsWith("remote:")) return false
	return [resolveForgeOSHome(), join(homedir(), ".agents", "skills")].some((root) => isPathWithin(root, filePath))
}
		...
		for (const pluginPath of discoverPluginModulePaths(root)) {
			entries.push(
				MarketplaceLocalInstalledEntry.create({
					...
					path: pluginPath,
					source: isGlobalForgeOSPath(pluginPath) ? "global" : "workspace",
	...
	const skillEntries = [
		...
		...refreshedSkills.localSkills.map((skill) =>
			MarketplaceLocalInstalledEntry.create({
				...
				description: skill.description,
				path: skill.path,
				source: isGlobalForgeOSPath(skill.path) ? "global" : "workspace",
				enabled: skill.enabled,
			}),
		),
	]
