...
async function activateBundle(
	...
	try {
		...
		// (built with FORGEOS_ROLLOUT_VARIANT). On fallback this runs in the legacy
		...
		console.error(
			"[forgeos-rollout] next bundle failed to activate, falling back to legacy:",
			error,
		);
...
function showNightlyBundleIndicator(
	...
	try {
		...
		item.text = bundle === "next" ? "ForgeOS: Next" : "ForgeOS: Legacy";
		const detail = fallbackFrom
			...
			: meta.override
				...
				: "rollout assignment";
		item.tooltip = `ForgeOS nightly A/B rollout: running the ${bundle === "next" ? "next (SDK)" : "legacy"} bundle (${detail}).`;
		item.show();
		context.subscriptions.push(item);
	} catch (error) {
		console.warn("[forgeos-rollout] could not show bundle indicator:", error);
	}
}
