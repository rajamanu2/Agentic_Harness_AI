...
// Publishes forgeos and all platform-specific binary packages to npm.
//
// Usage:
...
const wrapperPackageName = "forgeos";
...
const hostSdkPackages = [
	{ name: "@forgeos/sdk", directory: "sdk" },
	{ name: "@forgeos/core", directory: "core" },
	{ name: "@forgeos/agents", directory: "agents" },
	{ name: "@forgeos/llms", directory: "llms" },
	{ name: "@forgeos/shared", directory: "shared" },
] as const;
...
const platformTasks = Object.keys(binaries)
	.sort()
	.map(async (name) => {
		const dirName = name.replace("@forgeos/", "");
...
const wrapperPackageJson = {
	...
	version,
	description: description || "ForgeOS CLI",
