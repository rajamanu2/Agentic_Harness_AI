...
export function addRootOptions(cmd: Command): Command {
	return (
		cmd
			...
			.option("-P, --provider <id>", "Provider id (default: forgeos)")
			...
			.option("--config <path>", "Configuration directory (default: ~/.forgeos)")
			.option(
				"--data-dir <path>",
				"Use isolated local state at this directory path (default: ~/.forgeos/data)",
			)
			.option(
				"--hooks-dir <path>",
				"Directory path to additional hooks for runtime hook injection (default: ~/.forgeos/hooks)",
			)
			.option(
				"--worktree",
				"Auto-create a detached git worktree under ~/.forgeos/worktrees/ and run the task there",
			)
...
export function createProgram(): Command {
	const program = new Command("forgeos")
		.description("ForgeOS CLI - AI coding assistant in your terminal")
