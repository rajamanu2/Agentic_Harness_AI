...
const originalEnv = {
	FORGEOS_LOG_ENABLED: process.env.FORGEOS_LOG_ENABLED,
	FORGEOS_LOG_LEVEL: process.env.FORGEOS_LOG_LEVEL,
	FORGEOS_LOG_NAME: process.env.FORGEOS_LOG_NAME,
	FORGEOS_LOG_PATH: process.env.FORGEOS_LOG_PATH,
};
...
describe("desktop sidecar logging", () => {
	it("writes structured SDK logs to the configured file", () => {
		const directory = mkdtempSync(join(tmpdir(), "forgeos-code-logging-"));
		const destination = join(directory, "sidecar.log");
		process.env.FORGEOS_LOG_PATH = destination;
		process.env.FORGEOS_LOG_LEVEL = "debug";
		delete process.env.FORGEOS_LOG_ENABLED;
		...
		try {
			...
			expect(adapter.runtimeConfig.name).toBe("forgeos-code.sidecar");
		} finally {
	...
	it("warns once before falling back to stderr when the log file cannot open", () => {
		const directory = mkdtempSync(join(tmpdir(), "forgeos-code-fallback-"));
		process.env.FORGEOS_LOG_PATH = directory;
		delete process.env.FORGEOS_LOG_ENABLED;
	...
	it("rotates the active log before a write exceeds the size limit", () => {
		const directory = mkdtempSync(join(tmpdir(), "forgeos-code-rotation-"));
		const destination = join(directory, "sidecar.log");
		process.env.FORGEOS_LOG_PATH = destination;
		delete process.env.FORGEOS_LOG_ENABLED;
