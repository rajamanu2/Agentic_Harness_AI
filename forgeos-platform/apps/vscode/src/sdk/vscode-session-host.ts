// VscodeSessionHost — wraps ForgeOSCore with VSCode-specific customizations
//
// Uses ForgeOSCore.create() so the SDK owns session input normalization,
...
import {
	...
	ForgeOSCore,
	type ForgeOSCoreListHistoryOptions,
...
import { type AgentToolContext, type ToolApprovalRequest, type ToolApprovalResult, type ToolPolicy } from "@forgeos/shared"
...
export class VscodeSessionHost implements SdkSessionHost {
	...
	private readonly inner: ForgeOSCore

	private constructor(inner: ForgeOSCore) {
		this.inner = inner
	...
	static async create(options: VscodeSessionHostOptions): Promise<VscodeSessionHost> {
		...
		const inner = await ForgeOSCore.create({
			backendMode: "local",
			capabilities: {
			...
			prepare: async () => ({
				applyToStartSessionInput: async (input: ForgeOSCoreStartInput): Promise<ForgeOSCoreStartInput> => {
		...
		Logger.log("[VscodeSessionHost] Initialized with ForgeOSCore + VSCode extra tools")
		if (options.getTerminalManager) {
	...
	async start(input: ForgeOSCoreStartInput): Promise<StartSessionResult>
	async start(input: StartSessionInput | ForgeOSCoreStartInput): Promise<StartSessionResult> {
		return this.inner.start(input as ForgeOSCoreStartInput)
	}
	...
	async list(limit?: number, options: Omit<ForgeOSCoreListHistoryOptions, "limit"> = {}): Promise<SessionHistoryRecord[]> {
		return this.inner.list(limit, options)
	}

	async listHistory(options: ForgeOSCoreListHistoryOptions = {}): Promise<SessionHistoryRecord[]> {
