...
title: "Skills"
sidebarTitle: "Skills"
description: "Modular instruction sets that extend ForgeOS's capabilities for specific tasks."
---

Skills are modular instruction sets that extend ForgeOS's capabilities for specific tasks. Each skill packages detailed guidance, processes, and optional resources that ForgeOS loads only when relevant to your request.

Install multiple skills and ForgeOS only loads what it needs. A deployment skill stays dormant until you ask about deploying. Unlike [rules](/customization/forgeos-rules) (which are always active), skills load on-demand so they don't consume context when you're working on something unrelated.

<Note>
Manage skills from the Skills menu: click the scale icon at the bottom of the ForgeOS panel, to the left of the model selector, then switch to the **Skills** tab.
</Note>
...
When you send a message, ForgeOS sees a list of available skills with their descriptions. If your request matches a skill's description, ForgeOS activates it using the `use_skill` tool, which loads the full instructions from SKILL.md.
...
3. ForgeOS triggers that skill and loads its `SKILL.md` instructions.
...
Detailed instructions for ForgeOS to follow when this skill is activated.
...
- `description` tells ForgeOS when to use this skill (max 1024 characters)

## Creating a Skill

<Steps>
  <Step title="Open the Skills menu">
    Click the scale icon at the bottom of the ForgeOS panel, to the left of the model selector. Switch to the Skills tab.
  </Step>
  <Step title="Create a new skill">
    Click "New skill..." and enter a name for your skill (e.g., `aws-deploy`). ForgeOS creates a skill directory with a template `SKILL.md` file.
  </Step>
...
You can also create skills manually by creating the directory structure in your file system. Place skill directories in `.forgeos/skills/` (workspace) or `~/.forgeos/skills/` (global) and ForgeOS will detect them automatically.

Put the important information first in your SKILL.md. ForgeOS reads the file sequentially, so front-load the common cases. Use clear section headers like "## Error Handling" or "## Configuration" so ForgeOS can scan for relevant sections.
...
The description determines when ForgeOS activates the skill. A vague description means the skill won't trigger when you expect it to.
...
Keep SKILL.md under 5k tokens. If your skill needs more content, split it into separate files in a `docs/` directory and reference them from the main instructions. ForgeOS loads referenced files only when needed.
...
Project skills:
- `.forgeos/skills/` (recommended)
- `.forgeosrules/skills/`
- `.claude/skills/`

Global skills:
- `~/.forgeos/skills/` (macOS/Linux)
- `C:\Users\USERNAME\.forgeos\skills\` (Windows)

When a global skill and project skill have the same name, the global skill takes precedence. This lets you keep general-purpose skills globally while using project-specific skills in `.forgeos/skills/` so the whole team can use them.

Version control your project skills by committing `.forgeos/skills/`. Your team can share, review, and improve them together.

## Bundling Supporting Files

Skills can include additional files that ForgeOS accesses only when needed.
...
A deployment skill might have `docs/aws.md`, `docs/gcp.md`, and `docs/azure.md`. ForgeOS loads only the relevant platform guide based on your request.
...
A project setup skill could include `templates/dockerfile`, `templates/docker-compose.yml`, and `templates/.env.example` that ForgeOS customizes for each new project.
...
ForgeOS reads documentation files using `read_file` when the instructions reference them. Scripts can be executed directly, and only the script's output enters the context window.
...
Skills transform ForgeOS from a general-purpose assistant into a specialist that knows your domain. Start with one skill for a task you repeat often, test it, and iterate on the description until it triggers reliably.
