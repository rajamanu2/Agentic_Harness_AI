...
export function activate(context: vscode.ExtensionContext): void {
	...
	context.subscriptions.push(
		...
		vscode.window.registerWebviewViewProvider(
			"forgeosVscode.chatView",
			sidebarProvider,
			{ webviewOptions: { retainContextWhenHidden: true } },
		),
		vscode.commands.registerCommand("forgeosVscode.openChat", () => {
...
class ForgeOSChatViewProvider implements vscode.WebviewViewProvider {
	constructor(
...
type StartConfig = {
	...
	logger: BasicLogger;
	extensionContext?: import("@forgeos/shared").ExtensionContext;
...
class CoreChatWebviewController implements vscode.Disposable {
	...
	private host: ForgeOSCore | undefined;
	...
	constructor(
		...
		if (sharedTelemetry) {
			...
			const { telemetry } = createConfiguredTelemetryService(
				createForgeOSTelemetryServiceConfig({
					metadata: createForgeOSTelemetryServiceMetadata({
						extension_version: version,
						forgeos_type: displayName,
						platform: vscode.env.appName,
	...
	private async initialize(): Promise<void> {
		try {
			...
			await this.post({ type: "status", text: "ForgeOS is Ready" });
...
<body>
	...
	private buildExtensionContext(workspaceRoot: string, cwd: string) {
		return {
			client: { name: "forgeos-vscode", version },
			workspace: {
	...
	private async buildStartConfigFromSession(
		session: NonNullable<Awaited<ReturnType<ForgeOSCore["get"]>>>,
	): Promise<StartConfig> {
		const mode: "act" | "plan" =
	...
	private async getSessionHost(): Promise<ForgeOSCore> {
		if (!this.host) {
			await this.ensureHub();
			const defaults = this.resolveWorkspaceDefaults();
			this.host = await ForgeOSCore.create({
				backendMode: "hub",
