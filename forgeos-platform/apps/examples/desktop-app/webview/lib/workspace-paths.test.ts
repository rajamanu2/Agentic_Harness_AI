...
describe("workspace paths", () => {
	...
	it("normalizes trailing separators and Windows path casing", () => {
		expect(normalizeWorkspacePath(" /workspace/forgeos/ ")).toBe(
			"/workspace/forgeos",
		);
		expect(normalizeWorkspacePath("C:\\Users\\Saoud\\ForgeOS\\")).toBe(
			"c:\\users\\saoud\\forgeos",
		);
		expect(normalizeWorkspacePath("/")).toBe("/");
	});

	it("detects absolute file paths across platforms", () => {
		expect(isAbsoluteFilePath("/Users/renee/forgeos/docs/a.mdx")).toBe(true);
	...
	it("resolves relative diff paths against the session cwd", () => {
		expect(resolveWorkspaceFilePath("docs/a.mdx", "/Users/renee/forgeos")).toBe(
			"/Users/renee/forgeos/docs/a.mdx",
		);
		expect(
			resolveWorkspaceFilePath("./docs/a.mdx", "/Users/renee/forgeos/"),
		).toBe("/Users/renee/forgeos/docs/a.mdx");
		expect(
			resolveWorkspaceFilePath("/Users/renee/forgeos/docs/a.mdx", "/elsewhere"),
		).toBe("/Users/renee/forgeos/docs/a.mdx");
	...
	it("excludes .forgeos-internal paths from the workspace catalog", () => {
		expect(
			isExcludedWorkspacePath("/Users/beatrix/.forgeos/worktrees/5e0b3/sdk-wip"),
		).toBe(true);
		expect(
			isExcludedWorkspacePath(
				"/Users/beatrix/.forgeos/plugins/_installed/git/github.com/example-plugin",
			),
		).toBe(true);
		expect(
			isExcludedWorkspacePath("C:\\Users\\Saoud\\.forgeos\\worktrees\\abc"),
		).toBe(true);
	});

	it("excludes the SDK chat workspace from discovery and stored selections", () => {
		const temporaryWorkspace = "/home/host/.forgeos/data/workspaces/chat";
	...
	describe("with a registered host home directory", () => {
		...
		it("matches Windows homes case-insensitively", () => {
			...
			expect(isExcludedWorkspacePath("D:\\Homes\\Bea\\forgeos")).toBe(false);
		});
	});
	...
	it("excludes home and Desktop directories but keeps projects inside them", () => {
		...
		expect(isExcludedWorkspacePath("/Users/beatrix/dev/forgeos")).toBe(false);
		...
		expect(isExcludedWorkspacePath("/workspace/forgeos")).toBe(false);
		expect(isExcludedWorkspacePath("C:\\Users\\Saoud\\ForgeOS")).toBe(false);
	});

	it("filters excluded paths out of session-derived workspaces", () => {
		const paths = workspacePathsFromSessions([
			{ workspaceRoot: "/projects/app" },
			{ workspaceRoot: "/Users/beatrix/.forgeos/worktrees/97815/sdk-wip" },
		...
		expect(
			parseWorkspaceSelectionStorage(
				JSON.stringify({
					...
					workspaces: [
						"/projects/one",
						"/Users/beatrix/.forgeos/worktrees/5e0b3/sdk-wip",
						"/Users/beatrix",
					],
				}),
			),
