...
export const FORGEOS_FREE_MODEL_ID_PREFIX = "forgeos-free/"
/** Error code stamped by the host when it detects a retired free model (see message-translator). */
export const FORGEOS_FREE_PROMOTION_ENDED_ERROR_CODE = "forgeos_free_promotion_ended"

/**
 * Detects a request against a retired free model: once a promotion ends the
 * forgeos-free/ model is removed from the catalog and the backend answers "model
 * not found". The modelId gate keeps ordinary model-not-found errors on their
 * generic path. Mirrors the CLI's detection in apps/cli/src/utils/forgeos-pass-errors.ts.
 */
export function isForgeOSFreePromotionEndedMessage(message: string, modelId?: string): boolean {
	if (!modelId?.toLowerCase().startsWith(FORGEOS_FREE_MODEL_ID_PREFIX)) {
		return false
	}
...
export class ForgeOSError extends Error {
	...
	/**
	 * Parses a stringified error into a ForgeOSError instance.
	 */
	static parse(errorStr?: string, modelId?: string): ForgeOSError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return ForgeOSError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a ForgeOSError instance.
	 * Always returns a ForgeOSError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): ForgeOSError {
		try {
			// If already a ForgeOSError, return it directly to prevent infinite recursion
			if (error instanceof ForgeOSError) {
				return error
			}
			return new ForgeOSError(JSON.parse(error), modelId, providerId)
		} catch {
			return new ForgeOSError(error, modelId, providerId)
		}
	}

	public isErrorType(type: ForgeOSErrorType): boolean {
		return ForgeOSError.getErrorType(this) === type
	}
	 ...
	 */
	static getErrorType(err: ForgeOSError): ForgeOSErrorType | undefined {
		...
		if (code === "insufficient_credits" && typeof details?.current_balance === "number") {
			return ForgeOSErrorType.Balance
		}
		...
		if (code === "SPEND_LIMIT_EXCEEDED" || details?.code === "SPEND_LIMIT_EXCEEDED") {
			return ForgeOSErrorType.SpendLimit
		}

		if (
			rawMessage === getForgeOSOrgIndividualInferenceSubscriptionMessage() ||
			(detailMessage ? isForgeOSOrgIndividualInferenceSubscriptionMessage(detailMessage) : false) ||
			(rawMessage ? isForgeOSOrgIndividualInferenceSubscriptionMessage(rawMessage) : false)
		) {
			return ForgeOSErrorType.OrgForgeOSPassRestriction
		}

		if (
			(detailMessage ? isForgeOSNotSubscribedMessage(detailMessage) : false) ||
			(rawMessage ? isForgeOSNotSubscribedMessage(rawMessage) : false)
		) {
			return ForgeOSErrorType.Entitlement
		}

		if (
			(detailMessage ? isForgeOSFreeModelLimitMessage(detailMessage) : false) ||
			(rawMessage ? isForgeOSFreeModelLimitMessage(rawMessage) : false)
		) {
			return ForgeOSErrorType.ForgeOSFreeModelLimit
		}

		if (
			(detailMessage ? isForgeOSPassLimitMessage(detailMessage) : false) ||
			(rawMessage ? isForgeOSPassLimitMessage(rawMessage) : false)
		) {
			return ForgeOSErrorType.ForgeOSPassLimit
		}
		...
		if (
			...
			details?.code === FORGEOS_FREE_PROMOTION_ENDED_ERROR_CODE ||
			(detailMessage ? isForgeOSFreePromotionEndedMessage(detailMessage, err.modelId) : false) ||
			(rawMessage ? isForgeOSFreePromotionEndedMessage(rawMessage, err.modelId) : false)
		) {
			return ForgeOSErrorType.ForgeOSFreePromotionEnded
		}
		...
		if (code === "ERR_BAD_REQUEST" || err instanceof AuthInvalidTokenError || isAuthStatus) {
			return ForgeOSErrorType.Auth
		}

		if (code === "INFERENCE_CAP_ERROR") {
			return ForgeOSErrorType.QuotaExceeded
		}
		...
		if (message) {
			...
			if (message?.includes(FORGEOS_ACCOUNT_AUTH_ERROR_MESSAGE) || authErrorRegex.some((regex) => regex.test(message))) {
				return ForgeOSErrorType.Auth
			}
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return ForgeOSErrorType.RateLimit
			}
		}
...
class AuthInvalidTokenError extends Error {
	constructor(message: string) {
		super(message)
		this.name = ForgeOSErrorType.Auth
	}
}
