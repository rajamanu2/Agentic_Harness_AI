import type { MessageWithMetadata } from "@forgeos/llms";
import type {
	AgentConfig,
...
export type { ForgeOSCoreSettingsApi } from "../settings";
...
export interface ForgeOSCoreAutomationOptions {
...
export type ForgeOSAutomationSpec = CronSpecRecord;
export type ForgeOSAutomationRun = CronRunRecord;
export type ForgeOSAutomationEventLog = CronEventLogRecord;
export type ForgeOSAutomationEventSuppression = CronEventSuppression;
export type ForgeOSAutomationRunStatus =
	| "queued"
	| "running"
	| "done"
	| "failed"
...
export interface ForgeOSAutomationListSpecsOptions {
...
export interface ForgeOSAutomationListRunsOptions {
	specId?: string;
	status?: ForgeOSAutomationRunStatus | ForgeOSAutomationRunStatus[];
	limit?: number;
}

export interface ForgeOSAutomationListEventsOptions {
	eventType?: string;
	source?: string;
	processingStatus?:
		| "received"
...
export interface ForgeOSAutomationEventIngressResult {
	event: ForgeOSAutomationEventLog;
	duplicate: boolean;
	matchedSpecIds: string[];
	queuedRuns: ForgeOSAutomationRun[];
	suppressions: ForgeOSAutomationEventSuppression[];
}

export interface ForgeOSCoreAutomationApi {
	start(): Promise<void>;
	stop(): Promise<void>;
	...
	listEvents(
		options?: ForgeOSAutomationListEventsOptions,
	): ForgeOSAutomationEventLog[];
	getEvent(eventId: string): ForgeOSAutomationEventLog | undefined;
	listSpecs(options?: ForgeOSAutomationListSpecsOptions): ForgeOSAutomationSpec[];
	listRuns(options?: ForgeOSAutomationListRunsOptions): ForgeOSAutomationRun[];
}

export type ForgeOSCoreListHistoryOptions = SessionHistoryListOptions;

export interface ForgeOSCoreStartInput
	extends Omit<StartSessionInput, "config" | "localRuntime"> {
	config: ForgeOSCoreStartConfig;
...
export interface ForgeOSCoreOptions {
	/**
	...
	/**
	 * Feature flags service for this ForgeOSCore instance.
	...
	/**
	 * Enables file-based and event-driven automation through this ForgeOSCore
	 * instance. When configured, callers use `forgeos.automation.*` instead of
	 * constructing cron services directly.
	 */
	automation?: boolean | ForgeOSCoreAutomationOptions;
	/**
	 ...
	 *     `@forgeos/core/hub` for the scheduler
	 */
	fetch?: typeof fetch;
	/**
...
export interface StartSessionBootstrap {
	applyToStartSessionInput(
		input: ForgeOSCoreStartInput,
	): Promise<ForgeOSCoreStartInput> | ForgeOSCoreStartInput;
