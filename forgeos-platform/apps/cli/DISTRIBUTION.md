...
The ForgeOS CLI (`forgeos`) is distributed as compiled binaries via npm. Users run `npm i -g forgeos` and get a working `forgeos` command without needing Bun, Zig, or any other runtime installed.
...
| Package | Description |
|---|---|
| `@forgeos/cli-darwin-arm64` | macOS Apple Silicon binary |
| `@forgeos/cli-darwin-x64` | macOS Intel binary |
| `@forgeos/cli-linux-arm64` | Linux ARM binary |
| `@forgeos/cli-linux-x64` | Linux x64 binary |
| `@forgeos/cli-windows-x64` | Windows x64 binary |
| `@forgeos/cli-windows-arm64` | Windows ARM binary |
| `forgeos` | Wrapper package (pulls the right binary via `optionalDependencies`) |

Each platform package contains a compiled binary and a minimal `package.json` with `os` and `cpu` fields:

```json
{
  "name": "@forgeos/cli-darwin-arm64",
  "version": "0.1.0",
  "os": ["darwin"],
  "cpu": ["arm64"],
  "bin": {
...
The `forgeos` wrapper package contains no binary -- just the resolver script, postinstall script, and `optionalDependencies` pointing to all platform packages:
...
```json
{
  ...
  "optionalDependencies": {
    "@forgeos/cli-darwin-arm64": "0.1.0",
    "@forgeos/cli-darwin-x64": "0.1.0",
    "@forgeos/cli-linux-arm64": "0.1.0",
    "@forgeos/cli-linux-x64": "0.1.0",
    "@forgeos/cli-windows-x64": "0.1.0",
    "@forgeos/cli-windows-arm64": "0.1.0"
  }
}
```

After installing, users run `forgeos`:

```bash
npm i -g forgeos

forgeos              # interactive mode
forgeos "prompt"     # single-prompt mode
forgeos auth         # authenticate a provider
```
...
2. Choose the release version. Because this publishes over the existing `forgeos` package, the version must be greater than the current published `forgeos` version. The handoff release is `3.0.0`.
...
The release helper checks the working tree, verifies the tag points at `HEAD` locally and on `origin`, runs tests, builds all platform packages, and publishes the platform packages plus the generated `forgeos` wrapper package to npm. The package version and tag must match.

By default, `bun release cli` publishes with the npm dist-tag `latest` (what users get with `npm i -g forgeos`). To publish under a different dist-tag like `next`, pass `--tag`:
...
CI publishing uses npm trusted publishing. Configure npm trusted publishers for the `forgeos` wrapper package and every platform package before relying on the workflow.

## How It Works Under the Hood

```
User runs: npm i -g forgeos
  |
  v
npm installs forgeos (wrapper package)
  + optionalDependencies (only the matching platform gets installed):
    - @forgeos/cli-darwin-arm64
    - @forgeos/cli-darwin-x64
    - @forgeos/cli-linux-arm64
    - @forgeos/cli-linux-x64
    - @forgeos/cli-windows-x64
    - @forgeos/cli-windows-arm64
  |
  v
...
User runs: forgeos
  |
  v
bin/forgeos (Node.js resolver) executes:
  1. Check FORGEOS_BIN_PATH env var override
  2. Check cached binary at bin/.forgeos
...
apps/cli/
  bin/
    forgeos                   # Node.js resolver script (npm entry point)
  script/
...
2. Builds SDK packages (`bun run build:sdk`) and the CLI JS bundle (`bun -F @forgeos/cli build`)
3. For each target platform:
   ...
   - Runs a smoke test on the current platform's binary (`forgeos --version`)
...
2. Publishes all 6 platform packages in parallel (`@forgeos/cli-darwin-arm64`, etc.)
3. Generates a clean main package (`forgeos`) with:
   - `bin.forgeos` pointing to the resolver script
   - `postinstall` running the binary caching script
   - `optionalDependencies` listing all platform packages
4. Publishes the generated `forgeos` wrapper package

Platform packages must be published before the generated `forgeos` wrapper package because npm validates that `optionalDependencies` exist.

The publish script generates a separate `package.json` for the published `forgeos` wrapper package. The development `package.json` (with `bin` pointing to `src/index.ts` for `bun link`) is never published directly.

## Binary Resolver (`bin/forgeos`)

A Node.js script that serves as the entry point when users run `forgeos`. It finds and executes the correct platform-specific binary.
...
Resolution chain:
1. `FORGEOS_BIN_PATH` env var (for development or custom deployments)
2. `bin/.forgeos` cached hard link (created by postinstall for fast startup)
3. Walk up `node_modules` from the script directory to find the platform package

## Postinstall (`script/postinstall.mjs`)

Runs after `npm install forgeos`. Creates a hard link from the platform binary to `bin/.forgeos` for fast startup on subsequent runs. Falls back to file copy if hard linking fails (NFS, cross-device, network-mounted filesystems).
...
| `bun link` + `forgeos` | src/index.ts | Bun (source) | Yes |
| `npm i -g forgeos` | bin/forgeos resolver | Compiled binary | No |
...
Platform packages are published under the `@forgeos` scope. The generated wrapper package is published as `forgeos`, so npm trusted publishing must be configured for all 7 package names.
