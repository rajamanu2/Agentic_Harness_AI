...
async function runPluginMcpOAuthFollowup(
	...
	if (!isInteractivePluginInstall(options)) {
		...
		options.io?.writeln(
			'Run "forgeos mcp" and choose "Authorize OAuth" to authorize them.',
		);
		return;
	}
	...
	for (const candidate of selected) {
		try {
			...
			options.io?.writeErr(
				`Warning: failed to authorize MCP server ${candidate.name}: ${message}. Run "forgeos mcp" and choose "Authorize OAuth" to retry.`,
			);
		}
	}
}
