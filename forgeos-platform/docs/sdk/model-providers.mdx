...
The `@forgeos/llms` package provides provider registries, model catalogs, handlers, and a gateway. `@forgeos/agents` and `@forgeos/core` use this layer internally.
...
```typescript
import { DefaultGateway } from "@forgeos/llms"
