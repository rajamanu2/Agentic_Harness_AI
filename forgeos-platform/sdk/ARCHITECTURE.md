# ForgeOS SDK Architecture

This document is the architecture source of truth for the ForgeOS SDK repository. It describes how the system is organized, how components interact, and the design principles that guide development decisions.

**Who should read this?**
- SDK contributors working across multiple packages
- Developers building integrations or host applications using `@forgeos/core`
...
```mermaid
flowchart LR
  shared["@forgeos/shared"]
  llms["@forgeos/llms"]
  agents["@forgeos/agents"]
  core["@forgeos/core"]
  apps["Host Apps"]
...
- hub discovery, the detached hub daemon, and the `@forgeos/core/hub/daemon-entry` subpath
- host-side hub client adapters (`NodeHubClient`, `HubSessionClient`, `HubUIClient`, `connectToHub`) exported from `@forgeos/core/hub`
...
1. Host constructs a `RuntimeHost` through `@forgeos/core`.
2. `@forgeos/core` selects `LocalRuntimeHost` through `packages/core/src/runtime/host.ts`.
...
4. `@forgeos/core` prepares a local bootstrap artifact from `localRuntime`, then builds the runtime from it.
5. `@forgeos/core` creates an `Agent` from `@forgeos/agents`.
6. `@forgeos/agents` runs the loop using `@forgeos/llms` handlers.
7. `@forgeos/core` persists state, artifacts, and metadata.
...
ForgeOS's `attempt_completion`) is observed. `shutdownSession(...)`
...
1. Host constructs a `RuntimeHost` through `@forgeos/core`.
2. `@forgeos/core` selects `HubRuntimeHost` or `RemoteRuntimeHost` through `packages/core/src/runtime/host.ts`.
3. When no compatible local hub is already discovered, `@forgeos/core` can spawn a detached hub daemon and reconnect through discovery.
...
5. The hub-hosted runtime executes the agent loop using `@forgeos/agents` and `@forgeos/llms`.
6. `@forgeos/core` hub services broker sessions, events, approvals, schedules, and client-owned runtime capabilities such as session-local tool executors.
...
8. Hub client adapters exported from `@forgeos/core/hub` (`NodeHubClient`, `HubSessionClient`, `HubUIClient`, `connectToHub`) translate command/reply and event streams into host-facing APIs.
...
1. `@forgeos/llms` validates provider output once and creates canonical
...
2. Provider model tools are adapters, not raw AI SDK tools. An adapter owns its
   ...
   model-tool metadata.
3. `@forgeos/agents` appends media events at their exact stream position in the
...
4. `@forgeos/core` projects live media as `content_end(media)`. The hub publishes
   `assistant.media`, preserving the same media ID, and clients deduplicate live
   and hydrated content by that ID.
5. Web clients share `GeneratedMediaContent` from `@forgeos/ui` for image, audio,
...
`~/.forgeos/data/workspaces/chat`). The chat workspace is seeded with an
...
3. Hub-required flows such as `forgeos hub`, schedules, connectors, and `--zen` may still call the explicit ensure path because those commands require a live hub before proceeding.
...
1. `@forgeos/shared/db` owns the low-level SQLite connector store and the one-time legacy JSON import.
...
3. `@forgeos/core` owns connector autostart persistence and reconnect orchestration. The detached hub daemon is the sole startup reconnect owner, preventing dashboard startup from racing it and launching duplicate processes.
...
2. `@forgeos/shared/remote-config` caches the bundle when configured.
3. Shared remote-config materializes managed rules/workflows/skills under workspace-local `.forgeos/<plugin>/`.
4. Shared remote-config derives generic OpenTelemetry config and session blob upload metadata from the bundle.
5. `@forgeos/core` exposes the app-facing integration wrapper that applies extensions, telemetry, and session metadata to `StartSessionInput`.
6. `@forgeos/core` consumes the prepared local overrides during local bootstrap.
...
- `ForgeOSCore` delegates uniformly to `RuntimeHost` and does not branch on local vs hub behavior
- transport-specific translation belongs inside concrete hosts, not in top-level orchestration
- `RuntimeHost` inputs stay transport-safe, while `ForgeOSCore.start(...)` is the app-facing facade that normalizes broad local config before delegation
...
- pending prompt list/update/delete are exposed through the grouped
  `ForgeOSCore.pendingPrompts` service. Usage summary lookup and active-session
  model switching are also service-style capabilities exposed through
  `ForgeOSCore` when the concrete transport implements them. These service APIs
...
`ForgeOSCore.create(...)` exposes a generic `prepare(input)` hook.
...
Cross-package logging uses a small injected interface exported from `@forgeos/shared`:
...
- **`TelemetryLoggerSink` (`@forgeos/core`)** — an **`ITelemetryAdapter`** that mirrors telemetry events and metrics into a `BasicLogger`. It is a telemetry sink, not a host logging implementation.
...
- `@forgeos/agents` owns the generic turn-preparation seam:
...
- `@forgeos/core` owns compaction policy:
...
`FORGEOS_PLUGIN_IDLE_TIMEOUT_MS`), and the next plugin call starts and
...
Do not move these concerns into `@forgeos/agents`:
...
Do not make `@forgeos/core` organization- or provider-specific.

If a capability is truly generic and app-facing, add a generic core seam. Reusable remote-config parsing, materialization, and upload primitives belong in `@forgeos/shared/remote-config`.
...
## File-Based And Event-Driven Automation (`ForgeOSCore` / `CronService`)

`@forgeos/core` ships a file-based automation subsystem under
`packages/core/src/cron/`. It lets operators author recurring and one-off
tasks as Markdown files under global `~/.forgeos/cron/` by default, and
event-driven tasks as `events/*.event.md` specs. All trigger kinds run
through the same durable queue and runtime handlers. `ForgeOSCore` exposes the
SDK-facing `forgeos.automation.*` entry points; `CronService` is the internal
...
1. **Spec parser** (`cron/specs/cron-spec-parser.ts`): parses YAML frontmatter + body
   ...
   Types live in `@forgeos/shared` under `src/cron/cron-spec-types.ts`
...
2. **Store** (`cron/store/sqlite-cron-store.ts`): owns `cron.db` at
   `resolveCronDbPath()` (default `.forgeos/data/db/cron.db`). Schema is
...
3. **Reconciler** (`cron/specs/cron-reconciler.ts`): scans the configured cron specs
   directory (global `~/.forgeos/cron/` by default, or workspace-scoped when
...
8. **Reports** (`cron/reports/cron-report-writer.ts`): writes
   `.forgeos/cron/reports/<run-id>.md` with run frontmatter plus
...
9. **Service** (`cron/service/cron-service.ts`): orchestrates all of the above.
   `ForgeOSCore.create({ automation })` owns the SDK-facing lifecycle and exposes
   `forgeos.automation.*` methods. Hub-side callers can submit normalized events
   through the `cron.event.ingest` command.

The detached hub daemon passes its workspace root as `cronOptions`, so
normal CLI/hub startup watches `${workspaceRoot}/.forgeos/cron/` without a
...
- **`ForgeOSCore`** — `packages/core/src/index.ts` — the main SDK orchestrator
...
- `@forgeos/shared` — shared types, contracts, and low-level utilities
- `@forgeos/llms` — provider integrations and model manifests
- `@forgeos/agents` — the agent loop and tool orchestration
- `@forgeos/core` — the main SDK with session management, hub, and configuration
