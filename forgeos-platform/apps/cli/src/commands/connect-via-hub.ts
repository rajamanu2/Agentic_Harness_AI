 ...
 *
 * Deliberately does not start a hub: this exists for diagnostics, and `forgeos
...
export async function startConnectorViaHub(input: {
	...
	try {
		...
		input.io.writeln(
			"[connect] the hub will restart it if it exits; use `forgeos connect --stop` to retire it",
		);
		return { delegated: true, exitCode: 0 };
	} catch (error) {
		return {
