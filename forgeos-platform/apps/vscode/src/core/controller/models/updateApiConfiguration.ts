...
import { UpdateApiConfigurationRequestNew } from "@/shared/proto/index.forgeos"
...
export async function updateApiConfiguration(controller: Controller, request: UpdateApiConfigurationRequestNew): Promise<Empty> {
	try {
		...
		if (Object.keys(options).length > 0) {
			...
			clearOrganizationForForgeOSPassProviderSelection(controller, controller.stateManager.getApiConfiguration())
		}
