...
import type { UserInfo } from "@shared/proto/forgeos/account"
import { EmptyRequest } from "@shared/proto/forgeos/common"
import type { OpenRouterCompatibleModelInfo, ProviderModelsResponse } from "@shared/proto/forgeos/models"
import { OnboardingModelGroup, type TerminalProfile } from "@shared/proto/forgeos/state"
import { convertProtoToForgeOSMessage } from "@shared/proto-conversions/forgeos-message"
...
export interface ExtensionStateContextType extends ExtensionState {
	...
	setGlobalForgeOSRulesToggles: (toggles: Record<string, boolean>) => void
	setLocalForgeOSRulesToggles: (toggles: Record<string, boolean>) => void
...
export const ExtensionStateContextProvider: React.FC<{
	...
	// Convergent-replica state for forgeosMessages. The partial-message stream and the full state
					...
					try {
						...
						setState((prevState) => {
							...
							replicaRef.current = reducerApplyStateSnapshot(
								replicaRef.current,
								stateData.forgeosMessages ?? [],
								stateData.epoch ?? 0,
								stateData.stateVersion ?? 0,
								stateData.turnState,
							)
							stateData.forgeosMessages = replicaRef.current.messages
		...
		partialMessageUnsubscribeRef.current = UiServiceClient.subscribeToPartialMessage(EmptyRequest.create({}), {
			onResponse: (protoMessage: any) => {
				try {
					...
					const partialMessage = convertProtoToForgeOSMessage(protoMessage)
					setState((prevState) => {
						...
						return { ...prevState, forgeosMessages: replicaRef.current.messages }
					})
				} catch (error) {
	...
	const contextValue: ExtensionStateContextType = {
		...
		showWorktrees,
		showAnnouncement,
		globalForgeOSRulesToggles: state.globalForgeOSRulesToggles || {},
		localForgeOSRulesToggles: state.localForgeOSRulesToggles || {},
		...
		setShowMcp,
		closeMcpView,
		setGlobalForgeOSRulesToggles: (toggles) =>
			setState((prevState) => ({
				...prevState,
				globalForgeOSRulesToggles: toggles,
			})),
		setLocalForgeOSRulesToggles: (toggles) =>
			setState((prevState) => ({
				...prevState,
				localForgeOSRulesToggles: toggles,
			})),
