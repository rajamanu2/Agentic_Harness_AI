...
title: "Tools"
sidebarTitle: "Tools"
description: "Reference for ForgeOS's built-in tool system, aligned with SDK tool architecture."
---

ForgeOS tools are executable functions the model can call while working. The model decides which tool to call, ForgeOS runs it, then returns results back to the model.

## Built-In Tools (ForgeOSCore)

When running through `ForgeOSCore`, these built-ins are available:
...
<Note>
`Agent` from `@forgeos/agents` does not include built-ins by default. You provide tools explicitly.
</Note>
...
ForgeOS can run tools with approval or auto-approval, depending on settings/policies. Typical patterns:
...
ForgeOS can also call tools discovered from MCP servers configured in `.forgeos/mcp.json`.

MCP tools are loaded alongside built-ins, so ForgeOS can use both local tools and external integrations in one task.
...
Some older docs/examples reference XML-style names like `read_file`, `replace_in_file`, or `execute_command`. Current SDK/ForgeOSCore runtime uses the built-in tool names listed above (`read_files`, `apply_patch`, `bash`, etc.).
