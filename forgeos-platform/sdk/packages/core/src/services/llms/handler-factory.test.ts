import type { AgentConfig, AgentModel, ITelemetryService } from "@forgeos/shared";
...
describe("createAgentModelFromConfig", () => {
	...
	it("forwards SAP AI Core settings as gateway provider options", async () => {
		...
		const { createSapAiCoreProviderModule } = await import(
			// biome-ignore lint/style/noRestrictedImports: test asserts internal SAP provider module behavior not exposed via @forgeos/llms entrypoint
