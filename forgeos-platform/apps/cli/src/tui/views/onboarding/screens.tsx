...
function getForgeOSPassSubscriptionOptionId(index: number): string {
	return `forgeos-pass-subscription-option-${index}`;
}
...
export function OnboardingForgeOSModelScreen(props: {
	...
	return (
		...
		>
			...
			<ForgeOSModelPicker
				entries={props.forgeosEntries}
				selected={props.forgeosModelSelected}
...
export function OnboardingForgeOSPassSubscriptionScreen(props: {
	compact: boolean;
	contentWidth: number;
	...
	mouse: MouseTrackerState;
	openStatus: string;
	options: ForgeOSPassSubscriptionOption[];
	planFeatures: string[];
	selected: number;
	status: ForgeOSPassSubscriptionStatus;
	...
	useEffect(() => {
		...
		const scrollSelectedOptionIntoView = () => {
			scrollRef.current?.scrollChildIntoView(
				getForgeOSPassSubscriptionOptionId(props.selected),
			);
		};
			...
			>
				...
				>
					<box flexDirection="column" width="100%" flexShrink={0}>
						...
						>
							{isSubscribed
								? "ForgeOSPass subscription active"
								: "ForgeOSPass subscription required"}
						</text>

						{isLoading ? (
							<box flexDirection="row" gap={1} flexShrink={0}>
								<spinner name="dots" color="gray" />
								<text fg="gray">Checking your ForgeOSPass subscription...</text>
							</box>
						) : isSubscribed ? (
							<text fg={defaultFg} selectable flexShrink={0}>
								Current plan: {props.currentPlanName || "ForgeOSPass"}
							</text>
						) : isError ? (
							<text
								fg={defaultFg}
								selectable
								flexShrink={0}
								content="Could not verify your ForgeOSPass subscription. Re-check before choosing a ForgeOSPass model."
							/>
						) : (
							<text
								fg={defaultFg}
						...
						{!isSubscribed && (
							<box flexDirection="column" marginTop={1} flexShrink={0}>
								{props.options.map((option, i) => {
									...
									return (
										<box
											id={getForgeOSPassSubscriptionOptionId(i)}
											key={option.value}
											paddingX={1}
											flexDirection="row"
											gap={1}
...
export function OnboardingMainMenuScreen(props: {
	...
	return (
		...
		>
			...
			>
				<text fg={defaultFg}>
					<strong>Welcome to ForgeOS</strong>
				</text>
				<text fg="gray" marginTop={1}>
