...
- Added a dedicated ForgeOS provider for the ForgeOS gateway, replacing the generic OpenAI-compatible path. Extended thinking budgets and other gateway options now reach the wire for both `forgeos` and `forgeos-pass`, which had silently stopped applying to `forgeos-pass`
- Fixed two ForgeOS installations on different builds shutting each other's Hub daemon down in a loop, killing every live session with an abnormal socket close. Build identity is now compared through a total order, so at most one side of a pair can ever decide to retire the other
...
- `forgeos doctor fix` now reports honestly: processes that survived a kill are separated from ones that appeared while the fix ran, a live parent respawning a daemon is named, and a startup lock held by a running process is reported as held rather than leaked
...
- Involuntary ForgeOS logouts (a rejected refresh token) are now reported instead of credentials being cleared silently; a transient network failure refreshing the stored session on startup no longer books as a logout
...
- Refreshed the model catalog from models.dev, and surfaced `meta/muse-spark-1.2-contributor` for the ForgeOS provider
...
- Checkpoint restore is now a true workspace rewind: files ForgeOS created during a task are captured in the snapshot and restored to their checkpoint-time content, and files created after the checkpoint are removed. `.gitignore`d paths (build output, `node_modules`, `.env`) are left untouched, and a pre-restore recovery snapshot can roll the whole operation back. Checkpoints taken before this change keep the old conservative behavior of never touching untracked files
- Migrated users whose stored ForgeOS model id isn't in the runtime catalog now fall back to the default ForgeOS model instead of carrying an unknown id into every inference request
...
- `fetchForgeOSRecommendedModels` now returns display-ready model names, resolved through the model catalog (including Vercel and OpenRouter id aliases) under a single shared timeout budget, so hosts no longer have to map ids to names themselves
- ForgeOS free models now resolve their OpenRouter display names in the catalog
...
- The per-server `timeout` in `forgeos_mcp_settings.json` is now honored by the SDK's MCP clients for `initialize`, `tools/list`, and `tools/call` instead of hardcoded 1.5s and 5s limits — it defaults to 60 seconds and is clamped to 1–3600 seconds
...
- Added `ForgeOSCore.readLiveMessages` for reading a resident session's in-memory transcript, so a plan/act rebuild during an in-flight turn no longer starts from an empty history
...
- A legacy single-file `.forgeosrules` no longer aborts the config scan
- Telemetry events now carry `device_id`
- A malformed OTEL header entry no longer discards the valid ones

## 0.0.66

- Support for free ForgeOS models (`forgeos-free`): free models are labeled "(free)", priced at zero, and hitting the free tier now raises a dedicated limit error that includes the reset time
...
- `@forgeos/ui`: host-safe theme contract and Markdown exports
...
- Added Kimi K3 to the bundled ForgeOSPass model fallback
...
- New `@forgeos/ui` agent chat components with Storybook and npm packaging
- Updated the bundled model catalog
...
- Added a shared `@forgeos/ui` theme package
...
- Added the ForgeOSPass usage-limit error so limit-reached responses are surfaced clearly
...
- Added telemetry for auth-refresh outcomes and ForgeOS credential lifecycle debug logging

## 0.0.59

- You can now select ForgeOS free models on the ForgeOSPass provider
- The SDK now recognizes ForgeOSPass rate-limit responses and surfaces them as a typed `ForgeOSPassLimitError` (with `isForgeOSPassLimitMessage` / `extractForgeOSPassLimitMessage` helpers)
- Removed references to the retired ForgeOSPass GLM 5.1 model
...
- ForgeOS provider requests now send versioned ForgeOS client-identity headers
...
- Added a shared prompt-format helper in `@forgeos/shared` and simplified runtime host support
...
- Add ForgeOSPass subscription support to the account service

## 0.0.53

- Show when request cost is covered by the user's ForgeOS subscription
- List ForgeOSPass features in the not-subscribed message
...
- Updated the ForgeOSPass model list live, restored ForgeOSPass models in onboarding, fixed ForgeOSPass error mapping, and scoped the ForgeOSPass URL to the CLI
...
- Fixed Z.ai model metadata not resolving correctly when using Z.ai models through the ForgeOS provider; aliases now map to the right model metadata and user overrides are preserved

## 0.0.50

- Truncate every tool result by default (including MCP and custom tool output), with tightened `MessageBuilder` limits and tunable `FORGEOS_MESSAGE_BUILDER_*` env overrides, to keep provider requests within budget
...
- Add ForgeOSPass provider support (built-in provider, error handling, format compatibility)
...
- Reverted ForgeOSPass recommended-models support, removing the `forgeosPass` field from the recommended models data

## 0.0.48

- Added ForgeOSPass support and ForgeOSPass models
...
- Enforced a production singleton ForgeOS Hub so only one hub daemon runs, and a stale hub is respawned after an upgrade
...
- Added the ForgeOS Hub web app for managing and monitoring agent sessions
...
- Added a FORGEOS_PLUGIN_IMPORT_TIMEOUT_MS environment override for plugin import timeouts
...
- Improved ForgeOS provider migration
