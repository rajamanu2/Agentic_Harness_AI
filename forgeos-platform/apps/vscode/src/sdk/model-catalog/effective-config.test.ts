...
describe("buildEffectiveProviderConfig", () => {
	...
	it("keeps ForgeOS account auth in the auth envelope", async () => {
		const { buildEffectiveProviderConfig } = await import("./effective-config")
		mocks.setApiConfiguration({ forgeosApiKey: "forgeos-access-token", forgeosAccountId: "account-123" })

		expect(buildEffectiveProviderConfig(parseProviderId("forgeos"))).toEqual({
			providerId: parseProviderId("forgeos"),
			apiKey: "forgeos-access-token",
			auth: { accessToken: "forgeos-access-token", accountId: "account-123" },
		})
	})
})
