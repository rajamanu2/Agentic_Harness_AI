...
const {
	...
	mockResolveForgeOSBuildEnv: vi.fn(() => "production"),
	mockResolveProductionHubOwnerContext: vi.fn(() => ({
		ownerId: "production",
		discoveryPath: "/tmp/forgeos-data/locks/hub/production.json",
	})),
	mockResolveSharedHubOwnerContext: vi.fn(() => ({
		ownerId: "shared",
		discoveryPath: "/tmp/forgeos-data/locks/hub/owners/shared.json",
	})),
	mockStartHubWebSocketServer: vi.fn(),
}));

vi.mock("@forgeos/shared", () => ({
	resolveForgeOSBuildEnv: mockResolveForgeOSBuildEnv,
}));
...
const originalHubPort = process.env.FORGEOS_HUB_PORT;
...
describe("ensureHubServer", () => {
	afterEach(() => {
		...
		mockResolveForgeOSBuildEnv.mockClear();
		mockResolveForgeOSBuildEnv.mockReturnValue("production");
		...
		if (originalHubPort === undefined) {
			delete process.env.FORGEOS_HUB_PORT;
		} else {
			process.env.FORGEOS_HUB_PORT = originalHubPort;
		}
	});
	...
	it("allows port fallback by default in development when no port is explicit", async () => {
		delete process.env.FORGEOS_HUB_PORT;
		mockResolveForgeOSBuildEnv.mockReturnValue("development");
		const { ensureHubServer } = await import("./start-shared-server");
	...
	it("does not default port fallback when FORGEOS_HUB_PORT is explicit", async () => {
		process.env.FORGEOS_HUB_PORT = "30001";
		const { ensureHubServer } = await import("./start-shared-server");
