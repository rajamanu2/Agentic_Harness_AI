...
describe("cli e2e", () => {
	...
	const createIsolatedEnv = (
		...
		return {
			...process.env,
			HOME: homeDir,
			FORGEOS_DIR: path.join(homeDir, ".forgeos"),
			FORGEOS_DATA_DIR: dataDir,
			FORGEOS_DB_DATA_DIR: path.join(dataDir, "db"),
			FORGEOS_SESSION_DATA_DIR: sessionDir,
			FORGEOS_TEAM_DATA_DIR: teamDir,
			FORGEOS_SESSION_BACKEND_MODE: "local",
	...
	it("lists enabled workflows in text mode", () => {
		...
		const workflowsDir = path.join(workspace, ".forgeosrules", "workflows");
...
Review checklist.`,
	...
	it("includes Documents/ForgeOS workflows", () => {
		const homeDir = mkdtempSync(path.join(os.tmpdir(), "cli-e2e-home-"));
...
Release from docs path.`,
	...
	it("lists enabled rules", () => {
		...
		tempDirs.push(workspace);
		const rulesDir = path.join(workspace, ".forgeosrules");
...
Do not force push.`,
	...
	it("lists enabled skills", () => {
		...
		tempDirs.push(workspace);
		const skillsDir = path.join(workspace, ".forgeosrules", "skills", "commit");
...
Create a concise commit message.`,
	...
	it("includes Documents/ForgeOS rules and skills", () => {
		const homeDir = mkdtempSync(path.join(os.tmpdir(), "cli-e2e-home-"));
		const workspace = mkdtempSync(path.join(os.tmpdir(), "cli-e2e-workspace-"));
		tempDirs.push(homeDir, workspace);
		const docsRulesDir = path.join(homeDir, "Documents", "ForgeOS", "Rules");
...
Skill from docs path.`,
	...
	it("lists configured agents with source paths", () => {
		...
		const workspace = mkdtempSync(path.join(os.tmpdir(), "cli-e2e-workspace-"));
		tempDirs.push(homeDir, workspace);
		const globalAgentsDir = path.join(homeDir, ".forgeos", "agents");
		const workspaceAgentsDir = path.join(workspace, ".forgeos", "agents");
...
Break work into clear steps.`,
	...
	it("lists discovered plugins", () => {
		...
		const workspacePluginsDir = path.join(workspace, ".forgeos", "plugins");
		const userPluginsDir = path.join(homeDir, ".forgeos", "plugins");
		...
		expect(
			parsed.some((plugin) =>
				plugin.path.endsWith(
					path.join(".forgeos", "plugins", "workspace-plugin.ts"),
				),
			),
		).toBe(true);
		expect(
			parsed.some((plugin) =>
				plugin.path.endsWith(path.join(".forgeos", "plugins", "user-plugin.js")),
			),
		).toBe(true);
		expect(
			parsed.some((plugin) =>
				plugin.path.endsWith(
					path.join("Documents", "ForgeOS", "Plugins", "docs-plugin.ts"),
				),
			),
		).toBe(true);
	});
	...
	it("lists configured mcp servers", () => {
		...
		const settingsPath = path.join(tempRoot, "forgeos_mcp_settings.json");
		writeFileSync(
			settingsPath,
			JSON.stringify(
				{
		...
		const textResult = runCli(["config", "mcp"], {
			env: {
				...createIsolatedEnv(),
				FORGEOS_MCP_SETTINGS_PATH: settingsPath,
			},
		});
		expect(textResult.status).toBe(0);
	...
	it("routes mcp install and requires a TTY for the prefilled wizard", () => {
		...
		expect(result.status).toBe(1);
		expect(asText(result.stderr)).toContain(
			"forgeos mcp install opens the MCP wizard and requires a TTY.",
		);
	});
	...
	it("routes mcp uninstall and its rm alias", () => {
		...
		const uninstallResult = runCli(["mcp", "uninstall", "docs"], {
			env: { ...createIsolatedEnv(), FORGEOS_MCP_SETTINGS_PATH: settingsPath },
		});
	...
	it("lists available tools", () => {
		...
		const textResult = runCli(["config", "tools"], {
			...
			env: {
				...
				HOME: homeDir,
				FORGEOS_DATA_DIR: dataDir,
				FORGEOS_GLOBAL_SETTINGS_PATH: globalSettingsPath,
			},
		});
		expect(textResult.status).toBe(0);
	...
	it("accepts valid hook payloads and writes audit log", () => {
		...
		const result = runCli(["hook"], {
			env: {
				...
				HOME: homeDir,
				FORGEOS_SESSION_DATA_DIR: sessionDir,
				FORGEOS_HOOKS_LOG_PATH: hookPath,
			},
