...
afterEach(() => {
	...
	delete document.documentElement.dataset.forgeosFontSize;
});
...
describe("app font size", () => {
	...
	it("persists and applies the selected size", () => {
		...
		expect(document.documentElement.dataset.forgeosFontSize).toBe("18");
	...
	it("restores the saved size before the first paint", () => {
		...
		expect(document.documentElement.dataset.forgeosFontSize).toBe("19");
	});
	...
	it("uses the default before paint when storage is invalid", () => {
		...
		expect(document.documentElement.dataset.forgeosFontSize).toBe("15");
	});
});
