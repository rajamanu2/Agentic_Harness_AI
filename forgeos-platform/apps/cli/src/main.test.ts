...
const originalGlobalSettingsPath = process.env.FORGEOS_GLOBAL_SETTINGS_PATH;
...
const authMocks = vi.hoisted(() => ({
	...
	normalizeProviderId: vi.fn((providerId?: string) => providerId ?? "forgeos"),
...
const migrationNoticeMocks = vi.hoisted(() => ({
	getForgeOSCliMigrationNotice: vi.fn<
		(
			dataDir?: string,
	...
	>(() => undefined),
	markForgeOSCliMigrationNoticeShown: vi.fn(),
}));
...
vi.mock("@forgeos/core", async () => {
	return {
		...(await vi.importActual("@forgeos/core")),
...
describe("runCli lightweight command dispatch", () => {
	...
	beforeEach(() => {
		...
		globalSettingsRoot = mkdtempSync(join(tmpdir(), "forgeos-cli-main-test-"));
		process.env.FORGEOS_GLOBAL_SETTINGS_PATH = join(
			globalSettingsRoot,
		...
		authMocks.normalizeProviderId.mockImplementation(
			(providerId?: string) => providerId ?? "forgeos",
		);
		...
		migrationNoticeMocks.getForgeOSCliMigrationNotice.mockReset();
		migrationNoticeMocks.getForgeOSCliMigrationNotice.mockReturnValue(undefined);
		migrationNoticeMocks.markForgeOSCliMigrationNoticeShown.mockReset();
	...
	afterEach(() => {
		...
		if (originalGlobalSettingsPath === undefined) {
			...
			process.env.FORGEOS_GLOBAL_SETTINGS_PATH = originalGlobalSettingsPath;
		}
	...
	it("rejects a single bare positional prompt token", async () => {
		...
		expect(consoleError).toHaveBeenCalledWith(
			expect.stringContaining('Use "forgeos --help"'),
		);
	...
	it("creates a worktree and runs prompt sessions from it", async () => {
		...
		expect(runtimeMocks.runAgent).toHaveBeenCalledWith(
			"say hello",
			expect.objectContaining({
				cwd: "/tmp/forgeos-worktree",
				workspaceRoot: "/tmp/forgeos-worktree",
			}),
			expect.anything(),
		);
	});
	...
	it("passes the migration notice marker into interactive mode", async () => {
		const notice = {
			id: "forgeos-cli-forgeos-pass-intro",
			title: "Try ForgeOSPass",
		};
		migrationNoticeMocks.getForgeOSCliMigrationNotice.mockReturnValue(notice);
		process.argv = ["bun", "src/index.ts"];
		...
		expect(
			migrationNoticeMocks.markForgeOSCliMigrationNoticeShown,
	...
	it("passes the active ForgeOSPass provider into the migration notice gate", async () => {
		providerSettingsMocks.getLastUsedProviderSettings.mockReturnValue({
			provider: "forgeos-pass",
			model: "forgeos-pass/test-model",
		});
		process.argv = ["bun", "src/index.ts"];
		...
		await expect(runCli()).resolves.toBeUndefined();
		expect(
			migrationNoticeMocks.getForgeOSCliMigrationNotice,
	...
	it("does not start OAuth before onboarding in interactive mode", async () => {
		...
		authMocks.normalizeProviderId.mockReturnValue("forgeos");
		authMocks.getPersistedProviderApiKey.mockReturnValue(undefined);
	...
	describe("persisted general settings at startup", () => {
		function writePersistedSettings(settings: Record<string, unknown>) {
			const path = process.env.FORGEOS_GLOBAL_SETTINGS_PATH;
			if (!path) {
				throw new Error("FORGEOS_GLOBAL_SETTINGS_PATH is not set");
			}
	...
	it("opens history inside the interactive TUI for the history picker", async () => {
		migrationNoticeMocks.getForgeOSCliMigrationNotice.mockReturnValue({
			id: "forgeos-cli-forgeos-pass-intro",
			title: "Try ForgeOSPass",
		});
	...
	it("does not pass non-ForgeOS provider settings as ForgeOS account options", async () => {
		providerSettingsMocks.getLastUsedProviderSettings.mockReturnValue({
			provider: "openrouter",
		...
		expect(runtimeMocks.runInteractive).toHaveBeenCalledWith(
			...
			undefined,
			expect.objectContaining({
				forgeosApiBaseUrl: undefined,
				forgeosProviderSettings: undefined,
			}),
		);
	});

	it("passes ForgeOS provider settings as ForgeOS account options", async () => {
		const forgeosSettings = {
			provider: "forgeos",
		...
		providerSettingsMocks.getProviderSettings.mockReturnValue(forgeosSettings);
		authMocks.normalizeProviderId.mockImplementation(
			(providerId?: string) => providerId ?? "forgeos",
		);
		process.argv = ["bun", "src/index.ts"];
		...
		expect(runtimeMocks.runInteractive).toHaveBeenCalledWith(
			...
			undefined,
			expect.objectContaining({
				forgeosApiBaseUrl: "https://api.example.test",
				forgeosProviderSettings: forgeosSettings,
			}),
		);
	});

	it("seeds feature flag identity from persisted ForgeOS account id before refreshing flags", async () => {
		const forgeosSettings = {
			provider: "forgeos",
			model: "anthropic/claude-sonnet-4.6",
			auth: {
	...
	it("identifies saved ForgeOS accountId for telemetry before runtime events", async () => {
		// FORGEOS-2406: when persisted ForgeOS auth includes an accountId, the
	...
	it("does not call identifyTelemetryAccount in runtime path when no saved ForgeOS accountId", async () => {
		// FORGEOS-2406: when no persisted accountId is found (anonymous/unauthenticated),
	...
	it("does not call identifyTelemetryAccount from runtime path when provider is not forgeos", async () => {
		// FORGEOS-2406: identity identification from saved settings only applies
		// to ForgeOS-provider sessions; other providers use different auth flows.
		providerSettingsMocks.getLastUsedProviderSettings.mockReturnValue({
			provider: "openrouter",
			model: "openai/gpt-5",
		});
