...
export const FORGEOS_JS_RUNTIME_PATH_ENV = "FORGEOS_JS_RUNTIME_PATH";
...
export function resolveSubprocessRuntimeExecutable(
	...
	const explicit =
		options.runtimeExecutable?.trim() || env[FORGEOS_JS_RUNTIME_PATH_ENV]?.trim();
	if (explicit) {
		return explicit;
	}
	...
	start(): void {
		...
		const child = spawn(
			...
			command.slice(1),
			{
				...
				env: withResolvedForgeOSBuildEnv(process.env),
