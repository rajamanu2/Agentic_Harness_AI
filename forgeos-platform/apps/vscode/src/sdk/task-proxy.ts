...
import type { ForgeOSAskResponse } from "@shared/WebviewMessage"
...
export interface TaskProxy {
	...
	handleWebviewAskResponse: (askResponse: ForgeOSAskResponse, text?: string, images?: string[], files?: string[]) => Promise<void>
...
export interface MessageStateHandlerEvents {
	forgeosMessagesChanged: [change: ForgeOSMessageChange]
}

/**
 * Change event for message updates.
 * Mirrors ForgeOSMessageChange from src/core/task/message-state.ts.
 */
interface ForgeOSMessageChange {
...
/**
 * Message state handler that accumulates ForgeOSMessages and emits change events.
 ...
 * getForgeOSMessages() and event emitter interface that consumers expect.
 */
export class MessageStateHandler extends EventEmitter<MessageStateHandlerEvents> {
	private messages: ForgeOSMessage[] = []
	 ...
	 */
	addMessages(messages: ForgeOSMessage[]): void {
		for (const message of messages) {
			...
			if (existingIndex !== -1) {
				...
				this.emit("forgeosMessagesChanged", {
					type: "update",
				...
				this.emit("forgeosMessagesChanged", { type: "add", messages: this.messages, message })
			}
		}
	}

	/** Get all accumulated messages (returns a copy) */
	getForgeOSMessages(): ForgeOSMessage[] {
	...
	replaceMessages(messages: ForgeOSMessage[]): void {
		this.messages = [...messages]
		this.emit("forgeosMessagesChanged", { type: "set", messages: this.messages })
	}
}
...
interface TaskProxyState {
	askResponse?: ForgeOSAskResponse
