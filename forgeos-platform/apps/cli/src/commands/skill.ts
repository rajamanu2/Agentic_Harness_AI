...
// `forgeos skill` is a thin wrapper around the open skills CLI
...
// `forgeos skill` command we default these to ForgeOS unless the user picked their
...
// interactively instead — not what someone scoping to ForgeOS would expect.
const FORGEOS_SCOPED_SUBCOMMANDS = new Set([
	"add",
	"install",
	"i",
	"update",
...
/**
 * Build the argument list passed to `npx`, injecting `--agent forgeos` for
...
export function buildSkillsArgs(userArgs: readonly string[]): string[] {
	...
	if (
		subcommand &&
		FORGEOS_SCOPED_SUBCOMMANDS.has(subcommand) &&
		!hasAgentFlag(args)
	) {
		args.push("--agent", "forgeos");
	}
	...
	return new Promise<number>((resolve) => {
		...
		child.once("error", (error: NodeJS.ErrnoException) => {
			...
			if (error.code === "ENOENT") {
				io.writeErr(
					'npx was not found. Install Node.js (which includes npx) to use "forgeos skill".',
				);
			} else {
