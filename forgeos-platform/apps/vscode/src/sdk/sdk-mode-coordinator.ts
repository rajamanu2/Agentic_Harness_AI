import { getProviderAuthStorageId } from "@forgeos/core"
import { createModeSwitchNoticeTracker, type ModeSwitchNotice, type ModeSwitchNoticeTracker } from "@forgeos/shared"
import type { ChatContent } from "@shared/ChatContent"
import type { ForgeOSMessage, TurnPhase } from "@shared/ExtensionMessage"
...
export class SdkModeCoordinator {
	 ...
	 * CLI's round-trip-cancelling tracker (@forgeos/shared), scoped to the session
	...
	async togglePlanActMode(modeToSwitchTo: Mode, chatContent?: ChatContent): Promise<boolean> {
		...
		if (activeSession) {
			...
			const latestAssistantResult = [...forgeosMessages]
				.reverse()
				.find(
					(message) =>
						message.type === "say" &&
	...
	private async performRebuildSessionForMode(
		...
		try {
			...
			if (usesForgeOSAccountAuth(config.providerId) && !config.apiKey) {
				Logger.warn(
					`[SdkController] Mode rebuild: new mode '${newMode}' provider is '${config.providerId}' but no ForgeOS auth token - emitting auth error`,
				)
				...
				this.options.emitForgeOSAuthError()
				await this.options.postStateToWebview()
				return false
			}
			...
			if (options.autoContinue) {
				...
				if (userPrompt || userImages?.length || userFiles?.length) {
					const userMessage: ForgeOSMessage = {
						ts: Date.now(),
						type: "say",
						say: "user_feedback",
						text: userPrompt ?? "",
			...
			// messages (forgeosMessages ride on the state post), and auto-continue
			...
			const errorMessage: ForgeOSMessage = {
				ts: Date.now(),
				type: "say",
				say: "error",
	...
	private async cancelRunningTurnForModeChange(oldManager: SdkSessionHost, oldSessionId: string): Promise<void> {
		...
		if (task?.messageStateHandler) {
			const current = task.messageStateHandler.getForgeOSMessages()
		...
		const resumeMessage: ForgeOSMessage = {
			ts: Date.now(),
			type: "ask",
			ask: "resume_task",
			text: "",
