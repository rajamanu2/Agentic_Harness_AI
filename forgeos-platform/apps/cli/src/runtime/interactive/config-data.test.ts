...
import type { UserInstructionConfigService } from "@forgeos/core";
...
describe("interactive config data loader", () => {
	...
	async function writeSettingsPlugin(tempRoot: string): Promise<string> {
		const pluginsDir = join(tempRoot, ".forgeos", "plugins");
...
Find installable skills.`,
	...
	it("keeps plugin tool toggle behavior", async () => {
		...
		const settings = JSON.parse(
			await readFile(process.env.FORGEOS_GLOBAL_SETTINGS_PATH, "utf8"),
	...
	it("deletes a package-backed plugin and refreshes bundled slash commands", async () => {
		...
		const packageDir = join(tempRoot, ".forgeos", "plugins", "delete-plugin");
...
Erase stale plugin commands.`,
	...
	it("marks bundled package skills with their plugin owner", async () => {
		...
		await writeFile(
			...
			JSON.stringify(
				{
					name: "forgeos-installed-plugin-demo",
					forgeos: {
...
Review with the bundled skill.`,
		...
		expect(skill).toMatchObject({
			name: "review",
			pluginName: "forgeos-sdk-portable-agents",
			pluginPath,
			source: "workspace-plugin",
		});
	});
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			...
			process.env.FORGEOS_GLOBAL_SETTINGS_PATH = globalSettingsPath;
			process.env.FORGEOS_MCP_SETTINGS_PATH = settingsPath;
			const pluginPath = await writeMcpSettingsPlugin(tempRoot);
			await writeFile(
				settingsPath,
