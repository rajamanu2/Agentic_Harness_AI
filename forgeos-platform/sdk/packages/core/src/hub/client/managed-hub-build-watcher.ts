...
import { isHubDaemonProcess, resolveForgeOSBuildEnv } from "@forgeos/shared";
import {
	compareHubBuilds,
...
const WATCH_INTERVAL_ENV = "FORGEOS_HUB_BUILD_WATCH_INTERVAL_MS";
 ...
 *
 * A mismatch means another ForgeOS installation replaced the shared Hub with a
 ...
 * invoke `onMismatch` when another ForgeOS installation has replaced the Hub
 ...
 * prompts, so the watcher refuses to start when `FORGEOS_HUB_PORT` is set.
...
export function watchManagedHubBuildMismatch(
	...
	if (isHubDaemonProcess() || process.env.FORGEOS_HUB_PORT?.trim()) {
		return () => {};
	}
