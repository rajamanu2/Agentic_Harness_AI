...
function parseSubagentRowData(message: ForgeOSMessage): SubagentRowData | null {
	if (!message.text) {
		return null
	}
	...
	try {
		if (message.ask === "use_subagents" || message.say === "use_subagents") {
			const parsed = JSON.parse(message.text) as ForgeOSAskUseSubagents
		...
		const parsed = JSON.parse(message.text) as ForgeOSSaySubagentStatus
...
export default function SubagentStatusRow({ message, isLast, lastModifiedMessage }: SubagentStatusRowProps) {
	...
	const title = singular ? "ForgeOS wants to use a subagent:" : "ForgeOS wants to use subagents:"
