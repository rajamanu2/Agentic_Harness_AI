...
export function transformRemoteConfigToStateShape(remoteConfig: RemoteConfig): Partial<RemoteConfigFields> {
	...
	const forgeosSettings = remoteConfig.providerSettings?.ForgeOS
	if (forgeosSettings) {
		transformed.planModeApiProvider = "forgeos"
		transformed.actModeApiProvider = "forgeos"
		providers.push("forgeos")
	}
...
export function clearRemoteConfig() {
	try {
		...
		// the remote config forgeos rules toggle state is stored in global state
