...
import * as Llms from "@forgeos/llms";
import {
...
async function mergeKnownModels(
	...
	if (providerId === "forgeos-pass" && Object.keys(liveModels).length > 0) {
	...
	if (providerId === "forgeos") {
		// ForgeOS recommendations can use Vercel-style ids while the broader
...
function resolveCatalogModels(
	...
	// ForgeOS uses OpenRouter-backed catalog models, so live catalog lookups must
