...
e2e("Chat - can send messages and switch between modes", async ({ helper, sidebar }) => {
	...
	await expect(inputbox).toHaveValue("Hello, ForgeOS!")
	...
	await expect(sidebar.getByText("mock ForgeOS API response")).toBeVisible()
	...
	await expect(sidebar.getByText("Hello, ForgeOS!")).toBeVisible()
