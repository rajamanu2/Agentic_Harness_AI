...
import type { ConnectorCliLaunchSpec } from "@forgeos/shared";
...
describe("ConnectorSupervisor", () => {
	...
	it("strips daemon and connector-child markers from the child environment", async () => {
		process.env.FORGEOS_RUN_AS_HUB_DAEMON = "1";
		process.env.FORGEOS_SLACK_CONNECT_CHILD = "1";
		process.env.FORGEOS_CONNECTOR_DETACHED_CHILD = "1";
		process.env.FORGEOS_CONNECTOR_STARTING_INSTANCE = '{"channel":"slack"}';
		try {
			const harness = createHarness();
			await harness.supervisor.start({
				channel: "slack",
			...
			expect(env.FORGEOS_RUN_AS_HUB_DAEMON).toBeUndefined();
			expect(env.FORGEOS_SLACK_CONNECT_CHILD).toBeUndefined();
			expect(env.FORGEOS_CONNECTOR_DETACHED_CHILD).toBeUndefined();
			expect(env.FORGEOS_CONNECTOR_STARTING_INSTANCE).toBeUndefined();
			expect(env.PATH).toBe(process.env.PATH);
		} finally {
			delete process.env.FORGEOS_RUN_AS_HUB_DAEMON;
			delete process.env.FORGEOS_SLACK_CONNECT_CHILD;
			delete process.env.FORGEOS_CONNECTOR_DETACHED_CHILD;
			delete process.env.FORGEOS_CONNECTOR_STARTING_INSTANCE;
		}
	});
	...
	it("reaps a dead connector and restarts it with exponential backoff", async () => {
		...
		expect(harness.cleanups).toEqual(["slack:forgeos-slack"]);
	...
	it("cancels a pending backoff restart when a new start replaces the entry", async () => {
		// A user runs `forgeos connect` while the instance is waiting out its
	...
	it("serialises a boot-time restart with a concurrent user start", async () => {
		...
		// user `forgeos connect` for the same instance arrives over the hub.
		...
		const harness = createHarness({
			active: [{ type: "slack", instanceId: "forgeos-slack", pid: 700 }],
		});
	...
	it("waits for a stopped process to die, escalating to SIGKILL", async () => {
		...
		const pending = harness.supervisor
			.stop({ channel: "slack", instanceId: "forgeos-slack" })
			.then((stopped) => {
				resolved = true;
				return stopped;
			});
	...
	it("does not adopt an instance it already supervises", async () => {
		const harness = createHarness({
			active: [{ type: "slack", instanceId: "forgeos-slack", pid: 800 }],
		});
		await harness.supervisor.start({
			channel: "slack",
			instanceId: "forgeos-slack",
	...
	it("cannot restart an adopted connector with no stored arguments", async () => {
		mocks.getPersistedConnectorConnection.mockReturnValue(undefined);
		const harness = createHarness({
			active: [{ type: "slack", instanceId: "forgeos-slack", pid: 900 }],
		});
