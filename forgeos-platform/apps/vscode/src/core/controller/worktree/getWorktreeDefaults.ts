...
import { WorktreeDefaults } from "@shared/proto/forgeos/worktree"
...
export async function getWorktreeDefaults(_controller: Controller, _request: EmptyRequest): Promise<WorktreeDefaults> {
	...
	const suggestedBranch = `worktree/forgeos-${suffix}`

	// Generate suggested path in Documents/ForgeOS/Worktrees/<project-name>-<suffix>
	...
	const suggestedPath = path.join(documentsPath, "ForgeOS", "Worktrees", `${projectName}-${suffix}`)
