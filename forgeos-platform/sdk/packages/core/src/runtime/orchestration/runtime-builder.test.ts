...
import { setHomeDir } from "@forgeos/shared/storage";
...
describe("DefaultRuntimeBuilder", () => {
	...
	it("derives enabled provider tools without registering a local executor", async () => {
		const settingsRoot = mkdtempSync(join(tmpdir(), "forgeos-model-tools-"));
	...
	it("loads configured agent files as named subagent tools", async () => {
		const tempHome = mkdtempSync(join(tmpdir(), "forgeos-agent-home-"));
		const workspaceRoot = mkdtempSync(join(tmpdir(), "forgeos-agent-workspace-"));
		tempDirs.push(tempHome, workspaceRoot);
		setHomeDir(tempHome);

		const globalAgentsDir = join(tempHome, ".forgeos", "agents");
...
You are a code reviewer.`,
	...
	it("does not register root skills when only configured agents declare skills", async () => {
		...
		const agentsDir = join(workspaceRoot, ".forgeos", "agents");
...
Use the review guidance.`,
	...
	it("includes MCP tools from configured servers", async () => {
		...
		const settingsPath = join(tempRoot, "forgeos_mcp_settings.json");
		const previousSettingsPath = process.env.FORGEOS_MCP_SETTINGS_PATH;
  ...
  while (true) {
    ...
    handle(message);
		...
		try {
			...
			process.env.FORGEOS_MCP_SETTINGS_PATH = previousSettingsPath;
		}
	});
...
process.stdin.on("data", (chunk) => {
  ...
  process.stdout.write("{]\\n");
	...
	it("includes skills tool when workspace skills are available", async () => {
		...
		const skillDir = join(cwd, ".forgeos", "skills", "commit");
		mkdirSync(skillDir, { recursive: true });
		writeFileSync(
			join(skillDir, "SKILL.md"),
			`---
...
Use conventional commits.`,
	...
	it("includes skills bundled in discovered plugin packages", async () => {
		...
		setHomeDir(cwd);
		const pluginDir = join(cwd, ".forgeos", "plugins", "review-plugin");
...
Use conventional commits.`,
	...
	it("marks configured but disabled skills in executor metadata", async () => {
		...
		const enabledDir = join(cwd, ".forgeos", "skills", "commit");
		const disabledDir = join(cwd, ".forgeos", "skills", "review");
...
Disabled skill.`,
	...
	it("scopes skills tool to session-configured skills", async () => {
		...
		const commitDir = join(cwd, ".forgeos", "skills", "commit");
		const reviewDir = join(cwd, ".forgeos", "skills", "review");
...
Review skill.`,
	...
	it("does not register the skills tool when all configured skills are disabled", async () => {
		...
		const skillDir = join(cwd, ".forgeos", "skills", "review");
		mkdirSync(skillDir, { recursive: true });
		writeFileSync(
			join(skillDir, "SKILL.md"),
			`---
...
Review skill.`,
		...
		const userInstructionService = createUserInstructionConfigService({
			skills: { directories: [join(cwd, ".forgeos", "skills")] },
			rules: { directories: [] },
