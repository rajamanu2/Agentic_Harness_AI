...
import {
	...
	type ForgeOSModelPickerEntry,
	useForgeOSRecommendedModels,
} from "../../components/model-selector/forgeos-model-picker";
import {
	type SearchableItem,
	useSearchableList,
...
import {
	FORGEOS_PASS_SUBSCRIPTION_OPTIONS,
	type ForgeOSPassSubscriptionStatus,
	...
	shouldUseFeaturedForgeOSModelPicker,
	type ThinkingLevel,
...
export function useOnboardingController(props: OnboardingControllerProps) {
	...
	const [forgeosPassSubscriptionStatus, setForgeOSPassSubscriptionStatus] =
		useState<ForgeOSPassSubscriptionStatus>("loading");
	const [forgeosPassSubscriptionError, setForgeOSPassSubscriptionError] =
		useState("");
	const [forgeosPassCurrentPlanName, setForgeOSPassCurrentPlanName] = useState("");
	const [forgeosPassPlanFeatures, setForgeOSPassPlanFeatures] = useState<string[]>(
		[],
	);
	const [forgeosPassSubscriptionSelected, setForgeOSPassSubscriptionSelected] =
		useState(0);
	const [forgeosPassSubscriptionOpenStatus, setForgeOSPassSubscriptionOpenStatus] =
		useState("");
	const forgeosPassSubscriptionUrl = useMemo(() => getCliSubscriptionUrl(), []);
	...
	const createCustomModelItem = useCallback(
		(_search: string, filteredItems: SearchableItem[]) => {
			if (activeProviderId === "forgeos-pass") {
				return undefined;
			}
	...
	// ForgeOS featured model picker (ForgeOSPass gets Subscribed/Free sections)
	const recommended = useForgeOSRecommendedModels();
	const forgeosEntries: ForgeOSModelPickerEntry[] = useMemo(
		() =>
			recommended.data
	...
	const [forgeosModelSelected, setForgeOSModelSelected] = useState(0);
	const [forgeosModelReasoningIds, setForgeOSModelReasoningIds] = useState<
		Set<string>
	>(new Set());

	useEffect(() => {
		// The featured picker serves both forgeos and forgeos-pass, so pool
		// reasoning support from both catalogs. Display names need no catalog
		// here: fetchForgeOSRecommendedModels resolves them.
		void Promise.allSettled(
			["forgeos", "forgeos-pass"].map((providerId) =>
			...
			setForgeOSModelReasoningIds(ids);
		});
	}, []);
	...
	const refreshForgeOSPassSubscriptionStatus = useCallback(() => {
		setForgeOSPassSubscriptionStatus("loading");
		setForgeOSPassSubscriptionError("");
		setForgeOSPassCurrentPlanName("");
		setForgeOSPassSubscriptionOpenStatus("");
			...
			.then(({ currentPlanResult, availablePlansResult }) => {
				...
				if (currentPlanResult.status === "rejected") {
					...
					if (message.trim().toLowerCase() === "no plan found for user") {
						setForgeOSPassSubscriptionStatus("unsubscribed");
						return;
					}
					setForgeOSPassSubscriptionError(message);
					setForgeOSPassSubscriptionStatus("error");
					return;
				}
				...
				if (plan) {
					setForgeOSPassCurrentPlanName(
						plan.displayName || plan.name || plan.id || "ForgeOSPass",
					);
					setForgeOSPassSubscriptionStatus("subscribed");
				} else {
					setForgeOSPassSubscriptionStatus("unsubscribed");
				}
			});
	...
	const transitionToModelPicker = useCallback(
		(providerId: string) => {
			...
			if (shouldUseFeaturedForgeOSModelPicker(providerId)) {
				setForgeOSModelSelected(0);
				setStep("forgeos_model");
	...
	const transitionToForgeOSPassSubscription = useCallback(() => {
		setActiveProviderId("forgeos-pass");
		const provider = providers.find((p) => p.id === "forgeos-pass");
		setActiveProviderName(provider?.name ?? "ForgeOSPass");
		setModelsDefaultId(provider?.defaultModelId ?? "");
		setForgeOSPassSubscriptionSelected(0);
		setStep("forgeos_pass_subscription");
		refreshForgeOSPassSubscriptionStatus();
	}, [providers, refreshForgeOSPassSubscriptionStatus]);

	const handleAuthComplete = useCallback(
		(providerId: OnboardingOAuthProviderId) => {
			if (providerId === "forgeos-pass") {
				transitionToForgeOSPassSubscription();
				return;
			}
			transitionToModelPicker(providerId);
		},
		[transitionToForgeOSPassSubscription, transitionToModelPicker],
	);
	...
	const startOAuthFlow = useCallback(
		(providerId: OnboardingOAuthProviderId) => {
			if (isForgeOSProvider(providerId)) {
	...
	const continueFromForgeOSPassSubscription = useCallback(() => {
		transitionToModelPicker("forgeos-pass");
	}, [transitionToModelPicker]);

	const openForgeOSPassSubscriptionPage = useCallback(() => {
		setForgeOSPassSubscriptionOpenStatus("Opening subscription page...");
		void open(forgeosPassSubscriptionUrl, { wait: false })
			.then(() => {
				setForgeOSPassSubscriptionOpenStatus(
			...
			.catch(() => {
				setForgeOSPassSubscriptionOpenStatus(
					`Could not open browser automatically. Open ${forgeosPassSubscriptionUrl}`,
				);
			});
	}, [forgeosPassSubscriptionUrl]);

	useEffect(() => {
		if (
			step === "forgeos_pass_subscription" &&
			forgeosPassSubscriptionStatus === "subscribed"
		) {
			transitionToModelPicker("forgeos-pass");
		}
	}, [step, forgeosPassSubscriptionStatus, transitionToModelPicker]);
	...
	const saveForgeOSModelSelection = useCallback(
		(modelId: string, modelName: string) => {
			...
			setSelectedModelId(modelId);
			if (forgeosModelReasoningIds.has(modelId)) {
		...
		[activeProviderId, forgeosModelReasoningIds, providerSettingsManager],
	);
	...
	useOnboardingKeyboard({
		...
		modelList,
		forgeosEntries,
		forgeosModelSelected,
		forgeosPassSubscriptionStatus,
		forgeosPassSubscriptionOptions: FORGEOS_PASS_SUBSCRIPTION_OPTIONS,
		forgeosPassSubscriptionSelected,
		thinkingSelected,
		setStep,
		setMenuSelected,
		resetByoFields: () => {
		...
		setDeviceError,
		setDeviceStatus,
		setForgeOSModelSelected,
		setForgeOSPassSubscriptionSelected,
		setThinkingSelected,
		continueFromForgeOSPassSubscription,
		refreshForgeOSPassSubscriptionStatus,
		openForgeOSPassSubscriptionPage,
		abortOAuth: () => {
	...
	return {
		...
		forgeosEntries,
		forgeosModelSelected,
		forgeosPassCurrentPlanName,
		forgeosPassPlanFeatures,
		forgeosPassSubscriptionError,
		forgeosPassSubscriptionOpenStatus,
		forgeosPassSubscriptionOptions: FORGEOS_PASS_SUBSCRIPTION_OPTIONS,
		forgeosPassSubscriptionSelected,
		forgeosPassSubscriptionStatus,
		forgeosPassSubscriptionUrl,
