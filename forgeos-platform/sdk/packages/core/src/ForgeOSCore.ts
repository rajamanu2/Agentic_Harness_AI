import type { BasicLogger, ITelemetryService } from "@forgeos/shared";
import {
	ForgeOSCoreAutomationController,
	createForgeOSCoreAutomationExtensionContext,
	createForgeOSCoreAutomationRuntimeHandlers,
	normalizeAutomationCronScope,
	normalizeAutomationOptions,
} from "./forgeos-core/automation";
import {
	createForgeOSCorePendingPromptsApi,
	createForgeOSCoreSettingsApi,
	type RuntimeHostServiceExtensions,
} from "./forgeos-core/runtime-services";
import {
	normalizeForgeOSCoreStartInput,
	toForgeOSCoreStartInput,
} from "./forgeos-core/start-input";
import { emitSessionStartedTelemetry } from "./forgeos-core/telemetry";
import type {
	ForgeOSCoreAutomationApi,
	ForgeOSCoreAutomationOptions,
	ForgeOSCoreListHistoryOptions,
	ForgeOSCoreOptions,
	ForgeOSCoreSettingsApi,
...
export type {
	ForgeOSAutomationEventIngressResult,
	ForgeOSAutomationEventLog,
	ForgeOSAutomationEventSuppression,
	ForgeOSAutomationListEventsOptions,
	ForgeOSAutomationListRunsOptions,
	ForgeOSAutomationListSpecsOptions,
	ForgeOSAutomationRun,
	ForgeOSAutomationRunStatus,
	ForgeOSAutomationSpec,
	ForgeOSCoreAutomationApi,
	ForgeOSCoreAutomationOptions,
	ForgeOSCoreListHistoryOptions,
	ForgeOSCoreOptions,
...
/**
 * The primary entry point for the ForgeOS Core SDK.
 *
 * @example
 * ```ts
 ...
 * ```
 */
export class ForgeOSCore {
	readonly clientName: string | undefined;
	readonly runtimeAddress: string | undefined;
	readonly automation: ForgeOSCoreAutomationApi;
	readonly settings: ForgeOSCoreSettingsApi;
	readonly featureFlags: FeatureFlagsService;
	readonly pendingPrompts: PendingPromptsServiceApi;
	private readonly host: RuntimeHost;
	private readonly prepare: ForgeOSCoreOptions["prepare"] | undefined;
	...
	private constructor(
		...
		prepare: ForgeOSCoreOptions["prepare"],
		...
		automationOptions:
			| (ForgeOSCoreAutomationOptions & { logger?: BasicLogger })
			| undefined,
	) {
		...
		this.settings = createForgeOSCoreSettingsApi(host);
		this.pendingPrompts = createForgeOSCorePendingPromptsApi(host);
		this.automation = new ForgeOSCoreAutomationController(() => {
			if (!this.automationService) {
				throw new Error(
					"ForgeOSCore automation is not enabled. Pass `automation: true` or automation options to ForgeOSCore.create().",
				);
			}
		...
		this.automationService = automationOptions
			? new CronService({
					...
					runtimeHandlers: createForgeOSCoreAutomationRuntimeHandlers({
						host,
						getExtensionContext: () =>
							createForgeOSCoreAutomationExtensionContext({
	...
	/**
	 * Creates a new ForgeOSCore instance.
	 *
	 ...
	 * @returns A promise that resolves to a new ForgeOSCore instance
	 *
	 * @example
	 * ```ts
	 ...
	 * ```
	 */
	static async create(options: ForgeOSCoreOptions = {}): Promise<ForgeOSCore> {
	...
	/**
	 * Starts a new ForgeOS session with the provided configuration.
	 *
	...
	/**
	 * Starts a new ForgeOS session with extended core-specific configuration.
	 * This overload allows specifying local runtime options and config overrides.
	 */
	start(input: ForgeOSCoreStartInput): Promise<StartSessionResult>;
	async start(
		input: StartSessionInput | ForgeOSCoreStartInput,
	): Promise<StartSessionResult> {
		const forgeosCoreInput = toForgeOSCoreStartInput(input);
		const bootstrap = await this.prepare?.(forgeosCoreInput);
		try {
			const preparedInput = bootstrap
				? await bootstrap.applyToStartSessionInput(forgeosCoreInput)
				: forgeosCoreInput;
			const result = await this.host.startSession(
				normalizeForgeOSCoreStartInput(preparedInput, {
	 ...
	 * @example
	 * ```ts
	 * const usageSummary = await forgeos.getAccumulatedUsage(sessionId);
	...
	/**
	 * Disposes the ForgeOSCore instance and all associated resources.
	 *
	 ...
	 * @example
	 * ```ts
	 * const session = await forgeos.get(sessionId);
	 ...
	 */
	listHistory = async (
		options: ForgeOSCoreListHistoryOptions = {},
	 ...
	 * @example
	 * ```ts
	 * const sessions = await forgeos.list(50);
	...
	list = async (
		limit = 200,
		options: Omit<ForgeOSCoreListHistoryOptions, "limit"> = {},
	): Promise<SessionHistoryRecord[]> =>
	 ...
	 * @example
	 * ```ts
	 * const deleted = await forgeos.delete(sessionId);
	 * if (deleted) {
	 ...
	 * @example
	 * ```ts
	 * await forgeos.update(sessionId, {
	 ...
	 * @example
	 * ```ts
	 * const messages = await forgeos.readMessages(sessionId);
	...
	async restore(input: RestoreInput): Promise<RestoreResult> {
		const normalizedStart = input.start
			? normalizeForgeOSCoreStartInput(input.start, {
					defaultCapabilities: this.capabilities,
					withExtensionContext: (context) =>
						createForgeOSCoreAutomationExtensionContext({
							automationService: this.automationService,
	 ...
	 * @example
	 * ```ts
	 * const unsubscribe = forgeos.subscribe((event) => {
	 ...
	 * await forgeos.updateSessionModel(sessionId, "claude-opus-4-1");
	 * ```
	 */
