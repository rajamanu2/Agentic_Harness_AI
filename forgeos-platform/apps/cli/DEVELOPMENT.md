...
This guide covers everything you need to build and run the ForgeOS CLI locally after cloning the repository. It includes setup instructions, a tech stack overview, and a walkthrough of the TUI architecture.
...
```bash
FORGEOS_BUILD_ENV=development bun --conditions=development ./src/index.ts
```
...
The `build:sdk` step is required because `bun link` runs without the `--conditions=development` flag, so Bun resolves workspace packages (`@forgeos/llms`, `@forgeos/core`, etc.) via their `package.json` exports which point to `dist/`. Without the build, those dist files don't exist and you'll get "Cannot find module" errors.

After linking, you can run `forgeos` from any directory:
...
If you're using `bun run dev`, you don't need to rebuild after every SDK change -- dev mode resolves packages from source. But if you're using the linked `forgeos` binary, you do need to rebuild for changes to take effect.
...
```bash
bun run dev -- --interactive --config /tmp/forgeos-test
```

Or set `FORGEOS_FORCE_ONBOARDING=1` to force the onboarding view regardless of existing config.
...
bunx tuistory -s forgeos --cols 120 --rows 36 -- bun src/index.ts --provider anthropic -m claude-sonnet-4-6 -k test-key

# Wait reactively for the chat view (no sleep guessing)
bunx tuistory -s forgeos wait "What can I do for you?" --timeout 30000

# Interact and inspect
bunx tuistory -s forgeos type "/settings"
bunx tuistory -s forgeos press enter
bunx tuistory -s forgeos snapshot --trim     # current screen as text
bunx tuistory -s forgeos screenshot          # current screen as a styled PNG

# A human can watch/drive the same session from another terminal
tuistory attach -s forgeos

# Tear down
bunx tuistory -s forgeos close
```
...
```bash
cd apps/cli
FORGEOS_BUILD_ENV=development bun --conditions=development --inspect-brk=6499 ./src/index.ts
```
...
The CLI is published as the `forgeos` wrapper package on npm with platform-specific binaries under `@forgeos/cli-*`. The release flow lives in the `publish-cli` skill (`.forgeos/skills/publish-cli/SKILL.md` at the repo root).
...
`forgeos` uses a `pino`-backed adapter that targets the core `BasicLogger` contract:

- CLI runtime passes `logger` directly into local `@forgeos/core` sessions.
...
After login, OAuth credentials are persisted with `auth.expiresAt`, and `@forgeos/core` refreshes these tokens automatically during session turns. Provider auth and model settings should be changed through `forgeos auth`, the interactive config UI, or core provider-settings APIs rather than editing provider settings files directly.

On startup, `forgeos` also attempts a legacy settings import:

- Source files: `<FORGEOS_DATA_DIR>/globalState.json` and `<FORGEOS_DATA_DIR>/secrets.json`
- Target file: `<FORGEOS_DATA_DIR>/settings/providers.json` (or `FORGEOS_PROVIDER_SETTINGS_PATH`)
...
- Provider runtime settings continue to persist in `<FORGEOS_DATA_DIR>/settings/providers.json`.
...
- User-added OpenAI-compatible provider model catalogs are persisted in `<FORGEOS_DATA_DIR>/settings/models.json` (or alongside `FORGEOS_PROVIDER_SETTINGS_PATH`).
