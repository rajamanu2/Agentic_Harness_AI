...
import type { RuntimeCapabilities } from "@forgeos/core";
...
describe("disposeSidecarContext attachment cleanup", () => {
	...
	beforeEach(() => {
		previousSessionDataDir = process.env.FORGEOS_SESSION_DATA_DIR;
		testSessionDataDir = join(
			tmpdir(),
			`forgeos-desktop-dispose-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
