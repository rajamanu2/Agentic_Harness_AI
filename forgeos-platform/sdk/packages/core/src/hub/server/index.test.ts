...
describe("hub server startup", () => {
	...
	it("releases the listener when discovery publication fails", async () => {
		const root = await mkdtemp(join(tmpdir(), "forgeos-hub-publish-test-"));
	...
	it("does not reuse managed discovery from an older build", async () => {
		...
		vi.stubEnv("FORGEOS_HUB_BUILD_ID", "old-build");
		vi.stubEnv("FORGEOS_HUB_BUILD_EPOCH_MS", "1000");
		try {
			...
			vi.stubEnv("FORGEOS_HUB_BUILD_ID", "new-build");
			vi.stubEnv("FORGEOS_HUB_BUILD_EPOCH_MS", "2000");
			const result = await ensureHubWebSocketServer({
				owner,
				host: "127.0.0.1",
				port: 0,
	...
	it("reuses managed discovery from a build it cannot order itself against", async () => {
		...
		vi.stubEnv("FORGEOS_HUB_BUILD_ID", "other-build");
		try {
			...
			vi.stubEnv("FORGEOS_HUB_BUILD_ID", "current-build");
			const result = await ensureHubWebSocketServer({
				owner,
				host: "127.0.0.1",
				port: 0,
	...
	it("reuses managed discovery from a newer build instead of starting a rival", async () => {
		...
		vi.stubEnv("FORGEOS_HUB_BUILD_ID", "newer-build");
		vi.stubEnv("FORGEOS_HUB_BUILD_EPOCH_MS", "2000");
		try {
		...
		try {
			...
			const response = await sendRawHttpRequest(
				...
				[
					...
					"Connection: Upgrade",
					"Upgrade: websocket",
					`Sec-WebSocket-Protocol: forgeos-hub-auth.${authToken}`,
					"Sec-WebSocket-Version: 13",
					"Sec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==",
					"",
					"",
