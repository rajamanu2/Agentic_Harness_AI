...
export class SdkTaskControlCoordinator {
	...
	async cancelForgeOSTaskOnSignOut(isForgeOSManagedProvider: boolean): Promise<void> {
		const activeSession = this.options.sessions.getActiveSession()
		if (!isForgeOSManagedProvider || !activeSession?.isRunning) {
			return
		}
	...
	async showTaskWithId(taskId: string): Promise<HistoryItem | undefined> {
		...
		try {
			...
			const rawMessages = await this.options.taskHistory.getForgeOSMessages(taskId)
			if (isSuperseded()) {
				return historyItem
			}
			...
			// The final state update below includes the loaded forgeosMessages. Avoid pushing
	...
	private appendFreshResumeMessage(messages: ForgeOSMessage[], sessionStatus?: string): ForgeOSMessage[] {
	...
	private appendLegacyTaskWarningAndResumeMessage(messages: ForgeOSMessage[]): ForgeOSMessage[] {
		const cleanedMessages = messages.filter((m) => m.ask !== "resume_task" && m.ask !== "resume_completed_task")
		const now = Date.now()
		cleanedMessages.push(
			{
