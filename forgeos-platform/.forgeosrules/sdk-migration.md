...
The VSCode extension runs on the ForgeOS SDK (`@forgeos/core`, `@forgeos/llms`,
`@forgeos/shared`) through an adapter layer in `apps/vscode/src/sdk/`. The
...
2. **Reference the pre-SDK implementation when replacing a module.** Add a
   ...
   `kb_search(name="forgeos", commit="origin/main")` or
...
3. **Single entry point.** There is one codepath — the SDK adapter. No
   `FORGEOS_SDK` env flag.
4. **Use `{appBaseUrl}`**, never hardcode `app.forgeos.bot`.
