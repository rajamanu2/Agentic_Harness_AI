...
describe("formatMcpTimeoutErrorMessage", () => {
	it("names the server, effective bound, and setting to change", () => {
		expect(formatMcpTimeoutErrorMessage("slow-server", 120_000)).toBe(
			'MCP request to "slow-server" timed out after 120s. Increase the "timeout" field (in seconds) for this server in forgeos_mcp_settings.json.',
		);
	});
	...
	it("can include method context and sub-second precision", () => {
		expect(
			...
			'MCP request to "slow-server" (initialize) timed out after 1.5s. Increase the "timeout" field (in seconds) for this server in forgeos_mcp_settings.json.',
		);
	});
});
