...
//
// These helpers spawn forgeos as a child process and return stdout, stderr,
// and exit code - without going through the TUI harness. Use these for:
//   - Exit code assertions
//   - Pure stdout/stderr contract tests
//   - Piped stdin tests (echo "..." | forgeos -y ...)
//   - JSON output validation
//   - Timeout behavior
// ---------------------------------------------------------------------------

import { type SpawnSyncOptions, spawnSync } from "node:child_process";
import { FORGEOS_BIN } from "./constants.js";
import { forgeosEnv } from "./env.js";
...
/**
 * Run `forgeos [args]` synchronously and return stdout/stderr/exitCode.
 *
 * Suitable for deterministic, fast-exiting commands like:
 *   forgeos --help, forgeos --version, forgeos auth -p ... -k ..., etc.
 */
export function runForgeOS(args: string[], opts: RunOptions = {}): RunResult {
	const {
		config = "default",
		env: extraEnv = {},
		stdin,
	...
	const spawnOpts: SpawnSyncOptions = {
		...
		timeout,
		cwd,
		env: { ...forgeosEnv(config), ...extraEnv },
		input: stdin,
	};

	const result = spawnSync(FORGEOS_BIN, args, spawnOpts);
...
/**
 * Assert that a runForgeOS result exited with the expected code.
