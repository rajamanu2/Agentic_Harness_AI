...
vi.mock("@forgeos/core", () => ({
	resolveDefaultMcpSettingsPath: () =>
		process.env.FORGEOS_MCP_SETTINGS_PATH ?? "forgeos_mcp_settings.json",
	setMcpServerDisabled: (options: SetMcpServerDisabledOptions) => {
		const filePath =
			options.filePath ??
			process.env.FORGEOS_MCP_SETTINGS_PATH ??
...
describe("mcp manager dialog helpers", () => {
	...
	it("uses the loaded server path when toggling", async () => {
		...
		process.env.FORGEOS_MCP_SETTINGS_PATH = currentDefaultPath;
		const settings = {
			mcpServers: {
				docs: {
					transport: {
