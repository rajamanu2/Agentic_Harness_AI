...
import { setSdkLogger } from "@forgeos/core"
import { HostProvider } from "@/hosts/host-provider"
import { Logger } from "@/shared/services/Logger"
import type { StorageContext } from "@/shared/storage/storage-context"
import { clearOnboardingModelsCache } from "./core/controller/models/getForgeOSOnboardingModels"
...
import { ForgeOSTempManager } from "./services/temp"
...
/**
 * Performs intialization for ForgeOS that is common to all platforms.
 *
 * @param context
 * @returns The webview provider
 * @throws ForgeOSConfigurationError if endpoints.json exists but is invalid
 */
export async function initialize(storageContext: StorageContext): Promise<WebviewProvider> {
	...
	// ProviderSettingsManager, RuntimeOAuthTokenManager, and ForgeOS auth
	// flow through Logger.debug → ForgeOS output channel.
	// These components operate before/outside of ForgeOSCore sessions, so the
	...
	// Initialize ForgeOSEndpoint configuration (reads bundled and ~/.forgeos/endpoints.json if present)
	// This must be done before any other code that calls ForgeOSEnv.config()
	// Throws ForgeOSConfigurationError if config file exists but is invalid
	const { ForgeOSEndpoint } = await import("./config")
	await ForgeOSEndpoint.initialize(HostProvider.get().extensionFsPath)

	try {
		await StateManager.initialize(storageContext)
	} catch (error) {
		Logger.error("[ForgeOS] CRITICAL: Failed to initialize StateManager:", error)
	...
	if (!ForgeOSEndpoint.isSelfHosted()) {
	...
	ForgeOSTempManager.startPeriodicCleanup()
...
async function showVersionUpdateAnnouncement(stateManager: StateManager) {
	...
	const previousVersion = stateManager.getGlobalStateKey("forgeosVersion")
	// Perform post-update actions if necessary
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			Logger.log(`ForgeOS version changed: ${previousVersion} -> ${currentVersion}. First run or update detected.`)
			...
			if (lastShownAnnouncementId !== latestAnnouncementId) {
				...
				const message = previousVersion
					? `ForgeOS has been updated to v${currentVersion}`
					: `Welcome to ForgeOS v${currentVersion}`
				HostProvider.window.showMessage({
			...
			stateManager.setGlobalState("forgeosVersion", currentVersion)
		}
	} catch (error) {
 ...
 * If so, opens the ForgeOS sidebar and clears the state.
 */
async function checkWorktreeAutoOpen(stateManager: StateManager): Promise<void> {
	try {
		...
		if (arePathsEqual(currentPath, worktreeAutoOpenPath)) {
			...
			// Open the ForgeOS sidebar
			await HostProvider.workspace.openForgeOSSidebarPanel({})
		}
	} catch (error) {
...
/**
 * Performs cleanup when ForgeOS is deactivated that is common to all platforms.
 */
export async function tearDown(): Promise<void> {
	try {
		...
		ForgeOSTempManager.stopPeriodicCleanup()
	} finally {
		try {
			await StateManager.get().flushPendingState()
		} catch (error) {
			Logger.error("[ForgeOS] Failed to flush pending state during teardown:", error)
		}
	}
}
