 ...
 * Designed to be instantiated once in ForgeOS.ts and passed to file manipulation services.
 * Uses the 'ignore' library to support standard .gitignore syntax in .forgeosignore files.
 */
export class ForgeOSIgnoreController {
	private cwd: string
	private ignoreInstance: Ignore
	private fileWatcher?: FSWatcher
	forgeosIgnoreContent: string | undefined

	constructor(cwd: string) {
		this.cwd = cwd
		this.ignoreInstance = ignore()
		this.forgeosIgnoreContent = undefined
	}
	...
	async initialize(): Promise<void> {
		// Set up file watcher for .forgeosignore
		this.setupFileWatcher()
		await this.loadForgeOSIgnore()
	}

	/**
	 * Set up the file watcher for .forgeosignore changes
	 */
	private setupFileWatcher(): void {
		const ignorePath = path.join(this.cwd, ".forgeosignore")
		...
		this.fileWatcher.on("error", (error) => {
			Logger.error("Error watching .forgeosignore file:", error)
		})
	}

	/**
	 * Load custom patterns from .forgeosignore if it exists.
	 * Supports "!include <filename>" to load additional ignore patterns from other files.
	 */
	private async loadForgeOSIgnore(): Promise<void> {
		try {
			...
			if (await fileExistsAtPath(ignorePath)) {
				...
				this.forgeosIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".forgeosignore")
			} else {
				this.forgeosIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			Logger.error("Unexpected error loading .forgeosignore:", error)
		}
	}
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processForgeOSIgnoreIncludes(content)
	 ...
	 */
	private async processForgeOSIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			Logger.debug(`[ForgeOSIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .forgeosignore does not exist
		if (!this.forgeosIgnoreContent) {
			return true
		}
		try {
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .forgeosignore exists
		if (!this.forgeosIgnoreContent) {
			return undefined
		}
