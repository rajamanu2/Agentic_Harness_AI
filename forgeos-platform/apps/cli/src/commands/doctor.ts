...
function listMatchingProcesses(pattern: string): ProcessRecord[] {
	...
	// (e.g. the "--forgeos-hub-daemon" marker) are treated as patterns.
...
function resolveCliLogPath(): string {
	...
	return join(resolveForgeOSDataDir(), "logs", `${name}.log`);
}
...
export async function runDoctorCommand(
	...
	if (!fix) {
		...
		if (
			...
			io.writeln(
				"\nRun `forgeos doctor fix` to kill all stale local processes, including stale sidecars.",
			);
		}
		return 0;
	}
