...
import { ForgeOSDefaultTool, setDynamicToolUseNames } from "@shared/tools"
...
/** Default Directory for agent configurations: ~/Documents/ForgeOS/Agents */
...
const AgentBaseConfigSchema = z.object({
	...
	tools: z.array(z.nativeEnum(ForgeOSDefaultTool)).default([]),
...
function normalizeToolName(toolName: string): ForgeOSDefaultTool {
	const trimmed = toolName.trim()
	if (!trimmed) {
		throw new Error("Tool name cannot be empty.")
	}

	const asDefaultTool = trimmed as ForgeOSDefaultTool
	if (Object.values(ForgeOSDefaultTool).includes(asDefaultTool)) {
		return asDefaultTool
	}

	throw new Error(
		`Unknown tool '${trimmed}'. Expected a ForgeOSDefaultTool value (for example: read_file, list_files, search_files).`,
	)
}

function parseTools(tools: string | string[] | undefined): ForgeOSDefaultTool[] {
	if (!tools) {
		return []
	}
...
function getAgentsConfigPath(homeDir = os.homedir()): string {
	return path.join(homeDir, "Documents", "ForgeOS", AGENTS_CONFIG_DIRECTORY_NAME)
}
