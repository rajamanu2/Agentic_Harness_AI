...
/**
 * Sessions can run anywhere (ForgeOS-internal worktrees and plugin installs
 * under `.forgeos`, or a shell's default cwd like the home or Desktop
...
export function isExcludedWorkspacePath(path: string): boolean {
	...
	if (normalized.split(/[\\/]/).includes(".forgeos")) {
		return true;
	}
	return (
