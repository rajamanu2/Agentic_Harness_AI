...
Curated smoke tests that verify ForgeOS works correctly with LLM providers.
...
```bash
forgeos auth -p forgeos -k "$FORGEOS_API_KEY" -m anthropic/claude-sonnet-4.5
```
...
2. Add `config.json`:
   ```json
   {
     ...
     "prompt": "The task prompt for ForgeOS",
...
Smoke test CI is temporarily disabled while the build step is repointed at the SDK CLI. The remaining root script runs the scenarios against whichever `forgeos` binary is already on `$PATH`.

### Required Secrets

- `FORGEOS_API_KEY` - ForgeOS API key, needed when CI is re-enabled
