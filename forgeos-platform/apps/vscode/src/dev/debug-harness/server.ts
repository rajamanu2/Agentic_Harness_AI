 ...
 *
 * Launches VSCode with the ForgeOS extension in debug mode and provides
 * an HTTP API for:
...
const SCREENSHOT_DIR = path.join(os.tmpdir(), "forgeos-debug")
const DEFAULT_WORKSPACE = path.join(os.tmpdir(), "forgeos-debug-workspace")
const DEFAULT_FORGEOS_DIR = path.join(os.homedir(), ".forgeos2") // Separate profile from user's ~/.forgeos
...
const FORGEOS_DIR_ARG = getArg("--forgeos-dir") // Override the isolated FORGEOS_DIR
...
class DebugHarness {
	...
	private forgeosDir: string = DEFAULT_FORGEOS_DIR // The FORGEOS_DIR used for the debugee
	...
	async launch(opts: { workspace?: string; skipBuild?: boolean } = {}): Promise<any> {
		...
		// Resolve the FORGEOS_DIR for the debugee (separate from debugger's ~/.forgeos)
		this.forgeosDir = FORGEOS_DIR_ARG || DEFAULT_FORGEOS_DIR
		fs.mkdirSync(this.forgeosDir, { recursive: true })
		fs.mkdirSync(path.join(this.forgeosDir, "data"), { recursive: true })
		log(`Debugee FORGEOS_DIR: ${this.forgeosDir}`)

		// Clear any previously captured URLs
		this.capturedUrls = []
		// Also clear the captured URLs file on disk
		const captureFile = path.join(this.forgeosDir, "data", "debug-captured-urls.jsonl")
		try {
			fs.unlinkSync(captureFile)
		} catch {}

		// Create temp user data dir to avoid interfering with real VSCode profile
		const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "forgeos-debug-profile-"))
		...
		try {
			this.app = await _electron.launch({
				...
				env: {
					...
					FORGEOS_ENVIRONMENT: "production",
					// ── Data isolation: use separate profile from user's ~/.forgeos ──
					FORGEOS_DIR: this.forgeosDir,
					// ── Browser capture: intercept openExternal() for OAuth testing unless explicitly disabled ──
					FORGEOS_CAPTURE_BROWSER: BROWSER_CAPTURE ? "1" : "0",
					FORGEOS_DEBUG_HARNESS_PORT: String(PORT),
				},
	 ...
	 * The vsCodeApi is exposed on window.__forgeosVsCodeApi by platform.config.ts.
	...
	async webPostMessage(params: { message: any }): Promise<any> {
		...
		try {
			const result = await sidebar.evaluate((msg: any) => {
				const api = (window as any).__forgeosVsCodeApi
				if (!api) {
					throw new Error(
						"window.__forgeosVsCodeApi not found. " +
	...
	async uiOpenSidebar(): Promise<any> {
		if (!this.page) throw new Error("VSCode not running")
		try {
			await this.page.getByRole("tab", { name: /ForgeOS/ }).locator("a").click()
		} catch {
			// Activity bar might need a different approach
			await this.page.keyboard.press("Meta+Shift+p")
			await sleep(300)
			await this.page.keyboard.type("ForgeOS: Focus on ForgeOS View")
			await sleep(200)
	...
	async uiSendMessage(params: { text: string; images?: string[]; files?: string[]; responseType?: string }): Promise<any> {
		...
		return sidebar.evaluate(
			({
				...
				const api = (window as any).__forgeosVsCodeApi
				if (!api) {
					throw new Error("window.__forgeosVsCodeApi not found")
				}
	 ...
	 * FORGEOS_CAPTURE_BROWSER). Each entry has a timestamp and URL.
	...
	oauthReadStoredToken(): any {
		const secretsFile = path.join(this.forgeosDir, "data", "secrets.json")
		if (!fs.existsSync(secretsFile)) {
			return { found: false, path: secretsFile }
		}
		try {
			const data = JSON.parse(fs.readFileSync(secretsFile, "utf-8"))
			// Extract the ForgeOS account ID and related auth info
			const accountId = data["forgeos:forgeosAccountId"]
	 ...
	 *
	 * For ForgeOS OAuth: the SDK's local callback server captures the code
	 ...
	 *   provider - Provider name for /auth path (e.g., "forgeos", "oca")
		...
		try {
			...
			const result = await this.extCdp.send("Runtime.evaluate", {
				expression: `
					(() => {
						...
						// ui.command_palette with "ForgeOS: Handle URI" or similar.
			...
			return {
				...
				note:
					...
					"1. For ForgeOS OAuth (SDK local callback): the SDK captures the code " +
	...
	/**
	 * Read the captured URLs JSONL file from the debugee's FORGEOS_DIR.
