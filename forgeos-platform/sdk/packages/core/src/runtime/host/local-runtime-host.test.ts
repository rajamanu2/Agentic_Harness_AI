...
describe("LocalRuntimeHost", () => {
	...
	beforeEach(() => {
		...
		process.env.FORGEOS_DIR = join(isolatedHomeDir, ".forgeos");
	...
	afterEach(() => {
		...
		process.env.FORGEOS_DIR = envSnapshot.FORGEOS_DIR;
		if (envSnapshot.FORGEOS_DATA_DIR === undefined) {
			delete process.env.FORGEOS_DATA_DIR;
		} else {
			process.env.FORGEOS_DATA_DIR = envSnapshot.FORGEOS_DATA_DIR;
		}
		setHomeDir(envSnapshot.HOME ?? "~");
		setForgeOSDir(envSnapshot.FORGEOS_DIR ?? join("~", ".forgeos"));
	...
	it("emits session lifecycle telemetry when configured", async () => {
		...
		// `session.started` is emitted from `ForgeOSCore.start` (see
		// `ForgeOSCore.test.ts`), not from `LocalRuntimeHost`, so that the
