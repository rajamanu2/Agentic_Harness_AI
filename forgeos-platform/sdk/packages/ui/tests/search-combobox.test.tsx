...
const options = [
	{ label: "forgeos/forgeos", value: "forgeos" },
	{
		description: "Cloud dashboard",
		label: "forgeos/core-platform",
		value: "core-platform",
	},
];
...
describe("SearchCombobox", () => {
	it("filters and selects an option", async () => {
		...
		expect(trigger?.getAttribute("aria-label")).toBe("Repository: forgeos/forgeos");
		...
		expect(panel?.textContent).not.toContain("forgeos/forgeos");
		const match = Array.from(panel?.querySelectorAll("button") ?? []).find(
			(button) => button.textContent === "forgeos/core-platformCloud dashboard",
		);
	...
	it("renders loading and disabled states", async () => {
		...
		const optionButtons = container.querySelectorAll<HTMLButtonElement>(
			".forgeos-ui-search-combobox__option",
		);
