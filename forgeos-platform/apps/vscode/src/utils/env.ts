 ...
 *
 * When FORGEOS_CAPTURE_BROWSER is set (debug harness mode), the URL is captured
...
async function captureBrowserUrl(url: string): Promise<void> {
	...
	// Write to JSONL file in FORGEOS_DIR/data/
	try {
		const fs = await import("node:fs")
		const path = await import("node:path")
		const os = await import("node:os")
		const forgeosDir = process.env.FORGEOS_DIR || path.join(os.homedir(), ".forgeos")
		const dataDir = path.join(forgeosDir, "data")
	...
	const harnessPort = process.env.FORGEOS_DEBUG_HARNESS_PORT
	if (harnessPort) {
		try {
