...
async function startDaemon(): Promise<ReadyDaemon> {
	const dataDir = await mkdtemp(join(tmpdir(), "forgeos-hub-shutdown-e2e-"));
	...
	const child = spawn(
		...
		["--conditions=development", entryPath],
		{
			...
			env: {
				...
				FORGEOS_HUB_DISCOVERY_PATH: discoveryPath,
				FORGEOS_HUB_TEST_PORT: "0",
...
async function openAuthenticatedSocket(
	...
	const socket = new WebSocket(url, `forgeos-hub-auth.${authToken}`);
	await withTimeout(
