...
import type { ForgeOSFileStorage } from "@shared/storage/ForgeOSFileStorage"
import {
	applyTransform,
...
import { ForgeOSMemento } from "@/shared/storage"
import { StateManager } from "../StateManager"

// ─── File-backed storage readers (used by StateManager) ────────────────────

/**
 * Read secrets from a ForgeOSFileStorage instance.
 */
export function readSecretsFromStorage(store: ForgeOSFileStorage<string>): Secrets {
...
/**
 * Read workspace state from a ForgeOSFileStorage instance.
 */
export function readWorkspaceStateFromStorage(store: ForgeOSFileStorage): LocalState {
...
/**
 * Read global state from a ForgeOSFileStorage instance.
 */
export async function readGlobalStateFromStorage(store: ForgeOSMemento): Promise<GlobalStateAndSettings> {
	try {
