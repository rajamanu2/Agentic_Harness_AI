...
// stub API keeps working. `AuthService`/`ForgeOSAccountService` statics and the
...
describe("fetchRemoteConfig", () => {
	...
	let accountService: ForgeOSAccountService
	...
	beforeEach(() => {
		...
		accountService = new ForgeOSAccountService()
		sandbox.stub(ForgeOSAccountService, "getInstance").returns(accountService)
	...
	it("switches org when not in the chosen org", async () => {
		...
		const controller = {
			...
			stateManager: {
				getApiConfiguration: sandbox.stub().returns({ planModeApiProvider: "forgeos", actModeApiProvider: "forgeos" }),
