...
describe("SdkMessageCoordinator", () => {
	...
	it("appends messages to the current task", () => {
		...
		expect(task.messageStateHandler.getForgeOSMessages()).toEqual(messages)
	})
	...
	it("stamps seq and epoch from the shared minter on append", () => {
		...
		const a: ForgeOSMessage = { ts: 1, type: "say" as const, say: "text" as const, text: "a", partial: true }
		const b: ForgeOSMessage = { ts: 2, type: "say" as const, say: "text" as const, text: "b", partial: false }
	...
	it("gives an updated message a NEWER seq than its earlier copy (partial -> final)", () => {
		...
		const partial: ForgeOSMessage = { ts: 10, type: "say" as const, say: "text" as const, text: "Hel", partial: true }
		...
		const final: ForgeOSMessage = { ts: 10, type: "say" as const, say: "text" as const, text: "Hello", partial: false }
	...
	it("reflects a bumped epoch on subsequent messages", () => {
		...
		const before: ForgeOSMessage = { ts: 1, type: "say" as const, say: "text" as const, text: "before", partial: false }
		coordinator.appendMessages([before])
		minter.bumpEpoch()
		const after: ForgeOSMessage = { ts: 2, type: "say" as const, say: "text" as const, text: "after", partial: false }
	...
	it("leaves messages unstamped when no minter is wired (classic/legacy)", () => {
		const task = createTaskProxy("session-123", vi.fn(), vi.fn())
		const coordinator = new SdkMessageCoordinator({ getTask: () => task })
		const m: ForgeOSMessage = { ts: 1, type: "say" as const, say: "text" as const, text: "x", partial: false }
