...
describe("createCliCore", () => {
	...
	const envSnapshot = {
		FORGEOS_RPC_ADDRESS: process.env.FORGEOS_RPC_ADDRESS,
		FORGEOS_SESSION_BACKEND_MODE: process.env.FORGEOS_SESSION_BACKEND_MODE,
		FORGEOS_VCR: process.env.FORGEOS_VCR,
	};
	...
	beforeEach(() => {
		...
		delete process.env.FORGEOS_RPC_ADDRESS;
		delete process.env.FORGEOS_SESSION_BACKEND_MODE;
		delete process.env.FORGEOS_VCR;
		featureFlagsPoll.mockClear();
		featureFlagsDispose.mockClear();
	});

	afterEach(() => {
		process.env.FORGEOS_RPC_ADDRESS = envSnapshot.FORGEOS_RPC_ADDRESS;
		process.env.FORGEOS_SESSION_BACKEND_MODE =
			envSnapshot.FORGEOS_SESSION_BACKEND_MODE;
		process.env.FORGEOS_VCR = envSnapshot.FORGEOS_VCR;
	});

	it("passes hub client metadata through without forcing hub mode", async () => {
		process.env.FORGEOS_RPC_ADDRESS = "127.0.0.1:5001";
	...
	it("passes env-managed routing through to core when local is requested via env", async () => {
		process.env.FORGEOS_SESSION_BACKEND_MODE = "local";
