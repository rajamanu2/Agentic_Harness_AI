 ...
 *
 * Sends a Notification Center alert when a ForgeOS run completes successfully.
 *
 * CLI usage:
 *   forgeos plugin install https://github.com/forgeos/forgeos/blob/main/sdk/examples/plugins/mac-notify.ts --cwd .
 *   forgeos -i "Run the test suite"
 */

import { execFile } from "node:child_process";
import type { AgentPlugin, AgentRunResult } from "@forgeos/core";
...
const plugin: AgentPlugin = {
	...
	hooks: {
		afterRun({ result }) {
			...
			sendMacNotification("ForgeOS session completed", summarizeResult(result));
		},
	},
};
