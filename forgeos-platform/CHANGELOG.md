...
- Stop two ForgeOS installations on different builds from shutting each other's Hub daemon down in a loop, which killed live sessions with an abnormal socket close. Build identity is now compared through a total order, so at most one side of a pair can decide to retire the other.
...
- Don't discard a successfully refreshed ForgeOS token when the old one was already past expiry, which made the first request after a long idle period fail despite valid credentials.
...
- Report involuntary ForgeOS logouts (a rejected refresh token) instead of clearing credentials silently.
...
- Offer `meta/muse-spark-1.2-contributor` on the ForgeOS provider, alongside a refreshed model catalog.
...
- Retry empty model responses on every provider rather than only Ollama, fixing hard "Model returned empty response" failures on OpenRouter, ForgeOS, and OpenAI-compatible endpoints.
...
- Show proper display names for ForgeOS free models and recommended models in the model picker.
...
- Add support for free ForgeOS models, shown as "(free)" in the model picker, with a dedicated error card that includes the reset time when the free limit is reached.
...
- Add Claude Opus 5 across the Anthropic, Claude Code, Bedrock, Vertex, ForgeOS, and OpenRouter providers, including 1M context window variants.
...
- Add telemetry to track when ForgeOS reaches the consecutive mistake limit.
...
- Soften and shorten the message shown when ForgeOS hits the consecutive mistake limit.
...
- Add a ForgeOSPass limit-reached error with a one-click option to switch to ForgeOS usage-based billing.
- Allow selecting ForgeOS free models on the ForgeOSPass provider, organized into Subscribed and Free tabs with model descriptions.

### Changed

- Refine ForgeOSPass onboarding and provider settings copy, and open the "learn more" link via the in-app URL handler.
- Remove the ForgeOS model picker recommendation copy.
...
- Fully remove the ForgeOSPass feature flag so ForgeOSPass is available everywhere in the UI — onboarding, settings, the welcome promo banner, and the credit-limit "Switch to ForgeOSPass" action.

## [4.0.3]

### Changed

- Enable the ForgeOSPass provider for all users by removing the feature-flag gate that previously fell back to the standard ForgeOS provider.
...
- Improve the ForgeOSPass provider experience with clearer reasoning controls and model selection.

### Fixed

- Show reasoning effort controls for ForgeOSPass models and align ForgeOSPass model resolution with the rest of the provider.
- Prefer canonical ForgeOS Z.ai model ids and polish ForgeOSPass and Z.ai model metadata.
...
- Add the SDK-backed VS Code extension runtime. ForgeOS now runs tasks through the shared ForgeOS SDK session layer for agent turns, tools, Plan/Act mode coordination, MCP, checkpoints, telemetry, provider changes, compaction, mistake limits, and task history.
- Add ForgeOSPass to the VS Code extension, including onboarding, provider selection, signup and subscription handoff, live model lists, entitlement and organization error states, out-of-credit prompts, and clearer ForgeOSPass auth/error handling.
...
- ForgeOS Plugins: Plugins let you extend ForgeOS with custom tools, workflows, skills, and MCP-powered capabilities tailored to your team or project. Install them from the new Customize marketplace to add specialized behavior, connect external services, and package reusable automations—so ForgeOS can do more than code: it can adapt to the way you work.
- Add queued prompts in chat. Messages submitted while ForgeOS is already working are now queued, shown while the current turn streams, and can be cancelled before they run.
...
- Add additional SDK-backed provider exposure and model/provider updates, including ForgeOSPass models, refreshed ForgeOS catalog data, Fireworks GLM 5.2, Kimi K2.6 Fast, Kimi K2.7 Code, Qwen 3.7 Plus, MiniMax M3 updates, SAP AI Core wiring, LiteLLM model fetching, Codex OAuth credentials, and OpenAI-compatible model settings.
- Add MCP support for plugins and shared marketplace install/uninstall plumbing used by the VS Code extension.

### Changed

- Migrate the VS Code extension from the legacy task implementation to the shared ForgeOS SDK and move the extension build/package workflow to Bun.
...
- Add a debug section in settings for ForgeOS testers.
...
- Always use the upstream ForgeOS recommended models endpoint instead of gating it behind a feature flag.
...
- Show prompt-cache support for Qwen 3.7 Max in the ForgeOS provider.
...
- Remove the ForgeOS Kanban launch modal and bundled demo media from the VS Code extension startup flow.
...
- Improve forgeos-core memory diagnostics used by the extension runtime:
...
- Add dedicated "Quota Exceeded" error message in the chat error UI when ForgeOS account spend caps are hit

### Fixed

- Fix OOM crashes during long conversations by setting `--max-old-space-size=8192` for the forgeos-core Node.js process (was defaulting to ~2 GB)
...
- Fix cache reflection for ForgeOS and Vercel API handlers
...
- Add ForgeOS Kanban launch modal in webview; CLI now launches Kanban by default with a migration view
...
### Added
- Implement dynamic free model detection for ForgeOS API
- Add file read deduplication cache to prevent repeated reads
- Add feature tips tooltip during thinking state

### Fixed
- Replace error message when not logged in to ForgeOS
- Align ForgeOSRulesToggleModal padding with ServersToggleModal
...
- Improved parallel tool calling support for OpenRouter and ForgeOS providers
...
- New ForgeOS API docs: Getting Started, Auth, Chat Completions, Models, Errors, and SDK Examples
...
- Add `User-Agent` header to requests sent to the ForgeOS backend
- Add default auto-tag workflow for publish release flow
- Show ForgeOS SDK docs on the ForgeOS page
...
- Add dynamic ForgeOS provider model fetching from ForgeOS endpoint
...
- Fetch model info from API in CLI headless auth for ForgeOS and Vercel providers
...
- Added ForgeOS SDK API interface for programmatic access to ForgeOS features and tools, enabling integration into custom applications.
...
- Pull ForgeOS's recommended models from internal endpoint
...
- Fix ForgeOS auth with ACP flag
...
- ForgeOS CLI 2.0 now available. Install with `npm install -g forgeos`
- Anthopic Opus 4.6
...
- **CLI authentication:** Added Vercel AI Gateway and ForgeOS API key provider support for headless CI/automation workflows
...
- **CLI auth:** Fixed `forgeos auth` displaying incorrect provider information after configuration
...
- Users with ChatGPT Plus or Pro subscriptions can now use GPT-5 models directly through ForgeOS without needing an API key. Authentication is handled via OAuth through OpenAI's authentication system.
- Grok models are now moving out of free tier and into paid plans.
- Introduces comprehensive Jupyter Notebook support for ForgeOS, enabling AI-assisted editing of `.ipynb` files with full cell-level context awareness.
...
- Add version headers to ForgeOS backend requests
...
- Add new websearch tooling in ForgeOS provider
...
- Fix Openrouter and ForgeOS Provider model info
...
- Add: Hooks allow you to inject custom logic into ForgeOS's workflow
...
- Remove ForgeOS/code-supernova-1-million model
- Changes to allow users to manually enter model names (eg. presets) when using OpenRouter

## [3.34.0]

- ForgeOS Teams is now free through 2025 for unlimited users. Includes Jetbrains, RBAC, centralized billing and more.
- Use the “exacto” versions of GLM-4.6, Kimi-K2, and Qwen3-Coder in the ForgeOS provider for the best balance of cost, speed, accuracy and tool-calling.
...
- Add Claude Sonnet 4.5 to ForgeOS/OpenRouter/Anthropic providers
- Add /task deep link handler

## [3.32.1]

- Preserve reasoning traces for ForgeOS/OpenRouter/Anthropic providers to maintain conversation integrity
- Add automatically retry on rate limit errors with SAP AI Core provider
- Fix ForgeOS accounts using stale id token at refresh response
...
- Changes to inform ForgeOS about commands that are available on your system
...
- YOLO Mode: Enable in settings to let ForgeOS approve all actions and automatically switch between plan/act mode
...
- Fix: Updated insufficient balance URL for easy ForgeOS balance top-ups
- Accessibility: Improvements to screen readers in MCP, ForgeOS Rules, workflows, and history views.
...
- Synchronized Task History: Real-time task history synchronization across all ForgeOS instances
...
- Add 200k context window variant for Claude Sonnet 4 to OpenRouter and ForgeOS providers

## [3.26.6]

- Add free Grok Coder model to ForgeOS provider for users looking for a fast, free coding model option
...
- Fix OpenRouter context window error handling to properly extract error codes from error messages, resolving "Unexpected API Response" errors with GPT-5 on ForgeOS provider
- Fix GPT-5 context window configuration for OpenAI/OpenRouter/ForgeOS providers to use correct 272K limit
...
- Add ForgeOS Sonic Alpha model - experimental advanced model with 262K context window for complex coding tasks
...
- Fix OpenRouter showing forgeos credits error after 402 response
...
- **Focus Chain:** Automatically creates and maintains todo lists as you work with ForgeOS, breaking down complex tasks into manageable steps with real-time progress tracking
...
- Add support for 200k context window for Claude Sonnet 4 in OpenRouter and ForgeOS providers
...
- Fix bug where running out of credits on ForgeOS accounts would show '402 empty body' response instead of 'buy credits' component
...
- Fix prompt caching support for Opus 4.1 on OpenRouter/ForgeOS
...
- Fixed terminal bug where ForgeOS failed to capture output of certain fast-running commands
...
- Add user role display in organization UI for ForgeOS account users
...
- Show request ID in error messages returned by ForgeOS Accounts API to help debug user reported issues
...
- Add Kimi-K2 as a recommended model in the ForgeOS Provider, and route to Together/Groq for 131k context window and high throughput
- Added API Key support for Bedrock integration

## [3.18.14]

- Fix bug where ForgeOS account users logged in with invalid token would not be shown as logged out in webview presentation layer

## [3.18.13]

- Fix authentication issue where ForgeOS accounts users would keep getting logged out or seeing 'Unexpected API response' errors

## [3.18.12]

- Fix flaky organization switching behavior in ForgeOS provider that caused UI inconsistencies and double loading
- Fix insufficient credits error display to properly show error messages when account balance is too low
- Improve credit balance validation and error handling for ForgeOS provider requests

## [3.18.11]

- Fix authentication issues with ForgeOS provider by ensuring the client always uses the latest auth token
...
- Fix streaming reliability issues with ForgeOS provider that could cause connection problems during long conversations
- Fix authentication error handling for ForgeOS provider to show clearer error messages when not signed in and prevent recursive failed requests
...
- Update request header to include `"ai-client-type": "ForgeOS"` to SAP Api Provider
...
- Fix logging in with ForgeOS account not getting past welcome screen
...
- Optimized ForgeOS to work with the Claude 4 family of models, resulting in improved performance, reliability, and new capabilities
...
- Optimized ForgeOS to work with the Gemini 2.5 family of models
...
- Add configurable default terminal profile setting, allowing users to specify which terminal ForgeOS should use (Thanks @valinha!)
...
- Exclude .forgeosrules files from checkpoint tracking

## [3.17.13]

- Add Thinking UX for Gemini models, providing visual feedback during model reasoning
- Add support for Notifications MCP integration with ForgeOS
...
- **Free Grok Model Available!** Access Grok 3 completely free through the ForgeOS provider
...
- Add VSCode walkthrough to help new users get started with ForgeOS
...
- Aligning ForgeOS to work with Claude 4 model family (Experimental)
...
- Add prompt caching for Claude 4 models on ForgeOS and OpenRouter providers
...
- Add keyboard shortcut (Cmd+') to quickly focus ForgeOS from anywhere in VS Code
- Add lightbulb actions for selected text with options to "Add to ForgeOS", "Explain with ForgeOS", and "Improve with ForgeOS"
- Automatically focus ForgeOS window after extension updates
...
- Update Gemini in OpenRouter/ForgeOS providers to use implicit caching
...
- Add ability to allow the user to type their next message into the chat while ForgeOS is taking action
- Add ability to generate commit message via forgeos (Thanks zapp88!)
- Add improvements to caching for gemini models on OpenRouter and ForgeOS providers
...
- Update file size that can be read by ForgeOS, allowing larger files
...
- Add support for custom API request timeout. Timeouts were 15-30s, but can now be configured via settings for OpenRouter/ForgeOS & Ollama (Thanks @WingsDrafterwork!)
...
- Add cache ui for open router and forgeos provider
...
- Create forgeosrules folder when creating new rule if it's needed
...
- Add `/compact` command
- Add prompt caching to gemini models in forgeos / openrouter providers
...
- Add !include .file directive support for .forgeosignore (Thanks watany-dev!)
...
- Add ForgeOS rules popover under the chat field, allowing you to easily add, enable & disable workspace level or global rule files
- Add new slash command menu letting you type “/“ to do quick actions like creating new tasks
- Add ability to edit past messages, with options to restore your workspace back to that point
- Allow sending a message when selecting an option provided by the question or plan tool
- Add command to jump to ForgeOS's chat input
...
- Add the ability to fetch from global forgeos rules files
...
- Add favorite toggles for models when using the ForgeOS & OpenRouter providers
- Add auto-approve options for edits/reads outside of the workspace
- Improve diff editing animation for large files
- Add indicator showing number of diff edits when ForgeOS edits a file
...
- Add more robust error tracking for users opted in to telemetry (thank you for helping us make ForgeOS better!)

## [3.10.1]

- Add CMD+' keyboard shortcut to add selected text to ForgeOS
- ForgeOS now auto focuses the text field when using 'Add to ForgeOS' shortcut
- Add new 'Create New Task' tool to let ForgeOS start a new task autonomously!
...
- Add drag and drop of file/folders into forgeos chat (Thanks eljapi!)
...
- Add recommended models for ForgeOS provider
- Add ability to detect when user edits files manually so ForgeOS knows to re-read, leading to reduced diff edit errors
...
- Fix issue with ForgeOS accounts not showing user info in popout tabs
...
- Fix bug where ForgeOS would use plan_mode_response bug without response parameter
...
- Add 'Add to ForgeOS' as an option when you right-click in a file or the terminal, making it easier to add context to your current task
- Add 'Fix with ForgeOS' code action - when you see a lightbulb icon in your editor, you can now select 'Fix with ForgeOS' to send the code and associated errors for ForgeOS to fix. (Cursor users can also use the 'Quick Fix (CMD + .)' menu to see this option)
- Add Account view to display billing and usage history for ForgeOS account users. You can now keep track of credits used and transaction history right in the extension!
- Add 'Sort underling provider routing' setting to ForgeOS/OpenRouter allowing you to sort provider used by throughput, price, latency, or the default (combination of price and uptime)
- Improve rich MCP display with dynamic image loading and support for GIFs
- Add 'Documentation' menu item to easily access ForgeOS's docs
- Add OpenRouter's new usage_details feature for more reliable cost reporting
- Display total space ForgeOS takes on disk next to 'Delete all Tasks' button in History view
- Fix 'Context Window Exceeded' error for OpenRouter/ForgeOS Accounts (additional support coming soon)
...
- ForgeOS now displays selectable options when asking questions or presenting a plan, saving you from having to type out responses!
- Add support for a `.forgeosrules/` directory to load multiple files at once (thanks @ryo-ma!)
- Prevent ForgeOS from reading extremely large files into context that would overload context window
...
- Add ForgeOS API as a provider option, allowing new users to sign up and get started with ForgeOS for free
...
- Improve OpenRouter/ForgeOS error reporting
...
- Add .forgeosignore to block ForgeOS from accessing specified file patterns
...
- Add Plan/Act mode toggle to let you plan tasks with ForgeOS before letting him get to work
...
- Add ability to change viewport size and headless mode when ForgeOS asks to launch the browser
...
- Fix bug where ForgeOS couldn't read "@/" import path aliases from tool results
...
- Added checkpoints: Snapshots of workspace are automatically created whenever ForgeOS uses a tool
...
- ForgeOS now uses a search & replace diff based approach when editing large files to prevent code deletion issues.
...
- Adds ability to enable system notifications for when ForgeOS needs approval or completes a task.
- Adds support for a root-level `.forgeosrules` file that can be used to specify custom instructions for the project.

## [2.2.0]

- Add support for Model Context Protocol (MCP), enabling ForgeOS to use custom tools like web-search tool or GitHub tool
- Add MCP server management tab accessible via the server icon in the menu bar
- Add ability for ForgeOS to dynamically create new MCP servers based on user requests (e.g., "add a tool that gets the latest npm docs")
...
- Add stricter prompt to prevent ForgeOS from editing files during a browser session without first closing the browser

## [2.1.0]

- ForgeOS now uses Anthropic's new "Computer Use" feature to launch a browser, click, type, and scroll. This gives him more autonomy in runtime debugging, end-to-end testing, and even general web use. Try asking "Look up the weather in Colorado" to see it in action! (Available with Claude 3.5 Sonnet v2)
...
- Fix bug where modifying ForgeOS's edits would lead him to try to re-apply the edits
...
- Update system prompt to try to prevent ForgeOS from lazy coding (`// rest of code here...`)
...
- Update URLs to https://github.com/forgeos/forgeos

## [2.0.5]

- Fixed bug where ForgeOS's edits would stream into the active tab when switching tabs during a write_to_file
...
- New name! Meet ForgeOS, an AI assistant that can use your CLI and Editor
- Responses are now streamed with a yellow text decoration animation to keep track of ForgeOS's progress as he edits files
- New Cancel button to give ForgeOS feedback if he goes off in the wrong direction, giving you more control over tasks
