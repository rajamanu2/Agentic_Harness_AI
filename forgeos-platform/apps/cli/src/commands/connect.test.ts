...
import {
	FORGEOS_CONNECTOR_DETACHED_CHILD_ENV,
...
describe("runConnectAdapter", () => {
	const previousDetachedChild = process.env[FORGEOS_CONNECTOR_DETACHED_CHILD_ENV];
	const io: ConnectIo = {
		writeln: vi.fn(),
		writeErr: vi.fn(),
	};
	...
	beforeEach(() => {
		...
		mocks.run.mockImplementation(
			async (_args: string[], _io: ConnectIo, context: ConnectRunContext) => {
				context.setPersistenceInstanceId("forgeos_bot");
				return 0;
			},
		);
	...
	afterEach(() => {
		if (previousDetachedChild === undefined) {
			delete process.env[FORGEOS_CONNECTOR_DETACHED_CHILD_ENV];
		} else {
			process.env[FORGEOS_CONNECTOR_DETACHED_CHILD_ENV] = previousDetachedChild;
		}
	});
	...
	it("leaves persistence unchanged when an internal detached child exits", async () => {
		process.env[FORGEOS_CONNECTOR_DETACHED_CHILD_ENV] = "1";
	...
	it("validates a replacement before stopping the active instance", async () => {
		...
		mocks.listActiveConnectors.mockReturnValue([
			{
				id: "telegram:forgeos_bot",
				type: "telegram",
				instanceId: "forgeos_bot",
				pid: 123,
				hubUrl: "ws://127.0.0.1:4317",
				botUsername: "forgeos_bot",
			},
		]);
		mocks.getConnector.mockResolvedValue({
			name: "telegram",
	...
	it("restores the last successful launch when a replacement fails", async () => {
		...
		expect(stopInstance).toHaveBeenCalledWith("forgeos_bot", io);
	...
	it("does not count an already-running instance as a successful replacement", async () => {
		...
		expect(io.writeErr).toHaveBeenCalledWith(
			"[connect] replacement was not started because telegram instance forgeos_bot is still running",
		);
		expect(mocks.persistConnectorConnection).not.toHaveBeenCalled();
	});
});
...
describe("runCleanupConnectorInstance", () => {
	...
	it("reaps one instance without disabling its autostart", async () => {
		...
		await expect(
			runCleanupConnectorInstance("slack", "forgeos-slack", io),
		).resolves.toBe(0);

		expect(stopInstance).toHaveBeenCalledWith("forgeos-slack", io);
...
describe("hub-delegated connector starts", () => {
	...
	beforeEach(() => {
		...
		mocks.getConnector.mockResolvedValue({
			...
			validate: mocks.validate,
			showHelp: vi.fn(),
			resolveInstanceId: () => "forgeos-slack",
		});
	...
	afterEach(() => {
		delete process.env.FORGEOS_CONNECTOR_SUPERVISED;
	});
	...
	it("runs locally inside a supervised process instead of asking the hub again", async () => {
		process.env.FORGEOS_CONNECTOR_SUPERVISED = "1";
	...
	it("falls back to a local start when the hub deforgeoss", async () => {
		mocks.startConnectorViaHub.mockResolvedValue({
			delegated: false,
