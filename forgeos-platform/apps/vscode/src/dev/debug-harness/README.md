...
An HTTP-controlled debug server for the ForgeOS VSCode extension. Provides
...
- **Data isolation**: separate `~/.forgeos2` profile so debugee doesn't interfere with debugger
...
Options:
  ...
  --workspace PATH    Workspace directory to open (default: /tmp/forgeos-debug-workspace)
  --port PORT         Server port (default: 19229)
  --forgeos-dir PATH    Override the debugee's FORGEOS_DIR (default: ~/.forgeos2)
```
...
The debugee runs with `FORGEOS_DIR=~/.forgeos2` by default, keeping its data
separate from your real `~/.forgeos`. This prevents:
...
The isolated FORGEOS_DIR is reported in `status()` and `launch()` responses:

```bash
curl localhost:19229/api -d '{"method":"status"}'
# → { "forgeosDir": "/Users/you/.forgeos2", ... }
```

To use a different directory: `--forgeos-dir /tmp/test-forgeos-dir`

## Browser Capture & OAuth Testing

When the debug harness launches VSCode, it sets `FORGEOS_CAPTURE_BROWSER=1`
which intercepts all `openExternal()` calls in the debugee. Instead of
opening a real browser, URLs are:

1. **Logged to disk** at `$FORGEOS_DIR/data/debug-captured-urls.jsonl`
...
### Testing ForgeOS OAuth (login flow)

The ForgeOS OAuth flow uses the SDK's local callback server. When the user
...
curl localhost:19229/api -d '{"method":"ui.locator","params":{"text":"Login to ForgeOS","frame":"sidebar","action":"click"}}'

# 2. Check captured URLs to find the authorization URL
curl localhost:19229/api -d '{"method":"oauth.captured_urls"}'
# → { "urls": [{ "url": "https://api.forgeos.bot/auth/authorize?callback_url=http://127.0.0.1:PORT/..." }] }
...
# → { "found": true, "hasAccountId": true, "keys": ["forgeos:forgeosAccountId"] }
...
#    handler module. Instead, call the __forgeosHandleUri hook (see below):
curl localhost:19229/api -d '{
  "method": "ext.evaluate",
  "params": {
    "awaitPromise": true,
    "expression": "globalThis.__forgeosHandleUri(\"vscode://saoudrizwan.claude-dev/mcp-auth/callback/HASH?code=REAL_CODE&state=SAVED_STATE\")"
  }
}'
...
> **`globalThis.__forgeosHandleUri(url)` — debug-only URI delivery hook.**
> Registered in `src/extension.ts` during activation, **only** when
> `FORGEOS_CAPTURE_BROWSER` is set (which the harness always sets), so it never
...
|---------|--------------|
| `forgeos.accountButtonClicked` | Account / sign-in view |
| `forgeos.historyButtonClicked` | Task history view |
| `forgeos.settingsButtonClicked` | Settings view |
| `forgeos.mcpButtonClicked` | MCP servers view |
| `forgeos.plusButtonClicked` | New task (chat view) |
| `forgeos.worktreesButtonClicked` | Worktrees view |

```bash
# Navigate to account view
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"forgeos.accountButtonClicked"}}'

# Navigate to history view
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"forgeos.historyButtonClicked"}}'

# Navigate to settings view
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"forgeos.settingsButtonClicked"}}'

# Navigate to MCP view
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"forgeos.mcpButtonClicked"}}'

# Start a new task (return to chat view)
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"forgeos.plusButtonClicked"}}'
```
...
# 3. Check status (verify FORGEOS_DIR, browser capture, etc.)
curl localhost:19229/api -d '{"method":"status"}'
...
| `ui.open_sidebar` | | Open the ForgeOS sidebar |
...
`globalThis.__forgeosHandleUri("vscode://saoudrizwan.claude-dev/...?code=...&state=...")`.
It invokes the same `SharedUriHandler.handleUri` as VSCode's real URI handler
and is registered only when `FORGEOS_CAPTURE_BROWSER` is set (never in prod). See
...
3. **Data Isolation**: Sets `FORGEOS_DIR=~/.forgeos2` in the debugee's environment, ensuring
   the debugee uses a completely separate data directory from the user's real `~/.forgeos`.
...
4. **Browser Capture**: Sets `FORGEOS_CAPTURE_BROWSER=1` and `FORGEOS_DEBUG_HARNESS_PORT=19229`
   in the debugee's environment. When `openExternal()` is called in `src/utils/env.ts`, it
   checks for `FORGEOS_CAPTURE_BROWSER` and, if set, logs the URL to a JSONL file and POSTs
...
**⚠️ Data Isolation**: The debugee uses `~/.forgeos2` by default. If you need to test with
existing data from your real `~/.forgeos`, copy it: `cp -r ~/.forgeos ~/.forgeos2`. Be aware that
secrets (API keys, auth tokens) will be shared if you do this.

**⚠️ "Introducing ForgeOS Kanban" overlay**: On fresh launches, a full-screen promo overlay may
...
**Screenshots**: `ui.screenshot` and `ui.sidebar_screenshot` save PNG files to `/tmp/forgeos-debug/`
...
to open. For ForgeOS OAuth, the SDK's local callback server is still running and will accept
...
**ForgeOS OAuth with invalid codes**: If you simulate the OAuth callback with a fake code, the
...
**"Sidebar frame not found"**: The ForgeOS sidebar isn't open. Use `ui.open_sidebar` first.
...
**Screenshots directory**: Saved to `/tmp/forgeos-debug/` (configurable via SCREENSHOT_DIR).

**Debugee still uses ~/.forgeos**: Check that `FORGEOS_DIR` appears in the `status()` response.
