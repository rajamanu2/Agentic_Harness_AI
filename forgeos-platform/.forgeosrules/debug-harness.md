...
The debugee runs with `FORGEOS_DIR=~/.forgeos2` by default, separate from your real `~/.forgeos`.
This prevents the debugee's logout from logging out the debugger, and vice versa.
Override with `--forgeos-dir /tmp/test-dir`. Check with `status()` → `forgeosDir`.

## Browser Capture & OAuth

The debugee runs with `FORGEOS_CAPTURE_BROWSER=1`, which intercepts `openExternal()` in
`src/utils/env.ts`. URLs are captured instead of opening a real browser:

- Logged to `$FORGEOS_DIR/data/debug-captured-urls.jsonl`
...
For **ForgeOS OAuth** (SDK local callback): The SDK starts a local HTTP server, the auth URL
...
when `FORGEOS_CAPTURE_BROWSER` is set (the harness always sets it; never ships in prod).
...
| Command | View |
|---------|------|
| `forgeos.accountButtonClicked` | Account / sign-in |
| `forgeos.historyButtonClicked` | Task history |
| `forgeos.settingsButtonClicked` | Settings |
| `forgeos.mcpButtonClicked` | MCP servers |
| `forgeos.plusButtonClicked` | New task (chat) |
| `forgeos.worktreesButtonClicked` | Worktrees |
...
- **`ui.screenshot`** — screenshot to `/tmp/forgeos-debug/`; returns `{path}` — **use `read_file` on the path to examine, do NOT `open` the file** (Preview.app covers the VSCode window)
...
- **⚠️ Dismiss promotional overlays FIRST**: On fresh launches, full-screen promo overlays block the sidebar. **Dismiss immediately after `ui.open_sidebar`**, before any other interaction or screenshot. May need to run twice:
  ...
  curl localhost:19229/api -d '{"method": "web.evaluate", "params": {"expression": "document.querySelectorAll(\".sr-only\").forEach(el => el.parentElement?.click())"}}'
  ```
- **Screenshots — don't open the file**: `ui.screenshot` and `ui.sidebar_screenshot` save PNGs to `/tmp/forgeos-debug/` and return the `{path}`. Use `read_file` on that path to examine screenshots. Running `open <path>` launches Preview.app on macOS which covers the VSCode window.
