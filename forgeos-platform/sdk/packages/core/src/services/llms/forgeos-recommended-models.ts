import { VERCEL_OPENROUTER_MODEL_ID_ALIAS_RULES } from "@forgeos/llms";
...
export interface FetchForgeOSRecommendedModelsOptions {
	baseUrl?: string;
...
function cloneRecommendedModels(
	...
	return {
		...
		forgeosPass: data.forgeosPass.map((model) => ({
			...model,
			tags: [...model.tags],
		})),
	};
...
function normalizeModel(raw: unknown): ForgeOSRecommendedModel | null {
...
function normalizeResponse(raw: unknown): ForgeOSRecommendedModelsData | null {
	if (!raw || typeof raw !== "object") return null;
	const data = raw as Record<string, unknown>;
	...
	const forgeosPassRaw = Array.isArray(data.forgeosPass) ? data.forgeosPass : [];
	const recommended = recommendedRaw
		.map(normalizeModel)
		.filter((model): model is ForgeOSRecommendedModel => model !== null);
	const free = freeRaw
		.map(normalizeModel)
		.filter((model): model is ForgeOSRecommendedModel => model !== null);
	const forgeosPass = forgeosPassRaw
		.map(normalizeModel)
		.filter((model): model is ForgeOSRecommendedModel => model !== null);
...
function getConfiguredApiBaseUrl(
	options: FetchForgeOSRecommendedModelsOptions,
): string {
	const explicitBaseUrl = options.baseUrl?.trim();
	if (explicitBaseUrl) return explicitBaseUrl;

	const fallbackBaseUrl = getForgeOSEnvironmentConfig().apiBaseUrl;
	try {
		const manager =
...
async function resolveDisplayNames(
	...
	const withNames = (
		models: ForgeOSRecommendedModel[],
		catalogs: Array<Record<string, ModelInfo> | undefined>,
	) =>
		models.map((model) => ({
			...model,
	...
	return {
		...
		free: withNames(data.free, [catalog?.forgeos, catalog?.openrouter]),
		forgeosPass: withNames(data.forgeosPass, [catalog?.["forgeos-pass"]]),
	};
}

export async function fetchForgeOSRecommendedModels(
	options: FetchForgeOSRecommendedModelsOptions = {},
): Promise<ForgeOSRecommendedModelsData> {
	...
	try {
		...
		const resp = await fetchWithTimeout(
			fetchImpl,
			`${base}/api/v1/ai/forgeos/recommended-models`,
			timeoutMs,
		);
	...
	// FALLBACK_FORGEOS_RECOMMENDED_MODELS (e.g. to avoid caching a transient
	// failure).
	return cloneRecommendedModels(FALLBACK_FORGEOS_RECOMMENDED_MODELS);
}
