...
import { disposeAll, initVcr } from "@forgeos/shared";
...
const targets: ProviderVcrTarget[] = [
	{
		...
		cassetteName: "forgeos-anthropic-sonnet.json",
	},
	{
...
function readEnvironmentApiKey(providerId: string): string | undefined {
	switch (providerId) {
		...
		case "forgeos":
			return readString(process.env.FORGEOS_API_KEY);
		default:
			return undefined;
	}
}
...
function formatApiKey(providerId: string, apiKey: string): string {
	if (providerId === "forgeos" && !apiKey.startsWith("workos:")) {
...
function configureVcr(cassettePath: string): void {
	...
	process.env.FORGEOS_VCR = RECORD_MODE ? "record" : "playback";
	process.env.FORGEOS_VCR_CASSETTE = cassettePath;
	process.env.FORGEOS_VCR_INCLUDE_REQUEST_BODY = "1";
	process.env.FORGEOS_VCR_FILTER = "";
	initVcr(process.env.FORGEOS_VCR);
}
