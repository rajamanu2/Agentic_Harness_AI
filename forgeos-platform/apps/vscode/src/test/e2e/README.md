...
This directory contains the end-to-end tests for the ForgeOS VS Code extension using Playwright. These tests simulate user interactions with the extension in a real VS Code environment.
...
- **`fixtures/server/`** - Mock API server for testing ForgeOS's backend interactions
...
e2e("Test description", async ({ sidebar, helper, page }) => {
  ...
  await inputbox.fill("Hello, ForgeOS!")
...
- **`sidebar`** - Playwright Frame object for the ForgeOS extension's sidebar
...
await addSelectedCodeToForgeOSWebview(page)
```
...
2. **Playwright will open:**
   - A VS Code window with ForgeOS extension loaded
...
- **VS Code Configuration:**
  ...
  - Extension development mode with ForgeOS loaded
  - Temporary user data and extensions directories

- **Mock API Server:**
  - Runs on `http://localhost:7777`
  - Provides mock responses for ForgeOS API calls
...
- `FORGEOS_E2E_TESTS_VERBOSE=true` - Enable verbose logging
