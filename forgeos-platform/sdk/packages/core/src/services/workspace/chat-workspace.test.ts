...
describe("chat workspace", () => {
	...
	beforeEach(async () => {
		previousDataDir = process.env.FORGEOS_DATA_DIR;
	...
	it("creates the shared chat workspace under the forgeos data directory", async () => {
	...
	it("recognizes the default data-dir layout as the chat workspace", () => {
		expect(isChatWorkspacePath("/home/user/.forgeos/data/workspaces/chat")).toBe(
			true,
		);
		expect(
			isChatWorkspacePath("/home/user/.forgeos/data/workspaces/chat/my-app"),
		).toBe(false);
	});
