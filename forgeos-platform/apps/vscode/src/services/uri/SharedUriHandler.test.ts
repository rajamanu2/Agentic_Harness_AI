	...
	describe("handleUri", () => {
		describe("OpenRouter callback handling", () => {
			it("should successfully handle OpenRouter callback with code", async () => {
				const result = await SharedUriHandler.handleUri("vscode://forgeos.forgeos/openrouter?code=test123")
			...
			it("should return false when OpenRouter code is missing", async () => {
				const result = await SharedUriHandler.handleUri("vscode://forgeos.forgeos/openrouter")
			...
			it("should handle URL with plus signs in code parameter", async () => {
				const result = await SharedUriHandler.handleUri("vscode://forgeos.forgeos/openrouter?code=test+123+abc")
		...
		describe("Auth callback handling", () => {
			it("should successfully handle auth callback with idToken", async () => {
				const result = await SharedUriHandler.handleUri("vscode://forgeos.forgeos/auth?idToken=jwt123&provider=google")
			...
			it("should successfully handle auth callback without provider", async () => {
				const result = await SharedUriHandler.handleUri("vscode://forgeos.forgeos/auth?idToken=jwt123")
			...
			it("should return false when idToken is missing", async () => {
				const result = await SharedUriHandler.handleUri("vscode://forgeos.forgeos/auth?provider=google")
		...
		describe("Unknown path handling", () => {
			it("should return false for unknown paths", async () => {
				const result = await SharedUriHandler.handleUri("vscode://forgeos.forgeos/unknown?param=value")
		...
		describe("LG task URI handling", () => {
			it("should setup webhook files and create task from prompt-file", async () => {
				...
				try {
					...
					const result = await SharedUriHandler.handleUri(
						`vscode://forgeos.forgeos/lg-task?prompt-file=${encodeURIComponent(
							promptFilePath,
			...
			it("should return false when LG task parameters are missing", async () => {
				...
				writeHooksStub.resolves()
				const result = await SharedUriHandler.handleUri(
					"vscode://forgeos.forgeos/lg-task?prompt-file=%2Ftmp%2Fspec.md&webhook-url=https%3A%2F%2Fexample.com",
				)
		...
		describe("Query parameter parsing", () => {
			it("should correctly parse multiple query parameters", async () => {
				const result = await SharedUriHandler.handleUri(
					"vscode://forgeos.forgeos/auth?idToken=jwt123&provider=github&extra=param",
				)
			...
			it("should handle URL-encoded parameters", async () => {
				const result = await SharedUriHandler.handleUri(
					"vscode://forgeos.forgeos/auth?idToken=jwt%20with%20spaces&provider=google",
				)
