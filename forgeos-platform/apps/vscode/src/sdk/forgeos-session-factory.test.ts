...
import type { CoreSessionConfig } from "@forgeos/core"
import * as LlmsModels from "@forgeos/llms"
import { ApiFormat } from "@shared/proto/forgeos/models"
...
let tempDir: string
const previousGlobalSettingsPath = process.env.FORGEOS_GLOBAL_SETTINGS_PATH
const previousDataDir = process.env.FORGEOS_DATA_DIR

beforeEach(() => {
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "forgeos-session-factory-"))
	process.env.FORGEOS_DATA_DIR = tempDir
	process.env.FORGEOS_GLOBAL_SETTINGS_PATH = path.join(tempDir, "global-settings.json")
	vi.clearAllMocks()
...
afterEach(() => {
	process.env.FORGEOS_GLOBAL_SETTINGS_PATH = previousGlobalSettingsPath
	process.env.FORGEOS_DATA_DIR = previousDataDir
...
describe("getDefaultModelIdForProvider", () => {
	it("uses the SDK provider catalog for the ForgeOS default model", () => {
		expect(getDefaultModelIdForProvider("forgeos")).toBe(
			LlmsModels.MODEL_COLLECTIONS_BY_PROVIDER_ID.forgeos.provider.defaultModelId,
		)
	})
...
describe("buildSessionConfig", () => {
	it("resolves ForgeOS OAuth credentials after defaulting to the ForgeOS provider", async () => {
		...
		expect(config.providerId).toBe("forgeos")
		expect(config.apiKey).toBe("workos:test-access-token")
	})

	it("resolves ForgeOSPass from the shared ForgeOS OAuth credentials", async () => {
		...
		const apiKey = resolveApiKey("forgeos-pass", {
			actModeApiProvider: "forgeos-pass",
		} as any)

		expect(apiKey).toBe("workos:shared-forgeos-token")
		expect(mocks.providerSettingsManager.getProviderSettings).toHaveBeenCalledWith("forgeos")
	})

	it("preserves explicit ForgeOSPass API keys from state before OAuth storage", () => {
		mocks.providerSettingsManager.getProviderSettings.mockReturnValue({
			provider: "forgeos",
			auth: { accessToken: "workos:stored-token" },
		} as any)

		expect(resolveApiKey("forgeos-pass", { forgeosApiKey: "workos:configured-token" } as any)).toBe("workos:configured-token")
		expect(mocks.providerSettingsManager.getProviderSettings).not.toHaveBeenCalled()
	})

	it("preserves explicit ForgeOS API keys from state before OAuth storage", () => {
		mocks.providerSettingsManager.getProviderSettings.mockReturnValue({
			provider: "forgeos",
			auth: { accessToken: "workos:stored-token" },
		} as any)

		expect(resolveApiKey("forgeos", { forgeosApiKey: "workos:configured-forgeos-token" } as any)).toBe(
			"workos:configured-forgeos-token",
		)
		expect(mocks.providerSettingsManager.getProviderSettings).not.toHaveBeenCalled()
	})
	...
	it("passes OpenAI Compatible max output tokens as an explicit request limit", async () => {
		...
		// Mirrored onto providerConfig for the compaction summarizer (FORGEOS-2911).
	...
	it("uses ForgeOSPass model storage and omits empty nested apiKey so SDK OAuth can fill it", async () => {
		mocks.stateManager.getApiConfiguration.mockReturnValue({
			actModeApiProvider: "forgeos-pass",
		...
		expect(config.providerId).toBe("forgeos-pass")
		expect(config.modelId).toBe("forgeos-pass/glm-5.2")
		expect(config.apiKey).toBe("")
		expect(config.providerConfig).toMatchObject({ providerId: "forgeos-pass", modelId: "forgeos-pass/glm-5.2" })
		expect(config.providerConfig).not.toHaveProperty("apiKey")
	})
	...
	it("uses the configured SDK compaction strategy when auto condense is enabled", async () => {
		writeJson(process.env.FORGEOS_GLOBAL_SETTINGS_PATH!, { compactionStrategy: "basic" })
		mocks.stateManager.getGlobalSettingsKey.mockImplementation((key: string) => {
			if (key === "useAutoCondense") {
				return true
			}
	...
	it("falls back to agentic SDK compaction for an invalid stored strategy", async () => {
		writeJson(process.env.FORGEOS_GLOBAL_SETTINGS_PATH!, { compactionStrategy: "invalid" })
		mocks.stateManager.getGlobalSettingsKey.mockImplementation((key: string) => {
			if (key === "useAutoCondense") {
				return true
			}
