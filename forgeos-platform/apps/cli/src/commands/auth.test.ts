...
describe("getPersistedProviderApiKey", () => {
	it("does not double-prefix persisted ForgeOS OAuth tokens", () => {
		expect(
			getPersistedProviderApiKey("forgeos", {
				provider: "forgeos",
				auth: {
...
describe("loadAuthTuiRuntime", () => {
	it("loads OpenTUI React after provider catalog initialization", async () => {
		...
		const script = `
import { ProviderSettingsManager, ensureCustomProvidersLoaded, listLocalProviders } from "@forgeos/core";
