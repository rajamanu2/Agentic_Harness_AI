# ForgeOS CLI Changelog

## 3.0.55

- Auto-updates no longer install while a CLI is attached to the Hub. The update is recorded at startup and installed on exit, once the Hub confirms nothing else is attached, so a background update can no longer swap the package out from under a live session and kill it with `Hub connection closed (code=1006)`. `forgeos update` still installs immediately and now tells you the update applies on next start
...
- Fixed two ForgeOS installations on different builds shutting each other's Hub daemon down in a loop, which killed every live session with an abnormal socket close. Build identity is now compared through a total order, so at most one side of a pair can ever decide to retire the other (from SDK v0.0.75)
...
- `forgeos doctor fix` now reports honestly: processes that survived a kill are separated from ones that appeared while the fix ran, a live parent respawning a daemon is named, and a startup lock held by a running process is reported as held rather than leaked (from SDK v0.0.75)
...
- Managed Hub daemons now upgrade directionally: when another ForgeOS install ships a newer Hub build, the CLI attaches to the newer daemon and prompts you to update and restart instead of the two installs repeatedly retiring each other's daemons. Yolo and sandbox sessions, which never attach to the shared Hub, are not interrupted by that prompt (from SDK v0.0.74)
...
- Added `forgeos mcp uninstall` for removing an installed MCP server
...
- `meta/muse-spark-1.2-contributor` is now selectable on the ForgeOS provider, alongside a refreshed model catalog (from SDK v0.0.71)
- Error telemetry now reports the model that was actually in use for the run (from SDK v0.0.71)

## 3.0.50

- Added user-selectable color themes to the interactive TUI. Pick one with `/theme`, the command palette, or the Theme row in `/settings` — the picker previews each theme live. Built-in themes are Auto (terminal-adaptive, the default), ForgeOS Dark, ForgeOS Light, Tokyo Night, Gruvbox Dark, Nord, Dracula, Catppuccin Mocha, One Dark, Solarized Dark, and Solarized Light. Named themes paint the background, foreground, accents, syntax highlighting, and diff colors, and `FORGEOS_THEME` overrides the persisted choice at startup
...
- Empty model responses are now retried on every provider, not just Ollama — OpenRouter, ForgeOS, and OpenAI-compatible endpoints previously failed the task outright with "Model returned empty response" (from SDK v0.0.70)
...
- Checkpoint restore is now a full workspace rewind: files ForgeOS created during the task come back at their checkpoint-time content and files created after the checkpoint are removed, while `.gitignore`d paths (build output, `node_modules`, `.env`) are left alone (from SDK v0.0.69)
...
- Migrated users whose stored ForgeOS model id isn't in the catalog now fall back to the default model instead of sending an unknown model id on every request (from SDK v0.0.69)
- The ForgeOSPass promo dialog can be dismissed with any key (Enter still opens the subscription page), and it is marked as shown when it appears, so force-quitting no longer replays it on every launch
- Opening a URL no longer crashes the CLI on hosts without an opener binary (headless Linux without `xdg-open`); WSL2 containers now use `xdg-open`, Windows tries the absolute PowerShell path first, and `forgeos doctor log` converts Linux paths to `\\wsl$` UNC paths
...
- ACP: ForgeOSPass is selectable as a provider, organizations can be selected, session resolution and text rendering on session restart are fixed, and agent errors now describe the actual failure
...
- ForgeOS free models and recommended models now show their real display names in the model picker (from SDK v0.0.68)
...
- `forgeos history` now opens inside the existing TUI, with resume and delete actions, instead of rendering a second view in the same process
...
- `forgeos --help` now reports the real default `--config` and `--data-dir` paths
- The per-server `timeout` in `forgeos_mcp_settings.json` is now honored for `initialize`, `tools/list`, and `tools/call`, so slow MCP servers no longer fail against a hardcoded 5s limit (from SDK v0.0.67)
...
- A legacy single-file `.forgeosrules` no longer aborts the config scan (from SDK v0.0.67)
- Plugins can now emit telemetry through `ctx.telemetry` (from SDK v0.0.67)

## 3.0.47

- Free ForgeOS models are now supported end to end: free models show as "(free)", and hitting the free limit renders a dedicated card with the reset time (from SDK v0.0.66)
...
- Fixed out-of-credits detection so the CLI reliably recognizes the ForgeOS API's real `insufficient_credits` (402) error and shows the "add credits" card instead of a generic error

## 3.0.45

- Smaller install: the Claude Code and Codex providers are now optional and loaded on demand, cutting `npm i -g forgeos` from ~640MB to ~285MB (from SDK v0.0.65)
- Kimi K3 is now available as a ForgeOSPass model (from SDK v0.0.65)
...
- Added a manual API key escape hatch for ForgeOS OAuth providers, so you can enter a key by hand from settings
...
- The ForgeOSPass usage-limit error is now surfaced clearly when you hit the limit
- Session id is now preserved when continuing within the same session
- Hardened context compaction budget handling

## 3.0.39

- You can now select ForgeOS free models on the ForgeOSPass provider in the model picker
- Removed the retired ForgeOSPass GLM 5.1 model
...
- ForgeOS provider requests now send versioned client-identity headers
...
- Polished the status bar usage display and ForgeOSPass model name
...
- ForgeOSPass is now enabled for all CLI users
...
- Fixed the ForgeOSPass upgrade notice appearing immediately after completing onboarding.
- Improved the wording of the ForgeOSPass onboarding step.
- Streamlined the ForgeOS provider picker by merging the subscription and usage/billing options into one and removing the credits link.

## 3.0.33

- Show a ForgeOSPass subscription URL as a fallback during onboarding so you can still subscribe if the subscription screen can't open automatically
- Hide the ForgeOSPass promo for users who already have a ForgeOSPass subscription
- Use an adaptive plan accent color for ForgeOSPass prompts so they fit the active theme

## 3.0.32

- Improved the ForgeOSPass onboarding experience
- Added an intermediate step before going to ForgeOSPass model selection
- Made the ForgeOSPass subscription screen selectable
- Promoted ForgeOSPass in the startup notice
- Used "ForgeOSPass" as one word consistently and refined the provider UI copy
- More accurate context compaction and clearer error messages (from SDK v0.0.54)

## 3.0.31

- Show when request cost is covered by your ForgeOS subscription
- Prompt to switch to ForgeOSPass when you run out of credits, and list ForgeOSPass features in the not-subscribed message
- Added an option to open the subscription page from the ForgeOSPass options
...
- Updated ForgeOSPass models live, restored ForgeOSPass models in onboarding, and improved ForgeOSPass error messages
...
- Costs are now hidden for ForgeOS free models
- Fixed Z.ai model metadata resolution for Z.ai models accessed through the ForgeOS provider
...
- Added a ForgeOSPass onboarding flow with selectable ForgeOSPass models, plus improved ForgeOSPass error handling
...
- Fixed ForgeOS model display names so they resolve by model name
...
- Added a `forgeos skill` command to install and manage skills, matching `forgeos plugin install` and `forgeos mcp` (installs default to the ForgeOS agent directory)
...
- Reverted the expandable model picker sections and ForgeOSPass models, restoring the previous model-selection UI

## 3.0.25

- Added ForgeOSPass support, with selectable ForgeOSPass models in the model picker
...
- Fixed ForgeOS OAuth token formatting in provider config
...
- Enforced a single shared ForgeOS Hub, so a stale hub is respawned after an upgrade
...
- Added a ForgeOS credits refill link
...
- Bump the bundled SDK to v0.0.43, which forces a running ForgeOS Hub to restart so it picks up the latest SDK code.

## 3.0.17

- Fix a regression introduced in 3.0.15 where the interactive CLI could get stuck after stopping and restarting ForgeOS Hub and then pressing Escape to cancel a request. The CLI now detects stale or missing sessions, recovers any pending messages, and starts a fresh session instead of failing with "session not found".
...
- Install official ForgeOS plugins by slug off the new github.com/forgeos/plugins collection.
- Uninstall plugins using `forgeos plugin uninstall <plugin>` or in the TUI.
...
- Add a `FORGEOS_PLUGIN_IMPORT_TIMEOUT_MS` env override to control the plugin import timeout.

## 3.0.15

- Add ForgeOS Hub, a web app for monitoring connected clients, viewing and driving sessions, streaming assistant output, and restarting the local hub, with local, LAN, and tunnel usage gated by a room secret.
...
- Allow manual `forgeos update` runs to install the latest published version immediately, bypassing the release age gate that delays automatic updates.
...
- Add `--worktree` flag that auto-creates a fresh git worktree under `~/.forgeos/worktrees/` and runs the task there. Works with `--taskId` and `--continue` so you can resume a task in an isolated worktree to try a different approach.
...
Introducing our new ForgeOS CLI built on our new SDK and comes with a snappy new TUI.
...
- Dual-publish `@forgeosbot/cli` mirror wrapper so existing users who installed via `npm i -g @forgeosbot/cli` continue receiving updates
...
- Add `@forgeos/sdk` as a user-facing alias for `@forgeos/core`
...
- Fix configuration file paths for hooks and rules (now resolve from `~/.forgeos/hooks` and `~/.forgeos/rules`)
...
- Listing previous sessions no longer hydrates every full session, making `forgeos history` and the history picker snappy even with hundreds of sessions.
...
- Interactive setup wizards: `forgeos connect`, `forgeos schedule`, `forgeos mcp`
