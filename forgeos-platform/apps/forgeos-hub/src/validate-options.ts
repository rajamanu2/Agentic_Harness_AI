import { buildInviteUrl, resolveForgeOSHubServerOptions } from "./options";
...
const defaults = resolveForgeOSHubServerOptions({});
...
const lan = resolveForgeOSHubServerOptions({
	HOST: "0.0.0.0",
	FORGEOS_HUB_DASHBOARD_PORT: "9000",
...
const tailscale = resolveForgeOSHubServerOptions({
	HOST: "0.0.0.0",
...
expectThrows(
	() => resolveForgeOSHubServerOptions({ HOST: "0.0.0.0" }),
	"non-local bind without ROOM_SECRET",
);
expectThrows(
	() => resolveForgeOSHubServerOptions({ FORGEOS_HUB_DASHBOARD_PORT: "70000" }),
	"invalid dashboard port",
);
expectThrows(
	() => resolveForgeOSHubServerOptions({ PUBLIC_URL: "ftp://example.test" }),
	"invalid PUBLIC_URL protocol",
);

console.log("forgeos-hub option validation passed");
