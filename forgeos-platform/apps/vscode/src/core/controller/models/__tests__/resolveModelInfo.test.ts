...
describe("resolveModelInfo", () => {
	...
	it("resolves a free model id from the forgeos-pass catalog without coercing to the default", async () => {
		const { resolveModelInfo } = await import("../resolveModelInfo")
		const store = makeStore({ providerId: parseProviderId("forgeos-pass") })
		const catalog = makeCatalog()
		// The forgeos-pass catalog carries the endpoint's forgeosPass bucket plus the
		// ForgeOS free models (zero-priced, OpenRouter-style ids without the
		// forgeos-pass/ prefix). Selecting a free model must not be replaced by the
		// default pass model.
		vi.mocked(catalog.peekModels).mockReturnValue(
			peekResult(
				"forgeos-pass",
				[
					["forgeos-pass/glm-5.1", { name: "GLM 5.1", supportsPromptCache: false, contextWindow: 200_000 }],
