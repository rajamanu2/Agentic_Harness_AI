...
import { BlobStoreSettings, blobStorage } from "../../storage/ForgeOSBlobStorage"
...
export function getBlobStoreSettingsFromEnv(): BlobStoreSettings {
	return {
		adapterType: process?.env?.FORGEOS_STORAGE_ADAPTER || "unknown",
		bucket: process?.env?.FORGEOS_STORAGE_BUCKET || "forgeos",
		accessKeyId: process?.env?.FORGEOS_STORAGE_ACCESS_KEY_ID || "",
		secretAccessKey: process?.env?.FORGEOS_STORAGE_SECRET_ACCESS_KEY || "",
		region: process?.env?.FORGEOS_STORAGE_REGION,
		endpoint: process?.env?.FORGEOS_STORAGE_ENDPOINT,
		accountId: process?.env?.FORGEOS_STORAGE_ACCOUNT_ID,

		intervalMs: parseIntEnv(process.env.FORGEOS_STORAGE_SYNC_INTERVAL_MS, 30000),
		maxRetries: parseIntEnv(process.env.FORGEOS_STORAGE_SYNC_MAX_RETRIES, 5),
		batchSize: parseIntEnv(process.env.FORGEOS_STORAGE_SYNC_BATCH_SIZE, 10),
		maxQueueSize: parseIntEnv(process.env.FORGEOS_STORAGE_SYNC_MAX_QUEUE_SIZE, 1000),
		maxFailedAgeMs: parseIntEnv(process.env.FORGEOS_STORAGE_SYNC_MAX_FAILED_AGE_MS, SEVEN_DAYS_MS),
		backfillEnabled: process.env.FORGEOS_STORAGE_SYNC_BACKFILL_ENABLED === "true",
	}
}
