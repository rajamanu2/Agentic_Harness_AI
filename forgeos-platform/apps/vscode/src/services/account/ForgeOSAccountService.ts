...
import { buildBasicForgeOSHeaders } from "../EnvUtils"

export class ForgeOSAccountService {
	private static instance: ForgeOSAccountService
	...
	/**
	 * Returns the singleton instance of ForgeOSAccountService
	 * @returns Singleton instance of ForgeOSAccountService
	 */
	public static getInstance(): ForgeOSAccountService {
		if (!ForgeOSAccountService.instance) {
			ForgeOSAccountService.instance = new ForgeOSAccountService()
		}
		return ForgeOSAccountService.instance
	}

	/**
	 * Returns the base URL for the ForgeOS API
	 * @returns The base URL as a string
	 */
	get baseUrl(): string {
		return ForgeOSEnv.config().apiBaseUrl
	}

	/**
	 * Helper function to make authenticated requests to the ForgeOS API
	...
	private async authenticatedRequest<T>(
		...
		const forgeosAccountAuthToken = options?.authToken ?? (await this._authService.getAuthToken())
		if (!forgeosAccountAuthToken) {
			throw new Error("No ForgeOS account auth token found")
		}
		const requestConfig: AxiosRequestConfig = {
			...config,
			headers: {
				Authorization: `Bearer ${forgeosAccountAuthToken}`,
	...
	async fetchMe(): Promise<UserResponse | undefined> {
		try {
			const data = await this.authenticatedRequest<UserResponse>(FORGEOS_API_ENDPOINT.USER_INFO)
			return data
		} catch (error) {
	...
	async fetchFeaturebaseToken(): Promise<FeaturebaseTokenResponse | undefined> {
		try {
			const data = await this.authenticatedRequest<FeaturebaseTokenResponse>(FORGEOS_API_ENDPOINT.FEATUREBASE_TOKEN)
			return data
		} catch (error) {
	...
	async fetchUserRemoteConfig(): Promise<UserRemoteConfigDiscoveryResponse | undefined> {
		...
		const data = await this.authenticatedRequest<UserRemoteConfigDiscoveryResponse | null>(
			FORGEOS_API_ENDPOINT.USER_REMOTE_CONFIG,
			{},
	...
	async switchAccount(organizationId?: string | null): Promise<void> {
		...
		try {
			...
			const _response = await this.authenticatedRequest<string>(FORGEOS_API_ENDPOINT.ACTIVE_ACCOUNT, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				},
