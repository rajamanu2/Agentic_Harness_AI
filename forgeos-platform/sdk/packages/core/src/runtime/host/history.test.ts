...
const originalSessionDataDir = process.env.FORGEOS_SESSION_DATA_DIR;
...
async function writeMessagesFile(
	...
	if (!tempSessionDataDir) {
		tempSessionDataDir = await mkdtemp(join(tmpdir(), "forgeos-core-history-"));
	}
...
describe("session history", () => {
	afterEach(async () => {
		...
		if (originalSessionDataDir === undefined) {
			...
			process.env.FORGEOS_SESSION_DATA_DIR = originalSessionDataDir;
		}
	});
	...
	it("falls back to nested metadata provider/model ids before reading messages", async () => {
		...
		expect(row.provider).toBe("forgeos");
	...
	it("merges manifest fallback rows when the backend list is short", async () => {
		tempSessionDataDir = await mkdtemp(join(tmpdir(), "forgeos-core-history-"));
		process.env.FORGEOS_SESSION_DATA_DIR = tempSessionDataDir;
