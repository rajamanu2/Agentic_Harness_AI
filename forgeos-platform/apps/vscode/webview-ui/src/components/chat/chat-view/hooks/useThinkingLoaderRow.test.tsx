...
function say(ts: number, sayType: ForgeOSMessage["say"], partial?: boolean, text = ""): ForgeOSMessage {
...
function inputsFor(messages: ForgeOSMessage[], turnState: TurnState | undefined): ThinkingLoaderInputs {
	return {
		turnState,
...
describe("computeIsWaitingForResponse (legacy path)", () => {
	it("does not wait when the last raw message is an ask", () => {
		const ask: ForgeOSMessage = { ts: 1, type: "ask", ask: "followup", text: "?", partial: false }
...
describe("useThinkingLoaderRow anti-flash debounce", () => {
	...
	it("keeps an already-visible loader shown when a tool group tail finalizes", () => {
		const toolAsk = (partial: boolean): ForgeOSMessage => ({
			ts: 2,
			type: "ask",
			ask: "tool",
		...
		const toolGroupInputs = (tail: ForgeOSMessage): ThinkingLoaderInputs => {
			const group = Object.assign([tail], { _isToolGroup: true }) as ForgeOSMessage[]
			return {
				turnState: streaming(),
				lastRawMessage: tail,
