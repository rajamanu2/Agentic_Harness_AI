...
const DASHBOARD_PORT_ENV = "FORGEOS_HUB_DASHBOARD_PORT";
const WEBVIEW_DIST_ENV = "FORGEOS_HUB_WEBVIEW_DIST_DIR";
...
async function withDashboardEnvironment<T>(
	...
	const restore = [
		...
		setEnvValue("FORGEOS_DIR", options.configDir?.trim() || undefined),
	...
	if (options.dataDir || process.env.FORGEOS_SANDBOX?.trim() === "1") {
...
function resolveDefaultWebviewDistDir(): string | undefined {
	...
	const candidates = [
		...
		join(moduleDir, "../../../forgeos-hub/dist/webview"),
		// Node bundle: apps/cli/dist/index.js
		join(moduleDir, "forgeos-hub/webview"),
		// Compiled platform package: apps/cli/dist/<platform>/bin/forgeos
		join(dirname(process.execPath), "../forgeos-hub/webview"),
	];
...
function resolveInstalledPlatformPackageWebviewCandidates(): string[] {
	...
	const starts = [
		process.env.FORGEOS_WRAPPER_PATH
			? dirname(process.env.FORGEOS_WRAPPER_PATH)
			: undefined,
	...
	for (const start of starts) {
		...
		for (;;) {
			candidates.push(
				join(current, "node_modules", packageName, "forgeos-hub/webview"),
			);
...
function resolvePlatformPackageName(): string {
	...
	return `@forgeos/cli-${platformName}-${arch()}`;
}

async function startDefaultDashboardServer(): Promise<DashboardServerHandle> {
	const { startForgeOSHubDashboardServer } = await import("@forgeos/forgeos-hub");
	return await startForgeOSHubDashboardServer();
}
...
export async function runDashboardCommand(
	...
	try {
		...
		options.io.writeln(
			`${c.green}ForgeOS dashboard listening at${c.reset} ${dashboardUrl}`,
		);
		if (server.hubUrl) {
