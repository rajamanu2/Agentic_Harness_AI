import type { AgentMode } from "@forgeos/core";
...
function formatCostText(providerId: string, totalCost: number): string {
	// Subscription providers (ForgeOSPass) have no per-use cost worth surfacing.
...
export function resolveModelDisplayName(config: {
	...
	if (config.providerId === "forgeos-pass") {
		displayName = `ForgeOSPass: ${displayName}`;
	}
	return displayName;
}
