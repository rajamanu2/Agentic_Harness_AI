...
import { ForgeOSAuthContext, ForgeOSAuthContextType, ForgeOSAuthProvider, useForgeOSAuth } from "@/context/ForgeOSAuthContext"
import {
...
const ForgeOSAuthProviderWithOverrides: React.FC<{
	overrides?: Partial<ForgeOSAuthContextType>
	children: React.ReactNode
}> = ({ overrides, children }) => {
	const authContext = useForgeOSAuth()
	return <ForgeOSAuthContext.Provider value={{ ...authContext, ...overrides }}>{children}</ForgeOSAuthContext.Provider>
}

export const createStorybookDecorator =
	(overrideStates?: Partial<ExtensionStateContextType>, classNames?: string, authOverrides?: Partial<ForgeOSAuthContextType>) =>
	(Story: any) => (
		<ExtensionStateProviderWithOverrides overrides={overrideStates}>
			<ForgeOSAuthProviderWithOverrides overrides={authOverrides}>
				<div className={cn("max-w-lg mx-auto", classNames)}>
					<Story />
				</div>
			</ForgeOSAuthProviderWithOverrides>
