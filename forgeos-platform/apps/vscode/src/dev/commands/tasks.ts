...
export function registerTaskCommands(controller: Controller): vscode.Disposable[] {
	return [
		vscode.commands.registerCommand("forgeos.dev.expireMcpOAuthTokens", async () => {
			try {
		...
		vscode.commands.registerCommand("forgeos.dev.createTestTasks", async () => {
			const count = (
 ...
 */
function createRealisticMessageSequence(baseTimestamp: number, taskPrompt: string, taskIndex: number): ForgeOSMessage[] {
	...
	const messages: ForgeOSMessage[] = [
		// Initial task message - uses "say" with "text" which is the format used in ForgeOS.ts
		{
			ts: baseTimestamp,
			type: "say",
			say: "text",
