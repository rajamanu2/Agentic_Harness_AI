...
export class ForgeOSDataMock {
	...
	public switchToUserType(type: "personal" | "enterprise"): UserResponse {
		const token = ForgeOSDataMock.getDefaultToken(type)
		if (!token) {
	...
	private _createUserResponse(userData: (typeof ForgeOSDataMock.USERS)[0]): UserResponse {
	...
	public getUserByToken(token?: string): UserResponse {
		...
		const actualToken = token || ForgeOSDataMock.getDefaultToken("personal") || "test-personal-token"
	...
	private _getUserByToken(token: string): UserResponse {
		const match = ForgeOSDataMock.findUserByToken(token)
