...
2. Sets the `forgeos.sdkBundle` context key (gates cohort-specific menu items /
...
3. `require()`s exactly one bundle and calls its `activate()` with a
   ...
   `~/.forgeos/data` + VS Code storage they used as standalone extensions.
...
(`forgeos.rollout.nextActivationFailedVersion`), reports a `fallback` telemetry
...
- **Manual overrides, in either direction.** The `forgeos.rollout.bundleOverride`
  ...
  reload. `FORGEOS_BUNDLE_OVERRIDE=next|legacy` (env var) does the same for
...
- **Crash pinning is local, not remote.** If the next bundle throws during
  ...
  that VSIX version on this machine (`forgeos.rollout.nextActivationFailedVersion`);
...
- Menu entries / keybindings declared by only one get `when` AND-ed with
  `forgeos.sdkBundle` / `!forgeos.sdkBundle`, so a cohort never sees a button its
...
node scripts/stitch.mjs \
  ...
  --loader dist/extension.js --version 4.1.0 --out /tmp/forgeos-ab-staging
node scripts/smoke-loader.mjs /tmp/forgeos-ab-staging   # loader behavior smoke
cd /tmp/forgeos-ab-staging && vsce package --no-dependencies --allow-package-secrets sendgrid
```
...
entirely and everyone stays on legacy unless `FORGEOS_BUNDLE_OVERRIDE` is set.
...
publishes this same combined package as **`saoudrizwan.forgeos-nightly`**. Before
...
| | stable | nightly |
|---|---|---|
| manifest `name` | `claude-dev` | `forgeos-nightly` |
| contribution IDs / context key / settings | `forgeos.*` | `forgeos-nightly.*` |
...
status-bar indicator (`ForgeOS: Next` / `ForgeOS: Legacy`); stable builds never do.
...
- **`extension.rollout.bundle_activated`** (authoritative, captured by the
  ...
  requires the bundle to be built with `FORGEOS_ROLLOUT_VARIANT`): attempted vs
