...
describe("sortProvidersForApiKeySetup", () => {
	it("drops OAuth-managed providers and ranks popular ones first", () => {
		const sorted = sortProvidersForApiKeySetup([
			...
			makeProvider({ id: "forgeos", name: "ForgeOS" }),
...
describe("OnboardingView", () => {
	...
	it("walks from welcome to the connect step", async () => {
		...
		expect(container.textContent).toContain("Set up ForgeOS");
		expect(container.textContent).toContain("Sign in with ForgeOS");
	...
	it("records ForgeOS as the provider when a signed-in user continues", async () => {
	...
	it("connects with a ForgeOS API key when OAuth sign-in is not used", async () => {
		const onComplete = await render();
		await act(async () => {
			buttonByText("Get started").click();
		});
		...
		await act(async () => {
			buttonByText("Use a ForgeOS API key instead").click();
		});
		const keyInput = container.querySelector<HTMLInputElement>(
			'input[aria-label="ForgeOS API key"]',
		);
		...
		invoke.mockImplementation(async (command: string) => {
			if (command === "save_provider_settings") {
				return { providerId: "forgeos", enabled: true };
			}
			if (command === "forgeos_account") {
				return { email: "dev@example.com", displayName: "Dev" };
			}
		...
		await act(async () => {
			...
			setter?.call(keyInput, "forgeos_key_123");
		...
		expect(invoke).toHaveBeenCalledWith("save_provider_settings", {
			provider: "forgeos",
			enabled: true,
			api_key: "forgeos_key_123",
		});
		expect(container.textContent).toContain("You're all set");
		expect(container.textContent).toContain("Your ForgeOS account is connected");
		expect(
			parseModelSelectionStorage(
				window.localStorage.getItem(MODEL_SELECTION_STORAGE_KEY),
			).lastProvider,
	...
	it("rejects an invalid ForgeOS API key and rolls back the saved key", async () => {
		await render();
		await act(async () => {
			buttonByText("Get started").click();
		});
		...
		invoke.mockImplementation(
			async (command: string, args?: Record<string, unknown>) => {
				...
				if (command === "forgeos_account") {
					throw new Error("ForgeOS account request failed with status 401");
				}
				return {};
			},
		);
	...
	it("saves an API key provider and remembers the selection", async () => {
		...
		invoke.mockImplementation(async (command: string) => {
			if (command === "run_provider_oauth_login") {
				return { provider: "forgeos", accessToken: "token" };
			}
			if (command === "forgeos_account") {
				throw new Error("No ForgeOS account auth token found");
			}
