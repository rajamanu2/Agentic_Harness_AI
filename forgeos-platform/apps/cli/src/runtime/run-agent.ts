...
import type { ConsecutiveMistakeLimitContext } from "@forgeos/shared";
import { createSessionId } from "@forgeos/shared";
import { logCliError } from "../logging/errors";
import { createCliCore } from "../session/session";
import { resolveForgeOSWelcomeLine } from "../tui/interactive-welcome";
import {
	askQuestionInTerminal,
	requestToolApproval,
	submitAndExitInTerminal,
} from "../utils/approval";
import { formatCliErrorMessage } from "../utils/forgeos-pass-errors";
...
export async function runAgent(
	...
	if (config.verbose) {
		const forgeosWelcomeLine = await resolveForgeOSWelcomeLine({
			config,
			forgeosApiBaseUrl: options?.forgeosApiBaseUrl,
			forgeosProviderSettings: options?.forgeosProviderSettings,
		});
		if (forgeosWelcomeLine && config.outputMode !== "json") {
			writeln(forgeosWelcomeLine);
		}
	}
	...
	const shouldZeroCost = await shouldZeroForgeOSFreeModelCost(config);
