...
describe("prepareLocalRuntimeBootstrap", () => {
		...
		vi.doMock("../extensions/plugin/plugin-config-loader", () => ({
			resolveAndLoadAgentPlugins: vi.fn(
				async ({
					...
					extensions:
						providerId === "forgeos" && modelId === "anthropic/claude-haiku-4.5"
							? [
									{
	...
	it.each([
		...
		const bootstrap = await prepareLocalRuntimeBootstrap({
			...
			localRuntime: {
				extensionContext: {
					client: { name: "forgeos-cli", version: "3.0.38" },
				},
			},
			sessionId: "sess-forgeos-headers",
			providerSettingsManager: createProviderSettingsManager({
				provider: providerId,
	...
	it("uses host request headers for ForgeOS providers on core sessions", async () => {
		const { prepareLocalRuntimeBootstrap } = await import(
			"./local-runtime-bootstrap"
		);
