...
import { augmentNodeCommandForDebug } from "@forgeos/shared";
...
function createPayloadBase(
	...
	const userId =
		process.env.FORGEOS_USER_ID?.trim() || process.env.USER?.trim() || "unknown";
	const sessionContext: HookSessionContext = {
		rootSessionId: options.rootSessionId || ctx.conversationId,
	};
	return {
		forgeosVersion: process.env.FORGEOS_VERSION?.trim() || "",
