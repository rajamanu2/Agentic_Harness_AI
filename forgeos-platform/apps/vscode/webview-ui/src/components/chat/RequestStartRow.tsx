...
const getActivityText = (tool: ForgeOSSayTool): string | null => {
...
const collectToolsInRange = (
	...
	startIdx: number,
	endIdx: number,
	stopCondition?: (msg: ForgeOSMessage) => boolean,
	...
	for (let i = startIdx; i < endIdx; i++) {
		...
		try {
			const tool = JSON.parse(msg.text || "{}") as ForgeOSSayTool
...
const findCurrentApiReq = (messages: ForgeOSMessage[]): { index: number; hasCost: boolean } | null => {
...
const findPrevCompletedApiReq = (messages: ForgeOSMessage[], beforeIdx: number): number => {
...
export const RequestStartRow: React.FC<RequestStartRowProps> = ({
	...
	const hasCompletionResult = forgeosMessages.some(
		(msg) =>
	...
	const willBeAbsorbed = useMemo(() => {
		return isApiReqAbsorbable(message.ts, forgeosMessages)
	}, [message.ts, forgeosMessages])
	...
	const currentActivities = useMemo(() => {
		const currentApiReq = findCurrentApiReq(forgeosMessages)
		if (!currentApiReq) {
			return []
		}
		...
		if (!currentApiReq.hasCost) {
			...
			return collectToolsInRange(forgeosMessages, currentApiReq.index + 1, forgeosMessages.length)
		}
	...
	const hasCompletedTools = useMemo(() => {
		...
		return forgeosMessages.some((msg, idx) => {
			if (msg.say === "tool" && isLowStakesTool(msg)) {
				...
				for (let i = idx - 1; i >= 0; i--) {
					const prevMsg = forgeosMessages[i]
