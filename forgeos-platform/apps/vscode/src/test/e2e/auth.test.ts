...
e2e("Views - can set up API keys and navigate to Settings from Chat", async ({ sidebar }) => {
	...
	await expect(sidebar.getByRole("button", { name: "Login to ForgeOS" })).toBeVisible()
	...
	// Test ForgeOS provider option
	await providerSelectorInput.click({ delay: 100 })
	// Wait for dropdown to appear and find ForgeOS option
	await expect(sidebar.getByTestId("provider-option-forgeos")).toBeVisible()
	await sidebar.getByTestId("provider-option-forgeos").click({ delay: 100 })
	await expect(sidebar.getByRole("button", { name: "Sign Up with ForgeOS" })).toBeVisible()
	...
	await expect(sidebar.getByRole("button", { name: "Login to ForgeOS" })).not.toBeVisible()
	...
	// forgeos logo container
	const forgeosLogo = sidebar.locator(".size-20")
	await expect(forgeosLogo).toBeVisible()
