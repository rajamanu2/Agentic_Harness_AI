...
import {
	ForgeOSPassSubscriptionContent,
...
import { buildFeaturedModelEntries } from "../components/model-selector/forgeos-model-picker";
import {
	BROWSE_ALL_ACTION,
	ForgeOSModelSelectorDialogContent,
} from "../components/model-selector/forgeos-model-selector";
import {
	buildModelOptions,
...
function providerToExistingProviderOptions(input: {
	...
	if (input.providerId !== "forgeos-pass") {
		return [];
	}
...
async function runProviderChange(
	...
	// working; only the ForgeOS providers accept a dashboard API key.
	const supportsManualApiKey = isForgeOSProvider(newProviderId);
	...
	const openModelSelector = useCallback(
		async (options?: OpenModelSelectorOptions) => {
			...
			while (pickingModel) {
				...
				if (
					config.providerId === "forgeos" ||
					config.providerId === "forgeos-pass"
				) {
					// ForgeOSPass gets the same sectioned picker with Subscribed/Free
					// sections — free models are selectable while staying on ForgeOSPass
					const featuredProviderId = config.providerId;
					const forgeosResult = await dialog.choice<string>({
						style: { maxHeight: termHeight - 2 },
						content: (ctx: ChoiceContext<string>) => (
							<ForgeOSModelSelectorDialogContent
								{...ctx}
								...
								loadEntries={async () =>
									buildFeaturedModelEntries(
										featuredProviderId,
										await fetchForgeOSRecommendedModels(),
									)
								}
							/>
						),
					...
					if (forgeosResult === CHANGE_PROVIDER_ACTION) {
						await changeProvider();
						continue;
					}
					if (forgeosResult === BROWSE_ALL_ACTION) {
						const browseResult = await dialog.choice<string>({
							...
							content: (ctx: ChoiceContext<string>) => (
								<ModelSelectorContent
									...
									currentProviderName={providerDisplayName}
									models={modelOptions}
									showCustomModelId={config.providerId !== "forgeos-pass"}
								/>
							),
						});
					...
					config.modelId = forgeosResult;
					const selectedModel = modelOptions.find(
						(m: ModelOption) => m.key === forgeosResult,
					);
