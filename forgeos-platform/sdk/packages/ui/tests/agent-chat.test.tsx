...
describe("@forgeos/ui agent chat primitives", () => {
	it("marks message roles without requiring a runtime message schema", async () => {
		await render(
			<Message from="assistant">
				<MessageContent>Hello from ForgeOS</MessageContent>
			</Message>,
		);

		const message = container.querySelector(".forgeos-chat-message");
		expect(message?.getAttribute("data-role")).toBe("assistant");
		expect(message?.textContent).toContain("Hello from ForgeOS");
	});
	...
	it("gives the scrollable conversation log accessible defaults", async () => {
		...
		const viewport = container.querySelector(
			".forgeos-chat-conversation-viewport",
		);
	...
	it("renders non-expandable tool activity as static content", async () => {
		...
		const summary = container.querySelector(".forgeos-chat-tool-trigger");
	...
	it("swaps the icon for the spinner while the tool is in flight", async () => {
		...
		expect(container.querySelector(".forgeos-chat-tool-progress")).not.toBeNull();
		...
		expect(container.querySelector(".forgeos-chat-tool-progress")).toBeNull();
	...
	it("toggles expandable tool details", async () => {
		...
		expect(document.getElementById(panelId ?? "")).toBeNull();
		expect(
			container.querySelector(".forgeos-chat-disclosure-icon"),
		).not.toBeNull();
	...
	it("hides the disclosure chevron on request while keeping the row clickable", async () => {
		...
		const trigger = container.querySelector("button");
		expect(container.querySelector(".forgeos-chat-disclosure-icon")).toBeNull();
