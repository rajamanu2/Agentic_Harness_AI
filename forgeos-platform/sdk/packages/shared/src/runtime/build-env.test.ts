...
import {
	...
	FORGEOS_BUILD_ENV_ENV,
	FORGEOS_DEBUG_HOST_ENV,
	FORGEOS_DEBUG_PORT_BASE_ENV,
	resolveForgeOSBuildEnv,
	withResolvedForgeOSBuildEnv,
} from "./build-env";

describe("build env helpers", () => {
	it("prefers explicit FORGEOS_BUILD_ENV", () => {
		expect(
			resolveForgeOSBuildEnv({
				env: { [FORGEOS_BUILD_ENV_ENV]: "development", NODE_ENV: "production" },
			}),
		).toBe("development");
	});
	...
	it("defaults to production otherwise", () => {
		expect(resolveForgeOSBuildEnv({ env: {}, execArgv: [] })).toBe("production");
	});

	it("treats NODE_ENV=development as a development build", () => {
		expect(
			resolveForgeOSBuildEnv({ env: { NODE_ENV: "development" }, execArgv: [] }),
		).toBe("development");
	});

	it("does not treat NODE_ENV=test as a development build", () => {
		expect(
			resolveForgeOSBuildEnv({ env: { NODE_ENV: "test" }, execArgv: [] }),
		).toBe("production");
	});

	it("does not treat NODE_ENV=staging as a development build", () => {
		expect(
			resolveForgeOSBuildEnv({ env: { NODE_ENV: "staging" }, execArgv: [] }),
		).toBe("production");
	});

	it("materializes FORGEOS_BUILD_ENV when absent", () => {
		expect(
			withResolvedForgeOSBuildEnv({ NODE_ENV: "development" }, { execArgv: [] })[
				FORGEOS_BUILD_ENV_ENV
			],
		).toBe("development");
	});
		...
		expect(
			augmentNodeCommandForDebug(["node", "script.js"], {
				env: {
					...
					[FORGEOS_DEBUG_HOST_ENV]: "0.0.0.0",
					[FORGEOS_DEBUG_PORT_BASE_ENV]: "9500",
				},
