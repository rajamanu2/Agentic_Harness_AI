...
describe("ca-certs", () => {
	...
	beforeEach(() => {
		dir = mkdtempSync(join(tmpdir(), "forgeos-ca-"));
	});
	...
	describe("configureNodeExtraCaCerts", () => {
		it("writes a managed bundle and points the env var at it", () => {
			const env: Record<string, string> = { FORGEOS_DIR: dir };
		...
		it("reuses a stale bundle when the rewrite fails", () => {
			...
			const env2: Record<string, string> = { FORGEOS_DIR: dir };
