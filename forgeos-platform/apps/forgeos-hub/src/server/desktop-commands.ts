import {
	addLocalProvider,
	type ForgeOSAccountActionRequest,
...
async function resolveHubForgeOSAccountAuthToken(input: {
	settings?: ProviderSettings;
	apiBaseUrl: string;
}): Promise<string | undefined> {
	const credentials = input.settings
		? getProviderOAuthCredentialsFromSettings("forgeos", input.settings)
		: null;
	if (!credentials || !input.settings) {
		return getPersistedProviderApiKey("forgeos", input.settings);
	}
	...
	return formatProviderOAuthApiKey("forgeos", nextCredentials);
}
...
export async function handleDesktopCommand(
	...
	if (command === "forgeos_account") {
		...
		const authToken = await resolveHubForgeOSAccountAuthToken({
			settings,
			apiBaseUrl,
		});
		if (!authToken) {
