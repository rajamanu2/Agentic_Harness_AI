...
describe("checkpoint workspace comparison", () => {
	...
	beforeEach(() => {
		...
		git(dir, ["init"]);
		git(dir, ["config", "user.name", "ForgeOS Test"]);
		git(dir, ["config", "user.email", "forgeos@example.com"]);
	...
	it("uses the worktree snapshot stored in SDK stash checkpoints", async () => {
		...
		const stashRef = git(dir, ["stash", "create", "forgeos checkpoint test"]);
