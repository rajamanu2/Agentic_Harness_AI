...
import {
	buildForgeOSPostHogClient,
	PostHogFeatureFlagsProvider,
} from "@forgeos/core/services/feature-flags/posthog";
import { resolveForgeOSDataDir } from "@forgeos/shared/storage";

let cliFeatureFlagsContext: FeatureFlagsContext = { clientName: "forgeos-cli" };
...
function resolveCliFeatureFlagsCachePath(): string {
	return join(resolveForgeOSDataDir(), "cache", "feature-flags.json");
}
		...
		const provider =
			...
			process.env.E2E_TEST !== "true"
				? new PostHogFeatureFlagsProvider({
						client: buildForgeOSPostHogClient(apiKey),
						config: {
