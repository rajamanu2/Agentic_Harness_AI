import type { ProviderSettingsManager } from "@forgeos/core";
...
import {
	OnboardingForgeOSModelScreen,
	OnboardingForgeOSPassSubscriptionScreen,
...
export function OnboardingView(props: OnboardingViewProps) {
	...
	if (state.step === "forgeos_model") {
		return (
			<OnboardingForgeOSModelScreen
				forgeosEntries={state.forgeosEntries}
				forgeosModelSelected={state.forgeosModelSelected}
				compact={compact}
				contentWidth={contentWidth}
				mouse={mouse}
	...
	if (state.step === "forgeos_pass_subscription") {
		return (
			<OnboardingForgeOSPassSubscriptionScreen
				compact={compact}
				contentWidth={contentWidth}
				currentPlanName={state.forgeosPassCurrentPlanName}
				error={state.forgeosPassSubscriptionError}
				mouse={mouse}
				openStatus={state.forgeosPassSubscriptionOpenStatus}
				options={state.forgeosPassSubscriptionOptions}
				planFeatures={state.forgeosPassPlanFeatures}
				selected={state.forgeosPassSubscriptionSelected}
				status={state.forgeosPassSubscriptionStatus}
				subscriptionUrl={state.forgeosPassSubscriptionUrl}
			/>
		);
	}
