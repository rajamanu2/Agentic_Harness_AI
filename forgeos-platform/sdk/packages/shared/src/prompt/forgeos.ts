...
import { isForgeOSProvider } from "../providers/utils";
...
/**
 * Options for building the ForgeOS system prompt.
 *
 ...
 */
export interface ForgeOSSystemPromptOptions
	...
	/** Provider ID — used to gate ForgeOS-specific metadata injection */
	providerId?: string;
	/**
...
export function buildForgeOSSystemPrompt(
	options: ForgeOSSystemPromptOptions,
): string {
	const {
		ide = "Terminal Shell",
		mode,
	...
	const isForgeOS = isForgeOSProvider(providerId || "");
	...
	return basePrompt
		...
		.replace("{{FORGEOS_RULES}}", effectiveRules)
		.trim();
}
