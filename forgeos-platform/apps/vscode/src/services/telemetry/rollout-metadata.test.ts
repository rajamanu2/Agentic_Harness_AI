...
const originalVariant = process.env.FORGEOS_ROLLOUT_VARIANT

afterEach(() => {
	restoreEnv("FORGEOS_ROLLOUT_VARIANT", originalVariant)
})

describe("rollout telemetry metadata", () => {
	it("returns metadata for a rollout build", () => {
		process.env.FORGEOS_ROLLOUT_VARIANT = "next"
	...
	it("omits metadata for ordinary or invalid builds", () => {
		delete process.env.FORGEOS_ROLLOUT_VARIANT
		expect(getRolloutTelemetryMetadata()).toEqual({})

		process.env.FORGEOS_ROLLOUT_VARIANT = "invalid"
		expect(getRolloutTelemetryMetadata()).toEqual({})
	})

	it("exposes the variant for rollout builds only", () => {
		process.env.FORGEOS_ROLLOUT_VARIANT = "legacy"
...
function restoreEnv(key: "FORGEOS_ROLLOUT_VARIANT", value: string | undefined): void {
	if (value === undefined) {
