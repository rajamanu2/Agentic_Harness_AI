 ...
 *
 * @param dataDir Override for the ForgeOS data directory. Defaults to
 *   resolveDataDir() which checks FORGEOS_DATA_DIR, FORGEOS_DIR, then ~/.forgeos/data.
...
export function migrateProviders(dataDir?: string): ProviderMigrationResult {
	...
	try {
		...
		// the correct dataDir, not the default ~/.forgeos/data.
