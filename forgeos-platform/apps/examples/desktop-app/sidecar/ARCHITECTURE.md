# Sidecar Architecture — @forgeos/code

## Overview

The sidecar is a Bun process that adapts the desktop UI and native operations to
the shared ForgeOS Hub.

It imports `@forgeos/core`, discovers or starts the canonical shared Hub, registers
...
`ForgeOSCore` uses Hub mode without an explicit endpoint. Core therefore reuses
the same compatible Hub discovered by the CLI or starts the canonical detached
Hub when the desktop is the first client:

```typescript
const sessionManager = await ForgeOSCore.create({
  clientName: "forgeos-code",
  backendMode: "hub",
  hub: {
...
```typescript
import { ProviderSettingsManager, listLocalProviders, ... } from "@forgeos/core";
...
```typescript
import { SqliteSessionStore, resolveSessionBackend } from "@forgeos/core";
...
|---------|---------------|
| `chat_session_command` | shared Hub through `ForgeOSCore` |
