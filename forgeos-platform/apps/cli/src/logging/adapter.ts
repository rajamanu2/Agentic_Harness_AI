...
import type { BasicLogger, RuntimeLoggerConfig } from "@forgeos/core";
import { resolveForgeOSDataDir } from "@forgeos/core";
...
function normalizeRuntimeConfig(input: {
	...
	const enabledEnv = process.env.FORGEOS_LOG_ENABLED?.trim();
	const enabled =
		base?.enabled ??
		!(enabledEnv === "0" || enabledEnv?.toLowerCase() === "false");
	const level = normalizeLogLevel(base?.level ?? process.env.FORGEOS_LOG_LEVEL);
	const destination =
		base?.destination?.trim() ||
		process.env.FORGEOS_LOG_PATH?.trim() ||
		defaultDestination;
	const name =
		base?.name?.trim() ||
		process.env.FORGEOS_LOG_NAME?.trim() ||
