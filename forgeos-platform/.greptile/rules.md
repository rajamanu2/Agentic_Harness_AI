...
The SDK does **not** depend on the original `forgeos/forgeos` repo for telemetry. The two have
...
- `task.*`: emitted by core session lifecycle code in `sdk/packages/core/src/forgeos-core/` and
...
`submit_and_exit` tool call (the SDK analog of original ForgeOS's `attempt_completion`). For
...
The CLI accepts `--config <dir>`. The CLI **must** apply `setForgeOSDir(...)` and
`setHomeDir(...)` from `@forgeos/shared/storage` **before** calling
`captureCliExtensionActivated()`. Otherwise the telemetry singleton's persisted distinct-id
and any other on-disk telemetry state lands under `~/.forgeos` instead of the user's chosen
config dir.

The canonical pattern is in `apps/cli/src/main.ts`:

```ts
if (configDir) setForgeOSDir(configDir);
setHomeDir(homedir());
...
identifies from the cached forgeos account (re-resolved periodically, since the daemon often
...
Cross-reference `sdk/packages/core/src/auth/forgeos.ts` and `sdk/packages/core/src/auth/codex.ts` as
