import { ForgeOSMessage } from "@shared/ExtensionMessage"
...
// Mock data factories
const createMockMessage = (overrides: Partial<ForgeOSMessage> = {}): ForgeOSMessage => ({
	ts: Date.now(),
	type: "say",
	say: "error",
...
const meta: Meta<typeof ErrorRow> = {
	...
	parameters: {
		docs: {
			description: {
				component:
					"Displays different types of error messages in the chat interface, including API errors, credit limit errors, diff errors, and forgeosignore errors. Handles special error parsing for ForgeOS provider errors and provides appropriate user actions.",
			},
		},
	},
...
export const Default: Story = {
	...
	argTypes: {
		errorType: {
			control: { type: "select" },
			options: ["error", "mistake_limit_reached", "diff_error", "forgeosignore_error"],
...
// ForgeOS-specific errors
export const ForgeOSBalanceError: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
...
export const ForgeOSRateLimitError: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
...
export const ForgeOSSpendLimitDaily: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
...
export const ForgeOSSpendLimitMonthly: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
...
export const ForgeOSSpendLimitMinimal: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
...
// ForgeOSPass entitlement error (user not subscribed to a required model plan)
export const ForgeOSPassEntitlementError: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage:
	...
	parameters: {
		docs: {
			description: {
				story: "ForgeOSPass model returns the SDK ForgeOSNotSubscribedError message when the user is not subscribed. A human-readable message with a 'Get ForgeOSPass' subscribe link and a retry button is shown.",
			},
		},
	},
}
...
export const AuthErrorSignedIn: Story = {
	...
	decorators: [
		createStoryDecorator({
			forgeosUser: { id: "user123", email: "user@example.com" },
...
export const InteractiveSignIn: Story = {
	args: {
		...
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
			message: "Please sign in to access ForgeOS services.",
			code: "ERR_BAD_REQUEST",
	...
	play: async ({ canvasElement }) => {
		...
		const signInButton = canvas.getByRole("button", { name: /sign in to forgeos/i })
