...
import { AskResponseRequest, NewTaskRequest } from "@shared/proto/forgeos/task"
import { IntentEvent } from "@shared/proto/forgeos/ui"
 ...
 */
export function useMessageHandlers(messages: ForgeOSMessage[], chatState: ChatState): MessageHandlers {
	...
	const handleSendMessage = useCallback(
		async (text: string, images: string[], files: string[]) => {
			...
			// compacting the context window (FORGEOS-2503). `/newtask` aliases
			...
			if (hasContent) {
				...
				const beginPendingResponse = (pendingMessage?: ForgeOSMessage) => {
				...
				if (messages.length === 0) {
					...
					if (forgeosAsk === "resume_task" || forgeosAsk === "resume_completed_task") {
						...
						switch (forgeosAsk) {
							...
							case "report_bug": {
								...
								const showPendingMessage = forgeosAsk !== "followup" && turnState?.phase !== "streaming"
					...
					// No forgeosAsk set, but there is an existing conversation. Route this to the
					...
					//      does not emit a trailing ask:"completion_result", so forgeosAsk is
