# `@forgeos/ui` adoption primer

This guide is for ForgeOS engineering teams that want a web application to share
the ForgeOS visual language and agent-chat presentation without copying desktop
styles or adopting desktop product structure.

## The short version

`@forgeos/ui` has two opt-in layers:
...
- ForgeOS-owned Slate, Violet, Ruby, Green, Amber, and Sky solid/alpha palettes
...
This boundary gives ForgeOS products a shared visual and interaction language
without turning `@forgeos/ui` into a second agent runtime.
...
copy — that is the direction `@forgeos/ui` is headed, and more shared modules
are expected over time. Two exist today:

- `@forgeos/ui/components/agent-chat/tool-summary` — pure, framework-free
  ...
  `@forgeos/core` or transport events.
- `@forgeos/ui/components/agent-chat/tool-diff` — `ToolFileDiff`, a thin
...
`@forgeos/ui` is configured for public npm publication with its own version and
manual release workflow. Check availability with `npm view @forgeos/ui version`;
...
| --- | --- | --- | --- |
| Use only light/dark CSS variables | `@forgeos/ui/theme/tokens.css` | No | No |
| Render shared root React primitives without host resets | `scoped-tokens.css`, `components.css`, and `@forgeos/ui` | Tailwind v4 | React 18.3 or 19 |
| Use tokens through Tailwind utilities | `tokens.css` then `theme.css` | Tailwind v4 | No |
| Use the complete theme and shared base behavior | `@forgeos/ui/theme/index.css` | Tailwind v4 | No |
| Compose shared agent-chat presentation | `@forgeos/ui/components/agent-chat` plus its CSS | No, if tokens are mapped in plain CSS | React 18.3 or 19 |
...
There is no `@forgeos/ui/theme` shorthand. Use the explicit paths documented here
so dependencies remain visible.

## Install inside the ForgeOS monorepo
...
# Required for the documented Tailwind-backed theme and ForgeOS fonts
...
@import "tailwindcss";
@import "@forgeos/ui/theme/index.css";
```
...
@import "@fontsource-variable/geist-mono";
@import "tailwindcss";
@import "@forgeos/ui/theme/tokens.css";
@import "@forgeos/ui/theme/theme.css";
```

For an existing Tailwind v4 surface that must preserve its host shell, keep
Tailwind setup host-owned and add the scoped ForgeOS imports:
...
Render shared components inside `.forgeos-ui-theme`. Dark values activate when
...
registers the complete, generic ForgeOS utility vocabulary and is intended only
for applications that want ForgeOS to own those utility names globally.
...
If the application later opts into ForgeOS's global utility vocabulary and base
...
```css
@import "@forgeos/ui/theme/index.css";
@import "@forgeos/ui/components/agent-chat.css";
```
...
import {
  ...
  ToolActivityContent,
  ToolActivityTrigger,
} from "@forgeos/ui/components/agent-chat";
...
package depend on `@forgeos/core`, the Vercel AI SDK, desktop schemas, or transport
events.
...
```bash
bun -F @forgeos/ui storybook -- --host 0.0.0.0 --port 3490 --exact-port
```

Build the production Storybook bundle with:

```bash
bun -F @forgeos/ui build-storybook
```
...
The catalog currently runs from a ForgeOS monorepo checkout. Story sources and
...
Keep the following outside `@forgeos/ui`:
...
- Keep ForgeOS-owned palette values in `palette.css` and author UI through visual
  or status roles
...
1. Adopt the theme and chat primitives in a second production-shaped ForgeOS app.
...
- [Desktop theme integration test (monorepo)](https://github.com/forgeos/forgeos/blob/main/apps/examples/desktop-app/webview/styles/theme-integration.test.ts)
