...
import { TestConnectionResult } from "@shared/proto/forgeos/state"
import { Logger } from "@/shared/services/Logger"
import { blobStorage } from "@/shared/storage/ForgeOSBlobStorage"
...
export async function testPromptUploading(_controller: Controller, _: EmptyRequest): Promise<TestConnectionResult> {
	try {
		...
		const testKey = `forgeos-test-${Date.now()}.json`
