...
describe("Remote Config Schema", () => {
	...
	describe("ForgeOSSettingsSchema", () => {
		it("should accept valid ForgeOS provider settings", () => {
			const validSettings = {
				models: [{ id: "claude-3-5-sonnet-20241022" }, { id: "claude-3-5-haiku-20241022" }],
			}
			const result = ForgeOSSettingsSchema.parse(validSettings)
			expect(result).to.deep.equal(validSettings)
		})

		it("should accept empty settings object", () => {
			const result = ForgeOSSettingsSchema.parse({})
			expect(result.models).to.be.undefined
		})
		...
		it("should accept models with only id field", () => {
			...
			expect(() => ForgeOSSettingsSchema.parse(settings)).to.not.throw()
		})

		it("should reject models with missing id field", () => {
			expect(() =>
				ForgeOSSettingsSchema.parse({
					models: [{}],
				}),
			).to.throw()
		})
	...
	describe("RemoteConfigSchema", () => {
		...
		it("should handle complete config with all fields", () => {
			const config = {
				...
				providerSettings: {
					...
					Anthropic: {
						...
						baseUrl: "https://example.forgeos.bot",
					},
				},
				enterpriseTelemetry: {
					promptUploading: {
			...
			// Verify ForgeOS settings
			expect(result.providerSettings?.ForgeOS?.models).to.have.lengthOf(2)
			expect(result.providerSettings?.ForgeOS?.models?.[0].id).to.equal("claude-3-5-sonnet-20241022")
			expect(result.providerSettings?.ForgeOS?.models?.[1].id).to.equal("claude-3-5-haiku-20241022")
			...
			expect(result.providerSettings?.Anthropic?.baseUrl).to.equal("https://example.forgeos.bot")
