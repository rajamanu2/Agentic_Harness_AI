...
import { version as forgeosVersion } from "../../../package.json"
...
} from "../../shared/proto/forgeos/hooks"
 ...
 * provide--the other common parameters like forgeosVersion and userId are handled by the
 * hook system.
 */
...
export abstract class HookRunner<Name extends HookName> {
	 ...
	 * - forgeosVersion: Current ForgeOS extension version
	 ...
	 * - userId: ForgeOS user ID, machine ID, or generated UUID
	 *
	...
	protected async completeParams(params: NamedHookInput<Name>): Promise<HookInput> {
		...
		return {
			...
			workspaceRoots,
			userId: getDistinctId(), // Always available: ForgeOS User ID, machine ID, or generated UUID
			...params,
			model,
		}
	}
 ...
 *
 * Used in multi-root workspaces where both global hooks (from ~/Documents/ForgeOS/Hooks/)
 * and workspace-specific hooks (from each workspace's .forgeosrules/hooks/) exist for the
 * same hook type.
 *
 * Behavior:
...
function isExpectedHookError(error: unknown): boolean {
	...
	// Note: This is expected because users may have hooks in .forgeosrules that they don't want to execute
...
export class HookFactory {
	 ...
	 * Global hooks are located in paths containing "ForgeOS/Hooks" or "forgeos/hooks".
	 */
	 ...
	 *
	 * - Global hooks (from ~/Documents/ForgeOS/Hooks/): run from the primary workspace root
	 * - Workspace hooks (from workspaceRoot/.forgeosrules/hooks/): run from that specific workspace root
	 *
	...
	private determineHookCwd(
		...
		// Workspace hooks are at: workspaceRoot/.forgeosrules/hooks/
	 ...
	 * Global hooks are located in ~/Documents/ForgeOS/Hooks/
	 * Workspace hooks are located in workspace .forgeosrules/hooks/ directories
	 *
	 ...
	 * Includes both global hooks (from ~/Documents/ForgeOS/Hooks/) and workspace hooks
	 * (from .forgeosrules/hooks/ in each workspace root).
	 */
	...
	/**
	 * Finds the path to a hook in a .forgeosrules hooks directory.
	 *
	 * @param hookName the name of the hook to search for, for example 'PreToolUse'
	 * @param hooksDir the .forgeosrules directory path to search
