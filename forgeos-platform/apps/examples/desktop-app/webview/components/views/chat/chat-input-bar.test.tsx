...
vi.mock("@/lib/provider-model-catalog", () => ({
	...
	VOICE_INPUT_SETTINGS_CHANGED_EVENT: "forgeos:test-voice-input-settings-changed",
}));
...
beforeEach(() => {
	...
	loadProviderModelCatalogMock.mockReset().mockResolvedValue({
		providers: [],
		enabledProviderIds: ["forgeos"],
		providerModels: { forgeos: ["test-model"] },
		providerReasoningModels: { forgeos: [] },
		voiceInput: null,
	});
...
const workspaceValue = {
	workspaceRoot: "/workspace/forgeos",
	workspaces: ["/workspace/forgeos"],
	listWorkspaces: vi.fn(async () => ["/workspace/forgeos"]),
...
describe("ChatInputBar", () => {
	...
	it("discards a pending batch transcript when the voice target changes", async () => {
		...
		await act(async () => {
			window.dispatchEvent(
				new Event("forgeos:test-voice-input-settings-changed"),
			);
		});
		await vi.waitFor(() => {
			expect(speechInputMockState.current?.recordingMode).toBe("streaming");
	...
	it("preserves an explicit High selection across capability and status updates", async () => {
		...
		await render("idle");
		await vi.waitFor(() => {
			expect(loadProviderModelsMock).toHaveBeenCalledWith("forgeos");
		});
		const providerModelsListener =
			subscribeToProviderModelsMock.mock.calls[0]?.[0];
		await act(async () => {
			providerModelsListener?.("forgeos", [
		...
		expect(workspaceTrigger?.textContent).toContain("forgeos");
	...
	it("selects High from the supported model thinking menu", async () => {
		loadProviderModelCatalogMock.mockResolvedValue({
			...
			enabledProviderIds: ["forgeos"],
			providerModels: { forgeos: ["test-model"] },
			providerReasoningModels: { forgeos: ["test-model"] },
		});
		const onReasoningChange = vi.fn();
		await act(async () => {
			root.render(
	...
	it("does not overwrite the remembered model when rendering a session's provider/model", async () => {
		...
		window.localStorage.setItem(
			MODEL_SELECTION_STORAGE_KEY,
			JSON.stringify({
				lastProvider: "forgeos",
				lastModelByProvider: { forgeos: "test-model" },
			}),
		);
		loadProviderModelCatalogMock.mockResolvedValue({
			providers: [],
