...
describe("@forgeos/ui theme contract", () => {
	...
	it("maps readable roles onto the palettes and preserves shadcn names", () => {
		...
		expect(tokens).toContain('@import "@forgeos/ui/theme/palette.css";');
		expect(tokens).not.toContain("--forgeos-");
		for (const token of [
			...visualRoleTokens,
			...statusRoleTokens,
			...semanticTokens,
	...
	it("generates a fully scoped palette and semantic theme", () => {
		...
		expect(scoped).toContain(".forgeos-ui-theme");
		expect(scoped).toContain(".dark .forgeos-ui-theme");
		expect(scoped).toContain(".forgeos-ui-theme.dark");
		for (const paletteName of paletteNames) {
	...
	it("provides Tailwind mappings without registering raw palette steps", () => {
		...
		for (const token of mappedColorTokens) {
			...
			expect(componentTheme).toMatch(
				new RegExp(
					`--color-forgeos-ui-${token}:\\s*var\\(\\s*--${token}\\s*\\);`,
				),
			);
		}
		...
		expect(componentTheme).toContain("@custom-variant forgeos-ui-dark");
		expect(componentTheme).not.toContain("@custom-variant dark ");
		expect(componentTheme).toContain(
			"--font-weight-forgeos-ui-medium: var(--font-weight-medium);",
		);
		expect(componentTheme).toContain("--text-forgeos-ui-xs: var(--text-xs);");
		expect(componentTheme).toContain("--radius-forgeos-ui-lg: var(--radius-lg);");
		expect(componentTheme).not.toMatch(
			/--color-forgeos-ui-(?:neutral|accent|error|success|warning|info)-(?:a)?\d+:/,
		);
		expect(components).toContain('@import "./theme/component-theme.css";');
		expect(base).toContain(
			'@import "@forgeos/ui/components/markdown.css" layer(components);',
		);
		expect(markdown).toContain(":is(.markdown, .forgeos-markdown)");
	...
	it("keeps embedded component utility names host-safe", () => {
		...
		expect(componentSources).not.toMatch(/(?<!forgeos-ui-)dark:/);
		expect(componentSources).toContain("forgeos-ui-dark:");
	});
