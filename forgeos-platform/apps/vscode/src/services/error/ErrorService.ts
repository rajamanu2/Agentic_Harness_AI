...
import { ForgeOSError } from "./ForgeOSError"
import { ErrorProviderFactory } from "./ErrorProviderFactory"
import { IErrorProvider } from "./providers/IErrorProvider"

/**
 * ErrorService handles error logging and tracking for the ForgeOS extension
...
export class ErrorService {
	...
	captureException(error: Error | ForgeOSError, properties?: Record<string, unknown>) {
		return this.provider.captureException(error, properties)
	}

	public logException(error: Error | ForgeOSError, properties?: Record<string, unknown>): void {
	...
	public toForgeOSError(rawError: unknown, modelId?: string, providerId?: string): ForgeOSError {
		const transformed = ForgeOSError.transform(rawError, modelId, providerId)
