...
description: "Complete reference of OpenTelemetry log events emitted by ForgeOS"
---

This page documents all OpenTelemetry log events currently instrumented in ForgeOS. These events are emitted when OpenTelemetry integration is enabled and provide detailed insights into user behavior, task execution, and system operations.
...
ForgeOS emits events across several categories, each prefixed with a namespace:
...
| `task.rule_toggled` | ForgeOS rule toggled on/off | rule_name, enabled, is_global |
| `task.yolo_mode_toggled` | YOLO mode toggled | enabled |
| `task.forgeos_web_tools_toggled` | ForgeOS web tools setting toggled | enabled |
...
| Event | Description | Key Attributes |
|-------|-------------|----------------|
| `forgeos.test.connection` | OTEL connection test event from "Test OTEL Connection" flow | success, exporter_type, endpoint |

### Example: forgeos.test.connection

```json
{
  "event": "forgeos.test.connection",
...
| `extension_version` | string | ForgeOS extension version |
...
sequenceDiagram
    ...
    User->>ForgeOS: Start Task
    ForgeOS->>OTel: task.created
    
    User->>ForgeOS: Submit Message
    ForgeOS->>OTel: task.conversation_turn (user)
    
    ForgeOS->>ForgeOS: Process with AI
    ForgeOS->>OTel: task.tokens
    ForgeOS->>OTel: task.conversation_turn (assistant)
    
    ForgeOS->>ForgeOS: Use Tool
    ForgeOS->>OTel: task.tool_used
    
    User->>ForgeOS: Provide Feedback
    ForgeOS->>OTel: task.option_selected
    
    User->>ForgeOS: Complete Task
    ForgeOS->>OTel: task.completed
```
  ...
  "widgets": [
    {
      "definition": {
        ...
        "requests": [
          {
            "q": "sum:forgeos.task.completed{*}.as_count()",
            "display_type": "bars"
          }
        ],
