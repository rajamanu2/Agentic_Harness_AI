import type { SessionHistoryRecord } from "@forgeos/core"
...
import { sdkMessagesToForgeOSMessages } from "./message-translator"
...
vi.mock("@/hosts/host-provider", () => ({
	HostProvider: {
		get: vi.fn(() => ({ globalStorageFsPath: "/tmp/forgeos" })),
	},
}))
...
describe("SdkTaskHistory", () => {
	...
	it("converts SDK persisted conversation messages to ForgeOS messages", () => {
		const result = sdkMessagesToForgeOSMessages([
	...
	it("retags only the final turn's text, styled by the mode recovered from user_input wrappers", async () => {
		...
		const result = await history.getForgeOSMessages("task-1")
	...
	it("does not retag the terminal text when no session record exists (unknown outcome)", async () => {
		...
		const result = await history.getForgeOSMessages("task-without-record")
	...
	it("backfills SDK task size from the session artifact directory", async () => {
		...
		const { history, updateSession } = makeHistory([
			makeSessionRecord("task-1", {
				...
				messagesPath: "/tmp/forgeos/sessions/task-1/task-1.messages.json",
			}),
		])
		...
		expect(getFolderSize.loose).toHaveBeenCalledWith("/tmp/forgeos/sessions/task-1", { bigint: false })
	...
	it("reads legacy task UI messages without migrating", async () => {
		...
		const messages = await history.getForgeOSMessages("legacy-task")
	...
	it("uses pretty legacy UI messages plus resumed SDK messages when both stores exist", async () => {
		...
		const forgeosMessages = await history.getForgeOSMessages("legacy-task")
		...
		expect(forgeosMessages).toEqual([
	...
	it("includes legacy tasks from VS Code extension storage", async () => {
		legacyStateReaderMock.taskHistory = [makeHistoryItem("forgeos-dir-task", { task: "~/.forgeos task" })]
		...
		expect(result.map((item) => item.sessionId)).toEqual(["forgeos-dir-task", "extension-storage-task"])
	})
