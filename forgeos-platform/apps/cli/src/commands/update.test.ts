...
const originalBuildEnv = process.env.FORGEOS_BUILD_ENV;
const originalDataDir = process.env.FORGEOS_DATA_DIR;
const originalHubDiscoveryPath = process.env.FORGEOS_HUB_DISCOVERY_PATH;
...
const originalNoAutoUpdate = process.env.FORGEOS_NO_AUTO_UPDATE;
...
function createTempFile(pathSuffix: string): string {
	const root = mkdtempSync(join(tmpdir(), "forgeos-update-test-"));
	tempDirs.push(root);
...
describe("getInstallationInfo", () => {
	afterEach(() => {
		...
		if (originalBuildEnv === undefined) {
			...
			process.env.FORGEOS_BUILD_ENV = originalBuildEnv;
		}
		if (originalDataDir === undefined) {
			delete process.env.FORGEOS_DATA_DIR;
		} else {
			process.env.FORGEOS_DATA_DIR = originalDataDir;
		}
		if (originalHubDiscoveryPath === undefined) {
			delete process.env.FORGEOS_HUB_DISCOVERY_PATH;
		} else {
			process.env.FORGEOS_HUB_DISCOVERY_PATH = originalHubDiscoveryPath;
		}
		...
		if (originalNoAutoUpdate === undefined) {
			delete process.env.FORGEOS_NO_AUTO_UPDATE;
		} else {
			process.env.FORGEOS_NO_AUTO_UPDATE = originalNoAutoUpdate;
		}
		vi.restoreAllMocks();
	...
	it("detects npm installs from the wrapper path passed to the compiled binary", () => {
		const wrapperPath = createTempFile("lib/node_modules/forgeos/bin/forgeos");
		process.env.FORGEOS_WRAPPER_PATH = wrapperPath;
		process.argv = ["bun", "/$bunfs/root/forgeos", "update", "--verbose"];

		expect(getInstallationInfo("1.2.3")).toEqual({
			packageManager: PackageManager.NPM,
			packageName: "forgeos",
			updateCommand: "npm update -g forgeos --tag latest",
		});
	});
	...
	it("detects bun global installs from the resolved install path", () => {
		...
		expect(getInstallationInfo("1.2.3")).toEqual({
			...
			packageName: "forgeos",
			updateCommand: "bun add -g forgeos@latest",
		});
	});
...
describe("auto update settings", () => {
	...
	it("skips startup auto update when disabled globally", () => {
		...
		process.env.FORGEOS_GLOBAL_SETTINGS_PATH = settingsPath;
		delete process.env.IS_DEV;
		delete process.env.FORGEOS_NO_AUTO_UPDATE;
		const fetchSpy = vi
...
describe("hub restart owner selection", () => {
	...
	it("uses the shared hub owner outside production builds", () => {
		...
		process.env.FORGEOS_DATA_DIR = "/tmp/forgeos-update-test-data";
		delete process.env.FORGEOS_HUB_DISCOVERY_PATH;
		...
		expect(owner.discoveryPath).not.toBe(
			"/tmp/forgeos-update-test-data/locks/hub/production.json",
		);
	});
});
...
describe("deferred auto update", () => {
	...
	it("starts the detached install when no hub is discoverable", async () => {
		...
		process.env.FORGEOS_HUB_DISCOVERY_PATH = join(root, "production.json");
		const unref = vi.fn();
		mockSpawn.mockReturnValue({ unref } as unknown as ChildProcess);

		const outcome = await applyDeferredUpdate({
			command: "npm update -g forgeos --tag latest --min-release-age=0",
		});

		expect(outcome).toBe("started");
		expect(mockSpawn).toHaveBeenCalledWith(
			"npm update -g forgeos --tag latest --min-release-age=0",
...
describe("withMinimumReleaseAgeBypass", () => {
	it("adds the package-manager-specific cooldown bypass", () => {
		expect(
			withMinimumReleaseAgeBypass(
				"npm update -g forgeos --tag latest",
				PackageManager.NPM,
			).command,
		).toBe("npm update -g forgeos --tag latest --min-release-age=0");
		expect(
			withMinimumReleaseAgeBypass("bun add -g forgeos@latest", PackageManager.BUN)
				.command,
		).toBe("bun add -g forgeos@latest --minimum-release-age=0");
		expect(
			withMinimumReleaseAgeBypass(
				"yarn global add forgeos@latest",
				PackageManager.YARN,
			).command,
		).toBe("yarn global add forgeos@latest");
		expect(
			withMinimumReleaseAgeBypass(
				"yarn global add forgeos@latest",
				PackageManager.YARN,
