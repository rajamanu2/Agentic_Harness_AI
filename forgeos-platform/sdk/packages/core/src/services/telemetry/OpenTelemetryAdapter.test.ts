...
describe("OpenTelemetryAdapter", () => {
	...
	it("records metrics with merged telemetry attributes and retires gauge series", () => {
		...
		adapter.recordCounter("forgeos.turns.total", 2, { ulid: "01HXYZ" });
		adapter.recordHistogram("forgeos.api.duration.seconds", 1.5, {
			ulid: "01HXYZ",
		});
		adapter.recordGauge("forgeos.workspace.active_roots", 3, { workspace: "a" });
		...
		adapter.recordGauge("forgeos.workspace.active_roots", null, {
			workspace: "a",
		});
