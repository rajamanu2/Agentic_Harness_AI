...
export { buildMcpInstallTransport, uninstallMcpServer } from "@forgeos/core";
...
export async function runMcpInstallCommand(
	...
	try {
		...
		if (!isTty) {
			throw new Error(
				"forgeos mcp install opens the MCP wizard and requires a TTY. Pass --yes to install noninteractively.",
			);
		}
