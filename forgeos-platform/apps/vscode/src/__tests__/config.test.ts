...
import os from "os"
import { ForgeOSConfigurationError, ForgeOSEndpoint, ForgeOSEnv, Environment } from "../config"

describe("ForgeOSEndpoint configuration", () => {
	...
	beforeEach(async () => {
		...
		// Create .forgeos directory
		await fs.mkdir(path.join(tempDir, ".forgeos"), { recursive: true })
		...
		;(ForgeOSEndpoint as any)._instance = null
		;(ForgeOSEndpoint as any)._initialized = false
		;(ForgeOSEndpoint as any)._extensionFsPath = undefined
	})
	...
	describe("valid config parsing", () => {
		it("should parse valid endpoints.json with all required fields", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".forgeos", "endpoints.json"), JSON.stringify(validConfig), "utf8")

			await ForgeOSEndpoint.initialize(tempDir)

			const config = ForgeOSEndpoint.config
		...
		it("should work without endpoints.json (standard mode)", async () => {
			...
			config.appBaseUrl.should.equal("https://app.forgeos.bot")
			config.apiBaseUrl.should.equal("https://api.forgeos.bot")
		})
		...
		it("should accept URLs with paths", async () => {
			const validConfig = {
				...
				apiBaseUrl: "https://proxy.enterprise.com/forgeos/api",
				mcpBaseUrl: "https://proxy.enterprise.com/forgeos/mcp",
			}
			...
			const config = ForgeOSEndpoint.config
			config.appBaseUrl.should.equal("https://proxy.enterprise.com/forgeos/app")
		})
	})

	describe("invalid JSON handling", () => {
		it("should throw ForgeOSConfigurationError for invalid JSON syntax", async () => {
			await fs.writeFile(path.join(tempDir, ".forgeos", "endpoints.json"), "{ invalid json }", "utf8")

			try {
				await ForgeOSEndpoint.initialize(tempDir)
				throw new Error("Should have thrown")
			} catch (error: any) {
				error.should.be.instanceof(ForgeOSConfigurationError)
				error.message.should.containEql("Invalid JSON")
			}
		})

		it("should throw ForgeOSConfigurationError for truncated JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".forgeos", "endpoints.json"), '{"appBaseUrl": "https://test.com"', "utf8")
		...
		it("should throw ForgeOSConfigurationError for empty file", async () => {
			await fs.writeFile(path.join(tempDir, ".forgeos", "endpoints.json"), "", "utf8")
		...
		it("should throw ForgeOSConfigurationError for non-object JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".forgeos", "endpoints.json"), '"just a string"', "utf8")
		...
		it("should throw ForgeOSConfigurationError for array JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".forgeos", "endpoints.json"), "[]", "utf8")
		...
		it("should throw ForgeOSConfigurationError for null JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".forgeos", "endpoints.json"), "null", "utf8")
	...
	describe("missing required fields", () => {
		it("should throw ForgeOSConfigurationError when appBaseUrl is missing", async () => {
			const config = {
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}

			await fs.writeFile(path.join(tempDir, ".forgeos", "endpoints.json"), JSON.stringify(config), "utf8")
		...
		it("should throw ForgeOSConfigurationError when apiBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}
		...
		it("should throw ForgeOSConfigurationError when mcpBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				apiBaseUrl: "https://api.enterprise.com",
			}
		...
		it("should throw ForgeOSConfigurationError when all fields are missing", async () => {
			await fs.writeFile(path.join(tempDir, ".forgeos", "endpoints.json"), "{}", "utf8")
		...
		it("should throw ForgeOSConfigurationError when field is null", async () => {
			const config = {
				appBaseUrl: null,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw ForgeOSConfigurationError when field is empty string", async () => {
			const config = {
				appBaseUrl: "",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw ForgeOSConfigurationError when field is whitespace only", async () => {
			const config = {
				appBaseUrl: "   ",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw ForgeOSConfigurationError when field is non-string", async () => {
			const config = {
				appBaseUrl: 12345,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("invalid URL detection", () => {
		it("should throw ForgeOSConfigurationError for invalid URL format", async () => {
			const config = {
		...
		it("should throw ForgeOSConfigurationError for URL without protocol", async () => {
			const config = {
		...
		it("should throw ForgeOSConfigurationError for malformed URL", async () => {
			const config = {
				appBaseUrl: "https://",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("environment switching blocked in self-hosted mode", () => {
		it("should throw error when trying to change environment in self-hosted mode", async () => {
			...
			ForgeOSEndpoint.config.environment.should.equal(Environment.selfHosted)

			// Try to change environment - should throw
			try {
				ForgeOSEnv.setEnvironment("staging")
				throw new Error("Should have thrown")
			} catch (error: any) {
		...
		it("should allow environment switching in standard mode", async () => {
			...
			ForgeOSEndpoint.config.environment.should.not.equal(Environment.selfHosted)

			// Should be able to change environment
			ForgeOSEnv.setEnvironment("staging")
			ForgeOSEnv.getEnvironment().environment.should.equal("staging")

			ForgeOSEnv.setEnvironment("local")
			ForgeOSEnv.getEnvironment().environment.should.equal("local")

			ForgeOSEnv.setEnvironment("production")
			ForgeOSEnv.getEnvironment().environment.should.equal("production")
		})
	})
	...
	describe("self-hosted mode behavior", () => {
		it("should report selfHosted environment in self-hosted mode", async () => {
			...
			const envConfig = ForgeOSEndpoint.config
		...
		it("should use custom endpoints from file", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".forgeos", "endpoints.json"), JSON.stringify(customConfig), "utf8")
	...
	describe("initialization behavior", () => {
		it("should only initialize once", async () => {
			await ForgeOSEndpoint.initialize(tempDir)
			ForgeOSEndpoint.isInitialized().should.be.true()
		...
		it("should throw error when accessing config before initialization", async () => {
			...
			try {
				const _ = ForgeOSEndpoint.config
				throw new Error("Should have thrown")
			} catch (error: any) {
	...
	describe("isSelfHosted() method", () => {
		it("should return true when not initialized (safety fallback)", async () => {
			...
			ForgeOSEndpoint.isInitialized().should.be.false()
			ForgeOSEndpoint.isSelfHosted().should.be.true()
		})
		...
		it("should return false when in normal mode (no endpoints.json)", async () => {
			...
			ForgeOSEndpoint.isSelfHosted().should.be.false()
		})
	})
	...
	describe("bundled endpoints.json behavior", () => {
		...
		it("should use bundled endpoints.json when available", async () => {
			...
			await ForgeOSEndpoint.initialize(bundledDir)
		...
		it("should prefer bundled endpoints.json over user file", async () => {
			...
			// Set up both configs
			await fs.writeFile(path.join(bundledDir, "endpoints.json"), JSON.stringify(bundledConfig), "utf8")
			await fs.writeFile(path.join(tempDir, ".forgeos", "endpoints.json"), JSON.stringify(userConfig), "utf8")
		...
		it("should throw ForgeOSConfigurationError for invalid bundled file", async () => {
			const invalidConfig = {
		...
		it("should throw ForgeOSConfigurationError for invalid JSON in bundled file", async () => {
