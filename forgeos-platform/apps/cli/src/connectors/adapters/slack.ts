...
import type { ChatStartSessionRequest } from "@forgeos/core";
import {
	createUserInstructionConfigService,
	HubSessionClient,
} from "@forgeos/core";
import type { ConnectSlackOptions, SlackConnectorState } from "@forgeos/shared";
import {
	type Adapter,
	Chat,
	ConsoleLogger,
...
/**
 * Slack delivers `@forgeos hi` as `<@U0B8E8H3U1F> hi`, and the chat SDK
...
> {
		...
		return (
			super
				...
				.option(
					...
					"RPC address",
					process.env.FORGEOS_RPC_ADDRESS?.trim() ||
						resolveDefaultCliRpcAddress(),
				)
	...
	protected override readOptions(command: Command): ConnectSlackOptions {
		...
		return {
			...
			hookCommand:
				...
				process.env.FORGEOS_CONNECT_HOOK_COMMAND?.trim(),
			port,
	...
	protected override async runWithOptions(
		...
		const backgroundExitCode = await this.maybeRunInBackground({
			...
			childEnvVar: "FORGEOS_SLACK_CONNECT_CHILD",
			statePath,
			readState: (path) => this.readConnectorState(path),
			...
			foregroundHint:
				"[slack] use `forgeos connect slack -i ...` to run in the foreground",
