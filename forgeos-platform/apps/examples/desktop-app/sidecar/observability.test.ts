...
const mocks = vi.hoisted(() => ({
	...
	createForgeOSTelemetryServiceConfig: vi.fn((config: unknown) => config),
...
vi.mock("@forgeos/core", async () => {
	...
	return {
		...
		createForgeOSTelemetryServiceConfig: mocks.createForgeOSTelemetryServiceConfig,
...
describe("desktop observability", () => {
	...
	it("configures desktop telemetry, identity, activation, and lifecycle", async () => {
		...
		expect(mocks.createForgeOSTelemetryServiceConfig).toHaveBeenCalledWith({
			metadata: expect.objectContaining({
				forgeos_type: "desktop",
				platform: "ForgeOS Code",
			}),
		});
