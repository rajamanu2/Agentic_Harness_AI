...
import { setForgeOSDir } from "@forgeos/shared/storage";
...
describe("createTaskWorktree", () => {
	...
	beforeEach(async () => {
		sandboxRoot = await mkdtemp(path.join(tmpdir(), "forgeos-sdk-worktree-"));
		forgeosDir = path.join(sandboxRoot, ".forgeos");
	...
	afterEach(async () => {
		...
		setForgeOSDir(originalForgeOSDir ?? path.join("~", ".forgeos"));
		await rm(sandboxRoot, { recursive: true, force: true });
	});

	it("places worktrees under ~/.forgeos/worktrees", () => {
		expect(getTaskWorktreesHomePath()).toBe(path.join(forgeosDir, "worktrees"));
	});

	it("creates a detached worktree at ~/.forgeos/worktrees/<taskId>/<repoName>", async () => {
		...
		expect(result.path).toBe(
			path.join(forgeosDir, "worktrees", "my-task", "myrepo"),
		);
	...
	it("generates a Kanban-style short taskId when none is provided", async () => {
		...
		expect(result.path).toBe(
			path.join(forgeosDir, "worktrees", result.taskId, "myrepo"),
		);
	});
	...
	it("cleans up the task directory when git worktree add fails", async () => {
		...
		expect(await pathExists(path.join(forgeosDir, "worktrees", "empty"))).toBe(
			false,
		);
	});
