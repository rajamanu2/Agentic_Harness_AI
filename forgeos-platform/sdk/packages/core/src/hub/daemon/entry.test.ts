...
vi.mock("@forgeos/shared", async (importOriginal) => {
	const actual = await importOriginal<typeof import("@forgeos/shared")>();
	return {
		...actual,
		initVcr: mockInitVcr,
...
describe("hub daemon entry", () => {
	...
	it("starts the daemon with cron options for the daemon workspace root", async () => {
		const cwd = mkdtempSync(join(tmpdir(), "forgeos-hub-entry-test-"));
		tempDirs.push(cwd);
		process.argv = [
			"node",
			"entry.js",
	...
	it("ignores abort-family unhandled rejections instead of shutting down", async () => {
		...
		const { AgentRuntimeAbortError } = await import("@forgeos/agents");
