...
When your conversation approaches the model's context window limit, ForgeOS automatically summarizes it to free up space and keep working.

<Frame>
	<img
		src="https://storage.googleapis.com/forgeos_public_images/docs/assets/condensing.png"
...
ForgeOS monitors token usage during your conversation. When you're getting close to the limit, it:
...
Previously, ForgeOS would truncate older messages when hitting context limits, losing important context.
...
- ForgeOS remembers everything it's done
- You can work on much larger projects without interruption

<Tip>
Auto Compact works especially well for long-running tasks. Structured task lists can help maintain progress across summarizations so ForgeOS can stay on track across multiple context windows.
</Tip>
...
<Note>
With other models, ForgeOS falls back to standard rule-based context truncation, even if Auto Compact is enabled.
</Note>
