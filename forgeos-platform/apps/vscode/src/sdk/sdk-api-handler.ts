...
//
// Builds an SDK ApiHandler (from `@forgeos/llms`) directly from the extension's
// legacy ApiConfiguration. This is the single inference path: the main task
// loop runs through ForgeOSCore (see forgeos-session-factory.ts), and standalone
...
import { type ApiHandler, createHandler, type ProviderConfig } from "@forgeos/llms"
...
export function buildSdkProviderConfig(
	...
	const providerId = (mode === "plan" ? configuration.planModeApiProvider : configuration.actModeApiProvider) ?? "forgeos"
	...
	const base: ProviderConfig = {
		...
		// configuration (see .forgeosrules/network.md).
		fetch,
