...
//
// SDK-backed authentication service. Uses @forgeos/core OAuth functions
...
// disk — it's fetched from the ForgeOS API on startup and cached in memory.
// This matches the CLI's pattern (see apps/cli/src/runtime/interactive-welcome.ts).

import type { ITelemetryService, OAuthCredentials, ProviderSettings } from "@forgeos/core"
import {
...
import { AuthState, UserInfo } from "@shared/proto/forgeos/account"
import type { EmptyRequest, String } from "@shared/proto/forgeos/common"
...
import { buildBasicForgeOSHeaders } from "@/services/EnvUtils"
import { featureFlagsService } from "@/services/feature-flags"
import { telemetryService } from "@/services/telemetry"
import { FORGEOS_API_ENDPOINT } from "@/shared/forgeos/api"
...
export interface ForgeOSAuthInfo {
	idToken: string
	refreshToken?: string
	expiresAt?: number // seconds since epoch
	userInfo: ForgeOSAccountUserInfo
	provider: string
	startedAt?: number
}

export interface ForgeOSAccountUserInfo {
	createdAt?: string
	displayName: string
	email: string
	id: string
...
export interface ForgeOSAccountOrganization {
	active: boolean
	memberId: string
	name: string
...
/**
 * Read ForgeOS OAuth credentials from providers.json.
 * Returns { accessToken, refreshToken, expiresAt, accountId } or null.
 */
function readForgeOSCredentials(): {
	accessToken: string
	refreshToken?: string
	...
	try {
		...
		if (!settings?.auth?.accessToken) {
			sdkDebug("[SdkAuthService] readForgeOSCredentials: no auth.accessToken found")
			return null
		}
		...
		sdkDebug(
			`[SdkAuthService] readForgeOSCredentials: found credentials (accessHash=${hashSecret(result.accessToken)}, refreshHash=${hashSecret(result.refreshToken)}, expiresAt=${result.expiresAt}, sessionStartedAtMs=${result.sessionStartedAtMs})`,
		)
		return result
	} catch (error) {
...
/**
 * Write ForgeOS OAuth credentials to providers.json.
 */
function writeForgeOSCredentials(credentials: {
	accessToken: string
	refreshToken?: string
	...
	try {
		const manager = getProviderSettingsManager()
		const existing = manager.getProviderSettings("forgeos")
		...
		sdkDebug(
			`[SdkAuthService] writeForgeOSCredentials: wrote (accessHash=${hashSecret(credentials.accessToken)}, refreshHash=${hashSecret(credentials.refreshToken)}, expiresAt=${credentials.expiresAt}, sessionStartedAtMs=${sessionStartedAtMs})`,
		)
	} catch (error) {
...
/**
 * Clear ForgeOS OAuth credentials from providers.json.
 */
function clearForgeOSCredentials(): void {
	try {
		const manager = getProviderSettingsManager()
		const existing = manager.getProviderSettings("forgeos")
		if (existing) {
			sdkDebug("[SdkAuthService] clearForgeOSCredentials: clearing auth from providers.json")
			manager.saveProviderSettings(
				{
					...existing,
					provider: "forgeos",
...
export class AuthService {
	...
	private _forgeosAuthInfo: ForgeOSAuthInfo | null = null
	...
	// ---- SDK OAuth → ForgeOSAuthInfo conversion ----

	/**
	 * Convert SDK OAuthCredentials to our ForgeOSAuthInfo format.
	 * Also fetches full user info from the ForgeOS API.
	 */
	private async credentialsToAuthInfo(credentials: OAuthCredentials, provider: string): Promise<ForgeOSAuthInfo> {
	...
	/**
	 * Fetch user info from the ForgeOS API using an access token.
	 */
	private async fetchUserInfoFromApi(accessToken: string): Promise<ForgeOSAccountUserInfo | null> {
		try {
			const apiBaseUrl = ForgeOSEnv.config().apiBaseUrl
			...
			const response = await axios.get(`${apiBaseUrl}/api/v1/users/me`, {
				headers: {
					...
					...(await buildBasicForgeOSHeaders()),
				},
				...getAxiosSettings(),
			})
	...
	private toOAuthCredentials(authInfo: ForgeOSAuthInfo): OAuthCredentials {
		return {
	...
	private async resolveValidForgeOSCredentials(
		authInfo: ForgeOSAuthInfo,
		...
		return getValidForgeOSCredentials(
			this.toOAuthCredentials(authInfo),
			{ apiBaseUrl: ForgeOSEnv.config().apiBaseUrl, telemetry: this._telemetry },
	...
	async getAuthToken(): Promise<string | null> {
		if (!this._forgeosAuthInfo?.idToken) {
			return null
		}

		// Check if we need to refresh
		const expiresAt = this._forgeosAuthInfo.expiresAt
		if (expiresAt) {
		...
		// a successful refresh above replaces _forgeosAuthInfo, and the expiry
		// captured before it belongs to the old token — which has already passed
		// whenever the process sat idle beyond token lifetime.
		const currentExpiresAt = this._forgeosAuthInfo.expiresAt
		if (currentExpiresAt && Date.now() / 1000 >= currentExpiresAt) {
			return null
		}

		return `${WORKOS_TOKEN_PREFIX}${this._forgeosAuthInfo.idToken}`
	}

	/**
	 * Refresh the access token using the SDK's shared ForgeOS credential validator.
	 * Persists refreshed credentials to providers.json when credentials change.
	 */
	private async refreshAccessToken(): Promise<boolean> {
		if (this._refreshPromise) {
			await this._refreshPromise
			return this._forgeosAuthInfo?.idToken !== undefined
		}

		if (!this._forgeosAuthInfo?.refreshToken) {
			sdkDebug("[SdkAuthService] refreshAccessToken: no refresh token available")
			return false
		}

		sdkDebug(`[SdkAuthService] refreshAccessToken: starting (currentTokenHash=${hashSecret(this._forgeosAuthInfo.idToken)})`)
		this._refreshPromise = (async () => {
			try {
				const currentInfo = this._forgeosAuthInfo
				if (!currentInfo) {
					return undefined
				}
				const newCredentials = await this.resolveValidForgeOSCredentials(currentInfo, { forceRefresh: true })
				if (!newCredentials) {
					...
					this._forgeosAuthInfo = null
					this._authenticated = false
					clearForgeOSCredentials()
					setImmediate(() => {
				...
				if (credentialsChanged) {
					...
					writeForgeOSCredentials({
						accessToken: newCredentials.access,
						refreshToken: newCredentials.refresh,
						expiresAt: newCredentials.expires,
						accountId: this._forgeosAuthInfo.userInfo.id,
				...
				return this._forgeosAuthInfo.idToken
			} catch (error) {
	...
	getActiveOrganizationId(): string | null {
		if (!this._forgeosAuthInfo?.userInfo?.organizations) return null
		const activeOrg = this._forgeosAuthInfo.userInfo.organizations.find((org) => org.active)
	 ...
	 */
	getUserOrganizations(): ForgeOSAccountOrganization[] | undefined {
		return this._forgeosAuthInfo?.userInfo?.organizations
	}
	...
	getProviderName(): string | null {
		return this._forgeosAuthInfo?.provider ?? null
	}
	 ...
	 */
	getInfo(): AuthState {
		if (this._forgeosAuthInfo && this._authenticated) {
			const userInfo = this._forgeosAuthInfo.userInfo
			userInfo.appBaseUrl = ForgeOSEnv.config().appBaseUrl
	...
	/**
	 * Initiate ForgeOS OAuth login.
	 * Uses SDK's loginForgeOSOAuth() which spawns a local callback server.
	...
	async createAuthRequest(strict = false): Promise<String> {
		...
		if (strict && this._authenticated) {
			...
			const { String: ProtoString } = await import("@shared/proto/forgeos/common")
		...
		// of loginForgeOSOAuth(), which opens a real browser window the tests can't
		...
		void (async () => {
			try {
				...
				const authInfo = await this.credentialsToAuthInfo(credentials, "forgeos")
				this._forgeosAuthInfo = authInfo
				this._authenticated = true
				...
				Logger.error("[SdkAuthService] ForgeOS OAuth login failed:", error)
			}
		})()
	...
	private async createMockAuthRequest(): Promise<String> {
		if (ForgeOSEnv.config().environment !== "local") {
			throw new Error("E2E mock auth is only available when FORGEOS_ENVIRONMENT=local")
		}

		const apiBaseUrl = ForgeOSEnv.config().apiBaseUrl
		const tokenUrl = new URL(FORGEOS_API_ENDPOINT.TOKEN_EXCHANGE, apiBaseUrl)
		...
		await this.sendAuthStatusUpdate()
		Logger.log(`[SdkAuthService] E2E mock login completed as ${this._forgeosAuthInfo.userInfo.email}`)
	...
	async handleDeauth(reason: LogoutReason = LogoutReason.UNKNOWN): Promise<void> {
		try {
			telemetryService.captureAuthLoggedOut("forgeos", reason)
			this._forgeosAuthInfo = null
			this._authenticated = false
			clearForgeOSCredentials()
			await this.sendAuthStatusUpdate()
	...
	async handleAuthCallback(authorizationCode: string, provider: string): Promise<void> {
		try {
			// Exchange the authorization code for tokens using the ForgeOS API
			const apiBaseUrl = ForgeOSEnv.config().apiBaseUrl
			...
			const sessionStartedAtMs = Date.now()
			const authInfo: ForgeOSAuthInfo = {
				idToken: tokenData.accessToken,
				refreshToken: tokenData.refreshToken,
				userInfo: userInfo ?? {
					id: tokenData.userInfo.forgeosUserId || "",
					email: tokenData.userInfo.email || "",
					displayName: tokenData.userInfo.name || "",
					createdAt: new Date().toISOString(),
					organizations: [],
	...
	async restoreRefreshTokenAndRetrieveAuthInfo(): Promise<void> {
		try {
			const creds = readForgeOSCredentials()
			if (!creds) {
				this._authenticated = false
				this._forgeosAuthInfo = null
				return
			...
			const restoredAuthInfo: ForgeOSAuthInfo = {
			...
			try {
				validCredentials = await this.resolveValidForgeOSCredentials(restoredAuthInfo)
			} catch (error) {
			...
			if (this._authenticated && this._forgeosAuthInfo) {
				const userInfo = await this.fetchUserInfoFromApi(this._forgeosAuthInfo.idToken)
				if (userInfo) {
					this._forgeosAuthInfo.userInfo = userInfo
				} else {
			...
			// Notify BannerService of auth change (mirrors classic AuthService)
			BannerService.onAuthUpdate(this._forgeosAuthInfo?.userInfo?.id || null).catch((error) => {
			...
			telemetryService.captureAuthLoggedOut("forgeos", LogoutReason.RESTORE_ERROR)
			this._authenticated = false
			this._forgeosAuthInfo = null
		}
	}
	...
	async sendAuthStatusUpdate(): Promise<void> {
		...
		const authInfo = this._forgeosAuthInfo
