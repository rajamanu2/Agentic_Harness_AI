...
import type { GatewayResolvedProviderConfig } from "@forgeos/shared";
...
export async function createClaudeCodeProviderModule(
	...
	try {
		...
		throw new Error(
			"The Claude Code provider requires the optional 'ai-sdk-provider-claude-code' package. " +
				"Install it alongside @forgeos/llms to use this provider.",
			{ cause: error },
		);
	}
	...
	// @forgeos/core's buildGatewayProviderOptions). Anchor the spawned agent
	...
	// ForgeOS has no interactive permission prompt wired into the CLI session
...
export async function createSapAiCoreProviderModule(
	...
	const provider = createSAPAIProvider({
		...
		!Array.isArray(options.defaultSettings)
			...
			: {}),
		requestConfig: {
			headers: { "ai-client-type": "ForgeOS" },
			// Standard forgeos axios settings mirroring `getAxiosSettings()`
			adapter: "fetch",
