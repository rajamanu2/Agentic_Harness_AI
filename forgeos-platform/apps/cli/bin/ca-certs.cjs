// Auto-discovery of OS trust anchors for the ForgeOS CLI.
//
// Bun does not read the OS trust store, so the 3.x CLI cannot see corporate
// MITM / self-signed CAs out of the box. This runs in the Node `bin/forgeos`
...
function resolveForgeOSDir(env, os, path) {
	return env.FORGEOS_DIR?.trim() || path.join(os.homedir(), ".forgeos");
}

/**
 * True when the api-unavailable warning should print. Stamped per Node version
 * in the forgeos dir so the nudge shows once rather than on every command; a
...
function shouldWarnApiUnavailable(env, deps = {}) {
	...
	const dir = resolveForgeOSDir(env, os, path);
...
function configureNodeExtraCaCerts(env, deps = {}) {
	...
	const managedDir = resolveForgeOSDir(env, os, path);
