...
import { resolveForgeOSDir, resolveMcpSettingsPath } from "@forgeos/shared/storage";
...
export async function uninstallMarketplaceSkill(
	...
	if (isMarketplaceSkillInstalled(entry)) {
		throw new Error(
			`Skill uninstall completed, but ${entry.name ?? entry.id} is still present in ForgeOS's global skills directories.`,
		);
	}
	return {
		id: entry.id,
