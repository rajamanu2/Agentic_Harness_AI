...
// key) is needed; every other `@forgeos/llms` export stays real because
// `@forgeos/core` re-exports them.
vi.mock("@forgeos/llms", async (importOriginal) => ({
	...(await importOriginal<typeof import("@forgeos/llms")>()),
...
function createProviderSettingsManager(): ProviderSettingsManager {
	const tempDir = mkdtempSync(join(tmpdir(), "forgeos-cli-compact-"));
